#!/usr/bin/env python3
"""Drive the OmniCraft CLI through a PTY in a throwaway sandbox and verify it.
...
1. Builds an **isolated config/data sandbox** so nothing the CLI writes ever
   lands in the real ``~/.omnicraft`` — it sets the purpose-built
   ``OMNICRAFT_CONFIG_HOME`` / ``OMNICRAFT_DATA_DIR`` knobs (``omnicraft/cli.py``
...
2. Drives the real ``omnicraft`` binary through ``pexpect`` (a real PTY with a
...
5. Proves it left the real ``~/.omnicraft`` byte-for-byte unchanged.
...
try:
    ...
    sys.stderr.write(
        "verify_cli.py needs `pexpect`. Run it with the omnicraft project's "
        "venv python (it bundles pexpect), e.g.\n"
        "  <repo>/.venv/bin/python verify_cli.py ...\n"
    )
    raise

# --- PTY constants (mirrors tests/e2e/omnicraft/_pexpect_harness.py) ---------
...
# Stable onboarding anchors (omnicraft/cli.py:520, :10064, :10300).
...
def build_sandbox(
    *,
    ...
    """Create an isolated sandbox env that cannot touch the real ``~/.omnicraft``.
    ...
    ".omnicraft"`` (``omnicraft_ui_sdk/terminal/_config.py``) and ignores
    ``OMNICRAFT_CONFIG_HOME`` / ``OMNICRAFT_DATA_DIR``. So redirecting ``HOME`` is
    ...
    :param inherit_home: Opt OUT of home isolation — use the real ``HOME`` (and
        ...
        ``~/.omnicraft/logs`` (the broadened fingerprint catches this).
    :param strip_path: Reduce ``PATH`` to just the omnicraft binary's dir + an
        empty dir, so node/npm/tmux/claude/codex read as "not installed" — i.e.
        a brand-new machine.
    :param omnicraft_bin: Path to the ``omnicraft`` console script being driven.
    :returns: A :class:`Sandbox`.
    """
    root = Path(mkdtemp(prefix="omnicraft-verify-"))
    (root / "config").mkdir()
    (root / "data").mkdir()
    ...
    env["OMNICRAFT_CONFIG_HOME"] = str(root / "config")
    env["OMNICRAFT_DATA_DIR"] = str(root / "data")
    env["OMNICRAFT_NO_UPDATE_CHECK"] = "1"  # keep the update nag out of frames
    env["TERM"] = TERM
    ...
    if strip_path:
        empty = root / "emptybin"
        empty.mkdir()
        env["PATH"] = f"{omnicraft_bin.parent}:{empty}"

    return Sandbox(root=root, env=env, home_isolated=not inherit_home)


def fingerprint_real_config() -> dict[str, str]:
    """Fingerprint the real ``~/.omnicraft`` so we can prove we never wrote to it.
    ...
    * the set of ``logs/cli-*.log`` diagnostic files — what *any* non-help CLI
      invocation writes via the hardcoded ``Path.home()/.omnicraft`` state dir.
      A new ``cli-*.log`` basename after the run means we wrote into the real
      home (the precise violation that slips through ``OMNICRAFT_CONFIG_HOME`` /
      ``OMNICRAFT_DATA_DIR``). With home isolation on (the default) none appear;
    ...
    running omnicraft daemons churn them → false alarms). The single ``logs/``
...
def scenario_check_isolation(args, sandbox: Sandbox, result: Result) -> None:
    ...
    Runs ``omnicraft config list`` inside the sandbox (no PTY needed) and
    asserts (a) it executed, (b) the sandbox config home is now used, (c) the
    real ``~/.omnicraft`` fingerprint is unchanged. This is the first thing to
    run to trust every other scenario.
    """
    proc = subprocess.run(
        [str(args.omnicraft), "config", "list"],
        env=sandbox.env,
        cwd=str(args.repo),
        capture_output=True,
        text=True,
    ...
    result.add(
        ...
        Path(sandbox.env["OMNICRAFT_CONFIG_HOME"]).exists(),
        sandbox.env["OMNICRAFT_CONFIG_HOME"],
    )
...
def scenario_help_snapshot(args, sandbox: Sandbox, result: Result) -> None:
    """Render ``omnicraft [SUBCOMMAND] --help`` and lint it for known UX issues.
...
def stop_sandbox_server(args, sandbox: Sandbox) -> None:
    ...
    with contextlib.suppress(Exception):
        subprocess.run(
            [str(args.omnicraft), "server", "stop"],
            env=sandbox.env,
            cwd=str(args.repo),
            capture_output=True,
            text=True,
...
def resolve_omnicraft(repo: Path, explicit: str | None) -> Path:
    """Find the ``omnicraft`` console script to drive."""
    if explicit:
        return Path(explicit).resolve()
    venv = repo / ".venv" / "bin" / "omnicraft"
    if venv.exists():
        return venv.resolve()
    found = shutil.which("omnicraft")
    if found:
        return Path(found).resolve()
    sys.exit("Could not find an `omnicraft` binary; pass --omnicraft <path>.")
...
def parse_args(argv: Sequence[str]) -> argparse.Namespace:
    ...
    p.add_argument(
        "--omnicraft",
        help="Path to the omnicraft binary (default: <repo>/.venv/bin/omnicraft or PATH).",
    )
    p.add_argument(
        "--label",
        default="run",
    ...
    p.add_argument(
        ...
        "~/.omnicraft/logs. Use only to reach a real credentialed REPL.",
    )
    p.add_argument(
        "--strip-path",
        action="store_true",
...
def main(argv: Sequence[str]) -> int:
    ...
    result.add(
        ...
        untouched,
        "~/.omnicraft unchanged" if untouched else "REAL CONFIG MUTATED — investigate",
    )
