---
name: fucho-e2e-dev
description: End-to-end test the fucho multi-agent coding orchestrator's critical user journeys (CUJs). Two halves — a deterministic mock-LLM driver (fucho_cuj.py) that boots a throwaway local server + mock LLM and asserts the substrate (boot, bridged sys_* tool dispatch, the blast_radius / spawn_bounds / headless_subagent_purpose_guard guardrails, fan-out delegation), and a live real-CLI recipe (real claude/codex/pi, real worktrees/PRs) for fucho's actual judgment. Load when developing, testing, or debugging examples/fucho — its config.yaml, the claude_code/codex/pi sub-agents, the investigate/fanout/cross-review skills, or the omnicraft.inner.nessie.policies guardrails — or reproducing a fucho orchestration bug.
---

# fucho orchestrator: end-to-end CUJ dev & testing

`fucho` (`examples/fucho/`) is a multi-agent **coding orchestrator**: a
...
|------|----------------|-------|
| **Mock loop** (`fucho_cuj.py`) | The **substrate/mechanics** — the brain is *scripted*, so this proves bundle load, server-side policy resolution, bridged `sys_*` tool dispatch, the guardrail DENYs, and fan-out — deterministically, with no creds | nothing (mock LLM) |
| **Live recipe** | fucho's **judgment** — does the real brain preflight, decompose, delegate, cross-review, and pull in the human correctly | real `claude`/`codex`/`pi` + model creds + network |
...
## Part A — the deterministic mock loop (`fucho_cuj.py`)

The driver boots a throwaway local OmniCraft server (which carries
`omnicraft.inner.nessie.policies` — the module fucho's guardrails resolve) plus
the repo's mock-LLM server, rewrites the fucho bundle to the `openai-agents`
...
```bash
.venv/bin/python .claude/skills/fucho-e2e-dev/fucho_cuj.py --list-scenarios
.venv/bin/python .claude/skills/fucho-e2e-dev/fucho_cuj.py --scenario all
.venv/bin/python .claude/skills/fucho-e2e-dev/fucho_cuj.py --scenario guardrail_purpose --keep
```
...
fucho's judgment (whether the *real* brain preflights, decomposes, picks the
...
timeout 280 .venv/bin/omni run examples/fucho \
...
cat .fucho/registry.json 2>/dev/null               # fucho's task list
...
|---|---|---|
| roster preflight | first live turn on a box missing a CLI | fucho tells you which worker is unavailable; routes around it |
| investigate | prompt a read-only question ("explain/audit/why does X…") | `child_sessions` with `purpose: explore/search`; answer cites their reports, not fucho's own deep reads |
...
| plan gate / inbox | a multi-step task | fucho pauses for human approval at the plan gate; ends its turn after dispatch and is autowoken by the inbox (no busy-poll) |
...
- **Bundle / prompt / guardrails:** `examples/fucho/config.yaml`
- **Sub-agents:** `examples/fucho/agents/{claude_code,codex,pi}/config.yaml`
- **Orchestration skills:** `examples/fucho/skills/{investigate,fanout,cross-review}/SKILL.md`
...
```bash
# Existing pytest e2e for fucho (mock-LLM) — complementary to this skill:
uv run --frozen --extra dev python -m pytest \
  tests/e2e/test_fucho_e2e.py \
  tests/e2e/test_fucho_cost_advisor_e2e.py \
  tests/e2e/test_fucho_subagent_model_e2e.py -q
```
