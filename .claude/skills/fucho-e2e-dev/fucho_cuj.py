#!/usr/bin/env python3
"""Deterministic mock-LLM CUJ driver for the fucho coding orchestrator.

This is the *reproducible loop* half of the ``fucho-e2e-dev`` skill. It boots a
...
the repo's mock-LLM server, rewrites the ``examples/fucho`` bundle to the
...
fucho's live judgment (that is the live recipe in ``SKILL.md``).
...
# fucho_cuj.py -> fucho-e2e-dev -> skills -> .claude -> <repo root>
...
# The mock routes /v1/responses by the request's ``model`` field; the fucho
# brain spec is rewritten to send this exact key so we own its response queue.
_BRAIN_MODEL = "mock-fucho-brain"
...
# ── Bundle rewrite (inlined from tests/e2e/test_fucho_e2e.py) ─────────────────


def _mock_fucho_bundle(tmp: Path, mock_url: str, *, rewrite_subagents: bool = False) -> Path:
    """Copy ``examples/fucho`` into *tmp* and rewrite it to use the mock LLM.
    ...
    """
    src = (_repo() / "examples" / "fucho").resolve()
    dst = tmp / "fucho"
    if dst.exists():
        shutil.rmtree(dst)
...
def _run_env(mock_url: str) -> dict[str, str]:
    ...
    config_home = Path(tempfile.mkdtemp(prefix="fucho-cuj-config-"))
...
def scenario_boot(ctx: Ctx) -> Result:
    ...
    _mock_configure(
        s.mock_url,
        [{"text": "I am fucho: I plan a coding task and delegate it to sub-agents."}],
        key=_BRAIN_MODEL,
    )
    bundle = _mock_fucho_bundle(ctx.tmp / "boot", s.mock_url)
    proc = _run_fucho(bundle, s.server_url, "In one sentence, what are you?", s.mock_url)
...
def scenario_tool_dispatch(ctx: Ctx) -> Result:
    ...
    bundle = _mock_fucho_bundle(ctx.tmp / "tool", s.mock_url)
    proc = _run_fucho(bundle, s.server_url, "Write the sentinel via shell.", s.mock_url)
    _add_exit_check(res, proc)
...
def _guardrail_scenario(
    ...
    *right* guardrail fired, not merely that something was refused.
    ...
    bundle = _mock_fucho_bundle(ctx.tmp / name, s.mock_url, rewrite_subagents=rewrite_subagents)
    proc = _run_fucho(bundle, s.server_url, prompt, s.mock_url)
    _add_exit_check(res, proc)
...
def scenario_fanout_dispatch(ctx: Ctx) -> Result:
    ...
    *finding*: it is a stateful counter, but the server rebuilds the policy
    ...
    bundle = _mock_fucho_bundle(ctx.tmp / "fanout", s.mock_url, rewrite_subagents=True)
    proc = _run_fucho(bundle, s.server_url, "Fan out a wave of workers.", s.mock_url)
    _add_exit_check(res, proc)
...
def main(argv: list[str] | None = None) -> int:
    ...
    fucho_dir = _repo() / "examples" / "fucho" / "config.yaml"
    if not fucho_dir.exists():
        print(f"error: {fucho_dir} not found — is --repo correct?", file=sys.stderr)
        return 2
    ...
    tmp = Path(tempfile.mkdtemp(prefix="fucho-cuj-"))
    all_ok = True
    try:
