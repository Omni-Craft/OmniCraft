...
description: Reference guide for building new OmniCraft harness integrations — covers SDK/subprocess harnesses and native harnesses as separate tracks, each with their own feature matrix, implementation patterns, and prioritized checklist.
---

# Harness integration guide

This skill describes the **feature matrix** every OmniCraft harness must
consider. Use it when planning, reviewing, or implementing a new harness.

OmniCraft has two distinct harness tracks with different architectures and
feature sets:
...
- **Native harnesses** — wrap a vendor's own TUI or server and mirror its
  output into OmniCraft. They observe and relay, rather than drive.

---

## Part 1 — SDK / subprocess harnesses

These harnesses run the vendor model directly and bridge OmniCraft tools into
...
|---|---|
| **Connects to OmniCraft MCP** | Harness exposes/consumes tools via the MCP protocol (in-proc SDK MCP server) |
...
| **Streaming** | Harness forwards token-level or delta-level streaming to the OmniCraft forwarder |
| **OmniCraft policies** | Harness enforces OmniCraft-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the harness surfaces the approval request in the OmniCraft web UI so the user can approve or deny |
...
| **Tool-boundary steer** | OmniCraft can inject steering text at tool-call boundaries |
| **Resume/fork from OmniCraft transcript** | Rebuild a conversation from a stored OmniCraft transcript (replay history, seed prompt, or vendor session ID) |
...
| **Cost tracking** | Harness reports token usage and cost data back to OmniCraft for each turn |

### MCP connectivity

The harness must bridge OmniCraft's builtin MCP tools so the model can call
...
The harness must support the OmniCraft policy engine's three verdicts at two
checkpoints:
...
or tool result in the OmniCraft web UI as an approval card, then relay the
...
| Vendor session ID | Relies on the vendor's own session persistence (no OmniCraft-side rebuild) |
...
- [ ] Connects to OmniCraft MCP (in-proc SDK MCP server or vendor-specific bridge)
- [ ] Model override works (or document vendor lock-in)
- [ ] Auth is configured and documented (setup flow in `omni setup`)
- [ ] Streaming forwards to the OmniCraft forwarder
- [ ] OmniCraft policies enforce tool-use rules
...
- [ ] Resume/fork rebuilds conversation from OmniCraft transcript
...
OmniCraft. They relay the vendor's conversation into the OmniCraft session.
...
| **Connects to OmniCraft MCP** | Whether the native harness connects to the OmniCraft MCP server |
...
| **OmniCraft policies** | Whether the native harness enforces OmniCraft-side tool policies — must support ALLOW, ASK, and DENY verdicts for both tool calls and tool results |
| **Native elicitation** | When a policy verdict is ASK, the native harness surfaces the approval request in the OmniCraft web UI so the user can approve or deny |
| **Interrupt** | User can abort a running turn |
| **Bidirectional sync (TUI->Omni)** | TUI output mirrors into the OmniCraft conversation |
| **In-harness session-cmd sync** | Supports `clear`, `fork`, `resume`, `switch` commands from OmniCraft |
| **Resume/fork from OmniCraft transcript** | Can rebuild conversation from OmniCraft transcript (native rebuild, or fresh launch) |
...
| **Cost tracking** | Native harness reports token usage and cost data back to OmniCraft for each turn |
...
- [ ] Connects to OmniCraft MCP
- [ ] Auth configured (vendor login / config)
- [ ] Streaming forwarder works (deltas preferred; complete-only acceptable)
- [ ] OmniCraft policies enforce tool-use rules (ALLOW / ASK / DENY at both tool call and tool result)
- [ ] Native elicitation surfaces tool-approval requests to web UI
- [ ] Interrupt aborts the running turn
- [ ] Bidirectional sync mirrors TUI output into OmniCraft conversation
- [ ] Cost tracking reports token usage and cost per turn
...
- [ ] Session commands (clear, fork, resume) work from OmniCraft
- [ ] Resume/fork rebuilds from OmniCraft transcript
- [ ] Reasoning tokens are forwarded
