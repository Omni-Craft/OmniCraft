...
description: Spin up a live local OmniCraft server + runner and exercise the native Pi TUI harness (pi-native) end-to-end — launch the real `pi` CLI via `omnicraft pi`, drive turns through the web/bridge, smoke-test, and bug-bash. Load when developing, testing, or debugging the pi-native harness (omnicraft/inner/pi_native_executor.py, pi_native_harness.py, omnicraft/pi_native.py, pi_native_bridge.py, pi_native_credentials.py) or its bridge / extension / auth / model behavior.
---
...
(cursor / copilot / antigravity), it does **not** run in-process: `omnicraft pi`
ensures a host daemon, the daemon spawns a **runner** that launches `pi` inside a
runner-owned **tmux** terminal, and your TTY attaches to it. OmniCraft's web-UI
...
```
your TTY ── (attach / pexpect) ──► omnicraft pi (CLI, local)
                                        │ ensures
                                        ▼
                                  ...
                                  runner ── launches ──► pi (TUI, in tmux)
                                                              │ loads
                                                              ▼
                                                 omnicraft pi-native extension (JS)
```
...
2. **The `pi` CLI is on PATH** — the harness can't launch without it:
   ...
   # or point at an explicit binary:  export OMNICRAFT_PI_PATH=/path/to/pi
   .venv/bin/python -c "from omnicraft.onboarding.harness_readiness import harness_is_configured; print('pi-native ready:', harness_is_configured('pi-native'))"
   ```
...
5. **Auth is resolvable (booleans/ids only — never print keys).** Native Pi
   normally logs in from its own `~/.pi/agent`. OmniCraft bridges the provider you
   set with `omnicraft setup` instead, writing a managed per-session `models.json`
   and passing `--provider omnicraft --model <resolved>`. Verify what it will use:
   ```bash
   .venv/bin/python -c "from omnicraft.pi_native_credentials import resolve_pi_native_provider as r; p=r(); print('provider:', getattr(p,'provider_id',None), '| api:', getattr(p,'api',None), '| model:', getattr(p,'model',None))"
   ```
   `None` → no omnicraft provider configured; Pi falls back to its own `/login`
   (run `omnicraft setup`, or log into `pi` directly). A Databricks default
...
(`omnicraft pi --server ""` also auto-spawns a persistent local server and uses
...
`omnicraft pi` **attaches an interactive TUI**, so run it where you can hold it
open. Two patterns:
...
```bash
.venv/bin/omnicraft pi --server "$SERVER" 2>&1   # attaches the Pi TUI; leave it running
```
...
spawn `omnicraft pi --server <url>` in a PTY with `cwd=<checkout>`, capture the
...
`terminal_launch_args`), e.g. `omnicraft pi --server "$SERVER" -- --model <id>`;
omnicraft still injects `--provider omnicraft --model <resolved>` when a provider
...
```bash
.venv/bin/python -c "from omnicraft.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))"
# ~/.omnicraft/pi-native/<sha256(conv)[:32]>/
...
ls -la "$(.venv/bin/python -c "from omnicraft.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))")/inbox"
```
...
`HARNESS_PI_NATIVE_REQUEST_SESSION_ID`, `OMNICRAFT_PI_NATIVE_CONFIG`,
`OMNICRAFT_PI_PATH` (legacy `HARNESS_PI_PATH`), `PI_CODING_AGENT_DIR`.
...
| Resume | stop the TUI, `omnicraft pi --server "$SERVER" --resume "$CONV"` — reattaches; `--resume` (no value) opens the pi-native picker |
...
1. **It's a TUI, not `omni run`.** Use `omnicraft pi`. There is no
...
2. **`config.yaml`'s `server:` defaults to a remote server.** Always pass
   ...
   checkout (allowlist: `omnicraft/spec/_omnicraft_compat.py`).
...
- **Executor (bridge enqueue):** `omnicraft/inner/pi_native_executor.py`
- **Harness wrap (`harness: pi-native`):** `omnicraft/inner/pi_native_harness.py`
- **CLI launch / daemon-runner / tmux attach:** `omnicraft/pi_native.py`
  (`run_pi_native`); CLI command `pi(...)` in `omnicraft/cli.py`
- **Bridge (inbox, extension/config writers):** `omnicraft/pi_native_bridge.py`
- **Auth/model → Pi `models.json`:** `omnicraft/pi_native_credentials.py`
- **Extension (JS, polls inbox, posts events/policies):**
  `omnicraft/resources/pi_native/omnicraft_pi_native_extension.js`
- **Readiness gate:** `omnicraft/onboarding/harness_readiness.py`
...
# JS unit tests: node omnicraft/resources/pi_native/omnicraft_pi_native_extension.test.js
```
...
- **History on a rebuilt session** depends on Pi's own `--session-dir` state under
  the bridge dir, not on OmniCraft re-injecting transcript.
...
# rm -rf "$(.venv/bin/python -c "from omnicraft.pi_native import pi_bridge_dir_for_session as d; print(d('$CONV'))")"
```
