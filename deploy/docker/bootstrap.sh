...
# required secrets (POSTGRES_PASSWORD, OMNICRAFT_OIDC_COOKIE_SECRET)
...
#   - If OMNICRAFT_OIDC_COOKIE_SECRET is unset OR commented out,
...
cookie_current=$(current_value OMNICRAFT_OIDC_COOKIE_SECRET)
if [[ -z "$cookie_current" || "$cookie_current" == "<64-hex-chars>" ]]; then
  set_or_replace_kv OMNICRAFT_OIDC_COOKIE_SECRET "$(openssl rand -hex 32)"
  echo "→ generated OMNICRAFT_OIDC_COOKIE_SECRET"
else
  echo "→ OMNICRAFT_OIDC_COOKIE_SECRET already set, leaving alone"
fi
...
# the operator can switch modes by editing OMNICRAFT_AUTH_PROVIDER
# without re-running bootstrap.
accounts_cookie_current=$(current_value OMNICRAFT_ACCOUNTS_COOKIE_SECRET)
if [[ -z "$accounts_cookie_current" || "$accounts_cookie_current" == "<64-hex-chars>" ]]; then
  set_or_replace_kv OMNICRAFT_ACCOUNTS_COOKIE_SECRET "$(openssl rand -hex 32)"
  echo "→ generated OMNICRAFT_ACCOUNTS_COOKIE_SECRET"
else
  echo "→ OMNICRAFT_ACCOUNTS_COOKIE_SECRET already set, leaving alone"
fi

echo
echo "✓ deploy/docker/.env is ready. Next:"
echo "    docker compose up -d && docker compose logs omnicraft"
echo
...
echo "    OMNICRAFT_ACCOUNTS_BASE_URL=<your public URL>"
