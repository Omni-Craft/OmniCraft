...
> `ghcr.io/omnicraft-ai/omnicraft-host` directly — E2B cannot start an
> arbitrary registry image at create time. You must first build the
> OmniCraft host image into an E2B template (a one-time step, below); the
...
```bash
pip install 'omnicraft[e2b]'   # installs the e2b SDK extra
...
> Hobby**) with no "never expire" option. OmniCraft requests the 24 h
...
> `OMNICRAFT_E2B_MAX_LIFETIME_S` to request a specific lifetime and skip the
...
**Debian-based** and **single-stage**. The OmniCraft host image
...
# already bakes the full omnicraft install plus git / tmux / curl, so
...
`omnicraft-host` is the default template name the launcher looks for
([`DEFAULT_E2B_TEMPLATE`](../../omnicraft/onboarding/sandboxes/e2b.py)), so
...
with `sandbox.e2b.template` / `OMNICRAFT_E2B_TEMPLATE`.
...
This starts a sandbox from the `omnicraft-host` template, builds wheels
...
`OMNICRAFT_E2B_SANDBOX_ENV` in your shell to a comma-separated list of
...
> `omnicraft login` / App OAuth step is therefore skipped automatically
...
sandbox:
  ...
  e2b:
    template: omnicraft-host          # E2B template NAME (default: omnicraft-host)
...
> the other providers. Omit it to use the default `omnicraft-host`
> template.
...
`DATABRICKS_TOKEN`) in `OMNICRAFT_E2B_SANDBOX_ENV` before `create`. See
...
- **All managed sandboxes share one E2B account + API key.** Cross-user
  isolation between OmniCraft users rides on E2B's sandbox boundaries, and
...
- **The launch token's lifetime is ~25 h, derived from the *requested*
  ...
  across reconnects. The TTL is computed from `OMNICRAFT_E2B_MAX_LIFETIME_S`
  ...
  tighten this, **set `OMNICRAFT_E2B_MAX_LIFETIME_S` to your account cap** so
...
- **Sandbox URLs are public by default.** E2B exposes sandbox ports via
  public `*.e2b.app` URLs; OmniCraft never opens one (the host dials *out*
...
- **"E2B sandbox creation failed: template '…' is unavailable"** — the
  ...
  `--name omnicraft-host` (or set `sandbox.e2b.template` to your name).
...
- **Hard lifetime cap, no idle-stop disable.** `provision` requests
  `OMNICRAFT_E2B_MAX_LIFETIME_S` (default the 24 h Pro maximum); E2B rejects
...
- **Custom images** require rebuilding the template: `FROM` your image in
  ...
  `sandbox.e2b.template` / `OMNICRAFT_E2B_TEMPLATE`.
...
| `OMNICRAFT_E2B_TEMPLATE` | CLI machine / server | E2B template name to provision from (`sandbox.e2b.template` takes precedence; default `omnicraft-host`) |
| `OMNICRAFT_E2B_SANDBOX_ENV` | CLI machine / server | Comma-separated launcher-side env var names to inject (`sandbox.e2b.env` takes precedence for managed) |
| `OMNICRAFT_E2B_MAX_LIFETIME_S` | CLI machine / server | Requested sandbox lifetime in seconds (default 24 h); creation auto-clamps to the account cap if exceeded |
