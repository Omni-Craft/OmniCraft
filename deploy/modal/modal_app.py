"""Modal deploy glue for the OmniCraft server.

Runs the standard server image (``ghcr.io/omnicraft-ai/omnicraft-server``)
...
SERVER_IMAGE = "ghcr.io/omnicraft-ai/omnicraft-server:latest"
...
artifacts = modal.Volume.from_name("omnicraft-artifacts", create_if_missing=True)


@app.function(
    image=modal.Image.from_registry(SERVER_IMAGE),
    # DATABASE_URL, OMNICRAFT_ACCOUNTS_COOKIE_SECRET, and
    # OMNICRAFT_ACCOUNTS_BASE_URL — created in the README's step 1.
    secrets=[modal.Secret.from_name("omnicraft-deploy")],
