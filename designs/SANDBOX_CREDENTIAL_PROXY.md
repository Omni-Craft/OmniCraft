...
> Code: `omnicraft/inner/credential_proxy.py`,
> `omnicraft/inner/egress/proxy.py`, `omnicraft/spec/parser.py`.
...
`omnicraft/inner/egress/`). We extend it to attach credentials. The
...
The parser (`omnicraft/spec/parser.py`, `_parse_credential_proxy`)
...
`ValidationError` as an `OmniCraftError`. It then normalizes the four
...
`omnicraft/inner/credential_proxy.py`:
...
> `OMNICRAFT_CREDENTIAL_PROXY_GIT_HTTPS` env var are gone.

## Proxy rewrite

`omnicraft/inner/egress/proxy.py`: `EgressProxy` takes
...
`omnicraft/inner/egress/controller.py` threads `credential_rewrites`
...
- `omnicraft/inner/os_env.py` — `_start_locked` builds a scoped parent
...
- `omnicraft/inner/sandbox.py` — `credential_proxy` field on
