...
| **AI review** | **Fucho** (`fucho-review.yml`) reviews with full secrets, maintainer-triggered two ways: a `/review` comment, or **PR approval** (the `fucho-review-on-approval.yml` → `fucho-review-approval-dispatch.yml` relay dispatches it). Copilot is optional, on-demand via the maintainer's one-click **Reviewers → Copilot** button | Wired up | Keep Fucho. It **can't** auto-run on a fork `pull_request` (no secrets), so it's maintainer-triggered (comment or approval); either path runs trusted (default-branch checkout + diff via API, never executing PR code) and posts a fresh comment per run. Copilot is a one-click manual add (no workflow) and only yields a review once the org enables "allow unlicensed contributors" |
...
- **AI review tool** — **Fucho** is the implemented reviewer (cross-vendor, runs on our infra via the LLM gateway). Copilot stays available as an optional one-click manual reviewer; we **removed** the auto-request workflow (`copilot-review.yml`) as redundant — it produced a misleading always-green check and did nothing until the org *"allow unlicensed contributors"* policy is enabled, which the one-click button covers anyway. Lilo is a possible future alternative.
- **AI review trigger on forks** — the secret-bearing Fucho review can't fire automatically on a fork `pull_request` (GitHub withholds repo secrets), so it's maintainer-triggered two ways: (1) an `OWNER`/`MEMBER`/`COLLABORATOR` posts a `/review` comment (the `issue_comment` run executes in the trusted base context); or (2) a maintainer **approves** the fork PR, which a two-stage relay turns into a review — `fucho-review-on-approval.yml` records the PR number on the read-only review event, then `fucho-review-approval-dispatch.yml` (privileged, on `workflow_run`) re-validates *fork + maintainer-approval* from trusted API data and `MAINTAINER@main` before dispatching `fucho-review.yml`. Both paths check out the default branch and fetch the diff via API, never executing PR code, and each run posts a fresh review comment. If we want Fucho to auto-run on every fork PR with no maintainer action, switch its trigger to `pull_request_target` (same trusted pattern the security gate uses) — at the cost of a full review spinning up on every fork push.
...
3. **Front-load automation** — AI review via a maintainer `/review` comment (Fucho; auto-Copilot where org policy allows), and **promote coverage to a required check** with a changed-lines threshold, so the human review reduces to "is it safe?" and "do we want it?".
