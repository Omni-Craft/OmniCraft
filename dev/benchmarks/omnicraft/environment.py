...
OmniCraft ``server`` with no Databricks credentials. Two modes:
...
class BenchEnvironment:
    ...
    def _spawn_server(
        ...
        if self.with_runner:
            ...
            # the classifier only builds under OMNICRAFT_SMART_ROUTING=1.
            ...
            env["OMNICRAFT_RUNNER_TUNNEL_TOKEN"] = binding_token
        return subprocess.Popen(
            args,
            env=env,
    ...
    def _spawn_runner(
        ...
        runner_env = apply_runner_env(
            {
                **base_env,
                "OMNICRAFT_RUNNER_ID": self.runner_id,
                ...
                "OMNICRAFT_RUNNER_WORKSPACE": str(workspace),
            }
        )
        return subprocess.Popen(
    ...
    async def time_to_first_delta(
        ...
        times omnicraft's streaming-pipeline overhead to first token — with the
