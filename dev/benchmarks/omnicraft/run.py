"""OmniCraft user-journey benchmark runner.

Boots a real ``omnicraft server`` against a SQLite DB (no runner, no LLM),
...
Runs in the project venv — it imports ``omnicraft`` and ``tests._helpers`` and
...
project (which triggers a web-UI build that fails in a worktree)::
    ...
    uv run --no-sync dev/benchmarks/omnicraft/run.py --journeys list_sessions,get_session
    uv run --no-sync dev/benchmarks/omnicraft/run.py --requests 500 --concurrency 25 --runs 3
    uv run --no-sync dev/benchmarks/omnicraft/run.py --output bench.json --max-p50-ms 25
...
from dev.benchmarks.omnicraft.environment import BenchEnvironment
from dev.benchmarks.omnicraft.journeys import (
    ALL_JOURNEYS,
    Journey,
    resolve_journeys,
    run_latency,
...
from dev.benchmarks.omnicraft.measure import (
    RunResult,
    aggregate,
    check_thresholds,
    console,
...
from dev.benchmarks.omnicraft.schema import build_report
