...
When test code writes::
    ...
    with patch("omnicraft.tools.mcp.asyncio.sleep", new_callable=AsyncMock):
        ...

``unittest.mock``'s dotted-path resolver walks:

1. ``import omnicraft.tools.mcp``
2. ``getattr(omnicraft.tools.mcp, "asyncio")`` -> returns the
...
def _attribute_ends_in_asyncio(node: ast.expr) -> bool:
    ...
    e.g. matches ``mcp_mod.asyncio`` and ``omnicraft.tools.mcp.asyncio``.
    """
