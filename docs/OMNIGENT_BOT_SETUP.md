# OmniCraft bot identities & attribution setup

This doc explains how commits and automated PR reviews in the
`omnicraft-ai/omnicraft` repo are attributed, and how the supporting GitHub App
...
## The GitHub App: `omnicraft-ci[bot]`

> **Naming note.** The App was registered as **`omnicraft-ci`** (the bare
> `omnicraft` name was unavailable), so GitHub renders the actor as
> **`omnicraft-ci[bot]`**.

| Field | Value |
| --- | --- |
| App name | `omnicraft-ci` |
| Bot actor | `omnicraft-ci[bot]` |
| App ID | `4082516` |
| Bot numeric user ID | `294685417` |
| CI git author email | `294685417+omnicraft-ci[bot]@users.noreply.github.com` |
...
Created at `https://github.com/organizations/omnicraft-ai/settings/apps/new`:

- **GitHub App name:** `omnicraft-ci` → actor `omnicraft-ci[bot]`.
...
- **Where can this App be installed?** Only on `omnicraft-ai`.
- Installed into `omnicraft-ai`, scoped to the `omnicraft` repo.
...
```bash
gh api users/omnicraft-ci%5Bbot%5D --jq '.id'
# -> 294685417
```
...
In **`omnicraft-ai/omnicraft` → Settings → Secrets and variables → Actions**:

- **Variable** `OMNICRAFT_BOT_APP_ID` = `4082516`
- **Secret** `OMNICRAFT_BOT_APP_KEY` = the App's `.pem` private key

> The workflows gate on `vars.OMNICRAFT_BOT_APP_ID != ''`. If the variable is
...
> **`omnicraft-ci[bot]` replaced the old OSS regen bot.** The previous
...
- name: Mint App token
  id: app-token
  if: vars.OMNICRAFT_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.OMNICRAFT_BOT_APP_ID }}
    private-key: ${{ secrets.OMNICRAFT_BOT_APP_KEY }}
```

```bash
git config user.name  "omnicraft-ci[bot]"
git config user.email "294685417+omnicraft-ci[bot]@users.noreply.github.com"
```
...
- name: Mint App token
  id: app-token
  if: steps.trigger.outputs.skip != 'true' && steps.creds.outputs.available == 'true' && vars.OMNICRAFT_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.OMNICRAFT_BOT_APP_ID }}
    private-key: ${{ secrets.OMNICRAFT_BOT_APP_KEY }}
