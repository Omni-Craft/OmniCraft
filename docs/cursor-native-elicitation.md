...
`omnicraft/server/routes/sessions.py`, runner wiring in
`omnicraft/runner/native/orchestration.py`, `web/.../ApprovalCard.tsx`.
...
stays the source of truth — OmniCraft never modifies cursor's JS bundle and never suppresses
...
- Because a gate answered this way is never seen by a human, the accept logs the tool name and
  an argument preview at INFO: that line is the only record OmniCraft approved the call.
