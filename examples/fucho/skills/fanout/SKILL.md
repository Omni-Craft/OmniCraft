---
name: fanout
description: Run independent subtasks in parallel — one git worktree and one implementation sub-agent per task, each opening its own PR — then cross-review every PR. fucho never merges; the human does.
---
...
1. Per task, create an isolated worktree:
   `sys_os_shell("git worktree add .worktrees/<task_id> -b fucho/<task_id>")`.
...
2. Dispatch one implementation sub-agent per task, scoped to its worktree:
   ...
   line — `Co-authored-by: OmniCraft <305612714+Omnicraft-bot@users.noreply.github.com>`.
...
5. fucho does NOT merge — the PR is the deliverable. When cross-review passes,
...
- Because fucho never merges, cross-PR conflicts surface when the human merges,
