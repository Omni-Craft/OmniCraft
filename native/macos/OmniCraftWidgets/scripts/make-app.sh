#!/usr/bin/env bash
# Monta OmniCraftWidgets.app a partir do binário do SwiftPM.
#
...
BIN="$(swift build -c "$CONFIG" --show-bin-path)/OmniCraftWidgets"
[ -x "$BIN" ] || { echo "binário não encontrado: $BIN" >&2; exit 1; }

APP=".build/OmniCraftWidgets.app"
rm -rf "$APP"
mkdir -p "$APP/Contents/MacOS"

cp "$BIN" "$APP/Contents/MacOS/OmniCraftWidgets"
...
<dict>
    ...
    <string>OmniCraft Widgets</string>
    <key>CFBundleDisplayName</key>
    <string>OmniCraft Widgets</string>
    <key>CFBundleIdentifier</key>
    <string>app.omnicraft.widgets</string>
    <key>CFBundleExecutable</key>
    <string>OmniCraftWidgets</string>
