...
omnicraft grew up on Linux/macOS and bakes a number of POSIX assumptions into
...
#: Non-sensitive Windows environment variables that a spawned omnicraft
...
def stable_user_id() -> str:
    ...
    ``omnicraft-<id>`` / ``claude-<id>`` under the temp dir). On POSIX this is
