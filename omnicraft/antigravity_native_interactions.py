...
OmniCraft elicitation, waits for the human's verdict, and delivers it back to agy
...
WAITING step at a HIGHER ``stepIndex``**. OmniCraft elicitations wait on a human
...
* ``deliver`` — defaults to :func:`_deliver_via_rpc`, which offloads the blocking
  :func:`omnicraft.antigravity_native_rpc.handle_user_interaction` to a worker
...
:func:`omnicraft.codex_native_elicitation.codex_elicitation_id`), so a
...
from omnicraft.antigravity_native_steps import PendingInteraction, pending_interaction
from omnicraft.server.routes._antigravity_elicitation import (
...
class Deliver(Protocol):
    ...
    Matches :func:`omnicraft.antigravity_native_rpc.handle_user_interaction`
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    :func:`omnicraft.antigravity_native_bridge.send_interaction_keys_via_tui`) to a
    ...
    from omnicraft.antigravity_native_bridge import send_interaction_keys_via_tui
    from omnicraft.inner.antigravity_native_executor import _bridge_dir_from_env
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    """
    Build the OmniCraft elicitation id for one agy WAITING interaction.
    ...
    :func:`omnicraft.codex_native_elicitation.codex_elicitation_id`.
...
async def _deliver_via_rpc(
    ...
    *,
    ...
    :func:`omnicraft.antigravity_native_rpc.handle_user_interaction` is synchronous
...
async def bridge_interaction(
    ...
    *,
    ...
    :param deliver: Delivers the built payload to agy; defaults to
        ...
        :func:`omnicraft.antigravity_native_rpc.handle_user_interaction` to a
        worker thread.
