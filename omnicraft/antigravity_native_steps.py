...
RPC read driver (:mod:`omnicraft.antigravity_native_reader`) imports them from
this module.
...
# OmniCraft ``agent`` label stamped on mirrored assistant/function-call items so
...
class OutboundEvent:
    """
    One OmniCraft session event to POST for an agy step.
    ...
    :param event_type: OmniCraft session event type, e.g.
...
def _response_id(conversation_id: str, step_idx: int) -> str:
    """
    Build a stable OmniCraft response id for a RPC step.
...
def _user_input_text(user_input: object) -> str:
    ...
    :func:`omnicraft.antigravity_native_rpc.send_user_cascade_message` sends).
...
def map_step_to_events(
    ...
    *,
    ...
    """
    Map one agy RPC step to OmniCraft conversation-item events.
