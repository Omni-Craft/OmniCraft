"""Implementation of the ``omnicraft chat`` command.

The CLI always ends by connecting an OmniCraft client to a server URL. For
...
from omnicraft_client import (
    OmniCraftError as ClientOmniCraftError,
)
...
from omnicraft.conversation_browser import open_conversation_link_if_enabled
...
from omnicraft.inner.databricks_executor import _DatabricksBearerAuth, _read_databrickscfg
from omnicraft.native_coding_agents import native_coding_agent_for_wrapper_label
from omnicraft.spec import load as load_spec
from omnicraft.spec._omnicraft_compat import OMNICRAFT_EXECUTOR_TYPE
from omnicraft.spec.parser import discover_host_skills
from omnicraft.spec.types import AgentSpec, SkillSpec
...
# freshly-launched ``omnicraft run`` sessions don't burn a
...
# ``_DEFAULT_AD_HOC_MODEL`` so ``omnicraft run examples/hello_world.yaml``
# (a spec with no executor block) launches cleanly instead of
# failing the strict omnicraft validator with a cryptic
...
# Optional bearer token for remote omnicraft servers that sit
...
_REMOTE_AUTH_TOKEN_ENV = "OMNICRAFT_REMOTE_AUTH_TOKEN"

# Env-var override name. ``OMNICRAFT_MODEL=foo`` lets a user
...
# ``omnicraft/inner/cli.py:344``.
_OMNICRAFT_MODEL_ENV_VAR = "OMNICRAFT_MODEL"
...
def _default_cli_model() -> str:
    ...
    Reads ``OMNICRAFT_MODEL`` from the environment with
    ...
    Mirrors :func:`omnicraft.inner.cli._default_cli_model` so
    legacy and OmniCraft paths agree on the env-var contract.

    :returns: The default model identifier, e.g.
        ``"databricks-gpt-5-4"`` or whatever the user pinned in
        ``OMNICRAFT_MODEL``.
    """
    return os.environ.get(_OMNICRAFT_MODEL_ENV_VAR, _DEFAULT_AD_HOC_MODEL)


@dataclass(frozen=True)
class ChatOverrides:
    """
    CLI overrides from ``omnicraft run`` flags.
    ...
    :param harness: ``--harness`` value, e.g. ``"claude-sdk"``.
        ...
        ``executor.harness`` key for single-file omnicraft YAMLs and to
...
class LocalServer:
    """
    Handle to a locally-launched omnicraft server and its sibling runner.
    ...
    :param log_path: Path to the file that captures the
        ...
        e.g. ``Path("~/.omnicraft/logs/server/server-abc123.log")``.
...
def run_chat(
    ...
    *,
    ...
    """
    Main entry point for ``omnicraft run`` (and the ``attach`` client).
    ...
    :param ephemeral: When ``True``, place the local server's
        ...
        the persistent ``~/.omnicraft`` location. Maps to
    ...
    :param log: When ``True``, write a JSON dump of the active
        conversation to ``~/.omnicraft/logs/`` on REPL exit.
        Maps to ``--log`` on the CLI (default-on for the legacy
        path, default-off here so it stays explicit on
        OmniCraft mode). See ``omnicraft.repl._session_log`` for the
...
def run_prompt(
    ...
    *,
    ...
    ``omnicraft run ... -p``. It deliberately bypasses the
...
def run_attach(
    *,
    ...
    :param base_url: OmniCraft server hosting the session, e.g.
        ``"http://127.0.0.1:6767"``.
    ...
    if not info.runner_online:
        raise click.ClickException(
            ...
            "(`omnicraft run` locally, or reconnect it with `omnicraft host`), "
            "then attach again."
        )
...
def _remote_headers(
    ...
    Resolution order:
      1. explicit ``OMNICRAFT_REMOTE_AUTH_TOKEN`` env var
      2. stored OIDC token from ``~/.omnicraft/auth_tokens.json``
         (populated by ``omnicraft login``)
      3. stored Databricks Apps pointer record for ``server_url``
         (populated by ``omnicraft login <apps-url>``) — mints a
    ...
    This lets ``omnicraft run --server <apps-url>`` work against
    Databricks Apps after a one-time ``omnicraft login <apps-url>``,
    ...
    :returns: Headers to pass to httpx / OmniCraftClient.
    """
    ...
    if token and (token := token.strip()):
        ...
        from omnicraft.cli_auth import load_token

        # 2. Stored OIDC session token from `omnicraft login`.
...
def _stored_databricks_record_token(server_url: str) -> str | None:
    ...
    ``omnicraft login <apps-url>`` stores a pointer record naming the
    ...
    """
    from omnicraft.cli_auth import load_databricks_workspace_host
...
class _DatabricksTokenAuth(httpx.Auth):
    ...
    Resolution order:
      1. static env-var token (``OMNICRAFT_REMOTE_AUTH_TOKEN``)
      2. stored OIDC token (from ``omnicraft login``)
    ...
    def _sdk_token(self) -> str | None:
        ...
        pointer record for the server (from ``omnicraft login
    ...
    def auth_flow(self, request: httpx.Request) -> Generator[httpx.Request, httpx.Response, None]:
        ...
        if self._static_token:
            ...
            yield request
            return
        # Check stored OIDC token from `omnicraft login`.
        if self._server_url:
            from omnicraft.cli_auth import load_token
...
def _server_headers(
    *,
    ...
    """
    Build non-auth HTTP headers for an OmniCraft server client.
    ...
    :param runner_id: Optional runner UUID, e.g.
        ...
        request header.
    :returns: Static headers for ``httpx`` / ``OmniCraftClient``.
    """
    del runner_id
    return {}
...
def _server_auth(
    ...
    # Check stored `omnicraft login` records: a session JWT or a
    # Databricks Apps pointer record.
    if server_url:
        from omnicraft.cli_auth import load_databricks_workspace_host, load_token
...
def _chat_with_server(
    ...
    *agent_name* is supplied by an upstream setup step such as
    ...
    :param server_log_path: Path to the local server's
        ...
        ``Path("~/.omnicraft/logs/server/server-abc123.log")``. Shown in the
        Ctrl+O debug overview. ``None`` for remote servers.
    :param runner_log_path: Path to the local runner's
        stdout/stderr log file, e.g.
        ``Path("~/.omnicraft/logs/runner/runner-abc123.log")``. Shown in the
    ...
    # ``omnicraft.wrapper`` label on the conversation and re-dispatch
...
def _is_claude_native_conversation(
    *,
    ...
    :param conversation_id: OmniCraft conversation id.
...
def _redirect_native_resume_if_needed(
    *,
    ...
    """
    Redirect a terminal-native resume before OmniCraft attach liveness runs.

    :param base_url: OmniCraft server base URL, e.g. ``"https://example.com"``.
    :param conversation_id: OmniCraft conversation id, e.g. ``"conv_abc123"``.
...
def _finish_native_redirect_progress(
    *,
    ...
    """
    Finish any OmniCraft startup progress and print the native redirect notice.

    :param progress: Optional startup spinner to finish before writing.
    :param conversation_id: OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
    ...
    click.echo(
        (
            ...
            f"session — redirecting to `omnicraft {native_command} --resume`.\n"
        ),
        err=True,
    )
...
def _run_claude_native_resume_redirect(
    *,
    ...
    """
    Hand a claude-native conversation back to ``omnicraft claude``.
    ...
    :param conversation_id: OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
    :param auto_open_conversation: Browser-open preference for the wrapper.
    :param progress: Optional OmniCraft startup spinner to finish before redirect.
    :returns: None.
    """
...
def _run_codex_native_resume_redirect(
    *,
    ...
    """
    Hand a codex-native conversation back to ``omnicraft codex``.
...
def _run_pi_native_resume_redirect(
    *,
    ...
    """
    Hand a pi-native conversation back to ``omnicraft pi``.
...
def _run_kiro_native_resume_redirect(
    *,
    ...
    """Hand a kiro-native conversation back to ``omnicraft kiro``."""
    _finish_native_redirect_progress(
        progress=progress,
        conversation_id=conversation_id,
...
def _run_cursor_native_resume_redirect(
    *,
    ...
    """
    Hand a cursor-native conversation back to ``omnicraft cursor``.
    ...
    into the conversation. Resuming through the OmniCraft REPL would instead
    run an OmniCraft turn per message (which persists its own user item) *and*
    leave the forwarder mirroring the same message from the cursor store —
    recording each user message twice. Redirecting to ``omnicraft cursor``'s
...
def _run_kimi_native_resume_redirect(
    *,
    ...
    """
    Hand a kimi-native conversation back to ``omnicraft kimi``.

    The kimi-native session is driven by the ``kimi`` TUI in a runner-owned
    tmux pane. Resuming through the OmniCraft REPL would run an OmniCraft turn
    per message instead of attaching to the live TUI; redirecting to
    ``omnicraft kimi``'s direct tmux attach keeps the TUI the single source of
...
def _wrapper_label_for_conversation(
    *,
    ...
    the response's ``labels.omnicraft.wrapper`` field. ``None`` on any
    transport / parse error so a flaky server doesn't silently misroute
    the resume — the caller falls back to the normal OmniCraft REPL path and
...
def _await_accounts_first_run_setup(
    ...
    *,
    ...
    When ``omnicraft run`` (re)spawns the local OmniCraft server in accounts mode on
    ...
    the browser auto-opened (e.g. ``OMNICRAFT_ACCOUNTS_AUTO_OPEN=0``) — and
    ...
    :param base_url: Resolved local OmniCraft server URL, e.g.
        ``"http://127.0.0.1:6767"``.
...
async def _prepare_chat_session_via_daemon(
    *,
    ...
    :param base_url: OmniCraft server base URL, e.g. ``"http://127.0.0.1:8123"``.
    ...
    async with OmniCraftClient(base_url=base_url, headers=headers, auth=auth) as sdk:
    ...
    async with httpx.AsyncClient(
        ...
        # clear the ``omnicraft.stopped`` marker on resumed sessions. Must run
...
def _chat_via_daemon(
    ...
    *,
    ...
    :param base_url: Resolved OmniCraft server base URL (the daemon is already
...
def _wait_for_remote_runner(
    ...
    *,
    ...
    :param log_path: Optional path to the captured runner log
        ...
        e.g. ``Path("~/.omnicraft/logs/runner/runner-abcd.log")``.
    ...
    :param show_progress: When ``True`` (default), render a rich
        ...
        non-TTY stderr; see :mod:`omnicraft._runner_startup`.
    :returns: None.
        ...
        try:
            ...
            if resp.status_code in {401, 403}:
                raise click.ClickException(
                    ...
                    "run `omnicraft login <server-url>` or check remote auth credentials."
                    f"{format_runner_log_tail(log_path)}"
                )
        except httpx.HTTPError as exc:
            last_error = exc
...
def _bundle_agent(agent_path: Path) -> bytes:
    ...
    :raises OmniCraftError: If bundling fails, for example due to
        unresolved environment variables.
    """
    from omnicraft.cli import _bundle
...
def _chat_local(
    ...
    *,
    ...
    :param ephemeral: When ``True``, point the local server at a
        ...
        (default) uses the persistent ``~/.omnicraft``
        location so prior conversations remain reachable —
        see designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md.
    ...
    try:
        ...
        try:
            agent_spec = load_spec(spec_path)
        except (OmniCraftError, FileNotFoundError) as exc:
...
def _run_headless_prompt(
    ...
    *,
    ...
    try:
        asyncio.run(_main())
    except ClientOmniCraftError as exc:
...
async def _query_sessions_once(
    *,
    ...
    """
    from omnicraft_client import SessionsChat
    ...
    # raises ``OmniCraftError`` on that ``failed`` status, and the
    ...
    # ClientOmniCraftError`` and print the message to stderr + exit non-zero.
    turn_error = await _persisted_turn_error(client, bound.id)
    if turn_error is not None:
        raise ClientOmniCraftError(turn_error)
    return None
...
def _response_output_text(output: _ResponseOutput) -> str | None:
    """Extract assistant text from an OmniCraft response ``output`` list."""
    parts: list[str] = []
    for item in output:
...
async def _persisted_turn_text(
    ...
    * A transport-level runner disconnect publishes
      ...
      :meth:`SessionsChat.send` raises ``OmniCraftError`` on it.
...
def _resolve_resume_target(
    *,
    ...
    ``omnicraft/inner/cli.py:3082-3084`` ("No saved sessions
    to continue.").
...
def _assert_resume_conversation_exists(
    *,
    ...
    async def _lookup() -> None:
        async with OmniCraftClient(base_url=base_url, headers=headers) as client:
...
def _run_picker(
    *,
    ...
    :raises click.ClickException: When the agent has no prior
        ...
        out of.
    """
    from omnicraft.repl._resume_picker import pick_conversation_from_sdk

    async def _lookup() -> str | None:
        async with OmniCraftClient(base_url=base_url, headers=headers) as client:
            # Multipart ``omnicraft run <yaml>`` uploads now create a
...
async def _resolve_latest_conversation_id_async(
    *,
    ...
    ``OmniCraftClient`` connected to a real URL — the path
    used in production by ``_chat_local``.

    :param client: A connected :class:`OmniCraftClient`.
...
def _materialize_override_bundle(source: Path, overrides: ChatOverrides) -> Path:
    ...
    omnicraft validator rejects that shape, and the legacy
    argparse CLI used to paper over it by injecting
    :data:`_DEFAULT_AD_HOC_MODEL`. This preserves that behavior so
    ``omnicraft run examples/hello_world.yaml`` (minimal spec) still
    launches cleanly.
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="omnicraft-override-"))
    try:
        if source.is_file():
...
def _cleanup_materialized_override_bundle(materialized: Path) -> None:
    ...
    :param materialized: Path returned by
        ...
        ``Path("/tmp/omnicraft-override-abc/agent.yaml")``.
    :returns: None.
    """
...
def _load_yaml_if_single_file(source: Path) -> _YamlMapping | None:
    ...
    Directories (omnicraft-style with ``config.yaml``) are handled
...
def _effective_openai_auth_harness(
    ...
    if model is None:
        return None
    from omnicraft.llms.routing import infer_harness_from_model
...
def _should_inject_openai_env_auth_for_executor(
    ...
    # OMNICRAFT_CONFIG_HOME, so skipping injection loses nothing. The env
...
def _apply_overrides_to_raw(raw: _YamlMapping, overrides: ChatOverrides) -> None:
    ...
    # ``_DEFAULT_AD_HOC_MODEL`` directly so ``OMNICRAFT_MODEL=foo``
    # is honored on the ``omnicraft/cli.py`` → ``run_chat`` direct
    # path. Without this, that env var was silently dropped on the
    # OmniCraft path invoked through the ``omnicraft`` console
    # script (see ``designs/RUN_OMNICRAFT_REPL_PARITY.md``).
...
def _apply_harness_override_to_executor(
    ...
    :raises click.ClickException: If a ``spec_version`` bundle
        declares a non-omnicraft ``executor.type`` — those executors
        have no ``config.harness``, so the override cannot apply.
    """
    canonical = canonicalize_harness(harness) or harness
    # "spec_version" is the format discriminator (see is_omnicraft_yaml).
    if "spec_version" not in raw:
        executor_block["harness"] = canonical
        return
    etype = str(executor_block.get("type", OMNICRAFT_EXECUTOR_TYPE))
    if etype != OMNICRAFT_EXECUTOR_TYPE:
        raise click.ClickException(
            f"--harness only applies to specs with executor.type "
            f"{OMNICRAFT_EXECUTOR_TYPE!r}; this spec declares executor.type {etype!r}."
        )
    config = executor_block.get("config")
...
def _validate_agent_spec(agent_path: Path) -> None:
    ...
    ``_start_local_server``). Both ``OmniCraftError`` (parse/
...
def _extract_agent_name(agent_path: Path) -> str:
    ...
    Accepts both agent-image directories and standalone omnicraft
    YAML files. The heavy lifting lives in
    :func:`omnicraft.spec.load`, which dispatches on the source
    ...
    except (OmniCraftError, FileNotFoundError):
...
def _omnicraft_log_dir() -> Path:
    """
    Resolve the shared OmniCraft process log directory.
    ...
    :returns: ``~/.omnicraft/logs``, created if needed.
    """
    log_dir = Path.home() / ".omnicraft" / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    return log_dir


def _omnicraft_persistent_dir() -> Path:
    """
    Resolve the persistent omnicraft data directory.

    Honors ``OMNICRAFT_DATA_DIR`` (the data-isolation knob a worktree sets
    to avoid sharing ``~/.omnicraft/chat.db``), else lives at
    ``~/.omnicraft`` alongside the native paths ``sessions/`` and ``logs/``
    (see designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md). Created on first access;
    subsequent calls are idempotent.

    Must resolve identically to
    :func:`omnicraft.host.local_server._local_data_dir` — the local server
    writes its DB under that dir while ``omnicraft run`` reads the resume DB
    from here, so a divergence would silently lose history. ``OMNICRAFT_CONFIG_HOME``
    ...
    :returns: The absolute path to the persistent dir,
        ...
        subdir.
    """
    override = os.environ.get("OMNICRAFT_DATA_DIR")
    ap_dir = Path(override).expanduser() if override else Path.home() / ".omnicraft"
...
def _start_local_server(
    ...
    *,
    ...
    """
    Launch a local OmniCraft server.

    Server stdout/stderr are routed to ``server.log`` in a
    per-run directory under ``~/.omnicraft/logs`` so concurrent OmniCraft sessions don't
    ...
    ``~/.omnicraft/{chat.db,artifacts/}`` by default —
    persistent across runs so ``--continue`` / ``--resume``
    can resume prior conversations
    (designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md). Pass
    ...
    :param ephemeral: When ``True``, place the SQLite DB and
        ...
        ``~/.omnicraft`` location. Used for ``--no-session``
    ...
    """
    log_dir = _omnicraft_log_dir() / "server"
    log_dir.mkdir(parents=True, exist_ok=True)
    ...
    if ephemeral:
        ...
        data_dir = _omnicraft_persistent_dir()
    ...
    from omnicraft.cli import _start_cli_runner_process
    ...
    # we spawn below (read by server() via OMNICRAFT_RUNNER_TUNNEL_TOKEN).
    #
    # Accounts opt-in: when the parent shell selects accounts mode
    # (OMNICRAFT_AUTH_PROVIDER=accounts), inject the per-spawn
    # base URL + cookie secret so the spawned server's
    # AccountsConfig.from_env() can satisfy its required-fields check.
    # Mirrors the same logic in cli.py:_ensure_local_omnicraft_server; see
    # the comment block there for the full UX explanation. (Two
    # spawn paths exist because `omnicraft run --server ""` goes
    # through _ensure_local_omnicraft_server while `omnicraft run` with
    # an agent spec and no --server flag goes through here.)
    child_env = {
        **os.environ,
        "OMNICRAFT_RUNNER_TUNNEL_TOKEN": binding_token,
        # Single-user loopback runtime — see ensure_local_omnicraft_server for why
        ...
        "OMNICRAFT_LOCAL_SINGLE_USER": "1",
    }
    # Mirror create_auth_provider's resolution so this spawn path agrees
    # with the daemon path (host/local_server.py::ensure_local_omnicraft_server):
    # header is the env-unset default; OMNICRAFT_AUTH_ENABLED=1 opts into
    # accounts (or oidc when OMNICRAFT_OIDC_* is set). In header/oidc mode we
    ...
    if _accounts_mode:
        if "OMNICRAFT_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            child_env["OMNICRAFT_ACCOUNTS_COOKIE_SECRET"] = secrets.token_hex(32)
        ...
        child_env["OMNICRAFT_ACCOUNTS_BASE_URL"] = f"http://127.0.0.1:{port}"
    ...
    # This ensures the OmniCraft server and its runner subprocess resolve credentials
...
def _stop_local_server(server: LocalServer) -> None:
    ...
    """
    from omnicraft.cli import _stop_cli_runner_process
...
def _spec_used_families(agent_yaml: Path | None) -> list[str]:
    ...
    if agent_yaml.is_dir():
        ...
        return []
    try:
        from omnicraft.onboarding.provider_config import PI_SURFACE, harness_family
        from omnicraft.spec import parse
...
def _run_repl(
    ...
    *,
    ...
    :param auto_open_conversation: When ``True``, open the
        browser conversation URL when the session id becomes known.
    """
    from omnicraft.repl import run_repl
    from omnicraft.repl._session_log import DEFAULT_LOG_DIR
    from omnicraft.repl._tmux_pane import register_pane

    log_dir = DEFAULT_LOG_DIR if log else None

    # Mark this tmux pane as an omnicraft context source and wrap
    ...
    async def _main() -> None:
        ...
        from omnicraft.cli_diagnostics import install_asyncio_exception_handler
...
def _run_one_shot(
    *,
    ...
    try:
        ...
        # raises as an OmniCraftError. Surface its message as a clean
...
def _load_tool_handler(name: str) -> ToolHandler:
    ...
    """
    try:
        from omnicraft.client_tools import get_tool_set
    ...
    if hasattr(tool_set, "_TOOL_FNS"):
        ...
        from omnicraft_client.tools import build_tool_handler
