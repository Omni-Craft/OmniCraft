...
:func:`omnicraft.claude_native._claude_terminal_request` (local CLI) and
``_auto_create_claude_terminal`` in :mod:`omnicraft.runner.app` (managed-host
...
:data:`CLAUDE_LAUNCHER_ENTRY_POINT_GROUP` group::
    ...
    [project.entry-points."omnicraft.claude_launcher"]
...
environment the runner runs in -- no OmniCraft code change, no in-tree import
path. At launch time, the ``OMNICRAFT_CLAUDE_LAUNCHER`` environment variable
selects *which* registered launcher to use, by entry-point name (e.g.
``OMNICRAFT_CLAUDE_LAUNCHER=isaac``). Unset -> default launch. The selected
...
the command preserves the OmniCraft bridge unchanged.
...
CLAUDE_LAUNCHER_ENV_VAR = "OMNICRAFT_CLAUDE_LAUNCHER"

#: setuptools entry-point group launcher plugins register themselves in.
CLAUDE_LAUNCHER_ENTRY_POINT_GROUP = "omnicraft.claude_launcher"
...
class ClaudeLauncher(abc.ABC):
    ...
    OmniCraft instantiates the subclass (no-arg constructor) and calls
    ...
    def launch(self, command: str, args: list[str]) -> tuple[str, list[str]]:
        ...
        :param command: Default terminal command OmniCraft would otherwise spawn,
            e.g. ``"claude"``.
        :param args: Fully-augmented Claude CLI args (MCP config, hook settings
            ...
            the OmniCraft bridge.
        :returns: The ``(command, args)`` OmniCraft should spawn instead.
        """
