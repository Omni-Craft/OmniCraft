"""Native Claude Code terminal wrapper for the OmniCraft CLI.

The wrapper deliberately treats Claude Code as a terminal-first
program. It creates or binds an OmniCraft session, launches ``claude``
...
from omnicraft._native_resume_hint import echo_native_resume_hint
from omnicraft._runner_startup import RunnerStartupProgress, runner_startup_progress
from omnicraft._startup_profile import StartupProfiler
from omnicraft._terminal_picker_theme import (
...
from omnicraft.claude_native_forwarder import (
...
from omnicraft.conversation_browser import conversation_url, open_conversation_link_if_enabled
...
from omnicraft.native_coding_agents import native_shell_terminal_spec
from omnicraft.native_terminal import (
...
# other sessions inside the TUI.  OmniCraft owns session switching in its own
...
class PreparedClaudeTerminal:
    ...
    :param cold_resumed: ``True`` when we launched a fresh terminal
        against an existing OmniCraft session (i.e. ``--resume <conv>`` with
    ...
    :param tmux_socket: Runner tmux server socket path when the
        ...
        e.g. ``Path("/tmp/omnicraft-501/.../tmux.sock")``. ``None``
...
def build_native_claude_terminal_env(
    ...
    terminal stays pinned to the session the OmniCraft UI is showing.
...
def run_claude_native(
    *,
    ...
    """
    Launch Claude Code in an OmniCraft terminal and attach locally.

    :param server: Optional remote OmniCraft server URL. ``None`` starts a
        local OmniCraft server using the existing chat server machinery.
    ...
    :param claude_args: Args after ``claude``, e.g.
        ...
        ``-r`` is stripped defensively (OmniCraft owns resume).
    ...
    :param startup_profiler: Optional shared startup profiler from the
        ...
        ``OMNICRAFT_CLAUDE_STARTUP_PROFILE``.
    ...
    # login — so `omnicraft claude` honors the provider selection just like
    ...
    with TemporaryDirectory(prefix="omnicraft-claude-native-") as tmpdir:
...
def _resolve_session_id_for_resume(
    *,
    ...
    if not resume_picker:
        return None
    # Deferred — omnicraft_client / repl pull in heavy graphs we don't want at startup.
    from omnicraft_client import OmniCraftClient

    from omnicraft.repl._resume_picker import pick_conversation_by_wrapper_label_from_sdk
...
def _align_working_directory_with_session(
    ...
    *,
    ...
    :mod:`omnicraft.claude_native_state`); this helper reads it back
    ...
    * A user resuming from a different machine has no recorded
    ...
    :param session_id: Resolved OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
    :param base_url: OmniCraft server base URL used to look up Claude's
...
def _fetch_external_session_id_for_redirect(
    *,
    ...
    :param base_url: OmniCraft server base URL, or ``None`` when unavailable.
    :param headers: HTTP headers for the OmniCraft request.
...
def _record_launch_for_fresh_session(session_id: str) -> None:
    ...
    :param session_id: Newly created OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
    :returns: None.
    """
    try:
...
def _strip_resume_from_claude_args(args: tuple[str, ...]) -> tuple[str, ...]:
    ...
    ``omnicraft claude -- --resume <id>``) must not have it reach
    ...
    for arg in args:
        ...
        if arg in ("--resume", "-r"):
            _logger.warning(
                "Stripped stray %s from claude args; use `omnicraft claude --resume`.", arg
            )
            consume_value = True
            continue
...
def _ucode_config_for_profile(profile: str | None) -> ClaudeNativeUcodeConfig | None:
    ...
    from omnicraft.onboarding.ucode_state import read_ucode_state
    ...
    if agent_state is None:
        raise click.ClickException(
            ...
            "Run `omnicraft setup --internal-beta` to refresh ucode configuration."
        )
...
def _provider_config_for_native_claude(entry: ProviderEntry) -> ClaudeNativeUcodeConfig | None:
    ...
    model — so a Claude Code terminal launched by ``omnicraft`` routes
    through the configured provider exactly like the in-process claude-sdk
    harness does (:func:`omnicraft.runtime.workflow.configure_agent_harness_with_provider`).
    ...
    """
    from omnicraft.onboarding.provider_config import ANTHROPIC_FAMILY
...
def resolve_native_claude_config(
    *,
    ...
    ``omnicraft claude`` and the runner's host-spawned auto-create), so the
    native harness honors ``omnicraft setup`` exactly like the in-process
    claude-sdk harness. Precedence mirrors
    :func:`omnicraft.runtime.workflow._resolve_provider_for_build`:
    ...
    2. when spec-less (``omnicraft claude``): an explicit per-family default
    ...
    ``omnicraft setup`` provider config — there is no CLI/env profile
    override.

    :param spec: The agent spec, or ``None`` for the bare ``omnicraft
        claude`` launch.
    :returns: The launch config, or ``None`` to use Claude's own login.
    """
    ...
    from omnicraft.runtime.workflow import _load_global_auth, _resolve_provider_for_build
    ...
    # 2. Spec-less (omnicraft claude): explicit default wins first.
    ...
    _logger.info(
        ...
        "harness, no Databricks profile). Run `omnicraft setup --no-internal-beta` to route "
        "through a provider."
    )
    return None
...
def _materialize_claude_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first session agent spec used by ``omnicraft claude``.
    ...
    raw = {
        ...
        # ``omnicraft/runner/app.py``. Claude Code already operates
...
def _run_with_local_server(
    ...
    *,
    ...
    """
    Start a local OmniCraft server, launch Claude, and attach to it.
    ...
    startup_profiler = startup_profiler or StartupProfiler(name="omnicraft claude", enabled=False)
    ...
    try:
        ...
        if resolved_session_id is not None:
            ...
            # independent (writes to ``~/.omnicraft/``), so chdiring
            # now is safe.
...
class _AttachOutcome(Enum):
    ...
    :cvar EXITED: The user quit (stdin EOF / Ctrl-D), the terminal is
        ...
        launcher tears down the runner and OmniCraft terminal resource.
...
def _can_attach_direct_tmux(prepared: PreparedClaudeTerminal) -> bool:
    ...
    :func:`omnicraft.codex_native._can_attach_direct_tmux`.
...
async def _attach_direct_tmux(
    ...
    *,
    ...
    environment so a user who runs ``omnicraft claude`` from inside
    their own tmux can still attach to OmniCraft' server. After the
    ``tmux attach`` child exits, a ``has-session`` probe distinguishes a
    user *detach* (session still alive → keep the OmniCraft terminal resource
    ...
    """
    from omnicraft.terminals.ws_bridge import _check_pane_dead_definitive, _tmux_session_alive
...
async def _attach_with_transcript_forwarder(
    *,
    ...
    :param base_url: OmniCraft server base URL.
    :param headers: Static HTTP auth headers for OmniCraft requests. For
    ...
    :param auth: Optional httpx Auth that mints a fresh bearer token
        ...
        transcript forwarder's HTTP client so OmniCraft posts continue to
    ...
    # offset 0 contains turns OmniCraft already has from the previous run).
    ...
    try:
        ...
        if forwarder is not None:
            ...
            try:
                ...
                # uncaught in the parser) must not skip the OmniCraft terminal
        ...
        # the OmniCraft terminal resource must stay live (the web UI keeps
...
async def _attach_with_reconnect(
    *,
    ...
    :param base_url: OmniCraft server URL for the post-close terminal probe;
    ...
    :param active_session_id_reader: Optional callback that returns
        the latest active OmniCraft session id, e.g. ``"conv_new"``. This is
            ...
            if (
                ...
                async def _terminal_gone_probe(
                    *,
                    ...
                    :returns: ``True`` when the OmniCraft terminal resource
...
async def _is_terminal_resource_gone(
    *,
    ...
    :param base_url: OmniCraft server base URL.
    :param headers: HTTP auth headers for the OmniCraft server. Mutated in
...
async def _sleep(seconds: float) -> None:
    ...
    loop — see ``omnicraft-testing`` skill rule 14 for why globally
...
# (``omnicraft host``) rather than this CLI: ensure the daemon, ask
...
async def _wait_for_claude_terminal_ready(
    ...
    *,
    ...
    :param client: HTTP client pointed at the OmniCraft server.
...
async def _prepare_claude_terminal_via_daemon(
    *,
    ...
    :param base_url: OmniCraft server base URL.
    :param headers: Static HTTP auth headers for OmniCraft requests.
    ...
    """
    from omnicraft.claude_native_bridge import bridge_dir_for_conversation_id
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Claude on a remote OmniCraft server via the connect daemon.
    ...
    :param base_url: Remote OmniCraft server base URL without a trailing
    ...
    :returns: None.
    """
    from omnicraft.chat import _bundle_agent, _remote_headers, _server_auth
    from omnicraft.cli import _ensure_host_daemon
    ...
    try:
        ...
        with runner_startup_progress(initial_message="Preparing Claude...") as progress:
            ...
            try:
                ...
                # TCP connection — the OmniCraft server at this URL isn't reachable.
                # Fail loud with the URL instead of a raw httpx traceback.
                raise click.ClickException(
                    f"Could not reach the omnicraft server at {base_url}. "
...
async def _prepare_claude_terminal(
    *,
    ...
    :param base_url: OmniCraft server base URL.
    :param headers: Static HTTP auth headers for the OmniCraft server.
    :param session_id: Optional existing session id.
    ...
    async with httpx.AsyncClient(base_url=base_url, headers=headers, timeout=timeout) as client:
        ...
        # OmniCraft already holds the prior conversation history from the
        # earlier run. The forwarder must not re-read whatever the new
        # transcript file contains at startup and republish it as new
        # OmniCraft events. Both subcases — injected ``--resume <claude_sid>``
...
async def _fetch_claude_session_labels(
    ...
    """
    Fetch labels for an existing Claude-native OmniCraft session.

    :param client: HTTP client for the OmniCraft server.
    :param session_id: OmniCraft conversation id, e.g. ``"conv_abc123"``.
    ...
    if resp.status_code == 404:
        raise click.ClickException(
            ...
            "Run `omnicraft claude` (no --resume) to start a new session.",
        )
    if resp.status_code >= 400:
        raise click.ClickException(
...
async def _resolve_cold_resume_args(
    ...
    if transcript is None:
        ...
        # starting. Launch fresh — the OmniCraft conv survives.
        message = (
...
async def _ensure_local_claude_resume_transcript(
    ...
    *,
    ...
    Cross-machine resume has the OmniCraft conversation and Claude external
    ...
    wrapper always rewrites it from committed OmniCraft items before launch so
    OmniCraft remains the source of truth when a previous local Claude JSONL
    has diverged.

    :param client: HTTP client pointed at the OmniCraft server.
    :param session_id: OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
    ...
    :param workspace: Resolved directory Claude will run in — its
        ...
        ``OMNICRAFT_RUNNER_WORKSPACE``. Pass an already-resolved
    ...
    :returns: Path to the local transcript that was written; ``None`` if
        *external_session_id* is not a safe transcript stem, or if the AP
        ...
        fresh).
    :raises click.ClickException: If OmniCraft history cannot be fetched or
...
def _claude_transcript_records_from_session_items(
    ...
    *,
    ...
    """
    Convert OmniCraft session items into Claude Code transcript records.

    :param items: Flat OmniCraft item dicts in chronological order, e.g.
...
def _claude_transcript_record_from_session_item(
    ...
    *,
    ...
    """
    Convert one OmniCraft item into one Claude transcript record.

    :param item: Flat OmniCraft item dict, e.g.
...
def _synthetic_claude_transcript_uuid(
    *,
    ...
    :param external_session_id: Claude-native session id, e.g.
        ``"02857840-6362-408f-b41f-309e396ed7c6"``.
    :param item: OmniCraft item dict. ``id`` is used when present.
    ...
    return str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"omnicraft-claude-resume:{session_id}:{external_session_id}:{stable_item_id}",
        )
    )
...
def _claude_user_content_from_api_blocks(content: object) -> str | list[dict[str, Any]] | None:
    """
    Convert OmniCraft user message blocks into Claude message content.

    :param content: OmniCraft ``content`` value, e.g.
...
def _claude_assistant_content_from_api_blocks(content: object) -> list[dict[str, Any]] | None:
    """
    Convert OmniCraft assistant message blocks into Claude text blocks.
...
def _claude_text_blocks_from_api_content(
    ...
    *,
    ...
    """
    Extract text blocks from an OmniCraft content array.

    :param content: OmniCraft content array, e.g.
        ``[{"type": "input_text", "text": "hello"}]``.
    :param api_type: OmniCraft block type to include, e.g.
...
def _json_object_from_string(value: object) -> dict[str, Any]:
    ...
    :param value: JSON string from an OmniCraft function-call item, e.g.
        ``"{\"file_path\":\"README.md\"}"``.
...
def _preflight_local_tools(command: str) -> None:
    ...
    if shutil.which(command) is None:
        raise click.ClickException(
            ...
            "--server selects the OmniCraft server only; Claude still runs locally."
        )
...
async def _create_claude_session(
    ...
    *,
    ...
    ``omnicraft.wrapper = claude-code-native-ui`` label until the
...
async def _launch_claude_terminal(
    ...
    *,
    ...
    :param client: HTTP client pointed at the OmniCraft server. Its
        ``base_url`` and ``headers`` are reused as the
        ``PermissionRequest`` command hook's OmniCraft URL and auth. The hook
...
class _ClaudeTerminalTmux:
    ...
    :param socket: tmux server socket path the runner advertised in
        ...
        ``Path("/tmp/omnicraft-501/.../tmux.sock")``. ``None`` when
        absent.
...
def _claude_terminal_request(
    ...
    *,
    ...
    # default. See omnicraft.claude_launcher.
    ...
    spec: dict[str, Any] = {
        ...
        # deployments; ``omnicraft claude`` is a local-only
...
async def attach_local_terminal(
    ...
    *,
    ...
    """
    Attach the local TTY to an OmniCraft terminal WebSocket.
    ...
    :param terminal_gone_probe: Optional async callback returning
        ``True`` once the OmniCraft terminal resource is stopped. When set,
...
async def _close_ws_when_terminal_gone(
    ...
    *,
    ...
    """
    Close the client WebSocket when the OmniCraft terminal resource stops.
...
def _websocket_connect(attach_url: str, *, headers: dict[str, str]) -> Any:
    ...
    handshake_headers = {**headers, "Origin": OMNICRAFT_INTERNAL_WS_ORIGIN}
    try:
...
def claude_terminal_resource_id() -> str:
    """
    Return the deterministic terminal id used by ``omnicraft claude``.
