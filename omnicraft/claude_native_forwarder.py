...
from omnicraft.reasoning_effort import CLAUDE_EFFORTS, EFFORT_CLEAR_VALUES
...
# Claude Code hook event names → OmniCraft session-status values
...
class _ForwardHealth:
    """
    Process-level health of OmniCraft transcript/usage forwarding (#1120).
...
def _note_forward_failure(retry_key: str) -> None:
    ...
    if (
        ...
        _logger.error(
            "claude-native forward sync degraded: %d consecutive OmniCraft "
...
class CompactionForwardState:
    ...
    both reconcile against one durable token so exactly one OmniCraft
...
class SubagentEntry:
    ...
    One of these per Claude-side sub-agent. Tracks the OmniCraft child
    ...
    :param child_conversation_id: OmniCraft child Conversation id minted
...
class SubagentForwardState:
    ...
    :param subagents: Map from Claude-side ``subagent_id`` to the
        ...
        inserted here after the OmniCraft server returns a child
        Conversation id.
    """
...
class DeltaForwardState:
    ...
    Claude process and keeps growing across OmniCraft session rotations, so the
    offset stays monotonic and each new chunk is forwarded to whatever
    OmniCraft session is active when it is read.
...
class _PostRetryEntry:
    """
    In-memory retry state for one outbound OmniCraft event.
...
class _PostRetryDecision:
    """
    Result of recording one outbound OmniCraft post failure.
...
class _PostRetryTracker:
    """
    Track bounded retries and backoff for OmniCraft event posts.
    ...
    backoff so OmniCraft outages do not silently drop transcript data.
...
async def forward_claude_transcript_to_session(
    *,
    ...
    :param headers: Static HTTP headers for OmniCraft requests. Authorization
...
def _parse_json_response(resp: httpx.Response, *, context: str) -> Any:
    """
    Parse an OmniCraft JSON response, failing loudly on a non-JSON body.
...
async def _post_external_subagent_start(
    ...
    *,
    ...
    """
    POST ``external_subagent_start`` to the OmniCraft server and return the
    ...
    :returns: The OmniCraft child conversation id, e.g. ``"conv_child456"``.
    :raises httpx.HTTPError: If the OmniCraft request fails or is rejected.
...
async def _forward_available_subagents(
    *,
    ...
    """
    Discover new Claude Task-tool sub-agents on disk, mint OmniCraft child
    ...
    ``subagent_forwarder.json``) holds the OmniCraft child id and byte
...
async def _maybe_rotate_session_on_clear(
    *,
    ...
    """
    Rotate the active OmniCraft session when Claude reports ``/clear``.

    :param client: OmniCraft HTTP client.
    :param session_id: Currently active OmniCraft session id, e.g.
        ``"conv_old"``.
    :param bridge_dir: Native Claude bridge directory.
...
async def _seed_fork_transcript_forward_state(
    *,
    ...
    """
    Seed transcript forwarding after OmniCraft has forked history.

    Claude fork transcripts start with copied source-session records.
    The OmniCraft fork endpoint has already copied those conversation items,
...
async def _create_clear_replacement_session(
    *,
    ...
    """
    Create the fresh OmniCraft session for a Claude ``/clear`` event.
    ...
    :raises httpx.HTTPError: If OmniCraft rejects session creation, new-session
    ...
    clear_resp = await client.patch(
        ...
        json={
            ...
            # ``omnicraft claude --resume``) would put a second forwarder on the
...
async def _maybe_rotate_session_on_fork(
    *,
    ...
    """
    Fork the active OmniCraft session when Claude reports ``/fork``/``/branch``.
...
async def _create_fork_replacement_session(
    *,
    ...
    """
    Create the forked OmniCraft session for a Claude ``/fork``/``/branch``.
    ...
    :raises RuntimeError: If the OmniCraft fork response is malformed.
    """
    old = await _fetch_session_snapshot(client, old_session_id)
    runner_id = old.get("runner_id")
...
def _is_fork_hook_record(record: ClaudeHookRecord) -> bool:
    ...
    completed the OmniCraft fork.

    :param record: Claude hook record read from hooks.jsonl.
    :returns: ``True`` when the active OmniCraft session should be forked.
    """
...
async def _fetch_session_snapshot(
    ...
    """
    Fetch one OmniCraft session snapshot.

    :param client: OmniCraft HTTP client.
    :param session_id: OmniCraft session id, e.g. ``"conv_abc123"``.
    :returns: Parsed JSON snapshot.
    :raises httpx.HTTPError: If OmniCraft returns a non-2xx status.
...
async def _maybe_mirror_external_session_id(
    *,
    ...
    """
    Mirror Claude's native session id onto the OmniCraft conversation row.

    Reads the latest captured Claude-native session id from the
    bridge state file and, if present, PATCHes
    ``external_session_id`` on the OmniCraft conversation. Best-effort: a
    ...
    try:
        ...
        if 400 <= exc.response.status_code < 500:
            ...
            return True
        _logger.warning(
            "Transient OmniCraft error PATCHing external_session_id (%s); session=%s — will retry",
            exc.response.status_code,
            session_id,
        )
        return False
...
async def _handle_compact_summary_item(
    ...
    *,
    ...
    persists exactly one OmniCraft ``compaction`` boundary carrying the
...
async def _forward_available_items(
    *,
    ...
    for item in items:
        ...
        # Persist a durable OmniCraft ``compaction`` boundary here (never
    ...
    # statusLine stdin captured by ``omnicraft.claude_native_status``
...
async def _post_clear_supersession(
    ...
    *,
    ...
    :param client: OmniCraft HTTP client (``base_url`` = AP server).
    ...
    *,
    ...
    with (
        ...
        telemetry.span(
            ...
            attributes={"omnicraft.response_id": item.response_id},
        ),
    ):
        resp = await client.post(
...
async def _forward_available_deltas(
    *,
    ...
    :param client: OmniCraft HTTP client.
    :param session_id: OmniCraft session/conversation id deltas are forwarded
...
def _model_alias_for(model: str | None) -> str | None:
    ...
    :data:`omnicraft.claude_native._UCODE_CLAUDE_CUSTOM_TIER`) for the newer
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror model change to OmniCraft session=%s; model pill / "
...
async def _forward_model_from_status(
    ...
    *,
    ...
    wrapper (:mod:`omnicraft.claude_native_status`) persists that model into
...
async def _post_external_compaction_status(
    ...
    *,
    ...
    ``SessionStart`` (``source == "compact"``) hook. The OmniCraft server maps
...
async def _patch_external_session_id(
    ...
    *,
    ...
    """
    PATCH the OmniCraft conversation row with the Claude-native session id.
...
async def _maybe_sync_effort_from_slash_command(
    ...
    *,
    ...
    """
    Mirror an in-pane ``/effort`` change onto the OmniCraft session row.
    ...
    try:
        ...
        _logger.warning(
            "Failed to mirror in-pane /effort=%s to OmniCraft session=%s; "
...
def _is_permanent_http_error(exc: httpx.HTTPError) -> bool:
    """
    Return whether ``exc`` is a permanent OmniCraft rejection.

    :param exc: HTTP exception raised while posting an OmniCraft event.
...
def _usage_from_status_state(state: dict[str, Any]) -> dict[str, float] | None:
    """
    Convert statusLine ``current_usage`` (+ cost) into the OmniCraft usage shape.
