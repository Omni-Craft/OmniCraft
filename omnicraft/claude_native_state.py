"""Persistent client-side state for ``omnicraft claude`` sessions.
...
Why under ``~/.omnicraft/`` and not the existing bridge dir at
``/tmp/omnicraft-<uid>/claude-native/``: tmpfs gets cleared on
...
the day after creating it still gets the chdir prompt. ``~/.omnicraft/``
is where the persistent OmniCraft server SQLite db and other durable
single-user state already live.

Layout (per conversation):

    ~/.omnicraft/claude-native/<sha256(conv_id)[:32]>/launch.json

The directory is hashed (not the raw conv id) so a malicious server
returning an attacker-chosen conversation id like ``"../../../etc"``
cannot escape ``~/.omnicraft/claude-native/``. A short hash is enough
...
# ``~/.omnicraft/claude-native``. Tests should set this to a per-
# test ``tmp_path`` via monkeypatch.setenv so they never touch the
# user's real home directory.
_STATE_ROOT_ENV_VAR = "OMNICRAFT_CLAUDE_NATIVE_STATE_DIR"
...
# :mod:`omnicraft.claude_native_bridge`.
_ID_HASH_CHARS = 32
...
def _claude_native_state_root() -> Path:
    ...
    directories on disk). Lives under ``~/.omnicraft/claude-native/``
    ...
    return Path.home() / ".omnicraft" / "claude-native"
...
def read_launch_state(conversation_id: str) -> ClaudeNativeLaunchState | None:
    ...
    * the picker, to render the Workspace column for each row.
    ...
    user who wiped ``~/.omnicraft/claude-native/``). The callers
