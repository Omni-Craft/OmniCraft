...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m omnicraft.claude_native_status")
    ...
    # status bar still renders for omnicraft-launched sessions.
...
def _write_context_atomic(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    # event, so the OmniCraft relay's cost accumulation never runs for it).
    ...
    try:
        ...
        print(f"omnicraft claude status: write failed: {exc}", file=sys.stderr)
...
def _chain(command: str, stdin_payload: str) -> None:
    ...
    try:
        ...
        print(f"omnicraft claude status: chain failed: {exc}", file=sys.stderr)
        return
    if proc.stdout:
