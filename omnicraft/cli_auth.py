"""CLI-side auth storage for ``omnicraft login``.

Persists per-server auth state in ``~/.omnicraft/auth_tokens.json``
...
- **Databricks Apps pointer records**
  ...
  ``omnicraft login <apps-url>``. These deliberately store NO token:
...
def _token_file_path() -> Path:
    ...
    Uses the shared ``~/.omnicraft`` state directory.

    :returns: Path to ``~/.omnicraft/auth_tokens.json``.
    """
    from omnicraft_ui_sdk.terminal._config import state_dir
...
def _store_entry(server_url: str, entry: dict[str, str | float]) -> None:
    ...
    Writes ``~/.omnicraft/auth_tokens.json`` with user-only
...
DATABRICKS_EXTRA_HEADERS_ENV_VAR = "OMNICRAFT_DATABRICKS_EXTRA_HEADERS"
...
def databricks_request_headers(
    ...
    when ``omnicraft login https://<host>/?o=<id>`` recorded a selector, and
