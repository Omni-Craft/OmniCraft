...
log file under ``~/.omnicraft/logs/cli-*.log``. Separate from the
...
class CliLogContext:
    ...
    :param path: Absolute path to the current invocation's log file,
        e.g. ``~/.omnicraft/logs/cli-20260518-143012-12345-a1b2c3.log``.
...
def _log_dir() -> Path:
    ...
    Uses :func:`omnicraft_ui_sdk.state_dir` as the shared
    ``~/.omnicraft`` root so the path is defined in one place.

    :returns: ``~/.omnicraft/logs``.
    """
...
def setup_cli_logging(argv: list[str]) -> CliLogContext:
    ...
    installs the redaction filter, wires up the ``omnicraft`` and
    ``omnicraft_ui_sdk`` logger hierarchies, and prunes old log
    files beyond :data:`MAX_LOG_FILES`.

    Call as early as possible in :func:`omnicraft.cli.main` —
    ...
    # the file handler.
    for name in ("omnicraft", "omnicraft_ui_sdk"):
    ...
    root = logging.getLogger("omnicraft.cli_diagnostics")
...
def install_asyncio_exception_handler(loop: asyncio.AbstractEventLoop) -> None:
    ...
    :param loop: The running event loop (typically from
        ...
        context).
    """
    logger = logging.getLogger("omnicraft.asyncio")
...
def print_setup_hint() -> None:
    ...
    Used by the top-level :func:`omnicraft.cli.main` exception
    ...
    ``omnicraft setup`` keeps the recovery path obvious without
    ...
    print(
        ...
        "`omnicraft setup` to configure a model credential.",
        file=dest,
    )
...
def log_cli_exception(exc: BaseException, *, prefix: str = "CLI error") -> None:
    ...
    if current_cli_log_path() is None:
        return
    logging.getLogger("omnicraft.cli_diagnostics").error(
        "%s: %s",
        prefix,
        exc,
...
def _safe_mtime(path: Path) -> float:
    ...
    ``_prune_old_logs`` runs at the start of every ``omnicraft run``, so two
