"""
Async-dispatch demo tool set for ``omnicraft run --tools async_demo``.
...
Registered via :func:`omnicraft.client_tools.get_tool_set`,
which ``omnicraft run`` calls when ``--tools async_demo`` is
passed.
"""
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    """
    Execute a client-side tool call from ``omnicraft run``.
