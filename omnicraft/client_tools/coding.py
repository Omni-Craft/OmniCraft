...
functions and surfaced through the ``omnicraft_client``
...
(``examples/frontends/terminal.py``, ``omnicraft chat``'s raw-schema
path) keep working without modification.

Used by ``omnicraft chat --tools coding`` and the terminal TUI.
"""
...
from omnicraft_client.tools import build_tool_handler, tool
...
# ``omnicraft chat`` ``_load_tool_handler`` path reach into modules
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    ...
    Used by ``examples/frontends/terminal.py`` and ``omnicraft chat``'s
    raw-schema path. New consumers should construct a
    :class:`~omnicraft_client.tools.ToolHandler` via
    :func:`~omnicraft_client.tools.build_tool_handler` against
