...
from omnicraft.codex_native_bridge import write_policy_hook_config
...
# OmniCraft policy hook. Codex discovers it as a ``user``-layer hook
...
# might declare.
_POLICY_HOOK_MODULE = "omnicraft.codex_native_hook"
...
def _toml_table_header_name(line: str) -> str | None:
    ...
    :param line: One config line, e.g.
        ``"[mcp_servers.omnicraft] # generated\n"``.
    :returns: The table name, e.g. ``"mcp_servers.omnicraft"``, or
...
def _remove_toml_table(text: str, table_name: str) -> str:
    ...
    OmniCraft MCP server table. This avoids accumulating duplicate
    ``[mcp_servers.omnicraft]`` sections across terminal relaunches.
...
def _codex_mcp_server_config_section(
    ...
    :param python_executable: Python executable for serve-mcp, e.g.
        ...
        :data:`sys.executable`.
    :returns: TOML text for ``[mcp_servers.omnicraft]``.
    """
    ...
    return f"[mcp_servers.omnicraft]\ncommand = {json.dumps(python)}\nargs = [{args_toml}]\n"
...
def _inject_mcp_server_config(
    ...
    """
    Upsert OmniCraft MCP server config into ``config.toml``.

    Writes a ``[mcp_servers.omnicraft]`` section that points Codex
    ...
    if config_path.exists():
        ...
        existing = ""
    updated = _remove_toml_table(existing, "mcp_servers.omnicraft")
...
class CodexAppServerClient:
    ...
    :param client_name: App-server client name for the initialize
        handshake, e.g. ``"omnicraft-codex-native"``.
    """
    ...
    async def respond(self, request_id: int | str, result: CodexParams) -> None:
        ...
        The forwarder handles those requests through OmniCraft and replies
...
class CodexNativeAppServer:
    ...
    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.omnicraft/codex-native/<hash>")``. The policy hook
        subprocess is pointed at it via ``--bridge-dir`` and reads the
        session id + OmniCraft coordinates from it.
    :param ap_server_url: OmniCraft server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        and trusts the hook but writes no OmniCraft coordinates, so the hook
        no-ops (no enforcement) until coordinates exist.
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        OmniCraft requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    ...
    :param policy_notice_pending: One-shot flag: ``True`` once a degrade
        ...
        surfaces it to OmniCraft (which posts a single durable banner). Prevents
    ...
    async def start(self) -> None:
        ...
        if codex_version is not None and codex_version < _MIN_POLICY_HOOK_CODEX_VERSION:
            ...
            # Register the OmniCraft policy hook in this private CODEX_HOME
            # *before* launching the app-server so codex discovers it at
            # config load. The OmniCraft coordinates the hook subprocess
    ...
    async def _trust_policy_hooks(self) -> None:
        """
        Mark the registered OmniCraft policy hook as trusted.
        ...
        if self.listen_url and self.listen_url.startswith("ws://"):
            client = CodexAppServerClient(
                ...
                client_name="omnicraft-policy-trust",
            )
        else:
            client = CodexAppServerClient(
                self.socket_path,
    ...
    def _warn_policy_hook_disabled(self) -> None:
        ...
        :attr:`policy_hook_disabled_reason`. When OmniCraft coordinates are
...
def _codex_policy_hook_command(bridge_dir: Path, python_executable: str | None) -> str:
    ...
    :returns: A shell-escaped command string, e.g.
        ``"/path/python -m omnicraft.codex_native_hook evaluate-policy
        --bridge-dir /home/u/.omnicraft/codex-native/abc"``.
    """
    python = python_executable or sys.executable
    return shlex.join(
...
def _our_policy_hooks_from_list(listed: dict[str, Any], cwd: str) -> list[dict[str, Any]]:
    ...
    :param cwd: The cwd whose hook set to read, e.g.
        ``"/home/user/repo"``.
    :returns: The matching OmniCraft hook metadata dicts (possibly
...
def _untrusted_hook_detail(hooks: list[dict[str, Any]]) -> str:
    ...
    :param hooks: Untrusted OmniCraft hook metadata dicts from
        ``hooks/list``.
...
async def trust_native_policy_hooks(client: CodexAppServerClient, *, cwd: str) -> None:
    """
    Trust the OmniCraft policy hook so codex actually runs it.
    ...
    verifies every OmniCraft hook ended ``trusted``/``managed``.

    :param client: A connected Codex app-server client.
    :param cwd: The session cwd the hooks are scoped to, e.g.
        ``"/home/user/repo"``.
    :returns: None.
    :raises RuntimeError: If no OmniCraft hook is discovered (it was
    ...
    if not ours:
        raise RuntimeError(
            f"OmniCraft policy hook was not discovered for cwd {cwd!r}; "
    ...
    if still_untrusted:
        ...
        raise RuntimeError(
            "OmniCraft policy hook still untrusted after config/batchWrite; "
...
def build_codex_native_server(
    *,
    ...
    :param bridge_dir: Native Codex bridge directory; the policy hook is
        pointed at it and reads the session id + OmniCraft coordinates from it.
    :param ap_server_url: OmniCraft server base URL the policy hook POSTs tool
        calls to, e.g. ``"http://127.0.0.1:8787"``. ``None`` registers
        the hook but writes no OmniCraft coordinates (hook no-ops).
    :param ap_auth_headers: Outbound auth headers for the policy hook's
        OmniCraft requests, e.g. ``{"Authorization": "Bearer <token>"}``.
    :param python_executable: Python executable the policy hook command
    ...
    :param extra_config_overrides: Additional ``-c`` config overrides
        ...
        registration for the OmniCraft tool relay.
    ...
    if not resolved_codex:
        raise ImportError(
            ...
            "nvm-managed bin dir), set OMNICRAFT_CODEX_PATH=/path/to/codex."
        )
...
def codex_session_meta_model_provider(launch: NativeCodexLaunch) -> str:
    ...
    - a Databricks profile launch carries no override here; the provider
      ...
      ``omnicraft_databricks`` id (see ``_databricks_codex_config_overrides``);
...
def _resolve_subscription_launch(
    ...
    """
    from omnicraft.onboarding.ambient import codex_auth_has_credential
...
def resolve_native_codex_launch(*, model: str | None) -> NativeCodexLaunch:
    ...
    (:func:`omnicraft.runtime.workflow._resolve_provider_for_build`) for the
    ``openai`` surface, so ``omnicraft codex`` and a host-spawned native
    Codex session route through ``omnicraft setup``:
    ...
    Credentials are controlled exclusively by ``omnicraft setup``
    ...
    if entry is None:
        ...
        return NativeCodexLaunch(
            ...
            summary=(
                ...
                "screen and never starts a thread; run `omnicraft setup` to route through "
                "a provider"
            ),
        )
...
def client_for_transport(
    ...
    *,
    ...
    :param transport: App-server transport from bridge state, e.g.
        ...
        ``"/home/u/.omnicraft/codex-native/x/app-server.sock"``.
    :param client_name: App-server initialize-handshake client name,
        e.g. ``"omnicraft-codex-native"``.
...
async def preload_codex_thread_for_resume(transport: str, thread_id: str) -> None:
    ...
    client = client_for_transport(
        transport,
        client_name="omnicraft-codex-native-preload",
    )
    await client.connect()
    try:
        await client.request(
...
def build_codex_remote_args(
    ...
    *separate* process that loads its own config from ``CODEX_HOME`` and
    ...
    through makes the TUI resolve the OmniCraft provider
    ...
    :param remote_url: App-server endpoint the TUI attaches to, e.g.
        ``"unix:///home/user/.omnicraft/codex-native/x/app-server.sock"``
        or ``"ws://127.0.0.1:9876"``.
    :param config_overrides: Codex ``-c`` config override values to apply
        to the TUI, e.g.
        ``('model="databricks-gpt-5-5"', 'model_provider="omnicraft_databricks"')``.
