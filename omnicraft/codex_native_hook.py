"""Codex Code hook entrypoint for native OmniCraft policy enforcement.

Registered as the ``PreToolUse`` / ``PostToolUse`` command hook in the
per-session private ``CODEX_HOME`` (see
:mod:`omnicraft.codex_native_app_server`). Codex spawns this module as
...
the OmniCraft policy schema is shared with the Claude-native hook via
:mod:`omnicraft.native_policy_hook`.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"omnicraft codex hook: unknown subcommand {raw_argv[:1]!r}",
        file=sys.stderr,
    )
    return 0
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    :func:`omnicraft.native_policy_hook.hook_payload_to_evaluation_request`,
    ...
    return exit 0 with no output ("no opinion") so non-OmniCraft tool calls
    ...
    :mod:`omnicraft.codex_native_app_server`, not here, because a
    ...
    try:
        ...
        print(f"omnicraft codex evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("omnicraft codex evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if config is None:
        # No OmniCraft server configured for this session — nothing to enforce.
        return 0
    ...
    if eval_request is None:
        # Unrecognized hook event or an mcp__omnicraft__* tool (relay-enforced).
        return 0
    ...
    if not resp.content:
        print("omnicraft codex evaluate-policy hook: empty OmniCraft response", file=sys.stderr)
        return _fail_closed()
    ...
    try:
        ...
        print(
            "omnicraft codex evaluate-policy hook: malformed OmniCraft response",
            file=sys.stderr,
        )
        return _fail_closed()
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m omnicraft.codex_native_hook evaluate-policy")
