"""Persistent client-side state for ``omnicraft codex`` sessions.

The native Codex wrapper records the cwd used to create a session so a
later ``omnicraft codex --resume <conv_id>`` can launch Codex from the
...
on the shared OmniCraft server.

Layout (per conversation):

    ~/.omnicraft/codex-native/<sha256(conv_id)[:32]>/launch.json
"""
...
_STATE_ROOT_ENV_VAR = "OMNICRAFT_CODEX_NATIVE_STATE_DIR"
...
def _codex_native_state_root() -> Path:
    ...
    setups. Production defaults to ``~/.omnicraft/codex-native``.
    ...
    return Path.home() / ".omnicraft" / "codex-native"
