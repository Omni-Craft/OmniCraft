"""Helpers for opening OmniCraft conversation URLs from CLI frontends."""
...
# Databricks workspace-hosted omnicraft: the API proxy and the web UI are
...
WORKSPACE_API_PATH = "/api/2.0/omnicraft"
WORKSPACE_UI_PATH = "/omnicraft"


def is_workspace_hosted_url(base_url: str) -> bool:
    """
    Whether *base_url* is a Databricks workspace-hosted OmniCraft mount.

    True for the API proxy mount (``https://<ws>/api/2.0/omnicraft``) the
...
def display_server_url(base_url: str) -> str:
    """
    Map an OmniCraft server base URL to the user-facing form to show.

    Databricks workspace-hosted servers are connected to on the API proxy
    mount (``https://<ws>/api/2.0/omnicraft``), but the URL a user
    recognizes — and that the web UI lives on — is the workspace SPA mount
    (``https://<ws>/omnicraft``). Rewrites the API path to the UI path for
    those (dropping any ``?o=<org>`` query), so the startup banner shows
    the clean ``/omnicraft`` URL instead of the internal API path. Every
    other URL (local ``http://127.0.0.1:<port>``, a custom remote) is
    returned unchanged apart from a trailing-slash trim.

    :param base_url: OmniCraft server base URL, e.g.
        ``"https://example.databricks.com/api/2.0/omnicraft"`` or
        ``"http://127.0.0.1:6767"``.
    :returns: The display URL, e.g.
        ``"https://example.databricks.com/omnicraft"`` or
        ``"http://127.0.0.1:6767"``.
    """
...
def conversation_url(base_url: str, conversation_id: str) -> str:
    """
    Build the browser URL for an OmniCraft conversation.

    For Databricks workspace-hosted servers
    (``https://<ws>/api/2.0/omnicraft``) the web UI lives on the
    workspace SPA mount, so the link becomes
    ``https://<ws>/omnicraft/c/<id>`` — with the ``?o=<org>``
    workspace selector appended when ``omnicraft login`` recorded the
    org id.
