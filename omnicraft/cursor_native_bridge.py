...
executor then delivers OmniCraft web-UI messages into the *same* pane via
...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"omnicraft-{stable_user_id()}" / "cursor-native"
...
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "omnicraft-cursor-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/omnicraft-<uid>/cursor-native/<hash>``."""
...
#: when mirroring the user turn back to OmniCraft so the copied history isn't
#: duplicated in the timeline (see cursor_native_forwarder._unwrap_user_query).
FORK_HISTORY_OPEN_TAG = "<omnicraft_fork_history>"
FORK_HISTORY_CLOSE_TAG = "</omnicraft_fork_history>"
...
def _neutralize_fork_sentinels(text: str) -> str:
    ...
    could literally contain ``<omnicraft_fork_history>`` /
    ``</omnicraft_fork_history>`` (a user typed it, pasted logs, etc.). If left
    ...
    """
    return text.replace(FORK_HISTORY_OPEN_TAG, "[omnicraft_fork_history]").replace(
        FORK_HISTORY_CLOSE_TAG, "[/omnicraft_fork_history]"
    )
...
def wrap_fork_preamble(preamble: str, user_text: str) -> str:
    ...
    duplicated in the OmniCraft timeline.
...
def build_mcp_config(
    ...
    *,
    ...
    """Build Cursor's ``.cursor/mcp.json`` for the OmniCraft relay server.

    Cursor prompts for MCP tool approval before it sends ``tools/call`` to the
    server. OmniCraft tools already route through the OmniCraft ``/mcp`` proxy,
    ...
    hidden in-terminal approval prompt blocking the call before OmniCraft ever
    sees it, while preserving OmniCraft's own policy/elicitation gate.
    """
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config required by the shared OmniCraft MCP bridge."""
    _ensure_dir(bridge_dir)
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped Cursor MCP config for OmniCraft tools."""
...
def build_hooks_config(
    ...
    ``omnicraft.cursor_native_usage record-usage``, which appends the usage to
...
def approve_mcp_server_for_workspace(workspace: Path) -> None:
    """Approve the workspace-scoped OmniCraft MCP server in Cursor's state.
    ...
    ask ``cursor-agent mcp enable omnicraft`` to write the exact approval entry
...
def enable_mcp_for_workspace(workspace: Path) -> None:
    """Ensure Cursor does not keep the OmniCraft MCP disabled for this workspace."""
    disabled_path = (
...
def allow_mcp_tools_in_cli_config() -> None:
    """Allow OmniCraft MCP tool calls in Cursor's CLI permission config."""
...
def capture_cursor_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`omnicraft.cursor_native_permissions`) to detect cursor-agent's native
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`omnicraft.claude_native_bridge.kill_session`.
