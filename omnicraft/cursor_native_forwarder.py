...
The ``omnicraft cursor`` wrapper launches the real ``cursor-agent`` TUI in a
runner-owned tmux pane, and :mod:`omnicraft.cursor_native_bridge` injects web-UI
messages into it. That covers the web→TUI direction, but the *embedded terminal*
is then the only surface that reflects the agent's work — the OmniCraft
...
:mod:`omnicraft.claude_native_forwarder` (which tails Claude Code's JSONL
transcript) and :mod:`omnicraft.codex_native_forwarder` (which subscribes to the
...
cursor-native (see ``_publish_turn_status`` in :mod:`omnicraft.runner.app`),
...
:mod:`omnicraft.cursor_native_status`) — the SAME server contract
...
from omnicraft import cursor_native_status
from omnicraft._native_post_delivery import post_may_have_been_delivered
from omnicraft.cursor_native_bridge import FORK_HISTORY_CLOSE_TAG, FORK_HISTORY_OPEN_TAG
...
#: ``omnicraft.db.db_models.SqlConversationItem``). cursor's content-address blob
...
# first user message, fenced in <omnicraft_fork_history>…</omnicraft_fork_history>
...
# real text — the copied history already lives in the OmniCraft timeline, so
...
def _read_last_used_model(store_path: Path) -> str | None:
    ...
    catalog (:func:`omnicraft.cursor_native.cursor_base_model_options`) and the
    ...
    :func:`omnicraft.cursor_native_bridge.inject_model_command`).
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    un-pinned session falls back to omnicraft's default (e.g. "fable") in the Web
...
async def _patch_external_session_id(
    ...
    """PATCH the OmniCraft session with the cursor chat id for cold-resume.
...
async def _post_external_session_status(
    ...
    (:func:`omnicraft.cursor_native_status.record_turn_end`); this turns that
...
async def _post_external_compaction_status(
    ...
    :func:`omnicraft.claude_native_forwarder._post_external_compaction_status`.
...
async def supervise_cursor_forwarder(
    *,
    ...
    Mirrors :func:`omnicraft.claude_native_forwarder.supervise_forwarder`: the
