...
the SQLite chat store (tailed by :mod:`omnicraft.cursor_native_forwarder`) and
...
:func:`omnicraft.cursor_native_bridge.write_hooks_config`) whose command runs
...
def _cli_record_usage(bridge_dir: Path) -> int:
    ...
    (:func:`omnicraft.cursor_native_status.record_turn_end`) on EVERY firing — even
...
async def forward_cursor_usage_to_session(
    *,
    ...
    from omnicraft._native_post_delivery import post_external_session_status
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="omnicraft.cursor_native_usage")
