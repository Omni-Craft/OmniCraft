"""SQLAlchemy table definitions for the omnicraft database."""
...
from omnicraft.db.compression import CompressedText
...
class Base(DeclarativeBase):
    """Shared declarative base for all omnicraft tables."""
...
_current_workspace_id: ContextVar[int] = ContextVar(
    "omnicraft_workspace_id", default=DEFAULT_WORKSPACE_ID
)
...
class SqlAgent(Base):
    ...
    # Enum stored as a stable int code (see omnicraft.db.enum_codecs
...
class SqlConversationLabel(Base):
    ...
    :param updated_at: Unix epoch seconds of the last write.
        ...
        unchanged (matches omnicraft parity and keeps
...
class SqlPolicy(Base):
    ...
    # omnicraft.db.enum_codecs POLICY_TYPE: python=1, url=2).
    type: Mapped[int] = mapped_column(SmallInteger)
    ...
    # code (see omnicraft.db.enum_codecs POLICY_SCOPE: default=1, session=2).
...
class SqlHost(Base):
    ...
    :param created_at: Unix epoch seconds when the host was first
        registered (first ``omnicraft host``).
