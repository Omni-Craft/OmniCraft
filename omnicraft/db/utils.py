...
from omnicraft.entities import NewConversationItem
...
# resolvable (``OMNICRAFT_LAKEBASE_INSTANCE`` is set, or a provider was injected
...
# token-refresh mode.
_LAKEBASE_INSTANCE_ENV = "OMNICRAFT_LAKEBASE_INSTANCE"
...
def set_lakebase_token_provider(provider: LakebaseTokenProvider | None) -> None:
    ...
    *provider* once per new DBAPI connection, and uses the shorter
    ...
    fall back to the ``OMNICRAFT_LAKEBASE_INSTANCE`` env-var path.
...
def _databricks_lakebase_token_provider(instance_name: str) -> str:
    ...
    :param instance_name: The Lakebase database instance name, e.g.
        ``"omnicraft-db"`` (the value of ``OMNICRAFT_LAKEBASE_INSTANCE``).
...
def _create_engine(db_uri: str) -> Engine:
    ...
    multi-process workloads — REPL + OmniCraft server + runner subprocess
    ...
    # ``None`` unless ``OMNICRAFT_LAKEBASE_INSTANCE`` is set or a provider was
    ...
    if db_uri not in _engine_cache:
        with _engine_lock:
            if db_uri not in _engine_cache:
                ...
                from omnicraft.runtime.telemetry import instrument_sqlalchemy_engine
...
def _build_alembic_config(db_uri: str) -> Config:
    ...
    :func:`omnicraft debug db-upgrade` CLI command share the same
...
def _run_migrations(engine: Engine, db_uri: str) -> None:
    ...
    from omnicraft.db.db_models import Base
...
def _initialize_or_verify_schema(engine: Engine, db_uri: str) -> None:
    ...
    if current != head:
        _logger.warning(
            "OmniCraft database schema is out of date "
        ...
        if migrated != head:
            raise RuntimeError(
                f"OmniCraft automatic database migration did not reach head "
...
def extract_search_text(item: NewConversationItem) -> str:
    ...
    """
    from omnicraft.entities.conversation import CompactionData
