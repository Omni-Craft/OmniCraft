"""Helpers for OmniCraft-prefixed credential environment variables."""
...
OMNICRAFT_ENV_PREFIX = "OMNICRAFT_"

_ENV_REF_RE = re.compile(r"(?<!\$)\$(?:\{([A-Za-z_][A-Za-z0-9_]*)\}|([A-Za-z_][A-Za-z0-9_]*))")


def omnicraft_prefixed_env_name(name: str) -> str:
    """Return the OmniCraft-prefixed alias for *name*.

    :param name: Environment variable name, e.g. ``"ANTHROPIC_API_KEY"``.
    :returns: ``"OMNICRAFT_ANTHROPIC_API_KEY"`` unless *name* is already
        OmniCraft-prefixed.
    """
    return name if name.startswith(OMNICRAFT_ENV_PREFIX) else f"{OMNICRAFT_ENV_PREFIX}{name}"


def env_names_with_omnicraft_prefix(name: str) -> tuple[str, ...]:
    """Return the canonical env var name plus its OmniCraft-prefixed alias.
    ...
    """
    prefixed = omnicraft_prefixed_env_name(name)
    if prefixed == name:
        return (name,)
    return (name, prefixed)


def getenv_with_omnicraft_prefix(
    name: str, environ: Mapping[str, str] | None = None
) -> tuple[str, str] | None:
    """Read *name*, falling back to ``OMNICRAFT_<name>`` when unset.
    ...
    for candidate in env_names_with_omnicraft_prefix(name):
...
def getenv_nonempty_with_omnicraft_prefix(
    name: str, environ: Mapping[str, str] | None = None
) -> tuple[str, str] | None:
    """Read a non-empty env var with ``OMNICRAFT_`` fallback.
...
def expand_envvars_with_omnicraft_prefix(value: str) -> str:
    """Expand ``$VAR`` references with ``OMNICRAFT_VAR`` fallback.
    ...
    ``OMNICRAFT_VAR`` is set, the prefixed value is used. Unresolved references
    ...
    def _replace(match: re.Match[str]) -> str:
        ...
        resolved = getenv_with_omnicraft_prefix(name)
