"""Centralized error handling for the omnicraft server.

All user-facing errors should be raised as OmniCraftError with an
...
New code should prefer OmniCraftError for consistency.
"""
...
class ErrorCode:
    ...
    :cvar HARNESS_PROTOCOL_VIOLATION: A harness emitted an SSE
        sequence that violates the OmniCraft↔harness contract — e.g.
    ...
    :cvar HARNESS_NOT_CONFIGURED: The session's harness is not
        ...
        running ``omnicraft setup`` on the host machine).
    """
...
_CODE_TO_HTTP_STATUS: dict[str, int] = {
    ...
    # can't satisfy it until the user runs `omnicraft setup` there —
