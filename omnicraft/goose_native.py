"""Native Goose TUI wrapper for the OmniCraft CLI.

``omnicraft goose`` launches Block's Goose CLI interactive TUI (``goose session``)
inside an OmniCraft-runner-owned tmux terminal and attaches the local TTY — the
goose analog of ``omnicraft cursor`` / ``omnicraft codex`` / ``omnicraft pi``. The
runner spawns the process (see
:func:`omnicraft.runner.app._auto_create_goose_terminal`); this module owns the
...
``~/.config/goose/config.yaml`` + keyring); no OmniCraft-managed key is required.
...
from omnicraft._wrapper_labels import GOOSE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_GOOSE_PATH_ENV = "OMNICRAFT_GOOSE_PATH"
...
def run_goose_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Goose requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_goose_native."
        )
    with TemporaryDirectory(prefix="omnicraft-goose-native-") as tmpdir:
...
def _materialize_goose_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``omnicraft goose``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Goose on an OmniCraft server via a daemon-spawned runner.
