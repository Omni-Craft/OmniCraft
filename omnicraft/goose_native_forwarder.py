...
The ``omnicraft goose`` wrapper launches the real ``goose session`` TUI in a
runner-owned tmux pane, and :mod:`omnicraft.goose_native_bridge` injects web-UI
...
:mod:`omnicraft.cursor_native_forwarder`. Goose stores sessions in a SQLite
...
runner launches ``goose session --name <omnicraft-session-id>``, discovery is a
...
:mod:`omnicraft.runner.app`), exactly as for cursor-/claude-native.
"""
...
def _resolve_goose_session_id(db_path: Path, session_name: str) -> str | None:
    ...
    The runner launches ``goose session --name <omnicraft-session-id>``; the row
...
async def forward_goose_store_to_session(
    *,
    ...
    ``--name <omnicraft-session-id>`` the runner launched with), then polls its
...
async def supervise_goose_forwarder(
    *,
    ...
    Mirrors :func:`omnicraft.cursor_native_forwarder.supervise_cursor_forwarder`:
