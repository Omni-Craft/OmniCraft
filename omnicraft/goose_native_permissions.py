...
``prompt_tool_confirmation``). To also surface those approvals in the OmniCraft
...
Enter in the terminal). Mirrors :mod:`omnicraft.cursor_native_permissions`; the
...
from omnicraft.goose_native_bridge import capture_goose_pane, send_goose_pane_keys
...
def goose_permission_elicitation_id(session_id: str, token: str) -> str:
    """Return the deterministic OmniCraft elicitation id for a Goose prompt.
