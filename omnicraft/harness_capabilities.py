...
Like :mod:`omnicraft.harness_install_spec`, this type lives outside the
...
Each :class:`~omnicraft.harness_plugins.HarnessContribution` carries a
...
class Elicitation(str, Enum):
    """How a policy ASK / tool-approval is surfaced to the OmniCraft web UI."""

    NONE = "none"
    HOOK = "hook"  # vendor PreToolUse hook posts to OmniCraft
...
class Resume(str, Enum):
    ...
    COLD_ONLY = "cold-only"  # rebuild from OmniCraft transcript / history replay
...
class AuthModel(str, Enum):
    ...
    OMNICRAFT_CREDENTIAL = "omnicraft-credential"  # OmniCraft gateway / provider config
    OWN_AUTH = "own-auth"  # vendor login / API key, not OmniCraft-managed
...
class HarnessCapabilities:
    ...
    :param subagents: Whether the harness can spawn OmniCraft sub-agents.
