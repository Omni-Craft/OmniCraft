"""Native Hermes TUI wrapper for the OmniCraft CLI.

``omnicraft hermes`` launches Nous Research's Hermes Agent interactive TUI (the bare
``hermes`` command) inside an OmniCraft-runner-owned tmux terminal and attaches the
local TTY — the Hermes analog of ``omnicraft goose`` / ``omnicraft cursor``. The
runner spawns the process (see
:func:`omnicraft.runner.app._auto_create_hermes_terminal`); this module owns the
...
``~/.hermes/config.yaml``); no OmniCraft-managed key is required. Like goose there
...
from omnicraft._wrapper_labels import HERMES_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_HERMES_PATH_ENV = "OMNICRAFT_HERMES_PATH"
...
def run_hermes_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Hermes requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_hermes_native."
        )
    with TemporaryDirectory(prefix="omnicraft-hermes-native-") as tmpdir:
...
def _materialize_hermes_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``omnicraft hermes``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Hermes on an OmniCraft server via a daemon-spawned runner.
