...
then delivers OmniCraft web-UI messages into the *same* pane via
...
When OmniCraft policies are configured the runner writes a per-session
``HERMES_HOME`` with a ``pre_tool_call`` hook (via :func:`write_policy_hook_config`)
that evaluates tool calls against the OmniCraft policy engine — the same hook used
by the headless ``hermes`` harness (:mod:`omnicraft.inner.hermes_executor`). The
...
(:mod:`omnicraft.hermes_native_permissions`), which reads the pane via
...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"omnicraft-{os.getuid()}" / "hermes-native"
_TMUX_FILE = "tmux.json"
_TMUX_READY_TIMEOUT_S = 30.0
_TMUX_SEND_TIMEOUT_S = 10.0
_POLL_INTERVAL_S = 0.2
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "omnicraft-hermes-paste"
...
# starts the OmniCraft MCP server (a heavyweight ``python -m`` subprocess). A
...
# :mod:`omnicraft.runtime.pending_inputs` — the i-th persisted user row drains the
...
def clone_hermes_session(
    ...
    *,
    ...
    try:
        ...
        # messages (OmniCraft already has them from the fork item copy).
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/omnicraft-<uid>/hermes-native/<hash>``."""
...
def build_hermes_native_spawn_env(session_id: str) -> dict[str, str]:
    ...
    :class:`~omnicraft.inner.hermes_native_executor.HermesNativeExecutor` can find
...
def write_policy_hook_config(
    ...
    """Write per-session ``HERMES_HOME`` with OmniCraft policy hook and MCP server.

    Creates a ``config.yaml`` registering:

    1. A ``pre_tool_call`` shell hook that evaluates tool calls against the
       OmniCraft policy engine (same hook the headless ``hermes`` harness uses).
    2. An ``mcp_servers.omnicraft`` entry that launches the OmniCraft MCP stdio
       server (``serve-mcp``), exposing OmniCraft builtin tools
    ...
    :func:`omnicraft.inner.hermes_executor._populate_hermes_home`.
    ...
    # Register the OmniCraft MCP stdio server so Hermes can call
    # OmniCraft builtin tools (sys_session_*, sys_agent_*, load_skill, etc.).
    config["mcp_servers"] = {
...
def _write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :func:`omnicraft.codex_native_bridge.write_mcp_bridge_config`.
    """
...
def capture_hermes_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`omnicraft.hermes_native_permissions`) to detect Hermes' in-terminal
