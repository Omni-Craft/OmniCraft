...
The ``omnicraft hermes`` wrapper launches the real ``hermes`` TUI in a runner-owned
tmux pane, and :mod:`omnicraft.hermes_native_bridge` injects web-UI messages into
it. That covers the web→TUI direction, but the *embedded terminal* is then the
only surface that reflects the agent's work — the OmniCraft conversation view (chat
...
:mod:`omnicraft.goose_native_forwarder`. Hermes stores all sessions in a single
...
hermes-native (see :mod:`omnicraft.runner.app`), exactly as for goose-/cursor-native.
...
deduped against a persisted posted-count (:mod:`omnicraft.hermes_native_status`) so
...
from omnicraft import hermes_native_status
...
class _HermesUsageTracker:
    ...
    Follows the :class:`omnicraft.codex_native_forwarder._SessionUsageCoalescer`
...
def _read_new_items(
    ...
    # ``reasoning_content`` / ``reasoning`` chegaram em esquemas mais novos;
    # selecionar só as que existem, senão um DB antigo estoura em
    # ``no such column`` e o espelhamento inteiro morre calado.
    reasoning_cols = [c for c in ("reasoning_content", "reasoning") if c in cols]
    reasoning_select = "".join(f", {c}" for c in reasoning_cols)
    try:
        rows = con.execute(
    ...
    items: list[_MirrorItem] = []
    for row in rows:
        # As colunas de reasoning vêm no fim e só quando o esquema as tem;
        # desempacotar por posição, consultando quais entraram no SELECT.
        msg_id, role, content, tool_calls_json, tool_call_id, tool_name_val = row[:6]
        extras = dict(zip(reasoning_cols, row[6:], strict=False))
...
async def forward_hermes_store_to_session(
    *,
    ...
    # OmniCraft server so we do it at most once per forwarder lifetime.
...
async def supervise_hermes_forwarder(
    *,
    ...
    Mirrors :func:`omnicraft.goose_native_forwarder.supervise_goose_forwarder`:
