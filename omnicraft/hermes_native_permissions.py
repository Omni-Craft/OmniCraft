...
lives only in the TUI; to also surface it in the OmniCraft web UI (so a user can
...
terminal). Mirrors :mod:`omnicraft.cursor_native_permissions`. NB: Hermes only
...
from omnicraft.hermes_native_bridge import capture_hermes_pane, send_hermes_pane_keys
...
def hermes_permission_elicitation_id(session_id: str, token: str) -> str:
    """Return the deterministic OmniCraft elicitation id for a Hermes prompt.
