...
:class:`HostProcess` loop as ``omnicraft host``.

Two modes:

- ``--server <url>``: connect to an existing (remote or local) OmniCraft server.
- ``--local``: this daemon owns a local OmniCraft server — start (or reuse) a
  persistent background ``omnicraft server`` on loopback and connect to
...
def main() -> None:
    ...
    parser.add_argument(
        "--local",
        action="store_true",
        help="Start (or reuse) a local OmniCraft server and connect to it.",
    )
    ...
    if args.local:
        ...
        server_url = ensure_local_omnicraft_server().url
    else:
