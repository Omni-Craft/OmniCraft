"""Host process main loop for ``omnicraft host``.
...
def _runner_log_dir() -> Path:
    ...
    :returns: The host-runner log directory, e.g.
        ``Path.home() / ".omnicraft" / "logs" / "host-runner"``.
    """
    return Path.home() / ".omnicraft" / "logs" / "host-runner"
...
def _display_log_path(path: Path) -> str:
    ...
    :param path: Absolute path, typically under the user's state dir, e.g.
        ``Path("/Users/alice/.omnicraft/logs/host-runner/runner-ab12.log")``.
...
_RUNNER_ENV_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        #   - OMNICRAFT_CONFIG_HOME: where config.yaml / provider config live,
        #     so the runner resolves the same providers the CLI configured.
        #   - OMNICRAFT_DATA_DIR: where the sqlite db + pidfile live, so the
        ...
        # OMNICRAFT_AUTH_PROVIDER and the daemon-spawned local server
        # silently boots in accounts mode while the CLI thinks it's talking
        # to a header-mode server — every CLI request 401s (e.g. the
        # test_run_omnicraft_resumption suite). Not a secret; safe to propagate to
        # any subprocess.
        "OMNICRAFT_AUTH_PROVIDER",
        # Multi-user opt-in switch (create_auth_provider): OMNICRAFT_AUTH_ENABLED
        # turns the env-unset header/local default into accounts (or oidc, when
        # OMNICRAFT_OIDC_* is set); =0 opts back out. Must propagate down the
        # CLI → daemon → local-server chain or `omnicraft run`/`connect` would
        # spawn the wrong auth mode while the operator set the switch on the CLI.
        # Not a secret. OMNICRAFT_ACCOUNTS_ENABLED is the deprecated pre-rename
        # alias, still propagated so existing setups keep working.
        "OMNICRAFT_AUTH_ENABLED",
        "OMNICRAFT_ACCOUNTS_ENABLED",
        ...
        # ``OMNICRAFT_CLAUDE_SDK_NO_SANDBOX=1 omnicraft run …`` had no
        ...
        "OMNICRAFT_CLAUDE_SDK_NO_SANDBOX",
        # Native-Claude launcher plugin selector: the entry-point NAME of a
        # launcher registered in the ``omnicraft.claude_launcher`` group (e.g.
        # ``isaac``). Read by omnicraft.claude_launcher.resolve_claude_launch in
        ...
        # (and the harness it spawns) never see OMNICRAFT_TELEMETRY_ENABLED, so
        ...
        # telemetry is opt-in. Not a secret (a boolean). The OMNICRAFT_OTEL_*
        # knobs (capture-content, FastAPI toggle) ride the prefix allowlist below.
        "OMNICRAFT_TELEMETRY_ENABLED",
        ...
        "OMNICRAFT_DATABRICKS_EXTRA_HEADERS",
    }
...
# both the standard ``OTEL_*`` vars and OmniCraft's ``OMNICRAFT_OTEL_*`` knobs
# (capture-content, FastAPI toggle) so they reach the runner/harness too.
_RUNNER_ENV_ALLOWLIST_PREFIXES: tuple[str, ...] = ("LC_", "MLFLOW_", "OTEL_", "OMNICRAFT_OTEL_")
...
HARNESS_CREDENTIAL_ENV_VARS: frozenset[str] = frozenset(
    ...
    for name in env_names_with_omnicraft_prefix(canonical)
)
...
RUNNER_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_RUNNER_ENV_PASSTHROUGH"
...
class HostConnectError(Exception):
    ...
    instead of backing off, so ``omnicraft host`` exits with an
...
def _build_runner_env(
    ...
    *,
    ...
    :param base_env: Host process environment to filter, e.g.
        ``os.environ``.
    :param server_url: OmniCraft server URL the runner connects back to, e.g.
...
class HostProcess:
    ...
    :param server_url: OmniCraft server URL, e.g.
    ...
    def __init__(
        ...
        # commands in :mod:`omnicraft.host.git_worktree`) currently in flight.
    ...
    async def _orphan_reaper_loop(self) -> None:
        ...
        (``start_new_session=True`` — ``omnicraft.inner._proc.spawn_kwargs``),
    ...
    def _credentials_fix_hint(self) -> str:
        ...
        :returns: An actionable remedy sentence naming the exact
            command, e.g. ``"Run `omnicraft login <url>` ..."``.
        """
        return (
            f"Run `omnicraft login {self._server_url}` to authenticate (it "
    ...
    def _login_fix_hint(self) -> str:
        """Suggest ``omnicraft login`` as a remedy for an auth rejection.

        The host tunnel's bearer is resolved from a stored ``omnicraft
        login`` record first, then ambient Databricks credentials (see
        :func:`omnicraft.runner._entry._make_auth_token_factory`). When
        the server runs OmniCraft accounts or OIDC auth, a Databricks
        ...
        :returns: A one-sentence remedy naming the exact command, e.g.
            ``"If this server uses OmniCraft accounts or OIDC login, run
            `omnicraft login http://localhost:6767` to authenticate."``.
        """
        return (
            "If this server uses OmniCraft accounts or OIDC login, run "
            f"`omnicraft login {self._server_url}` to authenticate."
        )
    ...
    def _fatal_upgrade_error(self, exc: InvalidURI | InvalidStatus) -> HostConnectError | None:
        ...
        if isinstance(exc, InvalidURI):
            ...
            if self._login_redirect_streak == 1:
                ...
                # `omnicraft host` shows the auth problem and its fix instead
    ...
    async def _handle_launch(
        ...
        try:
            ...
            # session with `omnicraft debug logs --session <id>`. Cap at 32
    ...
    def _build_connect_headers(self) -> dict[str, str]:
        ...
        """
        from omnicraft.host.identity import HOST_TOKEN_ENV_VAR, MANAGED_HOST_TOKEN_HEADER
        ...
        headers: dict[str, str] = {"Origin": OMNICRAFT_INTERNAL_WS_ORIGIN}
        ...
        try:
            ...
            # stored ``omnicraft login`` token is silently skipped and
    ...
    async def _serve_frames(self, ws: websockets.asyncio.client.ClientConnection) -> None:
        ...
        hello = HostHelloFrame(
            ...
            # ~/.omnicraft/config.yaml. Recomputed on every (re)connect, so
...
def run_host_process(
    ...
    # Tell the user where logs land up front — `omnicraft host` used to run
