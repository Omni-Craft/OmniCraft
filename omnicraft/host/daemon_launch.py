...
(``run`` / ``claude`` / ``codex``) asks the OmniCraft server to launch
...
async def wait_for_runner_online(
    ...
    *,
    ...
    if last_error is not None:
        message += f" Last connection error: {last_error!r}."
    message += " Check the host-runner logs under ~/.omnicraft/logs/host-runner/."
    raise click.ClickException(message)
...
async def launch_or_reuse_daemon_runner(
    ...
    *,
    ...
    # registry while it (re)connects — e.g. just after `omnicraft host`
