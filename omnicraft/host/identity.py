"""Host identity management for ``omnicraft host``.

Reads or creates the ``host`` section in ``~/.omnicraft/config.yaml``.
The host identity is auto-generated on first ``omnicraft host``
...
CONFIG_PATH = Path.home() / ".omnicraft" / "config.yaml"
...
HOST_TOKEN_ENV_VAR = "OMNICRAFT_HOST_TOKEN"
HOST_ID_ENV_VAR = "OMNICRAFT_HOST_ID"
HOST_NAME_ENV_VAR = "OMNICRAFT_HOST_NAME"

# WebSocket upgrade header carrying a managed host's launch token.
# Mirrors the runner tunnel's X-OmniCraft-Runner-Tunnel-Token pattern:
...
# auth provider.
MANAGED_HOST_TOKEN_HEADER = "X-OmniCraft-Host-Token"
...
def load_or_create_host_identity(
    ...
    :param path: Path to the config YAML file, e.g.
        ``Path("~/.omnicraft/config.yaml")``. Defaults to
        :data:`CONFIG_PATH`.
