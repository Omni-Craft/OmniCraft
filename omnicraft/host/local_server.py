"""Persistent background local OmniCraft server lifecycle.
...
These helpers live under ``omnicraft/host/`` (not ``cli.py``) so the
...
def _local_data_dir() -> Path:
    ...
    Honors ``OMNICRAFT_DATA_DIR`` (the purpose-built data-isolation knob),
    else ``~/.omnicraft``. This lets a checkout/worktree isolate its local
    runtime DB: two worktrees otherwise share ``~/.omnicraft/chat.db``, and
    ...
    Must stay in lock-step with :func:`omnicraft.chat._omnicraft_persistent_dir`:
    the local server's DB lives here and ``omnicraft run`` resolves the
    resume DB there, so the two MUST agree. ``OMNICRAFT_CONFIG_HOME`` is
    ...
    inheriting a shared ``OMNICRAFT_CONFIG_HOME``).

    :returns: The data directory path (callers create it lazily).
    """
    value = os.environ.get("OMNICRAFT_DATA_DIR")
    if value:
        return Path(value).expanduser()
    return Path.home() / ".omnicraft"
...
# config change (e.g. flipping OMNICRAFT_AUTH_ENABLED) respawns the
...
# ``omnicraft server`` (its logs stream to the terminal, not a file).
...
def server_config_signature() -> str:
    ...
    The daemon (in local mode) spawns the OmniCraft server once and never
    ...
    the user flips ``OMNICRAFT_AUTH_ENABLED``, or upgrades the package) and
    ...
    * the installed package version — a running server holds its code in
      ...
      config-drift path in :func:`ensure_local_omnicraft_server` — no
    ...
    try:
        version = importlib.metadata.version("omnicraft")
...
def local_server_url_if_healthy() -> str | None:
    ...
    :func:`ensure_local_omnicraft_server`, the one place that decides reuse.
...
def _write_local_server_record(
    ...
    (``omnicraft server``) registration paths stay symmetric: a server
    that advertises itself in the pidfile ALWAYS stamps a matching sig,
    so reuse-matching in :func:`ensure_local_omnicraft_server` can't spuriously
    ...
    :param log_path: Absolute path of the spawned server's captured log file,
        e.g. ``Path("/Users/alice/.omnicraft/logs/server/local-server-ab12cd.log")``.
...
def _read_local_server_log_path() -> Path | None:
    ...
    :returns: The absolute log path the background server writes to, e.g.
        ``Path("/Users/alice/.omnicraft/logs/server/local-server-ab12cd.log")``, or
...
def _atomic_write(path: Path, text: str) -> None:
    ...
    :param path: Destination file, e.g. ``~/.omnicraft/local_server.pid``.
...
def _terminate_pid(pid: int) -> None:
    ...
    Shared by :func:`stop_local_omnicraft_server` (the pidfile-tracked server) and
...
def stop_local_omnicraft_server() -> None:
    ...
    :func:`ensure_local_omnicraft_server` spawns a fresh
    ...
    the off-switch (``omnicraft stop`` / ``server stop``) calls both.
...
class LocalServerInfo:
    ...
    :param log_path: Absolute path of the background server's captured log
        file, e.g. ``Path("/Users/alice/.omnicraft/logs/server/local-server-ab12cd.log")``.
...
def local_server_status() -> LocalServerInfo:
    ...
    Reads ``~/.omnicraft/local_server.pid`` for the recorded pid/port and
...
class LocalServerStartup:
    """Outcome of :func:`ensure_local_omnicraft_server`.
    ...
    :param spawned: ``True`` when this call started a NEW detached server
        ...
        (one started earlier by ``omnicraft server`` or by a prior
    ...
    :param log_path: Absolute path of the background server's captured log
        ...
        ``None`` when the running server is a foreground ``omnicraft server``
...
def ensure_local_omnicraft_server() -> LocalServerStartup:
    """Ensure a persistent background local OmniCraft server is running.

    Reuses a healthy server recorded in the pidfile; otherwise spawns a
    detached ``omnicraft server`` on a free loopback port, backed by the
    persistent ``~/.omnicraft`` data store so conversations survive across
    invocations (designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md). The server runs
    ...
    # same URL as a manual `omnicraft server` (and reuse via the pidfile
    ...
    while True:
        ...
        try:
            _wait_for_local_omnicraft_server(spawned.base_url, spawned.proc, spawned.log_path)
...
class _SpawnedLocalServer:
    ...
    :param proc: The ``omnicraft server`` subprocess handle.
    :param log_path: File capturing the child's stdout/stderr, e.g.
        ``Path("~/.omnicraft/logs/server/local-server-ab12cd.log")``.
...
def _spawn_local_server(port: int) -> _SpawnedLocalServer:
    ...
    (:func:`ensure_local_omnicraft_server`) records the server only after it
    ...
    :returns: The spawned subprocess plus its log path and base URL; the
        caller awaits readiness via :func:`_wait_for_local_omnicraft_server`.
    """
    ...
    # An explicit OMNICRAFT_DATABASE_URI wins over the per-data-dir sqlite
    # file (e.g. to point a worktree at its own Postgres). Defaults to the
    # isolated sqlite db under the runtime data dir.
    db_uri = os.environ.get("OMNICRAFT_DATABASE_URI") or f"sqlite:///{db_path}"
    ...
    # POV, `omnicraft run` (no --server) in accounts mode gets
    ...
    # the env-unset default; OMNICRAFT_AUTH_ENABLED=1 opts into accounts (or
    # oidc when OMNICRAFT_OIDC_* is set). In header/oidc mode we must NOT mint
...
def pick_local_port(preferred: int = _DEFAULT_LOCAL_PORT) -> int:
    ...
    URL is the same across ``omnicraft server`` and daemon spawns —
    ...
    omnicraft server happens via the pidfile (:func:`register_local_server`
...
def _local_server_health_ok(base_url: str) -> bool:
    """Return ``True`` if *base_url* answers ``/health`` as an OmniCraft server.

    Confirms a listener is actually an OmniCraft server (``GET /health`` →
...
def stop_untracked_local_server(port: int = _DEFAULT_LOCAL_PORT) -> int | None:
    ...
    server then escapes :func:`stop_local_omnicraft_server`, which only knows the
    pidfile PID — so ``omnicraft stop`` / ``server stop`` would leave it
    running. This sweep covers that hole: if a live OmniCraft server answers
    ``/health`` on the canonical loopback *port*, find its PID and terminate
    it. Call it AFTER :func:`stop_local_omnicraft_server` so a normally-tracked
...
def register_local_server(port: int) -> None:
    ...
    Lets a foreground ``omnicraft server`` advertise itself in the same
    ``local_server.pid`` the daemon reads, so ``omnicraft run`` /
    ...
    sig and reuse-matching in :func:`ensure_local_omnicraft_server` always sees
...
def clear_local_server_record() -> None:
    ...
    Called on ``omnicraft server`` shutdown so a clean exit doesn't
...
def _wait_for_local_omnicraft_server(
    base_url: str,
    proc: subprocess.Popen[bytes],
    log_path: Path,
