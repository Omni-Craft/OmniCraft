"""Expose OmniCraft's builtin tools to an ACP agent via ``session/new.mcpServers``.

Shared by the ACP executors (``acp`` generic, ``goose``, ``qwen``). Reuses the
*same* stdio ``serve-mcp`` relay the native harnesses use
(:mod:`omnicraft.claude_native_bridge`): the ACP agent spawns
``python -Im omnicraft.claude_native_bridge serve-mcp --bridge-dir <dir>`` as an
MCP server, which proxies each OmniCraft tool call back through ``tool_executor``
(→ :meth:`TurnContext.dispatch_tool` → the OmniCraft server, where TOOL_CALL /
TOOL_RESULT policy is enforced). The agent keeps its own filesystem/shell tools;
this only *adds* OmniCraft's builtin tools (``sys_session_*``, ``sys_agent_*``,
...
``OMNICRAFT_ACP_MCP=0`` kill switch) yields an empty ``mcpServers`` — the agent
just runs without OmniCraft tools, exactly as before this feature.
"""
...
_ENV_KILL_SWITCH = "OMNICRAFT_ACP_MCP"
...
class OmniCraftAcpMcp:
    """Lazily-started OmniCraft-tool relay + its ACP ``mcpServers`` entry.
    ...
    def session_new_servers(
        ...
        *,
        ...
        :param tools: OmniCraft tool schemas to advertise (each ``{"name", …}``).
        ...
        try:
            ...
            logger.info(
                "acp[%s] OmniCraft MCP relay ready (%d builtin tools bridged)",
                self._label,
                len(tools),
            )
            return self._acp_servers
