...
:class:`~omnicraft.inner.goose_executor.GooseExecutor` /
:class:`~omnicraft.inner.qwen_executor.QwenExecutor`: it spawns whatever command a
...
internally. This executor translates the ACP event stream into OmniCraft
:class:`ExecutorEvent`s and routes the agent's permission requests through
OmniCraft's TOOL_CALL policy + human-consent elicitation.

Vs. the Goose executor this generalizes, it additionally: renders the agent's
tool calls as OmniCraft tool cards (``tool_call`` → ``ToolCallRequest``,
...
from omnicraft.inner._acp_omnicraft_mcp import OmniCraftAcpMcp
...
class AcpAgentConfig:
    ...
    :param omnicraft_mcp: Expose OmniCraft's builtin tools to the agent via
        ``session/new.mcpServers`` (the shared ``serve-mcp`` relay). On by
        default; the global ``OMNICRAFT_ACP_MCP=0`` kill switch also disables it.
    """
...
class AcpExecutor(Executor):
    ...
    def __init__(
        ...
        # mid-turn ``session/request_permission`` routes through OmniCraft's
        ...
        # attribute the SDK harnesses use); backs the OmniCraft MCP relay.
        self._tool_executor: Any | None = None  # type: ignore[explicit-any]

        # OmniCraft-tool MCP bridge — exposes builtin tools to the agent via
        # session/new.mcpServers (lazily started at first session; torn down in
        # :meth:`close`). ``_omnicraft_tools`` is captured each turn for the relay.
        self._mcp = OmniCraftAcpMcp(label=config.name)
        self._omnicraft_tools: list[Any] = []  # type: ignore[explicit-any]
    ...
    async def _ensure_initialized(self) -> None:
        ...
        resp = await self._rpc(
            _AGENT_METHOD_INITIALIZE,
            {
                ...
                "clientInfo": {"name": "omnicraft", "version": "1.0"},
                "clientCapabilities": {
                    "fs": {
    ...
    async def _ensure_session(self) -> str:
        ...
        ``mcpServers`` carries OmniCraft's builtin tools (via the shared serve-mcp
        relay) unless disabled — see :class:`OmniCraftAcpMcp`.
        """
        if self._session_id is not None:
            return self._session_id

        mcp_servers = self._mcp.session_new_servers(
            tools=self._omnicraft_tools,
            tool_executor=getattr(self, "_tool_executor", None),
            loop=asyncio.get_event_loop(),
            enabled=self._config.omnicraft_mcp,
        )
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``session/request_permission`` — decide via OmniCraft's TOOL_CALL policy
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          advertised, execute through the OmniCraft OSEnvironment so the spec's
        ...
        try:
            if method == _AGENT_REQUEST_REQUEST_PERMISSION:
                ...
                error = {
                    "code": -32601,
                    "message": f"omnicraft: unsupported ACP request method {method!r}",
                }
    ...
    async def _ensure_os_environment(self) -> OSEnvironment:
        ...
        if self._os_environment is None:
            ...
            if env is None:
                raise _AcpRequestError(-32603, "omnicraft: no os_env for fs delegation")
    ...
    def _usage_from_result(result: dict[str, Any]) -> dict[str, Any] | None:  # type: ignore[explicit-any]
        """Map an agent's final ``result.usage`` to OmniCraft's usage keys.

        ACP does not standardize usage, but agents that report it (Goose) use
        ``{totalTokens, inputTokens, outputTokens}``; OmniCraft's
    ...
    async def run_turn(
        ...
        ``tools`` (OmniCraft's builtin tool schemas) are captured for the OmniCraft
        ...
        self._omnicraft_tools = tools or []
        try:
    ...
    async def close(self) -> None:
        ...
        # Tear down the OmniCraft MCP relay HTTP server + its bridge dir first.
