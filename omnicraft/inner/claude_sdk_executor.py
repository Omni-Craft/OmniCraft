...
underlying agent harness.  OmniCraft tools are bridged into the SDK session
...
This executor translates the SDK message stream into OmniCraft ExecutorEvents
...
from omnicraft.onboarding.databricks_config import DATABRICKS_CLAUDE_DEFAULT_MODEL
from omnicraft.reasoning_effort import CLAUDE_EFFORTS, validate_effort
...
# ---------------------------------------------------------------------------
# TypeAliases for OmniCraft JSON-shaped boundary values. The SDK exchanges
...
# with the OmniCraft tool executor and the SDK's MCP bridge.
...
# path the OmniCraft producer resolves the model instead (see workflow.py).
_DATABRICKS_CLAUDE_DEFAULT_MODEL = DATABRICKS_CLAUDE_DEFAULT_MODEL

_CLAUDE_API_KEY_HELPER_ENV_KEY = "OMNICRAFT_CLAUDE_API_KEY_HELPER"
...
class PreparedClaudeCli:
    """Result of wrapping the Claude CLI in an OmniCraft sandbox.
...
def _build_mcp_tools(
    ...
    """Build SdkMcpTool objects from OmniCraft tool schemas.

    Each tool is backed by a handler that calls the OmniCraft tool_executor
    ...
    for schema in tool_schemas:
        ...
        # itself does not accept ``None``. OmniCraft tool schemas always
...
def _augment_system_prompt_for_omnicraft_mcp_tools(
    system_prompt: str,
    tool_schemas: list[ToolSpec],
) -> str:
    """
    ...
    OmniCraft schemas use bare names such as ``sys_session_send``. The
    Claude SDK exposes tools from our in-process MCP server to the model
    as ``mcp__omnicraft__<bare_name>``. Bundled agent prompts and skills use
    ...
    if examples:
        example_text = "; ".join(
            f"use `mcp__omnicraft__{name}` when instructions say `{name}`" for name in examples
        )
        note = (
            "Claude SDK tool naming: OmniCraft tools are exposed as MCP tools. "
            f"{example_text}. For any other OmniCraft tool, use "
            "`mcp__omnicraft__<tool_name>` rather than the bare name."
        )
    else:
        note = (
            "Claude SDK tool naming: OmniCraft tools are exposed as MCP tools. "
            "When instructions mention a bare OmniCraft tool name, invoke "
            "`mcp__omnicraft__<tool_name>` rather than the bare name."
        )
...
class ClaudeSDKExecutor(Executor):
    ...
    The SDK runs Claude Code's full agent loop internally.  OmniCraft tools
    ...
    is set.  Even without ``os_env``, OmniCraft tries to place the Claude CLI
    ...
    turns by keeping a live Claude SDK client per OmniCraft session.
        ...
        *,
        ...
        Args:
            ...
            os_env: If set, enable built-in OS tools (Bash, Read, Edit, …)
                ...
                spec's sandbox is enabled, OmniCraft wraps the Claude CLI
                in the same sandbox. If omitted, OmniCraft still tries to
        ...
        if not gateway and model is not None and model.startswith("databricks-"):
            raise ValueError(
                ...
                "Databricks provider with `omnicraft setup`, to route through "
        ...
        # the OmniCraft elicitation system rather than silently allowed.
        # ``None`` until the adapter installs it on first use.
        self._elicitation_handler: ElicitationHandler | None = None
        # Live Claude SDK clients keyed by OmniCraft session id.
    ...
    async def _route_options_through_gateway_shim(self, options: SdkOptions) -> None:
        ...
        the :mod:`~omnicraft.inner.claude_gateway_shim` module
    ...
    async def _can_use_tool_for_permission(
        ...
        """
        Route a Claude SDK permission request through the OmniCraft elicitation system.

        Installed as ``options.can_use_tool`` when ``permission_mode`` is
        not ``"bypassPermissions"`` and an elicitation handler has been wired
        in by :class:`omnicraft.runtime.harnesses._executor_adapter.ExecutorAdapter`.
        ...
        if allowed:
            return PermissionResultAllow()
        return PermissionResultDeny(message="Denied via OmniCraft elicitation")
    ...
    async def _evaluate_tool_call_policy(
        ...
        evaluated against OmniCraft TOOL_CALL-phase policies BEFORE they
        ...
        Double-evaluation guard: OmniCraft's OWN tools are exposed as the
        single ``omnicraft`` SDK MCP server (the model sees
        ``mcp__omnicraft__*``). When the model calls one, the SDK wrapper
        routes it back through OmniCraft's dispatch bridge
        ...
        (see ``omnicraft/runner/app.py`` "All tool calls go through AP:/mcp
        ... which enforces TOOL_CALL + TOOL_RESULT policies server-side").
        Spec-declared MCP tools are surfaced through that same
        ``mcp__omnicraft__*`` server, so they are covered there too.
        Evaluating ``mcp__omnicraft__*`` here as well would double-count
        ...
        :returns: :class:`claude_agent_sdk.PermissionResultDeny` when a
            ...
            existing OmniCraft elicitation handler before returning
            ...
            ``mcp__omnicraft__*`` tool already gated on the dispatch path, or
        ...
        if _policy_eval is None:
            return None
        # OmniCraft's own tools are already TOOL_CALL-gated server-side via
        # the dispatch bridge / ProxyMcpManager — don't evaluate them twice.
        if tool_name.startswith("mcp__omnicraft__"):
            return None
        ...
        if _action == "POLICY_ACTION_ASK":
            ...
            reason = _verdict.reason or "Approval required by OmniCraft TOOL_CALL policy"
            if self._elicitation_handler is None:
                logger.warning(
        ...
        if _action == "POLICY_ACTION_DENY":
            ...
            reason = _verdict.reason or "Denied by OmniCraft TOOL_CALL policy"
            logger.info("TOOL_CALL policy denied tool=%s reason=%s", tool_name, reason)
            return PermissionResultDeny(message=reason)
        from claude_agent_sdk import PermissionResultDeny

        reason = f"Unexpected OmniCraft TOOL_CALL policy verdict: {_action!r}"
    ...
    async def run_turn(
        ...
        # Build MCP tools from OmniCraft tool schemas
        mcp_tools = _build_mcp_tools(tools, self._tool_executor) if tools else []

        # Create MCP server config for OmniCraft tools. The SDK's
        ...
        if mcp_tools:
            mcp_servers["omnicraft"] = sdk.create_sdk_mcp_server(
                name="omnicraft",
                version="1.0.0",
                tools=mcp_tools,
            )
            system_prompt = _augment_system_prompt_for_omnicraft_mcp_tools(
                system_prompt,
                tools,
            )

        # Build allowed_tools list.  OS-environment operations route
        # through OmniCraft ``sys_os_*`` MCP tools rather than the
        ...
        # to a ``databricks-*`` model: the OmniCraft producer always resolves a
        ...
        # stays because omnicraft owns conversation persistence
        # via its own conversation store.
        # OS-environment tools are provided via OmniCraft ``sys_os_*``
        ...
        # the CLI subprocess are evaluated against OmniCraft TOOL_CALL
        ...
        # callback, call it with the request data so the OmniCraft server
