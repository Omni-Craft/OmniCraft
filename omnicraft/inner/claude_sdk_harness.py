...
shared :mod:`omnicraft.runtime.harnesses._runner` invokes after
the parent process resolves ``"claude-sdk"`` to this module via
:data:`omnicraft.runtime.harnesses._HARNESS_MODULES`.

Internally, instantiates :class:`omnicraft.runtime.harnesses._executor_adapter.ExecutorAdapter`
around a :class:`omnicraft.inner.claude_sdk_executor.ClaudeSDKExecutor`
...
than per-spec data on the request body. OmniCraft sets these env vars
...
- ``HARNESS_CLAUDE_SDK_OS_ENV``: JSON-encoded :class:`OSEnvSpec`
  ...
  OmniCraft mode parity with the legacy non-AP path holds for
...
# Set by the OmniCraft workflow layer when executor.auth: {type: api_key, …} is
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    OmniCraft serialized via :func:`dataclasses.asdict` on its
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing — OmniCraft omits the env var when ``llm.retry`` matches
...
def _build_claude_sdk_executor() -> Executor:
    ...
    return ClaudeSDKExecutor(
        ...
        # OMNICRAFT_RUNNER_WORKSPACE (the cwd the user launched in, and the one
        # the tmux terminal uses), else the process cwd. Without the workspace
        # fallback the CLI ran out of the runner daemon's launch dir, so it
        # disagreed with the terminal and with the sandbox root.
        # Mirrors goose / kimi / pi / qwen / hermes harness cwd resolution.
...
def create_app() -> FastAPI:
    ...
    :data:`omnicraft.runtime.harnesses._HARNESS_MODULES`) and
