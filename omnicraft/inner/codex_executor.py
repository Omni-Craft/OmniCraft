...
OmniCraft session, persists the Codex thread across turns, and exposes
OmniCraft tools to Codex as App Server ``dynamicTools``.
"""
...
from omnicraft.reasoning_effort import CODEX_EFFORTS, EFFORT_ALIASES, validate_effort
from omnicraft.runner.identity import OMNICRAFT_SESSION_ENV_VAR
...
# generic-provider gateway path never uses this — it requires the OmniCraft producer
...
# to running sessions without any action from OmniCraft.
# hooks.json entra junto para os hooks do usuário existirem no home privado,
...
# OMNICRAFT_GOOSE_PATH. Set this when codex lives on a PATH the host
# daemon doesn't inherit (e.g. an nvm-managed global bin dir).
_CODEX_PATH_ENV = "OMNICRAFT_CODEX_PATH"
...
async def _create_subprocess_exec(*args: Any, **kwargs: Any) -> asyncio.subprocess.Process:
    ...
    ``omnicraft.inner.codex_executor.asyncio.create_subprocess_exec``
...
def _clean_codex_env(extra_allow: Iterable[str] = ()) -> dict[str, str]:
    ...
    allow_exact = {
        ...
        OMNICRAFT_SESSION_ENV_VAR,  # "inside OmniCraft" marker (CLAUDE_CODE/CODEX analog)
    } | set(extra_allow)
...
def _is_omnicraft_private_codex_home(path: Path) -> bool:
    """
    Return whether *path* is an OmniCraft-created private ``CODEX_HOME``.

    OmniCraft launches Codex with private homes for session state
    ...
    :param path: Candidate ``CODEX_HOME`` path, e.g.
        ``"/home/user/.omnicraft/codex-native/<hash>/codex-home"``.
    :returns: ``True`` when *path* matches a native bridge home or the
        wrapped executor's temporary ``omnicraft-codex-home-*`` naming.
    """
    ...
    if (
        ...
        and parts[-4] == ".omnicraft"
    ):
        return True
    return expanded.name.startswith("omnicraft-codex-home-")
...
def _private_codex_home_config_source(path: Path) -> Path | None:
    ...
    A parent OmniCraft launch bridges ``auth.json`` and ``config.toml`` into
...
def _resolve_codex_home_config_source(source_dir: Path, home_codex_home: Path) -> Path:
    ...
    points at an OmniCraft private home from a parent session. In that
    ...
    """
    if source_dir != home_codex_home and _is_omnicraft_private_codex_home(source_dir):
...
def _codex_home_config_source_from_env() -> Path:
    ...
    Codex stores subscription login state in ``CODEX_HOME``. OmniCraft
    launches Codex with private per-session homes for isolation, then bridges
    only auth/config from the user's source home. Nested OmniCraft processes
...
def _retarget_codex_hook_trust_keys(
    ...
    :param target_dir: Per-session private ``CODEX_HOME`` whose path the
        ...
        ``Path("/home/user/.omnicraft/codex-native/abc/codex-home")``.
    """
...
def _databricks_codex_config_overrides(
    *,
    ...
    """
    provider_name = "omnicraft_databricks"
    auth_command_json = json.dumps(auth_command)
    return [
        f"model={json.dumps(model)}",
        f'model_provider="{provider_name}"',
        (
            "model_providers.omnicraft_databricks="
            '{name="OmniCraft Databricks",'
...
def _provider_codex_config_overrides(
    *,
    ...
    serving the ``openai`` surface) by registering an ``omnicraft_provider``
    ...
    :returns: Codex TOML-fragment override strings.
    """
    provider_name = "omnicraft_provider"
    auth_command_json = json.dumps(auth_command)
...
class _PendingToolResult:
    ...
    :param name: The tool name Codex asked OmniCraft to run.
...
class _CodexAppServerSession:
    ...
    async def start(self) -> None:
        ...
        self._codex_home_dir = Path(
            tempfile.mkdtemp(prefix="omnicraft-codex-home-", dir=str(codex_home_root))
        )
    ...
    def __init__(
        ...
        *,
        ...
        :param gateway_host: Gateway workspace host origin, e.g.
            ...
            ``HARNESS_CODEX_GATEWAY_HOST`` (written by the OmniCraft workflow
        ...
        :param base_url_override: Override the Codex gateway base URL instead
            ...
            ``HARNESS_CODEX_GATEWAY_BASE_URL`` (written by the OmniCraft workflow
        ...
        if gateway:
            ...
            # gateway path the OmniCraft producer must have resolved a model, and the
            ...
            if host is None:
                ...
                # Resolve the base-URL host from the profile, not a
                # DATABRICKS_HOST env override that would point it at a
                # different workspace than the ``--profile``-pinned auth
                # command draws its token from.
                ...
                if model is None:
                    # Directly-supplied neutral gateway: the OmniCraft producer always
                    ...
                    raise OSError(
                        ...
                        "model: the OmniCraft producer must resolve one before spawning."
                    )
        ...
        if not enable_web_search:
            ...
            # tools exposed by OmniCraft as dynamicTools. The top-level web_search
