...
around a :class:`omnicraft.inner.codex_executor.CodexExecutor`
...
- ``HARNESS_CODEX_CWD``: working directory the executor launches
  the Codex CLI in. ``None`` falls back to
  ``OMNICRAFT_RUNNER_WORKSPACE`` if set, then to the subprocess's
  inherited cwd.
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    ``RetryPolicy()`` (defaults) when missing — OmniCraft omits the
