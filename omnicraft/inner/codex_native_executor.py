"""Executor that bridges OmniCraft messages into a native Codex TUI thread."""
...
from omnicraft.inner.native_attachments import materialize_attachment, parse_data_uri
...
class CodexNativeExecutor(Executor):
    """
    Harness-side executor for ``omnicraft codex`` web UI turns.
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        try:
            if pending:
                ...
                # cancel above already updated what OmniCraft shows, and a
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from OmniCraft. Ignored here;
        ...
        :param config: Per-turn executor config. Its ``model`` and
            ``extra["reasoning_effort"]`` (carrying the OmniCraft web
...
def _model_effort_overrides(config: ExecutorConfig | None) -> dict[str, Any]:
    ...
    A model or reasoning-effort change selected in the OmniCraft web UI is
...
def _latest_user_input_items(messages: list[Message], bridge_dir: Path) -> list[dict[str, Any]]:
    ...
    :param bridge_dir: Bridge directory for materializing image/file
        attachments, e.g. ``Path("/tmp/omnicraft/codex-native/<digest>")``.
...
def _file_block_to_input_item(block: dict[str, Any], bridge_dir: Path) -> dict[str, Any] | None:
    ...
    if path is not None:
        ...
        # omnicraft/entities/conversation.py. Keep in sync.
