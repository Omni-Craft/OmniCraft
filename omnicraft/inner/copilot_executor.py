...
per OmniCraft conversation, created once and reused turn to turn. Each
...
Crucially, OmniCraft's spec-declared tools (``sys_session_send`` et al.) are
bridged into Copilot **in-process** via the SDK's ``tools``: each
:class:`~omnicraft.inner.executor.ToolSpec` becomes a :class:`copilot.Tool`
...
*inside* the SDK rather than through OmniCraft's bridged-tool dispatch. They are
...
from omnicraft.reasoning_effort import COPILOT_EFFORTS, validate_effort
...
# OmniCraft's bridged-tool callback: (tool_name, args) -> awaitable result.
...
def _build_copilot_prompt(messages: list[Message], *, is_first_turn: bool) -> str:
    ...
    OmniCraft system prompt is delivered separately (``system_message``), so on
...
class _CopilotSessionState:
    """Per-OmniCraft-conversation SDK session state."""
...
class CopilotExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # OmniCraft's session (policy gating, sub-agent dispatch, logging).
    ...
    def _make_tools(self, tools: list[ToolSpec]) -> list[Any]:  # type: ignore[explicit-any]
        """Build the SDK ``tools`` list from OmniCraft ToolSpecs.
        ...
        ``skip_permission=True`` because OmniCraft's policy layer (and the
    ...
    def _make_handler(self, tool_name: str) -> Callable[[Any], Awaitable[Any]]:  # type: ignore[explicit-any]
        """Build an async ``handler`` that bridges a Copilot tool call to OmniCraft.
    ...
    async def _on_permission_request(self, request: Any, _invocation: dict[str, str]) -> Any:  # type: ignore[explicit-any]
        ...
        if evaluator is not None:
            ...
            if getattr(verdict, "action", "") == "POLICY_ACTION_DENY":
                ...
                return PermissionDecisionReject(feedback=f"Denied by OmniCraft policy: {reason}")
        ...
        if handler is not None:
            ...
            if not approved:
                return PermissionDecisionReject(feedback="Denied via OmniCraft approval UI")
    ...
    async def _ensure_session(
        ...
        try:
            ...
            # its built-in tools) and layers the OmniCraft agent's system prompt
...
def _unwrap_tool_result(raw: Any) -> Any:  # type: ignore[explicit-any]
    ...
    OmniCraft-convention result (e.g. ``{"blocked": True}``) passes through
