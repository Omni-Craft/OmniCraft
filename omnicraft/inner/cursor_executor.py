...
``AsyncAgent`` per OmniCraft conversation, created on a
...
evaluates PHASE_TOOL_CALL and cancels the run on DENY.  Bridged OmniCraft
...
:class:`~omnicraft.inner.executor.ToolSpec` becomes a ``cursor_sdk.CustomTool``
...
def _normalize_cursor_usage(raw: dict[str, Any], model: str) -> dict[str, Any]:
    """Map Cursor SDK usage fields to the standard OmniCraft usage dict."""
    ...
    # map to the OmniCraft-standard cache_read_input_tokens /
...
def _build_cursor_prompt(
    ...
    *,
    ...
    first turn the OmniCraft system prompt is prepended (the SDK has no separate
...
def _sdk_message_to_events(message: Any) -> list[ExecutorEvent]:  # type: ignore[explicit-any]
    ...
    if mtype == "tool_call":
        ...
        # OmniCraft tool name + args so the observed events (and any name-keyed
...
def _write_cursor_hooks(cwd: str, hook_script_path: str, server_url: str, session_id: str) -> Path:
    ...
    :param server_url: OmniCraft server URL, e.g. ``"http://127.0.0.1:6767"``.
    ...
    wrapper = hooks_dir / "omnicraft-hook.sh"
...
class CursorExecutor(Executor):
    ...
    async def _evaluate_native_tool_policy(
        ...
        <omnicraft.inner.claude_sdk_executor.ClaudeSDKExecutor>` exposes
    ...
    def _make_execute(
        ...
        """Build a sync ``execute`` that bridges a cursor tool call to OmniCraft.
    ...
    async def _close_state(self, state: _CursorSessionState) -> None:
        ...
        if state.hooks_file is not None:
            try:
                ...
                wrapper = state.hooks_file.parent / "omnicraft-hook.sh"
