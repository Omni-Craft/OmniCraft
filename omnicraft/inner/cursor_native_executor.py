"""Executor that bridges OmniCraft web-chat turns into the native Cursor TUI.

It does not launch cursor-agent — the ``omnicraft cursor`` wrapper already
...
class CursorNativeExecutor(Executor):
    """Harness-side executor for ``omnicraft cursor`` web-UI turns.
