...
Wraps a :class:`omnicraft.inner.cursor_native_executor.CursorNativeExecutor`,
which injects web-UI messages into the running ``cursor-agent`` TUI (launched by
``omnicraft cursor`` in the session terminal) via tmux. The bridge dir is read
from :data:`~omnicraft.cursor_native_bridge.BRIDGE_DIR_ENV_VAR` in the spawn env.

Tool policies: OmniCraft's PreToolUse/PostToolUse policy gates (which claude- and
...
approval prompts (and ``--force``/``--yolo``/sandbox config), which omnicraft does
not intercept. Treat the cursor TUI's own approval as the sole tool gate; do not
assume OmniCraft connector/tool deny-policies constrain a cursor-native session.
"""
...
from omnicraft.inner.cursor_native_executor import CursorNativeExecutor
