...
``Proxy-Authorization: Basic <base64(omnicraft:token)>`` header on
every inbound connection, with the token shared between parent and
helper via :class:`omnicraft.inner.sandbox.SandboxPolicy.
...
net protection gain. See ``omnicraft/inner/os_env.py:
