...
output streams into the OmniCraft conversation as chat, and Goose's mid-turn tool
...
ExecutorEvents and routes Goose's permission requests through OmniCraft's
...
class GooseExecutor(Executor):
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here, executed through the OmniCraft OSEnvironment so
    ...
    def _usage_from_result(result: dict[str, Any]) -> dict[str, Any] | None:  # type: ignore[explicit-any]
        """Map Goose's final ``result.usage`` to OmniCraft's usage keys.

        Goose reports ``{totalTokens, inputTokens, outputTokens}``; OmniCraft's
    ...
    async def run_turn(
        ...
        tools: list[Any],  # type: ignore[explicit-any]  # goose runs its own tools; used for the OmniCraft MCP relay
        system_prompt: str,
