...
Wraps a :class:`omnicraft.inner.goose_executor.GooseExecutor`, which drives
...
(:mod:`omnicraft.inner.goose_native_harness`). Goose's mid-turn tool approvals
surface as web elicitation cards (via OmniCraft's TOOL_CALL policy + ``ctx.elicit``
...
``~/.config/goose/config.yaml``); OmniCraft stores no Goose credential. A spec
...
- ``HARNESS_GOOSE_CWD``: working directory for the goose subprocess. ``None``
  falls back to ``OMNICRAFT_RUNNER_WORKSPACE`` then the inherited cwd.
