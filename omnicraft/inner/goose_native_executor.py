"""Executor that bridges OmniCraft web-chat turns into the native Goose TUI.

It does not launch ``goose`` — the ``omnicraft goose`` wrapper already launched
...
from omnicraft.goose_native_bridge import BRIDGE_DIR_ENV_VAR, inject_user_message
...
class GooseNativeExecutor(Executor):
    """Harness-side executor for ``omnicraft goose`` web-UI turns.
