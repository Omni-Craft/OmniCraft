...
turns to maintain conversational context across the OmniCraft session without
re-serialising the full history.

Each turn yields text output as ``TextChunk`` / ``TurnComplete`` events.
OmniCraft policies are enforced on Hermes' native tool calls via Hermes'
...
class HermesExecutor(Executor):
    ...
    history is maintained without OmniCraft re-serializing the full
    message list.
    ...
    ``config.yaml`` that registers an OmniCraft policy hook as a
    ...
    def _hermes_session_id(self, session_key: str) -> str | None:
        """Return the stored Hermes session ID for an OmniCraft session key."""
    ...
    def handles_tools_internally(self) -> bool:
        ...
        through OmniCraft's tool dispatch.  OmniCraft policies are enforced
        via Hermes' native ``pre_tool_call`` shell hook that evaluates
        ``PHASE_TOOL_CALL`` against the OmniCraft server before each tool
        execution.
        """
        return True
    ...
    async def run_turn(
        ...
        :param messages: Conversation history from OmniCraft.
    ...
    def _session_key(self, messages: list[Message]) -> str:
        """
        Derive a stable OmniCraft session key from the message list.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        via `hermes --resume` outside OmniCraft.
        """
