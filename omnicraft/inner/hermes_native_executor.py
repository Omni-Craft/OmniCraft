"""Executor that bridges OmniCraft web-chat turns into the native Hermes TUI.

It does not launch ``hermes`` — the ``omnicraft hermes`` wrapper already launched
...
:class:`omnicraft.inner.goose_native_executor.GooseNativeExecutor`.
"""
...
class HermesNativeExecutor(Executor):
    """Harness-side executor for ``omnicraft hermes`` web-UI turns.
