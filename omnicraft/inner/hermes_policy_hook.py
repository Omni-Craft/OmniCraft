"""Hermes ``pre_tool_call`` shell hook for OmniCraft policy enforcement.
...
The hook evaluates ``PHASE_TOOL_CALL`` policy via the OmniCraft server.
...
def main() -> None:
    ...
    # OmniCraft relay tools are already gated when the relay dispatches them back
    ...
    if tool_name.startswith(("mcp_omnicraft_", "mcp__omnicraft__")):
        json.dump({}, sys.stdout)
        return
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["reason"] = f"Tool '{tool_name}' denied by OmniCraft policy: {reason}"
        else:
            out["reason"] = f"Tool '{tool_name}' denied by OmniCraft policy"
