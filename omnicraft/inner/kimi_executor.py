...
and is managed out-of-band via ``kimi provider add`` (OmniCraft-side
...
class KimiExecutor(Executor):
    """Drive ``kimi -p`` per OmniCraft turn.
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        Mirrors :meth:`omnicraft.inner.qwen_executor.QwenExecutor._sandbox_launch_path`.
    ...
    def _translate_event(self, payload: dict[str, Any]) -> list[ExecutorEvent]:
        """Translate one kimi stream-json line into OmniCraft events.
        ...
        - ``"tool"``: kimi's own tool execution result delivered back to
          its loop. Surfaced as a ``ToolCallComplete`` so the OmniCraft
        ...
        if role == "assistant":
            ...
            # so the OmniCraft UI can render the result. The Session layer
    ...
    async def run_turn(
        ...
        if tools and not self._warned_tools_without_bridge:
            _logger.warning(
                "kimi executor received %d declared tool(s) but OmniCraft has no "
