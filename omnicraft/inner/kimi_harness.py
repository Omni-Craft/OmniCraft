...
:mod:`omnicraft.runtime.harnesses._runner` invokes after the parent
process resolves ``"kimi"`` to this module via
:data:`omnicraft.runtime.harnesses._HARNESS_MODULES`.

Wraps a :class:`omnicraft.inner.kimi_executor.KimiExecutor` that drives
...
``~/.kimi/config.toml`` (out-of-band from OmniCraft) — upstream kimi
has no per-spawn ``--config-file`` or env-var provider override.
OmniCraft-side provider injection remains a deferred follow-up.
"""
...
from omnicraft.inner.kimi_executor import KimiExecutor, _resolve_skills_dirs
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    has its own internal sandbox / approval flow, so OmniCraft does not
...
def _build_kimi_executor() -> Executor:
    ...
    return KimiExecutor(
        ...
        # else the runner's OMNICRAFT_RUNNER_WORKSPACE (the cwd the user launched
