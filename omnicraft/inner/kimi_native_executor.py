"""Executor that bridges OmniCraft web-chat turns into the native Kimi TUI.

It does not launch ``kimi`` — the ``omnicraft kimi`` wrapper already
...
This is a DIFFERENT executor from the headless :class:`~omnicraft.inner.
...
from omnicraft.kimi_native_bridge import BRIDGE_DIR_ENV_VAR, inject_user_message

logger = logging.getLogger(__name__)


class KimiNativeExecutor(Executor):
    """Harness-side executor for ``omnicraft kimi`` web-UI turns.
