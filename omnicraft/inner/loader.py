...
class _OmniCraftYamlLoader(yaml.SafeLoader):
...
_OmniCraftYamlLoader.yaml_implicit_resolvers = {
    key: value[:] for key, value in yaml.SafeLoader.yaml_implicit_resolvers.items()
}
for key, resolvers in list(_OmniCraftYamlLoader.yaml_implicit_resolvers.items()):
    _OmniCraftYamlLoader.yaml_implicit_resolvers[key] = [
...
_OmniCraftYamlLoader.add_implicit_resolver(  # type: ignore[no-untyped-call]
    ...
    *,
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        **calls** it. This is the guard for the untrusted
        agent-bundle upload path: ``omnicraft.spec.load`` routes a
        single-file omnicraft YAML bundle here during
        ...
        ``omnicraft run``, operator specs, the CLI) keep working with
    ...
    if isinstance(path_or_dict, (str, Path)):
        ...
        with open(path) as f:
            data = yaml.load(f, Loader=_OmniCraftYamlLoader)
...
def _read_contained_file(root: Path, value: str) -> str | None:
    ...
    Mirrors :func:`omnicraft.spec.parser._read_contained_file`: resolves
...
def _resolve_instructions(
    ...
    Mirrors :func:`omnicraft.spec.parser._resolve_instructions`
    so omnicraft-flavored YAMLs and native OmniCraft YAMLs treat the
    field identically:
    ...
    - File-path-shaped value that doesn't resolve: silently
      treated as inline text. Matches native OmniCraft behavior so users
...
def _parse_os_env_spec(data: YamlData | str | bool | None) -> OSEnvSpec | None:
    ...
    # Mirror the OmniCraft YAML parser's cross-field validation
    # (omnicraft/spec/parser.py:_parse_os_env). Keeping the two
    ...
    # accepts with weaker semantics than the OmniCraft parser would have
    # allowed).
...
def _effective_terminal_sandbox(
    ...
    :func:`omnicraft.inner.terminal.build_terminal_os_env_spec`: if the
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    if raw_type is None:
        ...
        # (same behavior as the OmniCraft YAML parser, kept in sync so legacy
        # and OmniCraft loaders agree on what an "untyped" sandbox block means).
    ...
    # Mirror the OmniCraft parser's hard reject of ``egress_rules`` paired with
    ...
    # (``omnicraft.spec.parser._parse_credential_proxy``) rather than a
    # second copy so the single-file omnicraft-YAML path and the
    ...
    from omnicraft.spec.parser import (
...
def load_agent_def_from_path(path_str: str) -> AgentDef:
    ...
    - ``foo.yaml`` (single-file omnicraft YAML) — parsed directly.
    - ``my-agent/`` (omnicraft AGENTSPEC directory with
      ``config.yaml``, or a directory containing exactly one
      omnicraft YAML at its root).
    ...
    from omnicraft.spec import _find_omnicraft_yaml_in_dir

    if path.is_dir():
        resolved_dir = path
        cleanup: tempfile.TemporaryDirectory[str] | None = None
    else:
        from omnicraft.spec.tar_utils import extract_safe

        cleanup = tempfile.TemporaryDirectory(prefix="omnicraft-bundle-")
    ...
    try:
        ...
        if config_yaml.is_file():
            from omnicraft.spec import load as load_agent_spec
        ...
        single = _find_omnicraft_yaml_in_dir(resolved_dir)
        if single is None:
            raise FileNotFoundError(
                f"{path}: directory has neither a config.yaml "
                f"nor a single-file omnicraft YAML at its root"
            )
