...
OmniCraft as the system of record for tools, policies, and session history.
OmniCraft tools are exposed as SDK FunctionTools whose callbacks route back
through the Session's tool registry.

Conversation state is delegated to the SDK's session layer. Each OmniCraft
Session gets its own SDK SQLiteSession keyed by the OmniCraft session id.
"""
...
from omnicraft.reasoning_effort import OPENAI_AGENTS_EFFORTS, validate_effort
...
# Tool executor callable wired in by ``omnicraft.Session``. The result
# is JSON-ish (dict[str, Any]) but the static type leaks ``Any`` through
# the kwargs dict. Constrained here to match omnicraft.executor.
...
# to the OmniCraft tool executor.
...
def _normalize_responses_items_for_chat(
    ...
    :func:`~omnicraft.inner.open_responses_sdk._convert_messages_to_responses`
...
def _normalize_content_blocks_for_chat(
    ...
    OmniCraft-only metadata (for example ``filename``) from known Responses
    content block types before the OpenAI Agents SDK serializes them to the
    provider API.

    Background: :mod:`omnicraft.runtime.content_resolver` resolves uploaded
    file IDs to inline ``data:<mime>;base64,<data>`` URIs stored in
    ``file_data`` / ``image_url`` while preserving UI/history metadata such as
    ``filename``.  That metadata is useful in OmniCraft, but the OpenAI API
    ...
    for block in content:
        ...
        if block_type == "input_file":
            ...
            # Preserve inline images; strip OmniCraft-only metadata.
...
def _copy_known_keys(block: dict[str, Any], keys: tuple[str, ...]) -> dict[str, Any]:
    ...
    OmniCraft content blocks can carry UI/history metadata such as
...
def _patch_openai_sse_keepalive_tolerance() -> None:
    ...
    if decoder is None or getattr(decoder, "_omnicraft_keepalive_patch", False):
        return
    ...
    decoder._omnicraft_keepalive_patch = True  # type: ignore[attr-defined]
...
def _get_openai_async_client(
    ...
    try:
        ...
        raise ImportError(
            ...
            "the package (`pip install 'omnicraft[databricks]'`) or set "
    ...
    def __init__(
        ...
        *,
        ...
        :param base_url_override: Override the OpenAI-compatible base URL
            ...
            the OmniCraft workflow layer). Required whenever ``gateway_host`` is set.
