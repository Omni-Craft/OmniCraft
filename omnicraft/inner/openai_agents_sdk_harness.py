...
via :data:`omnicraft.runtime.harnesses._HARNESS_MODULES`.

The registry key is ``"openai-agents"`` (matching the OmniCraft
YAML ``executor.harness`` spelling and ``OmniCraftExecutor``'s
...
around a :class:`omnicraft.inner.openai_agents_sdk_executor.OpenAIAgentsSDKExecutor`
...
- ``HARNESS_OPENAI_AGENTS_DATABRICKS_PROFILE``: Databricks-specific
  ...
  builder
  :func:`omnicraft.runtime.workflow._build_openai_agents_sdk_spawn_env`
