"""Executor that bridges OmniCraft web turns into a native OpenCode session.

Built on :class:`omnicraft.native_server_harness.NativeServerHarness`: the
runner owns the ``opencode serve`` process + SSE forwarder, and this
executor injects the latest web turn over the
:class:`omnicraft.opencode_http_transport.OpenCodeHttpTransport` using the
...
from omnicraft.native_server_harness import NativeServerHarness
from omnicraft.native_server_transport import NativePrompt
from omnicraft.opencode_http_transport import OpenCodeHttpTransport
...
class OpenCodeNativeExecutor(NativeServerHarness):
    """
    Harness-side executor for ``omnicraft opencode`` web UI turns.
