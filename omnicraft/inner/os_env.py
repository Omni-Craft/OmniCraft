...
from omnicraft._platform import IS_WINDOWS, WINDOWS_ENV_PASSTHROUGH
...
# mirror the pattern in ``omnicraft/sandbox.py`` and ``omnicraft/uc_tools.py``.
...
_DEFAULT_ENV_PASSTHROUGH: tuple[str, ...] = (
    ...
    "PYTHONFAULTHANDLER",
    # OmniCraft session marker: always pass the "inside OmniCraft" marker
    ...
    # the runner via runner.identity.OMNICRAFT_SESSION_ENV_VAR.
    OMNICRAFT_SESSION_ENV_VAR,
...
def build_helper_env(
    ...
    (:data:`~omnicraft.runner.identity.RUNNER_AUTH_SECRET_ENV_VARS`): the
...
def create_os_environment(spec: OSEnvSpec | None) -> OSEnvironment | None:
    ...
    if spec.fork:
        fork_dir = Path(tempfile.mkdtemp(prefix="omnicraft-fork-"))
    ...
    if isinstance(token_value, str) and token_value:
        ...
        for env_key in ("HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"):
            ...
            if not parts.hostname:
                continue
            netloc = f"omnicraft:{token_value}@{parts.hostname}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        raise SystemExit("usage: python -m omnicraft.inner.os_env <helper|launch> ...")
    ...
    if command == "helper":
        ...
        if len(args) != 3 or args[1] not in ("--config-fd", "--config-file"):
            raise SystemExit(
                "usage: python -m omnicraft.inner.os_env helper "
    ...
    if command == "launch":
        if len(args) < 3:
            raise SystemExit(
                "usage: python -m omnicraft.inner.os_env launch <sandbox> <target> [args...]"
            )
