...
event stream into OmniCraft ExecutorEvents.

OmniCraft tools are bridged into Pi via a generated JavaScript extension
that registers each tool with ``pi.registerTool()``.  Tool execution is
proxied over a local TCP socket to the OmniCraft Python process, so
policies, history recording, sub-agents, runtime, and all other OmniCraft
...
from omnicraft.inner.native_attachments import parse_data_uri
...
# ---------------------------------------------------------------------------
# TCP tool server — serves OmniCraft tools to the Pi extension
# ---------------------------------------------------------------------------
...
class _ToolServer:
    ...
    async def _evaluate_policy(  # type: ignore[explicit-any]
        ...
        OmniCraft outage can't wedge the agent mid-turn.
        """
...
# ---------------------------------------------------------------------------
# Pi extension generator — bridges OmniCraft tools into Pi
# ---------------------------------------------------------------------------
...
def _generate_extension_js(port: int, tool_schemas: list[ToolSpec], token: str) -> str:
    """Generate a JavaScript Pi extension that registers OmniCraft tools.

    Each tool is forwarded to the TCP tool server at ``127.0.0.1:<port>``.

    :param port: TCP port the OmniCraft tool server is listening on, e.g.
        ``54321``.
    :param tool_schemas: OmniCraft tool schemas to register with Pi.
    ...
    return f"""\
// Auto-generated OmniCraft tool bridge extension for Pi.
// Connects to the OmniCraft TCP tool server on port {port}.
...
/**
 * Ask the OmniCraft tool server for a TOOL_CALL policy verdict on a native
...
module.exports = function(pi) {{
  // Gate native (non-bridged) tool calls through OmniCraft policy. Pi's
  ...
  pi.on("tool_call", async (event) => {{
    ...
    if (verdict && verdict.block) {{
      return {{ block: true, reason: verdict.reason || "blocked by OmniCraft policy" }};
    }}
  }});

  for (const tool of TOOLS) {{
    // Pi passes tool.parameters directly to the LLM as JSON Schema, so we
    // can use the OmniCraft schema as-is without TypeBox conversion.
    pi.registerTool({{
      name: tool.name,
      label: tool.name,
...
async def _create_subprocess_exec(*args: Any, **kwargs: Any) -> asyncio.subprocess.Process:  # type: ignore[explicit-any]
    ...
    ``omnicraft.inner.pi_executor.asyncio.create_subprocess_exec``
...
def _clean_pi_env(extra_allowed: Sequence[str] | None = None) -> dict[str, str]:
    ...
    :func:`omnicraft.inner.codex_executor._clean_codex_env`. The previous
...
def _build_pi_prompt(
    ...
    :param messages: OmniCraft conversation history for the
        turn.
...
def _extract_pi_turn_usage(
    ...
    carries the resolved ``model``. This translates those into omnicraft's
    usage schema (see :class:`omnicraft.inner.executor.TurnComplete`).
...
def _aggregate_pi_turn_usage(
    ...
    A single OmniCraft turn drives Pi's full agent loop, which may make
...
class PiExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        :param gateway_host: Gateway workspace host origin, e.g.
            ...
            ``HARNESS_PI_GATEWAY_HOST`` (written by the OmniCraft workflow layer).
    ...
    async def _ensure_tool_server(self, tools: list[ToolSpec]) -> int | None:
        """Start the TCP tool server if there are OmniCraft tools to bridge."""
        if not tools:
            return None
    ...
    async def _gate_native_tool(  # type: ignore[explicit-any]
        ...
        """Evaluate a native Pi tool call against OmniCraft TOOL_CALL policy.
        ...
        PreToolUse hooks: the verdict is computed by the OmniCraft server
    ...
    def _build_env_and_dir(
        ...
        :param tools: OmniCraft tool schemas to bridge into Pi.
        :param tool_server_port: TCP port the OmniCraft tool server is
        ...
        """
        env = dict(self._env)
        tmp_dir = tempfile.mkdtemp(prefix="omnicraft_pi_")
        ...
        if self._gateway:
            ...
            from omnicraft.inner.pi_settings import prepare_managed_pi_agent_dir
        ...
        # Generate the OmniCraft tool bridge extension if tools are available.
        if tools and tool_server_port is not None:
            ...
            ext_path = os.path.join(tmp_dir, "omnicraft_tools.js")
            ...
            # an OmniCraft TOOL_CALL policy verdict; see
