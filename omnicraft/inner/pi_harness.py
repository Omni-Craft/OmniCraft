...
around a :class:`omnicraft.inner.pi_executor.PiExecutor`
...
- ``HARNESS_PI_CWD``: working directory the executor launches
  the Pi CLI in. ``None`` falls back to ``OMNICRAFT_RUNNER_WORKSPACE`` if set,
...
from omnicraft.inner.pi_executor import PiExecutor
...
def _build_pi_executor() -> Executor:
    ...
    return PiExecutor(
        cwd=os.environ.get(_ENV_CWD) or os.environ.get("OMNICRAFT_RUNNER_WORKSPACE"),
