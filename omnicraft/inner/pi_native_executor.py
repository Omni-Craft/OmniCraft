"""Executor that bridges OmniCraft messages into a native Pi TUI."""
...
class PiNativeExecutor(Executor):
    """
    Harness-side executor for ``omnicraft pi`` web UI turns.

    The native Pi process is already running in the session terminal with
    the OmniCraft Pi extension loaded. Each turn queues the latest user
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from OmniCraft. Ignored for now; native
