...
class QwenExecutor(Executor):
    ...
    def __init__(
        ...
        :param gateway_base_url: OpenAI-compatible base URL of an OmniCraft
        ...
        # ``session/request_permission`` routes through OmniCraft's TOOL_CALL
        ...
        # Adapter-injected tool bridge + the OmniCraft-tool MCP relay it backs.
        # Exposes OmniCraft builtin tools to qwen via session/new.mcpServers (the
        ...
        self._mcp = OmniCraftAcpMcp(label="qwen")
    ...
    async def _start_process(self) -> None:
        ...
        env = os.environ.copy()
        # Translate OmniCraft's provider/gateway routing into the OpenAI-compatible
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        Mirrors :func:`omnicraft.inner.pi_executor._try_sandbox_pi`. When
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``session/request_permission`` — decide via OmniCraft's TOOL_CALL
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here. Executed through the OmniCraft OSEnvironment
          so the spec's sandbox read/write roots are enforced at the Python
          layer and the I/O flows through OmniCraft rather than qwen touching
    ...
    def _accumulate_usage(  # type: ignore[explicit-any]
        ...
        ``MessageEmitter.emitUsageMetadata``). A single OmniCraft turn can drive
    ...
    async def run_turn(
        ...
        tools: list[Any],  # type: ignore[explicit-any]  # qwen runs its own tools; used for the OmniCraft MCP relay
        system_prompt: str,
        ...
        """
        # Captured for the OmniCraft MCP relay set up lazily at session/new.
