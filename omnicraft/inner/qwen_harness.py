...
around a :class:`omnicraft.inner.qwen_executor.QwenExecutor`
...
- ``HARNESS_QWEN_CWD``: working directory the executor launches
  the Qwen CLI in. ``None`` falls back to ``OMNICRAFT_RUNNER_WORKSPACE`` if set,
...
def _build_qwen_executor() -> Executor:
    ...
    """
    cwd_raw = os.environ.get(_ENV_CWD) or os.environ.get("OMNICRAFT_RUNNER_WORKSPACE")
    cwd = cwd_raw or None
