"""Executor that bridges OmniCraft web-chat turns into the native qwen TUI.

It does not launch ``qwen`` — the ``omnicraft qwen`` wrapper already launched the
...
:mod:`omnicraft.qwen_native_forwarder` mirrors the JSON event stream.
...
class QwenNativeExecutor(Executor):
    """Harness-side executor for ``omnicraft qwen`` web-UI turns.
