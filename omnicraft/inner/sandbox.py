...
from omnicraft.runner.identity import RUNNER_AUTH_SECRET_ENV_VARS
...
_SANDBOX_STRACE_ENV = "OMNICRAFT_SANDBOX_STRACE"
...
class SandboxPolicy:
    ...
    :param deny_unix_socket_paths: Absolute paths to AF_UNIX (pathname)
        ...
        ``[Path("/tmp/omnicraft-terminal-ab12/tmux.sock")]``.
    ...
    dropped — see :func:`omnicraft.inner.os_env._HelperProcessClient.
...
class SandboxBackend(ABC):
    ...
    def wrap_launcher_argv(
        ...
        Called by :class:`omnicraft.inner.os_env._HelperProcessClient`
        ...
        :param argv: The unwrapped command argv the parent intends to
            ...
            "omnicraft.inner.os_env", "helper", "<encoded>"]``.
    ...
    def post_spawn(self, policy: SandboxPolicy, pid: int) -> ContainmentHandle | None:
        ...
        Called by :class:`omnicraft.inner.os_env._HelperProcessClient` on
...
def run_launcher(encoded_sandbox: str, target_path: str, argv: list[str]) -> int:
    ...
    *argv* and returns its exit code.
    ...
    logger.info(
        "[omnicraft-sandbox] activating backend=%s active=%s target=%s",
        sandbox.backend_type,
        sandbox.active,
        target_path,
    )
    ...
    if os.environ.get(_SANDBOX_STRACE_ENV):
        ...
        if strace_bin is None:
            ...
            logger.warning(
                "[omnicraft-sandbox] strace active; wrapping target with %s",
                strace_bin,
            )
    try:
        activate_sandbox(sandbox)
        logger.info("[omnicraft-sandbox] activated; spawning target=%s", target_path)
        completed = subprocess.run(target_argv)
        logger.info("[omnicraft-sandbox] target exited rc=%s", completed.returncode)
...
def create_exec_launcher(target_path: str, sandbox: SandboxPolicy) -> str:
    ...
    fd, path = tempfile.mkstemp(prefix="omnicraft-sandbox-", suffix=".py")
    project_root = repr(str(_project_root()))
...
def get_backend(type_name: str) -> SandboxBackend:
    ...
    (:meth:`omnicraft.inner.os_env._HelperProcessClient._start_locked`)
...
def _default_sandbox_for_platform() -> OSEnvSandboxSpec:
    ...
    (e.g. :func:`omnicraft.inner.loader._parse_os_env_sandbox_spec`
    and :func:`omnicraft.spec.parser._parse_os_env_sandbox`) to fill
    ...
    :meth:`omnicraft.inner.bwrap_sandbox.BwrapSandboxBackend.resolve`,
...
def _project_root() -> Path:
    # File lives at omnicraft/inner/sandbox.py; climb two levels to the
