"""Shared terminal-output styling for the OmniCraft CLI.
...
that ``omnicraft``'s output reads as one coherent product. See
...
* Machine-readable output (IDs, paths, config dumps, the ``version``
  string) goes to stdout via :data:`console`, so ``omnicraft … | cat``
  stays clean.
...
#: ``OMNICRAFT_NO_SPINNER`` convention in :mod:`omnicraft._runner_startup`.
NO_BANNER_ENV_VAR = "OMNICRAFT_NO_BANNER"
...
console = Console(theme=OMNICRAFT_THEME, highlight=False)
#: Console for stderr — status, warnings, errors, and the brand banner.
err_console = Console(stderr=True, theme=OMNICRAFT_THEME, highlight=False)
...
def show_banner(*, isatty: bool | None = None, env: dict[str, str] | None = None) -> bool:
    ...
    and can be force-disabled with ``OMNICRAFT_NO_BANNER``. Color *within*
...
def print_landing(
    *,
    ...
    banner is suppressed (non-TTY or ``OMNICRAFT_NO_BANNER``).
...
def print_brandmark(subtitle: str | None = None) -> None:
    """
    Print the compact one-line brandmark (``✦ omnicraft``), TTY-gated.
