"""The OmniCraft brand wordmark and Otto lockup for CLI output.

A bold "ANSI-Shadow" block-letter ``omnicraft`` wordmark — the canonical
...
from :mod:`omnicraft.inner.mascots`, which it pairs with 1:1.
...
banner (TTY gating, ``OMNICRAFT_NO_BANNER``) lives one layer up in
:mod:`omnicraft.inner.ui`, which is the only module that should be imported
...
The brand color is Otto's magenta-pink ``#0fb5bd`` (see
:data:`omnicraft.inner.mascots.MASCOT_ART_COLOR`); the optional gradient
...
_GRADIENT_START = (0x0F, 0xB5, 0xBD)  # #0fb5bd
_GRADIENT_END = (0x9F, 0xEC, 0xEF)  # #9fecef
...
#: The rows of the ``omnicraft`` wordmark, as plain (uncolored) text.
...
def render_lockup(
    ...
    *,
    ...
    :param epilogue: Optional aligned label/value rows printed beneath the
        art, e.g. ``[("Version", "0.4.2"), ("Next", "omnicraft setup")]``.
    :returns: None.
    """
...
def render_compact(console: Console, *, subtitle: str | None = None) -> None:
    """
    Print the one-line brandmark: ``✦ omnicraft  <subtitle>``.
    ...
    line.append("omnicraft", style=f"bold {WORDMARK_COLOR}")
    if subtitle:
