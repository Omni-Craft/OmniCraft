"""Native Kimi TUI wrapper for the OmniCraft CLI.

``omnicraft kimi`` launches the Kimi CLI's interactive TUI (``kimi``
with no args) inside an OmniCraft-runner-owned tmux terminal and attaches the
local TTY — the kimi analog of ``omnicraft codex`` / ``omnicraft pi``. The runner
spawns the process (see :func:`omnicraft.runner.app._auto_create_kimi_terminal`);
...
from omnicraft._wrapper_labels import KIMI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIMI_PATH_ENV = "OMNICRAFT_KIMI_PATH"
...
def run_kimi_native(
    *,
    ...
    """
    Launch the Kimi TUI in an OmniCraft terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Kimi requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_kimi_native."
        )
    with TemporaryDirectory(prefix="omnicraft-kimi-native-") as tmpdir:
...
def _materialize_kimi_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``omnicraft kimi``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Kimi on an OmniCraft server via a daemon-spawned runner.
