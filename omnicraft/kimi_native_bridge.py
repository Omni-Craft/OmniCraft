...
# the active OmniCraft session). Written by the runner at terminal-create time;
# read by :mod:`omnicraft.kimi_native_hook` (PreToolUse deny-gate + the
...
_PASTE_BUFFER = "omnicraft-kimi-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/omnicraft-<uid>/kimi-native/<hash>``."""
...
def write_hook_config(
    ...
    *,
    ...
    """Record the OmniCraft routing details the kimi hook subprocess reads.
    ...
    :func:`omnicraft.claude_native_bridge` ``permission_hook.json`` plumbing.

    :param bridge_dir: The kimi-native bridge dir.
    :param server_url: OmniCraft server base URL, e.g. ``"http://127.0.0.1:8787"``.
    :param headers: Auth headers to replay on the hook's POSTs (may be empty).
    :param session_id: The OmniCraft session the hook events belong to.
    """
    _ensure_dir(bridge_dir)
    payload = {
...
def read_hook_config(bridge_dir: Path) -> dict[str, Any]:
    """Read OmniCraft routing details for the kimi hook subprocess.
...
def read_active_session_id(bridge_dir: Path) -> str | None:
    """Return the OmniCraft session id recorded for the hook subprocess.
...
#: "Approve once" re-prompts each call so OmniCraft governs every one.
APPROVE_KEY = "1"
DENY_KEY = "3"
