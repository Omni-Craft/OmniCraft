"""Per-session ``KIMI_CODE_HOME`` builder that injects OmniCraft hooks.
...
- carries a ``config.toml`` that is the user's config text with two OmniCraft
  ``[[hooks]]`` appended — a ``PreToolUse`` deny-gate and a ``PermissionRequest``
  read-only surface, both dispatched to :mod:`omnicraft.kimi_native_hook`.
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    """Render the two OmniCraft ``[[hooks]]`` entries as TOML text.

    Both hooks dispatch to :mod:`omnicraft.kimi_native_hook` with the bridge
    ...
    :param python_executable: Interpreter to run the hook module; ``None`` uses
        :data:`sys.executable` (the runner's interpreter, which has omnicraft).
    ...
    # ``sys.path[0]``. A workspace that contains its own ``omnicraft/`` directory
    ...
    # from the path, importing only the interpreter's own omnicraft. Mirrors
    # claude-native's ``python -I -m omnicraft.claude_native_hook``.
    base = f"{shlex.quote(python)} -I -m omnicraft.kimi_native_hook"
    ...
    return (
        "\n"
        "# --- OmniCraft native hooks (auto-generated; do not edit) ---\n"
        "[[hooks]]\n"
        'event = "PreToolUse"\n'
        f'command = "{pre}"\n'
        "timeout = 600\n"
...
def build_kimi_session_home(
    ...
    *,
    ...
    """Materialize a session-scoped ``KIMI_CODE_HOME`` with OmniCraft hooks.
    ...
    the user's config plus the OmniCraft hooks. Best-effort and idempotent:
