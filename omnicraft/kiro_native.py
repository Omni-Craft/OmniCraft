"""Native Kiro TUI wrapper for the OmniCraft CLI."""
...
from omnicraft._wrapper_labels import KIRO_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIRO_PATH_ENV = "OMNICRAFT_KIRO_PATH"
...
def kiro_base_model_options() -> list[dict[str, Any]]:
    ...
    Mirrors :func:`omnicraft.cursor_native.cursor_base_model_options`: each option
...
def run_kiro_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Kiro requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_kiro_native."
        )
    with TemporaryDirectory(prefix="omnicraft-kiro-native-") as tmpdir:
...
def _materialize_kiro_agent_spec(tmpdir: Path, *, model: str | None = None) -> Path:
    """Write the terminal-first agent spec used by ``omnicraft kiro``."""
...
def _run_with_remote_server(
    ...
    *,
    ...
    """Launch Kiro on an OmniCraft server via a daemon-spawned runner."""
