...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"omnicraft-{os.getuid()}" / "kiro-native"
_TMUX_FILE = "tmux.json"
_FORWARDER_READY_FILE = "kiro_session_forwarder_ready.json"
_ACP_RECORD_FILE = "kiro_acp_record.jsonl"
# Shared OmniCraft MCP relay (serve-mcp) registration for kiro.
...
_PASTE_BUFFER = "omnicraft-kiro-paste"
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared OmniCraft MCP bridge requires at boot.

    ``serve-mcp`` (``omnicraft.claude_native_bridge``) reads ``bridge.json`` and
...
def build_kiro_mcp_config(
    ...
    """Build the kiro ``mcpServers`` entry for the OmniCraft relay MCP server.
    ...
    per-server auto-approve field, so OmniCraft MCP tool calls surface through
...
def write_kiro_workspace_mcp_config(
    ...
    *,
    ...
    """Write the workspace-scoped kiro MCP config declaring the OmniCraft server.
    ...
    ``.cursor/mcp.json``). The OmniCraft entry is *merged* into any existing
...
def inject_interrupt(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    :func:`omnicraft.goose_native_bridge.inject_interrupt`.
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`omnicraft.goose_native_bridge.kill_session`.
