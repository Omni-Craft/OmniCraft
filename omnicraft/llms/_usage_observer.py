...
2. **Auto-recorder** that activates whenever
   ``OMNICRAFT_TOKEN_USAGE_JSON=<path>`` is set in the environment.
...
- :meth:`omnicraft.llms.client._ResponsesNamespace.create` (in-process
  LLM client).
...
runner, harness) inherit both ``OMNICRAFT_TOKEN_USAGE_JSON`` and
...
_ENV_VAR = "OMNICRAFT_TOKEN_USAGE_JSON"
...
def _current_test_path() -> Path | None:
    ...
    Derived from the ``OMNICRAFT_TOKEN_USAGE_JSON`` base path plus the
