...
def get_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    :raises OmniCraftError: If the provider is not supported.
    """
...
def _create_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    if provider in openai_compat_providers:
        ...
        from omnicraft.llms.adapters.openai import OpenAICompatibleAdapter

        return OpenAICompatibleAdapter(base_url=resolved_url)

    if provider == "anthropic":
        from omnicraft.llms.adapters.anthropic import AnthropicAdapter
    ...
    if provider == "bedrock":
        from omnicraft.llms.adapters.bedrock import BedrockAdapter
    ...
    if provider == "databricks":
        from omnicraft.llms.adapters.databricks import DatabricksAdapter
