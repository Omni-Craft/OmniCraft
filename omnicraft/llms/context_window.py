...
# OmniCraft's authoritative context-window registry, consulted BEFORE litellm
...
def _registry_context_window(model: str) -> int | None:
    """Resolve a model's context window from OmniCraft's authoritative registry.
...
def _fetch_context_window_from_mlflow(model: str) -> int | None:
    ...
    """
    if os.environ.get("OMNICRAFT_DISABLE_CATALOG_LOOKUP") == "1":
        return None
...
def get_model_context_window(model: str) -> int:
    ...
    2. :func:`_registry_context_window` — OmniCraft's own authoritative
