...
precedence as :func:`omnicraft.runtime.workflow._resolve_provider_for_build`
...
(:func:`omnicraft.model_override.normalize_model_for_provider`).
...
from omnicraft._platform import default_shell_argv
...
# ``omnicraft/llms/adapters/anthropic.py``).
...
def model_family_token(model_id: str) -> str:
    ...
    :func:`omnicraft.model_override.model_family_mismatch`: Claude ids
...
def resolve_model_provider(spec: Any, harness: str | None) -> ResolvedModelProvider:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    try:
        ...
        # OmniCraftError text is this codebase's own (secret-free); anything
        # else is redacted to its type name — raw detail stays at DEBUG.
        reason = str(exc) if isinstance(exc, OmniCraftError) else type(exc).__name__
...
def _resolve_model_provider_unsafe(spec: Any, harness: str | None) -> ResolvedModelProvider:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    Step 1 reuses :func:`~omnicraft.runtime.workflow._resolve_provider_for_build`
    ...
    from omnicraft.runtime.workflow import _resolve_provider_for_build
...
def _legacy_claude_sdk_provider(spec: Any) -> ResolvedModelProvider:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    :returns: A :class:`ResolvedModelProvider`.
    """
    from omnicraft.onboarding.configure_models import default_base_url_for_family
    from omnicraft.runtime.workflow import _load_global_auth
    from omnicraft.spec.types import ApiKeyAuth, DatabricksAuth
...
def list_models_for_worker(
    ...
    *,
    ...
    family rule from :func:`~omnicraft.model_override.model_family_mismatch`
