"""Shared conversion between native-harness hooks and OmniCraft policy events.
...
entrypoints (:mod:`omnicraft.claude_native_hook`,
:mod:`omnicraft.codex_native_hook`) share one implementation.
...
# ``omnicraft.runner.app._evaluate_policy_via_omnicraft``.
_EVAL_UNAVAILABLE_REASON = (
    "OmniCraft policy evaluation unavailable (could not reach or authenticate to the "
    "OmniCraft server); failing closed for this tool call."
)
_EVAL_UNAVAILABLE_REQUEST_REASON = (
    "OmniCraft policy evaluation unavailable (could not reach or authenticate to the "
    "OmniCraft server); failing closed for this request."
)
...
_AUTH_HEADERS_ENV = "_OMNICRAFT_AUTH_HEADERS"
...
def policy_hook_wrapper_script(server_url: str, session_id: str, hook_script_path: str) -> str:
    ...
    Resolves a one-shot OmniCraft-server token and bakes the auth +
    workspace-routing headers (via
    :func:`omnicraft.cli_auth.databricks_request_headers`) into
    ...
    :param server_url: OmniCraft server base URL the hook posts to.
    ...
    return (
        "#!/bin/sh\n"
        f"export _OMNICRAFT_SERVER_URL={shlex.quote(server_url)}\n"
        f"export _OMNICRAFT_SESSION_ID={shlex.quote(session_id)}\n"
...
class PolicyHookReauth:
    """Callable that re-mints the OmniCraft bearer for a policy hook subprocess.
...
def _is_login_redirect_or_unauthorized(response: httpx.Response) -> bool:
    ...
    Mirrors :func:`omnicraft.runner._entry._is_login_redirect_or_unauthorized`,
...
def hook_payload_to_evaluation_request(
    ...
    ``prompt`` field becomes the request content). OmniCraft MCP tools
    (``mcp__omnicraft__*``) are skipped because they are already
    policy-checked by the relay path (``ProxyMcpManager`` → OmniCraft
    ...
    :returns: An ``EvaluationRequest`` dict suitable for POSTing to
        ...
        policy-relevant (unknown event or an ``mcp__omnicraft__*`` tool).
    """
    ...
    # OmniCraft MCP tools are already policy-checked by the relay path
    ...
    if isinstance(tool_name, str) and tool_name.startswith("mcp__omnicraft__"):
        return None
...
def fail_closed_hook_output(
    ...
    :data:`omnicraft.policies.types.FAIL_CLOSED_PHASES` (the runner-side
...
def post_evaluate_with_retry(
    ...
    """
    POST to the OmniCraft policy evaluate endpoint, retrying on transient errors.
    ...
    A stable ``_omnicraft_elicitation_id`` is minted once and stamped on
    ...
    :param headers: Auth headers for the OmniCraft server.
    ...
    :param reauth: Optional callable that re-mints fresh auth headers when
        ...
        refresh-capable :class:`~omnicraft.runner._entry._RunnerDatabricksAuth`.
    ...
    request_body = {**eval_request, "_omnicraft_elicitation_id": elicitation_id}
    ...
    while True:
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"omnicraft {hook_label}: OmniCraft auth expired "
            ...
            if status < 500:
                print(
                    f"omnicraft {hook_label}: OmniCraft returned {status}"
                    + (f": {body_preview}" if body_preview else ""),
                    file=sys.stderr,
                )
                return None, last_error
            print(
                f"omnicraft {hook_label}: OmniCraft returned {status}; retrying",
                file=sys.stderr,
            )
        except (httpx.ConnectError, httpx.ConnectTimeout) as exc:
            last_error = f"connection error: {exc}"
            print(
                f"omnicraft {hook_label}: OmniCraft request failed; retrying: {exc}",
                file=sys.stderr,
            )
            ...
            print(
                f"omnicraft {hook_label}: OmniCraft request failed: {exc}",
                file=sys.stderr,
            )
            return None, last_error
        if time.monotonic() + backoff_s >= deadline:
            print(
                f"omnicraft {hook_label}: retry budget exhausted",
                file=sys.stderr,
            )
