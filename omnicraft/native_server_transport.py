...
(:class:`omnicraft.opencode_http_transport.OpenCodeHttpTransport`);
:class:`NativeServerTransport` is the seam so the orchestration in
:class:`omnicraft.native_server_harness.NativeServerHarness` stays
protocol-agnostic.
"""
...
class NativeLaunchConfig:
    ...
    :param omnicraft_session_id: OmniCraft conversation id, e.g.
        ``"conv_abc123"``.
...
class NativeServerTransport(Protocol):
    ...
    :class:`~omnicraft.native_server_harness.NativeServerHarness` calls only
    these methods.
    """
