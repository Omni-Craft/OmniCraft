---
name: build-omnicraft
description: Patterns and templates for generating valid OmniCraft agent directories. Load when ready to create files.
---
...
that `omnicraft server` uses. If your environment exposes the
...
**`omnicraft-knowledge`** skill if you need the deeper field reference
...
- `guardrails` — runtime policy gates (see `omnicraft-knowledge`).

## Step 2a: Choose an executor

`executor.type` must be one of **`claude_sdk`**, **`agents_sdk`**, or
**`omnicraft`**. There is **no `llm` executor** — do not use it.
...
| A CLI/coding harness, shell + file tools, sub-agents | `omnicraft` + a `config.harness` |

When `executor.type: omnicraft`, **`config.harness` is required** and must
...
only when asked; see `omnicraft-knowledge` for `executor.model` / `auth`.
...
Use the `omnicraft` executor with a coding harness when the agent needs to
...
Sub-agents need the `omnicraft` executor (it provides the spawn tools).
...
- [ ] `executor.type` is one of `claude_sdk`, `agents_sdk`, `omnicraft`
- [ ] If `executor.type: omnicraft`, `executor.config.harness` is set to a
      valid harness
- [ ] `instructions` (or `prompt`) points to a file that exists or is
      inline text
- [ ] When declaring `tools.agents`, the parent uses `executor.type:
      omnicraft`, and each entry has a matching `agents/` subdirectory
