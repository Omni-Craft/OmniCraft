---
name: omnicraft-knowledge
description: Deep reference on OmniCraft config format, executor types, skill/tool structure, and conventions. Load when you need to look up how the platform works.
---

# OmniCraft Knowledge Base
...
executor:
  # REQUIRED area. type must be one of: claude_sdk | agents_sdk | omnicraft.
  ...
  # type: omnicraft     — subprocess harness; requires config.harness below

  # Only for type: omnicraft — pick the harness that runs the loop.
...
| `omnicraft` | Coding/CLI harnesses, shell + file tools, sub-agents | Spawns a subprocess harness selected by `config.harness` |

**There is no `llm` executor type** — the only valid values are
`claude_sdk`, `agents_sdk`, and `omnicraft`. For **most new simple agents**,
use `claude_sdk` (or `agents_sdk`) — in-process, no extra config. Use
`omnicraft` when the agent needs a specific harness, shell/file access, or
...
from omnicraft.tools import tool
...
`executor.type: omnicraft` — that's what provides the spawn tools. Each
sub-agent is a full agent and may use any executor (`claude_sdk`,
`agents_sdk`, or `omnicraft`).
