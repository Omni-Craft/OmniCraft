"""Antigravity Gemini API-key credential storage for ``omnicraft setup``.
...
the omnicraft secret store and is referenced from a dedicated top-level
``antigravity:`` block in ``~/.omnicraft/config.yaml`` (``keychain:`` / ``env:``),
...
:mod:`omnicraft.onboarding.cursor_auth`.
"""
...
def antigravity_sdk_installed() -> bool:
    ...
    :func:`omnicraft.onboarding.databricks_config.databricks_sdk_installed`: uses
...
def install_antigravity_sdk() -> bool:
    ...
    :func:`omnicraft.onboarding.harness_install.install_harness_cli`.
...
def antigravity_api_key_settings(ref: str) -> dict[str, object]:
    ...
    :returns: The settings dict for :func:`omnicraft.cli._save_global_config`.
    """
