"""Rendering + flow helpers for ``omnicraft setup --no-internal-beta``.

The user-facing model-selection surface (chunk 2b of
``designs/oss-cuj/04-model-selection-implementation.md``) has two pieces:

- the ``omnicraft setup --no-internal-beta`` CLI command (interactive
  add/set-default/remove + a scriptable ``list`` subcommand), and
- the ``/model`` REPL readout/switch (in :mod:`omnicraft.repl._repl`).
...
:mod:`omnicraft.cli`; this module owns the look/feel and the
...
from omnicraft.onboarding.interactive import ACCENT, console
...
# see omnicraft.inner.banner._display_width.)
...
# :func:`omnicraft.onboarding.providers.get_all_providers`) to the
# omnicraft family it serves for a ``key`` provider. Anthropic serves the
# Claude (anthropic) surface; OpenAI and OpenAI-compatible vendors serve
# the Codex (openai) surface. Providers absent here have no omnicraft
...
def key_providers() -> list[str]:
    ...
    omnicraft family. Only providers in :data:`_CATALOG_PROVIDER_FAMILY`
...
def render_provider_listing_by_harness(
    ...
    several surfaces is listed under each. Used by ``omnicraft config list``.
    ...
    :param providers: Parsed provider entries keyed by name (from
        :func:`~omnicraft.onboarding.provider_config.load_providers`).
...
def render_provider_listing(
    ...
    Prints (via the shared :data:`~omnicraft.onboarding.interactive.console`)
    ...
    :param detected: Ambient detections from
        :func:`~omnicraft.onboarding.ambient.detect_providers`; entries
...
def build_bedrock_provider_entry(
    ...
    native ``omnicraft claude`` terminal in AWS Bedrock mode (the in-process /
