"""Cursor API-key credential storage for ``omnicraft setup`` and the runtime.

Cursor is deliberately outside the anthropic/openai provider-family + gateway
machinery (see :func:`omnicraft.runtime.workflow._build_cursor_spawn_env`): the
...
register a ``CURSOR_API_KEY`` once through ``omnicraft setup`` rather than
exporting it in every shell.

This module is that home. The key is stored exactly like the api-key
providers' secrets — in the omnicraft secret store (OS keychain, else a
``0600`` JSON file; see :mod:`omnicraft.onboarding.secrets`) — and referenced
from a dedicated top-level ``cursor:`` block in ``~/.omnicraft/config.yaml``::
...
(:func:`omnicraft.runtime.workflow._load_global_auth`) — a Cursor key parked
...
from omnicraft.onboarding.extra_install import extra_install_command
from omnicraft.onboarding.provider_config import load_config, resolve_secret
...
def cursor_sdk_installed() -> bool:
    ...
    (:mod:`omnicraft.inner.cursor_executor`), so a key can be set with no SDK;
    setup uses this to detect that and offer to install it. Mirrors
    :func:`omnicraft.onboarding.databricks_config.databricks_sdk_installed` /
    :func:`omnicraft.onboarding.antigravity_auth.antigravity_sdk_installed`:
...
def cursor_install_command() -> list[str]:
    ...
    Delegates to :func:`~omnicraft.onboarding.extra_install.extra_install_command`
...
def install_cursor_sdk() -> bool:
    ...
    visible. Mirrors
    :func:`omnicraft.onboarding.antigravity_auth.install_antigravity_sdk`.
...
def cursor_api_key_ref(config: dict[str, object] | None = None) -> str | None:
    ...
    :param config: A pre-loaded config mapping; ``None`` loads
        ``~/.omnicraft/config.yaml`` via :func:`load_config`.
...
def cursor_api_key_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`omnicraft.cli._save_global_config` (a shallow update, so
