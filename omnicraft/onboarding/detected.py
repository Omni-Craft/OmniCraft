...
:mod:`omnicraft.onboarding.ambient` discovers credentials already on the
...
``providers:`` shape consumed by :mod:`omnicraft.onboarding.provider_config`,
...
from omnicraft.env_credentials import getenv_nonempty_with_omnicraft_prefix
...
def codex_config_provider_dismissed(config: dict[str, object]) -> bool:
    ...
    :returns: ``True`` when ``~/.codex/config.toml`` selects a custom,
        ...
        dismissed in *config*.
    """
    from omnicraft.onboarding.ambient import codex_config_detection
...
def _synthesize_entry(det: DetectedProvider) -> dict[str, object] | None:
    ...
    :returns: A raw provider entry body (config shape), or ``None`` when the
        detection maps to no omnicraft harness surface (a ``family``-less
        ``key`` detection).
    """
    ...
    if det.family is None:
        ...
        # whose provider maps to no omnicraft family).
        return None
    ...
    if det.kind == "key":
        ...
        if vendor is not None:
            ...
            if det.family == OPENAI_FAMILY and env_var in (
                ...
                env_base_url = getenv_nonempty_with_omnicraft_prefix("OPENAI_BASE_URL")
...
def synthesize_detected_entries(
    ...
    :param detected: Detections from
        :func:`omnicraft.onboarding.ambient.detect_providers`, in priority
        order.
    :returns: Raw provider entries keyed by the detection name, e.g.
        ...
        surface) — see :data:`omnicraft.onboarding.ambient._ENV_KEY_FAMILY`.
        Only detections that map to no omnicraft family at all (a ``family``-less
...
def effective_config_with_detected(
    ...
    :param detected: Detections to merge; defaults to a live
        :func:`omnicraft.onboarding.ambient.detect_providers` call.
