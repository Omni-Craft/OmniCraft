"""Goose readiness + config reporting for ``omnicraft setup``.

Unlike :mod:`omnicraft.onboarding.cursor_auth`, OmniCraft manages **no** Goose
...
from omnicraft.onboarding.harness_install import GOOSE_KEY, harness_cli_installed

#: Override for the goose binary path (mirrors omnicraft.goose_native).
...
def _goose_binary() -> str | None:
    """Resolve the goose executable (``OMNICRAFT_GOOSE_PATH`` override, else PATH)."""
