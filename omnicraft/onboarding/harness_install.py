...
``omnicraft setup --no-internal-beta`` uses this to mark an uninstalled harness and
offer to ``npm install`` it; the first-run ``omnicraft run`` flow uses the
...
:mod:`omnicraft.onboarding.ambient` is file-based and subprocess-free on
...
# installer, not an npm package managed by ``omnicraft setup``.
KIRO_KEY = "kiro"
...
# ``~/.config/goose/config.yaml``) with no OmniCraft-managed credential, and ships
...
# :func:`omnicraft.onboarding.harness_readiness.harness_is_configured`). The key
...
# OmniCraft-managed credentials). The ``hermes`` binary must be on PATH.
HERMES_KEY = "hermes"
...
def _all_harness_install() -> dict[str, HarnessInstallSpec]:
    from omnicraft.harness_plugins import install_specs
...
def _all_harness_name_to_key() -> dict[str, str]:
    from omnicraft.harness_plugins import harness_install_keys
...
def harness_setup_hint(harness: str | None) -> str:
    ...
    model credential, both of which ``omnicraft setup`` handles — so they route
    ...
    and no ``package``) is **not** installed by ``omnicraft setup``: pointing a
    ...
    :returns: A remediation clause for the "harness not configured" message,
        ...
        login`"`` for native Cursor, or the ``omnicraft setup`` hint otherwise.
    """
