...
but never ran ``omnicraft setup`` to install it" case.
...
import omnicraft.onboarding.gemini_auth as _gemini_auth
from omnicraft.harness_aliases import HARNESS_ALIASES, canonicalize_harness
from omnicraft.harness_plugins import harness_install_keys, valid_harnesses
...
# auth backend and no OmniCraft provider family, so readiness is binary presence.
...
# Native Kimi TUI harnesses (``omnicraft kimi``). Like the other native CLIs,
...
def _install_key(canonical: str) -> str:
    ...
    :returns: ``"anthropic"`` / ``"openai"`` for the claude/codex CLIs,
        :data:`~omnicraft.onboarding.harness_install.KIMI_KEY` for kimi,
        :data:`~omnicraft.onboarding.harness_install.OPENCODE_KEY` for
        opencode-native,
        :data:`~omnicraft.onboarding.harness_install.QWEN_KEY` for qwen, or
        :data:`~omnicraft.onboarding.harness_install.PI_KEY` for pi.
    """
...
def harness_is_configured(harness: str) -> bool:
    ...
    if canonical == CURSOR_KEY:
        ...
        # readiness is whether a key is resolvable — stored by ``omnicraft setup``
        # (the ``cursor:`` block — see :mod:`omnicraft.onboarding.cursor_auth`)
        ...
        # (:mod:`omnicraft.inner.cursor_executor`); gating here would only
        # duplicate that, less actionably. So cursor keeps its single key check.
        from omnicraft.onboarding.cursor_auth import cursor_api_key_configured
    ...
    if canonical == COPILOT_KEY:
        ...
        # resolvable — one stored by ``omnicraft setup`` (the ``copilot:`` config
        # block — see :mod:`omnicraft.onboarding.copilot_auth`) or inherited from
...
def _harness_availability(canonical: str) -> HarnessAvailability:
    ...
    if (
        ...
        from omnicraft.codex_native import _codex_auth_unavailable_reason
