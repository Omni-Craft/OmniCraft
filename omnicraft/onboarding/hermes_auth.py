"""Hermes readiness + config reporting for ``omnicraft setup``.

Like :mod:`omnicraft.onboarding.goose_auth`, OmniCraft manages **no** Hermes
...
provider/model so ``omnicraft setup`` can show Hermes as ready (and which model
...
:func:`omnicraft.hermes_native_bridge._load_user_hermes_config`. A fresh install
...
from omnicraft.onboarding.harness_install import HERMES_KEY, harness_cli_installed
