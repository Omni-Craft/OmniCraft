"""OpenCode readiness + credential reporting for ``omnicraft setup``.

Like :mod:`omnicraft.onboarding.goose_auth`, OmniCraft stores **no** OpenCode
...
read-only reporter so ``omnicraft setup`` can show which providers OpenCode can
...
from omnicraft.onboarding.harness_install import OPENCODE_KEY, harness_cli_installed
