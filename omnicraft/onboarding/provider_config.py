"""Read/write the kind-typed model-provider config in ``~/.omnicraft/config.yaml``.
...
block in ``~/.omnicraft/config.yaml`` is the source of truth for the
...
from omnicraft.env_credentials import (
    expand_envvars_with_omnicraft_prefix,
    getenv_with_omnicraft_prefix,
    omnicraft_prefixed_env_name,
)
...
# :mod:`omnicraft.onboarding.gemini_auth`, not a ``gemini:`` family here).
...
# (native-``omnicraft claude`` only) can never drive pi. Resolution: an
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # Native CLI harnesses. Specs (and the omnicraft-compat translator)
    ...
    # per-spawn provider override flag, so OmniCraft cannot thread a generic
    ...
    # (file-based, checked in :mod:`omnicraft.onboarding.gemini_auth`) and the
...
class FamilyConfig:
    ...
    :param api_key_ref: A reference to a secret stored outside the config
        ...
        ``"keychain:<name>"`` (read from the omnicraft secret store — see
        :func:`resolve_secret` and :mod:`omnicraft.onboarding.secrets`),
...
class ProviderEntry:
    ...
    def family(self, name: str) -> FamilyConfig | None:
        ...
        :raises OmniCraftError: If the family references an unset
...
def resolve_secret(ref: str) -> str:
    ...
    - ``"keychain:<name>"`` — read ``<name>`` from the omnicraft secret
      store (OS keychain, else a ``0600`` JSON file). The store is
      populated by ``omnicraft setup --no-internal-beta`` — see
      :mod:`omnicraft.onboarding.secrets`.
    ...
    :raises OmniCraftError: If an ``env:`` / ``$VAR`` reference names an
    ...
    if ref.startswith("env:"):
        var = ref[len("env:") :]
        resolved = getenv_with_omnicraft_prefix(var)
        if resolved is None:
    ...
    expanded = expand_envvars_with_omnicraft_prefix(ref)
    check_unresolved_env_vars(ref, expanded)
    return expanded


def _config_path() -> str:
    """Return the path to the global omnicraft config file.

    Respects ``$OMNICRAFT_CONFIG_HOME`` for test isolation (matching the
    rest of the onboarding layer).

    :returns: Path to ``config.yaml``, e.g.
        ``"/home/u/.omnicraft/config.yaml"``.
    """
    config_home = os.environ.get("OMNICRAFT_CONFIG_HOME")
    if config_home:
        return os.path.join(config_home, "config.yaml")
    return os.path.join(os.path.expanduser("~"), ".omnicraft", "config.yaml")
...
def _expand(key: str, value: str) -> str:
    ...
    :raises OmniCraftError: If a referenced variable is unset.
    """
    expanded = expand_envvars_with_omnicraft_prefix(value)
...
def _expand_family(provider_name: str, family_name: str, family: FamilyConfig) -> FamilyConfig:
    ...
    :returns: A new :class:`FamilyConfig` with ``base_url`` expanded and,
        ...
        ``api_key_ref`` cleared).
    :raises OmniCraftError: If a referenced environment variable is unset,
...
def _parse_family(provider_name: str, family_name: str, raw: dict[str, object]) -> FamilyConfig:
    ...
    :raises OmniCraftError: If ``base_url`` is missing, the credential
...
def _parse_default_families(
    ...
    :raises OmniCraftError: If ``default`` is an unsupported type, or names
...
def _parse_provider(name: str, raw: dict[str, object]) -> ProviderEntry:
    ...
    :raises OmniCraftError: If ``kind`` is missing/invalid or the
    ...
    # omnicraft/inner/loader.py), so a YAML ``default: true`` arrives as
...
def load_config() -> dict[str, object]:
    """Load the global ``~/.omnicraft/config.yaml`` mapping.
    ...
    ``$OMNICRAFT_CONFIG_HOME`` for test isolation, exactly like the rest
...
def _cli_config_serves_pi(entry: ProviderEntry) -> bool:
    ...
    natively, and :func:`omnicraft.pi_native_credentials._cli_config_pi_provider`
    ...
    The capability check lives in :mod:`omnicraft.pi_native_credentials` (the
    ...
    if entry.kind != CLI_CONFIG_KIND:
        return False
    from omnicraft.pi_native_credentials import cli_config_pi_provider_capable
...
def provider_families(entry: ProviderEntry) -> frozenset[str]:
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is native-``omnicraft claude`` only — the in-process /
...
def get_default_provider(config: dict[str, object], family: str) -> ProviderEntry | None:
    ...
    :raises OmniCraftError: If any provider is malformed, or more than one
...
def default_provider_for_harness(config: dict[str, object], harness: str) -> ProviderEntry | None:
    ...
    unusable outside its own CLI, and ``bedrock`` is native-``omnicraft
    ...
    :raises OmniCraftError: If a provider is malformed, or more than one
    ...
    for fam in _PI_FALLBACK_FAMILIES:
        ...
        # native-``omnicraft claude`` only (configure_agent_harness_with_provider
...
def _source_descriptor(family: FamilyConfig) -> str:
    ...
    :raises OmniCraftError: If the family has no credential source (should
...
def describe_active_credential(
    ...
    :raises OmniCraftError: If the resolved provider is malformed, or more
...
def set_default_provider(
    ...
    :raises OmniCraftError: If *name* is absent, an entry is malformed, or
...
def provider_entry_settings(
    ...
    ``~/.omnicraft/config.yaml`` under ``providers:``. When *make_default*
