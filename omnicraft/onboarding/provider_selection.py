"""
Interactive provider and model selection for ``omnicraft create``.
...
def _read_credentials_from_env(provider: str) -> dict[str, str]:
    ...
    if env_var:
        ...
        if resolved is not None:
            ...
            if provider == "openai":
                base_url = getenv_nonempty_with_omnicraft_prefix("OPENAI_BASE_URL")
...
def _collect_env_credentials(
    ...
    for field in fields:
        ...
        resolved = getenv_nonempty_with_omnicraft_prefix(env_name)
        if resolved is not None:
