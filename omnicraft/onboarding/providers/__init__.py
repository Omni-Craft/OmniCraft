...
omnicraft-specific and lives here permanently.
"""
...
def _download_provider_catalog(provider: str) -> dict[str, Any] | None:
    ...
    Skipped when ``OMNICRAFT_DISABLE_CATALOG_LOOKUP=1`` (set by the test
...
def _fetch_provider_catalog(provider: str) -> dict[str, Any]:
    ...
    is disabled via ``OMNICRAFT_DISABLE_CATALOG_LOOKUP``) so callers
