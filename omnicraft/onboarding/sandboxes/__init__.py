"""
Sandbox launchers: run OmniCraft hosts in remote sandboxes.

Public API for the ``omnicraft sandbox`` CLI and anything else that
...
from omnicraft.onboarding.sandboxes.bootstrap import (
...
_LAUNCHERS: dict[str, str] = {
    "lakebox": "omnicraft.onboarding.sandboxes.lakebox:LakeboxLauncher",
    "modal": "omnicraft.onboarding.sandboxes.modal:ModalSandboxLauncher",
    "daytona": "omnicraft.onboarding.sandboxes.daytona:DaytonaSandboxLauncher",
    "boxlite": "omnicraft.onboarding.sandboxes.boxlite:BoxliteSandboxLauncher",
    # CoreWeave Sandbox via the official cwsandbox SDK (the
    # `omnicraft[cwsandbox]` extra), imported lazily like modal/daytona.
    "cwsandbox": "omnicraft.onboarding.sandboxes.cwsandbox:CWSandboxLauncher",
    "islo": "omnicraft.onboarding.sandboxes.islo:IsloSandboxLauncher",
    # E2B (https://e2b.dev) via the official `e2b` SDK (the
    # `omnicraft[e2b]` extra), imported lazily like modal/daytona.
    "e2b": "omnicraft.onboarding.sandboxes.e2b:E2BSandboxLauncher",
    "openshell": "omnicraft.onboarding.sandboxes.openshell:OpenShellSandboxLauncher",
    # On-demand Kubernetes runner Pod via the official kubernetes client (the
    # `omnicraft[kubernetes]` extra), imported lazily like modal/daytona.
    "kubernetes": "omnicraft.onboarding.sandboxes.kubernetes:KubernetesSandboxLauncher",
}
...
def available_providers() -> tuple[str, ...]:
    ...
    ``omnicraft sandbox`` command group.
...
def get_launcher(provider: str, *, workspace_host: str | None = None) -> SandboxLauncher:
    ...
    :param workspace_host: Databricks workspace fronting the target
        ...
        :func:`~omnicraft.onboarding.sandboxes.bootstrap.derive_workspace`),
    ...
    if provider == "lakebox" and workspace_host is not None:
        ...
        from omnicraft.onboarding.sandboxes.lakebox import LakeboxLauncher
