"""
Provider-agnostic interface for running OmniCraft hosts in remote sandboxes.
...
generic bootstrap flow in :mod:`omnicraft.onboarding.sandboxes.bootstrap`
...
from omnicraft.host.identity import HOST_ID_ENV_VAR, HOST_NAME_ENV_VAR, HOST_TOKEN_ENV_VAR
...
DEFAULT_HOST_IMAGE: str = "ghcr.io/omnicraft-ai/omnicraft-host:latest"
"""Default sandbox image across providers: the official prebaked
OmniCraft host image, published by CI from the ``host`` target of
``deploy/docker/Dockerfile`` (``:latest`` tracks main; ``:sha-<short>``
pins a commit). It bakes the full omnicraft install plus git / tmux /
...
def host_image_wheel_install_command(remote_tgz_path: str) -> str:
    ...
    omnicraft at the same ``0.1.0`` version. Without it, pip sees the
...
class SandboxLauncher(ABC):
    ...
    :func:`omnicraft.onboarding.sandboxes.get_launcher`. All methods
    ...
    # (``omnicraft sandbox create`` / ``connect``: wheel shipping via
    ...
    def start_host(
        ...
        *,
        ...
        """
        Start ``omnicraft host`` in the sandbox and return the workspace path.
        ...
        self.run_background(
            sandbox_id,
            f"{env_prefix} omnicraft host --server {shlex.quote(server_url)}",
        )
        return workspace
    ...
    def run_background(
        ...
        ``"ENV=val omnicraft host …"``), and ``nohup`` does NOT honor shell
        ...
        :param command: Shell command to background, e.g.
            ``"ENV=val omnicraft host --server https://…"``.
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        Used to hold ``omnicraft host`` open while the sandbox is
        registered with the App. CLI-bootstrap capability —
        managed-only launchers need not override the raising default.

        :param sandbox_id: Target sandbox.
        :param command: Shell command to execute remotely, e.g.
            ``"omnicraft host --server https://… --profile oss"``.
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        sandbox image (e.g. the Lakebox image bakes omnicraft and its
        ...
        :param remote_tgz_path: Where :func:`~omnicraft.onboarding.
