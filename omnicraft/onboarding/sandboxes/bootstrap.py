"""
Provider-agnostic sandbox bootstrap for OmniCraft hosts.

Composes a :class:`~omnicraft.onboarding.sandboxes.base.SandboxLauncher`
into the full host-bootstrap flow: build the OmniCraft wheels locally,
...
holding ``omnicraft host`` open in it. The end state is a sandbox whose
sessions are reachable from the OmniCraft server's UI, TUI, and
``omnicraft resume``.
...
:mod:`omnicraft.onboarding.sandboxes.lakebox` for the reference
implementation.
"""
...
if TYPE_CHECKING:
    ...
    from omnicraft.onboarding.sandboxes.base import RemoteProcess, SandboxLauncher
...
install: the python client SDK, the UI SDK, and the omnicraft package
...
DEFAULT_SANDBOX_NAME: str = "omnicraft-host"
"""Default label used when ``omnicraft sandbox create`` provisions a
new sandbox."""
...
def build_wheels(
    ...
    *,
    ...
    """
    Build the OmniCraft wheels and pack them into a single tarball.
    ...
    :param repo_root: Path to the omnicraft repo checkout, e.g.
        ``Path("/home/me/omnicraft")``.
    ...
    click.echo(f"▸ Building OmniCraft wheels → {tgz_path}")
    env = os.environ.copy()
...
def ship_wheels(
    ...
    *,
    ...
    """
    Install OmniCraft wheels into a sandbox.
...
def _read_login_url(stream: Iterable[str]) -> str | None:
    ...
    :returns: The authorize URL, or ``None`` when the stream ends
        ...
        ``omnicraft login`` reuses a cached workspace OAuth grant when
...
class DerivedWorkspace:
    ...
    probes of an OmniCraft server URL.
...
def derive_workspace(server_url: str) -> DerivedWorkspace | None:
    ...
    Runs the same unauthenticated detection ``omnicraft login``
    ...
    from omnicraft.cli import _databricks_workspace_login_target
...
def login_app_oauth_in_sandbox(
    ...
    *,
    ...
    """
    Log the sandbox in to *server_url* by running ``omnicraft login``
    **inside the sandbox**, driving the browser step from the local
    ...
    are rejected; only workspace OAuth tokens pass. ``omnicraft
    ...
    verifies), and stores a pointer record so ``omnicraft host`` and
    ...
    1. runs ``omnicraft login <server_url>`` inside the sandbox over a
       PTY;
    ...
    :param server_url: OmniCraft server URL to log in to, e.g.
    ...
    login = launcher.stream_exec(
        sandbox_id,
        f"omnicraft login {shlex.quote(server_url)}",
        pty=True,
    )
    try:
...
def _complete_browser_login(
    ...
    """
    Drive the browser half of an in-sandbox ``omnicraft login``.
    ...
    with launcher.forward_local_port(sandbox_id, port):
        ...
        if returncode != 0:
            raise click.ClickException(
                f"`omnicraft login` inside sandbox '{sandbox_id}' exited with code {returncode}."
            )
...
def set_sandbox_host_name(launcher: SandboxLauncher, sandbox_id: str, host_name: str) -> None:
    """
    Update the sandbox's ``~/.omnicraft/config.yaml`` to use a
    specific host name.
    ...
    ``omnicraft host`` will load that file as-is.
    ...
    """
    click.echo(f"  → setting host name to '{host_name}' in ~/.omnicraft/config.yaml")
    ...
    py = (
        "import os, uuid, yaml; "
        "p=os.path.expanduser('~/.omnicraft/config.yaml'); "
...
def connect_sandbox_host(
    ...
    *,
    ...
    """
    Register the sandbox as a host by running ``omnicraft host`` in it.
    ...
    The remote command is always the bare ``omnicraft host --server
    <url>``: ``omnicraft host`` no longer takes a ``--profile`` flag
    — it resolves credentials itself, via a stored
    ``omnicraft login`` token or the sandbox's ambient Databricks
    ...
    ``~/.omnicraft/config.yaml`` is updated so the host registers
    ...
    :param launcher: The provider's launcher.
    :param sandbox_id: Target sandbox.
    :param server_url: OmniCraft App URL the runner registers with.
    ...
    click.echo("  → running `omnicraft host` in the sandbox (Ctrl-C to detach)")
    returncode = launcher.exec_foreground(sandbox_id, f"omnicraft host --server {server_url}")
    if returncode != 0:
        raise click.ClickException(
            f"`omnicraft host` on sandbox '{sandbox_id}' exited with code {returncode}."
        )
...
def bootstrap_sandbox_host(
    ...
    *,
    ...
    keep-alive → build wheels → ship wheels → ``omnicraft login``
    inside the sandbox.
    ...
    :param sandbox_name: Label for a new sandbox (ignored when
        *sandbox_id* is set).
    :param server_url: OmniCraft server URL the sandbox logs in to.
    ...
    :param repo_root: Path to the omnicraft repo checkout.
