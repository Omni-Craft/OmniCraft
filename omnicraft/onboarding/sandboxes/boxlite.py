...
:class:`~omnicraft.onboarding.sandboxes.base.SandboxLauncher` for
...
optional dependency (``pip install 'omnicraft[boxlite]'``) imported lazily, so
...
- **Local** (no ``endpoint``): ``Boxlite.default()`` — boxes are micro-VMs on
  the omnicraft-server host itself (KVM on Linux / Hypervisor.framework on
...
prebaked host image and starts ``omnicraft host`` over ``run``; it never ships
...
stable, long-lived loop. omnicraft calls launcher methods synchronously off its
...
HOST_IMAGE_ENV_VAR: str = "OMNICRAFT_BOXLITE_HOST_IMAGE"
"""Environment variable overriding
:data:`~omnicraft.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for boxlite
boxes, e.g. an org-internal copy of the host image
(``ghcr.io/<your-org>/omnicraft-host:latest``)."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_BOXLITE_SANDBOX_ENV"
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'omnicraft[boxlite]'`. Local mode also "
    ...
    def __init__(
        ...
        *,
        ...
        :param endpoint: Remote ``boxlite serve`` URL, e.g.
            ...
            (boxes run on the omnicraft-server host via ``Boxlite.default()``).
        ...
        :param image: Registry image reference with omnicraft pre-installed, e.g.
            ``"docker.io/me/omnicraft-host:latest"`` — the server's
