...
'omnicraft[cwsandbox]'``) imported lazily, so the provider can be listed
...
HOST_IMAGE_ENV_VAR = "OMNICRAFT_CWSANDBOX_HOST_IMAGE"
SANDBOX_ENV_PASSTHROUGH_ENV_VAR = "OMNICRAFT_CWSANDBOX_SANDBOX_ENV"
MAX_LIFETIME_ENV_VAR = "OMNICRAFT_CWSANDBOX_MAX_LIFETIME_S"
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'omnicraft[cwsandbox]'`, then set "
...
class CWSandboxLauncher(SandboxLauncher):
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        click.echo(
            ...
            "its max_lifetime_seconds (override OMNICRAFT_CWSANDBOX_MAX_LIFETIME_S)."
        )
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        # future, not the remote exec) in an unpredictably-named dir under
        # /tmp: `mkdir -m 700` (no -p) fails closed if the path already exists,
        # so a less-privileged co-resident process can't pre-seed a symlink the
        # (more-privileged) foreground would write its pid through. `echo $$ …
        # && exec` keeps the pid across the shell swap, so the interrupt path
        # can kill it with a second exec. See :func:`foreground_pidfile` for
        # the shared rationale (and why this is not a same-uid boundary).
