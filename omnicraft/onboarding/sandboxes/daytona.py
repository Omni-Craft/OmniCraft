...
(``pip install 'omnicraft[daytona]'``) imported lazily, so the provider
...
bootstrap flow (``omnicraft sandbox create`` / ``connect`` — file
...
HOST_IMAGE_ENV_VAR: str = "OMNICRAFT_DAYTONA_HOST_IMAGE"
"""Environment variable overriding
:data:`~omnicraft.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for
Daytona sandboxes, e.g. an org-internal copy of the host image
(``ghcr.io/<your-org>/omnicraft-host:latest``)."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_DAYTONA_SANDBOX_ENV"
...
# OmniCraft host must survive arbitrary idle gaps between turns, so
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'omnicraft[daytona]'`, "
...
class DaytonaSandboxLauncher(SandboxLauncher):
    ...
    def run(self, sandbox_id: str, command: str, *, check: bool = True) -> RemoteCommandResult:
        ...
        :class:`~omnicraft.onboarding.sandboxes.base.RemoteCommandResult`
    ...
    def terminate(self, sandbox_id: str) -> None:
        ...
        # not an omnicraft dependency — pulling it in for a 3-iteration
