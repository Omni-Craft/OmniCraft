...
the SDK is an optional dependency (``pip install 'omnicraft[e2b]'``)
...
- **Templates, not registry images.** Unlike every other launcher, E2B
  ...
  a pre-built E2B *template*. The OmniCraft host image must therefore be
  ...
  built FROM the same host image (omnicraft ``0.1.0`` baked).
...
TEMPLATE_ENV_VAR: str = "OMNICRAFT_E2B_TEMPLATE"
"""Environment variable overriding :data:`DEFAULT_E2B_TEMPLATE` — the
NAME (or id) of the pre-built E2B template the OmniCraft host image was
built into (``e2b template build``). NOT a registry image reference:
E2B boots from templates, not arbitrary images."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_E2B_SANDBOX_ENV"
...
DEFAULT_E2B_TEMPLATE: str = "omnicraft-host"
...
MAX_LIFETIME_ENV_VAR: str = "OMNICRAFT_E2B_MAX_LIFETIME_S"
...
def _template_build_hint(template: str, exc: Exception) -> click.ClickException:
    ...
    return click.ClickException(
        ...
        "incompatible. Build the OmniCraft host image into an E2B template first "
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'omnicraft[e2b]'`, then set "
...
class E2BSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, template: str | None = None, env: Sequence[str] | None = None) -> None:
        ...
        :param template: Optional E2B template NAME (or id) to provision
            ...
            OmniCraft host image was built into (``e2b template build``),
    ...
    def stream_exec(self, sandbox_id: str, command: str, *, pty: bool = False) -> RemoteProcess:
        ...
        try:
            ...
            # backs exec_foreground, which holds `omnicraft host` open for the
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        FROM the prebaked host image (omnicraft ``0.1.0`` baked).
