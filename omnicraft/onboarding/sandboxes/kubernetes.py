...
:class:`~omnicraft.onboarding.sandboxes.base.SandboxLauncher` for an
...
dependency (``pip install 'omnicraft[kubernetes]'``) imported lazily, so the
provider can be listed and the module probed without it.

The model is **entrypoint-as-host**: the Pod's container command IS
``omnicraft host``. :meth:`~KubernetesSandboxLauncher.provision` only RESERVES
...
- **PID-1 reaper.** The in-sandbox host re-parents orphaned runner processes to
  ...
  ``omnicraft host``, reaps any children, and forwards SIGTERM for prompt,
...
- **Least privilege.** ``automountServiceAccountToken: false`` keeps the runner
  ...
  filesystem stays writable (the host writes ``/tmp`` and ``~/.omnicraft``).
...
from omnicraft.host.identity import (
    HOST_ID_ENV_VAR,
    HOST_NAME_ENV_VAR,
    HOST_TOKEN_ENV_VAR,
)
...
HOST_IMAGE_ENV_VAR: str = "OMNICRAFT_KUBERNETES_HOST_IMAGE"
"""Environment variable overriding
:data:`~omnicraft.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for Kubernetes
sandbox Pods (published multi-arch: amd64 + arm64)."""

NAMESPACE_ENV_VAR: str = "OMNICRAFT_KUBERNETES_NAMESPACE"
...
SANDBOX_SECRET_ENV_VAR: str = "OMNICRAFT_KUBERNETES_SECRET"
...
SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_KUBERNETES_SANDBOX_ENV"
...
SERVICE_ACCOUNT_ENV_VAR: str = "OMNICRAFT_KUBERNETES_SERVICE_ACCOUNT"
...
KUBECONFIG_ENV_VAR: str = "OMNICRAFT_KUBERNETES_KUBECONFIG"
...
_DEFAULT_NAMESPACE: str = "omnicraft-sandboxes"
_DEFAULT_SERVICE_ACCOUNT: str = "omnicraft-runner"
...
# (or a future reconciler) can select omnicraft-managed objects for GC.
_MANAGED_BY_LABEL: str = "app.kubernetes.io/managed-by"
_MANAGED_BY_VALUE: str = "omnicraft"
_ROLE_LABEL: str = "omnicraft.ai/role"
...
# (``omnicraft host …``) as a child, forwards SIGTERM/SIGINT for prompt graceful
...
# in :mod:`omnicraft.server.managed_hosts` (which validate config-sourced names);
...
def _ensure_sdk() -> None:
    ...
    optional dependency — the base ``omnicraft`` install does not pull it in.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'omnicraft[kubernetes]'`."
        ) from exc
...
def _new_pod_name(label: str) -> str:
    ...
    :returns: A Pod name like ``"omnicraft-managed-a1b2c3d4-1a2b3c"``.
    """
    base = re.sub(r"[^a-z0-9-]+", "-", label.lower()).strip("-")
    base = re.sub(r"-+", "-", base) or "host"
    return f"omnicraft-{base[:40]}-{uuid.uuid4().hex[:6]}"
...
def _token_secret_name(pod_name: str) -> str:
    ...
    :returns: The Secret name, e.g. ``"omnicraft-managed-a1b2c3d4-1a2b3c-token"``.
    """
...
def _render_workspace_prep_command(
    ...
    :param workspace: The workspace root to create, e.g. ``"/home/omnicraft/workspace"``.
...
def _render_host_command(server_url: str) -> list[str]:
    """
    Render the main container command that runs ``omnicraft host`` under the
    PID-1 reaper.
    ...
    ``omnicraft host --server <url>`` — as its child. Identity + token reach the
    ...
    script = (
        ...
        f"omnicraft host --server {shlex.quote(server_url)}"
    )
    return ["bash", "-lc", script]
...
def build_pod_manifest(
    *,
    ...
    - An **init container** (:data:`_INIT_CONTAINER_NAME`) creates the workspace
      ...
      (:data:`_CONTAINER_NAME`) runs ``omnicraft host`` under the PID-1 reaper.
    ...
    - Pod + container ``securityContext`` satisfy Pod Security "restricted"
      ...
      root filesystem stays writable (the host writes ``/tmp`` + ``~/.omnicraft``).
...
class KubernetesSandboxLauncher(SandboxLauncher):
    ...
    ``omnicraft host``, and :meth:`terminate` deletes both. All transport rides the
    ...
    def __init__(
        ...
        *,
        ...
        :param image: Host image reference — the ``sandbox.kubernetes.image``
            ...
            :data:`~omnicraft.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE`.
    ...
    def provision(self, name: str) -> str:
        ...
        Entrypoint-as-host: the Pod (which boots running ``omnicraft host``) is
    ...
    def start_host(
        ...
        *,
        ...
        :meth:`~omnicraft.onboarding.sandboxes.base.SandboxLauncher.start_host`
        ...
        main container runs ``omnicraft host``, which dials back over the
    ...
    def _delete_with_retry(self, kind: str, name: str, delete: Callable[[], object]) -> None:
        ...
        click.echo(
            ...
            "and carries the omnicraft managed-by/role labels for GC.",
            err=True,
        )
