...
dependency (``pip install 'omnicraft[openshell]'``) imported lazily, so
...
- **Custom host image.** OmniCraft boots its prebaked host image, which
...
HOST_IMAGE_ENV_VAR: str = "OMNICRAFT_OPENSHELL_HOST_IMAGE"
"""Environment variable overriding :data:`DEFAULT_HOST_IMAGE` for
OpenShell sandboxes."""

SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "OMNICRAFT_OPENSHELL_SANDBOX_ENV"
...
_EXEC_TIMEOUT_S = 300
# A foreground host (`omnicraft host`) is held open until Ctrl-C, so its
...
# rather than changing the shared image -- otherwise ``omnicraft host`` resolves
...
def _ensure_sdk() -> None:
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'omnicraft[openshell]'`, then select a "
...
class OpenShellSandboxLauncher(SandboxLauncher):
    ...
    def exec_foreground(self, sandbox_id: str, command: str) -> int:
        ...
        Holds ``omnicraft host`` open for ``omnicraft sandbox connect``;
        Ctrl-C kills the remote process and re-raises ``KeyboardInterrupt``.
        """
        client = self._openshell()
        # Record the remote pid in an unpredictably-named dir under /tmp:
        ...
        # (more-privileged) foreground would write its pid through. `echo $$ …
        # && exec` keeps the pid across the shell swap, so cancelling the local
        # gRPC stream (which stops our reads but not the remote command) can
        # still kill it. See :func:`foreground_pidfile` for the shared
        # rationale (and why this is not a same-uid boundary).
