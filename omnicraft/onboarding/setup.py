"""
Databricks profile onboarding for OmniCraft.
...
:mod:`omnicraft.onboarding.internal_beta`, which is excluded from the public
...
SKIP_ENV_VAR = "OMNICRAFT_SKIP_ONBOARD"
...
class ProfileSpec:
    """
    One Databricks profile OmniCraft depends on.
...
def _databrickscfg_path() -> Path:
    ...
    a temporary copy (e.g. during ``omnicraft setup``) can redirect all
...
def _derive_workspace_profile_name(workspace_url: str, existing: dict[str, str]) -> str:
    ...
    2. A bundled :data:`DEFAULT_PROFILES` spec whose host matches — use its
       ...
       in
       :func:`omnicraft.onboarding.databricks_config.get_workspace_url_for_profile`.
...
def login_databricks_workspace(workspace_url: str, *, console: Console | None = None) -> str:
    ...
    place OmniCraft triggers a Databricks CLI login: it fires solely when
    a user explicitly adds a Databricks provider in
    ``omnicraft setup --no-internal-beta``, never on a bare ``omnicraft run``.
...
def maybe_run_onboarding() -> None:
    """
    Pre-flight onboarding for ``omnicraft run``.
    ...
    Skipped when ``$OMNICRAFT_SKIP_ONBOARD`` is set or stdin isn't a
