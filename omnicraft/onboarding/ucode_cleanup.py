...
:mod:`omnicraft.onboarding.ucode_setup`), which wires the Claude / Codex
...
- **Codex < 0.134.0 (ucode's legacy layout):** ucode deep-merges into the
  ...
  which makes every *bare* ``codex`` run (and OmniCraft' inner codex, which
...
Removing the provider entry from ``~/.omnicraft/config.yaml`` alone leaves
...
def strip_ucode_codex_config(config_path: Path | None = None) -> bool:
    ...
    :raises OmniCraftError: If the file exists but is not valid TOML —
...
def remove_ucode_wiring() -> UcodeWiringRemoval:
    ...
    :raises OmniCraftError: If ``~/.codex/config.toml`` exists but is not
