"""Setup helpers for invoking ucode from OmniCraft."""
...
def build_ucode_configure_command(
    ...
    *,
    ...
    """Build the ``ucode configure`` command OmniCraft runs.
...
def configure_ucode_for_workspace(
    ...
    *,
    ...
    ``kind: databricks`` provider via ``omnicraft setup --no-internal-beta``.
    ucode writes ``~/.ucode/state.json``, which OmniCraft then reads for
