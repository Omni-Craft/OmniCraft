...
:class:`omnicraft.native_server_harness.NativeServerHarness` drives it through
...
from omnicraft.native_server_transport import (
    NativeEvent,
    NativeLaunchConfig,
...
from omnicraft.opencode_native_bridge import read_bridge_state
from omnicraft.opencode_native_client import OpenCodeClient
...
def _attachment_to_part(attachment: Mapping[str, Any]) -> dict[str, Any] | None:
    """
    Convert an OmniCraft attachment block into an OpenCode file part.
    ...
    async def create_or_resume_session(self, launch: NativeLaunchConfig) -> str:
        ...
        try:
            ...
            created = await client.create_session(
                {"title": f"omnicraft:{launch.omnicraft_session_id}"}
            )
            return created.id
        finally:
            await client.aclose()
