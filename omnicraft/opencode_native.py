...
to ``omnicraft.codex_native._materialize_codex_agent_spec``). The runner
...
This module also hosts the interactive local ``omnicraft opencode`` CLI wrapper
(:func:`run_opencode_native`, the analog of ``omnicraft codex`` / ``omnicraft pi``):
...
the runner's ambient env / ``omnicraft setup`` config (a profile-bound spec routes
...
from omnicraft._wrapper_labels import OPENCODE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from omnicraft.native_terminal import bind_session_runner as _bind_session_runner
from omnicraft.native_terminal import url_component
from omnicraft.opencode_native_state import read_launch_state, write_launch_state
...
def run_opencode_native(  # pragma: no cover
    *,
    ...
    """
    Launch the OpenCode TUI in an OmniCraft terminal (the ``omnicraft opencode`` path).

    Mirrors ``omnicraft codex`` / ``omnicraft pi``: ensure a local daemon + runner,
    ...
    :param server: Resolved OmniCraft server URL. ``None`` is an error (the CLI
        must resolve a backend first).
    :param session_id: Optional existing OmniCraft conversation id to resume.
    ...
    if server is None:
        raise click.ClickException(
            "OpenCode requires a resolved OmniCraft server URL. The CLI should resolve "
            "a backend before run_opencode_native."
        )
    with TemporaryDirectory(prefix="omnicraft-opencode-native-") as tmpdir:
...
def _run_with_remote_server(  # pragma: no cover
    ...
    *,
    ...
    """Launch OpenCode on an OmniCraft server via a daemon-spawned runner."""
...
def _resolve_session_id_for_resume(
    *,
    ...
    # e2e, not unit tests (it opens an OmniCraftClient and an arrow-key picker).
    from omnicraft_client import OmniCraftClient  # pragma: no cover
