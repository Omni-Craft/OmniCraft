...
Layout (per bridge id):
    ...
    ~/.omnicraft/opencode-native/<sha256(bridge_id)[:32]>/
...
# codex-native ``omnicraft.codex_native.bridge_id`` label.
OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY = "omnicraft.opencode_native.bridge_id"
...
# Token file the shared ``omnicraft.claude_native_bridge serve-mcp`` reads to
...
# OmniCraft builtin tools (``sys_*``/``load_skill``/``web_fetch``) advertised in
...
# the elicitation against the OmniCraft server (mirrors codex-native's
# ``policy_hook.json``; consumed by ``omnicraft.native_cost_popup``).
_COST_POPUP_CONFIG_FILE = "cost_popup.json"
# Filename of the opencode plugin that bridges opencode's lifecycle hooks to the
# OmniCraft policy engine (REQUEST + TOOL_RESULT phases the reactive
# ``permission.asked`` path can't reach).
_POLICY_PLUGIN_FILE = "omnicraft-policy.js"
...
# function exports as plugins (legacy shape). It reads its OmniCraft coordinates
...
// OmniCraft policy bridge for opencode-native (generated; do not edit).
// Forwards opencode lifecycle hooks to the OmniCraft policy engine so
...
const BASE = (process.env.OMNICRAFT_POLICY_URL || "").replace(/\/+$/, "");
const SESSION = process.env.OMNICRAFT_SESSION_ID || "";
...
let POLICY_HEADERS = {};
try {
  POLICY_HEADERS = JSON.parse(process.env.OMNICRAFT_POLICY_HEADERS || "{}") || {};
} catch (e) {
  POLICY_HEADERS = {};
}
...
export const OmniCraftPolicyPlugin = async () => ({
  ...
  "chat.message": async (_input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      ...
      throw new Error(
        "OmniCraft policy blocked this prompt: " + (verdict.reason || "request denied"),
      );
    }
  },
  ...
  "tool.execute.after": async (input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      output.output = "[OmniCraft policy withheld this tool result: " +
...
def write_opencode_policy_plugin(bridge_dir: Path) -> Path:
    """
    Write the OmniCraft policy-bridge plugin into *bridge_dir* and return its path.

    The runner registers the returned path in the synthesized ``opencode.json``
    ``plugin`` field and stamps ``OMNICRAFT_POLICY_URL`` / ``OMNICRAFT_SESSION_ID``
    / ``OMNICRAFT_POLICY_HEADERS`` on the ``opencode serve`` process so the plugin
...
_STATE_VERSION = 1
_BRIDGE_ROOT = Path.home() / ".omnicraft" / "opencode-native"
_ID_HASH_CHARS = 32
...
def bridge_root() -> Path:
    ...
    :returns: Absolute root for OpenCode-native bridge directories, e.g.
        ``Path("~/.omnicraft/opencode-native")``.
    """
    return _BRIDGE_ROOT
...
def write_relay_bridge_config(bridge_dir: Path) -> None:
    ...
    The shared ``omnicraft.claude_native_bridge serve-mcp`` stdio server (which
...
def write_cost_popup_config(
    ...
    ``omnicraft.native_cost_popup`` subprocess inside a ``tmux display-popup`` on
    the opencode pane; it must POST the verdict to the OmniCraft server but cannot
    ...
    :param bridge_dir: OpenCode-native bridge directory.
    :param ap_server_url: OmniCraft server base URL, e.g. ``"http://127.0.0.1:6767"``.
...
def update_model_override(bridge_dir: Path, model_override: str | None) -> bool:
    """
    Persist a new per-session model override (OmniCraft→opencode model switch).
