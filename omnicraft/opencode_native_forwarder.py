"""SSE consumer that mirrors OpenCode events into an OmniCraft session.
...
OpenCode events into OmniCraft session-stream events posted to
...
Design references: the SSE-event → OmniCraft-event translation table in
...
from omnicraft.opencode_native_bridge import update_active_message_id, update_last_event_id
from omnicraft.opencode_native_client import OpenCodeClient, OpenCodeEvent
from omnicraft.opencode_native_permissions import (
    PolicyDecision,
    decision_to_reply,
...
# OmniCraft session-event types (must match the server's ingestion route;
...
# back to OmniCraft so the web model pill stays in sync (claude-native contract).
...
# ``omnicraft/onboarding/opencode_auth.py``).
...
class OpenCodeNativeForwarder:
    """
    Translate one OpenCode session's SSE stream into OmniCraft events.
    ...
    :param server_client: HTTP client for the OmniCraft server (event posts).
    ...
    :param policy_evaluator: Optional async policy resolver. Production
        ...
        ``omnicraft.runner.app._build_opencode_policy_evaluator``) — the SAME
    ...
    def __init__(
        ...
        *,
        ...
        # Last model mirrored to OmniCraft (provider/id), to dedupe switches.
    ...
    async def handle_event(self, event: OpenCodeEvent) -> None:
        """
        Translate one OpenCode event into OmniCraft session events.
    ...
    async def _post_event(self, event_type: str, data: dict[str, Any]) -> httpx.Response | None:
        """
        POST one OmniCraft session event with a single retry.

        :param event_type: OmniCraft event type, e.g.
    ...
    async def _on_part_updated(self, event: OpenCodeEvent) -> None:
        ...
        if part_type == "text":
            ...
            # transcript (omnicraft persists no separate user item for these
    ...
    async def _on_compaction_started(self, event: OpenCodeEvent) -> None:
        ...
        server-side. The OmniCraft server maps ``external_compaction_status``
    ...
    async def _on_model_switched(self, event: OpenCodeEvent) -> None:
        ...
        When the user switches model in the opencode TUI, reflect it to OmniCraft
    ...
    async def _on_permission_asked(self, event: OpenCodeEvent) -> None:
        ...
        try:
            await self._opencode.reply_permission(
                request.request_id, reply_body(reply, message="omnicraft-policy")
            )
    ...
    async def _resolve_permission(self, *, request_dict: Any) -> PolicyDecision:
        ...
        normalized = normalize_for_policy(
            request_dict,
            omnicraft_session_id=self._session_id,
...
_HANDLERS: dict[str, Callable[[OpenCodeNativeForwarder, OpenCodeEvent], Awaitable[None]]] = {
    ...
    # Mirror a TUI model switch back to OmniCraft (in-harness session-cmd sync).
