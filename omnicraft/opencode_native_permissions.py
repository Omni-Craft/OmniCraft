...
OpenCode's permission surface and OmniCraft's policy/approval model:

1. Normalize a raw permission request into a flat policy-evaluation input.
2. Map an OmniCraft policy verdict (allow / allow-always / deny / ask) onto
   an OpenCode reply.
...
# OmniCraft-side normalized decisions used by the forwarder.
...
def normalize_for_policy(
    ...
    *,
    ...
    """
    Build an OmniCraft policy-evaluation input from a permission request.
    ...
    :param omnicraft_session_id: Owning OmniCraft conversation id.
    ...
    return {
        ...
        "omnicraft_session_id": omnicraft_session_id,
...
def map_verdict_to_decision(verdict: Mapping[str, Any] | None) -> PolicyDecision:
    """
    Map an OmniCraft policy verdict onto a normalized decision.
...
def decision_to_reply(decision: PolicyDecision) -> OpenCodeReply | None:
    ...
    ``permission/index.ts``), which bypasses the OmniCraft policy engine and
