"""Persistent client-side state for ``omnicraft opencode`` sessions.

The native OpenCode wrapper records the cwd used to create a session so a
later ``omnicraft opencode --resume <conv_id>`` can launch OpenCode from
...
the shared OmniCraft server. Mirrors :mod:`omnicraft.codex_native_state`.

Layout (per conversation):

    ~/.omnicraft/opencode-native/<sha256(conv_id)[:32]>/launch.json
"""
...
_STATE_ROOT_ENV_VAR = "OMNICRAFT_OPENCODE_NATIVE_STATE_DIR"
...
def _opencode_native_state_root() -> Path:
    ...
    Production defaults to ``~/.omnicraft/opencode-native``.
    ...
    return Path.home() / ".omnicraft" / "opencode-native"
