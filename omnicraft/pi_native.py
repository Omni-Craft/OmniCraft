"""Native Pi TUI wrapper for the OmniCraft CLI."""
...
from omnicraft._wrapper_labels import PI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from omnicraft.pi_native_bridge import bridge_dir_for_session_id

_DEFAULT_PI_COMMAND = "pi"
_PI_PATH_ENV = "OMNICRAFT_PI_PATH"
...
def run_pi_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Pi requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_pi_native."
        )
    with TemporaryDirectory(prefix="omnicraft-pi-native-") as tmpdir:
...
def _materialize_pi_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``omnicraft pi``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Pi on an OmniCraft server via a daemon-spawned runner.
...
async def _create_pi_session(
    ...
    *,
    ...
    :param client: HTTP client pointed at OmniCraft.
