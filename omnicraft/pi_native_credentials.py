"""Translate the omnicraft-configured model provider into native Pi config.

A native Pi session launches the ``pi`` CLI, which authenticates from its own
config directory (``~/.pi/agent``). Without help, a user who ran ``omnicraft
setup`` would still have to run ``pi`` ``/login`` separately — unlike
claude-native / codex-native, which route through the provider that ``omnicraft
setup`` configured.

This module closes that gap. It resolves the provider configured for the Pi
surface (``~/.omnicraft/config.yaml``) and writes a per-session ``models.json``
...
from omnicraft.model_override import normalize_model_for_provider
...
if TYPE_CHECKING:
    ...
    from omnicraft.onboarding.ambient import CodexConfigTransport
...
def _databricks_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    from omnicraft.inner.databricks_executor import _read_databrickscfg_host
...
def _cli_config_databricks_transport(entry: ProviderEntry) -> CodexConfigTransport | None:
    ...
    :class:`~omnicraft.onboarding.ambient.CodexConfigTransport` when so, else
...
def cli_config_pi_provider_capable(entry: ProviderEntry) -> bool:
    ...
    (:mod:`omnicraft.onboarding.provider_config`) consults to decide whether a
...
def _cli_config_pi_provider(entry: ProviderEntry, *, model: str | None) -> PiProviderConfig | None:
    ...
    into ``~/.codex/config.toml`` and ``omnicraft setup`` adopts it as a
...
def resolve_pi_native_provider(
    *,
    ...
    """Resolve the omnicraft-configured provider for a native Pi session.

    Reads the default provider for the Pi surface from
    ``~/.omnicraft/config.yaml`` and translates it into Pi ``models.json``
    ...
    try:
        ...
        # Pi is multi-family; ``omnicraft setup`` marks defaults per family, not
        ...
        if entry is None:
            _LOGGER.info(
                "pi-native: no omnicraft-configured provider for the pi/anthropic/openai "
        ...
        _LOGGER.warning(
            "pi-native: failed to resolve the omnicraft-configured provider; Pi will "
            "use its own login.",
            exc_info=True,
        )
        return None
