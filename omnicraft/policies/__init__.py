...
- :class:`EvaluationContext`, :class:`PolicyResult`,
  ...
  evaluate boundary (see :mod:`omnicraft.policies.types`).
- :class:`Policy` ABC (:mod:`omnicraft.policies.base`) and
...
parking, fail-closed) live in :mod:`omnicraft.runtime.policies`.

Agent-author callables should import :class:`EvaluationContext`
and :class:`PolicyResult` from here, not from
``omnicraft.spec.types`` — those are runtime evaluation
