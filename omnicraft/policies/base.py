...
the engine (in :mod:`omnicraft.runtime.policies.engine`) does
the filter-gate-dispatch-compose orchestration (see
POLICIES.md §4).

The two concrete subclasses live next to this module:

- :class:`omnicraft.policies.function.FunctionPolicy`
- :class:`omnicraft.policies.prompt.PromptPolicy`
...
:mod:`omnicraft.runtime.policies`.
"""
...
from omnicraft.spec.types import PolicySpec
...
class Policy(ABC):
    ...
    async def evaluate(
        ...
        :param context: Read-only context bundle from the
            ...
            with omnicraft' signature (see POLICIES.md §9.1).
    ...
    def reset_turn(self) -> None:  # noqa: B027 — intentional no-op default (see docstring)
        ...
        turn. Mirrors :meth:`omnicraft.runtime.policies.engine.PolicyEngine.reset_turn`.

        The runtime calls this once per "turn" — defined as one
        user prompt → terminal assistant response cycle, which
        in omnicraft corresponds to one ``_run_agent_loop``
        ...
        as :meth:`omnicraft.runtime.policies.engine.PolicyEngine.reset_turn`.
        """
