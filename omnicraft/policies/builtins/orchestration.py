...
(``omnicraft/runner/policy.py``) and add no server routes.
"""
...
def blast_radius(
    *,
    ...
    def _evaluate(event: _Json, config: _Json) -> _Json:  # noqa: ARG001
        ...
        """
        # Match the OmniCraft built-in OS shell, the Claude/Codex native
...
def spawn_bounds(
    *,
    ...
    hook the runner calls (``omnicraft/runner/policy.py``). This is the v1
...
def worktree_guard(
    *,
    ...
    # Match OmniCraft built-in OS write/edit, Claude/Codex native Write/Edit
    # (surfaced via the PreToolUse hook), and Pi's native lowercase
    # write/edit (surfaced via the pi ``tool_call`` hook). Pi uses the same
    # ``path`` argument key as the OmniCraft tools, so no Pi-specific arg
