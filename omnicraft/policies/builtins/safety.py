...
# PostToolUse hook contract (see ``omnicraft.native_policy_hook``).
# These bypass OmniCraft' ``sys_os_*`` MCP tools and execute directly
...
# (see ``omnicraft.inner.cursor_policy_hook``). Cursor uses ``Shell``
...
# extension hook (see ``omnicraft.inner.pi_executor._gate_native_tool``).
...
# as the OmniCraft ``sys_os_*`` tools (``path`` / ``command``), so the
...
# (see ``omnicraft.inner.hermes_policy_hook``). Hermes uses its own naming
...
def ask_on_os_tools(event: PolicyEvent) -> PolicyResponse:
    ...
    - **OmniCraft built-in OS tools** (``sys_os_read``,
    ...
    if tool in _all_os_tools:
        ...
        if tool in ("sys_os_shell", "Bash", "bash", "Shell", "terminal", "developer__shell"):
            ...
            # OmniCraft tools use ``path``; Claude native tools use ``file_path``.
            preview = (
...
def block_skills(blocked: list[str]) -> PolicyCallable:
    ...
    2. **Native ``Skill`` tool** — Claude Code's built-in ``Skill`` tool,
       ...
       the OmniCraft server's ``/policies/evaluate`` endpoint. This is how
    ...
    3. **Slash commands** — ``/skill-name`` commands submitted by the
       user (or UI). The OmniCraft server evaluates these at the ``request``
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        if event_type == "tool_call":
            ...
            # Path 1: OmniCraft runner tools (load_skill / read_skill_file).
            ...
            # Fired via PreToolUse hook → OmniCraft /policies/evaluate.
        ...
        # The OmniCraft server converts ``/skill-name args`` into a synthetic
...
def deny_pii_in_llm_request(
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        - ``request``: user message text, enforced universally on
          the OmniCraft server for every harness (including supervisor,
          native).
