...
class FunctionPolicy(Policy):
    ...
    def reset_turn(self) -> None:
        ...
        Stateful policy callables (notably the legacy omnicraft
        ...
        no-op. Modern omnicraft-native callables that need
...
def resolve_function_policy(spec: FunctionPolicySpec) -> FunctionPolicy:
    ...
    # function.py → _omnicraft_legacy_shim → policies.types →
...
def _coerce_to_policy_result(raw: Any, *, spec_name: str) -> PolicyResult:
    ...
    - A foreign PolicyResult-shaped object (any object with at
      least ``.action``, e.g. ``omnicraft.policies.PolicyResult``
      returned by callables ported from the omnicraft examples).
    ...
    if hasattr(raw, "action"):
        ...
        # ``omnicraft.policies.PolicyResult`` that happen to
