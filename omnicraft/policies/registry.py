...
Usage::
    ...
    from omnicraft.policies.registry import load_registry, get_registry
    ...
    schema = get_params_schema("omnicraft.policies.builtins.safety.max_tool_calls_per_turn")
"""
...
class PolicyRegistryEntry:
    ...
    :param handler: Full dotted import path to the callable,
        e.g. ``"omnicraft.policies.builtins.safety.ask_on_os_tools"``.
...
def load_registry(
    ...
    """
    from omnicraft.policies.builtins import BUILTIN_POLICY_MODULES
...
def is_registered_handler(handler: str) -> bool:
    ...
    (:func:`omnicraft.server.bundles.validate_agent_bundle`, which scans
    parsed specs / raw bundle YAML before any handler is resolved or
    called). Trusted spec loading (local ``omnicraft run``, operator
...
def get_params_schema(handler: str) -> dict[str, Any] | None:
    ...
    :param handler: Full dotted import path, e.g.
        ``"omnicraft.policies.builtins.safety.max_tool_calls_per_turn"``.
