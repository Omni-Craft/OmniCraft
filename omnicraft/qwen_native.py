"""Native qwen TUI wrapper for the OmniCraft CLI.

``omnicraft qwen`` launches Qwen Code's interactive TUI (``qwen``) inside an
OmniCraft-runner-owned tmux terminal and attaches the local TTY — the qwen analog
of ``omnicraft goose`` / ``omnicraft cursor``. The runner spawns the process (see
:func:`omnicraft.runner.app._auto_create_qwen_terminal`), pointing qwen at the
...
``/auth`` command persisted under ``~/.qwen``); no OmniCraft-managed key is
...
from omnicraft._native_resume_hint import echo_native_cold_resume_hint, echo_native_resume_hint
from omnicraft._runner_startup import RunnerStartupProgress, runner_startup_progress
from omnicraft._wrapper_labels import QWEN_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_QWEN_PATH_ENV = "OMNICRAFT_QWEN_PATH"
...
class PreparedQwenTerminal:
    ...
    :param cold_resumed: ``True`` when resuming an existing OmniCraft session whose
...
def run_qwen_native(
    *,
    ...
    """
    Launch the qwen TUI in an OmniCraft terminal.

    :param server: Resolved OmniCraft server URL.
    ...
    if server is None:
        raise click.ClickException(
            "qwen requires a resolved OmniCraft server URL. The CLI should call "
            "_ensure_backend before run_qwen_native."
        )
    with TemporaryDirectory(prefix="omnicraft-qwen-native-") as tmpdir:
...
def _materialize_qwen_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``omnicraft qwen``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch qwen on an OmniCraft server via a daemon-spawned runner.
