...
- ``--json-file`` (:func:`events_file_path`): ``qwen`` streams structured JSON
  events here while the TUI renders normally; :mod:`omnicraft.qwen_native_forwarder`
  tails it.
...
#: Fixed namespace for deriving a stable qwen ``--session-id`` from an OmniCraft
...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"omnicraft-{os.getuid()}" / "qwen-native"
...
#: Token config the shared OmniCraft MCP relay (``serve-mcp``) reads from the
#: bridge dir. Mirrors cursor-/claude-native (``cursor_native_bridge.py``).
_BRIDGE_CONFIG_FILE = "bridge.json"
#: Name qwen lists the OmniCraft MCP server under (shows in ``/mcp``).
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/omnicraft-<uid>/qwen-native/<hash>``."""
...
def qwen_session_id_for_conversation(conversation_id: str) -> str:
    """Return the deterministic qwen ``--session-id`` for an OmniCraft conversation.
...
def qwen_session_recording_path(session_id: str, workspace: Path | str) -> Path:
    ...
    ``chat_compression`` marker (see :mod:`omnicraft.qwen_native_forwarder`).
    """
    return (
        Path.home()
        / ".qwen"
...
def _qwen_text_from_api_content(content: object, api_type: str) -> str:
    """Concatenate the text of an OmniCraft content array's blocks of *api_type*.

    :param content: OmniCraft content array, e.g. ``[{"type":"input_text","text":"hi"}]``.
...
def qwen_session_records_from_session_items(
    ...
    *,
    ...
    """Convert OmniCraft session items into qwen chat-recording JSONL records.
...
def write_qwen_session_recording(
    ...
    *,
    ...
    try:
        hostname = socket.gethostname() or "omnicraft"
    except OSError:
        hostname = "omnicraft"
...
def _ensure_secure_bridge_dir(bridge_dir: Path) -> None:
    ...
    an attacker could pre-create ``$TMPDIR/omnicraft-<uid>`` (or a deeper ancestor)
    ...
    """
    from omnicraft.claude_native_bridge import _ensure_secure_dir
...
def build_qwen_native_spawn_env(session_id: str) -> dict[str, str]:
    ...
    :param session_id: The OmniCraft session id (keys the bridge dir).
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :func:`omnicraft.cursor_native_bridge.write_mcp_bridge_config`.
...
def build_mcp_server_entry(
    ...
    *,
    ...
    """Build qwen's ``mcpServers.omnicraft`` entry for the OmniCraft relay.

    ``trust: true`` auto-approves the qwen-side MCP tool gate so the TUI doesn't
    add a second in-terminal prompt: OmniCraft already gates these calls through
...
def write_mcp_config(
    ...
    *,
    ...
    """Write the per-session OmniCraft MCP config for qwen's ``--mcp-config`` flag.

    Writes ``{"mcpServers": {"omnicraft": ...}}`` to a file *inside the bridge dir*
