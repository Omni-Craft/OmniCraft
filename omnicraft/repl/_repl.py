"""Rich-based REPL for omnicraft — built on the UI SDK framework.
...
# ``from omnicraft_ui_sdk import FormattedItem``.
from omnicraft_ui_sdk.terminal._completer import FileMentionCompleter
from omnicraft_ui_sdk.terminal._formatter import FormattedItem
from omnicraft_ui_sdk.terminal._theme import LIGHT_THEME, get_theme
...
if TYPE_CHECKING:
    from omnicraft.server.schemas import SessionStatusEvent
...
SlashCommandHandler = Callable[
    [str, Session, OmniCraftClient, TerminalHost, RichBlockFormatter],
    Awaitable[None],
]
...
def _load_startup_theme() -> TerminalTheme:
    ...
    On first launch (no persisted theme in ``~/.omnicraft/config.yaml``),
    ...
    from omnicraft.repl._theme_picker import startup_theme_picker
...
def _display_cwd() -> str:
    ...
    :returns: The cwd with ``$HOME`` collapsed to ``~`` (e.g.
        ``"~/omnicraft"``), or the absolute path when it is not
...
def _header_glyph(kind: str) -> str:
    ...
    keeps its :func:`kind_glyph`. CLI surfaces (``omnicraft setup``, the
    ...
    from omnicraft.onboarding.provider_config import SUBSCRIPTION_KIND
...
def _render_startup_banner_ansi(
    ...
    *,
    ...
    starfish magenta-pink brand color (``#0fb5bd``) so the box border,
    ...
    """
    from omnicraft.conversation_browser import display_server_url, is_workspace_hosted_url
    from omnicraft.inner.banner import BannerLine, startup_banner_strings

    remote = _is_remote_server_url(server_url)
    # User-facing form of the URL: a Databricks workspace-hosted server is
    # connected to on its ``/api/2.0/omnicraft`` API mount, but the banner
    # should show the recognizable workspace ``/omnicraft`` URL. Non-Databricks
...
async def _fetch_server_version(client: OmniCraftClient) -> str | None:
    ...
    Routed through the REPL's already-connected :class:`OmniCraftClient` so
...
def _is_remote_server_url(url: str | None) -> bool:
    ...
    A local ``omnicraft run`` spawns its own OmniCraft server on
...
class _ApprovalState:
    ...
    def remember_always(self, policy_name: str, phase: str) -> None:
        ...
        cache is NEVER persisted to disk; closing ``omnicraft chat``
...
def _build_elicitation_content_from_schema(
    ...
    See :func:`omnicraft.tools._elicitation_schema.build_accept_content_from_schema`
    ...
    """
    from omnicraft.tools._elicitation_schema import build_accept_content_from_schema
...
def _elicitation_resolve_session_id(sdk_event: object, fallback_session_id: str) -> str:
    ...
    :param sdk_event: The translated
        :class:`omnicraft_client._events.ElicitationRequest`; its
...
def _server_event_to_sdk_event(event: object) -> object | None:
    ...
    :mod:`omnicraft_client._events`. Returns ``None`` for variants
    ...
    from omnicraft_client._types import Response as SDKResponse
...
class _SessionsChatReplAdapter:
    ...
    def __init__(
        ...
        :param client: The :class:`OmniCraftClient` used to
        ...
        :param attach_only: When ``True``, run as a pure co-drive client:
            ...
            existing host-bound runner). Used by ``omnicraft attach``.
    ...
    async def _runner_recover_watch(self) -> None:
        ...
        :returns: None.
        """
        _dbg = bool(os.environ.get("OMNICRAFT_SESSIONS_ADAPTER_DEBUG"))
        while True:
            try:
    ...
    async def _ensure_session(self) -> str:
        ...
        Set ``OMNICRAFT_SESSIONS_ADAPTER_DEBUG=1`` to trace
    ...
    def _emit_runner_recovery_error_once(self, exc: Exception) -> None:
        ...
        :param exc: Failure raised while relaunching or rebinding a
            runner, e.g. an SDK ``OmniCraftError`` from
    ...
    def _is_terminal_runner_recovery_error(self, exc: Exception) -> bool:
        ...
        :returns: ``True`` for typed server runner-state errors,
            ...
            retrying.
        """
        if not isinstance(exc, OmniCraftError):
            return False
        code = exc.code or ""
    ...
    async def _stream_pump(self) -> None:
        ...
        """
        from omnicraft.server.schemas import SessionStatusEvent as _StatusEv
    ...
    async def send(
        ...
        *,
        ...
        try:
            ...
            # _on_event.
            from omnicraft_client._events import ResponseCompleted
            from omnicraft_client._types import Response as SDKResponse
    ...
    async def send_skill_slash_command(
        ...
        The OmniCraft server persists a visible ``slash_command`` item and
    ...
    def _spawn_client_tool(
        ...
        async def _run() -> None:
            try:
                ...
                args = {}
            from omnicraft_client._tool_handler import ToolCallInfo
    ...
    async def _handle_elicitation(
        ...
        from omnicraft_client._tool_handler import ElicitationRequestCtx
...
class _TurnProseTracker:
    ...
    (``_flush_relay_text`` in ``omnicraft/server/routes/sessions.py``).
...
async def run_repl(
    ...
    *,
    ...
    :param client: Connected OmniCraftClient.
    ...
    :param resume_conversation_id: When set, the REPL opens
        ...
        ``--resume <id>`` (see designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md).
    ...
    :param log_dir: When set, write a JSON dump of the active
        ...
        the CLI ``--log`` flag (and the ``~/.omnicraft/logs/``
        default location); see ``omnicraft.repl._session_log`` for
        the schema. The dump runs in the SAME ``async with
        OmniCraftClient(...)`` scope as the REPL itself, so the
    ...
    :param server_url: Base URL of the OmniCraft server the REPL is
    ...
    # behavior (omnicraft/inner/cli.py:2979 + :2984): when a user
    ...
    from omnicraft.repl._event_tape import EventTape, PipelineCounters, TapeEntry
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, _StatusEv):
            ...
            if event.status == "running":
                from omnicraft_client import BlockContext, ResponseStartBlock
                ...
                from omnicraft_client import TextDone
        ...
        from omnicraft.server.schemas import ElicitationResolvedEvent
        ...
        if isinstance(sdk_ev, _TD):
            from omnicraft_client import TextChunk
        ...
        if isinstance(sdk_ev, _RS):
            from omnicraft_client import BlockContext, ReasoningStartBlock
        ...
        if isinstance(sdk_ev, _RD | _RSD):
            from omnicraft_client import BlockContext
            from omnicraft_client import ReasoningChunk as _RC
        ...
        from omnicraft_client._events import ResponseCompleted as _Completed
        ...
        from omnicraft_client._events import ErrorEvent as _ErrorEvent
        ...
        from omnicraft_client._events import ResponseFailed as _Failed
    ...
    def _maybe_log_tape_entry(entry: TapeEntry) -> None:
        ...
        if _event_log_fh is not None:
            from omnicraft.repl._event_tape import log_entry_jsonl
    ...
    # Paint the composer in the omnicraft-logo green while the line is a "!"
    ...
    async def on_input(text: str, attachments: list[PendingAttachment] | None = None) -> None:
        ...
        try:
            ...
            # was the exact user-reported bug where ``omnicraft chat`` would
    ...
    # usage totals) is omnicraft-specific. The SDK's
    ...
    # fires in `omnicraft chat`'s pinned-prompt mode. Ctrl+O is not grabbed
    # by the common terminal emulators we target (iTerm2, Terminal.app,
    # Warp) and prompt-toolkit binds it cleanly.
    from omnicraft_ui_sdk import Overlay
    ...
    # ``omnicraft/inner/cli.py:1791-1797``.
    from omnicraft_ui_sdk import OverlayAction
    ...
    if debug_events and _event_tape is not None:
        from omnicraft.repl._event_tape import build_tape_detail, build_tape_targets
        ...
        async def _tape_targets() -> list[OverlayTarget]:
            ...
            """
            from omnicraft.repl._event_tape import _OverlayTargetLike
    ...
    async with host:
        ...
        if debug_events:
            ...
            from omnicraft.repl._event_tape import open_event_log
        ...
        # OmniCraft REPL feels identical at boot. Raw stdout write
        # (matching ``omnicraft/inner/cli.py:2962``) — the banner
        ...
        # streamed yet. See
        # ``designs/RUN_OMNICRAFT_REPL_PARITY.md``.
        import sys as _sys
        ...
        from omnicraft.conversation_browser import is_workspace_hosted_url
        ...
        from omnicraft_ui_sdk import StreamingText
    ...
    # Top-level ``omnicraft resume`` only dispatches claude-native today;
...
async def _maybe_write_session_log(
    ...
    :param client: Connected OmniCraftClient — REUSED, not opened
...
async def _cmd_help(
    ...
    client: OmniCraftClient,  # noqa: ARG001
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    from rich.text import Text
...
async def _cmd_theme(
    ...
    client: OmniCraftClient,  # noqa: ARG001 — dispatch-contract params
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    """
    from omnicraft_ui_sdk.terminal._theme import DARK_THEME, LIGHT_THEME
    from rich.text import Text

    from omnicraft.repl._theme_picker import _build_preview, build_theme_confirmation
...
def _model_readout_harness(active_model: str | None) -> str:
    ...
    :func:`omnicraft.llms.routing.infer_harness_from_model`, falling back
...
def _build_model_readout_lines(
    ...
    it goes through ``omnicraft setup --no-internal-beta`` + a restart. Falls
    ...
    :param config: The parsed effective config mapping (``providers:``
        block), e.g. from
        :func:`omnicraft.onboarding.provider_config.load_config`.
    ...
    if others:
        ...
        lines.append(
            "  /model <name> changes the model. To switch provider: omnicraft setup (then restart)."
        )
    return lines
...
def _match_configured_provider(config: dict[str, object], token: str) -> str | None:
    ...
    """
    from omnicraft.onboarding.configure_models import provider_display_name
...
def _model_validation_warning(model: str) -> str | None:
    ...
    (:func:`omnicraft.onboarding.providers.get_chat_models`). An unknown
    ...
    from omnicraft.llms.routing import parse_model_string
    from omnicraft.onboarding.providers import get_chat_models
...
async def _cmd_model(
    ...
    (it goes through ``omnicraft setup --no-internal-beta`` + a restart).
    ...
    if not value:
        from omnicraft.onboarding.detected import effective_config_with_detected
        from omnicraft.onboarding.provider_config import load_config
    ...
    from omnicraft.onboarding.configure_models import kind_glyph, provider_display_name
    from omnicraft.onboarding.detected import effective_config_with_detected
    from omnicraft.onboarding.provider_config import (
...
async def _attach_to_conversation(
    ...
    *,
    ...
    - ``run_repl(resume_conversation_id=...)`` on startup
      ...
      designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md) — passes
    ...
    :param client: The :class:`OmniCraftClient` for fetching
        items.
    ...
    if last_response_id is None:
        ...
        # `omnicraft run` the daemon hands the REPL a freshly-created session
    ...
    if cw:
        ...
        if last_total is not None:
            ...
            from omnicraft.runtime.compaction import count_tokens
...
# Mirrors _DEFAULT_TRIGGER_THRESHOLD from omnicraft.runtime.compaction.
...
async def _refresh_session_metadata(
    ...
    :param client: OmniCraft HTTP client used to fetch the snapshot.
...
async def _update_context_ring_estimate(
    ...
    :param client: OmniCraft HTTP client used to query items.
...
def _build_github_issue_url(
    ...
    body_parts: list[str] = [
        ...
        *info_lines,
        ...
        "     --debug-events for a JSONL event log in ~/.omnicraft/debug/ -->",
    ]

    body = "\n".join(body_parts)
    base = "https://github.com/omnicraft-ai/omnicraft/issues/new"
    return (
        f"{base}"
...
async def _cmd_logs(
    ...
    from omnicraft.cli_diagnostics import current_cli_log_path
    from omnicraft.repl._session_log import write_logs_zip, write_session_log
...
async def _cmd_report(
    arg: str,
    session: Session,
    client: OmniCraftClient,  # noqa: ARG001 — dispatch-contract param
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    try:
        version = _pkg_version("omnicraft")
    except Exception:  # noqa: BLE001
        version = None
...
async def _collect_terminals_for_conversations(
    ...
    *,
    ...
    :param client: The omnicraft HTTP client.
...
class _TerminalInfo:
    ...
    :param target: Tmux target. Always ``"main"`` per the
        ...
        (``omnicraft/inner/terminal.py:167``); kept on the
...
def _parse_terminal_tool_output(raw: object) -> dict[str, object] | None:
    ...
    string (default executor, omnicraft builtins) and
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    ``omnicraft/inner/cli.py::_open_current_terminal_window``
    so users with muscle memory from the legacy CLI see the
    same behavior under OmniCraft mode.
    ...
    :param client: OmniCraft HTTP client — used to re-walk the
...
def _terminal_attach_command(info: _TerminalInfo) -> str:
    ...
    :meth:`omnicraft.inner.cli._terminal_attach_command`
...
async def _build_terminal_overview(
    ...
    *,
    ...
    :meth:`omnicraft.inner.cli._render_overview_terminal_text`
    output (``cli.py:2232``):
...
def _tmux_session_alive(socket: str, target: str) -> bool:
    ...
    :param socket: Tmux socket path the agent's
        ...
        ``"/tmp/omnicraft-terminal-xyz/tmux.sock"``.
    :param target: Tmux session/target name. Always
        ``"main"`` per
        :class:`omnicraft.inner.terminal.TerminalInstance.tmux_target`.
...
def _parse_sub_agent_handle(raw: str) -> dict[str, object] | None:
    ...
    Native omnicraft builtins (``sys_session_send`` /
...
async def _build_debug_overview(
    ...
    *,
    ...
    The overview intentionally mirrors the ``omnicraft run``
    ...
    2. **Event stream** — all items from the conversation,
       ...
       ``[N] type=...`` shape omnicraft uses. Responses API
       items map onto the omnicraft event vocabulary as
    ...
    :param client: The omnicraft client used for the items fetch.
    ...
    :param cli_log_path: Optional path to the always-on CLI
        diagnostics log (``~/.omnicraft/logs/cli-*.log``).
    ...
    # pair — matching the omnicraft event view. When an output
...
def _render_overview_message_event(
    ...
    """
    Render a ``message`` item as an omnicraft event.

    Splits on role so the header reads ``user_message`` or
    ``assistant_message`` (matching omnicraft' event vocabulary)
...
def _coerce_arguments_dict(raw: object) -> dict[str, object]:
    ...
    The omnicraft API surfaces tool-call arguments as a JSON
...
def _render_message_history_item(
    ...
    if role == "user":
        ...
        return
    if role == "assistant":
        # Skip empty assistant messages. The omnicraft workflow
...
_BANG_TIMEOUT_S: float = float(os.environ.get("OMNICRAFT_BANG_TIMEOUT_S") or 120.0)
_BANG_DISPLAY_MAX: int = int(os.environ.get("OMNICRAFT_BANG_DISPLAY_MAX") or 30_000)
_BANG_CONTEXT_MAX: int = int(os.environ.get("OMNICRAFT_BANG_CONTEXT_MAX") or 16_000)

# The omnicraft-logo green. Marks a "!" shell command consistently: the composer
...
class _BangInputLexer(Lexer):
    ...
    the shell via the passthrough; painting it in the omnicraft-logo green is
...
def _write_bang_overflow(cmd: str, stdout: str, stderr: str) -> str | None:
    ...
    if len(plain_out) + len(plain_err) <= _BANG_CONTEXT_MAX:
        return None
    fd, path = tempfile.mkstemp(prefix="omnicraft-bang-", suffix=".log")
