...
Conversations come from OmniCraft' conversation store.
...
# ``omnicraft._wrapper_labels``. Imported here so the picker module
...
from omnicraft.claude_native_state import read_launch_state as _read_claude_launch_state
from omnicraft.codex_native_state import read_launch_state as _read_codex_launch_state
from omnicraft.harness_plugins import CODEX_NATIVE_CODING_AGENT
...
# ``❯`` mirrors :meth:`omnicraft_ui_sdk.RichBlockFormatter.\
...
class _ConversationRow(Protocol):
    ...
    Both the SDK's :class:`omnicraft_client.types.Conversation`
    and the store's
    :class:`omnicraft.entities.conversation.Conversation`
    ...
    :param labels: Session-scoped labels (read for the Runtime
        metadata badge — the ``omnicraft.wrapper`` key identifies
...
def _runtime_badge(row: _ConversationRow) -> str:
    ...
    The badge surfaces which OmniCraft wrapper owns the session so a
...
def pick_conversation(
    ...
    *,
    ...
    :param show_runtime: When ``True``, render runtime metadata
        ...
        picker (``omnicraft resume``) where a user is choosing
...
async def pick_conversation_by_wrapper_label_from_sdk(
    ...
    *,
    ...
    """Picker scoped to one wrapper kind (``omnicraft.wrapper=<value>``).
    ...
    (``~/.omnicraft/claude-native/``); sessions created on a
...
async def pick_conversation_cross_agent_from_sdk(
    ...
    *,
    ...
    ``omnicraft resume``'s runtime-dispatch UX."""
...
def _render_workspace_cell(row: _ConversationRow, *, current_cwd: Path) -> Any | None:
    ...
    *current_cwd* (resolved). When state is absent -- legacy session
    ...
    ``~/.omnicraft/claude-native/<hash>/launch.json`` -- fast on
...
async def _collect_previews_async(
    ...
    :param client: The :class:`omnicraft_client.OmniCraftClient`.
...
def _last_message_preview_from_entities(items: list[Any]) -> _Preview | None:
    ...
    :class:`omnicraft.entities.ConversationItem` objects whose
