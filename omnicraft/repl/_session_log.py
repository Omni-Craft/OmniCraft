"""
JSON dump of an omnicraft conversation — the OmniCraft mode port of the
legacy ``--log`` feature (see designs/RUN_OMNICRAFT_REPL_PARITY.md).

The legacy non-AP path wrote a session-shaped JSON to
``~/.omnicraft/logs/`` at REPL exit (`_write_session_log` /
``_session_log_dict`` in ``omnicraft/inner/cli.py``). That format
was tied to the in-memory :class:`Session` machinery — omnicraft
...
``omnicraft/repl/_repl.py``), so spawn handles are recognized
...
from omnicraft_ui_sdk import state_dir
...
_LOG_FORMAT = "omnicraft-conversation"

# Default log directory. Derives from the shared ``state_dir()``
# (``~/.omnicraft``) so the root is defined in one place. Mirrors
# the legacy non-AP path's ``~/.omnicraft/logs/`` (see
# ``_default_session_log_path`` in ``omnicraft/inner/cli.py``) so
...
def default_log_zip_path(
    ...
    *,
    ...
    Format: ``{output_dir}/omnicraft-logs-{session_slug}-{timestamp}.zip``
    when a session id is available, otherwise
    ``{output_dir}/omnicraft-logs-{timestamp}.zip``. ``None`` uses
    :data:`DEFAULT_LOG_ZIP_DIR`, i.e. ``~/.omnicraft/logs``.
    ...
    if slug is not None:
        return base / f"omnicraft-logs-{slug}-{timestamp}.zip"
    return base / f"omnicraft-logs-{timestamp}.zip"
...
def collect_log_files(log_paths: list[Path]) -> list[tuple[Path, str]]:
    ...
    This helper does **not** walk ``~/.omnicraft/logs`` or
    ``~/.omnicraft/debug`` wholesale; doing so would include unrelated
    sessions.
...
def default_log_path(conversation_id: str, log_dir: Path | None = None) -> Path:
    ...
    treatment in ``omnicraft/inner/cli.py::_default_session_log_path``.
...
async def write_session_log(
    ...
    *,
    ...
    in ``omnicraft/repl/_repl.py`` — same parser the Ctrl+O debug
    ...
    :param client: A connected :class:`OmniCraftClient`. The
    ...
    :param agent_name: Agent name to include in the dump (for
        ...
        e.g. ``"resume_test"``.
    :param log_dir: Override the default location. ``None`` uses
        :data:`DEFAULT_LOG_DIR` (``~/.omnicraft/logs/``).
...
def _extract_child_conversation_ids(items: list[dict[str, object]]) -> list[str]:
    ...
    from omnicraft.repl._repl import _parse_sub_agent_handle
...
def write_session_log_from_store(
    ...
    *,
    ...
    :class:`omnicraft.stores.conversation_store.ConversationStore`
    directly instead of the SDK.

    Used by the one-shot ``omnicraft run <yaml> -p "…" --log`` path
    where the in-process ASGI app doesn't have a connected
    :class:`OmniCraftClient` — the run goes through raw httpx +
...
def _fetch_all_items_sync(conv_store: Any, conversation_id: str) -> list[dict[str, object]]:
    ...
    self-contained — a reader doesn't need any of OmniCraft'
...
async def _fetch_all_items_via_sessions(
    ...
    :param client: The connected :class:`OmniCraftClient`.
