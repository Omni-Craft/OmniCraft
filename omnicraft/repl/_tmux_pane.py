...
1. Marks the pane with custom options so other tooling can identify
   it as an omnicraft pane and recover the launch args.
2. Discovers the user's existing prefix-table ``split-window`` /
   ...
   - When the focused pane has the ``@omnicraft-conv-id`` option set
     (i.e. it's an omnicraft pane), runs ``omnicraft pane-split``
...
non-omnicraft panes (the else branch is the user's untouched
command), so the global mutation of the ``prefix`` table is
behaviorally invisible everywhere except inside an omnicraft pane.
...
# branch is the user's exact original command, so non-omnicraft
# panes keep working.
PANE_INTEGRATION_ENABLED = False

# Pane-option keys we set on the omnicraft pane. Each is read by
# ``omnicraft pane-picker`` to reconstruct the launch context for
# the new pane.
OPT_CONV_ID = "@omnicraft-conv-id"
OPT_AGENT_NAME = "@omnicraft-agent-name"
OPT_AGENT_YAML = "@omnicraft-agent-yaml"
OPT_LAUNCH_ARGV = "@omnicraft-launch-argv"
OPT_SERVER_URL = "@omnicraft-server-url"

# Sentinel value for ``OPT_CONV_ID`` before the first conversation
# id is known. The wrapper's ``#{?#{@omnicraft-conv-id},...}``
# truthiness check fires for any non-empty string, so this
# preserves "yes, this is an omnicraft pane" even when there's no
# real conv id yet.
...
class SplitBinding:
    ...
    :param direction: One of ``'v'`` (vertical split), ``'h'``
        ...
        value passed to ``omnicraft pane-split``.
    :param original_command: The full command string from the
        ...
        wrapper's else branch so non-omnicraft panes get
...
def _resolve_omnicraft_argv() -> list[str]:
    """
    Resolve a callable argv prefix for invoking the running
    omnicraft installation from a context that doesn't share our
    ...
    1. ``sys.argv[0]`` is path-shaped (contains ``/``) — abspath
       it. Covers ``./.venv/bin/omnicraft …`` and absolute-path
       invocations.
    2. ``sys.argv[0]`` is a bare name like ``"omnicraft"`` — try
    ...
    3. Fallback — ``[sys.executable, "-m", "omnicraft.cli"]``.
       Always works because if Python is running this code, the
       ``omnicraft.cli`` module is importable. Trades aesthetics
       (the binding shows ``python -m omnicraft.cli``) for
    ...
    :returns: A list of one or two argv elements. Length-1 means
        ...
        ``[python, "-m", "omnicraft.cli"]``. The caller appends
        the subcommand and its args (e.g. ``["pane-split", "-v",
        ...]``) to this prefix.
    """
    argv0 = sys.argv[0] if sys.argv else ""
    # When the process was launched via ``python -m omnicraft.cli``
    ...
    if argv0.endswith(".py"):
        return [sys.executable, "-m", "omnicraft.cli"]
    ...
    # ``omnicraft.cli`` module is importable since we're running
...
# match is the launcher prefix (the omnicraft binary, ``python
# -m omnicraft.cli``, etc.) and gets stripped during
...
def _user_args_after_launcher(launch_argv: list[str]) -> list[str]:
    ...
    - ``[<omnicraft-binary>, run, …]`` (PATH-resolved console script)
    - ``[<python>, -m, omnicraft.cli, run, …]`` (python-m fallback)
    - ``[<python>, -m, omnicraft.cli, -m, omnicraft.cli, run, …]``
    ...
    :param launch_argv: ``sys.argv`` (or stored ``launch_argv``)
        of the running ``omnicraft`` invocation.
...
def _unset_pane_options(pane_id: str) -> None:
    """
    Remove every ``@omnicraft-*`` pane option this module sets.
    ...
    ``if-shell -F '#{?#{@omnicraft-conv-id},...}'`` wrapper falls
...
def _classify(cmd_tokens: list[str]) -> str | None:
    ...
    routing in omnicraft panes. Acceptable v1 limitation.

    :param cmd_tokens: The command portion of a bind-key line,
        e.g. ``['split-window', '-h', '-c', '#{pane_current_path}']``.
    :returns: Direction code consumed by ``omnicraft pane-split``,
        or ``None``.
    """
    if not cmd_tokens:
        return None
...
_WRAPPER_MARKER_FORMAT = "#{?#{@omnicraft-conv-id},1,0}"


def _unwrap_existing_wrapper(cmd_tokens: list[str]) -> list[str] | None:
    """
    Peel off an existing omnicraft wrapper to recover the user's
...
def _discover_split_bindings() -> list[SplitBinding]:
    ...
    an unfamiliar flag set, are silently skipped. Non-omnicraft
    panes still see those bindings unchanged (they're never
    rewritten); omnicraft panes just don't get chooser routing
    on those keys.
    """
...
def _wrap_binding(binding: SplitBinding, omnicraft_argv: list[str]) -> None:
    """
    Replace one of the user's prefix-table bindings with an
    ``if-shell -F`` wrapper.

    True branch (focused pane is an omnicraft — the
    ``@omnicraft-conv-id`` option is set and non-empty): run
    the resolved ``omnicraft pane-split`` invocation via
    ...
    when the omnicraft branch fires.

    :param binding: One of the user's prefix-table split-window /
        new-window bindings.
    :param omnicraft_argv: Argv prefix for invoking the running
        omnicraft installation, as returned by
        :func:`_resolve_omnicraft_argv`. Length 1 (e.g.
        ``["/venv/bin/omnicraft"]``) for a direct binary
        invocation, or length 3 (``[sys.executable, "-m",
        "omnicraft.cli"]``) for the python-m fallback. Either
...
def register_pane(
    *,
    ...
    """
    Mark the current tmux pane as an omnicraft pane and wrap the
    ...
    :param agent_yaml: Path to the agent spec, or ``None`` when
        ...
        ``OPT_AGENT_YAML``.
    :param launch_argv: ``sys.argv`` of the running ``omnicraft
        run`` process. Re-played verbatim by the chooser when the
        user picks "new conversation with same agent".
    :param server_url: OmniCraft server base URL (e.g.
    ...
    if not PANE_INTEGRATION_ENABLED:
        ...
        # ``#{?#{@omnicraft-conv-id},1,0}`` check returns 0, so the
    ...
    pane_id = _tmux_pane_id()
    if pane_id is None:
        _LOGGER.warning("$TMUX is set but $TMUX_PANE is not — skipping omnicraft pane integration")
        return
    if not _tmux_version_ok():
        _LOGGER.warning(
            "tmux >= %s required for omnicraft pane integration; skipping",
            ".".join(str(n) for n in _MIN_TMUX_VERSION),
        )
        return

    # Resolve a callable ``omnicraft`` argv prefix. tmux's
    ...
    # ``omnicraft`` lives — so a bare ``omnicraft`` exits 127.
    # ``sys.argv[0]`` is the bare name when the user invoked us
    # via ``$PATH`` (e.g. typed ``omnicraft`` after activating
    # a venv), so we can't just pass through ``launch_argv[0]``.
    # See :func:`_resolve_omnicraft_argv` for the resolution
    # ladder (abspath → ``shutil.which`` → ``python -m`` fallback).
    omnicraft_argv = _resolve_omnicraft_argv()
    ...
    # ``omnicraft_argv``. The user's subcommand and args
    # (``run agent.yaml …``) survive untouched.
    #
    # Idempotency under repeat calls: ``_user_args_after_launcher``
    # walks past leading launcher tokens (the omnicraft binary,
    # ``python``, ``-m``, ``omnicraft.cli`` — including doubled
    ...
    # ``-m omnicraft.cli`` markers each time is exactly what this
    # repairs.
    user_args = _user_args_after_launcher(launch_argv)
    normalized_launch_argv = list(omnicraft_argv) + user_args
    ...
    for binding in _discover_split_bindings():
        _wrap_binding(binding, omnicraft_argv)
