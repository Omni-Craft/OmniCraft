// Auto-generated OmniCraft bridge extension for native Pi sessions.
const fs = require("fs");
...
// (omnicraft.native_policy_hook.post_evaluate_with_retry, ~1-day client
...
// _omnicraft_elicitation_id so the server RE-ATTACHES to the existing
...
// omnicraft.policies.types.FAIL_CLOSED_PHASES and the Python native hook's
// fail_closed_hook_output(PreToolUse) → deny.
const _FAIL_CLOSED_REASON =
  "blocked: OmniCraft policy server unreachable — failing closed (PHASE_TOOL_CALL)";
...
function readConfig() {
  const configPath = process.env.OMNICRAFT_PI_NATIVE_CONFIG;
...
/**
 * Evaluate a TOOL_CALL policy for a native Pi tool via the OmniCraft server's
 ...
 * hooks. It does NOT require an active OmniCraft turn context on the harness
 ...
 *       (headersTimeout) we re-POST the SAME _omnicraft_elicitation_id so the
...
async function evalNativePolicyHttp(config, toolName, args) {
  ...
  const body = JSON.stringify({
    ...
    _omnicraft_elicitation_id: elicitationId,
  });
  const reqHeaders = {
  ...
  while (true) {
    ...
    if (result === "POLICY_ACTION_DENY") {
      return {
        block: true,
        reason: json.reason || "blocked by OmniCraft policy",
      };
    }
 ...
 *
 * Pi expects ``{ content: [{ type: "text", text }], isError }``. The OmniCraft
...
function piResultFromMcpResponse(json) {
  if (json && typeof json === "object" && json.error) {
    const msg =
      (json.error && json.error.message) || "OmniCraft tool call failed";
  ...
  // masquerading as a successful tool result. callOmniCraftTool detects and
  ...
  if (result && typeof result === "object" && result.resultType === "input_required") {
    return {
      content: [
        {
          type: "text",
          text: "OmniCraft tool call requires approval that was not resolved",
        },
      ],
      isError: true,
    };
 ...
 *
 * The OmniCraft server keys ``inputRequests`` by the server-minted elicitation
      ...
      return {
        piResult: {
          content: [
            {
              type: "text",
              text: `OmniCraft tool call failed: HTTP ${resp.status}`,
            },
          ],
          isError: true,
        },
    ...
    return {
      piResult: {
        content: [
          {
            type: "text",
            text: `OmniCraft tool call failed: ${err && err.message ? err.message : String(err)}`,
          },
        ],
        isError: true,
      },
...
/**
 * Execute an OmniCraft tool by POSTing a JSON-RPC ``tools/call`` request to the
 * server's per-session MCP proxy endpoint
 * (``POST /v1/sessions/{sessionId}/mcp``).
 *
 * This is the SAME endpoint the runner's ``ProxyMcpManager`` uses: the OmniCraft
 ...
 * agent loop.
 */
async function callOmniCraftTool(config, toolName, args) {
  if (
    !config ||
    !config.serverUrl ||
    !config.sessionId ||
    ...
    return {
      content: [
        { type: "text", text: "OmniCraft tool bridge is not configured" },
      ],
      isError: true,
    };
  }
  ...
  if (!initial.elicitationId || !initial.requestState) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "OmniCraft tool call requires approval but the server sent no resolvable elicitation",
        },
      ],
      isError: true,
    };
  ...
  if (verdict === null) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "OmniCraft tool call requires approval but the policy server was unreachable",
        },
      ],
      isError: true,
    };
  ...
  if (mcpInputRequired(retry.json) !== null) {
    ...
    return {
      content: [
        {
          type: "text",
          text: "OmniCraft tool call still requires approval after one round — not retrying",
        },
      ],
      isError: true,
    };
 ...
 * (omnicraft/inner/pi_executor.py): Pi (``@earendil-works/pi-coding-agent``)
...
async function postEvent(config, body) {
  ...
  try {
    ...
    // Keep Pi responsive even if OmniCraft is temporarily unavailable.
  }
}
...
function setOmniCraftStatus(config, ctx, state) {
  if (!ctx || !ctx.ui || !config) return;
  const urlLabel = config.conversationUrl
    ? `OmniCraft: ${config.conversationUrl}`
    : "OmniCraft";
  const label = state ? `${urlLabel} · ${state}` : urlLabel;
  try {
    ctx.ui.setTitle(`OmniCraft: ${config.sessionId}`);
    ctx.ui.setStatus("omnicraft", label);
    ctx.ui.setStatus("omnicraft_state", undefined);
  } catch (_err) {}
}
 ...
 * must run here (the OmniCraft server's AP-side compaction would only
    ...
    await postEvent(config, {
      ...
      data: {
        ...
        item_data: {
          ...
          message:
            "OmniCraft: /compact is unavailable for this Pi session. The " +
...
function startInboxPoller(pi, config, handleInterrupt, handleCompact) {
  if (!config || !config.inboxDir || pi.__omnicraftInboxPoller) return;
  ...
  pi.__omnicraftInboxPoller = setInterval(() => {
    let files = [];
    try {
      files = fs
          ...
          postEvent(config, {
            ...
            data: {
              ...
              item_data: {
                ...
                message:
                  `OmniCraft: a queued follow-up message (id ${droppedId}) could ` +
...
module.exports = function (pi) {
  ...
  // Names of the OmniCraft tools registered via pi.registerTool below. Bridged
  ...
  if (config && Array.isArray(config.tools)) {
    for (const tool of config.tools) {
      ...
      // OmniCraft schema is usable as-is. execute() round-trips the call to the
      // OmniCraft server's MCP proxy and returns the result to Pi.
      if (typeof pi.registerTool === "function") {
        pi.registerTool({
          ...
          async execute(_toolCallId, params) {
            return callOmniCraftTool(config, name, params || {});
          },
        });
      }
    }
  ...
  pi.registerCommand("omnicraft", {
    description: "Show the OmniCraft conversation URL",
    async handler(_args, ctx) {
      setOmniCraftStatus(config, ctx, "linked");
      if (ctx && ctx.ui && config && config.conversationUrl) {
        ctx.ui.notify(`OmniCraft: ${config.conversationUrl}`, "info");
      }
    },
  });
  ...
  pi.on("agent_start", async (_event, ctx) => {
    ...
    agentRunning = true;
    setOmniCraftStatus(config, ctx, "running");
  ...
  pi.on("agent_end", async (event, ctx) => {
    ...
    clearPendingInterrupt();
    agentRunning = false;
    setOmniCraftStatus(config, ctx, "idle");
    activeResponseId = null;
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    // Bridged OmniCraft tools (registered via pi.registerTool above) are
    ...
    // Evaluate TOOL_CALL policy via the OmniCraft server's session-level HTTP
    ...
    // of whether an OmniCraft turn is currently in flight.
    ...
    if (verdict && verdict.block) {
      return {
        block: true,
        reason: verdict.reason || "blocked by OmniCraft policy",
      };
    }
  });
  ...
  pi.on("message_end", async (event, ctx) => {
    rememberContext(ctx);
    replayPendingInterrupt(ctx);
    setOmniCraftStatus(config, ctx, undefined);
