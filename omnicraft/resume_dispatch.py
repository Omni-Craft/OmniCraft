"""Top-level ``omnicraft resume`` dispatch.
...
- ``omnicraft resume <conv_id>`` — fetch the conversation, read its
  ``omnicraft.wrapper`` label, and dispatch by runtime.
- ``omnicraft resume`` (no id) — open the cross-agent picker (see
  :func:`omnicraft.repl._resume_picker.pick_conversation_cross_agent_from_sdk`)
...
``omnicraft run --resume`` invocation — the agentless ``run
...
def run_resume(
    *,
    ...
    leak state and collide with any other in-flight ``omnicraft``
    process the user has running.

    :param target: Optional conversation id, e.g. ``"conv_abc123"``.
        ``None`` selects the picker form.
    :param server: Optional remote OmniCraft server URL. Required in the
    ...
    if target is None:
        if server is None:
            raise click.UsageError(
                "`omnicraft resume` (no id) requires `--server <url>`. "
                "Pass a conversation id (`omnicraft resume conv_...`) "
...
def _pick_conversation_for_resume(
    *,
    ...
    from omnicraft.repl._resume_picker import pick_conversation_cross_agent_from_sdk
    ...
    async def _drive() -> str | None:
        ...
        :returns: Selected conversation id, ``None`` on cancel /
            empty list.
        """
        # Deferred import: ``omnicraft_client`` carries the full SDK
        # surface and is only needed for the picker, not for every
        # ``omnicraft`` invocation.
...
def _dispatch_by_runtime(
    *,
    ...
    the existing ``omnicraft run --resume`` invocation — the
    ...
    :param server: Optional remote OmniCraft server URL. ``None`` when
    ...
    if server is not None:
        ...
        raise click.ClickException(
            ...
            f"`omnicraft run --resume {target} <agent.yaml> --server "
    ...
    raise click.ClickException(
        f"Conversation {target!r} is not a terminal-native session "
        f"(wrapper={wrapper!r}). To resume it, run "
        f"`omnicraft run --resume {target} <agent.yaml>`. "
...
def _dispatch_wrapper(
    *,
    ...
    :param wrapper: Value from ``labels.omnicraft.wrapper``.
    :param server: OmniCraft server base URL without trailing slash, or
...
def _read_wrapper_label_local(*, conv_id: str) -> str | None:
    ...
    :param conv_id: Local OmniCraft conversation id, e.g. ``"conv_abc123"``.
    :returns: Value of ``labels.omnicraft.wrapper``, or ``None`` when
        no wrapper label is present.
    :raises click.ClickException: If the conversation id is not found
        in the local persistent store.
    """
    from omnicraft.chat import _omnicraft_persistent_dir
    from omnicraft.stores.conversation_store.sqlalchemy_store import (
        SqlAlchemyConversationStore,
    )

    db_path = _omnicraft_persistent_dir() / "chat.db"
    ...
    if conversation is None:
        raise click.ClickException(
            ...
            "Pass --server if the conversation lives on a remote OmniCraft server.",
        )
...
def _read_wrapper_label_remote(
    *,
    ...
    :param server: Remote OmniCraft server URL.
    :param conv_id: OmniCraft conversation id.
    :returns: Wrapper label string, or ``None`` when no
        ``omnicraft.wrapper`` label is present on the row.
