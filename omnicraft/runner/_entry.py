...
- ``RUNNER_SERVER_URL``: OmniCraft server base URL for outbound calls
...
from omnicraft.runner.transports.ws_tunnel.serve import RUNNER_TUNNEL_REJECTION_PREFIX
from omnicraft.version import VERSION

if TYPE_CHECKING:
    from omnicraft.runner.app import ResolvedSpec
    from omnicraft.runner.transports.ws_tunnel.serve import _ASGIApp

_RUNNER_SERVER_URL_ENV_VAR = "RUNNER_SERVER_URL"
_RUNNER_PREWARM_SPEC_PATH_ENV_VAR = "RUNNER_PREWARM_SPEC_PATH"
# The runner advertises the omnicraft version it is actually running (shared
...
def _server_url_from_env() -> str:
    """Return the required OmniCraft server URL from the runner environment.
...
def _runner_config_path() -> Path:
    """Return the global OmniCraft config path visible to the runner.

    Respects :envvar:`OMNICRAFT_CONFIG_HOME` for test isolation and
    subprocess consistency with the CLI/onboarding layer.

    :returns: Config path, e.g. ``Path("~/.omnicraft/config.yaml")``.
    """
    config_home = os.environ.get(_RUNNER_CONFIG_HOME_ENV_VAR)
    if config_home:
        return Path(config_home).expanduser() / "config.yaml"
    return Path.home() / ".omnicraft" / "config.yaml"
...
class _RunnerDatabricksAuth(httpx.Auth):
    ...
    Used by the runner's HTTP client for callbacks to the OmniCraft server
    ...
    def __init__(
        ...
        :param server_url: OmniCraft server URL used to look up the ``?o=``
...
def _make_auth_token_factory(
    ...
    Resolution order:
      1. Stored OIDC token from ``~/.omnicraft/auth_tokens.json``
         (populated by ``omnicraft login``), keyed by ``server_url``.
      2. Databricks OAuth token (refreshed via the SDK) — host-keyed
         when a Databricks Apps pointer record is stored for
         ``server_url`` (``omnicraft login <apps-url>``), ambient
         otherwise.
    ...
    :param server_url: Server URL to look up the stored OIDC token
        ...
        non-runner callers (e.g. ``omnicraft host``) must pass
    ...
    - :class:`_RunnerDatabricksAuth` for the httpx client
      (refreshed on each HTTP callback to the OmniCraft server).
    - ``omnicraft/host/connect.py`` for the host tunnel's WS upgrade
      headers.
    ...
    def _sdk_token() -> str | None:
        ...
        if not sdk_auth_resolved:
            ...
            # ``omnicraft login <apps-url>``) names the exact workspace
    ...
    def _factory() -> str | None:
        ...
        Checks the stored OIDC token first (from ``omnicraft login``),
...
class _ManagedMintTokenFactory:
    ...
    def __init__(self, mint_url: str, server_url: str, binding_token: str) -> None:
        ...
        :param server_url: OmniCraft server base URL.
...
def _mint_managed_owner_token(
    ...
    :param binding_token: The runner's tunnel binding token, sent as the
        ``X-OmniCraft-Runner-Tunnel-Token`` header to authenticate the mint.
...
def _runner_parent_pid_from_env() -> int | None:
    ...
    """
    from omnicraft.runner.identity import RUNNER_PARENT_PID_ENV_VAR
...
def _runner_workspace_from_env() -> Path | None:
    ...
    """
    from omnicraft.runner.identity import RUNNER_WORKSPACE_ENV_VAR
...
def _runner_isolate_session_from_env() -> bool:
    """Return ``True`` when ``OMNICRAFT_RUNNER_ISOLATE_SESSION`` is ``"1"``.

    See :data:`RUNNER_ISOLATE_SESSION_ENV_VAR` for the contract.
    """
    from omnicraft.runner.identity import RUNNER_ISOLATE_SESSION_ENV_VAR
...
async def _resolve_agent_spec_from_server(
    ...
    """
    Fetch, cache, and parse one agent spec bundle from the OmniCraft server.
...
def create_app(
    ...
    # Stamp the OmniCraft session marker into the runner's environment so
    # every process this runner spawns can detect it is running inside an
    # OmniCraft agent session, the way Claude Code sets CLAUDE_CODE and
    ...
    os.environ[OMNICRAFT_SESSION_ENV_VAR] = OMNICRAFT_SESSION_ENV_VALUE

    # Keep the harness manager on its default /tmp/omnicraft root.
    ...
    server_client = httpx.AsyncClient(
        ...
        headers={"Origin": OMNICRAFT_INTERNAL_WS_ORIGIN, **databricks_request_headers(server_url)},
    ...
    async def spec_resolver(agent_id: str, session_id: str | None = None) -> ResolvedSpec | None:
        """
        Fetch agent spec from the OmniCraft server, extract under the
    ...
    from omnicraft.inner.terminal import reap_orphaned_terminals
    ...
    async def _start_pm() -> None:
        ...
        if prewarm_path and mcp_manager is not None:
            try:
                from omnicraft.spec import load as _load_spec
...
async def _run_tunnel_from_env() -> None:
    ...
    :returns: None.
    """
    from omnicraft.runner.identity import get_stable_runner_id
    from omnicraft.runner.transports.ws_tunnel.serve import serve_tunnel
...
def main() -> None:
    ...
    :returns: None.
    """
    log_level = os.environ.get("OMNICRAFT_LOG_LEVEL", "INFO").upper()
    logging.basicConfig(
