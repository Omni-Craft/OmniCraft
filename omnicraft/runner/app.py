...
if TYPE_CHECKING:
    ...
    from omnicraft.codex_native_app_server import CodexAppServerClient
    from omnicraft.terminals.registry import TerminalListEntry

import httpx
from fastapi import FastAPI, HTTPException, Query, Request, WebSocket
from fastapi.responses import JSONResponse, Response, StreamingResponse

from omnicraft._platform import IS_WINDOWS
...
from omnicraft.harness_plugins import load_object, model_env_keys, spawn_env_builders
from omnicraft.llms.summarize import (
...
from omnicraft.runner.proxy_mcp_manager import ProxyMcpManager
...
from omnicraft.runtime.harnesses.process_manager import HarnessProcessManager, NoLiveHarnessError
from omnicraft.spec.skill_sources import SkillSourceContext, resolve_harness_skills
from omnicraft.spec.types import AgentSpec, LocalToolInfo, SkillSpec
from omnicraft.terminals.control_bridge import bridge_tmux_control_to_websocket
...
# ASK gate: policy evaluation (``_evaluate_policy_via_omnicraft``) and sub-agent
...
# Terminal resource hosting the framework's own TUI (the OmniCraft REPL,
# ``omnicraft attach``) for runner-hosted SDK sessions — the SDK mirror of
...
# delivery signal for the last child of a fan-out, and OmniCraft routinely
...
def _publish_tmux_target_for_bridge(
    *,
    ...
    from omnicraft.claude_native_bridge import bridge_dir_for_bridge_id, write_tmux_target
...
class _CodexNativeLaunchConfig:
    ...
    :param policy_server_url: OmniCraft server URL for the Codex policy hook and
    ...
    :param fork_source_id: SOURCE conversation id stamped on a forked
        clone (``omnicraft.fork.source_id``), used to locate the
    ...
    :param fork_source_external_id: SOURCE Codex thread id stamped on a
        forked clone (``omnicraft.fork.source_external_session_id``),
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a
        native target (``omnicraft.fork.carry_history``); when no source
        rollout exists to clone (an SDK or cross-family source) the runner
        builds the clone's rollout from the copied OmniCraft items instead (see
        ``_ensure_local_codex_resume_rollout``).
    :param bypass_sandbox: ``True`` when the session opted into Codex's
        DANGEROUS full-bypass stance (``omnicraft.codex_native.bypass_sandbox``
...
class _PiNativeLaunchConfig:
    ...
    :param server_url: OmniCraft server URL for the extension/forwarder.
    ...
    :param fork_source_external_id: SOURCE Pi session id stamped on a forked
        clone (``omnicraft.fork.source_external_session_id``); consulted only
        when the clone has no native session of its own yet.
    :param fork_carry_history: ``True`` on a forked clone bound to a native
        target (``omnicraft.fork.carry_history``); when no source session
        exists to clone, the clone's session is rebuilt from its OWN copied
        OmniCraft items (see :func:`_auto_create_pi_terminal`). Also consumed by
...
def _codex_session_workspace(session_workspace: str | None) -> Path:
    ...
    runner's ``OMNICRAFT_RUNNER_WORKSPACE``.
    ...
    :raises RuntimeError: If no workspace is available (neither the
        session snapshot nor ``OMNICRAFT_RUNNER_WORKSPACE``).
    """
    raw = session_workspace or _required_runner_env("OMNICRAFT_RUNNER_WORKSPACE")
...
async def _pi_native_launch_config(
    *,
    ...
    :param server_client: Runner OmniCraft server client.
...
class _OpenCodeNativeLaunchConfig:
    ...
    :param policy_server_url: OmniCraft server URL for the forwarder.
    ...
    :param fork_carry_history: ``True`` on a forked clone whose prior
        ...
        (``omnicraft.fork.carry_history``); opencode has no native session to
        clone, so the runner rehydrates from the copied OmniCraft transcript.
    """
...
async def _opencode_native_launch_config(
    *,
    ...
    from omnicraft.stores.conversation_store import FORK_CARRY_HISTORY_LABEL_KEY
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    :param server_client: Runner OmniCraft server HTTP client.
    :param ensure_comment_relay: Callback that starts the OmniCraft builtin-tool
        relay for this session's bridge dir (the nested
        ``_ensure_comment_relay_started``). ``None`` skips wiring the OmniCraft
    ...
    from omnicraft.opencode_native_forwarder import OpenCodeNativeForwarder
    ...
    # OmniCraft builtin-tool relay (wired below) can start. Safe to call before
    ...
    from omnicraft.opencode_native_bridge import xdg_config_home_for_bridge_dir
    ...
    # Build opencode's ``mcp`` block: the OmniCraft builtin-tool relay (so the
    # model can call sys_*/load_skill/web_fetch — the real "connects to OmniCraft
    ...
    # through OmniCraft's policy engine via the forwarder's permission gate —
    ...
    if server_client is not None and ensure_comment_relay is not None:
        mcp_block.update(build_opencode_omnicraft_mcp_server(bridge_dir))
    if mcp_block:
        config.setdefault("$schema", "https://opencode.ai/config.json")
        config["mcp"] = mcp_block
        config["permission"] = "ask"

    # Load the OmniCraft policy-bridge plugin so opencode's lifecycle hooks reach
    ...
    # PostToolUse hooks); coordinates come from the OMNICRAFT_* env stamped on
    ...
    if server_client is not None and runner_server_url:
        ...
        policy_env["OMNICRAFT_POLICY_URL"] = runner_server_url
        policy_env["OMNICRAFT_SESSION_ID"] = session_id
        ...
        from omnicraft.runner._entry import _make_auth_token_factory

        _policy_factory = _make_auth_token_factory()
        _policy_token = _policy_factory() if _policy_factory is not None else None
        if _policy_token:
            from omnicraft.cli_auth import databricks_request_headers
            ...
            # to the omnicraft server out-of-process, so without the selectors it
            # could land on a different server instance than the runner's.
            policy_env["OMNICRAFT_POLICY_HEADERS"] = json.dumps(
    ...
    # Start the OmniCraft builtin-tool relay BEFORE opencode boots, so
    ...
    # relay POSTs each call back through the OmniCraft server (policy enforced).
    if server_client is not None and ensure_comment_relay is not None:
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) — we'll rehydrate from the OmniCraft transcript
                    # below instead of silently starting empty.
                    resume_lost_history = True
            if opencode_session_id is None:
                created = await client.create_session({"title": f"omnicraft:{session_id}"})
                ...
                # seed the copied OmniCraft transcript as a noReply preamble.
                if resume_lost_history or launch_config.fork_carry_history:
                    await _rehydrate_opencode_session_from_transcript(
                        ...
                        omnicraft_session_id=session_id,
                        server_client=server_client,
                        model_override=model_override,
                    )
    ...
    # lifetime; ``server_client`` is the runner's OmniCraft client. The
...
def _build_opencode_policy_evaluator(
    *,
    ...
    :param server_client: Runner's OmniCraft server HTTP client.
    :param conversation_id: Owning OmniCraft session id, e.g. ``"conv_abc"``.
    :returns: An async evaluator returning a verdict mapping, or a deny
        verdict on failure.
    """
    from omnicraft.opencode_native_permissions import OPENCODE_NATIVE_HARNESS
...
def _opencode_native_model_from_spec(agent_spec: Any | None) -> str | None:
    ...
    if agent_spec is None:
        return None
    try:
        from omnicraft.runtime.workflow import _resolve_spec_model
...
def _resolve_opencode_compact_model(
    ...
    opencode's ``/summarize`` requires an explicit model, but OmniCraft
...
def _render_opencode_transcript_text(items: list[Any]) -> str:
    """
    Render committed OmniCraft message items into a plain-text transcript.
...
async def _rehydrate_opencode_session_from_transcript(
    *,
    ...
    persisted opencode session is gone) inject the OmniCraft transcript as a
    ...
    try:
        resp = await server_client.get(
            f"/v1/sessions/{urllib.parse.quote(omnicraft_session_id, safe='')}/items",
    ...
    try:
        ...
        _logger.warning(
            "opencode resume: rehydration seed failed for %s", omnicraft_session_id, exc_info=True
        )
        return False
    return True
...
def _pi_args_have_session_control(args: list[str]) -> bool:
    ...
    :returns: ``True`` when OmniCraft should not add resume/session flags.
    """
    session_flags = {
        "--session-dir",
        "--session",
...
def _pi_args_have_provider(args: list[str]) -> bool:
    ...
    OmniCraft must not inject the ``omnicraft setup`` provider on top — the
    explicit choice wins.

    :param args: User pass-through Pi CLI args.
    :returns: ``True`` when OmniCraft should not add provider/model args.
    """
...
async def _resolve_pi_resume_session(
    *,
    ...
    1. **Cold resume** — the session already carries a captured Pi
       ...
       file from committed OmniCraft items so ``pi --session <id>`` opens with
    ...
    2. **Fork rebuild** — a forked clone bound to a pi-native target with NO
       ...
       session id, build its file from the clone's OWN copied OmniCraft items,
       and patch the server so OmniCraft reflects the clone's session id and a
    ...
    from omnicraft.pi_native_resume import ensure_local_pi_resume_session, mint_pi_session_id
    ...
    model = ""
    try:
        from omnicraft.pi_native_credentials import resolve_pi_native_provider
    ...
    # yet. Build the clone's session from its OWN copied OmniCraft items under a
    ...
    if launch_config.fork_carry_history:
        ...
        if built is not None:
            # Record the minted id so OmniCraft reflects the clone's own Pi
...
async def _auto_create_pi_terminal(
    ...
    *,
    ...
    from omnicraft.pi_native_bridge import extension_path as pi_extension_path
    from omnicraft.runner._entry import _make_auth_token_factory
    ...
    # Build the OmniCraft tool surface (sys_* tools) the Pi extension registers
    ...
    # can call OmniCraft tools with centralized server-side policy enforcement
    ...
    try:
        from omnicraft.runner.tool_dispatch import build_native_relay_tool_schemas
    ...
    # Rebuild the local Pi session JSONL from committed OmniCraft items so a
    ...
    pi_env = {
        ...
        "OMNICRAFT_PI_NATIVE_BRIDGE_DIR": str(bridge_dir),
    }
    # Route the runner-owned Pi process through the provider configured by
    # ``omnicraft setup`` (Databricks gateway / API key), so a separate
    ...
    if not _pi_args_have_provider(launch_config.terminal_launch_args or []):
        from omnicraft.pi_native_credentials import (
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from omnicraft.cursor_native import resolve_cursor_executable
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from omnicraft.cursor_native import is_valid_cursor_chat_id
    ...
    from omnicraft.cursor_native_forwarder import clear_cursor_bridge_state, preseed_resume_state
    from omnicraft.cursor_native_status import clear_cursor_status_state
    from omnicraft.cursor_native_usage import clear_cursor_usage_state
    ...
    # Render the copied OmniCraft items once and stash them; the executor prepends
    ...
    if launch_config.fork_carry_history and not preseeded and server_client is not None:
        try:
            from omnicraft.claude_native import _fetch_all_session_items_for_claude_resume
    ...
    # passthrough launch args (``omnicraft cursor -- --model X`` or the joined
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from omnicraft.cursor_native_bridge import write_tmux_target
    ...
    # Mirror the Cursor TUI's conversation back into the OmniCraft session so the
    ...
    from omnicraft.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    async def _supervise_cursor_native_bridges() -> None:
        ...
        calls (see :mod:`omnicraft.cursor_native_permissions`) — more reliable
        ...
        :mod:`omnicraft.cursor_native_usage`).
        """
...
async def _auto_create_goose_terminal(
    ...
    *,
    ...
    ``~/.config/goose/config.yaml``), so HOME is inherited and OmniCraft writes no
    ...
    :returns: Created terminal resource view.
    """
    from omnicraft.goose_native import resolve_goose_executable
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from omnicraft.goose_native_bridge import bridge_dir_for_session_id, write_tmux_target
    from omnicraft.goose_native_forwarder import clear_goose_bridge_state
    ...
    # Mirror the Goose TUI's conversation back into the OmniCraft session so the
    ...
    from omnicraft.goose_native_forwarder import supervise_goose_forwarder
    from omnicraft.goose_native_permissions import supervise_goose_approval_mirror
    ...
    async def _supervise_goose_native_bridges() -> None:
        ...
        a web elicitation (see :mod:`omnicraft.goose_native_permissions`).
        """
        await asyncio.gather(
...
async def _auto_create_hermes_terminal(
    ...
    *,
    ...
    ``~/.hermes/config.yaml``), so HOME is inherited and OmniCraft writes no vendor
    ...
    ``cwd`` + ``started_at`` floor (see :mod:`omnicraft.hermes_native_forwarder`).
    ...
    :returns: Created terminal resource view.
    """
    from omnicraft.hermes_native import resolve_hermes_executable
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    from omnicraft.hermes_native_forwarder import clear_hermes_bridge_state
    from omnicraft.hermes_native_status import clear_hermes_status_state
    ...
    # Write a per-session HERMES_HOME with the OmniCraft policy hook so the
    # native TUI evaluates tool calls against OmniCraft policies.
        ...
        if _source_db is not None and _source_db.is_file():
            ...
            if _target_db is not None:
                try:
                    ...
                    # the forwarder only mirrors NEW messages (OmniCraft already
                    # has the cloned ones from the fork item copy).
                    if _clone_max_id > 0:
                        from omnicraft.hermes_native_forwarder import (
                            _ForwardState,
                            _write_state,
                        )
    ...
    # Mirror the Hermes TUI's conversation back into the OmniCraft session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from omnicraft.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from omnicraft.hermes_native_bridge import read_hermes_home
    from omnicraft.hermes_native_forwarder import supervise_hermes_forwarder
    from omnicraft.hermes_native_permissions import supervise_hermes_approval_mirror
    ...
    async def _supervise_hermes_native_bridges() -> None:
        ...
        elicitation (see :mod:`omnicraft.hermes_native_permissions`).
        """
...
async def _auto_create_kiro_terminal(
    ...
    *,
    ...
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from omnicraft.kiro_native import build_kiro_launch
    ...
    bridge_dir = prepare_bridge_dir(session_id)
    # Declare the OmniCraft MCP server in the workspace-scoped kiro config so
    # kiro-cli can call OmniCraft tools. Only when the tool relay will actually
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from omnicraft.kiro_native_bridge import write_tmux_target
    ...
    # Start the OmniCraft builtin-tool relay (writes tool_relay.json into the kiro
    # bridge dir) so the serve-mcp server declared in the workspace mcp.json can
    # route OmniCraft tool calls back through the session's policy/elicitation
    ...
    from omnicraft.kiro_native_permissions import supervise_kiro_permission_mirror
    from omnicraft.kiro_native_session_forwarder import supervise_kiro_session_forwarder
...
async def _persist_qwen_external_session_id(
    ...
    """Record the qwen session id on the OmniCraft session as ``external_session_id``.
    ...
    ``fork_conversation`` stamps as ``omnicraft.fork.source_external_session_id``
    ...
    :param server_client: Runner OmniCraft server client (``None`` skips the write).
    :param session_id: OmniCraft session/conversation id.
...
async def _build_qwen_fork_recording(
    ...
    *,
    ...
    """Synthesize a qwen chat recording for a forked clone from its OmniCraft items.

    A forked clone has its OWN copied OmniCraft items but no qwen recording yet
    ...
    :param server_client: Runner OmniCraft server client.
    :param session_id: The forked clone's OmniCraft conversation id.
    ...
    """
    from omnicraft.pi_native_resume import fetch_all_session_items_for_pi_resume
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    env vars or ``~/.qwen`` from ``/auth``), so HOME is inherited and OmniCraft
    ...
    """
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from omnicraft.qwen_native import resolve_qwen_executable
    ...
    from omnicraft.qwen_native_forwarder import clear_qwen_bridge_state
    ...
    # is persisted on the OmniCraft session and read back from the snapshot
    # (``launch_config.external_session_id``), which also lets a fork carry history
    # (``omnicraft.fork.source_external_session_id``). qwen is cleaner than
    ...
    # clone's copied OmniCraft items and force ``--resume``. Gated on a NULL
    ...
    # Expose OmniCraft's builtin tools (sys_*, load_skill, web_fetch, …) to qwen
    ...
    if mcp_enabled:
        try:
            ...
            _logger.warning(
                ...
                "OmniCraft MCP wiring for session %s.",
                session_id,
                exc_info=True,
            )
        else:
    ...
    # the default in-terminal prompt (the embedded pane shows it) — OmniCraft-side
    ...
    # Mirror the qwen TUI's conversation back into the OmniCraft session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from omnicraft.runner._entry import _make_auth_token_factory, _RunnerDatabricksAuth
    ...
    from omnicraft.qwen_native_bridge import qwen_session_recording_path
    from omnicraft.qwen_native_forwarder import (
        supervise_qwen_compaction_mirror,
        supervise_qwen_forwarder,
    )
    from omnicraft.qwen_native_permissions import supervise_qwen_approval_mirror
    ...
    async def _supervise_qwen_native_bridges() -> None:
        ...
        elicitations (see :mod:`omnicraft.qwen_native_permissions`); the compaction
        ...
        :func:`omnicraft.qwen_native_forwarder.supervise_qwen_compaction_mirror`).
        """
        await asyncio.gather(
            supervise_qwen_forwarder(
                base_url=server_url,
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    :func:`omnicraft.kimi_native_credentials.build_kimi_session_home`) that
    mirrors the user's global ``kimi login`` (symlinked ``oauth`` / providers)
    and adds the OmniCraft tool-policy hooks — a ``PreToolUse`` deny-gate and a
    ``PermissionRequest`` read-only surface dispatched to
    :mod:`omnicraft.kimi_native_hook`. The hook subprocess reads its routing
    from ``hook_config.json`` in the bridge dir.

    A background forwarder (:func:`omnicraft.kimi_native_forwarder.
    supervise_kimi_forwarder`) tails kimi's per-session ``wire.jsonl`` transcript
    and mirrors each user prompt + assistant reply into the OmniCraft chat, so the
    ...
    :param resource_registry: Session resource registry for launching the
        terminal.
    :param publish_event: Runner session event publisher.
    :param server_client: Runner OmniCraft server client (used only for the
    ...
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from omnicraft.kimi_native import resolve_kimi_executable
    from omnicraft.kimi_native_bridge import (
        bridge_dir_for_session_id,
        write_hook_config,
        write_tmux_target,
    )
    from omnicraft.kimi_native_credentials import build_kimi_session_home
    from omnicraft.kimi_native_forwarder import clear_kimi_bridge_state, supervise_kimi_forwarder
    from omnicraft.runner._entry import _make_auth_token_factory
    ...
    # launch args (``omnicraft kimi -- <args>``) are persisted on the session
    # snapshot and threaded here.
    kimi_args = list(launch_config.terminal_launch_args or [])

    # Wire the OmniCraft tool-policy hooks: kimi reads a single
    ...
    # ``omnicraft.kimi_native_hook``. The hook subprocess reads the server URL +
    ...
    # Mirror the kimi TUI transcript into the OmniCraft chat: tail the per-session
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        surface the new terminal on the live stream (the OmniCraft relay
    ...
    :param server_client: Runner's OmniCraft server HTTP client. Used to read
    ...
    from omnicraft.codex_native_app_server import (
    ...
    # Route across all offerings: a configured provider (omnicraft setup),
    ...
    from omnicraft.inner.codex_executor import _find_codex_cli
    ...
    if (
        ...
        from omnicraft.codex_native import _clone_codex_rollout, _mint_codex_thread_id
        ...
        if cloned_rollout is not None:
            ...
            # Record the assigned thread id now so OmniCraft reflects the clone's
            ...
            if server_client is not None:
                try:
                    ...
                    # unset on the OmniCraft session and a future relaunch of this
        ...
        # rollout from its OWN copied OmniCraft items under a thread id we mint, then flip
        ...
        from omnicraft.codex_native import (
            _ensure_local_codex_resume_rollout,
    ...
    if launch_config.external_session_id is not None and original_external_session_id is not None:
        from omnicraft.codex_native import _ensure_local_codex_resume_rollout
    ...
    # OmniCraft coordinates for the codex-native policy hook. The hook runs as a
    ...
    event_client = CodexAppServerClient(
        ws_url=codex_ws_url,
        client_name="omnicraft-codex-native-auto",
    )
    if launch_config.external_session_id is None:
        ...
        from omnicraft.codex_native_bridge import CodexNativeBridgeState, write_bridge_state
        ...
        terminal_view = await resource_registry.launch_auxiliary_terminal(
            ...
            spec=TerminalEnvSpec(
                ...
                args=build_codex_remote_args(
                    ...
                    # OmniCraft provider instead of falling back to the
                ...
                # Match the local ``omnicraft codex`` terminal scrollback.
                scrollback=100_000,
...
async def _codex_discover_thread_and_forward(
    *,
    ...
    :param session_id: OmniCraft session/conversation id, e.g. ``"conv_abc123"``.
    ...
    from omnicraft.codex_native_forwarder import (
        supervise_forwarder,
        wait_for_thread_started,
    )
    from omnicraft.runner._entry import (
    ...
    try:
        ...
        # Mirror the discovered Codex thread id onto the OmniCraft session as its
        ...
        # ``omnicraft.fork.source_external_session_id`` from
        ...
        # would resume fresh. Best-effort: a transient OmniCraft failure here still
...
async def _codex_forward_known_thread(
    *,
    ...
    :returns: None. Runs until cancelled or the app-server connection
        closes.
    """
    from omnicraft.codex_native_forwarder import supervise_forwarder
    from omnicraft.runner._entry import (
        _make_auth_token_factory,
        _RunnerDatabricksAuth,
    )
...
async def _run_antigravity_reader(
    *,
    ...
    :func:`omnicraft.antigravity_native_reader.run_reader_with_bridge` (used by both
    this runner path and the CLI ``omnicraft antigravity`` attach fallback); it
...
async def _auto_create_antigravity_terminal(
    ...
    *,
    ...
    :func:`omnicraft.antigravity_native._launch_antigravity_terminal`, which
    ...
    *attended* (``headless=False``). Auto-bypass comes only from the user's
    ...
    :param ensure_comment_relay: The runner's relay starter
        (``_ensure_comment_relay_started``). When provided, the OmniCraft MCP
    ...
    from omnicraft.antigravity_native_launch import build_agy_launch
    ...
    # Wire the OmniCraft MCP relay so the wrapped agy gets the sys_* tools
    # (spawn sub-agent sessions, drive OmniCraft terminals, list agents/models,
    ...
    terminal_view = await resource_registry.launch_required_terminal(
        ...
        spec=TerminalEnvSpec(
            ...
            args=list(argv[1:]),
            env=env_overrides,
            # Match the local ``omnicraft antigravity`` terminal scrollback.
            scrollback=100_000,
            ...
            # ``omnicraft antigravity`` path keeps start-on-attach: there a human
            # TTY is the driver.)
            tmux_start_on_attach=False,
        ),
    )
    ...
    # elicitations via the Task 9 hook. The reader owns its own OmniCraft client
...
def _mint_runner_agy_conversation_id() -> str:
    ...
    :func:`omnicraft.antigravity_native._mint_agy_conversation_id`.
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    :func:`omnicraft.antigravity_native_rpc.resolve_cold_start_agy_rpc_port`:
    ...
    The cold-started id is also PATCHed onto the OmniCraft session as
    ...
    *timeout_s*, or ``StartCascade`` erroring) must NOT abort the auto-create:
    ...
    :param server_client: Runner OmniCraft server client used for the
    ...
    from omnicraft.antigravity_native_rpc import (
        AntigravityRpcError,
...
def _terminal_tmux_pane(
    ...
    # instance (see omnicraft.inner.terminal). Guard defensively so a registry
...
async def _session_payload_for_host_spawn_check(
    ...
    :param server_client: The runner's OmniCraft server HTTP client, or
...
async def _codex_session_needs_runner_terminal(
    ...
    :param server_client: The runner's OmniCraft server HTTP client, or ``None`` in
...
def _codex_native_model_from_spec(agent_spec: AgentSpec | ResolvedSpec | None) -> str | None:
    ...
    Lê o campo canônico ``spec.executor.model`` — o mesmo que o harness
    em processo consome —, caindo em ``executor.config["model"]`` para specs
    de bundle que pinam o modelo dentro do bloco de config do harness. O
    carregador de agente em arquivo único nunca preenche o segundo, então ler
    só ele trocava calado o ``model:`` declarado pelo padrão do provedor.
...
def _pi_native_model_from_spec(agent_spec: AgentSpec | ResolvedSpec | None) -> str | None:
    ...
    :func:`omnicraft.pi_native_credentials.resolve_pi_native_provider`). The
...
def _cursor_fork_history_preamble(items: list[dict[str, Any]]) -> str:
    ...
    :func:`omnicraft.cursor_native_bridge.wrap_fork_preamble`.

    :param items: Committed OmniCraft items (``GET /v1/sessions/{id}/items``),
        chronological.
...
def _build_claude_native_base_args(
    *,
    ...
    bridge / MCP / hook / OmniCraft wiring. The order is: ``--resume`` for a
    ...
    :param resume_external_session_id: Claude-native session id to
        ...
        :func:`omnicraft.claude_native._clone_claude_transcript`), so
...
def _publish_terminal_pending(
    ...
    fails. The OmniCraft relay caches the latest value and republishes it, and
...
def _publish_native_terminal_start_error(
    ...
    outside a transcript turn; OmniCraft writes and publishes the turn-scoped
...
def _codex_ensure_response_with_policy_notice(
    ...
    ``policy_hook_disabled_reason`` exactly once so OmniCraft can post a single
...
def _ensure_orchestrator_skills_in_bundle(
    ...
    """
    Link the ``build-omnicraft`` skill into a bundle's ``skills/`` dir.

    Called before native bridge launches so ``--plugin-dir`` (claude) or
    ``CODEX_HOME/skills/`` (codex) picks up the skill. Injects
    unconditionally for every agent — every ``omnicraft claude`` /
    ``omnicraft codex`` user should be able to author new agents. The
    ...
    :param bundle_dir: Materialized agent-bundle root, e.g.
        ``/tmp/omnicraft-ap-chat-xyz/bundle``.
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        ...
        republishes it to the web UI) so the Terminal toggle enables
        without a refresh.
    :param server_client: OmniCraft server client used to fetch the session
    ...
    from omnicraft.claude_native_forwarder import reset_transcript_forward_state
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec

    workspace = os.environ.get("OMNICRAFT_RUNNER_WORKSPACE", str(Path.cwd()))
    ...
    # The OmniCraft server URL + auth are needed in two places below: the
    ...
    from omnicraft.claude_launcher import resolve_claude_launch
    from omnicraft.claude_native import (
    ...
    # build the clone's native transcript from the copied OmniCraft items
    ...
    if server_client is not None and session_external_id is not None:
        from omnicraft.claude_native import _ensure_local_claude_resume_transcript
        ...
        from omnicraft.claude_native import _clone_claude_transcript
        ...
        if _cloned is not None:
            ...
            # Record the assigned id now so OmniCraft reflects the clone's own
        ...
        # native transcript from its OWN copied OmniCraft items under a uuid we
        ...
        if _built is not None:
            resume_external_session_id = our_uuid
            # Record the assigned id so OmniCraft reflects the clone's own Claude
    ...
    # (omnicraft setup), a Databricks ucode profile from provider config, or
    ...
    try:
        ...
        _logger.warning(
            ...
            "`omnicraft setup --no-internal-beta` "
    ...
    # managed-host path. Identity by default. See omnicraft.claude_launcher.
    ...
    from omnicraft.entities.session_resources import session_resource_view_to_dict
    ...
    # back to the OmniCraft server. Normally the CLI client runs this,
    ...
    # offset 0 already holds every item OmniCraft has. Starting the forwarder at
    ...
    # ``claude_native.py``.
    from omnicraft.claude_native_forwarder import supervise_forwarder
...
async def _auto_create_repl_terminal(
    ...
    *,
    ...
    """
    Auto-create an OmniCraft REPL terminal for a runner-hosted SDK session.
    ...
    terminal hosts the framework's own TUI (``omnicraft attach
    ...
    like the claude-/codex-native terminals — with the OmniCraft REPL as
    the TUI.
    ...
    attach the session is fully live (``omnicraft attach`` fails loud on a
    non-live session) with the REPL sized to the real attached terminal.

    Auth parity with the native terminals: the spawned ``omnicraft
    attach`` resolves credentials for ``--server`` the same way a
    user-launched CLI does (``OMNICRAFT_REMOTE_AUTH_TOKEN`` env → stored
    OIDC token from ``omnicraft login`` → ``~/.databrickscfg``), which
    ...
    :param publish_event: The runner's per-session SSE emitter,
        ...
        without a refresh.
    :param server_client: OmniCraft server client used to stamp the
        ``omnicraft.ui: terminal`` presentation label that makes the web
        UI show the Chat/Terminal toggle.
    :returns: The launched terminal's :class:`SessionResourceView`.
    """
    from omnicraft._wrapper_labels import UI_MODE_LABEL_KEY, UI_MODE_TERMINAL_VALUE
    from omnicraft.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    env_spec = TerminalEnvSpec(
        ...
        # The runner's interpreter is the venv with omnicraft installed;
        # ``python -m omnicraft`` avoids depending on the console script
        # being on the tmux pane's PATH.
        command=sys.executable,
        args=["-m", "omnicraft", "attach", session_id, "--server", server_url],
        scrollback=50000,
    ...
    terminal_view = await resource_registry.launch_auxiliary_terminal(
        ...
        resource_role=OMNICRAFT_REPL_TERMINAL_ROLE,
    )
    ...
    # show a dead pill. The ``omnicraft.wrapper`` label is deliberately
    ...
    _logger.info(
        "Auto-created omnicraft REPL terminal for session %s: terminal_id=%s "
...
async def _delete_native_bridge_dirs(
    *,
    ...
    ``/tmp/omnicraft-<uid>/<harness>-native/<digest>`` (some use ``~/.omnicraft``)
    ...
    :param server_client: OmniCraft server client used to resolve rotated bridge
...
async def _claude_native_bridge_id_for_session(
    *,
    ...
    :param server_client: OmniCraft server client used to fetch the session
        snapshot.
    :param session_id: OmniCraft session/conversation id, e.g.
        ``"conv_abc123"``.
    :returns: Opaque bridge id from
        ``omnicraft.claude_native.bridge_id`` when present, otherwise
        *session_id* for legacy single-session bridges.
    """
    from omnicraft.claude_native_bridge import BRIDGE_ID_LABEL_KEY
...
async def _claude_native_terminal_arrives_via_transfer(
    *,
    ...
    :param server_client: OmniCraft client to resolve the bridge id label;
...
async def _antigravity_native_terminal_arrives_via_transfer(
    *,
    ...
    :func:`omnicraft.antigravity_native_reader._rotate_session_for_cascade`) binds the
...
async def _evaluate_policy_via_omnicraft(
    *,
    server_client: httpx.AsyncClient,
    ...
    """
    Proxy a policy evaluation request from the harness to the OmniCraft server.

    Called by the runner's ``proxy_stream`` when it intercepts a
    ``policy_evaluation.requested`` SSE event from the harness. Posts
    the evaluation request to the OmniCraft server's
    ...
    - ``PHASE_LLM_REQUEST`` / ``PHASE_LLM_RESPONSE`` fail OPEN
      (``POLICY_ACTION_ALLOW``) so a transient OmniCraft outage does not
    ...
    verdict_reason: str | None = (
        f"OmniCraft policy evaluation unavailable; failing closed for {phase}."
        if _fail_closed
        else None
    )
...
def _is_spec_local_native_python_tool(spec: Any, tool_name: str) -> bool:
    ...
    return any(
        ...
        and getattr(info, "language", None) in ("python", "omnicraft-python-callable")
...
# Language constant the omnicraft YAML translator stamps on callable-backed
# tools (omnicraft/spec/omnicraft.py:OMNICRAFT_TOOL_LANGUAGE). Duplicated rather
# than imported to avoid pulling the heavy translator module in for one
# string — same rationale as omnicraft/tools/local_callable.py.
_OMNICRAFT_CALLABLE_LANGUAGE = "omnicraft-python-callable"
...
def _looks_like_file_path(path: str) -> bool:
    ...
    source extension (see :func:`omnicraft.spec.parser._discover_local_tools`).
...
def _spec_with_workdir_paths(spec: Any, workdir: Path | None) -> Any:
    ...
    for info in local_tools:
        ...
        if (
            path
            and getattr(info, "language", None) != _OMNICRAFT_CALLABLE_LANGUAGE
...
async def _resolve_forwarded_message_content(
    ...
    *,
    ...
    Remote OmniCraft servers can forward session messages with raw file IDs
...
def register_subagent_work(
    *,
    ...
    :param wrapper_label: Optional child ``omnicraft.wrapper``
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    OmniCraft returns while the parent's runner tunnel is reconnecting), as
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure — the POST may never have reached OmniCraft.
        return True
...
async def _deliver_subagent_wake_post(
    ...
    *,
    ...
    :param server_client: OmniCraft HTTP client for the runner subprocess.
...
def _subagent_delivery_not_confirmed_response(
    ...
    *,
    ...
    was created as a sub-agent. For known sub-agents, OmniCraft must not receive a
...
def create_runner_app(
    *,
    ...
    :param server_client: httpx.AsyncClient pointed at the AP
        ...
        In-process: pointed at the OmniCraft ASGI app.
    ...
    app = FastAPI(title="omnicraft-runner")
    ...
    if terminal_registry is not None:
        from omnicraft.runtime import _globals as _rt_globals
    ...
    # Same guard for the OmniCraft REPL (``omnicraft attach``) terminal
    ...
    # ``omnicraft.runner.pending_approvals`` so the runner-side
    # policy gate (``omnicraft.runner.tool_dispatch``) can register
    ...
    def _child_preview_from_status(
        ...
        *,
        ...
        history fallback because OmniCraft owns native transcript persistence. The
    ...
    def _publish_session_status(session_id: str, status: str) -> None:
        ...
        own turns lets the OmniCraft server relay it through the normal status
        ...
        if event.last_output:
            ...
            parts.extend(
                [
                    ...
                    "OmniCraft captured a pane snapshot.",
                ]
            )
        return "\n".join(parts)
    ...
    from omnicraft.entities.environment_filesystem import (
        FilesystemEntry,
        ResourceError,
    )
    ...
    async def create_session(request: Request) -> JSONResponse:
        ...
        if spec is not None:
            ...
            if harness_name == "pi-native" and spawn_env is None:
                from omnicraft.pi_native_bridge import build_pi_native_spawn_env
            ...
            if harness_name == "cursor-native" and spawn_env is None:
                from omnicraft.cursor_native_bridge import build_cursor_native_spawn_env

                spawn_env = build_cursor_native_spawn_env(session_id)
            if harness_name == "kiro-native" and spawn_env is None:
                from omnicraft.kiro_native_bridge import build_kiro_native_spawn_env
            ...
            if harness_name == "goose-native" and spawn_env is None:
                from omnicraft.goose_native_bridge import build_goose_native_spawn_env
            ...
            if harness_name == "qwen-native" and spawn_env is None:
                from omnicraft.qwen_native_bridge import build_qwen_native_spawn_env

                spawn_env = build_qwen_native_spawn_env(session_id)
            if harness_name == "kimi-native" and spawn_env is None:
                from omnicraft.kimi_native_bridge import build_kimi_native_spawn_env
        ...
        # created: the OmniCraft relay's ``GET /stream`` can race ahead of this
        ...
        if harness_name == "claude-native":
            ...
            async with _ensure_lock:
                ...
                if not _has_terminal and not _terminal_inbound:
                    ...
                    # Auto-inject orchestrator skills (build-omnicraft)
        ...
        # Auto-bootstrap the OmniCraft REPL terminal for non-native
        # (SDK-harness) top-level sessions: host the framework's own TUI
        # (``omnicraft attach``) in a tmux pane so the web UI can embed it
            ...
            async with _repl_lock:
                ...
                if not _has_repl_terminal:
                    ...
                    try:
                        ...
                        _logger.exception(
                            "Failed to auto-create omnicraft REPL terminal for %s",
                            session_id,
                        )
                    finally:
                        _publish_terminal_pending(_publish_event, session_id, False)
        ...
        # turn with no assistant item, not an unanswered OmniCraft task to replay.
    ...
    async def stream_session(session_id: str) -> StreamingResponse:
        ...
        async def _event_generator() -> AsyncIterator[bytes]:
            ...
            HTTP connection, leaving the OmniCraft relay on a half-open
            ...
            # Immediate ready ack: OmniCraft waits for this frame before
    ...
    async def _codex_native_bridge_state_for_session(
        ...
        *,
        ...
        can have a bridge id distinct from the OmniCraft session id, so the
        lookup first resolves ``omnicraft.codex_native.bridge_id`` from
    ...
    async def _handle_codex_native_interrupt(conv_id: str) -> Response:
        ...
        injecting a ``[System: interrupted]`` bubble into the OmniCraft mirror would
        ...
        *own* ``[Request interrupted by user]`` record, which is real.
        ...
        """
        from omnicraft.codex_native_app_server import client_for_transport
        from omnicraft.codex_native_bridge import (
            CODEX_NATIVE_BRIDGE_ID_LABEL_KEY,
            bridge_dir_for_bridge_id,
        ...
        codex_client = client_for_transport(
            state.socket_path,
            client_name="omnicraft-codex-native-runner",
        )
        try:
            await codex_client.connect()
            if pending_mcp:
                # Startup interrupt first and best-effort: the local
                # cancel above already updated what OmniCraft shows.
                try:
    ...
    async def _handle_codex_native_settings_update(
        ...
        injection path: model/effort changes persisted by OmniCraft are
    ...
    async def _handle_pi_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from omnicraft.pi_native_bridge import bridge_dir_for_session_id, enqueue_interrupt
    ...
    async def _teardown_session_terminals(conv_id: str) -> None:
        ...
        :returns: None.
        """
        from omnicraft.entities.session_resources import terminal_resource_id
        from omnicraft.runner.tool_dispatch import _publish_terminal_deleted_event
    ...
    async def _handle_claude_native_stop(conv_id: str) -> Response:
        ...
        This is the runner-side handler for the OmniCraft web UI's "Stop
    ...
    async def _handle_cursor_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from omnicraft.cursor_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_goose_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from omnicraft.goose_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_kiro_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from omnicraft.kiro_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_kimi_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from omnicraft.kimi_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_goose_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from omnicraft.goose_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_kiro_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from omnicraft.kiro_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_kimi_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from omnicraft.kimi_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_hermes_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from omnicraft.hermes_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_hermes_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from omnicraft.hermes_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_qwen_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from omnicraft.qwen_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_qwen_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from omnicraft.qwen_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_claude_native_effort_change(
        ...
        * *effort* is in ``EFFORT_VALUES`` but not in
        ...
        if effort is None or effort not in CLAUDE_EFFORTS:
            # Persistence already happened on the OmniCraft server; the
    ...
    async def _handle_claude_native_compact(conv_id: str) -> Response:
        ...
        The OmniCraft server's own compaction path (``compact_conversation_now``)
        ...
        Returns 200 (not 204) on successful injection so the OmniCraft server
        can tell the control was handled in the terminal and skip its
        own AP-side compaction. Other harnesses 204 no-op in the
        ``post_session_events`` dispatch and the OmniCraft server runs its
    ...
    async def _handle_codex_native_compact(conv_id: str) -> Response:
        ...
        Returns 200 on successful injection so the OmniCraft server
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        ...
        session ``model`` field → bridge-state ``model_override``) — OmniCraft
        creates the session without a model, so the session field alone is
        usually empty — ask opencode to compact, and return 200 so the OmniCraft
        ...
        """
        from omnicraft.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from omnicraft.opencode_native_client import OpenCodeClientError
    ...
    async def _handle_opencode_native_model_change(conv_id: str, model: str | None) -> Response:
        """
        Apply an OmniCraft-initiated model switch to an opencode-native session.
    ...
    async def _handle_cursor_native_compact(conv_id: str) -> Response:
        ...
        :mod:`omnicraft.cursor_native_forwarder` when it observes cursor write the
        ...
        complete it).  Returns 200 so the OmniCraft server knows the control was
        ...
        """
        from omnicraft.cursor_native_bridge import bridge_dir_for_session_id, inject_user_message
    ...
    async def _handle_pi_native_compact(conv_id: str) -> Response:
        ...
        process, so explicit ``/compact`` must run there. The OmniCraft
        ...
        Returns 200 (not 204) on successful enqueue so the OmniCraft server
        ...
        """
        from omnicraft.pi_native_bridge import bridge_dir_for_session_id, enqueue_compact
    ...
    def _inject_codex_compact(socket_path: str, target: str) -> None:
        ...
        sequence that :func:`~omnicraft.claude_native_bridge.inject_slash_command`
        ...
        """
        from omnicraft.claude_native_bridge import _run_tmux
    ...
    async def _handle_hermes_native_compact(conv_id: str) -> Response:
        ...
        injection so the OmniCraft server knows the control was handled in the
    ...
    async def _handle_qwen_native_compact(conv_id: str) -> Response:
        ...
        :func:`omnicraft.qwen_native_forwarder.supervise_qwen_compaction_mirror` when
        ...
        """
        from omnicraft.qwen_native_bridge import bridge_dir_for_session_id, submit_user_message
    ...
    async def _handle_claude_native_cost_popup(
        ...
        terminal — who never sees the web card — the OmniCraft server forwards a
    ...
    async def _handle_codex_native_cost_popup(
        ...
        harness-agnostic :func:`omnicraft.native_cost_popup.launch_cost_popup`.
        ...
        """
        from omnicraft.native_cost_popup import launch_cost_popup
    ...
    async def _handle_opencode_native_blocked_notice(
        ...
        :returns: 204 once dispatched (or skipped); 503 if launching raised.
        """
        from omnicraft.native_cost_popup import launch_blocked_notice
    ...
    async def _native_cost_popup_config_file(conv_id: str, harness: str) -> Path:
        ...
        """
        from omnicraft.cli_auth import databricks_request_headers
        from omnicraft.opencode_native_bridge import write_cost_popup_config
        from omnicraft.runner._entry import _make_auth_token_factory

        if harness == "claude-native":
            from omnicraft import claude_native_bridge as _cnb
            ...
            from omnicraft import codex_native_bridge as _cxb
    ...
    async def _repop_pending_cost_popup_on_attach(
        ...
        if harness not in ("claude-native", "codex-native", "opencode-native"):
            return
        from omnicraft.native_cost_popup import launch_cost_popup, wait_for_tmux_client
    ...
    async def _cancel_inprocess_turn(conv_id: str) -> None:
        ...
        try:
            ...
            await harness_client.post(
                f"/v1/sessions/{conv_id}/events",
                json={"type": "interrupt"},
                # Bounded under the OmniCraft server's 5s stop deadline.
                timeout=3.0,
            )
    ...
    async def _ensure_comment_relay_started(
        ...
        *,
        ...
        - ``create_session_terminal`` (the ``bridge_inject_dir`` branch), which
          ...
          initial ``tools/list``. This is the normal ``omnicraft claude``
        ...
        # giving centralized policy evaluation on the OmniCraft server. The exact
        ...
        async def _relay_tool_executor(
            ...
            """
            Relay one MCP tool call through the OmniCraft server's /mcp endpoint.

            Routes the call through
            :class:`~omnicraft.runner.proxy_mcp_manager.ProxyMcpManager`
            so the OmniCraft server evaluates TOOL_CALL and TOOL_RESULT policies
    ...
    async def _run_turn_bg(
        ...
        try:
            ...
            # with no resolvable model raising OmniCraftError from
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if cached_spec is None and spec_resolver is not None:
            ...
            if _aid:
                ...
                except (OmniCraftError, httpx.HTTPError, RuntimeError):
                    _logger.warning(
        ...
        if cached_spec is not None:
            ...
            # create, forwarded by the OmniCraft server in the message
            ...
            spawn_env = _build_spawn_env_from_spec(
                ...
                # readout). Forwarded by the OmniCraft server in the message body.
                model_override=msg_body.get("model_override"),
            )
            from omnicraft.runtime.prompt import (
                build_instructions,
            )
        ...
        if conv not in _session_tool_schemas:
            ...
            if cached_spec is not None:
                try:
                    from omnicraft.tools.manager import (
                        ToolManager,
                    )
        ...
        if cached_spec and cached_spec.mcp_servers:
            from omnicraft.runner.mcp_manager import compute_spec_hash
        ...
        # user message is injected. The normal ``omnicraft claude`` /
        # ``omnicraft codex`` path already started it at terminal launch, in
        ...
        if harness_name == "claude-native":
            ...
            # serve-mcp can dispatch OmniCraft builtin tools. Hermes starts
    ...
    async def _stream_message_to_harness(
        ...
        if harness_name == "claude-native" and spawn_env is None:
            from omnicraft.claude_native_bridge import build_claude_native_spawn_env
                            ...
                            if event is not None:
                                ...
                                if is_action_required(event):
                                    ...
                                    if _should_dispatch and _response_id:
                                        ...
                                        # (ProxyMcpManager in OmniCraft mode), which
                                        ...
                                        _dispatch_tasks.append(
                                            _asyncio.create_task(
                                                dispatch_tool_locally(
                                                    ...
                                                    conversation_id=conv_id,
                                                    task_id=_omnicraft_task_id or _response_id,
                                ...
                                # proxies the request to the OmniCraft server's
                                ...
                                if _evt_type == "policy_evaluation.requested":
                                    ...
                                    _dispatch_tasks.append(
                                        _asyncio.create_task(
                                            _evaluate_policy_via_omnicraft(
                                                server_client=server_client,
                                                harness_client=client,
                                                conversation_id=conv_id,
    ...
    async def post_session_events(
        ...
        """
        Inbound surface for the OmniCraft server's post-migration session
        ...
        / ``ApprovalEvent``) — see
        :class:`omnicraft.runtime.harnesses._scaffold.InboundEventRequest`.
        ...
        * ``interrupt`` / ``tool_result`` / ``approval``: control
        ...
        if body_type == "message" or body_type is None:
            ...
            try:
                ...
                if conversation_id in _session_histories:
                    ...
                    # store (invariant I1, omnicraft/server/routes/sessions.py:
        ...
        if body_type == "external_session_status":
            ...
            # Keep this allowlist in sync with OmniCraft server's
            ...
            if status == "idle":
                # Native transcripts are owned by AP. If OmniCraft did not forward
        ...
        if body_type == "stop_session":
            # OmniCraft server forwards a "stop session" request here. Native harnesses
        ...
        if body_type == "effort_change":
            # OmniCraft server forwards the persisted reasoning_effort here
        ...
        if body_type == "model_change":
            # OmniCraft server forwards the persisted model_override here so
        ...
        if body_type == "compact":
            # OmniCraft server forwards explicit /compact here. claude-native
        ...
        if body_type == "clear":
            # OmniCraft server forwards an explicit /clear here. opencode-native
        ...
        if body_type == "cost_approval_popup":
            # OmniCraft server forwards a cost-budget checkpoint here so it can
            ...
            # body) and is always set by the OmniCraft server forwarder; fall back
        ...
        try:
            ...
            # 502 so the OmniCraft route's "best-effort cancel" branch
    ...
    async def list_session_resources(
        ...
        """
        from omnicraft.entities.pagination import paginate_in_memory
    ...
    async def create_session_terminal(
        ...
        if (
            ...
            # ``omnicraft antigravity`` CLI wrapper POSTs ``ensure_native_terminal``
        ...
        if declared_terminal is not None:
            ...
            # allow_cwd_override).
            from omnicraft.tools.builtins.sys_terminal import (
            ...
            # (omnicraft claude) lands here; the launched terminal
        ...
        # Opt-in: callers (e.g. the ``omnicraft claude`` wrapper) can ask the
        ...
        try:
            ...
            resource_view = await launch_method(
                ...
                parent_os_env=agent_os_env,
                # The bridge-inject path is the ``omnicraft claude``
    ...
    async def _ensure_native_terminal_for_turn(conv_id: str, harness_name: str | None) -> None:
        ...
        if instance is not None:
            ...
            # Relê o registro antes de fechar: um caminho concorrente de
            # ensure/recreate pode já ter trocado esta entrada por um painel
            # vivo entre o nosso get() e agora. Só fecha se ainda apontar para
            # a mesma instância morta que sondamos.
            current = terminal_registry.get(conv_id, terminal_name, "main")
            if current is instance:
                # is_alive() deixa instance.running=False como efeito colateral;
                # restaura para o close() emitir o tmux kill-server.
    ...
    # Exposto para teste: o self-heal é um closure, e a única forma de exercitá-lo
    # de verdade (em vez de simular o que ele faz) é chamá-lo daqui.
    app.state.ensure_native_terminal_for_turn = _ensure_native_terminal_for_turn
    ...
    async def transfer_session_terminal(
        ...
        checks: the runner has no OmniCraft permission store. Public callers
        must use the OmniCraft session-resource transfer route, which validates
    ...
    async def _recreate_repl_terminal(
        ...
        """Re-create a dead embedded OmniCraft REPL terminal for attach.
        ...
        exits — the user pressing Ctrl+C inside the REPL, or ``omnicraft
        ...
        async with lock:
            ...
            if existing is None or not existing.running or not await existing.is_alive():
                ...
                try:
                    ...
                    _logger.exception(
                        "Failed to recreate omnicraft REPL terminal for %s",
                        session_id,
                    )
                    return None
        ...
        async with lock:
            ...
            if existing is None or not existing.running or not await existing.is_alive():
                ...
                try:
                    ...
                    # crash the WS route.
                    _logger.exception(
                        "Failed to recreate omnicraft qwen terminal for %s",
                        session_id,
                    )
                    return None
        return resolve_terminal_entry_by_resource_id(session_id, terminal_id, registry)
    ...
    async def terminal_resource_attach_ws(
        ...
        The embedded OmniCraft REPL terminal (role
        :data:`OMNICRAFT_REPL_TERMINAL_ROLE`) and qwen-native terminal
        ...
        if entry is None or not entry.instance.running or not await entry.instance.is_alive():
            if terminal_role == OMNICRAFT_REPL_TERMINAL_ROLE:
    ...
    async def search_environment_files(
        ...
        """
        from omnicraft.runner.environment_filesystem import (
    ...
    async def list_filesystem_changes(
        ...
        """
        from omnicraft.runtime.filesystem_registry import GitStatusUnavailable
    ...
    async def read_environment_file_diff(
        ...
        from omnicraft.entities.environment_filesystem import InvalidPath
        from omnicraft.runner.environment_filesystem import _validate_path
    ...
    async def _resolve_session_spec_entry(session_id: str) -> Any | None:
        ...
        :raises OmniCraftError: If the server returns malformed data
    ...
    async def _resolve_session_skills(session_id: str) -> list[SkillSpec]:
        ...
        * user-global host skills (``~/.claude/skills`` etc., scanned by
        ...
        :raises OmniCraftError: If the session's spec cannot be
            resolved.
        """
    ...
    async def get_session_skills(session_id: str) -> JSONResponse:
        ...
        ``~/.claude/skills/``), not the OmniCraft server's. The server overlays
    ...
    async def get_session_models(session_id: str) -> JSONResponse:
        ...
        The OmniCraft server calls this before routing a turn so the
        ...
        from omnicraft.model_catalog import catalog_for_spec
    ...
    # ``_fetch_model_options`` in omnicraft/server/routes/sessions.py), so it
    ...
    async def resolve_session_skill(session_id: str, request: Request) -> JSONResponse:
        ...
        resolves at runtime. The OmniCraft server calls this, persists the
    ...
    async def reset_session_agent_cache(session_id: str, request: Request) -> JSONResponse:
        ...
        if not isinstance(agent_id, str) or not agent_id:
            agent_id = _session_agent_ids.get(session_id)
        if not agent_id:
            with contextlib.suppress(OmniCraftError, httpx.HTTPError, RuntimeError):
                snapshot = await _session_snapshot(session_id)
    ...
    async def mcp_execute(session_id: str, request: Request) -> JSONResponse:
        ...
        Called by the OmniCraft server's ``POST /v1/sessions/{id}/mcp`` handler
        **after** TOOL_CALL policy evaluation.  The OmniCraft server owns policy
        ...
        - **Runner-local tools** (bare names: ``sys_os_read``,
          ...
          :func:`~omnicraft.runner.tool_dispatch.execute_tool` using
        ...
        - ``tools/list`` — return namespaced MCP tool schemas for the
          ...
          injected by the OmniCraft server in the turn request body).
        ...
        if method == "tools/list":
            ...
            # Return schemas + failures so the OmniCraft server can surface
        ...
        if method == "tools/call":
            ...
            # MRTR retry: OmniCraft server forwards inputResponses + requestState
            ...
            if "__" in tool_name:
                ...
                try:
                    from omnicraft.tools.mcp import McpElicitationRequired

                    if input_responses is not None:
                        # MRTR retry: the OmniCraft server already showed the
                    ...
                    # (MRTR). Pass it back to the OmniCraft server so it can
    ...
    def _resolve_summarize_connection(
        ...
        1. :class:`ProviderAuth` — resolve named provider from
           ``~/.omnicraft/config.yaml``, extract ``api_key`` + ``base_url``
        ...
        """
        from omnicraft.spec.types import ApiKeyAuth, DatabricksAuth, ProviderAuth
        ...
        if auth is None and not _spec_has_legacy_profile:
            from omnicraft.runtime.workflow import _load_global_auth
    ...
    def _resolve_provider_connection(
        ...
        Loads providers from ``~/.omnicraft/config.yaml`` and extracts
    ...
    def _resolve_databricks_connection(
        ...
        :returns: A connection dict with ``"base_url"`` and ``"api_key"``,
            or ``None`` on failure.
        """
        from omnicraft.runtime.credentials.databricks import (
    ...
    async def summarize(request: Request) -> JSONResponse:
        ...
        the runner's credentials are used, not the OmniCraft server's static
        token.
    ...
    if (
        ...
        from omnicraft.native_cost_popup import _list_tmux_clients
        from omnicraft.runner.tool_dispatch import _publish_terminal_deleted_event
        from omnicraft.terminals.pane_reaper import NativePaneReaper, PaneRef
...
def create_runner_app_from_env() -> FastAPI:
    ...
    minimal :class:`httpx.AsyncClient` for the OmniCraft server, then delegates
    ...
    :class:`~omnicraft.runner.transports.tcp.RunnerTCPSubprocess` and
    :class:`~omnicraft.runner.transports.uds.RunnerSubprocess`.  It is
    intentionally lighter than :func:`omnicraft.runner._entry.create_app`
...
_HARNESS_MODEL_ENV_KEY: dict[str, str] = {
    ...
    # :mod:`omnicraft.inner.kimi_executor`; without this mapping a per-session
...
async def _evaluate_agent_start_gate(
    ...
    """
    from omnicraft.runner.policy import RunnerToolPolicyGate
