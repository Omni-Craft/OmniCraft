...
RUNNER_ID_ENV_VAR = "OMNICRAFT_RUNNER_ID"
RUNNER_PARENT_PID_ENV_VAR = "OMNICRAFT_RUNNER_PARENT_PID"
...
RUNNER_WORKSPACE_ENV_VAR = "OMNICRAFT_RUNNER_WORKSPACE"
RUNNER_TUNNEL_BINDING_TOKEN_ENV_VAR = "OMNICRAFT_RUNNER_TUNNEL_BINDING_TOKEN"
RUNNER_TUNNEL_TOKEN_HEADER = "X-OmniCraft-Runner-Tunnel-Token"
...
OMNICRAFT_INTERNAL_WS_ORIGIN = "omnicraft://internal"
...
RUNNER_ISOLATE_SESSION_ENV_VAR = "OMNICRAFT_RUNNER_ISOLATE_SESSION"

# Marker env var stamped into every agent-facing environment so any
# process launched inside an OmniCraft agent session can detect it is
# running under OmniCraft. This is the analog of Claude Code's
# ``CLAUDE_CODE`` / ``CLAUDECODE`` and Codex's ``CODEX``. It is set once
# on the runner process (see :mod:`omnicraft.runner._entry`) and inherited
...
# scrub.
OMNICRAFT_SESSION_ENV_VAR = "OMNICRAFT"
OMNICRAFT_SESSION_ENV_VALUE = "1"
...
def get_stable_runner_id() -> str:
    ...
    the id is loaded from ``~/.omnicraft/runners/runner_id`` or
...
def token_bound_runner_id(token: str) -> str:
    ...
    digest = hashlib.sha256(f"omnicraft-runner:{stripped}".encode()).hexdigest()
...
def load_or_create_runner_id(path: Path) -> str:
    ...
    :param path: Path to the runner id cache file, e.g.
        ``Path.home() / ".omnicraft" / "runners" / "runner_id"``.
...
def _default_runner_id_path() -> Path:
    ...
    :returns: ``~/.omnicraft/runners/runner_id``.
    """
    return Path.home() / ".omnicraft" / "runners" / "runner_id"
