...
from omnicraft.spec.types import AgentSpec, MCPServerConfig
from omnicraft.tools.base import is_valid_tool_name
...
def _build_accept_content(
    ...
    :func:`omnicraft.tools._elicitation_schema.build_accept_content_from_schema`.
...
def _mcp_tool_schema(
    ...
    """
    from omnicraft.tools.mcp import _normalize_input_schema
...
class RunnerMcpManager:
    ...
    def __init__(
        ...
        :param server_client: ``httpx.AsyncClient`` pointed at the
            OmniCraft server. When provided, inline MCP elicitations are
            surfaced to the user via the OmniCraft server's session events
    ...
    def _build_elicitation_callback(
        ...
        async def _elicit(
            ...
            When an OmniCraft server client is available, POSTs a
            ...
            if server_client is None:
                _logger.warning(
                    "MCP elicitation callback: no OmniCraft server client available — declining",
                )
            ...
            try:
                ...
                _logger.warning(
                    "MCP elicitation callback: OmniCraft server POST failed (%s) — declining",
                    exc,
                )
                return ElicitResult(action="decline")
            ...
            if not elicitation_id:
                _logger.warning(
                    "MCP elicitation callback: OmniCraft server returned no "
            ...
            # won't fire on timeout/cancellation, so the OmniCraft server's
            # sidebar badge may stay stale. Same pattern as
            # proxy_mcp_manager. A future enhancement could POST
            # the resolved event back to the OmniCraft server here.
