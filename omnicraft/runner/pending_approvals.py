...
Lifted out of ``omnicraft.runner.app`` so the gate that lives in
``omnicraft.runner.tool_dispatch`` can register and wait without
...
async def wait_for_user_approval(
    *,
    ...
    OmniCraft server's pending-elicitations index (powers the sidebar
    ...
    :param elicitation_id: Correlation id minted by the OmniCraft server's
    ...
    try:
        ...
        # Signal the OmniCraft server's pending-elicitations index that
