...
Pre-refactor the OmniCraft server's :class:`PolicyEngine` enforced
...
``evaluate_policy=True`` to the OmniCraft server, which independently
re-evaluates and parks an elicitation; the runner then awaits the
verdict via :mod:`omnicraft.runner.pending_approvals`. The dual
...
from omnicraft.policies import FunctionPolicy, resolve_function_policy
...
class PolicyVerdict:
    ...
    on ASK (and escalate to the OmniCraft server) rather than treating
    ...
    :param action: ``"allow"`` (tool may proceed), ``"deny"`` (tool
        ...
        the OmniCraft server for a user verdict; ``policy_name`` and
    ...
    def from_spec(cls, spec: AgentSpec) -> RunnerToolPolicyGate:
        ...
        for ps in guard.policies:
            ...
            try:
                ...
                # Pular deixaria o portão vazio, e portão vazio LIBERA toda
                # chamada de ferramenta — o inverso do que a política pede.
