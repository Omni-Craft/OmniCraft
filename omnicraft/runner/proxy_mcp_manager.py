...
Routes all MCP calls through the OmniCraft server's
...
The OmniCraft server holds the live connections in its
:class:`omnicraft.server.mcp_pool.ServerMcpPool` and enforces
...
interface as :class:`omnicraft.runner.mcp_manager.RunnerMcpManager` so
it can be substituted transparently at every dispatch site in
``runner/app.py``.

When to use each implementation:

- **AP mode** (out-of-process runner paired with OmniCraft server):
  ``ProxyMcpManager(session_id, server_client)`` — all tool calls flow
  through the OmniCraft server, policy enforced centrally.
- **No-AP / test mode**:
...
from omnicraft.runner.tool_dispatch import MCP_PROXY_CALL_TIMEOUT_S
...
class ProxyMcpManager:
    """Routes runner-side MCP calls through the OmniCraft server MCP proxy endpoint.

    Drop-in substitute for :class:`omnicraft.runner.mcp_manager.RunnerMcpManager`
    used in OmniCraft mode.  The runner creates one instance per session on first MCP use
    and passes it to :func:`omnicraft.runner.tool_dispatch.execute_tool` in place
    of a direct manager.
    ...
    :param ap_client: An :class:`httpx.AsyncClient` pointed at the OmniCraft server.
        Must already carry the runner's service auth (e.g. Databricks bearer
        token) so requests are accepted by the OmniCraft server's auth middleware.
    """
    ...
    def __init__(
        ...
        :param ap_client: HTTP client pointed at the OmniCraft server.
        ...
        self._omnicraft_client = ap_client
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        """Fetch tool schemas from the OmniCraft server MCP proxy (``tools/list``).

        Sends a ``tools/list`` JSON-RPC 2.0 request to the OmniCraft server's MCP
        proxy endpoint.  Returns tool schemas in the same flat OpenAI
        function-tool format as
        :meth:`omnicraft.runner.mcp_manager.RunnerMcpManager.schemas_for`.

        Tool names are returned with the server namespace prefix applied by
        the OmniCraft server (e.g. ``github__search``).  This ensures the harness
        ...
        try:
            resp = await self._omnicraft_client.post(
                self._mcp_url,
                json=payload,
                timeout=30.0,
            )
        ...
        for tool in tools_list:
            ...
            if not name:
                continue
            # The OmniCraft server returns ``inputSchema`` (JSON Schema from MCP).
    ...
    async def call_tool(
        ...
        """Dispatch a tool call via the OmniCraft server MCP proxy (``tools/call``).

        Sends a ``tools/call`` JSON-RPC 2.0 request.  The OmniCraft server enforces
        TOOL_CALL and TOOL_RESULT policies before forwarding to the real MCP
        server.

        **ASK policy / approval flow**: when the OmniCraft server returns an
        ...
        :returns: Tool output string.  On denial the OmniCraft server returns an
        ...
        """
        del spec  # OmniCraft server resolves spec from session context
        ...
        for _attempt in range(2):
            ...
            # ── ASK: OmniCraft server returned InputRequiredResult ───────────────
            ...
            if result.get("resultType") == "input_required":
                ...
                # The OmniCraft server uses the elicitation_id as the key in
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op — the OmniCraft server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op — the OmniCraft server owns and manages MCP connections."""
