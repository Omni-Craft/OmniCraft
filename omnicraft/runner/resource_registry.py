...
# Role marker for the embedded OmniCraft REPL terminal auto-created for
# runner-hosted SDK sessions (``omnicraft attach`` in a tmux pane — the
...
OMNICRAFT_REPL_TERMINAL_ROLE = "omnicraft-repl"
...
class SessionResourceRegistry:
    ...
    def set_session_status_publisher(
        ...
        event onto the session's SSE queue (which the OmniCraft server relays
    ...
    def compute_default_env_root(
        ...
        1. ``self._runner_workspace`` (sourced from
           ``OMNICRAFT_RUNNER_WORKSPACE``) — when set, ALWAYS
