...
if TYPE_CHECKING:
    from omnicraft.runtime.filesystem_registry import FilesystemRegistry
...
from omnicraft.native_coding_agents import public_agent_name
...
from omnicraft.tools.builtins.os_env import (
    SysOsEditTool,
    SysOsReadTool,
    SysOsShellTool,
    SysOsWriteTool,
...
from omnicraft.tools.builtins.upload_file import UploadFileTool, safe_resolve
...
_SESSION_WRAPPER_LABEL_KEY = "omnicraft.wrapper"
...
# runner (runner → OmniCraft server → runner). ``sys_os_shell`` accepts caller-provided
...
# state via the OmniCraft server's existing REST endpoints (GET /items, GET
...
# (omnicraft.spec.types.SharePolicy) that enable the sys_session_share
...
# these proxy the OmniCraft server's REST endpoints (GET /v1/sessions/{id}/agent,
...
# The runner proxies the OmniCraft server's session policy REST endpoint.
...
# rides through the OmniCraft ``/mcp`` endpoint (comment, session read/write,
...
def build_native_relay_tool_schemas(spec: Any | None) -> list[dict[str, Any]]:
    """Build the flat OmniCraft tool surface for native harness bridges.
    ...
    centralized policy enforcement on the OmniCraft server.
    ...
    if spec is not None:
        ...
        from omnicraft.tools.builtins.policy import SysAddPolicyTool, SysPolicyRegistryTool
...
# sys_os_write, e.g. following the ``build-omnicraft`` skill).
_AGENT_CONFIG_SUBDIR = ".omnicraft/agent-configs"
...
async def _execute_uc_function_tool(
    ...
    *,
    ...
    :func:`omnicraft.runner.uc_function.execute_uc_function`.
    ...
    :returns: Tool output as a string, or an error message.
    """
    from omnicraft.runner.uc_function import execute_uc_function
...
async def _list_child_sessions(
    *,
    ...
    :param server_client: OmniCraft server client.
...
async def _build_subagent_message_content(
    ...
    *,
    ...
    :param server_client: Authenticated OmniCraft server client.
...
def _normalize_subagent_model(
    ...
    *,
    ...
    :func:`omnicraft.model_override.normalize_model_for_provider` for
    ...
    """
    from omnicraft.model_catalog import resolve_model_provider
...
async def _execute_list_models_tool(*, agent_spec: Any | None) -> str:
    ...
    cached in :mod:`omnicraft.model_catalog`).
...
async def _execute_subagent_tool(
    ...
    *,
    ...
    :param server_client: httpx client pointed at the OmniCraft server.
    ...
    if existing is not None:
        ...
        # and the requested harness must canonicalize into OMNICRAFT_HARNESSES.
        ...
        # override against the GLOBAL ``OMNICRAFT_HARNESSES`` (plus the omnicraft
        ...
        if harness_override is not None:
            ...
            if canonical not in OMNICRAFT_HARNESSES:
                return (
                    f"Error: sys_session_send 'harness' {harness_override!r} is "
                    f"not a known harness; must be one of {sorted(OMNICRAFT_HARNESSES)}."
                )
        ...
        from omnicraft.onboarding.harness_install import missing_harness_cli
    ...
    from omnicraft.server.schemas import SessionCreatedEvent
    ...
    if created_child:
        ...
        # has no subscribers (they live in the OmniCraft server), so a direct
        # publish here is silently dropped. ``publish_event`` enqueues onto
        # the parent's queue, which the OmniCraft server's relay republishes onto
...
async def _execute_session_create(
    ...
    *,
    ...
    :param server_client: HTTP client pointed at the OmniCraft server; ``None``
...
def _bundle_local_agent_source(source: Path) -> bytes:
    ...
    :param source: Local agent config YAML, agent directory, or
        bundle file, e.g.
        ``Path("/work/.omnicraft/agent-configs/helper.yaml")``.
...
async def _upload_config_bundle(
    ...
    *,
    ...
    :param config_path: Caller-supplied path to the agent config YAML,
        ...
        cwd, e.g. ``".omnicraft/agent-configs/helper.yaml"``.
...
def _has_subagent(
    ...
    dict (omnicraft inner loader) for a matching name.
    ...
    for sa in sub_agents:
        if getattr(sa, "name", None) == sub_agent_name:
            return True
    # OmniCraft inner loader: tools dict with AgentTool entries
    tools = getattr(agent_spec, "tools", None)
...
async def _execute_timer_set(
    ...
    *,
    ...
    """
    from omnicraft.runner import app as _app
...
async def _execute_comment_tool(
    ...
    *,
    ...
    The runner is a separate subprocess from the OmniCraft server and has no
    in-process ``CommentStore``. This handler uses ``server_client`` to
    call the OmniCraft server's REST API (``GET/PATCH
...
def _project_api_item(item: dict[str, Any]) -> dict[str, str | None]:
    ...
    Mirrors :func:`omnicraft.tools.builtins.spawn._project_activity_item`
...
async def _execute_session_query_tool(
    ...
    *,
    ...
    OmniCraft server's existing REST endpoints over ``server_client``:
    ...
    :mod:`omnicraft.tools.builtins.spawn` so the LLM sees identical
...
def _omnicraft_error_message(resp: httpx.Response) -> str | None:
    """
    Extract the human-readable message from an OmniCraft error response.

    The server renders :class:`omnicraft.errors.OmniCraftError` as
    ``{"error": {"code": ..., "message": ...}}`` (see the exception
    handler in ``omnicraft/server/app.py``). Return that ``message`` so a
...
async def _session_share_via_rest(
    ...
    if resp.status_code >= 400:
        ...
        detail = _omnicraft_error_message(resp)
...
async def _execute_agent_tool(
    ...
    *,
    ...
    these proxy the OmniCraft server's REST endpoints over ``server_client``:

    - ``sys_agent_get`` → ``GET /v1/sessions/{id}/agent`` (project the
      :class:`~omnicraft.server.schemas.AgentObject`)
...
async def _agent_get_via_rest(
    ...
    Projects the :class:`~omnicraft.server.schemas.AgentObject` fields
...
async def _session_get_history_via_rest(
    ...
    # lives only in the OmniCraft server's pending-elicitations index, replayed
...
async def _session_close_via_rest(
    ...
    On success marks the child with ``omnicraft.closed=true`` and
...
class _PeekMeta:
    ...
    :param pending_elicitations: Outstanding
        ...
        parked on, replayed on the snapshot from the OmniCraft server's
        :mod:`omnicraft.runtime.pending_elicitations` index. Empty list
...
def _clone_os_env_spec(spec: Any) -> Any:
    ...
    Symmetric with :func:`omnicraft.inner.terminal._clone_sandbox_spec`;
...
def _runner_default_os_env_cwd(conversation_id: str | None) -> str:
    ...
    root = Path(
        os.environ.get(
            "OMNICRAFT_RUNNER_OS_ENV_ROOT",
            str(Path(tempfile.gettempdir()) / "omnicraft-runner-os-envs"),
        )
    )
...
def _effective_runner_os_env_spec(
    ...
    - When ``runner_workspace`` is set, it ALWAYS wins — whether
      ...
      passes it via ``OMNICRAFT_RUNNER_WORKSPACE``) and
    ...
    :param runner_workspace: Authoritative runtime cwd for the
        runner, sourced from ``OMNICRAFT_RUNNER_WORKSPACE``.
...
async def _seed_os_env_snapshot(
    ...
    :meth:`~omnicraft.runtime.filesystem_registry.FilesystemRegistry.seed_snapshot`
    ...
    :param os_env: The :class:`~omnicraft.inner.os_env.OSEnvironment` used for
...
async def _execute_os_env_tool(
    ...
    *,
    ...
    """
    from omnicraft.inner.os_env import _DEFAULT_READ_LIMIT, create_os_environment
...
async def _execute_file_tool(
    ...
    *,
    ...
    :param server_client: HTTP client for the OmniCraft server.
    ...
    :param runner_workspace: Authoritative runtime cwd for the runner,
        sourced from ``OMNICRAFT_RUNNER_WORKSPACE``. Combined with
...
async def _execute_terminal_tool(
    ...
    *,
    ...
    # the OmniCraft server's relay republishes onto ``session_stream``.
...
async def _emit_terminal_resource_event(
    *,
    ...
    (:func:`omnicraft.server.routes.sessions._publish_and_persist_resource_event`)
...
async def _publish_terminal_created_event(
    *,
    ...
    if resource_registry is not None:
        ...
        from omnicraft.entities.session_resources import terminal_resource_view
...
def _subagent_tool_result_policy_request(
    ...
    """
    Build the OmniCraft policy-evaluation request for delayed child output.
...
async def _post_subagent_policy_verdict(
    *,
    ...
    """
    POST delayed sub-agent output to OmniCraft policy evaluation.

    :param server_client: HTTP client pointed at OmniCraft server.
...
def _apply_subagent_policy_verdict(
    ...
    """
    Apply an OmniCraft policy verdict to a sub-agent inbox payload.

    :param payload: Original completed sub-agent payload.
    :param verdict: Parsed OmniCraft policy response, e.g.
...
def _inject_orchestrator_skills(
    ...
    """
    Auto-inject built-in platform skills for every omnicraft agent.

    The ``build-omnicraft`` skill teaches the LLM how to author valid
    agent configs. Every agent on the platform should have access to it
    — whether it declares ``tools.agents`` or not — so that any
    ``omnicraft claude`` user can author and launch new agents. The
    skill is injected from the canonical source at
    ``omnicraft/onboarding/agent/skills/build-omnicraft/`` when not
    ...
    if "build-omnicraft" in existing_names:
        return skills
    from omnicraft.spec.parser import _discover_skills
...
def _execute_skill_tool(
    ...
    *,
    ...
    :returns: Tool output string.
    """
    from omnicraft.tools.builtins.load_skill import LoadSkillTool
    from omnicraft.tools.builtins.read_skill_file import ReadSkillFileTool

    bundled_skills = list(getattr(agent_spec, "skills", None) or [])
    skills_filter = getattr(agent_spec, "skills_filter", "all")
    # Auto-inject the build-omnicraft skill for agents that opt into the
