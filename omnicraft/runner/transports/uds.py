...
The CLI integration that wires `omnicraft run` to spawn both a
...
class RunnerSubprocess:
    ...
    Used by tests + Phase 2's `omnicraft run` to ship the runner as
    ...
    :param app_factory_path: Module-and-attribute path uvicorn will
        ...
        ``"omnicraft.runner.app:create_runner_app_from_env"``. Uvicorn's
    ...
    :param startup_timeout_s: Max wait for the subprocess to be
        ...
        factory cold-imports ``omnicraft.runner.app`` (and the whole
    ...
    app_factory_path: str = "omnicraft.runner.app:create_runner_app_from_env"
    ...
    def __enter__(self) -> RunnerSubprocess:
        if self.socket_path is None:
            ...
            self._tmp_dir = tempfile.TemporaryDirectory(prefix="omnicraft-runner-")
        ...
        # (including the omnicraft source) are importable.
        cmd = [
            sys.executable,
            "-m",
            "uvicorn",
