...
from omnicraft.runtime import _globals
...
def get_terminal_registry() -> TerminalRegistry:
    ...
    ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.2.
...
def set_runner_router(router: RunnerRouter | None) -> None:
    ...
    Production OmniCraft servers use this instead of a process-wide runner
    client so each dispatch resolves the correct runner from
    ``conversations.runner_id`` and the live tunnel registry.

    :param router: :class:`omnicraft.runner.routing.RunnerRouter`,
...
def get_runner_router() -> RunnerRouter | None:
    ...
    :returns: :class:`omnicraft.runner.routing.RunnerRouter` or
        ``None``.
    """
...
def set_harness_process_manager(manager: HarnessProcessManager | None) -> None:
    ...
    ``omnicraft/server/app.py`` after ``HarnessProcessManager.start()``.
...
def get_harness_process_manager() -> HarnessProcessManager:
    ...
    :raises RuntimeError: If the lifespan startup hasn't run yet
        (e.g. workflow invoked before OmniCraft boot completed, or in a
    ...
    if pm is None:
        raise RuntimeError(
            "HarnessProcessManager not initialized — OmniCraft lifespan startup "
