...
Never import this module outside of omnicraft.runtime.
...
# ``omnicraft.runtime``. Conversation-scoped: one entry per
...
# lifespan in ``omnicraft/server/app.py``; accessed via
# ``get_harness_process_manager()`` in ``omnicraft.runtime``.
...
# ``omnicraft.runtime``), the runner routes
...
# in :mod:`omnicraft.server.routes.terminal_attach` produces the
...
def init(
    *,
    ...
    # ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.2 for the design.
