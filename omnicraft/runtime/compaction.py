...
from omnicraft.llms.adapters._content import redact_inline_data_uris
...
class _CompactionState:
    ...
    :param conversation_id: Conversation id, e.g.
        ...
        the runner's credentials rather than the OmniCraft server's.
    """
...
def _clear_binary_content(
    ...
    for i, msg in enumerate(messages):
        ...
        # Blocos image/file cobrem o formato estruturado; a varredura abaixo
        # pega o base64 embutido como data URI dentro de texto — que é como
        # ele chega no resultado de uma ferramenta.
...
async def summarize_history(
    ...
    runner's credentials are used instead of the OmniCraft server's. Falls
...
async def _summarize_via_runner_uncached(
    ...
    so the OmniCraft server needs no LLM auth for this call.
...
def compaction_to_history_items(
    ...
    :param compaction_item: The compaction item from the store,
        ...
        ``data`` of type :class:`~omnicraft.entities.CompactionData`.
    :returns: Two :class:`~omnicraft.entities.ConversationItem`
...
async def compact(
    ...
    *messages* copy passed to the LLM is compacted.
    ...
    :param runner_client: Optional ``httpx.AsyncClient`` pointed at
        ...
        credentials are used instead of the OmniCraft server's. ``None``
