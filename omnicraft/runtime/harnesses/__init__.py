...
subset of the OmniCraft REST API.
...
serves to external clients (re-use ``omnicraft.server.schemas`` —
...
_HARNESS_MODULES: dict[str, str] = {
    ...
    # omnicraft/inner/claude_sdk_harness.py.
    "claude-sdk": "omnicraft.inner.claude_sdk_harness",
    # User-facing alias accepted in specs / OmniCraft harness dispatch.
    "claude": "omnicraft.inner.claude_sdk_harness",
    # Native Claude Code terminal bridge used by ``omnicraft claude``.
    "claude-native": "omnicraft.inner.claude_native_harness",
    # Native Codex TUI terminal bridge used by ``omnicraft codex``.
    "codex-native": "omnicraft.inner.codex_native_harness",
    # Step 4c: codex harness wrap. See
    # omnicraft/inner/codex_harness.py.
    "codex": "omnicraft.inner.codex_harness",
    # Step 4d: pi harness wrap. See
    # omnicraft/inner/pi_harness.py.
    "pi": "omnicraft.inner.pi_harness",
    # Native Pi TUI bridge used by ``omnicraft pi``.
    "pi-native": "omnicraft.inner.pi_native_harness",
    # Native Antigravity (agy) TUI terminal bridge used by
    # ``omnicraft antigravity``. The in-process SDK counterpart is the
    # canonical ``antigravity`` harness registered below.
    "antigravity-native": "omnicraft.inner.antigravity_native_harness",
    # Step 4e: openai-agents harness wrap. See
    # omnicraft/inner/openai_agents_sdk_harness.py. Registry
    # key is the OmniCraft-side spelling (``openai-agents``,
    # no ``-sdk`` suffix) to match
    # ``OmniCraftExecutor``'s harness allowlist and the
    # ``executor.harness`` field used in OmniCraft YAML; the
    ...
    # ``NATIVE_HARNESSES``. See omnicraft/inner/cursor_native_harness.py.
    "cursor-native": "omnicraft.inner.cursor_native_harness",
    # Native Kiro TUI bridge used by ``omnicraft kiro``.
    "kiro-native": "omnicraft.inner.kiro_native_harness",
    ...
    # omnicraft/inner/goose_native_harness.py.
    "goose-native": "omnicraft.inner.goose_native_harness",
    ...
    # omnicraft/inner/qwen_native_harness.py.
    "qwen-native": "omnicraft.inner.qwen_native_harness",
    # Native Kimi Code TUI bridge used by ``omnicraft kimi``. Drives the resident
    ...
    # above. See omnicraft/inner/kimi_native_harness.py.
    "kimi-native": "omnicraft.inner.kimi_native_harness",
    # Google Antigravity SDK harness wrap. See
    # omnicraft/inner/antigravity_harness.py. In-process SDK harness
    # (``google-antigravity``), like openai-agents — OmniCraft spawns no CLI
    ...
    "antigravity": "omnicraft.inner.antigravity_harness",
    # Qwen Code harness wrap. See omnicraft/inner/qwen_harness.py.
    # Drives the ``qwen`` CLI in ACP mode (``qwen --acp``) for agent execution.
    "qwen": "omnicraft.inner.qwen_harness",
    # Headless Goose harness wrap. See omnicraft/inner/goose_harness.py.
    ...
    "goose": "omnicraft.inner.goose_harness",
    # Native OpenCode server bridge used by ``omnicraft opencode``. The runner
    ...
    # omnicraft/inner/opencode_native_harness.py.
    "opencode-native": "omnicraft.inner.opencode_native_harness",
    # ``opencode`` is accepted as a friendly alias for the canonical
    # ``opencode-native`` (there is no separate SDK ``opencode`` harness).
    "opencode": "omnicraft.inner.opencode_native_harness",
    # GitHub Copilot SDK harness wrap. See omnicraft/inner/copilot_harness.py.
    ...
    # OmniCraft spawns no separately-installed CLI. Authenticates against GitHub's
    # Copilot backend with a GitHub token (no Databricks gateway).
    "copilot": "omnicraft.inner.copilot_harness",
    ...
    # session store. See omnicraft/inner/hermes_harness.py and
    # omnicraft/inner/hermes_executor.py. The ``hermes`` binary must be
    # on PATH (or set by HARNESS_HERMES_PATH).
    "hermes": "omnicraft.inner.hermes_harness",
    ...
    # omnicraft/inner/hermes_native_harness.py.
    "hermes-native": "omnicraft.inner.hermes_native_harness",
}
