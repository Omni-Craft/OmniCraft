...
- Per-turn translation of OmniCraft :class:`CreateResponseRequest` →
  inner :class:`Message` list + :class:`ExecutorConfig`.
- Per-turn translation of inner :class:`ExecutorEvent`s →
  typed OmniCraft SSE events emitted via :meth:`TurnContext.emit`.
...
from omnicraft.inner.tracing import TracingContext, is_tracing_enabled
from omnicraft.policies.types import FAIL_CLOSED_PHASES
from omnicraft.runtime.harnesses._scaffold import HarnessApp, PolicyVerdictPayload, TurnContext
...
# (e.g. ``mcp__omnicraft__sys_terminal_launch``). Tools whose
...
def _strip_mcp_tool_prefix(name: str) -> str:
    ...
    (e.g. ``mcp__omnicraft__sys_terminal_launch``). The bare
    name (``sys_terminal_launch``) is what the OmniCraft wire shape
    and persisted conversation items carry — kept in sync with
    :func:`omnicraft.runtime.workflow._observed_tool_call_sse_dicts`
    and :func:`_build_observed_tool_items` so the SSE name, the
    store-item name, and the adapter's emitted name all agree.

    Mirrors :func:`omnicraft.runtime.workflow._strip_mcp_tool_prefix`
...
class ExecutorAdapter(HarnessApp):
    ...
    def __init__(
        ...
        # already carried — that's what lets the OmniCraft REPL dedupe
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        ...
        can round-trip spec-declared tools through OmniCraft via the
        ...
        # OmniCraft server (routed through the runner).
        ...
        from omnicraft.runtime.telemetry import current_session_id, session_scope
        ...
        try:
            ...
            if tctx:
                try:
                    from omnicraft.runtime.telemetry import trace_context_for_response
                ...
                async for event in executor.run_turn(
                    ...
                    if ctx.cancelled.is_set():
                        if tctx is not None and agent_span is not None:
                            from omnicraft.runtime.telemetry import record_cancellation
                    ...
                    if tctx is not None:
                        if isinstance(event, ToolCallRequest):
                            ...
                            if event.usage is not None:
                                from omnicraft.runtime.telemetry import record_llm_usage
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        :raises OmniCraftError: 404 (from the base handler) when no turn is
            in flight.
        """
    ...
    async def _stable_tool_executor(
        ...
        :param tool_name: Tool name from the LLM's call. Carries
            ...
            ``"mcp__omnicraft__sys_terminal_launch"``).
        ...
        # strips the ``mcp__omnicraft__`` prefix before
    ...
    async def _stable_elicitation_handler(
        ...
        Called by :class:`omnicraft.inner.claude_sdk_executor.ClaudeSDKExecutor`
    ...
    async def _stable_policy_evaluator(
        ...
        :returns: The policy verdict from the OmniCraft server.
        """
        ctx = self._current_ctx
        if ctx is None:
            ...
            # the runner's phase-aware default in _evaluate_policy_via_omnicraft —
    ...
    def _translate_event(self, event: ExecutorEvent, ctx: TurnContext) -> None:
        """
        Translate one inner :class:`ExecutorEvent` into OmniCraft SSE
        ...
        if isinstance(event, TextChunk):
            ...
            # Translate inner reasoning to the OmniCraft wire shape so the
            ...
            # (``mcp__omnicraft__sys_terminal_launch``) but the
            # OmniCraft wire shape and persisted conversation items
            # carry the bare form — kept consistent with
            # ``omnicraft/runtime/workflow.py``'s
            ...
            # payload to populate TurnComplete.usage on the OmniCraft side.
            if event.usage is not None:
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        (:data:`omnicraft.runtime.harnesses._client_executor._RETRYABLE_HARNESS_ERROR_CODES`).
        ...
        1. :class:`omnicraft.errors.OmniCraftError` (incl.
        ...
        :returns: An :class:`ErrorDetail` whose ``code`` matches
            the OmniCraft allowlist for known retryable failures, and is
        ...
        if isinstance(exception, OmniCraftError):
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    """
    Map an OpenAI SDK exception onto the OmniCraft semantic code allowlist.
    ...
    extending the allowlist would just produce strings OmniCraft ignores.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the OmniCraft allowlist (e.g.
    ...
    from omnicraft.llms.errors import is_context_length_exceeded
...
def _classify_claude_sdk_exception(exception: BaseException) -> str | None:
    """
    Map a :mod:`claude_agent_sdk` exception onto the OmniCraft semantic
    code allowlist.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the OmniCraft allowlist, or ``None``
...
def _classify_httpx_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`httpx` exception onto the OmniCraft semantic code allowlist.
    ...
    # no semantic code, and the OmniCraft retry classifier treats them
    # as permanent.
...
def _classify_anthropic_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`anthropic` SDK exception onto the OmniCraft semantic
    code allowlist.
    ...
    ``[llm] APIConnectionError`` etc., which the OmniCraft retry
...
def classify_inner_exception(exception: BaseException) -> str | None:
    """
    Map any inner-SDK exception onto the OmniCraft semantic code allowlist.
...
def _normalize_tool_schemas(
    ...
    OmniCraft emits :meth:`omnicraft.tools.base.Tool.get_schema` in
    OpenAI Chat Completions shape: ``{"type": "function",
    "function": {"name", "description", "parameters"}}``. The
    inner :class:`Executor` ABC and its
    :func:`omnicraft.inner.claude_sdk_executor._build_mcp_tools`
...
async def _bridge_one_dispatch(
    ...
    *,
    ...
    # Try to parse the output as JSON — if OmniCraft serialized the
...
def _extract_role_keyed_messages(
    ...
    """
    Pull role-keyed message items out of an OmniCraft ``input`` list.
    ...
    :param input_value: The OmniCraft ``input`` list.
...
def _serialize_tool_result(event: ToolCallComplete) -> str:
    ...
    - A list of typed content blocks (``{"type": "text", "text":
      ...
      the LLM-rendered tool output reaches OmniCraft intact. Without
    ...
    - Empty string is a last resort — the inner Executor
      ...
      logged so a regression surfaces in the OmniCraft logs.
