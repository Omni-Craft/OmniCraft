...
Invoked by :class:`omnicraft.runtime.harnesses.process_manager.HarnessProcessManager`
...
- ``--module <python_path>``: importable module that exports
  ...
  :data:`omnicraft.runtime.harnesses._HARNESS_MODULES` before
...
- ``--conversation-id <id>``: the conversation this process serves.
  ...
  doc. OmniCraft allocates the id; the runner does NOT parse it from the
...
_GRACEFUL_SHUTDOWN_TIMEOUT_S = float(os.environ.get("OMNICRAFT_HARNESS_SHUTDOWN_TIMEOUT_S", "5"))
...
_HARD_EXIT_TIMEOUT_S = float(
    os.environ.get(
        "OMNICRAFT_HARNESS_HARD_EXIT_TIMEOUT_S",
...
def _load_harness_app(harness: str, module_path: str, conversation_id: str) -> FastAPI:
    ...
    :param module_path: Fully-qualified Python module to import,
        e.g. ``"omnicraft.inner.claude_sdk_harness"``. Must
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    :param argv: Argument list (without the program name),
        ...
        "omnicraft.inner.claude_sdk_harness", "--socket",
        "/tmp/omnicraft/<id>/conv-abc.sock", "--conversation-id",
    ...
    parser = argparse.ArgumentParser(
        prog="python -m omnicraft.runtime.harnesses._runner",
        description=(
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Bind the endpoint OmniCraft allocated for this conversation: a Unix socket
