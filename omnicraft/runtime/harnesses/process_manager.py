...
orphan sweep so a previous OmniCraft crash doesn't leave runner processes
behind.

This module knows nothing about the harness API — it spawns a
:mod:`omnicraft.runtime.harnesses._runner` subprocess per
...
:mod:`omnicraft.server.schemas`; everything FastAPI-shaped is in
...
from omnicraft.harness_plugins import missing_install_packages
from omnicraft.inner import _proc
from omnicraft.inner._subprocess_lifecycle import close_subprocess_transport
from omnicraft.runner.identity import strip_runner_auth_secrets
...
# sockets (POSIX) and the AP_PID sentinel file. Each OmniCraft instance gets a
...
# a still-running OmniCraft (leave alone) or a crashed one (kill its
...
_HARNESS_IDLE_TIMEOUT_ENV = "OMNICRAFT_HARNESS_IDLE_TIMEOUT_S"


def _resolve_harness_idle_timeout_s() -> float:
    """Resolve the harness idle-reap window in seconds.

    Honors :envvar:`OMNICRAFT_HARNESS_IDLE_TIMEOUT_S` (``0`` disables reaping);
...
# or model-thinking gaps. OmniCraft doesn't bound harness-side latency —
...
def _default_tmp_parent() -> Path:
    ...
    Operators can set :envvar:`OMNICRAFT_HARNESS_TMP_PARENT` when
...
def _socket_path(instance_dir: Path, conversation_id: str) -> Path:
    ...
    :param instance_dir: This OmniCraft instance's directory, e.g.
        ``/tmp/omnicraft/ap-abc123``.
...
def _resolve_module_path(harness: str) -> str:
    ...
    raise RuntimeError(
        ...
        f"omnicraft.runtime.harnesses._HARNESS_MODULES)"
    )
    ...
    while True:
        if process.returncode is not None:
            ...
            raise RuntimeError(
                ...
                f"{process.returncode} during spawn (see OmniCraft stderr)"
            )
...
class _SubprocessEntry:
    ...
    :param client: The :class:`httpx.AsyncClient` OmniCraft uses to talk
        to this subprocess over its Unix socket.
    :param socket_path: Absolute Unix socket path the runner
        bound, e.g.
        ``Path("/tmp/omnicraft/ap-abc/conv-xyz.sock")``.
...
class HarnessProcessManager:
    ...
    Use ``start()`` once at OmniCraft boot to create the per-instance
    ...
    terminal state, and ``shutdown()`` at OmniCraft shutdown to release
    ...
    :param reaper_interval_s: Seconds between idle-reaper passes.
        Defaults to 60.
    :param tmp_parent: Override for the parent ``/tmp/omnicraft``
        ...
        :envvar:`OMNICRAFT_HARNESS_TMP_PARENT` to choose a
    ...
    def __init__(
        ...
        *,
        ...
        # OMNICRAFT_HARNESS_IDLE_TIMEOUT_S env var, else the 30-minute default.
    ...
    def instance_dir(self) -> Path:
        """
        This OmniCraft instance's per-instance directory.

        :returns: Path like ``/tmp/omnicraft/ap-<uuid>``.
        """
    ...
    async def get_client(
        ...
        async with spawn_lock:
            ...
            if entry is None:
                ...
                # immediately rather than registering a process that would then
                # depend on ``shutdown``'s ``release`` drain to reap it. Release
                # bumps generation only while holding this spawn lock, so a
                # concurrent release cannot invalidate mid-spawn — it runs after
                # we drop the lock.
                if self._shutting_down:
                    await self._close_entry(entry)
                    raise RuntimeError(
    ...
    async def forward_cancel(
        ...
        :param conversation_id: The OmniCraft conversation id whose
    ...
    async def release(
        ...
        conversation ids the OmniCraft instance has ever spawned for. If
    ...
    async def shutdown(self) -> None:
        ...
        OmniCraft exits after shutdown).
        """
        if not self._started:
            return
        # Flip before cancelling the reaper / releasing so an in-flight
        # ``get_client`` that is past its spawn-lock wait discards its spawn
        # instead of registering or returning a client during teardown. Spawns
        # already in ``_spawn_locks`` are drained by the release loop below;
        # the flag closes the window for one still finishing its bind.
    ...
    async def _spawn_entry(
        ...
        # with OmniCraft logs. The runner doesn't currently prefix lines
    ...
    async def _idle_reaper_loop(self) -> None:
        ...
        ``OMNICRAFT_HARNESS_IDLE_TIMEOUT_S=0``) disables reaping — each
        pass is a no-op.
        ...
        while True:
            ...
            # (``OMNICRAFT_HARNESS_IDLE_TIMEOUT_S=0``). Without this guard a 0
    ...
    async def _sweep_orphans(self) -> None:
        ...
        for child in self._tmp_parent.iterdir():
            ...
            try:
                ...
                continue
            if _pid_alive(pid):
                # Sibling OmniCraft is still running — leave it alone.
                continue
            _logger.info(
                "sweeping orphaned OmniCraft instance dir %s (pid %d not running)",
                child,
                pid,
            )
