...
* :func:`omnicraft.runtime.session_stream` is pure fan-out with no
...
:func:`omnicraft.runtime.session_stream.publish` (the single SSE
chokepoint, same as :mod:`omnicraft.runtime.pending_elicitations`):
...
the durable transcript. The index lives only in the OmniCraft process, so it
...
``yield``/``await``*. On the single OmniCraft event loop where the relay
...
# message_id is retired, and :func:`omnicraft.runtime.session_stream.publish`
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> bool:
    ...
    * a ``session.status`` event whose status is terminal (see
    ...
    :returns: ``True`` when the event must be WITHHELD from the live
        ...
        (:func:`omnicraft.runtime.session_stream.publish`) must skip
...
def discard(conversation_id: str) -> None:
    ...
    Called from the OmniCraft relay's teardown (``_relay_runner_stream``'s
