...
Lets the OmniCraft server answer "which sessions have a pending approval
...
lifecycle: when the OmniCraft process dies, both the index and every parked
...
:func:`omnicraft.runtime.session_stream.publish` whenever a
...
* In-memory only; multi-replica OmniCraft deploys would each see their own
...
* Events emitted before the OmniCraft server starts (e.g. between turns,
...
def record_publish(conversation_id: str, event: dict[str, Any]) -> None:
    ...
    * ``response.elicitation_resolved`` — drop the elicitation id
      ...
      OmniCraft server picks it up via ``_relay_runner_stream`` without
    ...
    :param event: The event dict as passed to
        :func:`omnicraft.runtime.session_stream.publish`. Reads
...
def counts_for(conversation_ids: list[str]) -> dict[str, int]:
    ...
    :class:`omnicraft.server.schemas.SessionListItem` in one
...
def project_for_peek(event: dict[str, Any]) -> dict[str, Any]:
    ...
    :class:`omnicraft.tools.builtins.spawn.SysSessionGetHistoryTool`
