...
:class:`omnicraft.server.schemas.ElicitationResult` and
...
async def _await_elicitation(
    *,
    ...
    in :func:`omnicraft.runtime.workflow._drive_policy_approval`.
    ...
    :class:`~omnicraft.errors.ElicitationDeclinedError` so callers can
...
def _resolve_elicitation_mode() -> str:
    ...
    Reads ``OMNICRAFT_ELICITATION_MODE`` (``"url"`` or ``"form"``).
    Defaults to ``"url"`` when unset — the standalone approval page
    is the default experience.

    :returns: ``"url"`` or ``"form"``.
    """
    raw = os.environ.get("OMNICRAFT_ELICITATION_MODE", "url").lower().strip()
...
def build_elicitation_request_event(
    ...
    *,
    ...
    :param elicitation_id: Unique id correlating this request to the
        consumer's :class:`omnicraft.server.schemas.ElicitationResult`
...
def _is_explicit_decline(raw: str | None) -> bool:
    ...
    :class:`~omnicraft.errors.ElicitationDeclinedError` instead of
