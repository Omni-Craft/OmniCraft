...
from omnicraft.policies.base import Policy
...
from omnicraft.policies.types import PolicyLLMClient
...
_USER_DAILY_COST_POLICY_PATH = "omnicraft.policies.builtins.cost.user_daily_cost_budget"
...
_SUBAGENT_COST_POLICY_PATH = "omnicraft.policies.builtins.cost.subagent_cost_budget"
...
_ASK_ON_ADD_POLICY_SPEC = FunctionPolicySpec(
    ...
    function=FunctionRef(
        path="omnicraft.policies.builtins.safety.ask_on_add_policy",
        arguments=None,
    ),
)
...
def _load_user_daily_cost(
    ...
    """
    from omnicraft.db.utils import now_epoch, utc_day
...
def build_policy_engine(
    *,
    ...
    :param server_llm: Server-level LLM configuration from
        ...
        :class:`~omnicraft.policies.types.PolicyLLMClient` is
...
def _build_policy_llm_client(
    ...
    The :class:`~omnicraft.llms.client.Client` is instantiated lazily
...
def _resolve_databricks_connection(profile: str) -> dict[str, str]:
    ...
    Uses
    :func:`~omnicraft.runtime.credentials.databricks.resolve_databricks_workspace`
...
def _load_default_policy_specs(
    ...
    :raises OmniCraftError: If an enabled policy has an unsupported type.
    """
    if policy_store is None:
        return []
    from omnicraft.db.db_models import current_workspace_id
...
def invalidate_default_policy_specs_cache() -> None:
    ...
    workspace context via :func:`~omnicraft.db.db_models.current_workspace_id`.
    """
    from omnicraft.db.db_models import current_workspace_id
...
def _load_session_policy_specs(
    ...
    raises :class:`OmniCraftError` rather than being skipped, so a
    ...
    :raises OmniCraftError: If an enabled policy has an unsupported
        ``type`` (e.g. ``type="url"``).
    """
    if policy_store is None:
        return []
...
def _stored_policy_to_spec(policy: StoredPolicy) -> PolicySpec:
    ...
    For ``type="url"``, raises :class:`OmniCraftError` (URL policy evaluation
    ...
    :raises OmniCraftError: If the policy ``type`` cannot be evaluated yet
        (e.g. ``type="url"``).
    """
    if policy.type == "python":
