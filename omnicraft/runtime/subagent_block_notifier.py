...
``omnicraft/runner/app.py`` leaves the orchestrator idle and unaware. This
...
coroutine (kept free of HTTP/server knowledge) wired up by the OmniCraft server
...
class SubagentBlockNotifier:
    ...
    One instance is constructed and registered per OmniCraft server process.
    Its :meth:`observe` method is registered with
    :func:`omnicraft.runtime.pending_elicitations.set_elicitation_observer`;
    ...
    :param loop: The OmniCraft server's event loop, captured at registration
