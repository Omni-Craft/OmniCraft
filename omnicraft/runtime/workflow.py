...
from omnicraft.env_credentials import expand_envvars_with_omnicraft_prefix
...
from omnicraft.onboarding.ucode_state import UcodeAgentState, read_ucode_state
from omnicraft.runtime import (
    get_artifact_store,
    get_conversation_store,
    get_file_store,
)
from omnicraft.runtime.compaction import (
    CompactionResult,
    SummaryMetadata,
    _CompactionState,
    compact,
...
from omnicraft.runtime.content_resolver import resolve_content_references
from omnicraft.runtime.prompt import build_instructions, history_to_input_items
from omnicraft.spec import AgentSpec
from omnicraft.spec.parser import check_unresolved_env_vars
...
from omnicraft.stores import ConversationStore
...
# Translates OmniCraft spec executor types (underscore) to harness subprocess names
...
def _get_runner_client_for_compaction(
    ...
    profile) instead of the OmniCraft server's.
    ...
    Propagates :class:`OmniCraftError` when the pinned runner is
    ...
    if conversation_id is None:
        return None
    from omnicraft.runtime import get_runner_router
...
# (``omnicraft/onboarding/provider_config.py`` ``_HARNESS_FAMILY``);
...
_HARNESS_DATABRICKS_PROFILE: dict[AgentHarnessType, str] = {
    ...
    # so OmniCraft cannot thread a Databricks gateway through. Users configure
    # providers via ``kimi provider add`` in ``~/.kimi/config.toml``
    # (OmniCraft-side provider injection is a deferred follow-up).
}
...
def _provider_auth_command(family: FamilyConfig) -> str:
    ...
    :raises OmniCraftError: If the family carries neither a static
...
def configure_agent_harness_with_provider(
    ...
    *,
    ...
    of ``~/.omnicraft/config.yaml``) and emits the **same** vendor-neutral
    ...
    :param harness_type: Canonical harness type, e.g. ``"claude-sdk"``.
    :raises OmniCraftError: If an inline-family provider lacks the family
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is wired only into the native ``omnicraft claude`` launch
        # (:func:`omnicraft.claude_native._bedrock_config_for_native_claude`),
        ...
        raise OmniCraftError(
            ...
            f"native 'omnicraft claude' terminal, not the {harness_type!r} harness. "
            "For agents / 'omnicraft run', use a 'gateway' provider "
    ...
    if entry.kind == CLI_CONFIG_KIND:
        ...
        if harness_type != "codex":
            raise OmniCraftError(
                ...
                "harness in ~/.omnicraft/config.yaml.",
                code=ErrorCode.INVALID_INPUT,
            )
...
# Maps an omnicraft provider family to the bundled catalog provider name
...
def _catalog_default_model(family_name: str) -> str | None:
    ...
    :func:`omnicraft.onboarding.providers.default_chat_model` for the rule
    ...
    :param family_name: The omnicraft family, ``"anthropic"`` or
        ``"openai"``.
...
def _apply_provider_family(
    ...
    :raises OmniCraftError: If no model is resolvable (neither the spec nor
    ...
    if cfg.model_key not in env:
        ...
        raise OmniCraftError(
            ...
            "'models: {default: ...}' to that provider family in ~/.omnicraft/config.yaml.",
            code=ErrorCode.INVALID_INPUT,
        )
    if harness_type == "codex":
...
def _optional_provider_family(entry: ProviderEntry, family_name: str) -> FamilyConfig | None:
    ...
    The only :class:`OmniCraftError` raised at family-access time comes from
...
def _apply_provider_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    :raises OmniCraftError: If no configured family's credentials resolve,
...
def _apply_cli_config_databricks_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    The gateway-harness pi launch (``omnicraft run`` / agents) and pi-native
    ...
    (:func:`omnicraft.pi_native_credentials._cli_config_pi_provider`) — which
    ...
    :param entry: The resolved ``cli-config`` provider entry (a Databricks
        ...
        reaches here).
    :raises OmniCraftError: If the cli-config entry cannot be translated into a
    ...
    from omnicraft.pi_native_credentials import _cli_config_pi_provider
...
def _resolve_provider_for_build(
    ...
    *for_launch*) and the readout / cost / native paths (without). Precedence,
    ...
    :raises OmniCraftError: If a named :class:`ProviderAuth` references a
    ...
    if isinstance(auth, ProviderAuth):
        ...
        if entry is None:
            raise OmniCraftError(
                ...
                "configured under 'providers:' in ~/.omnicraft/config.yaml. "
                "Run `omnicraft setup --no-internal-beta` to configure one.",
                code=ErrorCode.INVALID_INPUT,
            )
        return entry
...
def _build_claude_sdk_spawn_env(
    ...
    *,
    ...
    vars defined in ``omnicraft/inner/claude_sdk_harness.py``.
    Per the v1 spec-config-flow design (see §Step 5b in the
    design doc), per-spawn env overrides are how OmniCraft threads
    ...
    # 3. Global config ~/.omnicraft/config.yaml auth: — only when spec has
    ...
    # for omnicraft-style specs (the inner CLI auto-creates a
    # ``caller_process`` os_env when ``--os`` is set, and the
    # SDK's bundled CLI exposes the natives unconditionally in
    # some configurations); routing through OmniCraft mode without a
    # similar default leaves OmniCraft mode users staring at a
    ...
    # active and approval requests surface via OmniCraft elicitation. Read from
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    defined in ``omnicraft/inner/codex_harness.py``. Mirrors
    ...
    :func:`omnicraft.inner.executor_factory.create_executor`
    ...
    on the OmniCraft server directly (they propagate to the subprocess
...
def _build_pi_spawn_env(
    ...
    *,
    ...
    defined in ``omnicraft/inner/pi_harness.py``. Mirrors
...
def _build_qwen_spawn_env(
    ...
    *,
    ...
    defined in ``omnicraft/inner/qwen_harness.py``. Mirrors
    :func:`_build_claude_sdk_spawn_env` /
    ...
    # variants, the qwen wrap (omnicraft/inner/qwen_harness.py) and
...
def _build_goose_spawn_env(
    ...
    *,
    ...
    ``omnicraft/inner/goose_harness.py``. Unlike the SDK harnesses, Goose owns its
...
def _build_acp_spawn_env(
    ...
    *,
    ...
    ``HARNESS_ACP_*`` env vars defined in ``omnicraft/inner/acp_harness.py``.
    ...
    from omnicraft.onboarding.acp_auth import acp_agents, resolve_acp_agent
...
def _load_global_auth() -> ApiKeyAuth | DatabricksAuth | None:
    """
    Load the ``auth:`` block from ``~/.omnicraft/config.yaml``.

    Reads the user-level global config file (respecting
    ``$OMNICRAFT_CONFIG_HOME`` for test isolation) and parses the
    ...
    so the user only configures auth once during ``omnicraft setup``
    ...
    :returns: A :class:`ApiKeyAuth` or :class:`DatabricksAuth`, or
        ...
        file is missing.
    """
    config_home = os.environ.get("OMNICRAFT_CONFIG_HOME")
    path = (
        Path(config_home) / "config.yaml"
        if config_home
        else Path.home() / ".omnicraft" / "config.yaml"
    )
    if not path.exists():
        return None
    try:
    ...
    if auth_type == "api_key":
        ...
        api_key = expand_envvars_with_omnicraft_prefix(api_key)
        check_unresolved_env_vars("auth.api_key", api_key)
        raw_base_url = raw_auth.get("base_url")
        base_url: str | None = None
        if raw_base_url:
            base_url = expand_envvars_with_omnicraft_prefix(str(raw_base_url))
...
def _build_openai_agents_sdk_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    env vars defined in
    ``omnicraft/inner/openai_agents_sdk_harness.py``. Threads
    ...
    3. Global config ``~/.omnicraft/config.yaml`` ``auth:`` block —
    ...
    if isinstance(auth, ApiKeyAuth):
        ...
        if not profile and model and model.startswith(("databricks-", "databricks/")):
            ...
            # `omnicraft setup` provider config, never by shell environment.
            profile = "DEFAULT"
        if profile:
...
def _build_cursor_spawn_env(
    ...
    *,
    ...
    in ``omnicraft/inner/cursor_harness.py``. Unlike the gateway-backed
    ...
    ``CURSOR_API_KEY`` registered once via ``omnicraft setup`` (the dedicated
    ``cursor:`` config block — see :mod:`omnicraft.onboarding.cursor_auth`) is
    ...
    if isinstance(spec.executor.auth, ApiKeyAuth):
        ...
        from omnicraft.onboarding.cursor_auth import resolve_cursor_api_key
...
def _build_kimi_spawn_env(
    ...
    *,
    ...
    defined in :mod:`omnicraft.inner.kimi_harness`.
    ...
    :param cwd: Runtime working directory for the kimi subprocess — the
        ...
        When unset, the harness wrap falls back to ``OMNICRAFT_RUNNER_WORKSPACE``.
        Mirrors :func:`_build_pi_spawn_env`'s ``cwd`` handling.
    :returns: A dict of env-var overrides.
    :raises OmniCraftError: If the spec declares ``executor.auth`` —
    ...
    if spec.executor.auth is not None:
        raise OmniCraftError(
            ...
            "resulting model id in the agent spec. OmniCraft-side provider "
...
def _build_antigravity_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    dedicated ``antigravity:`` config block from ``omnicraft setup``; (3) an
...
def _build_copilot_spawn_env(
    ...
    *,
    ...
    ``omnicraft/inner/copilot_harness.py``. Like the cursor / antigravity
    ...
    no auth at all, a GitHub token registered once via ``omnicraft setup`` (the
    dedicated ``copilot:`` config block — see
    :mod:`omnicraft.onboarding.copilot_auth`) is used instead, so a user need not
...
def _load_initial_history(
    ...
    # the empty synthetic pair — the OmniCraft thinks context is ~0 tokens
...
async def compact_conversation_now(
    *,
    ...
    :returns: The :class:`CompactionResult` returned by
        :func:`omnicraft.runtime.compaction.compact`.
    """
    ...
    if preserve_recent_window is not None:
        ...
        from omnicraft.spec.types import CompactionConfig
...
def _route_bare_model_for_compaction(llm_config: LLMConfig) -> LLMConfig:
    ...
    whose :func:`~omnicraft.llms.routing.parse_model_string` defaults any
...
def _maybe_persist_compaction_item(
    ...
    # session while the OmniCraft thinks context is near-zero and never
...
def _find_spec_by_name(
    ...
    from omnicraft.tools.builtins.web_fetch import RESEARCHER_NAME

    if name == RESEARCHER_NAME:
        owner = _find_web_fetch_owner(spec)
        if owner is not None:
            from omnicraft.tools.builtins.web_fetch import build_researcher_spec
