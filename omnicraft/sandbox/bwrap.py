...
:mod:`omnicraft.inner.bwrap_sandbox` calls
...
:mod:`omnicraft.sandbox.seatbelt`).

The bwrap backend is the Linux platform default when the ``bwrap``
binary is on ``PATH`` (selected by
:func:`omnicraft.inner.sandbox._default_sandbox_for_platform`). When
...
from omnicraft.inner.bwrap_sandbox import BwrapSandboxBackend
