...
|---|---|---|
| Agent Management (`/api/agents`) | OmniCraft (ours) | No external reference — this is our own API. |
| Conversations (`/v1/conversations`) | OmniCraft (ours) | No external reference. |
| Sessions (`/v1/sessions`) | OmniCraft (ours) | No external reference. Session-first API for long-running agent interactions. See "Sessions API" section below. |
| Session file resources (`/v1/sessions/{session_id}/resources/files`) | OmniCraft (ours) | No external reference. Files are scoped to their owning session. |
...
|---|---|---|
| Sessions (`/v1/sessions`) | OmniCraft (ours) | No external reference. Purpose-built for agent-native workflows: live tail, queued input, interrupt. |
...
Fields:
  ...
  external_session_id (string or null)
    ...
    Code's session uuid for `omnicraft claude` sessions). Populated
  ...
  pending_inputs (array, default `[]`)
    ...
    writer), so the OmniCraft server holds these in-memory and replays them
  ...
  terminal_pending (boolean, default `false`)
    ...
    resource exists. Set by two paths: (1) directly by the OmniCraft server at
...
`SessionResponse` shape with `id` set to the new conversation id.
  ...
  host_type (string, "external" | "managed", default "external")
    ...
    caller-managed compute — a host registered via `omnicraft host`
    (pass `host_id`) or a caller-managed runner (no `host_id`).
    `"managed"`: the SERVER provisions a sandbox host from its
    `sandbox:` config (see `omnicraft/server/managed_hosts.py`),
    starts `omnicraft host` inside it, binds the session to it, and
  ...
  terminal_launch_args (array of strings or null)
    ...
    dir, OmniCraft URL, auth), which stay runner-owned. Bounds (count /
  ...
  type (string, required)
    Event/input discriminator. Recognized values:
      ...
      - "compact"               — explicit context compaction. Forwarded
                                  ...
                                  its own context) and the OmniCraft server
                                  skips its own compaction. For in-process
                                  harnesses the runner 204 no-ops and the
                                  OmniCraft server runs the compaction itself
...
`omnicraft/server/schemas.py`; the discriminated union
...
> `omnicraft.server.schemas` is the supported pattern.
...
See the per-class docstring in `omnicraft/server/schemas.py` for
