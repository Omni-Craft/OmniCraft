...
The server's :mod:`omnicraft.server.routes.terminal_attach` route
...
if TYPE_CHECKING:
    ...
    from omnicraft.runner.transports.ws_tunnel.registry import (
        RunnerSession,
        TunnelRegistry,
        WSChannelState,
    )
...
def make_tunnel_ws_factory(
    ...
    Install via :func:`omnicraft.runtime.set_runner_ws_factory`. The
    factory shape (callable ``(runner_path) -> async-context``) is
    what :mod:`omnicraft.server.routes.terminal_attach` already
    expects.
...
class _TunneledWSConn:
    ...
    :func:`omnicraft.server.routes.terminal_attach._shuttle_ws_frames`
