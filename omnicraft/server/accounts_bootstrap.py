...
Run once at server startup when ``OMNICRAFT_AUTH_PROVIDER=accounts``.
...
1. **Password flag** — ``--admin-password`` /
   ``OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD``. Headless / CI. Bootstrap
   creates the admin from it directly (this module).
2. **Terminal prompt** — ``omnicraft server`` on a TTY asks for a
...
bootstrap writes a session JWT to ``~/.omnicraft/auth_tokens.json``
(via :mod:`omnicraft.cli_auth`) keyed to this spawn's URL, so the next
``omnicraft run`` is signed in without a prompt. Skipped for
...
from omnicraft.server.auth import _RESERVED_USERS
...
def resolve_admin_username() -> str:
    ...
    1. ``OMNICRAFT_ACCOUNTS_INIT_ADMIN_USERNAME`` env var — explicit
    ...
    """
    explicit = os.environ.get("OMNICRAFT_ACCOUNTS_INIT_ADMIN_USERNAME", "").strip()
    if explicit:
...
class BootstrapResult:
    ...
    :param needs_setup: ``True`` when no admin exists yet and none was
        ...
        ``omnicraft server`` terminal prompt, or a password flag.
    ...
    :param tui_token_written: ``True`` when a CLI session JWT was
        written to ``~/.omnicraft/auth_tokens.json`` (loopback only),
        so the next ``omnicraft run`` is signed in without a prompt.
    """
...
def _mint_loopback_cli_token(
    ...
    *,
    ...
    ``~/.omnicraft/auth_tokens.json`` keyed by the **exact** server
    ...
    per spawn, ``omnicraft run`` 401s against its own loopback
    ...
    try:
        ...
        logger.warning(
            ...
            "`omnicraft run` may need `omnicraft login`",
            exc,
        )
        return False
...
def bootstrap_admin(
    ...
    *,
    ...
    :param account_store: The accounts-specific store backing user
        ...
        for accounts concerns — see ``omnicraft/server/accounts_store.py``
        for the boundary rationale.
    :param init_admin_password: Optional operator-provided
        password from ``OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD``.
    ...
    :param session_ttl_hours: TTL for the CLI session token
        written to ``~/.omnicraft/auth_tokens.json``.
    ...
    if any(u.has_password for u in account_store.list_users()):
        ...
        if init_admin_password:
            ...
            print(
                ...
                "    (--admin-password / OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD).\n"
        ...
        # one-time. Without this, `omnicraft run` 401s against its own
    ...
    #      OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD — headless / CI;
    #   2. the `omnicraft server` terminal prompt (cli.py, on a TTY);
    ...
    # Loopback CLI handoff so `omnicraft run` is signed in without a
