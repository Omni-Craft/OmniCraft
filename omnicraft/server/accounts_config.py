...
Mirrors the cookie + session bits of :class:`omnicraft.server.oidc.OIDCConfig`
...
- ``BASE_URL`` — required. The user-facing base URL of the
  deployment, e.g. ``"https://omnicraft.example.com"`` or
...
class AccountsConfig:
    ...
    def from_env() -> AccountsConfig:
        """Read and validate every required ``OMNICRAFT_ACCOUNTS_*`` env var.
        ...
        def _require(name: str) -> str:
            ...
            if not val:
                raise RuntimeError(
                    ...
                    f"(OMNICRAFT_AUTH_PROVIDER=accounts requires it)"
                )
            return val

        cookie_secret_hex = _require("OMNICRAFT_ACCOUNTS_COOKIE_SECRET")
        try:
            cookie_secret = bytes.fromhex(cookie_secret_hex)
        except ValueError as exc:
            raise RuntimeError(
                "OMNICRAFT_ACCOUNTS_COOKIE_SECRET must be a valid hex string"
            ) from exc
        if len(cookie_secret) < 32:
            raise RuntimeError(
                "OMNICRAFT_ACCOUNTS_COOKIE_SECRET must be at least 32 bytes "
        ...
        init_admin = os.environ.get("OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD") or None
