...
A user-set ``OMNICRAFT_ACCOUNTS_COOKIE_SECRET`` always wins —
...
Caller pattern:
    ...
    os.environ.setdefault("OMNICRAFT_ACCOUNTS_COOKIE_SECRET", secret_hex)
...
def load_or_generate_cookie_secret(data_dir: str | os.PathLike[str]) -> str:
    ...
    1. ``OMNICRAFT_ACCOUNTS_COOKIE_SECRET`` env var — operator-set,
    ...
    """
    env_value = os.environ.get("OMNICRAFT_ACCOUNTS_COOKIE_SECRET")
    if env_value:
        return env_value
    ...
    logger.info(
        ...
        "(set OMNICRAFT_ACCOUNTS_COOKIE_SECRET to share across instances)",
        path,
    )
    return fresh
