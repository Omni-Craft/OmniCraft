...
def resolve_admin_list_path() -> Path:
    ...
    :returns: ``OMNICRAFT_ADMIN_LIST_PATH`` if set, else
        ``<data_dir>/admins`` (see :func:`resolve_data_dir`).
    """
    explicit = os.environ.get("OMNICRAFT_ADMIN_LIST_PATH", "").strip()
    if explicit:
...
class AdminFlagStore(Protocol):
    ...
    Both :class:`~omnicraft.stores.permission_store.PermissionStore`
    and :class:`~omnicraft.server.accounts_store.SqlAlchemyAccountStore`
