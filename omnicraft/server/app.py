"""FastAPI application — main entry point for the omnicraft server."""
...
from omnicraft._platform import resolve_repo_symlink
...
from omnicraft.resources import examples as _examples_resources
...
from omnicraft.server.managed_hosts import ManagedSandboxConfig
from omnicraft.server.mcp_pool import ServerMcpPool
from omnicraft.server.performance_metrics import (
...
from omnicraft.server.routes.mcp_catalog import create_mcp_catalog_router
...
from omnicraft.server.routes.terminal_attach import create_terminal_attach_router
from omnicraft.server.ws_origin import WebSocketOriginMiddleware
from omnicraft.stores import (
    AgentStore,
    ArtifactStore,
    ConversationStore,
    FileStore,
...
from omnicraft.stores.conversation_store import SessionConnectivity
...
def _server_version() -> str:
    ...
    Reads :data:`omnicraft.version.VERSION`, the single source of truth shared
...
# OMNICRAFT_WEB_UI_DIST, without rebuilding or repackaging.
_WEB_UI_DIST = Path(
    os.environ.get("OMNICRAFT_WEB_UI_DIST") or (Path(__file__).parent / "static" / "web-ui")
)
...
def _normalize_tarinfo(tarinfo: tarfile.TarInfo) -> tarfile.TarInfo:
    ...
    ``omnicraft.spec.tar_utils.extract_safe``), so on-disk permissions
    don't depend on it.
...
def _ensure_builtin_agent(
    ...
    *,
    ...
    one. Mirrors the upsert in :func:`omnicraft.cli._register_yaml_bundle`.
    ...
    from omnicraft.db.utils import builtin_agent_id
...
_EXTRA_BUILTIN_AGENTS_ENV = "OMNICRAFT_BUILTIN_AGENT_DIRS"
...
def _build_opencode_native_bundle() -> bytes:
    ...
    from omnicraft.opencode_native import _materialize_opencode_agent_spec
    from omnicraft.spec import materialize_bundle
...
def _build_pi_native_bundle() -> bytes:
    ...
    from omnicraft.pi_native import _materialize_pi_agent_spec
    from omnicraft.spec import materialize_bundle
...
def _ensure_default_cursor_agent(
    ...
    ``omnicraft cursor`` CLI first registers it). Content-aware via
...
def _build_antigravity_native_bundle() -> bytes:
    ...
    from omnicraft.antigravity_native import _materialize_antigravity_agent_spec
    from omnicraft.spec import materialize_bundle
...
def _build_qwen_native_bundle() -> bytes:
    ...
    from omnicraft.qwen_native import _materialize_qwen_agent_spec
    from omnicraft.spec import materialize_bundle
...
def _ensure_default_qwen_agent(
    ...
    built-in native-terminal agent on every deployment (not only after the
    ``omnicraft qwen`` CLI first registers it). Content-aware via
    :func:`_ensure_builtin_agent`.
...
def _build_kimi_native_bundle() -> bytes:
    ...
    from omnicraft.kimi_native import _materialize_kimi_agent_spec
    from omnicraft.spec import materialize_bundle
...
def _ensure_default_kimi_native_agent(
    ...
    built-in native-terminal agent on every deployment (not only after the
    ``omnicraft kimi`` CLI first registers it). Content-aware via
    :func:`_ensure_builtin_agent`.
...
def create_app(
    ...
    :param allowed_domains: Allowed email domains from the server
        ...
        Union'd with ``OMNICRAFT_OIDC_ALLOWED_DOMAINS`` and the
    ...
    if auth_provider is not None and account_store is not None:
        ...
        if isinstance(auth_provider, UnifiedAuthProvider) and auth_provider._source == "accounts":
            from omnicraft.server.accounts_bootstrap import bootstrap_admin
    ...
    async def _lifespan(
        ...
        ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.4.
        ...
        # Apply OMNICRAFT_LOG_LEVEL to the omnicraft namespace after
        # uvicorn's dictConfig runs (dictConfig resets existing handlers,
        # making a pre-run basicConfig call ineffective).
        import os as _os

        _log_level_name = _os.environ.get("OMNICRAFT_LOG_LEVEL", "INFO").upper()
        logging.getLogger("omnicraft").setLevel(getattr(logging, _log_level_name, logging.INFO))
        ...
        from omnicraft.runtime import set_resource_registry
        ...
        from omnicraft.server._runner_ws_tunnel import make_tunnel_ws_factory
        ...
        # passes OMNICRAFT_ACCOUNTS_AUTO_OPEN=0 for --no-open). Broad
        ...
        if _bootstrap_result is not None and _bootstrap_result.open_url:
            ...
            if env_var_is_truthy("OMNICRAFT_ACCOUNTS_AUTO_OPEN", default=True):
                import webbrowser
    ...
    app = FastAPI(title="OmniCraft Server", lifespan=_lifespan)
    ...
    # change (no restart).
    from omnicraft.server.admin_list import load_admin_list
    ...
    from omnicraft.server.managed_hosts import ManagedLaunchTracker
    ...
    async def health(
        ...
        :class:`~omnicraft.server.routes.sessions.SessionLiveness`.
    ...
    async def version() -> dict[str, str]:
        """
        Return the installed omnicraft package version.
    ...
    async def info() -> dict[str, bool | str | None]:
        ...
        Returned at app boot by the frontend (and by ``omnicraft
        ...
        databricks_features = (
            importlib.util.find_spec("omnicraft.onboarding.sandboxes.lakebox") is not None
        )
        ...
        # server_version is the installed omnicraft package version (same
        ...
        # a RoutingClient is explicitly configured (OMNICRAFT_SMART_ROUTING=1
        ...
        try:
            from omnicraft.runtime._globals import _caps
    ...
    if auth_provider is not None and getattr(auth_provider, "login_url", None):
        ...
        if (
            ...
            from omnicraft.server.routes.accounts_auth import (
            ...
            from omnicraft.server.routes.auth import create_auth_router

            # OIDC invites are opt-in (OMNICRAFT_OIDC_ALLOW_INVITES) and
    ...
    # built into ``omnicraft/server/static/web-ui/`` by ``web/``'s Vite
