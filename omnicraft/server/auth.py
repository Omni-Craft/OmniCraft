...
- ``"header"`` (default): reads the ``X-Forwarded-Email`` header
  ...
  (``OMNICRAFT_LOCAL_SINGLE_USER=1``), in which case they fall back
...
# Opt-in multi-user switch. ``OMNICRAFT_AUTH_ENABLED`` is the current
# name; ``OMNICRAFT_ACCOUNTS_ENABLED`` is the pre-rename name, still
# honored as a deprecated alias (see :func:`_auth_enabled`).
_AUTH_ENABLED_ENV = "OMNICRAFT_AUTH_ENABLED"
_AUTH_ENABLED_ENV_DEPRECATED = "OMNICRAFT_ACCOUNTS_ENABLED"
...
# paths (`omnicraft run` in chat.py, the daemon's
# host/local_server.py) and by the canonical bare loopback
# `omnicraft server` (cli.py) — never by deployed multi-user servers.
...
_LOCAL_SINGLE_USER_ENV = "OMNICRAFT_LOCAL_SINGLE_USER"
...
_AUTH_HEADER_ENV = "OMNICRAFT_AUTH_HEADER"
...
_AUTH_HEADER_STRIP_PREFIX_ENV = "OMNICRAFT_AUTH_HEADER_STRIP_PREFIX"
...
def local_single_user_enabled() -> bool:
    ...
    Reads ``OMNICRAFT_LOCAL_SINGLE_USER``, the marker the managed
...
def resolve_auth_header() -> str:
    ...
    Reads ``OMNICRAFT_AUTH_HEADER`` and falls back to
    ...
    e.g. ``OMNICRAFT_AUTH_HEADER=Cf-Access-Authenticated-User-Email`` for
...
def resolve_auth_header_strip_prefix() -> str:
    ...
    Reads ``OMNICRAFT_AUTH_HEADER_STRIP_PREFIX`` and returns it
    ...
    ``OMNICRAFT_AUTH_HEADER=X-Goog-Authenticated-User-Email`` at IAP's
    identity header and set
    ``OMNICRAFT_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:`` so the
...
def _auth_enabled() -> bool:
    ...
    Reads ``OMNICRAFT_AUTH_ENABLED``. The pre-rename name
    ``OMNICRAFT_ACCOUNTS_ENABLED`` is still honored as a deprecated
    ...
    semantics — ``OMNICRAFT_AUTH_ENABLED=0`` disables auth even though
...
def resolve_auth_source() -> str:
    ...
    - An explicit ``OMNICRAFT_AUTH_PROVIDER`` (case-insensitive) always
      wins, e.g. ``"header"`` / ``"oidc"`` / ``"accounts"``. This is the
      low-level escape hatch.
    - Otherwise ``header`` is the default, unless the opt-in switch
      ``OMNICRAFT_AUTH_ENABLED`` is truthy (see :func:`_auth_enabled`,
      which also honors the deprecated ``OMNICRAFT_ACCOUNTS_ENABLED``
      alias). When enabled, the mode depends on whether OIDC config was
      supplied:

      - ``OMNICRAFT_OIDC_ISSUER`` is set → ``"oidc"`` (the operator
    ...
    :returns: The resolved source string, e.g. ``"accounts"``,
        ...
        ``OMNICRAFT_AUTH_PROVIDER``). The caller is responsible for
        rejecting unknown values.
    """
    raw_source = os.environ.get("OMNICRAFT_AUTH_PROVIDER")
    ...
    if _auth_enabled():
        ...
        if os.environ.get("OMNICRAFT_OIDC_ISSUER", "").strip():
            return "oidc"
        return "accounts"
    return "header"
...
class UnifiedAuthProvider(AuthProvider):
    ...
    ``OMNICRAFT_AUTH_PROVIDER``. OIDC and accounts modes share
    ...
    :param local_single_user: When ``True``, header mode falls back
        ...
        ``OMNICRAFT_LOCAL_SINGLE_USER`` at construction (see
    ...
    :param header_name: The trusted identity header read in header
        ...
        ``OMNICRAFT_AUTH_HEADER`` at construction, falling back to
    ...
    :param header_strip_prefix: A prefix stripped from the identity
        ...
        ``OMNICRAFT_AUTH_HEADER_STRIP_PREFIX`` at construction, falling
    ...
    def mint_runner_token(self, user_id: str, ttl_seconds: int) -> str | None:
        ...
        if cookie_config is None:
            return None
        from omnicraft.server.oidc import mint_session_token
    ...
    def _check_cookie(self, request: HTTPConnection) -> str | None:
        ...
        authenticated via ``omnicraft login``). Both carry the same
        HS256-signed JWT.
        ...
        from omnicraft.server.oidc import hmac_digest
    ...
    def _check_header(self, request: HTTPConnection) -> str | None:
        ...
        by default, overridable via ``OMNICRAFT_AUTH_HEADER`` — e.g.
        ``Cf-Access-Authenticated-User-Email`` for Cloudflare Access).

        When :attr:`_header_strip_prefix` is set (from
        ``OMNICRAFT_AUTH_HEADER_STRIP_PREFIX``), it is removed from the
        ...
        ``OMNICRAFT_LOCAL_SINGLE_USER=1``): there the absent header
...
def create_auth_provider() -> AuthProvider:
    ...
    -----------------------------------
    Set ``OMNICRAFT_AUTH_ENABLED=1`` (or any truthy value) to turn on
    multi-user auth. With no OIDC config present this selects
    ``accounts`` mode — the built-in login flow with
    first-user-is-admin setup. Set the ``OMNICRAFT_OIDC_*`` env vars
    (at minimum ``OMNICRAFT_OIDC_ISSUER``) alongside it and the same
    ...
    leaves it off. An explicit ``OMNICRAFT_AUTH_PROVIDER`` always wins
    over this switch — it only governs the env-unset default. Deploys
    behind an SSO proxy that injects ``X-Forwarded-Email`` set
    ``OMNICRAFT_AUTH_PROVIDER=header`` (Databricks Apps, oauth2-proxy);
    ...
    (``OMNICRAFT_AUTH_ENABLED`` is the renamed opt-in gate,
    commit ``b23e886e``, formerly ``OMNICRAFT_ACCOUNTS_ENABLED``:
    ...
    if source not in ("header", "oidc", "accounts"):
        raise RuntimeError(
            f"Unknown OMNICRAFT_AUTH_PROVIDER={source!r}. Valid: 'header', 'oidc', 'accounts'"
        )
    ...
    if source == "oidc":
        ...
        # — either OMNICRAFT_AUTH_PROVIDER=accounts or the
        # OMNICRAFT_AUTH_ENABLED=1 opt-in without OIDC config
