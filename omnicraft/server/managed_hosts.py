...
The external host flow has a human run ``omnicraft host`` on their own
...
starts ``omnicraft host`` inside it, and waits for that host to
...
:meth:`omnicraft.stores.host_store.HostStore.register_managed_host` and
the managed-token branch in
:mod:`omnicraft.server.routes.host_tunnel`) — the user's own
...
1. **Server YAML** (OSS / self-hosted): :func:`parse_sandbox_config`
   ...
   (``omnicraft server -c`` / ``OMNICRAFT_CONFIG`` /
   ``<data_dir>/config.yaml``)::
       ...
       sandbox:
         ...
         modal:                   # optional block
           image: docker.io/me/omnicraft-host:latest  # default: official image
           secrets: [omnicraft-llm]  # Modal secrets injected as sandbox env
                                     # (harness LLM keys, gateway URLs)
         boxlite:                 # optional block (provider: boxlite)
           image: docker.io/me/omnicraft-host:latest    # shared; default: official
   ...
   (``ghcr.io/omnicraft-ai/omnicraft-host:latest``; see
   :data:`omnicraft.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` and
...
2. **Direct construction** (embedding deployments): build
   ...
   :class:`~omnicraft.onboarding.sandboxes.base.SandboxLauncher`
...
if TYPE_CHECKING:
    from omnicraft.onboarding.sandboxes import SandboxLauncher
...
# operator-overridable (OMNICRAFT_CWSANDBOX_MAX_LIFETIME_S), so the TTL is
...
_HOST_LOG_PATH = "/tmp/omnicraft-host.log"
...
class ManagedSandboxConfig:
    ...
    :param server_url: Public URL of THIS server that the sandboxed
        host dials back to, e.g. ``"https://omnicraft.example.com"``
...
def _modal_launcher_factory(
    ...
    :param image: Registry image reference with omnicraft pre-installed,
        e.g. ``"docker.io/me/omnicraft-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :func:`omnicraft.onboarding.sandboxes.modal._build_sandbox_image`).
    :param secrets: Modal secret names whose env vars (harness LLM
        credentials, gateway URLs) are injected into every sandbox,
        e.g. ``["omnicraft-llm"]``, or ``None`` to resolve from the
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.modal import ModalSandboxLauncher
...
def parse_sandbox_config(raw: object) -> ManagedSandboxConfig | None:
    ...
    :param raw: The raw ``sandbox`` value from the server config YAML,
        ...
        "modal": {"image": "docker.io/me/omnicraft-host:latest"}}``.
    ...
    if provider == "modal":
        ...
        from omnicraft.onboarding.sandboxes.cwsandbox import managed_token_ttl_s

        launcher_factory = _cwsandbox_launcher_factory(
            _parse_cwsandbox_image(raw), _parse_cwsandbox_env(raw)
        )
        # Derived from OMNICRAFT_CWSANDBOX_MAX_LIFETIME_S so the token always
        ...
        from omnicraft.onboarding.sandboxes.e2b import managed_token_ttl_s

        launcher_factory = _e2b_launcher_factory(
            _parse_e2b_template(raw), _parse_provider_env(raw, "e2b")
        )
        # Derived from OMNICRAFT_E2B_MAX_LIFETIME_S so the token always
...
def _parse_modal_image(raw: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with omnicraft pre-installed, e.g. "
            "'docker.io/me/omnicraft-host:latest' (omit it to use the "
            "official image)"
        )
    return image.strip()
...
def _parse_modal_secrets(raw: dict[str, object]) -> list[str] | None:
    ...
    :param raw: The raw ``sandbox`` mapping (provider already known to
        be ``"modal"``).
    :returns: The validated secret names, e.g. ``["omnicraft-llm"]``,
    ...
    if not isinstance(secrets, list) or not all(
        ...
        raise ValueError(
            ...
            "secret names, e.g. ['omnicraft-llm']"
        )
...
def _daytona_launcher_factory(
    ...
    :param image: Registry image reference with omnicraft pre-installed,
        e.g. ``"docker.io/me/omnicraft-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :class:`omnicraft.onboarding.sandboxes.daytona.DaytonaSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.daytona import DaytonaSandboxLauncher
...
def _parse_daytona_image(raw: dict[str, object]) -> str | None:
    ...
    (env-overridable; see
    :mod:`omnicraft.onboarding.sandboxes.daytona`). A
...
def _boxlite_launcher_factory(
    ...
    :param endpoint: Remote ``boxlite serve`` URL (cloud mode), or ``None`` for
        LOCAL mode — boxes run on the omnicraft-server host as embedded micro-VMs
        (no daemon, no ``boxlite serve``).
    :param image: Registry image reference with omnicraft pre-installed, or
        ``None`` to use the official prebaked host image (env-overridable; see
        :class:`omnicraft.onboarding.sandboxes.boxlite.BoxliteSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.boxlite import BoxliteSandboxLauncher
...
def _parse_boxlite_image(section: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with omnicraft pre-installed, e.g. "
            "'docker.io/me/omnicraft-host:latest' (omit it to use the official image)"
        )
    return image.strip()
...
def _cwsandbox_launcher_factory(
    ...
    def _build() -> SandboxLauncher:
        from omnicraft.onboarding.sandboxes.cwsandbox import CWSandboxLauncher
...
def _parse_cwsandbox_image(raw: dict[str, object]) -> str | None:
    ...
    if not isinstance(image, str) or not image.strip():
        raise ValueError(
            ...
            "reference with omnicraft pre-installed (omit it to use the official image)"
        )
    return image.strip()
...
def _e2b_launcher_factory(
    ...
    :param template: E2B template NAME the OmniCraft host image was built
        ...
        :class:`omnicraft.onboarding.sandboxes.e2b.E2BSandboxLauncher`).
    :param env: Names of server-process environment variables (harness
        LLM credentials, gateway URLs, ``GIT_TOKEN``) injected into
        every sandbox, e.g. ``["OPENAI_API_KEY", "GIT_TOKEN"]``, or
        ``None`` to resolve from the launcher's env-var fallback /
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.e2b import E2BSandboxLauncher
...
def _parse_e2b_template(raw: dict[str, object]) -> str | None:
    ...
    OmniCraft host image was built into — NOT a registry image reference
    ...
    the launcher resolves :data:`~omnicraft.onboarding.sandboxes.e2b.TEMPLATE_ENV_VAR`
    ...
    if not isinstance(template, str) or not template.strip():
        raise ValueError(
            ...
            "E2B template the omnicraft host image was built into (e.g. "
            "'omnicraft-host'; see deploy/e2b/README.md) — NOT a registry image "
...
def _islo_launcher_factory(
    *,
    ...
    :param image: Registry image reference with omnicraft pre-installed,
        e.g. ``"docker.io/me/omnicraft-host:latest"``, or ``None`` to
        use the official prebaked host image (env-overridable; see
        :class:`omnicraft.onboarding.sandboxes.islo.IsloSandboxLauncher`).
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.islo import IsloSandboxLauncher
...
def _openshell_launcher_factory(
    *,
    ...
    :param image: Registry image reference with omnicraft pre-installed,
        e.g. ``"docker.io/me/omnicraft-host:latest"``, or ``None`` to use
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.openshell import OpenShellSandboxLauncher
...
# env-var overrides — see omnicraft.onboarding.sandboxes.kubernetes).
...
def _kubernetes_launcher_factory(
    *,
    ...
    :param image: Registry image with omnicraft pre-installed, or ``None`` for
        the official prebaked host image (env-overridable).
    ...
    def _build() -> SandboxLauncher:
        ...
        from omnicraft.onboarding.sandboxes.kubernetes import KubernetesSandboxLauncher
...
async def launch_managed_host(
    *,
    ...
    → start ``omnicraft host`` inside the sandbox with the token +
...
async def resume_managed_host(
    ...
    re-execs ``omnicraft host``, and waits for it to re-register. The caller's
