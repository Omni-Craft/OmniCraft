...
1. **Domain allowlist.** The email's domain is in the effective
   allowlist — the union of ``OMNICRAFT_OIDC_ALLOWED_DOMAINS`` (env,
   frozen at startup in :class:`~omnicraft.server.oidc.OIDCConfig`)
...
3. **Individual invite.** The email was pre-authorized via an opt-in
   OIDC invite (``OMNICRAFT_OIDC_ALLOW_INVITES`` — see
   :mod:`omnicraft.server.accounts_store` /
...
from omnicraft.server.admin_list import AdminList, MtimeCachedIdentitySet, resolve_data_dir


def resolve_allowed_domains_path() -> Path:
    """Resolve the path to the optional allowed-domains file.

    :returns: ``OMNICRAFT_OIDC_ALLOWED_DOMAINS_PATH`` if set, else
        ``<data_dir>/allowed_domains`` (see
        :func:`omnicraft.server.admin_list.resolve_data_dir`).
    """
    explicit = os.environ.get("OMNICRAFT_OIDC_ALLOWED_DOMAINS_PATH", "").strip()
    if explicit:
...
class InvitedEmailLookup(Protocol):
    ...
    Implemented by
    :class:`~omnicraft.server.accounts_store.SqlAlchemyAccountStore`
...
class OidcAdmissionPolicy:
    ...
    :param env_allowed_domains: The frozen domain allowlist parsed from
        ``OMNICRAFT_OIDC_ALLOWED_DOMAINS`` (``OIDCConfig.allowed_domains``).
