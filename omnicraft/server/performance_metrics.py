...
class RequestDurationAccessFormatter(AccessFormatter):
    """
    Uvicorn access formatter that appends OmniCraft request duration.
...
def _create_request_otel_instruments(meter: MeterLike) -> _RequestOtelInstruments:
    ...
    return _RequestOtelInstruments(
        started=meter.create_counter(
            "omnicraft.server.http.requests.started",
            unit="{request}",
            description="HTTP requests started by the OmniCraft server.",
        ),
        completed=meter.create_counter(
            "omnicraft.server.http.requests.completed",
            unit="{request}",
            description="HTTP requests completed by the OmniCraft server.",
        ),
        failed=meter.create_counter(
            "omnicraft.server.http.requests.failed",
            unit="{request}",
...
def _create_processing_otel_instruments(
    ...
    return _ProcessingOtelInstruments(
        ...
        avg_30s_ms=meter.create_gauge(
            "omnicraft.server.http.request.processing.avg_30s",
            unit="ms",
            description=(
