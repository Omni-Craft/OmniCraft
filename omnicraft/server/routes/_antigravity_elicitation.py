...
* :func:`to_elicitation_params` — converts a :class:`PendingInteraction`
  dict (produced by :func:`omnicraft.antigravity_native_steps.pending_interaction`)
  into an :class:`~omnicraft.server.schemas.ElicitationRequestParams` that
  the web UI can render.

* :func:`to_interaction_payload` — converts the user's
  :class:`~omnicraft.server.schemas.ElicitationResult` back into the
...
def to_tui_selection_keys(
    ...
    the pane via
    :func:`omnicraft.antigravity_native_bridge.send_interaction_keys_via_tui`,
