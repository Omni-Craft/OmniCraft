...
The JSON Content-Type guard in :mod:`omnicraft.server.routes._content_type`
...
enforces (:func:`omnicraft.server.ws_origin.origin_allowed`) so HTTP and
...
- the first-party sentinel (:data:`OMNICRAFT_INTERNAL_WS_ORIGIN`,
  ``"omnicraft://internal"``) and any origin in
  ``OMNICRAFT_WS_ALLOWED_ORIGINS`` pass;
...
from omnicraft.server.ws_origin import origin_allowed, parse_allowed_origins
...
def require_trusted_origin(request: Request) -> None:
    ...
    may announce themselves with ``Origin: omnicraft://internal``
    (:data:`OMNICRAFT_INTERNAL_WS_ORIGIN`), but are not required to.
