...
Mounted at ``/auth`` when ``OMNICRAFT_AUTH_PROVIDER=accounts``.
...
from omnicraft.server.auth import _RESERVED_USERS, RESERVED_USER_LOCAL, UnifiedAuthProvider
...
def create_accounts_auth_router(
    ...
    :param admin_list: File-backed admin roster, consulted on each
        ...
        :mod:`omnicraft.server.admin_list`).
    ...
    async def setup(body: SetupRequest) -> Response:
        ...
        that pre-seeds ``OMNICRAFT_ACCOUNTS_INIT_ADMIN_PASSWORD`` also
        ...
        # `omnicraft run` needs a CLI token too, not just the browser cookie.
