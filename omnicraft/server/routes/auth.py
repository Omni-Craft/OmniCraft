...
/ ``cli-poll`` pair supports the ``omnicraft login`` CLI command.

See ``designs/OIDC_AUTH.md`` for the complete design.

These routes are only mounted when ``OMNICRAFT_AUTH_PROVIDER=oidc``.
"""
...
from omnicraft.server.admin_list import AdminList, promote_if_listed
...
from omnicraft.server.oidc_access import OidcAdmissionPolicy, resolve_allowed_domains_path
...
def create_auth_router(
    ...
    :param admin_list: File-backed admin roster. Consulted on each
        ...
        :mod:`omnicraft.server.admin_list`). OIDC's only admin signal.
    :param account_store: Invite-token persistence, required only when
        ``OMNICRAFT_OIDC_ALLOW_INVITES`` is on. ``None`` disables the
    ...
    :param allowed_domains: Domains from the server config's
        ...
        ``OMNICRAFT_OIDC_ALLOWED_DOMAINS`` and the runtime-editable file
    ...
    # decides who may sign in — see omnicraft/server/oidc_access.py.
    ...
    async def logout() -> Response:
        ...
        If ``OMNICRAFT_OIDC_LOGOUT_REDIRECT_URI`` is configured,
...
def _resolve_oidc_email(
    ...
    *not* prove the user controls the email address. Without the
    ...
    ``OMNICRAFT_OIDC_SKIP_EMAIL_VERIFICATION``) waives the gate for
    ...
    # operator opted out (OMNICRAFT_OIDC_SKIP_EMAIL_VERIFICATION) for
    ...
    if not _claim_is_verified_true(claims.get("email_verified")):
        if config.skip_email_verification:
            _logger.info(
                ...
                "(OMNICRAFT_OIDC_SKIP_EMAIL_VERIFICATION is set)",
                email,
            )
            return email
        _logger.warning(
