...
registered at startup with ``omnicraft server --agent``. They are the
...
from omnicraft.server.routes._auth_helpers import require_user as _require_user
from omnicraft.server.schemas import AgentObject, MCPServerSummary, PaginatedList, SkillSummary
from omnicraft.stores import AgentStore
...
def _to_agent_object(agent: Agent, agent_cache: AgentCache) -> AgentObject:
    ...
    return AgentObject(
        ...
        # by a same-named ``omnicraft run`` upload, but lets a newer
