...
Hosts (machines running ``omnicraft host``) connect here via
...
from omnicraft.host.identity import MANAGED_HOST_TOKEN_HEADER
...
from omnicraft.server.host_registry import (
    HostConnection,
    HostRegistry,
    RunnerExitReports,
)
...
def create_host_tunnel_router(
    ...
    *,
    ...
    :param local_single_user: When ``True``, allow a host to re-own a
        ...
        ``None`` (the default) resolves from ``OMNICRAFT_LOCAL_SINGLE_USER``
...
async def _ping_loop(
    ...
    (:data:`omnicraft.stores.host_store.HOST_LIVENESS_TTL_S`): when a
