...
from omnicraft.stores.host_store import HostStore, host_is_live
...
def create_hosts_router(
    ...
    *,
    ...
    :param agent_store: Agent store used to resolve a session's agent
        ...
        :func:`omnicraft.server.app.create_app` always supplies it.
    ...
    async def launch_runner(
        ...
        # CreatedWorktree | None — set ONLY when OmniCraft creates a worktree
        ...
        if result.get("status") == "failed":
            ...
            if result.get("error_code") == HARNESS_NOT_CONFIGURED_ERROR_CODE:
                ...
                # (`omnicraft setup` on the host machine). Surface the
