...
from omnicraft.server.auth import LEVEL_EDIT, LEVEL_READ, AuthProvider, local_single_user_enabled
...
from omnicraft.spec import AgentSpec, extract_safe
...
def create_session_mcp_servers_router(
    ...
    async def _bound_agent(
        ...
        if conv.agent_id is None:
            raise OmniCraftError("Session has no agent binding", code=ErrorCode.INVALID_INPUT)
    ...
    async def list_mcp_servers(request: Request, session_id: str) -> dict[str, Any]:
        ...
        if agent_cache is None:
            raise OmniCraftError("Agent cache not configured", code=ErrorCode.INTERNAL_ERROR)
...
def _summary_from_spec(spec: AgentSpec, name: str) -> MCPServerSummary:
    ...
    raise OmniCraftError("MCP server was not saved", code=ErrorCode.INTERNAL_ERROR)
