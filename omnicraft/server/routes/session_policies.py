...
from omnicraft.entities import Policy
...
from omnicraft.runtime.policies.builder import invalidate_session_policy_specs_cache
from omnicraft.server.auth import LEVEL_EDIT, LEVEL_READ, AuthProvider
from omnicraft.server.routes._auth_helpers import get_user_id, require_access
from omnicraft.server.schemas import (
    _DOTTED_PATH_RE,
    CreateSessionPolicyRequest,
    UpdateSessionPolicyRequest,
)
from omnicraft.spec.types import FunctionPolicySpec, PolicySpec
...
def create_session_policies_router(
    ...
    def _require_session_exists(session_id: str) -> None:
        ...
        :param session_id: The session to check, e.g.
            ``"conv_abc123"``.
        :raises OmniCraftError: 404 if the session is not found.
        """
    ...
    async def create_policy(
        ...
        :raises OmniCraftError: 401/403 if the user lacks edit
        ...
        if body.type == "python":
            ...
            if validation_error:
                raise OmniCraftError(validation_error, code=ErrorCode.INVALID_INPUT)
    ...
    async def list_policies(
        ...
        :raises OmniCraftError: 401/403 if the user lacks read
    ...
    async def get_policy(
        ...
        if policy is None:
            raise OmniCraftError("Policy not found", code=ErrorCode.NOT_FOUND)
        return _entity_to_response(policy)
