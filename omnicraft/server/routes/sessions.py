...
from omnicraft.codex_native_elicitation import codex_elicitation_id
from omnicraft.cost_plan import (
    COST_CONTROL_LABEL_NAMESPACE,
    reserved_cost_control_keys,
)
from omnicraft.db.db_models import LABEL_VALUE_MAX_LEN
from omnicraft.db.utils import generate_agent_id, generate_task_id
...
from omnicraft.errors import ElicitationDeclinedError, ErrorCode, OmniCraftError
...
from omnicraft.policies.types import (
    ElicitationRequest,
    EvaluationContext,
    PolicyAction,
    PolicyResult,
)
from omnicraft.reasoning_effort import (
    EFFORT_CLEAR_VALUES,
    EFFORT_VALUES,
    validate_effort,
)
...
from omnicraft.runner.routing import RunnerRouter
...
from omnicraft.runtime.policies.builder import build_policy_engine, load_session_usage
from omnicraft.runtime.policies.engine import PolicyEngine
from omnicraft.runtime.tool_output import cap_tool_output
from omnicraft.server import presence
from omnicraft.server._elicitation_registry import (
...
from omnicraft.server.routes._auth_helpers import (
...
from omnicraft.server.routes._codex_elicitation import parse_codex_elicitation_request
from omnicraft.server.routes._content_type import (
...
from omnicraft.session_lifecycle import (
    is_session_closed,
...
from omnicraft.stores import AgentStore, ConversationStore
...
from omnicraft.stores.host_store import Host, HostStore
from omnicraft.stores.permission_store import PermissionStore
from omnicraft.tools.client_specified import parse_client_side_tool_specs
...
# assistant response outside the OmniCraft task runtime and need to
...
# semantic item observed outside the OmniCraft task runtime. Unlike a
...
# assistant text delta observed outside the OmniCraft task runtime. The
...
# explicit ``session.interrupted`` edge observed outside the OmniCraft
...
# session.status event observed outside the OmniCraft task runtime
# (e.g. ``omnicraft claude`` mirroring Claude Code's Stop hook into
...
# ``.meta.json`` appears so the OmniCraft server can mint a child Conversation
...
# sub-agents from omnicraft-spawned ones at the data layer.
_CLAUDE_NATIVE_SUBAGENT_WRAPPER_LABEL_VALUE = "claude-code-native-ui-subagent"
_CLAUDE_NATIVE_SUBAGENT_ID_LABEL_KEY = "omnicraft.claude_native.subagent_id"
_CLAUDE_NATIVE_TOOL_USE_ID_LABEL_KEY = "omnicraft.claude_native.tool_use_id"
...
_CLAUDE_NATIVE_DESCRIPTION_LABEL_KEY = "omnicraft.claude_native.description"
...
# forwarder converts each receiver thread into this event so OmniCraft can mint a
...
# omnicraft-spawned and claude-native sub-agents.
...
_CODEX_NATIVE_SUBAGENT_THREAD_ID_LABEL_KEY = "omnicraft.codex_native.subagent_thread_id"
_CODEX_NATIVE_SUBAGENT_PARENT_THREAD_ID_LABEL_KEY = "omnicraft.codex_native.parent_thread_id"
_CODEX_NATIVE_SUBAGENT_TOOL_CALL_ID_LABEL_KEY = "omnicraft.codex_native.collab_tool_call_id"
_CODEX_NATIVE_SUBAGENT_PROMPT_LABEL_KEY = "omnicraft.codex_native.prompt"
_CODEX_NATIVE_SUBAGENT_NICKNAME_LABEL_KEY = "omnicraft.codex_native.agent_nickname"
_CODEX_NATIVE_SUBAGENT_ROLE_LABEL_KEY = "omnicraft.codex_native.agent_role"
...
# reload for sessions where no OmniCraft task carries usage (claude-native).
_LAST_CONTEXT_TOKENS_LABEL_KEY: str = "omnicraft.last_context_tokens"
_LAST_CONTEXT_WINDOW_LABEL_KEY: str = "omnicraft.last_context_window"
...
_LAST_TASK_ERROR_CODE_LABEL_KEY: str = "omnicraft.last_task_error_code"
_LAST_TASK_ERROR_MESSAGE_LABEL_KEY: str = "omnicraft.last_task_error_message"
...
# Session labels stamped by ``omnicraft claude``. A matching session
# is terminal-owned: OmniCraft web-chat input must be forwarded to the local
# runner for tmux injection, and rendered transcript items must come
# back through ``external_conversation_item`` only.
_CLAUDE_NATIVE_WRAPPER_LABEL_KEY = "omnicraft.wrapper"
...
_CLAUDE_NATIVE_UI_LABEL_KEY = "omnicraft.ui"
...
# ``omnicraft codex``. Held at one day like the Claude permission hook:
...
# Maximum time OmniCraft waits for its runner->AP SSE relay to observe the
...
def _native_ask_gate_lock(conversation_id: str, deciding_policy: str) -> asyncio.Lock:
    ...
    :param conversation_id: OmniCraft conversation id whose ASK gate is being
...
def _publish_and_persist_resource_event(
    ...
    """
    from omnicraft.entities.conversation import ResourceEventData
...
async def _publish_and_wait_for_harness_elicitation(
    ...
    *,
    ...
    Mirrors the ``omnicraft claude`` permission hook contract: the
...
def _signal_terminal_resolved_harness_elicitation(
    ...
    :param session_id: OmniCraft conversation id whose forwarder mirrored the
...
def _client_supplied_hook_elicitation_id(
    ...
    :param payload: Parsed PermissionRequest hook body. Reads the
        optional ``_omnicraft_elicitation_id`` key.
    ...
    :raises OmniCraftError: 400 when the id is malformed or is
        currently parked by a different session.
    """
    raw = payload.get("_omnicraft_elicitation_id")
    if raw is None:
        return None
    if not isinstance(raw, str) or not _HOOK_ELICITATION_ID_RE.fullmatch(raw):
        raise OmniCraftError(
            "PermissionRequest hook '_omnicraft_elicitation_id' must match "
...
def _signal_harness_elicitation_resolved_by_id(
    ...
    :param elicitation_id: Harness elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: None.
    :raises OmniCraftError: If the id is malformed or belongs to a
        different session.
    """
...
def _permission_level_from_grants(
    ...
    Mirrors :func:`omnicraft.server.routes._auth_helpers._get_permission_level_sync`
...
def _owner_from_grants(grants: list[SessionPermission]) -> str | None:
    ...
    Mirrors :func:`omnicraft.server.routes._auth_helpers.get_session_owner_id`
    but operates on grants already held in memory so callers can batch the
    permission-store query across many sessions at once.
...
async def _best_effort_stop(
    ...
    session (or any direct sub-agent child) is still running, attempt
    to stop it via the runner. Failures are swallowed so the caller can
...
def _build_session_response(
    ...
    :raises OmniCraftError: If ``conv.agent_id`` is ``None``.
    """
    ...
    # ``omnicraft.ui = "terminal"`` label. That flag is fully determined by the
...
def _publish_input_consumed(
    ...
    :param cleared_pending_id: When this message drained a
        :mod:`omnicraft.runtime.pending_inputs` entry (native-terminal
...
def _resolve_llm_model(conv: Conversation | None) -> str | None:
    ...
    try:
        ...
        from omnicraft.runtime._globals import _agent_store
...
def _validated_harness_override(value: str | None, agent: Agent) -> str | None:
    ...
    in ``omnicraft/chat.py``): the canonical name must be a known bundle
    harness, and the bound agent must be an ``executor.type: omnicraft``
    ...
    :raises OmniCraftError: ``invalid_input`` for an unknown harness, a
        non-omnicraft executor type, or an unloadable agent bundle.
    """
    if value is None:
        return None
    from omnicraft.harness_aliases import canonicalize_harness
    from omnicraft.runtime import get_agent_cache
    from omnicraft.spec._omnicraft_compat import (
        OMNICRAFT_EXECUTOR_TYPE,
        OMNICRAFT_HARNESSES,
    )

    canonical = canonicalize_harness(value) or value
    if canonical not in OMNICRAFT_HARNESSES:
        raise OmniCraftError(
            f"invalid harness_override: must be one of "
            f"{sorted(OMNICRAFT_HARNESSES)}, got {value!r}",
            code=ErrorCode.INVALID_INPUT,
        )
    try:
        loaded = get_agent_cache().load(
    ...
    if executor_type != OMNICRAFT_EXECUTOR_TYPE:
        raise OmniCraftError(
            f"harness_override only applies to executor.type "
            f"{OMNICRAFT_EXECUTOR_TYPE!r} agents; agent {agent.name!r} "
    ...
    **only when the turn is priced** (catalog pricing available or a
    ...
    if llm_model:
        if has_provider_cost:
            ...
            from omnicraft.llms.context_window import compute_llm_cost, fetch_model_pricing
...
def _persist_native_cumulative_usage(
    ...
    Unlike the OmniCraft relay path (:func:`_accumulate_session_usage`), which adds
    ...
    :raises OmniCraftError: When a cumulative field is the wrong type.
    """
...
def _coerce_cumulative_field(
    ...
    *,
    ...
    :raises OmniCraftError: When present but the wrong type / negative.
    """
    value = data.get(key)
    if value is None:
        return None
...
async def _persist_external_session_usage(
    ...
    :raises OmniCraftError: On missing / malformed fields.
    """
    ...
    # from the OmniCraft relay's per-response accumulation. Persist this session's
...
async def _persist_external_model_change(
    ...
    OmniCraft session: writes ``model_override`` so the value survives reload
    ...
    :raises OmniCraftError: If ``data.model`` is missing or not a
        non-empty string.
    """
...
def _validate_external_reasoning_effort(body: SessionEventInput) -> str | None:
    ...
    :raises OmniCraftError: If the payload is missing or unsupported.
    """
...
async def _persist_external_reasoning_effort_change(
    ...
    Mirrors a native-terminal thinking-level change onto the OmniCraft session.
...
async def _persist_external_codex_collaboration_mode_change(
    ...
    :returns: None.
    :raises OmniCraftError: If ``data.mode`` is missing or unsupported.
    """
...
async def _persist_model_change_note(
    ...
    ``omnicraft.wrapper`` native label, via ``_is_native_terminal_session``)
...
def _handle_external_session_todos(
    ...
    :raises OmniCraftError: When ``data.todos`` is missing or not a list.
    """
...
def _publish_external_output_text_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises OmniCraftError: If ``data.delta`` is not a string, or any
...
def _publish_external_output_reasoning_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises OmniCraftError: If ``data.delta`` is not a string, or ``data.started``
...
def _parse_external_assistant_message(
    ...
    :raises OmniCraftError: If required fields are missing or
        malformed.
    """
...
async def _persist_external_assistant_message(
    ...
    """
    Persist and broadcast assistant text produced outside OmniCraft tasks.

    The event is append-only conversation history. It intentionally
    bypasses the legacy persist path so mirroring a
    Claude terminal response does not create or steer an OmniCraft
    agent task.
...
async def _persist_external_subagent_start(
    ...
    never POSTs to OmniCraft to register them. The forwarder watches the
    ...
    omnicraft agent), stamp identifying labels, and publish the
    same ``session.created`` event omnicraft-spawned children fire
    ...
    :raises OmniCraftError: 400 if the payload is missing any of
    ...
    if parent_conv.agent_id is None:
        ...
        # by ``omnicraft claude`` (the synthetic Claude bundle).
    ...
    # Title format mirrors omnicraft-spawned children
...
def _subagent_delivery_status(
    ...
    A claude-native session running as an OmniCraft sub-agent relabels its
...
async def _persist_external_codex_subagent_start(
    ...
    :raises OmniCraftError: If ``thread_id`` is missing or parent has
        no bound agent.
    """
...
async def _persist_external_conversation_item(
    ...
    without starting or steering the placeholder OmniCraft agent.
...
def _is_kiro_native_session(conv: Conversation) -> bool:
    ...
    return conv.labels.get("omnicraft.wrapper") == "kiro-native-ui"
...
def _latest_assistant_text_from_store(
    ...
    helper lets OmniCraft forward the durable assistant output with the
...
def _require_external_status_forward(
    ...
    :returns: None.
    :raises OmniCraftError: If the runner was unavailable or
...
async def _heal_subagent_runner_binding_via_parent(
    ...
    the ancestor chain (immediate parent → root) to find the nearest live
    runner, heals the child's DB row to point at that runner, and returns the
    live ``httpx.AsyncClient`` so the caller can proceed as if the binding were
    always current.

    After healing, the caller must set ``_runner_needs_session_init = True`` and
    re-read the conversation row: rebinding the row does not by itself prove the
    replacement runner has initialized that child's harness/session state.
    ...
    :param conversation_store: Store used to look up ancestors and persist the
        healed ``runner_id``.
    :returns: The live client for the nearest live ancestor runner after
        healing, or ``None`` when no live ancestor could be found.
    """
    # Sobe a cadeia (pai imediato, depois raiz): um salto cobre o caso comum,
    # dois cobrem sub-agente aninhado cujo pai também está com runner morto.
    ...
    for ancestor_id in candidate_ids:
        ...
        # Espera curta pelo túnel do ancestral — cobre a janela de reconexão
        # logo depois de um relançamento. Sem registro (in-process / testes),
        # resolve direto, em melhor esforço.
    ...
    if live_runner_id != child_conv.runner_id:
        ...
        try:
            ...
            # removed mid-teardown). Degrade gracefully rather than surfacing a
...
def _require_collaboration_mode_forward(
    ...
    app-server. Persisting the OmniCraft label without a successful runner
    ...
    :param runner_result: HTTP result returned by the runner, or ``None``
        when no runner could be reached.
    :returns: None.
    :raises OmniCraftError: If no runner was reachable or the runner rejected
...
def _drive_terminal_resolved_elicitation(session_id: str, persisted: ConversationItem) -> None:
    ...
    :param session_id: OmniCraft conversation id the item was mirrored for,
        e.g. ``"conv_abc123"``.
...
def _publish_sandbox_status(session_id: str, stage: str, error: str | None = None) -> None:
    ...
    :param stage: The launch stage just entered, e.g.
        ``"provisioning"`` — one of
        :data:`omnicraft.server.schemas.SandboxLaunchStage`.
...
def _publish_session_superseded(session_id: str, target_conversation_id: str) -> None:
    ...
    ``omnicraft/claude_native_forwarder.py``): a client actively viewing
...
async def _get_runner_client(
    ...
    """
    from omnicraft.runtime import get_runner_client
    ...
    if runner_router is not None:
        ...
        except (LookupError, httpx.HTTPError, OmniCraftError):
            _logger.debug(
...
async def _validate_session_workspace(
    *,
    ...
    :mod:`omnicraft.server.routes._workspace_validation` and
    raises :class:`OmniCraftError` on failure so the route layer
    ...
    :raises OmniCraftError: With ``ErrorCode.INVALID_INPUT`` on
    ...
    """
    from omnicraft.server.routes._workspace_validation import (
    ...
    from omnicraft.server.routes._host_launch import resolve_host_owner
...
class _HostLaunchAttempt:
    ...
    :param error: Human-readable failure message from the host, e.g.
        ...
        `omnicraft setup` ..."``; ``None`` when there was no error.
    """
...
async def _launch_runner_on_host(
    ...
    """
    from omnicraft.host.frames import HostLaunchRunnerFrame, encode_host_frame
...
async def _provision_managed_sandbox(
    *,
    ...
    """
    from omnicraft.server.managed_hosts import launch_managed_host, relaunch_managed_host
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    :param tunnel_registry: Runner-tunnel registry used to await the
        ...
        wirings (the rendezvous then settles at frame-send).
    """
    from omnicraft.server.managed_hosts import terminate_managed_host
...
async def _await_settled_managed_launch(launch: ManagedLaunch) -> None:
    ...
    :raises OmniCraftError: 503 when the launch failed or is still
        running at the timeout.
    """
    from omnicraft.server.managed_hosts import MANAGED_LAUNCH_RENDEZVOUS_TIMEOUT_S
...
async def _maybe_relaunch_managed_sandbox(
    *,
    ...
    :raises OmniCraftError: 503 when the relaunch failed or timed out.
    """
...
def _kick_managed_relaunch(
    *,
    ...
    """
    from omnicraft.server.managed_hosts import MANAGED_REPO_LABEL_KEY, parse_repo_workspace
...
async def _run_managed_wake(
    *,
    ...
    """
    from omnicraft.server.managed_hosts import resume_managed_host
...
async def _get_runner_client_for_resource_access(
    ...
    """
    from omnicraft.runtime import get_runner_client, get_runner_router
...
async def _reset_runner_resources_after_switch(session_id: str) -> None:
    ...
    except (httpx.HTTPError, HTTPException, OmniCraftError, RuntimeError):
        # Best-effort: a runner hiccup must not break the (already-committed)
        # switch. OmniCraftError covers the session-not-runner-bound / runner-
...
def _native_coding_agent_for_session(conv: Conversation) -> NativeCodingAgent | None:
    ...
    * the ``omnicraft.wrapper`` presentation label — set for the built-in
      terminal-first wrapper sessions (``omnicraft claude`` / ``omnicraft
...
def _is_native_terminal_session(conv: Conversation) -> bool:
    ...
    True for both a built-in terminal-first wrapper (``omnicraft.wrapper``
...
def _native_terminal_runtime(conv: Conversation) -> tuple[str, str, str]:
    ...
    :raises OmniCraftError: If the session is not a native terminal harness.
    """
...
def _native_terminal_name_for_harness(harness: str) -> str:
    ...
    :raises OmniCraftError: If *harness* is not a supported native
        terminal harness.
    """
...
async def _persist_host_launch_failure_turn(
    ...
    *,
    ...
    (which names the fix, ``omnicraft setup``), then publishes the same
    ...
    :param host_error: The host's human-readable refusal, e.g.
        ``"harness 'codex' is not configured on host 'laptop' — run
        `omnicraft setup` ..."``. ``None`` falls back to a generic
        ``omnicraft setup`` pointer so the banner is never empty.
...
async def _forward_native_subagent_terminal_failure(
    ...
    :returns: None.
    :raises OmniCraftError: If the parent's runner could not be reached
...
def _build_native_terminal_message_event(
    ...
    :raises OmniCraftError: If the event is not a user message.
    """
...
async def _forward_native_terminal_message(
    ...
    """
    Forward one OmniCraft web-chat message to the native terminal harness.
...
def _extract_claude_native_runner_failure(resp: httpx.Response) -> str | None:
    ...
    ``response.failed``. Claude-native OmniCraft forwarding must treat that
...
async def _forward_session_change_to_runner(
    ...
    * Explicit ``compact`` — the caller inspects the returned status
      ...
      200) or the OmniCraft server must run its own in-process compaction
    ...
    skipped — the persisted value on the OmniCraft side is the authoritative
    ...
    in the OmniCraft log — otherwise the POST succeeds at the httpx layer
...
async def _stop_session_via_runner(
    ...
    :raises OmniCraftError: ``RUNNER_UNAVAILABLE`` (HTTP 503) if the
...
async def _stop_session_host_runner(
    ...
    the host's ``omnicraft host`` tunnel is open while the user drives
...
def _parse_skill_slash_command(body: SessionEventInput) -> tuple[str, str]:
    ...
    :raises OmniCraftError: If the payload is not a skill command
...
def _build_skill_slash_command_policy_body(body: SessionEventInput) -> SessionEventInput:
    ...
    :raises OmniCraftError: If the slash-command payload is invalid.
    """
...
async def _resolve_skill_meta_text_via_runner(
    ...
    :returns: The hidden ``<skill>`` meta text for a single
        ``input_text`` block.
    :raises OmniCraftError: If the skill is not exposed for the session
...
async def _dispatch_skill_slash_command_to_runner(
    ...
    * a hidden ``message`` item with ``is_meta=True`` containing the
    ...
    :raises OmniCraftError: If the skill is not exposed for the
...
async def _forward_event_to_runner(
    ...
    if _should_route:
        from omnicraft.server.smart_routing import route_turn
...
class _SessionEventDispatchResult:
    ...
    :param pending_id: Id of the :mod:`omnicraft.runtime.pending_inputs`
...
async def _dispatch_session_event_to_runner(
    ...
    * **transcript-forwarded native + ``type == "message"``**: web-chat user
      ...
      server. The OmniCraft would otherwise persist an AP-side copy AND
    ...
    * **All other cases**: persist the item AP-side (invariant I1:
    ...
    :mod:`omnicraft.runtime.pending_inputs`). It is rolled back if the
    ...
    :param created_by: Authenticated identity of the posting actor,
        ...
        :func:`omnicraft.runtime.pending_inputs.record` and applied
        ...
        async with runner_client.stream(
            ...
            async for chunk in resp.aiter_text():
                ...
                while "\n\n" in buffer:
                    ...
                    # (the OmniCraft subscriber generates its own heartbeats).
...
async def _ensure_runner_relay_ready(
    ...
    The runner stream has no replay buffer. For item events, OmniCraft must
    subscribe to runner output before it forwards the input event; a
    fast harness can otherwise complete before OmniCraft is listening, leaving
    ...
    :returns: The active relay handle, or ``None`` when no runner is
        bound.
    :raises OmniCraftError: If the relay cannot observe the
...
def _agent_provider_family(agent: Agent) -> str | None:
    ...
    """
    from omnicraft.onboarding.provider_config import provider_family_for_harness
...
def _same_provider_family(a: Agent, b: Agent) -> bool:
    ...
    native transcript from OmniCraft items instead).
...
# OmniCraft items and relaunch the CLI with --resume, so prior turns reappear as
...
# session file from copied items (omnicraft/pi_native_resume.py), the same
...
def _agent_carries_native_fork_history(agent: Agent) -> bool:
    ...
    session file that the runner rebuilds from the copied OmniCraft items on
...
def _presentation_labels_for_agent(agent: Agent) -> dict[str, str]:
    ...
    main view), gated on ``omnicraft.ui == "terminal"`` plus the matching
    ``omnicraft.wrapper`` value; an SDK agent runs as plain chat (no such
...
def _publish_input_deny_terminal(session_id: str, conv: Conversation, reason: str) -> None:
    ...
    :class:`omnicraft_client.SessionsChat.send`) iterate until a
...
async def _persist_policy_deny_sentinel(
    ...
    live deny only exists as the ``_publish_input_deny_terminal`` sentinel
    delta, which the web folds into a provisional ``live:`` preview block
    that the terminal ``response.completed`` sweeps; the deny then only
    reappeared after a refresh re-hydrated the persisted item. Emitting the
    commit event lets the web reconcile the preview into a durable,
    itemId-keyed block that survives the sweep, a reconnect, and a refresh
    alike.
...
async def _wake_parent_for_blocked_child(
    ...
    *outcome* is reported back so the notifier can release its per-block
    ...
    except (httpx.HTTPError, OmniCraftError):
        _logger.warning(
...
def configure_subagent_block_notifier(
    ...
    :mod:`omnicraft.runtime.pending_elicitations` so a sub-agent that
    ...
    """
    from omnicraft.runtime import pending_elicitations as _pending_elicitations
    from omnicraft.runtime.subagent_block_notifier import SubagentBlockNotifier
...
def _validate_terminal_launch_args(value: list[str] | None) -> list[str] | None:
    ...
    a caller to smuggle internal launch wiring (bridge dir, OmniCraft URL,
...
def _validated_cost_control_mode_override(value: str | None) -> str | None:
    ...
    :raises OmniCraftError: 400 (``invalid_input``) when *value* is
...
def _parse_session_create_metadata(metadata: str) -> SessionCreateMetadata:
    ...
    :raises OmniCraftError: If the JSON fails the request schema.
    """
    try:
...
def _require_host_conn_for_worktree(host_id: str | None, request: Request) -> HostConnection:
    ...
    :raises OmniCraftError: ``invalid_input`` when ``host_id`` is
...
async def _create_session_worktree(
    *,
    ...
    :raises OmniCraftError: ``invalid_input`` for a bad branch name,
...
def _spec_config_flag_explicitly_disabled(spec: AgentSpec, key: str) -> bool:
    ...
    ``omnicraft/spec/parser.py`` — ``{str(k): str(v) ...}``), so a YAML
...
def _derive_terminal_launch_args_from_spec(sub_spec: AgentSpec) -> list[str] | None:
    ...
    if harness == _CODEX_NATIVE_HARNESS:
        ...
        # session; the omnicraft executor's later turn/start inherits that
...
def _native_subagent_wrapper_labels(
    *,
    ...
    The pill is gated on the conversation's ``omnicraft.wrapper`` +
    ``omnicraft.ui`` labels (see ``web`` ``TerminalFirstContext``), but
...
def _reject_reserved_cost_control_label_seed(labels: dict[str, str]) -> None:
    ...
    :param labels: The client-supplied initial labels, e.g.
        ``{"team": "ml"}``.
    :raises OmniCraftError: 400 when any ``cost_control.*`` key is
        present.
    """
...
def _reject_server_reserved_label_seed(labels: dict[str, str] | None) -> None:
    ...
    :raises OmniCraftError: 400 when any reserved key is present.
    """
...
def _require_cost_control_label_authority(
    *,
    ...
    :param tunnel_token: Value of the ``X-OmniCraft-Runner-Tunnel-Token``
    ...
    :raises OmniCraftError: 403 when the caller presents no acceptable
...
async def _create_session_from_existing_agent(
    ...
    :raises OmniCraftError: 404 if no agent matches ``body.agent_id``;
    ...
    # Validated against the loaded spec (known harness + omnicraft
    ...
    try:
        ...
        # NOT git_branch: only a worktree OmniCraft created here may be
...
def _create_session_from_bundle(
    ...
    reusing a template by name would make a fresh ``omnicraft run
    ...
    :raises OmniCraftError: If bundle validation or agent insert
    ...
    # ``omnicraft run`` uploads the operator's own bundle through this same
...
def _persist_stored_session_bundle(
    ...
    *,
    ...
    :returns: Response with the new session id.
    :raises OmniCraftError: If the agent insert violates integrity
...
async def _authorize_bundled_parent_and_inherit_runner(
    ...
    *,
    ...
    :raises OmniCraftError: 403/404 when the caller may not access the
        parent session.
    """
    await _require_access(
        user_id,
...
def _registered_runner_id(
    ...
    *,
    ...
    :raises OmniCraftError: If the id is empty, the router is
...
def _child_session_summary_from_conversation(
    ...
    :func:`omnicraft.tools.builtins.spawn._spawn_one`, plus the
...
async def _handle_advise_models_mcp(
    ...
    *,
    ...
    from omnicraft.model_catalog import spec_harness
    from omnicraft.server.smart_routing import fetch_runner_models, infer_models
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The OmniCraft server's role here is routing only —
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    if namespaced_name in ("sys_advise_models", "mcp__omnicraft__sys_advise_models"):
    ...
    # tunnel the runner already opened to the OmniCraft server at startup.
    runner_client = await _get_runner_client(session_id, runner_router)
    if runner_client is None:
        from omnicraft.runtime import get_runner_client
    ...
    if runner_client is None:
        return _mcp_error_response(rpc_id, -32000, f"No runner bound for session {session_id!r}")
    try:
        from omnicraft.runner.tool_dispatch import MCP_PROXY_FORWARD_TIMEOUT_S
...
def create_sessions_router(
    ...
    def _has_runner_created_by_authority(request: Request, conv: Any) -> bool:
        """Whether this request may declare `created_by` for a session event.

        Só o próprio runner pode: ele prova isso com o token de túnel, seja um
        dos tokens do servidor, seja o token ligado ao runner desta conversa.
        """
    ...
    async def create_session(
        ...
        :raises OmniCraftError: If metadata, bundle, or agent lookup
        ...
        if launch_host_id is not None and resp.runner_id is None:
            ...
            if host_registry is not None and host_store_inst is not None:
                ...
                from omnicraft.runner.identity import token_bound_runner_id
                from omnicraft.server.routes._host_launch import resolve_host_launch
                ...
                if result.get("status") == "failed":
                    ...
                    # can be stale (the user may have run `omnicraft setup`
    ...
    async def _create_bundled_session_from_multipart(
        ...
        :raises HTTPException: 422 when a required multipart part is
            absent.
        :raises OmniCraftError: If metadata or bundle validation
    ...
    async def put_read_state(
        ...
        :raises OmniCraftError: 403 if the caller lacks read access.
        """
        user_id = _require_user(request, auth_provider)
    ...
    async def get_session(
        ...
        :raises OmniCraftError: 404 if no session exists.
        """
    ...
    from omnicraft.server.routes.codex.sessions import register_codex_session_routes
    ...
    async def update_session(
        ...
        :raises OmniCraftError: 400 if the runner is not
        ...
        # dispatch — OmniCraft server stays harness-agnostic; the runner
    ...
    async def fork_session(
        ...
        :raises OmniCraftError: 404 if *source_id* does not exist
        ...
        # the copied OmniCraft transcript on its own — mark the fork so the
        ...
        # copied OmniCraft items. Cross-family: the source's native transcript
        # is the wrong format, so ALWAYS rebuild from the copied OmniCraft
        # items (the converters consume OmniCraft's normalized item shape, so
    ...
    async def switch_session_agent(
        ...
        :raises OmniCraftError: 404 if the session or target agent does
        ...
        # OmniCraft items (a format-agnostic conversion). SDK targets replay
    ...
    async def claude_permission_request_hook(
        ...
        the same Bearer token used for every other OmniCraft request. For
        ...
        :param session_id: OmniCraft conversation id from the URL path.
        :returns: Claude PermissionRequest hookSpecificOutput JSON,
            or ``200`` with empty body on timeout (fail-ask).
        :raises OmniCraftError: 404 if the session doesn't exist,
    ...
    async def evaluate_policy(
        ...
        command hooks (via ``omnicraft.claude_native_hook``) to
        ...
        raw_elicitation_id = payload.get("_omnicraft_elicitation_id")
        ...
        if raw_elicitation_id is not None:
            if not isinstance(raw_elicitation_id, str) or not (
                ...
                raise OmniCraftError(
                    "Policy evaluate '_omnicraft_elicitation_id' must match "
    ...
    async def codex_elicitation_request_hook(
        ...
        by ``omnicraft codex`` (for example
        ...
        :raises OmniCraftError: 404 if the session does not exist,
    ...
    async def antigravity_elicitation_request_hook(
        ...
        :class:`~omnicraft.server.schemas.ElicitationResult` so the bridge
        ...
        :class:`~omnicraft.server.schemas.ElicitationRequestParams` dict.
        ...
        :raises OmniCraftError: 404 if the session does not exist, 400 if
    ...
    async def cursor_permission_request_hook(
        ...
        (:mod:`omnicraft.cursor_native_permissions`), publishes the standard
        ...
        :returns: An ``ElicitationResult`` (``{"action": …}``) on a web
            ...
            disconnect.
        :raises OmniCraftError: 404 if the session does not exist, 400 if the
            body is malformed.
        """
        user_id = _get_user_id(request, auth_provider)
        await _require_access(
    ...
    async def list_child_sessions(
        ...
        :raises OmniCraftError: 403 if the caller lacks READ on
    ...
    async def list_session_resources(
        ...
        if runner_client is not None:
            ...
            from omnicraft.runtime import get_terminal_registry
    ...
    async def _validate_session(
        ...
        :raises OmniCraftError: 401/403/404 on auth or access failure.
        """
    ...
    async def create_session_terminal(
        ...
        or ``bridge_inject_dir`` — the ``omnicraft claude`` / ``codex``
        ...
        :raises OmniCraftError: 400 when the requested terminal is not
    ...
    async def _proxy_fs_response(
        ...
        *,
        ...
        if status >= 400:
            ...
            if status == 404:
                raise OmniCraftError(message, code=ErrorCode.NOT_FOUND)
    ...
    async def get_elicitation(
        ...
        :mod:`omnicraft.runtime.pending_elicitations` index — no
    ...
    async def post_event(
        ...
        - ``"external_output_reasoning_delta"`` publishes a transient
          ...
          observed outside the OmniCraft task runtime, without persisting an
          item or starting / steering a task.
        - ``"external_session_interrupted"`` publishes a
          ``session.interrupted`` event observed outside the OmniCraft task
        ...
        - ``"external_codex_collaboration_mode_change"`` persists the
          ...
          (``omnicraft.codex_native.collaboration_mode``).
        ...
        - ``"message"`` on an ``omnicraft claude`` terminal session
        ...
        if body_created_by is not None:
            # O runner reivindica o ator humano que disparou o turno do filho,
            # para o despertar do pai não aparecer como se ninguém o tivesse
            # pedido. Só ele pode, e só se o ator ainda tiver acesso de edição.
        ...
        if body.tools:
            try:
                ...
                raise OmniCraftError(str(exc), code=ErrorCode.INVALID_INPUT) from exc
        ...
        if body.type == _COMPACT_TYPE:
            ...
            # The OmniCraft server stays harness-agnostic: it runs its own
        ...
        if runner_client is None and conv.kind == "sub_agent":
            # Um sub-agente copia o runner_id do pai na criação e nunca é
            # repontado quando o runner do pai é relançado. Com o runner morto
            # mas o pai com substituto vivo, conserta a ligação subindo a
            # cadeia e segue pelo fluxo normal de init + despacho.
            ...
            if runner_client is None and _host_reg is not None:
                ...
                if _host_conn is not None:
                    ...
                    if launch_attempt.error_code == _HARNESS_NOT_CONFIGURED_ERROR_CODE:
                        ...
                        # fix, `omnicraft setup`) the web renders as a
        ...
        if runner_client is None:
            ...
            if body.type == "message" and _is_native_terminal_session(conv):
                ...
                return {"queued": True, "item_id": item_id}
            # Raise so the OmniCraft server doesn't persist an item the
    ...
    async def delete_session(
        ...
        :raises OmniCraftError: 404 if no session or no access,
    ...
    def _policy_description(spec: PolicySpec) -> str | None:
        ...
        if isinstance(spec, FunctionPolicySpec) and spec.function:
            from omnicraft.policies.registry import get_entry
    ...
    async def get_session_agent(
        ...
        :raises OmniCraftError: If the session or agent is not found.
        """
        user_id = _require_user(request, auth_provider)
        access = await _require_access_and_level(
            user_id, session_id, LEVEL_READ, permission_store, conversation_store
    ...
    async def get_session_agent_contents(
        ...
        :raises OmniCraftError: If the session, agent, or bundle is
            not found.
        """
        user_id = _require_user(request, auth_provider)
        access = await _require_access_and_level(
    ...
    async def update_session_agent(
        ...
        if spec.name is None:
            raise OmniCraftError("spec missing name", code=ErrorCode.INVALID_INPUT)
    ...
    async def mcp_proxy(
        ...
        - Policy runs on the OmniCraft server where the ConversationStore and
          label state live.
        ...
        if method == "initialize":
            ...
            return _mcp_ok_response(
                rpc_id,
                {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "omnicraft-mcp-proxy", "version": "1.0.0"},
                },
            )
...
async def _fetch_model_options(
    ...
    * **cursor-native** — a curated *static* base catalog
      (:func:`omnicraft.cursor_native.cursor_base_model_options`), returned
    ...
    * **codex-native** — a *live*, account-scoped catalog only the bound runner
    ...
    if wrapper == _CURSOR_NATIVE_WRAPPER_LABEL_VALUE:
        from omnicraft.cursor_native import cursor_base_model_options

        return cursor_base_model_options()
    if wrapper == _KIRO_NATIVE_WRAPPER_LABEL_VALUE:
        from omnicraft.kiro_native import kiro_base_model_options
...
async def _get_session_snapshot(
    ...
    :raises OmniCraftError: 404 if no session exists, 500 if the
        ...
        try:
            ...
            if agent is not None:
                ...
                if agent.bundle_location is not None:
                    ...
                    from omnicraft.llms.context_window import (
