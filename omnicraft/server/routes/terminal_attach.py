...
- **In-process runner**: the server passes
  :func:`omnicraft.runtime.get_terminal_registry` into
  :func:`omnicraft.runner.create_runner_app`, so the registry is
...
- **Out-of-process runner** over the WebSocket tunnel: the runner
  ...
  (``omnicraft/server/_runner_ws_tunnel.py``), and the runner's
  resource-addressed WS route runs ``tmux attach`` and bridges the
  PTY.

The proxy uses a factory configured via
:func:`omnicraft.runtime.set_runner_ws_factory` in the server
...
from omnicraft.server.auth import LEVEL_OWNER, LEVEL_READ, AuthProvider
from omnicraft.server.routes._auth_helpers import require_access
...
def create_terminal_attach_router(
    *,
    ...
    :func:`omnicraft.server.app.create_app`. The list/CRUD endpoints
