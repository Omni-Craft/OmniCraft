...
class AgentObject(BaseModel):
    ...
    :param harness: The agent's harness/kind, e.g. ``"codex"``,
        ...
        ``executor.type: omnicraft`` agents, otherwise the executor
    ...
    :param builtin: Whether this is a server-*seeded* built-in
        ...
        ``omnicraft server --agent``) or a session-scoped upload.
        The Web UI's new-session picker uses this to decide
        whether a same-named ``omnicraft run`` upload may shadow
...
class ConversationObject(BaseModel):
    ...
    :param labels: Session-scoped guardrails labels, mirroring
        ...
        with the legacy ``omnicraft run`` Ctrl+G overview.
    """
...
class ChildSessionSummary(BaseModel):
    ...
    :param title: Sub-agent title, ``"{agent_type}:{session_name}"``
        as written by :func:`omnicraft.tools.builtins.spawn._spawn_one`,
        e.g. ``"researcher:auth"``. ``None`` only for legacy /
        malformed rows; the spawn path always sets it.
    :param tool: UI-facing sub-agent label. For OmniCraft-spawned
    ...
    :param pending_elicitations_count: Number of approval / input
        ...
        server's :mod:`omnicraft.runtime.pending_elicitations`
...
class ElicitationResult(BaseModel):
    ...
    OmniCraft clients deliver this shape inside the session-scoped
...
class SessionCreateRequest(BaseModel):
    ...
    :param host_type: How the session's host is obtained.
        ...
        host they registered via ``omnicraft host`` (pass
    ...
    :param harness_override: Optional per-session brain-harness
        ...
        ``OMNICRAFT_HARNESSES`` and the bound agent must be an
        ``executor.type: omnicraft`` spec. ``None`` (the default) uses
    ...
    def _check_managed_host_fields(self) -> SessionCreateRequest:
        ...
        from omnicraft.server.managed_hosts import is_repo_workspace, parse_repo_workspace
...
class SessionResponse(BaseModel):
    ...
    :param external_session_id: Runtime-native session id this
        ...
        ``omnicraft claude`` sessions. ``None`` for regular
    ...
    :param pending_elicitations: Outstanding approval prompts on
        ...
        the OmniCraft server's in-memory
        :mod:`omnicraft.runtime.pending_elicitations` index.
    :param pending_inputs: Un-consumed web-composer user messages on
        ...
        :mod:`omnicraft.runtime.pending_inputs` index.
    ...
    :param todos: Current Claude Code todo list items for
        ``omnicraft claude`` sessions, as raw dicts from Claude's
        ...
        from the OmniCraft server's in-memory ``_session_todos_cache``.
    ...
    :param terminal_pending: ``True`` while the runner is auto-creating
        ...
        exists. Sourced from the OmniCraft server's in-memory
    ...
    :param sandbox_status: Managed-sandbox launch progress while the
        ...
        Sourced from the OmniCraft server's in-memory
    ...
    # Source: :mod:`omnicraft.runtime.pending_inputs`.
...
class UpdateSessionRequest(BaseModel):
    ...
    :param external_session_id: Runtime-native session id captured
        ...
        ``omnicraft claude`` sessions). Idempotent on same-value
...
class CodexGoalObject(BaseModel):
    ...
    Mirrors Codex app-server's ``ThreadGoal`` shape using OmniCraft's
...
class UpdateCodexGoalStatusRequest(BaseModel):
    ...
    updates. OmniCraft exposes the two user-driven transitions explicitly:
...
class SessionListItem(BaseModel):
    ...
    :param pending_elicitations_count: Number of approval prompts
        ...
        opening each chat. Sourced from the OmniCraft server's in-memory
        :mod:`omnicraft.runtime.pending_elicitations` index,
...
# ─────────────────────────────────────────────────────────────────────
#
# Single source of truth for the omnicraft SSE event stream. Every
...
class _SSEEventBase(BaseModel):
    ...
    - ``sequence_number``: monotonic per-stream sequence number
      ...
      (``_format_sse`` in ``omnicraft/server/routes/sessions.py``).
...
class SessionStatusEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py``) — i.e. while the parent
...
class SessionUsageEvent(_SSEEventBase):
    ...
    out-of-AP runtime (e.g. the ``omnicraft claude`` transcript
...
class SessionModelEvent(_SSEEventBase):
    ...
    ``omnicraft claude`` transcript forwarder when the model is
...
class SessionCollaborationModeEvent(_SSEEventBase):
    ...
    Category: **transient** (SSE-only). The server also writes
    ``omnicraft.codex_native.collaboration_mode`` on the conversation labels,
...
class SessionTodosEvent(_SSEEventBase):
    ...
    ``omnicraft claude`` transcript forwarder, which captures todo
    updates via ``PostToolUse``/``TodoWrite`` hook events from Claude
    Code and forwards them to the OmniCraft server. Lets web render a
...
class SessionTerminalPendingEvent(_SSEEventBase):
    ...
    1. The OmniCraft server at ``POST /v1/sessions`` for host-launched
    ...
    2. The OmniCraft relay when the runner's ``session.terminal_pending`` frame
...
class SessionSandboxStatusEvent(_SSEEventBase):
    ...
    A managed create returns before its sandbox exists; the OmniCraft
...
class SessionSkillsEvent(_SSEEventBase):
    ...
    ``omnicraft/server/routes/sessions.py``) and serves ``[]`` until
...
class SessionInputConsumedPayload(BaseModel):
    ...
    :param type: The item type discriminator — ``"message"`` for
        ...
        results, etc. Mirrors
        :class:`omnicraft.server.schemas.SessionEventInput`'s
        ``type`` field.
    ...
    :param cleared_pending_id: When this consumed message drains a
        :mod:`omnicraft.runtime.pending_inputs` entry (a native-
...
class SessionInterruptedPayload(BaseModel):
    ...
    ``omnicraft/server/routes/sessions.py``.
...
class SessionInterruptedEvent(_SSEEventBase):
    ...
    ``omnicraft/server/routes/sessions.py`` when a client posts
...
class SessionCreatedEvent(_SSEEventBase):
    ...
    Emitted by ``omnicraft/tools/builtins/spawn.py:_spawn_one``
...
class SessionSupersededEvent(_SSEEventBase):
    ...
    ``omnicraft/server/routes/sessions.py`` when the claude-native
...
class OutputTextDeltaEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py:1352-1356``.
...
class ReasoningStartedEvent(_SSEEventBase):
    ...
    status. Wire shape matches ``omnicraft/runtime/workflow.py:1350``.
...
class ReasoningTextDeltaEvent(_SSEEventBase):
    ...
    shape matches ``omnicraft/runtime/workflow.py:1358-1364``.
...
class ReasoningSummaryTextDeltaEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py:1370-1373``.
...
class OutputItemDoneEvent(_SSEEventBase):
    ...
    :param item: The completed item dict. Heterogeneous and
        ...
        ``omnicraft/entities/conversation.py`` for the
...
class OutputFileDoneEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py`` once per file annotation in
...
class HeartbeatEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py`` (15 seconds at the time of
    writing). Wire shape matches the existing emit at
    ``omnicraft/runtime/workflow.py:4636-4639``.
...
class SessionPresenceEvent(_SSEEventBase):
    ...
    each other's lists. See ``omnicraft/server/presence.py`` and
...
class ElicitationRequestParams(BaseModel):
    ...
    ``omnicraft/runtime/policies/approval.py:175``.
    ...
    :param phase: OmniCraft policy-engine phase the elicitation
        belongs to, e.g. ``"pre_tool_use"``.
    :param policy_name: OmniCraft policy that triggered the
    ...
    # ``elicitation/create`` traversing harness → OmniCraft → client)
...
class ElicitationRequestEvent(_SSEEventBase):
    ...
    Emitted by OmniCraft (or, under the new contract, by a harness)
    ...
    ``type == "approval"`` and
    :class:`omnicraft.server.schemas.ElicitationResult` fields in
    ...
    :param method: MCP method literal — always
        ...
        ``omnicraft/runtime/policies/approval.py``).
...
class ElicitationResolvedEvent(_SSEEventBase):
    ...
    Idempotent on the consumer side: the OmniCraft server's index
...
class PolicyDeniedEvent(_SSEEventBase):
    ...
    prompt through OmniCraft's policy engine via the vendor command-hook
...
class CreatedEvent(_SSEEventBase):
    ...
    :class:`omnicraft.server.schemas.ResponseObject` (status will
...
class CompletedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`omnicraft.server.schemas.ResponseObject`.
...
class FailedEvent(_SSEEventBase):
    ...
    Carries the final
    :class:`omnicraft.server.schemas.ResponseObject` whose
...
class RetryEvent(_SSEEventBase):
    ...
    Emitted by ``omnicraft/runtime/llm_retry.py`` (LLM calls)
    and ``omnicraft/runtime/tool_retry.py`` (tool calls) before
...
class ErrorEvent(_SSEEventBase):
    ...
    ``omnicraft/runtime/workflow.py`` — terminal LLM failures
...
class CompactionInProgressEvent(_SSEEventBase):
    ...
    Emitted by ``omnicraft/runtime/compaction.py`` while a
...
class CompactionFailedEvent(_SSEEventBase):
    ...
    Emitted by ``omnicraft/server/routes/sessions.py`` when
...
class ClientTaskCancelEvent(_SSEEventBase):
    ...
    Emitted by ``omnicraft/runtime/workflow.py`` when a parent
...
def is_known_event(name: str) -> bool:
    ...
    patch :func:`omnicraft.runtime.session_stream.publish` to
...
# ``type`` on the raw frame, see ``omnicraft/runner/app.py`` proxy_stream)
...
class PolicyEvaluationRequestEvent(_SSEEventBase):
    ...
    policies on the OmniCraft server. The runner intercepts this event in
    ``proxy_stream``, calls the OmniCraft server's
    ...
    :param data: Event data dict passed to the OmniCraft server's
