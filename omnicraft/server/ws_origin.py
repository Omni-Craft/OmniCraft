"""WebSocket ``Origin`` enforcement for the OmniCraft server.
...
OmniCraft server and drive the agent, read session updates, or attach to a
...
- :data:`OMNICRAFT_INTERNAL_WS_ORIGIN` — the sentinel ``Origin`` the
...
from omnicraft.server.auth import local_single_user_enabled
...
# ``omnicraft.runner.identity`` and re-exported here for the server-side
# policy. See :data:`OMNICRAFT_INTERNAL_WS_ORIGIN` there for rationale.
__all__ = [
...
_ALLOWED_ORIGINS_ENV = "OMNICRAFT_WS_ALLOWED_ORIGINS"
...
def parse_allowed_origins() -> frozenset[str]:
    ...
    Reads ``OMNICRAFT_WS_ALLOWED_ORIGINS`` (comma-separated). Whitespace
...
def origin_allowed(
    ...
    *,
    ...
    - The first-party sentinel (:data:`OMNICRAFT_INTERNAL_WS_ORIGIN`) and
    ...
    :param local_mode: Whether the server is a single-user local runtime
        (``OMNICRAFT_LOCAL_SINGLE_USER`` truthy).
    ...
    """
    if origin == OMNICRAFT_INTERNAL_WS_ORIGIN:
        return True
