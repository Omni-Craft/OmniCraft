...
# OmniCraft compat: imported surgically from a dedicated module so
# the integration's tech debt is removable in one shot. See
# omnicraft/spec/_omnicraft_compat.py.
from omnicraft.spec._omnicraft_compat import (
    diagnose_yaml_rejection,
    is_omnicraft_yaml,
    load_omnicraft_yaml,
)
from omnicraft.spec.parser import expand_env_vars, parse, parse_default_policies, parse_server_llm
from omnicraft.spec.tar_utils import ExtractionError, extract_safe
...
from omnicraft.spec.validator import ValidationResult, validate
...
def materialize_bundle(source: Path, dest: Path) -> Path:
    ...
    (``config.yaml`` + bundled assets) or a standalone omnicraft
    YAML file. Downstream code (``_preregister_agent``,
    ``_prepare_omnicraft_yaml_bundle``) always wants to operate on a
    ...
    :returns: *dest*, always as a populated directory. For the
        ...
        *source*. For the file case the YAML is placed at the root
        of *dest* so
        :func:`omnicraft.spec._find_omnicraft_yaml_in_dir` picks
...
def _find_omnicraft_yaml_in_dir(root: Path) -> Path | None:
    """
    Return the omnicraft YAML inside *root* when the directory is
    a single-file omnicraft bundle, or ``None`` otherwise.

    A directory qualifies as a single-file omnicraft bundle when
    there is no ``config.yaml`` at the root and exactly one file
    at the root is recognised by :func:`is_omnicraft_yaml`. Any
    other shape (``config.yaml`` present, zero or multiple omnicraft
    YAMLs, YAMLs in subdirectories) returns ``None`` and the caller
    falls through to the standard omnicraft directory-parse path.

    :param root: Path to an extracted bundle directory.
    :returns: The matched omnicraft YAML path, or ``None`` if the
        directory is not a single-file omnicraft bundle.
    """
    if (root / "config.yaml").exists():
        return None
    matches = [p for p in root.iterdir() if p.is_file() and is_omnicraft_yaml(p)]
    if len(matches) == 1:
        return matches[0]
    return None
...
def load(
    ...
    *,
    ...
    If *source* is a file path (tarball or omnicraft YAML) or raw
    bytes, dispatch accordingly.

    :param source: Path to an agent image directory, ``.tar.gz``
        bundle, omnicraft single-file YAML, or raw tarball bytes
    ...
    :param expand_env: Whether to expand ``${VAR}`` references in
        ...
        (local ``omnicraft run``, ``--agent`` preregistration). It
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        :func:`omnicraft.server.bundles.validate_agent_bundle`. It is
        applied for both bundle shapes: the omnicraft single-file YAML
        ...
        so trusted spec loading (local ``omnicraft run``, operator
    ...
    :param prune_invalid_sub_agents: When ``True``, a sub-agent that
        ...
        **execution** paths (runner spec resolution, server
        :class:`~omnicraft.runtime.agent_cache.AgentCache`): a bundle
        ...
        (``omnicraft run``,
        :func:`omnicraft.server.bundles.validate_agent_bundle`) stay
        strict and surface real authoring mistakes to the author.
    :returns: A validated :class:`AgentSpec`.
    :raises OmniCraftError: If the spec fails validation, if a policy
    ...
    if isinstance(source, bytes):
        ...
        # OmniCraft single-file YAML dispatch — see
        # omnicraft.spec._omnicraft_compat. Tech-debt aside;
        # remove this branch when omnicraft compat ends.
        if is_omnicraft_yaml(source):
            return load_omnicraft_yaml(
                source,
                enforce_handler_allowlist=enforce_handler_allowlist,
                prune_invalid_sub_agents=prune_invalid_sub_agents,
            )
        if source.suffix.lower() in {".yaml", ".yml"}:
            # The path is a YAML file but failed the omnicraft check
            ...
            raise OmniCraftError(
                f"{source}: not a valid omnicraft YAML — {reason}",
                code=ErrorCode.INVALID_INPUT,
            )
        if dest is None:
            raise OmniCraftError(
    ...
    # OmniCraft single-file YAML dispatch (extracted-bundle variant)
    # — when a bundle (directory, tarball, or raw bytes) resolves to
    # a root that contains exactly one omnicraft YAML and no
    # ``config.yaml``, route to the omnicraft adapter. This is the
    # shape produced by ``omnicraft server --agent`` when it wraps a
    # YAML into a single-file tarball for the agent store. See
    # omnicraft/spec/_omnicraft_compat.py for the tech-debt
    # ownership; remove when omnicraft compat ends.
    # The omnicraft single-file path (load_omnicraft_yaml) copies
    ...
    candidate = _find_omnicraft_yaml_in_dir(root)
    ...
    if enforce_handler_allowlist:
        ...
        # The single-file omnicraft YAML path is guarded earlier, inside
    ...
    *spec* in place: the failing sub-agent is removed from
    ...
    for sa in spec.sub_agents:
        ...
        _logger.warning(
            ...
            "produced by a newer OmniCraft server with a feature this client does "
...
def _reject_unregistered_spec_policy_handlers(spec: AgentSpec) -> None:
    ...
    :class:`~omnicraft.spec.types.FunctionPolicySpec` entries whose
    ...
    :raises OmniCraftError: If a function policy names an unregistered
