"""
Compatibility shim: legacy omnicraft ``(content, phase)`` policy
callables → omnicraft ``(ctx, context)`` convention.

Legacy omnicraft function policies were written like::
...
callable runs under omnicraft it silently short-circuits to
``"allow"`` on the very first phase check (``phase`` is a dict
there, not a string) and any policy written in the old style
becomes a no-op.

This module bridges the gap. The omnicraft→omnicraft
translator in :mod:`omnicraft.spec.omnicraft` routes every
...
every call. Modern callables (omnicraft-native ``(ctx)`` /
...
from omnicraft.spec.types import Phase

# Parameter names that identify a legacy omnicraft policy
# callable — the first two positional parameters must be exactly
# ``content`` then ``phase``. A callable whose first two params
# are ``(foo, bar)`` is treated as omnicraft-native even though
...
# Maps OmniCraft' :class:`Phase` enum to the string literal
...
def build(
    ...
    loading an omnicraft-sourced function policy.

    Imports *target*, optionally unwraps a factory call with
    *factory_kwargs*, then decides whether the resolved callable
    is a legacy omnicraft policy (parameter names
    ``(content, phase)``) or an omnicraft-native one
    ...
    :param configured_phases: The YAML ``on:`` list (verbatim
        ...
        :class:`omnicraft.inner.policies.FunctionPolicy`
        ...
        (:func:`omnicraft.spec.omnicraft._translate_function_policy_yaml`)
        always passes a list.
    :returns: A callable matching OmniCraft'
...
def _coerce_legacy_return(result: Any) -> Any:
    ...
    by :func:`omnicraft.policies.function._coerce_to_policy_result`.
...
def _wrap_legacy(
    ...
    *,
    ...
    wrapper when present, so the omnicraft ``FunctionPolicy``
    can call it at turn boundaries — preserving the legacy
    omnicraft per-turn-reset contract for stateful policies
    ...
    :param configured_phases: When set, every wrapper invocation
        ...
        omnicraft-native would. ``None`` means the key is
    ...
    # Capture once at wrap time. The omnicraft FunctionPolicy
...
def _v0_event_to_legacy_phase(event: dict[str, Any]) -> str:
    ...
    OmniCraft' ``Phase`` enum. Provide a direct mapping here.
...
def _convert_args(
    ...
    *,
    ...
    ``(content, phase, context)`` from OmniCraft'
    ...
    - Inner system: the omnicraft :class:`FunctionPolicy` builds an
...
def _legacy_content(ctx: EvaluationContext) -> Any:
    ...
    :func:`omnicraft.runtime.workflow._enforce_tool_call_policy`
    and siblings). The legacy omnicraft shape matches those
    normalized shapes almost exactly — this helper bridges the
    per-phase differences:

    - ``INPUT`` / ``OUTPUT``: omnicraft passes the message
      text directly; legacy omnicraft expects the same. Pass
      through.
    - ``TOOL_CALL``: omnicraft passes
      ``{"tool": name, "args": parsed_args_dict}``; legacy
      omnicraft expects the same. Pass through.
    - ``TOOL_RESULT``: omnicraft passes the raw tool output
      string. Legacy omnicraft passes the parsed result *dict*
      via
      :func:`omnicraft.inner.mcp_tools._extract_call_result_payload`
      ...
      structured payload), native omnicraft hands the policy
      ...
      JSON-parse the omnicraft string; on parse failure pass
...
def _maybe_parse_json(value: Any) -> Any:
    ...
    omnicraft-native's behavior: when an MCP text block is
...
def _legacy_context(
    ...
    *,
    ...
    Legacy omnicraft passed ``{"labels": {...},
    "configured_phases": [...], "tool_name": "sleep"}`` — but
    ``tool_name`` is *only* added on ``TOOL_RESULT`` (see
    :meth:`omnicraft.inner.session.Session._apply_tool_result_policy`,
    which builds ``context = {"tool_name": tool_name}`` for
    that phase). On ``TOOL_CALL``, native omnicraft passes no
    ...
    :class:`omnicraft.inner.policies.FunctionPolicy.evaluate`,
