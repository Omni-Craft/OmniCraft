"""Bidirectional translator between omnicraft ``AgentSpec`` and omnicraft ``AgentDef``.

Two functions live here:

- :func:`agent_spec_to_agent_def` — forward direction, consumed by
  ``OmniCraftExecutor`` at executor-construction time.
- :func:`agent_def_to_agent_spec` — reverse direction, consumed by
  :func:`omnicraft.spec.load` when handed an omnicraft YAML so
  ``omnicraft chat foo.yaml`` works transparently.

The bidirectional invariant
``agent_spec_to_agent_def(agent_def_to_agent_spec(d)) == d`` must
hold for every representative omnicraft YAML; the round-trip test
under ``tests/spec/test_omnicraft_roundtrip.py`` enforces it.

Design invariants (see designs/OMNICRAFT_INTEGRATION.md §1 and §2):
...
- Unsupported concepts **fail loud** with an :class:`OmniCraftError`
  naming the specific field. Currently unsupported (each is an
  omnicraft spec gap to fix, not a translation to paper over):
...
from omnicraft.inner.datamodel import AgentDef, OSEnvSpec, TerminalEnvSpec
from omnicraft.inner.datamodel import ExecutorSpec as OmniExecutorSpec
from omnicraft.inner.tools import (
    AgentTool,
...
from omnicraft.llms.routing import infer_harness_from_model as _infer_harness_from_model
...
# selects ``OmniCraftExecutor``. Both directions of the translator
# treat this as the discriminator between native omnicraft specs
# and omnicraft-sourced ones.
OMNICRAFT_EXECUTOR_TYPE = "omnicraft"

# Value placed in :attr:`LocalToolInfo.language` for tools that were
# sourced from an omnicraft YAML. Distinguishes them from native
# omnicraft tools (``"python"`` / ``"typescript"``) which live as
# files on disk under ``tools/python/``.
OMNICRAFT_TOOL_LANGUAGE = "omnicraft-python-callable"

# Version stamped on AgentSpecs synthesized from omnicraft YAMLs.
# OmniCraft YAMLs do not declare ``spec_version`` (it's an
# omnicraft concept), so the adapter writes ``1`` — the currently
# valid omnicraft schema version (see omnicraft/spec/validator.py).
...
# translation. We resolve eagerly because omnicraft spawns
# each child as an independent task — no live parent session
# exists at runtime to walk.
_OS_ENV_INHERIT_SENTINEL = "inherit"

# OmniCraft loader policy-type discriminators. Used to dispatch
...
def agent_spec_to_agent_def(spec: AgentSpec) -> AgentDef:
    """
    Translate an omnicraft ``AgentSpec`` into an omnicraft
    ``AgentDef`` suitable for
    :func:`omnicraft.executor_factory.create_executor`.
    ...
    :param spec: The omnicraft spec to translate. ``spec.llm``
        must be set (the omnicraft harness needs a model name);
        ``spec.executor.type`` must be ``"omnicraft"`` with
        ``executor.config.harness`` populated.
    :returns: A populated :class:`omnicraft.datamodel.AgentDef`
        ...
        rejected upstream.
    :raises OmniCraftError: If the spec uses an unsupported
    ...
    if spec.executor.model is None:
        raise OmniCraftError(
            "executor.type='omnicraft' requires a model (set executor.model)",
            code=ErrorCode.INVALID_INPUT,
        )

    # For native OmniCraft v1 specs ``executor.config`` is empty; infer the
    ...
    return AgentDef(
        ...
        tools=_translate_tools_to_omnicraft(spec),
...
def _reject_unsupported_concepts(spec: AgentSpec) -> None:
    ...
    to remove from the spec (or what omnicraft bug to file to
    close the gap).

    :param spec: The omnicraft spec to check.
    :raises OmniCraftError: On any of:
    ...
    # Policies are enforced by the omnicraft workflow layer
    # (see ``omnicraft.runtime.policies.build_policy_engine``)
    # BEFORE the harness receives a turn — the
    # :class:`OmniCraftExecutor` is invoked with tool calls
    ...
    # Sandbox declarations (omnicraft ``tools.sandbox.container_image``
    # and the omnicraft OSEnvSandboxSpec) are unsupported. Fail loud
    # if either is populated.
    if spec.tools.sandbox.container_image is not None:
        raise OmniCraftError(
            "tools.sandbox translation to omnicraft OSEnvSpec is unsupported; "
            "the adapter rejects specs with sandbox rather than silently dropping it",
            code=ErrorCode.INVALID_INPUT,
        )

    # ``cancellable_function`` is an omnicraft concept — OmniCraft'
    ...
    for tool in spec.local_tools:
        ...
        if _is_cancellable_function_path(tool.path):
            raise OmniCraftError(
                ...
                "omnicraft; cancellation support is an omnicraft spec extension",
                code=ErrorCode.INVALID_INPUT,
            )
...
def _is_cancellable_function_path(path: str) -> bool:
    ...
    explicitly in the omnicraft spec; omnicraft has no spec
...
def _translate_tools_to_omnicraft(spec: AgentSpec) -> dict[str, Tool]:
    """
    Build the ``AgentDef.tools`` dict from OmniCraft' tool model.
    ...
    :class:`omnicraft.tools.FunctionTool`. Filesystem paths
    (``"tools/python/foo.py"``) are not supported — the YAML adapter
    that produces them must encode dotted paths.

    :param spec: The omnicraft spec. ``spec.local_tools`` lists
    ...
    :returns: A ``dict[str, Tool]`` mapping tool name to
        ...
        local tools.
    :raises OmniCraftError: If a tool path is a filesystem path
    ...
    for tool_info in spec.local_tools:
        ...
        # the omnicraft tool's ``input_schema``. Re-attach it so
    ...
    # so the omnicraft harness surfaces them to the LLM via its
    # own MCP machinery (``omnicraft.inner.mcp_tools``). The
    # forward direction translated ``MCPTool`` →
    # :class:`MCPServerConfig` (stdio or http); this is the inverse.
    # ``OmniCraftExecutor.from_spec`` calls this reverse path when
    # wrapping an omnicraft spec for an omnicraft harness; if the
...
def _mcp_server_to_mcp_tool(config: MCPServerConfig) -> MCPTool:
    ...
    :returns: An :class:`MCPTool` with the equivalent transport
        shape for the inner omnicraft runtime.
    :raises OmniCraftError: If the config is missing the
...
def _sub_spec_to_agent_tool(sub: AgentSpec) -> AgentTool:
    """
    Rebuild an omnicraft :class:`AgentTool` from a nested
    omnicraft :class:`AgentSpec`.

    Inverse of :func:`_agent_tool_to_sub_spec`. Reads the sub-spec's
    ``llm.model`` and ``executor.config`` (harness / profile) to
    reconstruct the omnicraft :class:`ExecutorSpec`. Lossy fields
    (``max_sessions``, ``os_env``, ``pass_history``,
    ``pass_histories``) are left at omnicraft defaults.

    :param sub: The nested :class:`AgentSpec` representing a
        sub-agent exposed to the parent as a tool.
    :returns: A populated :class:`omnicraft.tools.AgentTool`.
    """
    # Infer harness from model prefix when executor.config is empty
    # (native OmniCraft v1 specs). Also forward os_env so sub-sessions get
...
def _resolve_dotted_attr(dotted_path: str, tool_name: str) -> Any:
    ...
    :returns: The resolved attribute object (callable or
        runner or other).
    :raises OmniCraftError: If the path is not a dotted import
    ...
    if "/" in dotted_path or dotted_path.endswith(".py"):
        raise OmniCraftError(
            ...
            "omnicraft translator requires dotted import paths like "
            "'examples._shared.tool_functions.get_current_time'",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    try:
        ...
        # single OmniCraftError so the spec author sees a
...
def _resolve_dotted_callable(
    ...
    :raises OmniCraftError: If the module can't be imported,
...
# ── Policy translation (omnicraft YAML dict → omnicraft
# GuardrailsSpec) ────────────────────────────────────────────────
#
...
# parser (``omnicraft.spec.parser._parse_guardrails``) is invoked
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    omnicraft YAML into an omnicraft :class:`GuardrailsSpec`.

    OmniCraft declares labels/policies at the top level
    (``labels:``, ``label_schema:``, ``policies:``,
    ``ask_timeout:``); omnicraft groups them under a single
    ``guardrails:`` block. This helper transforms the omnicraft
    shape into the omnicraft dict shape and delegates to
    :func:`omnicraft.spec.parser._parse_guardrails` for
    ...
    :param raw_policies: OmniCraft YAML ``policies:`` mapping,
    ...
    :param raw_labels: OmniCraft YAML top-level ``labels:`` map
        ...
        ``initial: None``.
    :param raw_label_schema: OmniCraft YAML top-level
        ``label_schema:`` map of
        ``{label_key: {values: [...], monotonic: "max"|"min"|"none"}}``.
    :param raw_ask_timeout: OmniCraft YAML top-level
        ``ask_timeout:`` (seconds), e.g. ``30``. Passed through
        to ``guardrails.ask_timeout``. ``None`` means use the
        omnicraft default (see
        :data:`omnicraft.spec.types.DEFAULT_ASK_TIMEOUT`).
    ...
    :returns: A validated :class:`GuardrailsSpec`, or ``None``
        ...
        source YAML.
    :raises OmniCraftError: On malformed entries — same error
        shape the omnicraft YAML parser produces for a native
        ``guardrails:`` block, so YAML authors see consistent
        error messages whether their spec came from omnicraft
        or omnicraft.
    """
    if not raw_policies and not raw_labels and not raw_label_schema and raw_ask_timeout is None:
        return None
    # Build the omnicraft-shaped guardrails dict, then hand it to
    ...
    from omnicraft.spec.parser import _parse_guardrails
    ...
    # ``expand_env`` is False because the upstream omnicraft
...
def _translate_labels_yaml(
    ...
    """
    Merge omnicraft' separate ``labels:`` (initial values) and
    ``label_schema:`` (schemas) into OmniCraft' unified
...
def _translate_policies_yaml(
    ...
    *,
    ...
    """
    Translate the omnicraft ``policies:`` mapping entry-by-entry
    into the omnicraft shape.

    :param raw_policies: Raw ``policies:`` map from the omnicraft
...
def _translate_policy_entry_yaml(
    ...
    *,
    ...
    :raises OmniCraftError: When the entry declares a policy
    ...
    if policy_type not in _KNOWN_POLICY_TYPES:
        raise OmniCraftError(
            f"omnicraft policy {policy_name!r}: unknown type "
...
def _translate_function_policy_yaml(
    ...
    """
    Translate an omnicraft ``type: function`` policy to the
    omnicraft shape.
    ...
    - Other fields (``condition``, ``action``, ``set_labels``,
      ...
      longer accept ``on:`` on the omnicraft side; the handler
    ...
    # omnicraft parser side. ``on:`` is intentionally omitted —
    ...
    # If ``function:`` is already present in omnicraft native
    ...
    if callable_path is None:
        # Let the omnicraft parser surface the "function:
        # required" error — it already does so with a clear
        # message referencing the policy name.
        return out
    # Route every omnicraft-sourced function policy through the
    # legacy-compat shim so author callables written with the
    # omnicraft ``(content, phase)`` / ``(content, phase, context)``
    # convention keep working under OmniCraft'
    # ``(ctx, context)`` convention. For omnicraft-native
    # callables the shim is a pass-through (detected by parameter
    # names at policy-build time; see _omnicraft_legacy_shim).
    ...
    out["function"] = {
        "path": "omnicraft.spec._omnicraft_legacy_shim.build",
        "arguments": shim_args,
    }
    return out
...
def _resolve_profile_to_connection(profile: str) -> dict[str, str] | None:
    ...
    Used at spec-translation time to make omnicraft LLMConfig
    self-contained — OmniCraft' LLM adapters take credentials
    via explicit ``connection:`` fields (per the "spec
    self-containment" design principle) and do not read env vars.
    Meanwhile omnicraft declares Databricks creds via the
    ...
    """
    # Import locally to avoid a top-level dependency on omnicraft
    # runtime details from the spec module — only this single
    # translator path needs it.
    from omnicraft.inner.databricks_executor import _read_databrickscfg
...
def _translate_prompt_policy_yaml(
    ...
    *,
    ...
    """
    Translate an omnicraft ``type: prompt`` policy to the
    omnicraft shape.

    Field mapping:

    - ``executor: {model: X}`` → ``llm: {model: X}``. The
      omnicraft prompt policy uses ``executor`` to override
      the classifier LLM; omnicraft uses ``llm``.
    - When *parent_profile* is set AND the policy has no
      ...
      omnicraft classifier actually reaches the Databricks
    ...
    :param parent_profile: Databricks profile from the parent
        ...
        omnicraft LLMConfig so the classifier call at runtime
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    """
    Translate an omnicraft :class:`AgentDef` into an omnicraft
    :class:`AgentSpec`.

    Reverse direction of :func:`agent_spec_to_agent_def`. Used by
    :func:`omnicraft.spec.load` when handed an omnicraft YAML
    so ``omnicraft chat foo.yaml`` routes the spec through
    ``OmniCraftExecutor`` at runtime.

    The produced spec sets ``executor.type = "omnicraft"`` so the
    runtime picks ``OmniCraftExecutor``, with ``harness`` and
    ``profile`` carried in ``executor.config``.

    :param agent_def: Parsed omnicraft agent definition,
        typically from :func:`omnicraft.loader.load_agent_def`.
    ...
    :param raw_yaml: Optional raw YAML dict the ``agent_def`` was
        ...
        OmniCraft' ``guardrails:`` block — the omnicraft
        ...
        (``load_omnicraft_yaml``) read the raw YAML themselves
        and pass it through.
    ...
    :raises OmniCraftError: When *agent_def* uses an omnicraft
        concept OmniCraft' :class:`AgentSpec` cannot currently
    ...
    # ``omnicraft.inner.loader.load_agent_def``) wins over ``prompt:``
    ...
    # omnicraft gets this "for free" via Session → factory; the
    # omnicraft spawn path runs each sub-agent as an
    ...
    if executor_spec.type == "omnicraft":
    ...
    # translation time — the OmniCraft path spawns each child as an
    ...
    # Split omnicraft tools by subtype:
    #   FunctionTool / CancellableFunctionTool
    #       → AgentSpec.local_tools (omnicraft treats every
    ...
    #         OmniCraft' ToolManager, srt-wrapped when
    #         available — see omnicraft/tools/mcp.py).
    ...
    # raw YAML. Policies/labels are lifted from the omnicraft
    # top-level YAML fields into OmniCraft' ``guardrails:``
    # block; enforcement then happens in the omnicraft workflow
    # (outside the omnicraft harness) via the standard
    ...
    if raw_yaml is not None:
        guardrails = _translate_guardrails_yaml(
            ...
            # Baked-in at translation time so OmniCraft'
            # classifier LLM calls honor Databricks routing
            # (omnicraft adapters don't read env vars per the
            # spec-self-containment design principle).
            parent_profile=parent_profile,
        )

    # Per designs/OMNICRAFT_TERMINAL_BRIDGE.md §6.1 step 2 + §3 non-goal
    ...
    # ``terminals`` field in the omnicraft grammar — matches legacy
    # non-AP mode exactly.
    terminals = dict(agent_def.terminals) if agent_def.terminals else None

    # Top-level ``skills:`` from the raw YAML (the omnicraft
    # loader doesn't carry this field on AgentDef — it's
    # omnicraft spec config that controls Claude SDK harness
...
def _translate_skills_filter_from_yaml(
    ...
    omnicraft-format YAML mapping and validate the same shapes
    the omnicraft spec parser accepts (``"all"`` / ``"none"`` /
    list of names).

    The omnicraft loader (``inner.loader.load_agent_def``) does
    NOT read this field — it's omnicraft spec config that
    controls the Claude SDK harness's host-skill loading, not an
    omnicraft runtime concept. So when the YAML is consumed via
    ...
    :raises OmniCraftError: When the value isn't one of the
...
def _self_agent_tool_to_sub_spec(
    ...
    *,
    ...
    the OmniCraft runtime spawns when the parent's LLM dispatches
...
def _agent_tool_to_sub_spec(
    ...
    *,
    ...
    """
    Translate an omnicraft inline :class:`AgentTool` (sub-agent
    exposed as a tool) into a nested omnicraft :class:`AgentSpec`.
    ...
    :class:`AgentSpec` with ``executor.type == "omnicraft"`` so the
    :class:`OmniCraftExecutor` runs it when spawned.

    Lossy fields (not modeled on OmniCraft' AgentSpec yet):
    ``max_sessions``, ``pass_history``, ``pass_histories``.
    omnicraft' runtime falls back to its defaults for these on
    the reverse trip.

    :param tool_name: The YAML key under which this AgentTool is
        declared on the parent, e.g. ``"claude_worker"``.
    :param tool: The parsed omnicraft :class:`AgentTool`.
    :param parent_profile: Databricks profile carried on the
        ...
        empty field does.
    :param parent_harness: OmniCraft harness carried on the
        ...
        omnicraft resolves this at ``create_executor`` time;
        omnicraft has to do it at spec-translation time because
    ...
    :param parent_os_env: OS environment on the parent agent.
        ...
        inline AgentTool — legacy omnicraft resolves it from the
        live Session at runtime, but omnicraft runs each child
    ...
    :param raw_executor: The raw ``executor:`` dict for this inline
        ...
        omnicraft' dataclass parsing dropped unknown keys). When set,
        fields the omnicraft :class:`~omnicraft.inner.datamodel.ExecutorSpec`
    ...
    # (see ``omnicraft/tools/manager.py:426`` — "if not
...
def _resolve_inline_agent_tool_os_env(
    ...
    OmniCraft' :class:`AgentTool.os_env` is
    ...
    if isinstance(tool_os_env, str):
        ...
        # strings would be an omnicraft YAML that the loader
...
def _fail_on_unsupported_concepts_def(agent_def: AgentDef) -> None:
    """
    Raise :class:`OmniCraftError` for every omnicraft concept
    OmniCraft' :class:`AgentSpec` cannot currently represent.

    Each unsupported concept gets its own clear error message
    naming the specific field.

    :param agent_def: Parsed omnicraft agent definition.
    :raises OmniCraftError: On the first unsupported concept
...
def _fail_on_unsupported_tool(
    ...
    """
    Raise :class:`OmniCraftError` when *tool* uses an omnicraft
    tool concept omnicraft cannot currently represent.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"glean_search"``.
    :param tool: The parsed omnicraft tool instance.
    :raises OmniCraftError: On MCP tools or cancellable_function
        tools.
    """
    ...
    if isinstance(tool, MCPTool):
        ...
        # Databricks-managed MCP endpoint that OmniCraft'
        ...
        if tool.databricks_server is not None:
            raise OmniCraftError(
                f"omnicraft agent {agent_name!r} declares tool "
                f"{tool_name!r} of type `mcp` with "
                f"``databricks_server={tool.databricks_server!r}`` — "
                f"OmniCraft' MCPServerConfig doesn't understand the "
...
def _translate_name_from_def(raw_name: str | None) -> str | None:
    """
    Translate omnicraft ``AgentDef.name`` to :attr:`AgentSpec.name`.

    ``AgentDef.name`` is ``str | None`` on the omnicraft side;
    omnicraft also uses ``None`` for absence. An empty string on
    ...
    :param raw_name: The raw ``name`` field from
        ...
        ``None``.
    :returns: The omnicraft name, or ``None`` when *raw_name*
        is absent or empty.
    """
...
def _translate_prompt_from_def(raw_prompt: str | None) -> str | None:
    """
    Translate omnicraft ``AgentDef.prompt`` to
    :attr:`AgentSpec.instructions`.

    ``AgentDef.prompt`` is ``str | None`` on the omnicraft side;
    omnicraft also uses ``None`` for absence.

    :param raw_prompt: The raw ``prompt`` field from
        :class:`AgentDef`, e.g. ``"You are a friendly
        assistant."``, ``""``, or ``None``.
    :returns: The omnicraft instructions text, or ``None``
...
def _translate_llm_from_def(
    ...
    *,
    ...
    """
    Translate omnicraft ``executor.model`` into an omnicraft
    :class:`LLMConfig`.

    omnicraft stores the model string on its own
    :class:`omnicraft.datamodel.ExecutorSpec`. omnicraft splits
    ...
    ``type = "omnicraft"`` plus the ``harness`` / ``profile``
    config.

    :param oa_executor: The omnicraft
        :class:`~omnicraft.datamodel.ExecutorSpec`, or ``None``
        when the YAML omitted the ``executor:`` block. Example:
        ``OmniExecutorSpec(model="databricks-claude-sonnet-4")``.
    :param raw_executor: Optional raw omnicraft YAML ``executor:``
        ...
        through to :class:`omnicraft.executor.ExecutorConfig.extra`.
        The omnicraft loader drops unknown fields on
        ``ExecutorSpec`` at parse time, so we read them back from
        the raw dict here.
    :returns: An :class:`LLMConfig` with ``model`` populated, or
        ``None`` when omnicraft declared no model.
    """
...
def _translate_executor_from_def(
    ...
    *,
    ...
    """
    Build the omnicraft :class:`ExecutorSpec` for an omnicraft
    agent.

    Returns ``type="omnicraft"`` when a harness can be resolved,
    routing the agent through ``OmniCraftExecutor``. Raises
    :class:`OmniCraftError` when a model is declared but no
    ...
    The ``harness`` and ``profile`` fields from omnicraft land in
    ``executor.config`` so :meth:`OmniCraftExecutor.from_spec` can
    ...
    3. :func:`_infer_harness_from_model` on the resolved model
       string — mirrors pure omnicraft' CLI auto-pick
       (``databricks-claude-*`` → ``claude-sdk``, etc.) so YAMLs
       that declare only a model continue to work under OmniCraft mode.
    ...
    Empty-string coercion mirrors omnicraft' own dataclass
    defaults (``ExecutorSpec.harness: str = ""``,
    ``ExecutorSpec.profile: str = ""``) so the round-trip is
    bit-exact against the omnicraft side.

    :param oa_executor: The omnicraft ``ExecutorSpec`` or
        ``None``.
    ...
    :param raw_executor: Optional raw YAML ``executor:`` mapping.
        ...
        API surface (chat/completions vs. responses). The omnicraft
        loader silently drops unknown fields on its own
        :class:`~omnicraft.inner.datamodel.ExecutorSpec`, so we
        have to recover this field from the raw dict here.
    :returns: An :class:`ExecutorSpec` with ``type="omnicraft"``
        when a harness is known.
    :raises OmniCraftError: When a model is declared but no
        harness can be inferred from it.
    """
    # OmniCraft' :class:`ExecutorSpec.{harness,profile,model}`
    ...
    # ``use_responses`` is not a field on the omnicraft inner
    ...
    if oa_executor is not None and oa_executor.auth is not None:
        ...
        from omnicraft.spec.parser import _parse_executor_auth
...
def _translate_mcp_tool_from_def(
    ...
    """
    Translate one omnicraft :class:`MCPTool` into a native
    ...
    - ``tool.command`` populated → ``transport="stdio"`` with
      ...
      subprocess spawns unsandboxed (matching legacy omnicraft,
    ...
    :param tool: The parsed omnicraft :class:`MCPTool` instance.
    :returns: A fully populated :class:`MCPServerConfig`.
    :raises OmniCraftError: If *tool* has neither ``url`` nor
    ...
    raise OmniCraftError(
        f"omnicraft MCP tool {tool_name!r} has neither 'url' nor "
...
def _translate_function_tool_from_def(
    ...
    """
    Translate one omnicraft function tool into a
    :class:`LocalToolInfo`.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"get_current_time"``.
    :param tool: The parsed omnicraft tool. Must be a
    ...
    :returns: A :class:`LocalToolInfo` with ``name``, ``path``
        ...
        :data:`OMNICRAFT_TOOL_LANGUAGE`.
    :raises OmniCraftError: When *tool* is not a
        :class:`FunctionTool` (defensive guard).
    """
    if isinstance(tool, CancellableFunctionTool):
        raise OmniCraftError(
            f"omnicraft tool {tool_name!r}: "
    ...
    if not isinstance(tool, FunctionTool):
        raise OmniCraftError(
            f"omnicraft tool {tool_name!r}: expected FunctionTool, got {type(tool).__name__}.",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    if tool.runtime == "client":
        return LocalToolInfo(
            name=tool_name,
            path=None,
            language=OMNICRAFT_TOOL_LANGUAGE,
    ...
    # for typed-parameter tools. Without this fallback, omnicraft
...
def _recover_callable_path(
    ...
    omnicraft' loader resolves the YAML's ``callable:`` field
    ...
    :returns: The dotted module path, e.g.
        ``"examples._shared.tool_functions.get_current_time"``.
    :raises OmniCraftError: When the callable is missing
    ...
    raise OmniCraftError(
        f"omnicraft tool {tool_name!r}: callable {callable_obj!r} "
