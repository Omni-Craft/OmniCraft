...
if TYPE_CHECKING:
    ...
    # omnicraft.policies.types); PhaseSelector.matches takes
...
class ProviderAuth:
    ...
    # Required: provider name from ~/.omnicraft/config.yaml providers:.
    name: str
...
class ExecutorSpec:  # type: ignore[explicit-any]  # config: dict[str, Any] field (see below)
    ...
    :param type: Executor type. ``"omnicraft"`` (default),
    ...
    :param profile: The Databricks workspace profile name from
        ...
        omnicraft-compat sunset this is lifted from raw YAML's
        ``executor.profile`` in the omnicraft path too. ``None``
    ...
    :param config: Executor-type-specific configuration. For
        ``type == "omnicraft"`` this carries ``"harness"`` (e.g.
        ``"claude-sdk"`` or ``"codex"``), optional ``"profile"``
        (e.g. ``"<your-profile>"``), and optional ``"os_env"`` — the
        latter is a nested mapping mirroring the omnicraft
        ...
        🚨 **TECH DEBT — REMOVE WHEN OMNICRAFT COMPAT ENDS.**
        This field exists *solely* to carry harness / profile /
        os_env data for the OmniCraft integration (see
        ``designs/OMNICRAFT_INTEGRATION.md``). A free-form
        ``dict[str, Any]`` on a spec dataclass is the kind of
        bag-of-values escape hatch we'd otherwise reject in
        review — acceptable here only because the OmniCraft
        executor is a temporary bridge with explicit sunset
        criteria. Once OmniCraft is consolidated (phase 6 of the
    ...
    :param auth: Explicit LLM authentication configuration. When set,
        ...
        ``~/.omnicraft/config.yaml``). ``None`` means fall back to
    ...
    # During the omnicraft-compat sunset, lifted from raw YAML's
    # executor.profile in the omnicraft path too. None = resolve
    # via env vars / DEFAULT section. See class docstring.
    # DEPRECATED: use executor.auth: {type: databricks, profile: <name>} instead.
    profile: str | None = None
    # TECH DEBT (omnicraft compat only — see class docstring).
    # Remove when OmniCraft consolidation lands; do NOT extend.
    ...
    def harness_kind(self) -> str:
        ...
        For ``type == "omnicraft"`` the kind lives in
...
class MCPServerConfig:
    ...
    - ``"stdio"`` — local subprocess, spawned via
      ...
      legacy inner stack at ``omnicraft/inner/mcp_tools.py``
      ...
      sandboxing through the ``omnicraft/environments/``
...
# PolicyEngine live under omnicraft.runtime.policies in later
# phases.
...
class AgentSpec:  # type: ignore[explicit-any]  # params: dict[str, Any] field (see below)
    ...
    :param async_enabled: Whether the LLM-callable async-dispatch
        ...
        ``omnicraft/inner/datamodel.py::AgentDef.async_enabled``
        — the same YAML must produce the same tool surface
        whether the user opts into OmniCraft mode or runs the legacy
    ...
    :param os_env: The agent's OS environment, e.g.
        ...
        :mod:`omnicraft.inner.bwrap_sandbox`), selected when the
        ...
        Native OmniCraft YAML accepts this as a top-level ``os_env:``
        mapping; the omnicraft-compat translator copies
        ``AgentDef.os_env`` into this field directly. The
        ``"inherit"`` string sentinel from omnicraft inline
    ...
    :param terminals: Declared interactive tmux terminals
        ...
        Populated by the omnicraft-compat translator from
        ``AgentDef.terminals``. Native OmniCraft YAML support is
        deferred — see ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §3.
    :param timers: Whether the LLM-callable timer builtins
        ...
        ``omnicraft/inner/datamodel.py::AgentDef.timers`` —
        ...
        stack uses in-memory asyncio tasks; OmniCraft supersedes that
