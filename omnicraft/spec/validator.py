...
from omnicraft.spec.types import AgentSpec, ToolRuntime
...
# ``omnicraft.server.routes.sessions``.
...
# omnicraft/spec/_omnicraft_compat.py. Placed after the module's
...
from omnicraft.spec._omnicraft_compat import (  # noqa: E402
    OMNICRAFT_EXECUTOR_TYPE,
    validate_omnicraft_executor,
)
...
def _validate_executor_type(
    ...
    elif etype == OMNICRAFT_EXECUTOR_TYPE:
        validate_omnicraft_executor(spec, result)
...
def _validate_local_tools(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    # spec load (the validator runs from ``omnicraft.spec``,
    # which is a lower layer than ``omnicraft.tools``).
    from omnicraft.tools.builtins import BUILTIN_NAMES
...
# allow-list in ``omnicraft/inner/loader.py``. ``none`` is excluded
...
def _validate_os_env(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    The OmniCraft parser already enforces these on the YAML-loaded path,
    but ``AgentSpec`` may also be constructed programmatically —
    by tests, by the OmniCraft compat shim, or by future API
