...
FORK_SOURCE_LABEL_KEY = "omnicraft.fork.source_id"
...
# (see ``omnicraft.claude_native._clone_claude_transcript`` and the
# fork-resume branch in ``omnicraft.runner.app``), so the clone opens with
...
FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY = "omnicraft.fork.source_external_session_id"
...
# CLI ignores the OmniCraft transcript, so the runner rebuilds the target's
...
# the native transcript from the fork's copied OmniCraft items
# (``_ensure_local_claude_resume_transcript`` /
# ``_ensure_local_codex_resume_rollout`` — the converters consume OmniCraft's
...
FORK_CARRY_HISTORY_LABEL_KEY = "omnicraft.fork.carry_history"
...
SWITCH_PREVIOUS_BUILTIN_LABEL_KEY = "omnicraft.switch.previous_builtin_id"
...
# ``omnicraft.runner.app._codex_native_launch_config`` and
...
CODEX_NATIVE_BYPASS_SANDBOX_LABEL_KEY = "omnicraft.codex_native.bypass_sandbox"
...
_INSTANCE_SCOPED_LABEL_KEYS = frozenset(
    {
        ...
        "omnicraft.codex_native.bridge_id",
        "omnicraft.last_context_tokens",
        "omnicraft.last_context_window",
...
class SessionConnectivity:
    ...
    :param needs_workspace: ``True`` when this is an unbound fork of a
        ...
        ``omnicraft.fork.source_id`` label is set). Forces the online
    ...
    def list_conversations(
        ...
        :param agent_id: When set, only return conversations
            ...
            *conversation* columns (``created_at`` /
            ...
            OmniCraft mode ``--continue`` flag (resume the
            most-recent conversation for the agent that
            *this YAML* registers as) — see
            ``designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md``. ``None``
            disables the filter.
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param carry_history_into_native: When ``True``, stamp
            ...
            native transcript, or build from the copied OmniCraft items) instead
        ...
        :param resume_source_native_session: When ``True`` (default), a
            ...
            native transcript from the copied OmniCraft items instead.
        :param presentation_labels: When not ``None``, replace the source's
            harness-presentation labels (``omnicraft.ui`` /
            ``omnicraft.wrapper``) on the clone with these. Used when the
    ...
    def switch_conversation_agent(
        ...
        *,
        ...
        :param presentation_labels: Replace the session's
            ``omnicraft.ui`` / ``omnicraft.wrapper`` labels with these so
