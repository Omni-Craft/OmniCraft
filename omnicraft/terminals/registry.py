...
Per ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.2, this rewrite swaps to:
...
- No idle reaper: terminals are explicit-launch only and the LLM is
  expected to ``sys_terminal_close`` when done. OmniCraft shutdown still
  closes everything; per-conversation cleanup runs from the workflow's
  finally block.

The registry is constructed once at OmniCraft startup
(``omnicraft.runtime._globals.init``) and accessed via
``omnicraft.runtime.get_terminal_registry()`` from tools and the
workflow.
...
from omnicraft.inner.terminal import TerminalInstance, create_terminal_instance

logger = logging.getLogger(__name__)

# Per-instance close timeout. Bounds the cleanup window so a wedged
# tmux session can't block OmniCraft shutdown or workflow finalization
...
def conversation_link_for_id(
    ...
    *,
    ...
    :param conversation_id: Conversation/session id, e.g.
        ``"conv_abc123"``.
    :param base_url: Optional OmniCraft server base URL, e.g.
    ...
    # API→UI mount swap (``/api/2.0/omnicraft`` → ``/omnicraft``) and the
...
class TerminalRegistry:
    ...
    def __init__(self, *, conversation_link_base_url: str | None = None) -> None:
        ...
        :param conversation_link_base_url: Optional OmniCraft server base URL
        ...
        # See ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §9.1.
    ...
    def native_panes(self) -> list[tuple[str, str, Path]]:
        ...
        (:mod:`omnicraft.terminals.pane_reaper`); the reaper's wiring additionally
        ...
        """
        from omnicraft.terminals.pane_reaper import NATIVE_PANE_TERMINAL_NAMES
    ...
    def transfer(
        ...
        process and tmux pane keep running, but the OmniCraft session that owns
    ...
    async def cleanup_conversation(self, conversation_id: str) -> None:
        ...
        Per ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.4, this is **not**
    ...
    async def shutdown(self) -> None:
        ...
        a stuck instance shouldn't block the rest of OmniCraft shutdown.
        """
        with self._lock:
