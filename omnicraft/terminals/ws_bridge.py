...
Lives under :mod:`omnicraft.terminals` (alongside the
...
- :mod:`omnicraft.server.routes.terminal_attach`, when no runner WS
  factory is configured (in-process / test setups, where the tmux
  socket lives on the same filesystem the server can see).
- :mod:`omnicraft.runner.app`, where the runner exposes its own WS
...
# breaks headless hosts (managed sandboxes, ``omnicraft sandbox
...
async def _tmux_session_alive(socket_path: str, tmux_target: str) -> bool:
    ...
    :param socket_path: Filesystem path to the tmux server socket,
        e.g. ``"/tmp/omnicraft-xyz/tmux.sock"``.
