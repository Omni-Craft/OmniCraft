...
``omnicraft_client.tools`` — import them from there.

Imports are lazy so that loading ``omnicraft.tools`` does not pull
...
if TYPE_CHECKING:
    ...
    from omnicraft.tools.client_specified import (
        ClientSideTool,
        ClientSideToolSpec,
    )
    from omnicraft.tools.local import LocalPythonTool
...
# `omnicraft_client.tools` package (see sdks/python-client/). Import
# them from there — they used to be re-exported here but the
# duplicate surface was removed in the SDK carve-out.
_LAZY_EXPORTS: dict[str, tuple[str, str]] = {
    "Tool": ("omnicraft.tools.base", "Tool"),
    "ToolContext": ("omnicraft.tools.base", "ToolContext"),
    "ClientSideTool": ("omnicraft.tools.client_specified", "ClientSideTool"),
    "ClientSideToolSpec": (
        "omnicraft.tools.client_specified",
        "ClientSideToolSpec",
    ),
    "LocalPythonTool": ("omnicraft.tools.local", "LocalPythonTool"),
    "ToolManager": ("omnicraft.tools.manager", "ToolManager"),
}
...
def __getattr__(name: str) -> Any:
    ...
    ``tool`` decorator from ``omnicraft_client`` (not here),
