...
``omnicraft/inner/mcp_tools.py`` (which has never sandboxed
stdio MCPs). Future per-MCP sandboxing — if reintroduced —
should flow through the ``omnicraft/environments/`` primitive
with explicit outbound-host allowlists, not srt-defaults.

The PTY-mode wrap used by :class:`~omnicraft.terminals.shell.Shell`
