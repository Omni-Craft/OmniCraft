...
each proxies the OmniCraft server's existing REST endpoints over
``server_client`` (same channel and security posture as the
``sys_session_*`` family in
:mod:`omnicraft.tools.builtins.spawn`). They ship as schema-only
:class:`~omnicraft.tools.base.Tool` subclasses — the base-class
...
class SysAgentGetTool(Tool):
    ...
    :class:`~omnicraft.server.schemas.AgentObject`. Returns
...
class SysAgentListTool(Tool):
    ...
    - **local configs**: agent config YAMLs authored locally with
      ``sys_os_write`` (e.g. following the ``build-omnicraft`` skill) — a
