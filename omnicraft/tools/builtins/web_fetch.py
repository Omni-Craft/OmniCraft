...
``omnicraft/runner/tool_dispatch.py::_execute_web_fetch_tool``) which
...
# spawned — the runner aborts with ``unknown harness 'omnicraft'``.
_UNBOOTABLE_DEFAULT_HARNESS: str = "omnicraft"
...
def _ensure_default_sandbox_runnable() -> None:
    ...
    ``omnicraft.inner.sandbox._default_sandbox_for_platform``) without
    ...
    :raises OmniCraftError: On Linux when ``bwrap`` is not on ``PATH``,
...
def build_researcher_spec(parent_spec: AgentSpec) -> AgentSpec:
    ...
    - The parent executor's harness (``config``), ``auth``, ``model``, and
      ...
      parent's provider. Without this the child defaults to ``type="omnicraft"``
      with no harness, which the runner rejects as ``unknown harness
      'omnicraft'`` before any model routing (Layer 1), and even past that
    ...
    - An ``os_env`` block — registers ``sys_os_shell`` for one-shot
      ...
      per ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §3a in favor of
    ...
    - The parent's ``os_env.sandbox`` (filesystem grants, egress
      ...
      ``omnicraft/inner/os_env.py::create_os_environment``). If the
    ...
    :raises OmniCraftError: If the parent declares no bootable harness, so the
        researcher would spawn the unspawnable literal harness ``"omnicraft"``;
    ...
    # ``type="omnicraft"`` harness and strips the parent's provider. Drop
    ...
    if child_executor.harness_kind == _UNBOOTABLE_DEFAULT_HARNESS:
        raise OmniCraftError(
            ...
            f"unknown harness 'omnicraft'. Set executor.config.harness on the parent "
