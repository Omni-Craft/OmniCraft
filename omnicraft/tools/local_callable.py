"""
In-process callable tools for the omnicraft-style spec adapter.

The omnicraft YAML translator stores user-declared function
tools as :class:`omnicraft.spec.types.LocalToolInfo` entries
with ``language == "omnicraft-python-callable"`` and ``path``
holding a dotted-import string (e.g.
``"examples._shared.tool_functions.calculate"``). These are
distinct from native OmniCraft local tools, which live as
``tools/python/*.py`` files in the agent bundle and execute via
:class:`omnicraft.tools.local.LocalPythonTool` in a subprocess.
...
- :func:`load_local_callable_tools` — helper that
  :class:`omnicraft.tools.manager.ToolManager` calls alongside
  :func:`omnicraft.tools.local.load_local_python_tools` to
  pick up the omnicraft-translated subset.
...
when the omnicraft compat path retires (the legacy executor
...
from omnicraft.spec.types import LocalToolInfo
from omnicraft.tools.base import Tool, ToolContext

_logger = logging.getLogger(__name__)

# Match the language constant used by the omnicraft YAML
# translator (omnicraft/spec/omnicraft.py:OMNICRAFT_TOOL_LANGUAGE).
# Duplicated here because importing from spec/omnicraft.py would
...
class LocalCallableTool(Tool):
    ...
    :attr:`omnicraft.spec.types.LocalToolInfo.path` and is
    ...
    def dotted_path(self) -> str:
        ...
        Used by :func:`omnicraft.runtime.workflow._dispatch_local_callable_tool_async`
    ...
    def description(cls) -> str:
        ...
        :returns: Generic class-level description. Per-instance
            ...
            :meth:`get_schema`.
        """
        return "User-declared function tool from an omnicraft-style YAML."
    ...
    def get_schema(self) -> dict[str, Any]:
        ...
        used (matching what the omnicraft harness advertises
...
def load_local_callable_tools(
    ...
    """
    Build :class:`LocalCallableTool` instances for omnicraft tools.

    Filters *local_tools* to the ones the omnicraft YAML
    translator produced — entries whose ``language`` is
    ``"omnicraft-python-callable"``. Other entries (native AP
    file-based tools with ``language == "python"``) are
    handled by :func:`omnicraft.tools.local.load_local_python_tools`.

    :param local_tools: All :class:`LocalToolInfo` entries from
        the agent spec.
    :returns: One :class:`LocalCallableTool` per omnicraft-style
        entry. Empty list when none are present (which is the
        case for native OmniCraft specs).
    """
    from omnicraft.spec.types import ToolRuntime

    result: list[LocalCallableTool] = []
    for info in local_tools:
        if info.language != _OMNICRAFT_CALLABLE_LANGUAGE:
            continue
...
def _stringify(value: Any) -> str:
    ...
    The legacy omnicraft path stringifies tool results the same
