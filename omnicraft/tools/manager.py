...
from omnicraft.inner.os_env import OSEnvironment
from omnicraft.runtime import get_caps
from omnicraft.spec import AgentSpec
from omnicraft.spec.types import SharePolicy, ToolRuntime
from omnicraft.tools._srt import is_srt_available
from omnicraft.tools.base import Tool, ToolContext, is_valid_tool_name
from omnicraft.tools.builtins import (
    ListCommentsTool,
    LoadSkillTool,
    ReadSkillFileTool,
    SysAdviseModelsTool,
...
from omnicraft.tools.local import load_local_python_tools
...
class ToolManager:
    ...
    def _register_policy_tools(self) -> None:
        ...
        the OmniCraft server's REST endpoints.
        """
    ...
    def _register_timer_tools(self) -> None:
        ...
        match ``omnicraft/inner/datamodel.py::AgentDef.timers``).
    ...
    def _create_web_fetch(self) -> Tool:
        ...
        """
        from omnicraft.tools.builtins.web_fetch import WebFetchTool
    ...
    def _register_task_lifecycle_tools(self) -> None:
        ...
        """
        from omnicraft.tools.builtins.async_inbox import SysCancelTaskTool
    ...
    def _register_terminal_tools(self) -> None:
        ...
        Per ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` §4.3, the five
        ...
        :func:`omnicraft.runtime.get_terminal_registry`) so state
    ...
    def _register_local_tools(self, workdir: Path | None) -> None:
        ...
        for client_info in client_local_tools:
            ...
            # The validator (see ``omnicraft/spec/validator.py``)
        ...
        # Native OmniCraft local tools (``language == "python"``) need a
        # workdir on disk so the subprocess loader can locate the
        # ``tools/python/*.py`` files. OmniCraft-style tools
        # (``language == "omnicraft-python-callable"``) come
        ...
        if workdir is not None:
            for tool in load_local_python_tools(
                ...
                if not is_valid_tool_name(tool.name()):
                    ...
                    continue
                self._tools[tool.name()] = tool
        # OmniCraft-style callable tools — sibling loader for the
        # ``omnicraft-python-callable`` language entries the YAML
        # translator emits. See
        # :mod:`omnicraft.tools.local_callable` for the reasoning;
        ...
        from omnicraft.tools.local_callable import load_local_callable_tools

        for callable_tool in load_local_callable_tools(server_local_tools):
            if not is_valid_tool_name(callable_tool.name()):
                _logger.warning(
                    "OmniCraft callable tool %r has invalid name — skipping",
                    callable_tool.name(),
                )
                continue
            if callable_tool.name() in self._tools:
                ...
                raise ValueError(
                    f"omnicraft callable tool {callable_tool.name()!r} "
                    f"collides with an already-registered tool"
                )
    ...
    def _register_client_tools(
        ...
        Raises :class:`OmniCraftError` if a tool name violates the
        ...
        :param specs: List of :class:`ClientSideToolSpec` objects to
            register, e.g.
            ``[ClientSideToolSpec(name="get_weather", ...)]``.
        :raises OmniCraftError: If any tool name is invalid.
        """
        for spec in specs:
