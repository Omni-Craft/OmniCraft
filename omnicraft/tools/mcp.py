...
The runner's :class:`omnicraft.runner.mcp_manager.RunnerMcpManager`
...
class McpServerConnection:
    ...
    async def _invoke_tool(
        ...
        ``/mcp/execute`` → OmniCraft server) can surface the elicitation
    ...
    async def call_tool_with_elicitation(
        ...
        Called by the OmniCraft server after the user has responded to an
        ...
        # InputRequiredResult on the retry. Raise so the OmniCraft server
    ...
    async def _open_stdio_transport(
        ...
        (``omnicraft/inner/mcp_tools.py``); OmniCraft now matches that
        baseline. Per-MCP sandboxing — if reintroduced — should
        flow through the ``omnicraft/environments/`` primitive
...
async def _sleep(seconds: float) -> None:
    ...
    ``asyncio.sleep`` globally (patching ``omnicraft.tools.mcp.asyncio.sleep``
...
async def _call_tool_with_reconnect(
    ...
    :param session_id: OmniCraft session id forwarded to
