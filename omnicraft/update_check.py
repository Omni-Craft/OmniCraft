...
* **Dev clone** (``git clone … && uv sync`` / ``pip install -e .``): we
  ...
  to ``~/.omnicraft/.update_check.json`` so the (potentially slow)
  ``git fetch`` only runs once per staleness window. The notice points
  the user at ``git pull``.

* **Installed wheel** (``uv tool install omnicraft``,
  ``pip install omnicraft``, ``pipx install``, Homebrew, etc.): no clone
  ...
  old. The index is resolved from ``OMNICRAFT_INDEX_URL`` /
...
_ENV_SKIP = "OMNICRAFT_NO_UPDATE_CHECK"
_CACHE_DIR = Path.home() / ".omnicraft"
...
# explicit ``OMNICRAFT_INDEX_URL`` wins; then uv's and pip's. When none is
...
class _InstalledWheelInfo:
    ...
    :param vcs_url: The git URL recorded in ``direct_url.json``
        ...
        ``"git+https://github.com/omnicraft-ai/omnicraft.git"``.
...
def _resolve_index_url() -> str:
    ...
    1. index env vars (:data:`_INDEX_ENV_VARS`) — explicit
       ``OMNICRAFT_INDEX_URL`` first, then uv's and pip's;
...
def fetch_latest_version(
    ...
    *,
    ...
    """Fetch the latest ``omnicraft`` release from the configured index.
...
def _versions_from_filenames(filenames: Iterable[str]) -> list[Version]:
    ...
    :param filenames: Distribution filenames, e.g.
        ``["omnicraft-0.2.0-py3-none-any.whl", "omnicraft-0.2.0.tar.gz"]``.
    ...
    with contextlib.suppress(OSError, ValueError):
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from omnicraft.update_check import refresh_update_cache as r; r()",
            ],
...
def _find_repo_root() -> Path | None:
    ...
    DIRECT parent of the ``omnicraft/`` package, and must contain
    both ``.git/`` AND a ``pyproject.toml``. The layout we accept
    is exactly what a dev clone of this repo looks like::

        <repo>/.git/
        <repo>/pyproject.toml
        <repo>/omnicraft/update_check.py    ← this file

    We do NOT walk further up the filesystem tree. The previous
    implementation did, which caused a real bug: when ``omnicraft``
    is installed via ``uv tool install`` at
    ``~/.local/share/uv/tools/omnicraft/…``, the unbounded walk-up
    ...
    """
    package_dir = Path(__file__).resolve().parent  # <candidate>/omnicraft/
...
def _run_check(repo_root: Path) -> _CacheEntry | None:
    ...
    :param repo_root: Absolute path to the Git repository root,
        e.g. ``Path("/home/user/omnicraft-2")``.
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    The ``omnicraft/_build_info.py`` module is generated by
    ...
    """
    try:
        from omnicraft import _build_info  # type: ignore[attr-defined]
    except ImportError:
        return None
    try:
...
def _get_distribution() -> importlib.metadata.Distribution | None:
    ...
    :returns: The ``Distribution`` for the ``omnicraft`` package,
...
def _unredact_ssh_userinfo(vcs_url: str) -> str:
    ...
    :param vcs_url: The normalized ``<vcs>+<scheme>://…`` reinstall URL,
        e.g. ``"git+ssh://****@github.com/omnicraft-ai/omnicraft.git"``.
    :returns: The same URL with a redacted SSH user restored to ``git``,
        e.g. ``"git+ssh://git@github.com/omnicraft-ai/omnicraft.git"``;
...
def _pip_invocation() -> str:
    ...
    *other* environment's ``pip`` shadows the one running ``omni`` (a
    ...
    copy of omnicraft while the running one stays put. ``<sys.executable>
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    ``omnicraft`` and cached its metadata — re-reading in-process returns the
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    The version is read via ``importlib.metadata`` (no ``omnicraft`` import,
    ...
    probe = (
        ...
        "try:\n"
        "    print(m.version('omnicraft'))\n"
        "except Exception:\n"
        "    print('')\n"
        "try:\n"
        "    from omnicraft.update_check import _read_installed_wheel_info as r\n"
        "    i = r()\n"
...
def _remote_git_head(vcs_url: str) -> str | None:
    ...
    :param vcs_url: A normalized VCS URL, e.g.
        ``"git+https://github.com/omnicraft-ai/omnicraft.git"`` or
        ``"git+https://…/omnicraft.git@main"``.
