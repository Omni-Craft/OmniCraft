...
def _rotate_session_on_clear(bridge_dir: Path) -> str | None:
    ...
    try:
        ...
        print(f"omnigent claude clear hook: OmniCraft rotation failed: {exc}", file=sys.stderr)
        return None
...
def _rotate_session_on_fork(bridge_dir: Path) -> str | None:
    ...
    try:
        ...
        print(f"omnigent claude fork hook: OmniCraft fork failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
    ...
    while True:
        ...
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    if refreshed:
                        ...
                        reauthed = True
                        print(
                            f"omnigent {hook_label} hook: OmniCraft auth expired "
            ...
            if exc.response.status_code < 500:
                print(
                    f"omnigent {hook_label} hook: OmniCraft request rejected: {exc}",
                    file=sys.stderr,
                )
                return None
            ...
            print(
                f"omnigent {hook_label} hook: OmniCraft request failed; retrying: {exc}",
                file=sys.stderr,
            )
        except httpx.HTTPError as exc:
            ...
            print(
                f"omnigent {hook_label} hook: OmniCraft request failed ({kind}); retrying: {exc}",
                file=sys.stderr,
            )
        if is_hard_failure:
...
def _main_permission_request(argv: list[str]) -> int:
    ...
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("omnigent claude permission hook: OmniCraft server URL missing", file=sys.stderr)
        return 0
...
def _main_ask_user_question(argv: list[str]) -> int:
    ...
    ap_server_url = args.omnigent_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("omnigent ask-user-question hook: OmniCraft server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.omnigent_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    if not resp.content:
        print("omnigent evaluate-policy hook: empty OmniCraft response", file=sys.stderr)
        return _fail_closed()

    try:
        eval_response = resp.json()
    except json.JSONDecodeError:
        print("omnigent evaluate-policy hook: malformed OmniCraft response", file=sys.stderr)
        return _fail_closed()
