"""OmniCraftClient — the top-level client tying all namespaces together."""
...
class OmniCraftClient:
    """Typed Python client for the omnicraft server API.
    ...
    def __init__(
        ...
        *,
        ...
        default_headers = {"Origin": OMNICRAFT_INTERNAL_WS_ORIGIN}
        if headers:
    ...
    async def _fetch_agent_tools(
        ...
        :returns: List of tool-entry dicts with at least ``name``
            ...
            predates F1.
        :raises OmniCraftError: If the agents endpoint returns
            a non-2xx (e.g. 404).
        """
    ...
    async def __aenter__(self) -> OmniCraftClient:
        return self
