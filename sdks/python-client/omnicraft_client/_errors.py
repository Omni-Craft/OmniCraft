"""Typed exceptions for the omnicraft client."""
...
class OmniCraftError(Exception):
    """Base exception for all omnicraft client errors."""
...
class AgentNotFoundError(OmniCraftError):
    """Agent name or ID not found (HTTP 404)."""


class ResponseNotFoundError(OmniCraftError):
    """Response ID not found (HTTP 404)."""


class FileNotFoundError(OmniCraftError):
    """File ID not found (HTTP 404)."""


class ConversationNotFoundError(OmniCraftError):
    """Conversation ID not found (HTTP 404)."""


class InvalidInputError(OmniCraftError):
    """Bad request — invalid input, missing fields, etc. (HTTP 400)."""


class ConflictError(OmniCraftError):
    """Resource conflict — duplicate name, stale state, etc. (HTTP 409)."""


class BundleInvalidError(OmniCraftError):
    """Agent bundle is invalid — corrupt tarball, bad config, etc. (HTTP 400)."""


class ServerError(OmniCraftError):
...
def raise_for_status(status_code: int, body: dict[str, object] | str) -> None:
    ...
    _CODE_MAP: dict[str, type[OmniCraftError]] = {
        "not_found": OmniCraftError,
    ...
    if status_code == 404:
        ...
        # OmniCraftError or the specific subclass at the call site.
        raise OmniCraftError(message, status_code, code)
...
def require_json_object(resp: httpx.Response, endpoint: str) -> dict[str, object]:
    ...
    protocol/base-URL/auth problem, so raise :class:`OmniCraftError`
    ...
    :raises OmniCraftError: If the body is not a JSON object.
    """
    try:
        data = resp.json()
