"""LocalServer — start and manage a local omnicraft server."""
...
if TYPE_CHECKING:
    from ._client import OmniCraftClient
...
class LocalServer:
    """Context manager that starts a local omnicraft server.
    ...
    def __init__(
        ...
        *,
        ...
        self._client: OmniCraftClient | None = None
    ...
    @property
    def client(self) -> OmniCraftClient:
    ...
    async def __aenter__(self) -> LocalServer:
        ...
        self._client = OmniCraftClient(base_url=self.base_url)
        return self
    ...
    def _start(self) -> None:
        ...
        self._tmpdir = tempfile.mkdtemp(prefix="omnicraft-client-")
        db_uri = f"sqlite:///{self._tmpdir}/chat.db"
        art_loc = f"{self._tmpdir}/artifacts"

        # Resolve the omnicraft project root so Alembic can find
        # its migrations directory. Walk up from the agent path or
        # from this file's location looking for omnicraft/cli.py.
