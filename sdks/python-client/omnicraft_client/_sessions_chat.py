...
A higher-level wrapper over :class:`omnicraft_client._sessions.SessionsNamespace`
that mirrors enough of :class:`omnicraft_client._session.Session`'s
public surface for downstream consumers (terminal REPL, ``omnicraft
...
:class:`omnicraft_client._session.Session` synthesizes a multi-turn
...
``omnicraft/server/API.md`` ("Sessions API"). Per the same spec
...
:data:`omnicraft.server.schemas.ServerStreamEvent` envelopes
...
:class:`omnicraft_client._session.Session` and renaming would break
...
from ._errors import OmniCraftError
...
# :class:`omnicraft.server.schemas.SessionEventInput`. Kept as a
...
class SessionToolCallInfo:
    ...
    :class:`omnicraft_client._tool_handler.ToolCallInfo` — that
...
# terminal set listed in ``omnicraft/server/schemas.py`` (and
# the ``_TERMINAL_STATUSES`` set in
# :mod:`omnicraft_client._session`).
...
class SessionsChat:
    ...
    Usage mirrors :class:`omnicraft_client._session.Session` for
    ...
    session is an HTTP call. Use :meth:`OmniCraftClient.sessions_chat`
    ...
    :param namespace: The :class:`SessionsNamespace` this helper
        ...
        :class:`OmniCraftClient`; the chat helper does not own
    ...
    async def create(
        ...
        *,
        ...
        :returns: A :class:`SessionsChat` bound to the newly created
            session.
        :raises OmniCraftError: If session creation fails.
        """
    ...
    async def refresh(self) -> Session:
        ...
        :raises OmniCraftError: If the session no longer exists
    ...
    async def tree_busy(self, *, max_depth: int = 3) -> bool:
        ...
        :raises OmniCraftError: On non-2xx status.
        """
    ...
    async def send(
        ...
        *,
        ...
        :raises OmniCraftError: If the underlying ``post_event`` or
        ...
        try:
            ...
            while True:
                ...
                if isinstance(event, SessionStatusEvent) and event.status == "failed":
                    ...
                    raise OmniCraftError(message, code=code)
                try:
        ...
        * a key in ``tool_callables`` that does not name a
        ...
        if self._tool_callables and self._agent_tools_getter is None:
            raise RuntimeError(
                ...
                "OmniCraftClient.sessions_chat which wires it "
    ...
    async def cancel(self) -> None:
        ...
        :raises OmniCraftError: If the session does not exist
            (404).
        """
    ...
    async def query(
        ...
        *,
        ...
        :raises OmniCraftError: If the underlying HTTP calls fail.
    ...
    async def await_turn(self, *, timeout: float | None = 1200.0) -> QueryResult:
        ...
        :raises OmniCraftError: Propagated from :meth:`stream` if the
    ...
    async def _build_content(
        ...
        :meth:`omnicraft_client._session.Session._build_input_with_files`
...
class _FilesUploader(Protocol):
    ...
    :meth:`omnicraft_client._files.FilesNamespace.upload`. Defined
...
class _FilesGetter(Protocol):
    ...
    Matches the signature of
    :meth:`omnicraft_client._files.FilesNamespace.get`.
    """
...
def _build_tool_call_info(item: dict[str, Any]) -> SessionToolCallInfo:
    ...
    ``omnicraft/server/schemas.py:OutputItemDoneEvent``) —
