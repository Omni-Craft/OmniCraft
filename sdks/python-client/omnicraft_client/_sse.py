...
from omnicraft.server import schemas as _srv_events
...
_log = logging.getLogger("omnicraft_client.sse")
...
def _wire_type(cls: type) -> str:
    ...
    Each :mod:`omnicraft.server.schemas` event class pins its
    ...
    :param cls: A subclass of the server's ``_SSEEventBase``,
        e.g. :class:`omnicraft.server.schemas.OutputTextDeltaEvent`.
...
def _parse_event(event_type: str, data: dict[str, Any]) -> StreamEvent | None:
    ...
    :param event_type: Wire name of the SSE ``event:`` field, e.g.
        ...
        :mod:`omnicraft.server.schemas`) to dispatch.
