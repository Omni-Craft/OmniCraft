...
from omnicraft_client import TERMINAL_TASK_STATUSES, child_session_busy
...
_OVERLAY_REQUEST_SENTINEL: str = "\x00__omnicraft_ui_sdk.overlay_trigger__\x00"
...
_SUBAGENT_SELECT_SENTINEL: str = "\x00__omnicraft_ui_sdk.subagent_select__\x00"
...
# that omnicraft' cli.py uses so the two REPLs look identical
...
class _SubagentNode:
    ...
    # labels / title via :func:`omnicraft.session_lifecycle.is_session_closed`.
...
class Overlay:
    ...
    * **Sidebar + content** (``targets_builder`` is supplied):
    ...
    :param builder: Async callable invoked each time the overlay
        ...
        See ``omnicraft/repl/_repl.py::_build_debug_overview``
    ...
    :param sidebar_width: Column width (in characters) for the
        ...
        — matches the omnicraft debug panel and fits the
...
def _install_csi_u_sequences() -> None:
    ...
    ``omnicraft/inner/cli.py:1186-1239`` so behavior is
...
class TerminalHost:
    ...
    :param history_file: Path for persistent input history.
        Defaults to ``"~/.omnicraft_history"`` to match the
        legacy ``omnicraft run`` CLI's location
        (``omnicraft/inner/cli.py:_cli_history_file_path``) so
        users who flip between legacy and OmniCraft mode see the same
        ↑ / Ctrl+R recall in both. SDK consumers outside
        omnicraft can override.
    ...
    def __init__(
        ...
        *,
        prompt_marker: str = "❯",
        accent_color: str = "#0fb5bd",
        history_file: str = "~/.omnicraft_history",
    ...
    def _try_set_window_title(self) -> None:
        ...
        (``omnicraft/inner/cli.py:2717-2723``) swallows the same
        way for the same reason; mirroring keeps behavior
        identical so a session that boots green on legacy boots
        green on OmniCraft mode regardless of terminal quirks.
        """
    ...
    async def __aenter__(self) -> TerminalHost:
        ...
        try:
            from omnicraft.cli_diagnostics import redirect_stderr_to_log
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        try:
            from omnicraft.cli_diagnostics import restore_stderr
    ...
    async def _show_overlay(self, overlay: Overlay) -> None:
        ...
        async def _refresh_loop() -> None:
            ...
            hammer the server. Matches omnicraft' overview polling
            strategy (see ``omnicraft/cli.py::_refresh_loop``),
            with the interval bumped from 50 ms → 500 ms because
            OmniCraft' builder crosses a real HTTP boundary
            while omnicraft' builder just reads in-process state.
            """
            try:
                while True:
                    await asyncio.sleep(0.5)
        ...
        def _get_sidebar() -> ANSI:
            ...
            even with variable-length labels. Matches the omnicraft
    ...
    def upsert_subagent(
        ...
        *,
        ...
        # the ``omnicraft`` server package on the path.
        if not node.closed:
            from omnicraft.session_lifecycle import is_session_closed
    ...
    def build_toolbar(self) -> FormattedText:
        ...
        omnicraft' main REPL shows at the bottom-right. A
