"""Custom setuptools build for omnicraft.

Generates ``omnicraft/_build_info.py`` at wheel build time so the
CLI's update-check (``omnicraft/update_check.py``) can tell the user
...
class _GenerateBuildInfo(build_py):
    ...
    def _build_web_ui(self) -> None:
        ...
        (``omnicraft/server/app.py``); when absent it serves an
        ...
        - Skip if ``OMNICRAFT_SKIP_WEB_UI=true``. The hardened CI
        ...
        - Skip if the bundle already exists, UNLESS
          ``OMNICRAFT_BUILD_WEB_UI=1`` forces a rebuild. This keeps
        ...
        - Otherwise the build MUST succeed: a missing ``npm`` or a
          ...
          install with an actionable error. OmniCraft needs Node +
        ...
        root = Path(__file__).resolve().parent
        web_src = root / "web"
        bundle = root / "omnicraft" / "server" / "static" / "web-ui" / "index.html"

        if not (web_src / "package.json").is_file():
            return
        # CI opt-out: exact "true" only — this is set by our own
        # workflows, not user-facing config.
        if os.environ.get("OMNICRAFT_SKIP_WEB_UI") == "true":
            return
        force_raw = os.environ.get("OMNICRAFT_BUILD_WEB_UI")
        ...
        if npm is None:
            raise SystemExit(
                "omnicraft build: npm not found on PATH, so the web UI "
                "cannot be built. OmniCraft requires Node.js 22 LTS or "
                ...
                "server), set OMNICRAFT_SKIP_WEB_UI=true."
            )
        try:
            ...
            raise SystemExit(
                f"omnicraft build: web UI build failed ({exc}). Fix the "
    ...
    def _write_build_info(self) -> None:
        """Write ``omnicraft/_build_info.py`` into the source tree.
        ...
        any later non-build code path that does ``from omnicraft
        import _build_info`` works without re-running the build.
        """
        target = Path(__file__).resolve().parent / "omnicraft" / "_build_info.py"
        commit = _git_sha()
        ...
        target.write_text(
            ...
            "defensively (``try: from omnicraft import _build_info``)\n"
