...
1. **Server redirect (Config 1)** — pin the ``omnicraft.cli server`` subprocess
   to an older build (``OMNICRAFT_COMPAT_SERVER_PYTHON``) while the client,
   runner, host, and tests stay on main. Skip newer-than-server features with
   ``@pytest.mark.min_server_version(...)``.
2. **Runner/host redirect (Config 2)** — pin the ``omnicraft.runner._entry`` and
   ``omnicraft.host._daemon_entry`` subprocesses to an older build
   (``OMNICRAFT_COMPAT_RUNNER_PYTHON``) while the server, client, and tests stay
...
COMPAT_RUNNER_VERSION_ENV = "OMNICRAFT_COMPAT_RUNNER_VERSION"
...
def _compat_python(env_var: str) -> str | None:
    ...
    :param env_var: The redirect env var, e.g.
        ``"OMNICRAFT_COMPAT_SERVER_PYTHON"``.
...
def _compat_cwd(env_var: str, label: str) -> str | None:
    ...
    ``python -m omnicraft...`` puts the CWD on ``sys.path[0]``, so a subprocess
    launched from the repo checkout would import the worktree's ``omnicraft/``
    ...
    if label not in _compat_cwds:
        _compat_cwds[label] = tempfile.mkdtemp(prefix=f"omnicraft-compat-{label}-cwd-")
...
def compat_server_python() -> str | None:
    ...
    :returns: The value of ``OMNICRAFT_COMPAT_SERVER_PYTHON`` (a venv python
...
def server_executable() -> str:
    """
    Interpreter to launch ``omnicraft.cli server`` with.
...
def compat_server_cwd() -> str | None:
    ...
    empty directory so the worktree's ``omnicraft/`` doesn't shadow the pinned
...
def compat_runner_python() -> str | None:
    ...
    :returns: The value of ``OMNICRAFT_COMPAT_RUNNER_PYTHON`` (a venv python
...
def runner_executable() -> str:
    """
    Interpreter to launch ``omnicraft.runner._entry`` / ``omnicraft.host._daemon_entry``.
...
def pinned_runner_version() -> str | None:
    ...
    workflow-set ``OMNICRAFT_COMPAT_RUNNER_VERSION`` is authoritative. ``None``
...
def reconcile_server_version(
    ...
    *,
    ...
    :param reported: Version from ``GET /api/version``, or ``None`` if it
        couldn't be read.
    :param override: ``OMNICRAFT_COMPAT_SERVER_VERSION`` value, or ``None``.
...
def resolve_server_version(base_url: str) -> str:
    ...
    ``OMNICRAFT_COMPAT_SERVER_VERSION`` covers an unreadable endpoint and
