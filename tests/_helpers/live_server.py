"""
Shared fixtures for tests that need a live ``omnicraft.cli server`` subprocess.

Lifted out of ``tests/e2e/conftest.py`` so the inner test suite
(``tests/inner/test_integration.py`` running with OmniCraft mode) can
...
- :func:`make_live_server_fixture` — factory that builds a
  ...
  ``omnicraft.cli server`` subprocess. The subprocess inherits
...
def start_live_server(
    *,
    ...
    """
    Spawn an ``omnicraft.cli server`` subprocess and wait for
    health.
    ...
    # column" errors. In compat mode (OMNICRAFT_COMPAT_SERVER_PYTHON set) this
    ...
    proc = subprocess.Popen(
        ...
        env=env,
        # Compat mode: neutral CWD so the worktree's omnicraft/ on sys.path[0]
...
def make_live_server_fixture(
    ...
    ``omnicraft.cli server`` subprocess.
