...
from omnicraft import cli_diagnostics
...
def _capture_logger_snapshots() -> dict[str, _LoggerSnapshot]:
    ...
    for name in ("", "omnicraft", "omnicraft_ui_sdk", "databricks.sdk"):
...
def test_main_logs_click_exceptions(
    ...
    # raises before any daemon/network work. (A bare `omnicraft run` no longer
    # errors — it drops into first-run `configure harnesses` — so it can't be
    # the trigger here.)
    monkeypatch.setattr(sys, "argv", ["omnicraft", "run", "--harness", "not-a-real-harness"])
...
async def test_slash_command_exceptions_reach_cli_log(
    ...
    from omnicraft.repl._repl import handle_slash_command
...
def test_prune_old_logs_survives_file_vanishing_mid_sort(
    ...
    """Concurrent ``omnicraft run`` launches race to prune the same logs; a file
