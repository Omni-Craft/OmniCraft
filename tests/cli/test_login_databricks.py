"""Tests for ``omnicraft login`` against Databricks-fronted servers.
...
- workspace-hosted omnicraft (the API proxy answers 401 with a
...
_WORKSPACE_API_URL = f"{_WORKSPACE}/api/2.0/omnicraft"
...
def token_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    the user-level default (config.yaml, via OMNICRAFT_CONFIG_HOME). Both
...
def test_login_workspace_hosted_401_uses_url_host(
    ...
    Hosted omnicraft lives at ``https://<workspace>/api/2.0/omnicraft`` —
    ...
    assert result.exit_code == 0, result.output
    # Keyed by the full server URL (with the /api/2.0/omnicraft path) and
...
def test_login_apps_fails_loud_without_databricks_extra(
    ...
    assert "omnicraft[databricks]" in result.output
...
def test_login_accounts_mode_not_hijacked_by_databricks_detection(
    ...
    omnicraft accounts server has no such header and must keep its
...
def test_login_threads_org_id_through_workspace_login_and_verify(
    ...
    """
    from omnicraft.cli_auth import load_databricks_org_id, load_databricks_workspace_host
...
def test_login_sets_default_server(monkeypatch: pytest.MonkeyPatch, token_dir: Path) -> None:
    ...
    Without this, a freshly-logged-in user who runs a bare ``omnicraft`` is
...
def test_login_header_mode_sets_default_server(
    ...
    ``omnicraft login <url>`` against a header-mode server needs no
    credentials (a proxy injects identity), but the user's intent — "make
    this my server" — is the same, so a bare ``omnicraft`` afterwards
...
def test_login_accounts_mode_sets_default_server(
    ...
    fake = _FakeHttpx(responses=[_response(401, body={"login_url": "/login"})])
    _patch_login_env(monkeypatch, fake_httpx=fake)
    monkeypatch.setattr("omnicraft.cli._accounts_login", lambda server: None)
...
def test_login_failure_leaves_default_server_unchanged(
    ...
    reach would strand every later bare ``omnicraft`` on that dead URL.
    """
...
def test_workspace_url_expands_bare_workspace_host(
    ...
    """A bare workspace URL expands to its /api/2.0/omnicraft mount.
...
def test_workspace_url_leaves_apps_edge_alone(
    ...
    workspace, so a regression here would bolt /api/2.0/omnicraft onto
    every app URL.
    """
...
def test_workspace_url_expands_when_mount_hidden_from_anonymous_probe(
    ...
    Azure workspace edges answer the anonymous /api/2.0/omnicraft probe
    ...
    fake = _FakeHttpx(
        responses=[
            ...
            _response(404, headers={"server": "databricks"}),
            # Mount, authenticated: omnicraft answers.
...
def test_workspace_url_hints_when_authed_probe_also_misses(
    ...
    A 404 with valid credentials means omnicraft is genuinely not
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "dbc-a5d4177a-49dc.cloud.databricks.com/omnicraft",
...
def test_with_default_scheme(raw: str, expected: str) -> None:
    ...
    (``<ws>/omnicraft``); defaulting to https lets it be pasted verbatim,
...
def test_resolve_server_url_defaults_scheme_and_expands(
    ...
    ``/omnicraft`` web URL both reach the ``/api/2.0/omnicraft`` mount over
    https.
    """
    ...
    assert cli_mod._resolve_server_url("example.databricks.com/omnicraft") == _WORKSPACE_API_URL
...
def test_resolve_server_url_strips_query_and_expands(
    ...
    expansion — ``omnicraft run --server https://<ws>/?o=<id>`` has to reach
    ``/api/2.0/omnicraft`` (the selector rides via the login record /
...
def test_resolve_server_url_strips_query_on_full_mount(
    ...
    """A full ``/api/2.0/omnicraft?o=`` URL returns the clean mount with no probe.
...
def test_workspace_url_expands_web_ui_path_to_api_mount(
    ...
    """The guide's web-UI URL (``<ws>/omnicraft``) expands to the API mount.

    The internal user guide hands out ``<ws>/omnicraft`` for the browser;
    a user who pastes it into ``omnicraft login`` must reach the API mount
    (``/api/2.0/omnicraft``), not 404 the UI path's own /v1/me probe.
    """
    ...
    assert cli_mod._workspace_api_server_url(f"{_WORKSPACE}/omnicraft") == _WORKSPACE_API_URL
    # The bare root and its API mount were probed — never the UI path itself.
    assert f"{_WORKSPACE}/omnicraft/v1/me" not in probed


def test_workspace_url_web_ui_path_left_alone_off_workspace(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A ``/omnicraft`` URL on a non-workspace host is returned untouched.
    ...
    served under ``/omnicraft`` still works.
    """
    ...
    assert (
        cli_mod._workspace_api_server_url("https://omni.example.com/omnicraft")
        == "https://omni.example.com/omnicraft"
    )
...
def test_login_defaults_scheme_to_https(monkeypatch: pytest.MonkeyPatch, token_dir: Path) -> None:
    ...
    result = CliRunner().invoke(cli_group, ["login", "example.databricks.com/api/2.0/omnicraft"])
