...
import omnicraft.cli as cli
from omnicraft.cli import _list_opencode_models, _load_global_config, _set_opencode_default_model


@pytest.fixture
def _isolated_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    """Point the global config at a tmp file so saves don't touch ``~/.omnicraft``."""
    path = tmp_path / "config.yaml"
    monkeypatch.setattr("omnicraft.cli._GLOBAL_CONFIG_PATH", path)
    return path
...
def test_list_models_parses_nonblank_lines(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "omnicraft.onboarding.harness_install.harness_install_spec", lambda _key: _fake_spec()
    )
    monkeypatch.setattr(
        cli.subprocess,
        "run",
...
def test_list_models_empty_when_cli_absent(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        "omnicraft.onboarding.harness_install.harness_install_spec", lambda _key: None
    )
...
def test_set_default_model_no_models_short_circuits(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("omnicraft.onboarding.interactive.select", _select)
    status = _set_opencode_default_model(current=None)
