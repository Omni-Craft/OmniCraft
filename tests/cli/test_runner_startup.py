"""Tests for ``omnicraft._runner_startup`` (startup UX helpers).
...
def test_spinner_disabled_by_env_var(value: str) -> None:
    """Any truthy ``OMNICRAFT_NO_SPINNER`` value disables the spinner.
...
def test_spinner_not_disabled_by_falsy_env_var() -> None:
    """``OMNICRAFT_NO_SPINNER=0`` (or empty) leaves the spinner on.
...
def test_runner_startup_progress_plain_mode_prints_to_stderr(
    ...
    assert "omnicraft: Starting local runner" in captured.err
    assert "omnicraft: Waiting for runner to register" in captured.err
...
def test_runner_startup_progress_plain_mode_does_not_swallow_exceptions(
    ...
    # suppress it.
    assert "omnicraft: Starting" in capsys.readouterr().err
...
def test_startup_phase_labels_avoid_internal_jargon(label: str) -> None:
    ...
    # vocabulary (it is the ``omnicraft server`` they may run directly).
    jargon = {
        "daemon",
        "host",
        "runner",
