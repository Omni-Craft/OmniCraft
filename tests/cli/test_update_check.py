"""Tests for :mod:`omnicraft.update_check`."""
...
def test_find_repo_root_no_git_integration(
    ...
    fake_file = tmp_path / "omnicraft" / "update_check.py"
    fake_file.parent.mkdir(parents=True)
    fake_file.write_text("")

    import omnicraft.update_check as mod
...
def test_find_repo_root_ignores_unrelated_ancestor_git(
    ...
    omnicraft was installed under ``~/.local/share/uv/tools/`` —
...
def test_find_repo_root_requires_pyproject_alongside_git(
    ...
    """``.git/`` next to ``omnicraft/`` without ``pyproject.toml`` → None.

    Defense in depth against a hypothetical layout where someone
    has a directory tree like ``some_repo/omnicraft/`` (e.g. a
    ...
    happens to contain a folder called ``omnicraft/``.
    """
    repo_like = tmp_path
    (repo_like / ".git").mkdir()
    # Deliberately NO pyproject.toml here.
    pkg_dir = repo_like / "omnicraft"
    pkg_dir.mkdir()
    fake_file = pkg_dir / "update_check.py"
    fake_file.write_text("")
...
def test_find_repo_root_accepts_git_plus_pyproject(
    ...
    """``.git/`` AND ``pyproject.toml`` directly above ``omnicraft/`` → repo root."""
    repo = tmp_path / "omnicraft"
    repo.mkdir()
    (repo / ".git").mkdir()
    (repo / "pyproject.toml").write_text('[project]\nname = "omnicraft"\n')
    pkg_dir = repo / "omnicraft"
    pkg_dir.mkdir()
    fake_file = pkg_dir / "update_check.py"
    fake_file.write_text("")
    ...
    monkeypatch.setattr(mod, "__file__", str(fake_file))
    # Returns exactly the repo root — the parent of omnicraft/.
...
def test_read_cache_returns_none_when_missing(
    ...
    monkeypatch.setattr("omnicraft.update_check._CACHE_FILE", tmp_path / "nope.json")
...
def test_read_cache_returns_none_on_corrupt_json(
    ...
    monkeypatch.setattr("omnicraft.update_check._CACHE_FILE", cache_file)
    assert _read_cache() is None
...
def test_write_then_read_cache_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("omnicraft.update_check._CACHE_DIR", tmp_path)
    monkeypatch.setattr("omnicraft.update_check._CACHE_FILE", cache_file)
...
def test_fetch_and_count_git_not_found(tmp_path: Path) -> None:
    ...
    with patch("omnicraft.update_check.subprocess.run", side_effect=FileNotFoundError):
...
def test_fetch_and_count_fetch_fails(tmp_path: Path) -> None:
    ...
    with patch(
        "omnicraft.update_check.subprocess.run",
...
def test_fetch_and_count_success(tmp_path: Path) -> None:
    ...
    with patch("omnicraft.update_check.subprocess.run", side_effect=fake_run):
...
def test_skipped_when_env_set(
    ...
    """No-op when ``OMNICRAFT_NO_UPDATE_CHECK`` is set."""
    monkeypatch.setenv("OMNICRAFT_NO_UPDATE_CHECK", "1")
    maybe_show_update_notice()
...
def test_no_repo_root_routes_to_wheel_check(
    ...
    monkeypatch.setattr("omnicraft.update_check._run_installed_wheel_check", _stub_wheel_check)
    with patch("omnicraft.update_check._find_repo_root", return_value=None):
        maybe_show_update_notice()
...
def test_fresh_cache_shows_notice(
    ...
    with patch("omnicraft.update_check._find_repo_root", return_value=tmp_path):
        maybe_show_update_notice()
...
def test_fresh_cache_clears_after_pull(
    ...
    with (
        patch("omnicraft.update_check._find_repo_root", return_value=tmp_path),
        patch("omnicraft.update_check._get_head_sha", return_value="new_sha"),
        patch("omnicraft.update_check._local_rev_list_count", return_value=0),
    ):
        maybe_show_update_notice()
...
def test_stale_cache_triggers_check(
    ...
    with (
        patch("omnicraft.update_check._find_repo_root", return_value=tmp_path),
        patch("omnicraft.update_check.subprocess.run", side_effect=fake_run),
    ):
        maybe_show_update_notice()
...
from omnicraft.update_check import (  # noqa: E402
...
def _block_build_info_import(monkeypatch: pytest.MonkeyPatch) -> None:
    """Make ``from omnicraft import _build_info`` fail in every test.
    ...
    1. ``sys.modules["omnicraft._build_info"] = None`` — Python's
       documented "this import raises ImportError" sentinel.
    2. ``delattr(omnicraft, "_build_info")`` — once a previous test
       has done ``from omnicraft import _build_info`` successfully
       (via its own ``sys.modules`` override with a fake module),
       Python *also* sets ``_build_info`` as an attribute on the
       ``omnicraft`` package. Subsequent ``from omnicraft import
    ...
    omnicraft.update_check import _read_build_info`` inside a test
    body would resolve to the stubbed module attribute).
    """
    monkeypatch.setitem(sys.modules, "omnicraft._build_info", None)
    ...
    monkeypatch.delattr("omnicraft._build_info", raising=False)
...
def _no_real_background_refresh(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setattr("omnicraft.update_check._spawn_background_refresh", lambda: None)
...
def _write_fake_dist_info(
    ...
    *,
    ...
    """
    dist_info = tmp_path / "omnicraft-0.1.0.dist-info"
    dist_info.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "git+ssh://****@github.com/omnicraft-ai/omnicraft.git",
            "git+ssh://git@github.com/omnicraft-ai/omnicraft.git",
        ),
        # Same redaction, but the URL was stored without the ``git+``
        # VCS prefix (the shape uv wrote on the machine in the report).
        (
            "ssh://****@github.com/omnicraft-ai/omnicraft.git",
            "ssh://git@github.com/omnicraft-ai/omnicraft.git",
        ),
...
def test_read_wheel_info_repairs_redacted_ssh_user(
    ...
    """
    redacted_url = "ssh://****@github.com/omnicraft-ai/omnicraft.git"
    dist = _write_fake_dist_info(
        tmp_path,
        installer="uv",
        direct_url={
    ...
    assert info.vcs_url == "git+ssh://git@github.com/omnicraft-ai/omnicraft.git"
    ...
    assert (
        suggestion.command
        == "uv tool install --reinstall git+ssh://git@github.com/omnicraft-ai/omnicraft.git"
    )
...
def test_read_wheel_info_pip_registry_install(
    ...
    """``pip install omnicraft`` from PyPI: no direct_url, mtime fallback."""
...
def test_build_upgrade_suggestion_matrix(
    ...
    try to ``subprocess.run`` the literal string "reinstall omnicraft
...
def _point_cache_at(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("omnicraft.update_check._CACHE_DIR", tmp_path)
    monkeypatch.setattr("omnicraft.update_check._CACHE_FILE", tmp_path / ".update_check.json")
...
def test_wheel_check_bails_for_editable_install(
    ...
    spawned: list[bool] = []
    monkeypatch.setattr(
        "omnicraft.update_check._spawn_background_refresh", lambda: spawned.append(True)
    )
    dist = _write_fake_dist_info(
        tmp_path,
        installer="uv",
...
def test_wheel_check_env_var_disables(
    ...
    """``OMNICRAFT_NO_UPDATE_CHECK`` skips the wheel check entirely.
...
def test_is_newer_pep440_ordering() -> None:
    ...
    from omnicraft.update_check import _is_newer
...
_INDEX_ENV_VARS = ("OMNICRAFT_INDEX_URL", "UV_DEFAULT_INDEX", "UV_INDEX_URL", "PIP_INDEX_URL")
...
def _clear_index_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("omnicraft.update_check._index_from_uv_config", lambda: "")
    monkeypatch.setattr("omnicraft.update_check._index_from_pip_config", lambda: "")
...
def test_fetch_latest_version_pep691_json(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from omnicraft.update_check import fetch_latest_version
    ...
    assert captured["url"] == "https://pypi.org/simple/omnicraft/"
...
def test_fetch_latest_version_from_files_when_no_versions_key(
    ...
    body = {
        "files": [
            {"filename": "omnicraft-0.1.0-py3-none-any.whl"},
            {"filename": "omnicraft-0.2.0.tar.gz"},
            {"filename": "omnicraft-0.3.0rc1-py3-none-any.whl"},  # prerelease → excluded
...
def test_build_upgrade_suggestion_allow_prerelease() -> None:
    ...
    assert (
        ...
        == "uv tool install --reinstall git+https://x/omnicraft.git --prerelease allow"
    )
...
def test_resolve_index_url_precedence(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from omnicraft.update_check import _resolve_index_url
    ...
    monkeypatch.setenv("OMNICRAFT_INDEX_URL", "https://override.example/simple")
    assert _resolve_index_url() == "https://override.example/simple"

    # Multiple whitespace/comma-separated URLs → the first (primary) index.
    monkeypatch.setenv("OMNICRAFT_INDEX_URL", "https://a.example/simple, https://b.example/simple")
...
def test_refresh_update_cache_writes_latest_and_preserves_notified(
    ...
    dist = _write_fake_dist_info(tmp_path, installer="uv")
    monkeypatch.setattr("omnicraft.update_check._get_distribution", lambda: dist)
    monkeypatch.setattr("omnicraft.update_check._find_repo_root", lambda: None)
    monkeypatch.setattr("omnicraft.update_check.fetch_latest_version", lambda: "0.2.0")

    from omnicraft.update_check import refresh_update_cache
...
def test_refresh_update_cache_noop_for_clone(
    ...
    monkeypatch.delenv("OMNICRAFT_NO_UPDATE_CHECK", raising=False)
    _point_cache_at(tmp_path, monkeypatch)
    monkeypatch.setattr("omnicraft.update_check._find_repo_root", lambda: tmp_path)

    def _must_not_fetch() -> str:
        raise AssertionError("PyPI fetch attempted in a dev clone")

    monkeypatch.setattr("omnicraft.update_check.fetch_latest_version", _must_not_fetch)
...
def test_upgrade_command_for_installed(
    ...
    from omnicraft.update_check import upgrade_command_for_installed
...
def test_wheel_info_prefers_build_info_over_uv_cache(
    ...
    monkeypatch.setattr("omnicraft.update_check._get_distribution", lambda: dist)
    monkeypatch.setattr(
        "omnicraft.update_check._read_build_info",
...
def test_wheel_info_falls_back_to_uv_cache_when_build_info_missing(
    ...
    monkeypatch.setattr("omnicraft.update_check._read_build_info", lambda: None)
...
def test_read_build_info_returns_none_when_module_missing() -> None:
    ...
    ``sys.modules["omnicraft._build_info"]`` by setting it to
...
def test_read_build_info_returns_values_when_module_present(
    ...
    monkeypatch.setitem(sys.modules, "omnicraft._build_info", fake_module)
...
def test_legacy_cache_without_kind_field_defaults_to_clone(
    ...
    Backward-compat for users whose ``~/.omnicraft/.update_check.json``
...
def test_run_upgrade_command_invokes_subprocess(
    ...
    from omnicraft.update_check import _run_upgrade_command
...
def test_run_upgrade_command_returns_minus_one_when_binary_missing(
    ...
    console = Console(stderr=True)
    code = _run_upgrade_command("uv tool upgrade omnicraft", console)
...
def test_format_version_falls_back_to_bare_version_when_build_info_missing() -> None:
    """Without ``_build_info``, ``--version`` prints ``omnicraft <ver>``.

    Source checkouts (and any wheel built without our setup.py hook)
    hit this path. The line must remain stable across releases —
    scripts that grep for "omnicraft X.Y.Z" must keep working.
    """
    # The autouse fixture has already blocked sys.modules['omnicraft._build_info']
    # via the None sentinel, so _read_build_info returns None.
    from omnicraft.cli import _format_version
...
def test_split_vcs_url_strips_prefix_and_separates_revision() -> None:
    ...
    from omnicraft.update_check import _split_vcs_url

    assert _split_vcs_url("git+https://github.com/o/omnicraft.git") == (
        "https://github.com/o/omnicraft.git",
        None,
    )
    assert _split_vcs_url("git+https://github.com/o/omnicraft.git@main") == (
        "https://github.com/o/omnicraft.git",
        "main",
    )
...
def test_split_vcs_url_strips_pip_fragment() -> None:
    ...
    assert _split_vcs_url("git+https://github.com/o/omnicraft.git#egg=omnicraft") == (
        "https://github.com/o/omnicraft.git",
        None,
    )
    assert _split_vcs_url("git+https://github.com/o/omnicraft.git@main#subdirectory=pkg") == (
        "https://github.com/o/omnicraft.git",
        "main",
    )
...
def test_split_vcs_url_ssh_userinfo_is_not_a_revision() -> None:
    ...
    assert _split_vcs_url("git+ssh://git@github.com/o/omnicraft.git") == (
        "ssh://git@github.com/o/omnicraft.git",
        None,
    )
    # …but a real trailing revision still parses, even with SSH userinfo.
    assert _split_vcs_url("git+ssh://git@github.com/o/omnicraft.git@v1") == (
        "ssh://git@github.com/o/omnicraft.git",
        "v1",
    )
