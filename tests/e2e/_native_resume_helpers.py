...
Both ``omnicraft claude`` and ``omnicraft codex`` support resuming a prior
...
# Generous PTY width so the CLI's "OmniCraft: <url>/c/<conv_id>" and
# "Resume with: … --resume <conv_id>" lines (which carry the full id) are not
# wrapped/truncated by the terminal.
_PTY_ROWS = 60
_PTY_COLS = 220

# Env vars that, leaked from a parent omnicraft/Claude/Codex process into the
...
_STALE_ENV_VARS = (
    ...
    "TMUX",
    "RUNNER_SERVER_URL",
    "OMNICRAFT_RUNNER_WORKSPACE",
    "OMNICRAFT_RUNNER_ID",
    "OMNICRAFT_RUNNER_TUNNEL_BINDING_TOKEN",
)


def omnicraft_console_script() -> Path:
    """
    Return the ``omnicraft`` console-script path in the active venv.

    Driving the installed console script (rather than ``python -m omnicraft``)
    matches how users invoke the CLI; ``PYTHONPATH`` (set by :func:`cli_env`)
    points it at the worktree's code.

    :returns: Absolute path to the ``omnicraft`` entry point next to the
        running interpreter, e.g.
        ``"/Users/…/omnicraft/.venv/bin/omnicraft"``.
    :raises RuntimeError: If the console script is not found beside
        ``sys.executable``.
    """
    candidate = Path(sys.executable).parent / "omnicraft"
    if not candidate.is_file():
        raise RuntimeError(
            f"`omnicraft` console script not found at {candidate}; the test venv "
            f"must have omnicraft installed."
        )
    return candidate
...
def cli_env(*, profile: str | None = None) -> dict[str, str]:
    """
    Build the subprocess environment for an ``omnicraft <harness>`` PTY run.
    ...
    otherwise leak from this (possibly omnicraft-hosted) process into the
    ...
    :param profile: Databricks profile for the LLM gateway. When set, an
        isolated ``OMNICRAFT_CONFIG_HOME`` is created containing an
    ...
    if profile is not None:
        config_home = Path(tempfile.mkdtemp(prefix="omnicraft-native-config-"))
...
def run_cli_oneshot(args: list[str], *, env: dict[str, str], timeout: float) -> str:
    """
    Run ``omnicraft <harness> … -p <prompt>`` under a PTY until it exits.
    ...
    :param args: Full argv, e.g.
        ``["/…/omnicraft", "claude", "--server", url, "-p", "hi"]``.
...
class PtyHandle:
    """
    A backgrounded ``omnicraft <harness>`` PTY session kept alive for the test.
...
def spawn_cli_background(
    ...
    """
    Spawn ``omnicraft <harness> …`` under a PTY and keep it running.
    ...
    :param args: Full argv (no ``-p`` — the session stays interactive), e.g.
        ``["/…/omnicraft", "codex", "--server", url, "--resume", "conv_abc"]``.
    :param env: Subprocess environment from :func:`cli_env`.
...
def wait_for_conversation_id(handle: PtyHandle, *, timeout: float) -> str:
    ...
    Both CLIs print ``OmniCraft: <url>/c/conv_<hex>`` shortly after creating
...
def assert_native_cli_resume_restores_history(
    *,
    ...
    Harness-agnostic regression check for ``omnicraft <harness> --resume``:
    ...
    :param profile: Databricks CLI profile for the model gateway, e.g.
        ...
        ``auth:`` block + ``DATABRICKS_CONFIG_PROFILE`` (the omnicraft
    ...
    :param force_cold_resume: When ``True``, delete the harness's local
        ...
        transcript and must instead go through OmniCraft' cold-resume
    ...
    # yields "Not logged in"). The cost is that a *concurrent* ``omnicraft``
    # process on the same machine can thrash the shared host daemon
    # (``~/.omnicraft/host.pid``); run this opt-in test on an otherwise-idle
    # machine.
