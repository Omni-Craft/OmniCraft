...
def _enforce_min_server_version(request: pytest.FixtureRequest) -> None:
    ...
    (``OMNICRAFT_COMPAT_SERVER_VERSION`` set). The latter makes the
    ...
    compat_pinned = os.environ.get("OMNICRAFT_COMPAT_SERVER_VERSION")
...
def _enforce_min_runner_version(request: pytest.FixtureRequest) -> None:
    ...
    ``omnicraft.runner._entry`` / ``omnicraft.host._daemon_entry`` subprocesses to
    an older build and sets ``OMNICRAFT_COMPAT_RUNNER_VERSION``. The runner/host
...
def live_server(
    ...
    """
    Start a real ``omnicraft server`` subprocess and yield its base URL.
    ...
    env = {
        **os.environ,
        ...
        "OMNICRAFT_BUILTIN_AGENT_DIRS": str(builtin_sdk_chat_spec),
    }
    ...
    # happening silently — each e2e run polluted ``omnicraft.db``
    ...
    proc = subprocess.Popen(
        ...
        # Compat mode: neutral CWD so the worktree omnicraft/ doesn't shadow
...
def upload_agent(
    ...
    *,
    ...
    :param rewrite_model_for_databricks: When True, rewrite any
        ...
        in single-file omnicraft YAMLs.
...
def register_inline_agent(
    ...
    *,
    ...
    """
    Register a single-file omnicraft agent built in-memory.
...
def _materialize_builtin_sdk_chat_spec(
    ...
    *,
    ...
    ``OMNICRAFT_BUILTIN_AGENT_DIRS``, which reads the spec verbatim — it
...
def sys_terminal_test_agent(
    ...
    """
    Upload the ``sys-terminal-test`` agent (omnicraft-flavored
...
def send_user_message_to_session(
    ...
    *,
    ...
    but runner-native sessions do not create OmniCraft DBOS task rows for
...
def poll_session_until_terminal(
    ...
    *,
    ...
    pollable OmniCraft ``Task`` for ``GET /v1/responses/{response_id}``. This
...
def poll_for_pending_tool_calls(
    ...
    *,
    ...
    runner-native session turns, which do not create OmniCraft DBOS task rows
...
def resume_test_server(
    ...
    """
    Spawn a real ``omnicraft server`` that accepts the CLI's own runner.

    Used by the native-CLI resume e2e tests
    (``test_claude_native_cli_resume_e2e`` / ``test_codex_native_cli_resume_e2e``),
    which drive the real ``omnicraft claude/codex --server`` CLI. It differs
    from :func:`live_server` in two ways, both required for that:

    * **No tunnel-token allow-list.** ``OMNICRAFT_RUNNER_TUNNEL_TOKEN``
    ...
    * **Accounts auth.** The server defaults to accounts mode, which
    ...
    env.pop("OMNICRAFT_RUNNER_TUNNEL_TOKEN", None)
