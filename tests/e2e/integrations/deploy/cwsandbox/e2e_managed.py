#!/usr/bin/env python3
"""
End-to-end test: create a managed session against an OmniCraft server, and have
...
Two modes:
  ...
  1. --server <url>: use an EXISTING omnicraft server (already configured with
     ...
     serverless users who can't expose a public service.
         ...
         python tests/e2e/integrations/deploy/cwsandbox/e2e_managed.py \
             --server http://my-omnicraft:6767

  2. (default) spin the server up inside a CW Sandbox with a public service. The
     sandbox runs a prebaked image with this fork's omnicraft + the cwsandbox SDK
     (build/push first; pass --image), and the driver injects the LLM creds.
         ...
         python tests/e2e/integrations/deploy/cwsandbox/e2e_managed.py \
             --image docker.io/<you>/omnicraft-cwsandbox:test
"""
...
def _child_env(wandb_key: str) -> dict[str, str]:
    ...
    OMNICRAFT_RUNNER_ENV_PASSTHROUGH. The config's `sandbox.cwsandbox.env` name
...
def configure_and_start_server(sb: Sandbox, server_url: str, wandb_key: str) -> None:
    """Write config + an openai-agents agent, then launch `omnicraft server`."""
    log(f"[2/6] starting omnicraft server (server_url={server_url})")
    ...
    start = (
        f"OMNICRAFT_CONFIG_HOME={CONFIG_HOME} OMNICRAFT_LOCAL_SINGLE_USER=1 "
        f"setsid nohup omnicraft server --host 0.0.0.0 --port {SERVER_PORT} "
        f"--config {CONFIG_HOME}/config.yaml --no-open --agent /root/e2e-agent "
        "> /tmp/omnicraft-server.log 2>&1 < /dev/null & echo started"
    )
...
def _dump_server_logs(sb: Sandbox | None) -> None:
    ...
    out = sb.exec(
        [
            "bash",
            "-lc",
            "tail -50 ~/.omnicraft/logs/cli-*.log 2>/dev/null; "
            "echo '--- stdout ---'; tail -15 /tmp/omnicraft-server.log",
        ]
    ).result()
    log(out.stdout)


def _omnicraft_children() -> list:
    """List sandboxes the launcher tags 'omnicraft' (managed hosts); [] on error."""
    try:
        return Sandbox.list(tags=["omnicraft"]).result()
...
def dump_child_logs(exclude: set[str]) -> None:
    ...
    # belonging to other runs / deployments that share the 'omnicraft' tag.
    children = [c for c in _omnicraft_children() if c.sandbox_id not in exclude]
    ...
    try:
        out = target.exec(
            [
                "bash",
                "-lc",
                "tail -60 ~/.omnicraft/logs/*.log 2>/dev/null; echo '--- host log ---'; "
                "tail -40 /tmp/omnicraft-host.log 2>/dev/null",
            ]
        ).result()
        log(out.stdout)
    except Exception as exc:
...
def main() -> int:
    ...
    parser.add_argument(
        "--server",
        default=None,
        help="Use an EXISTING omnicraft server at this URL (e.g. http://host:6767) "
    ...
    parser.add_argument(
        "--image",
        default=None,
        help="Prebaked omnicraft+cwsandbox image (required only when spinning up the server).",
    )
    parser.add_argument(
        "--agent-id",
        default=None,
    ...
    if external:
        ...
        log(f"[*] using existing omnicraft server at {base}")
    else:
        sb, ip = start_server_sandbox(args.image, cw_key, wandb_key)
        base = f"http://{ip}:{SERVER_PORT}"

    # Children carry the shared "omnicraft" tag, so snapshot which ones already
    # existed before this run — we only ever touch the ones WE cause to appear,
    # never another run's / deployment's managed hosts.
    pre_children = {c.sandbox_id for c in _omnicraft_children()} if sb is not None else set()
    ...
    try:
        ...
        if sb is not None and not args.keep:
            ...
            for child in _omnicraft_children():
