...
Drives the REAL :class:`~omnicraft.onboarding.sandboxes.e2b.E2BSandboxLauncher`
...
pre-built omnicraft host template — it validates the launcher's SDK wiring
in isolation. Pass ``--template omnicraft-host`` (after ``e2b template
build``; see deploy/e2b/README.md) to smoke the real host template too.

    pip install 'omnicraft[e2b]'
...
# omnicraft package) isn't importable rather than a raw traceback.
try:
    ...
    print("Run from the repo root with omnicraft installed.", file=sys.stderr)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "'omnicraft-host' to smoke the real host template once built).",
    )
    ...
    marker_name = "OMNICRAFT_E2B_SMOKE_MARKER"
    ...
    try:
        ...
        payload = b"e2b-omnicraft-smoke\x00\x01binary\n"
