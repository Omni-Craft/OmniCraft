# `--omnicraft` example-YAML coverage: known gaps

Companion to `test_run_omnicraft_example_agents.py` and
`test_run_omnicraft_adapter_rejections.py`. Tracks gaps the e2e audit
...
Pure omnicraft' CLI auto-picks a harness from the model's
prefix: `databricks-claude-*` → `claude-sdk`, `databricks-gpt-*`
→ `openai-agents`, etc. The `--omnicraft` adapter's validator refused
specs without an explicit `harness:` field, so every YAML that
relied on pure-omnicraft auto-selection failed at spec-load:

```
invalid agent spec synthesized from omnicraft YAML:
  executor.config.harness: required when executor.type is
  'omnicraft' — must be one of ['claude-sdk', 'codex',
  'openai-agents', 'pi']
```

Fix: added `_infer_harness_from_model` helper in
`omnicraft.spec.omnicraft` with a `_HARNESS_FOR_MODEL_PREFIX`
lookup. `_translate_executor_from_def` calls it as the final
fallback when no explicit or parent harness is present. The
omnicraft CLI's `_apply_overrides_to_yaml` no longer injects a
...
### Fixed — Gap 7: Stock omnicraft policy callables don't work under OmniCraft

`resolve_function_policy` now detects legacy `(content, phase)` callables at
load time and wraps them in `_omnicraft_legacy_shim` — a one-argument adapter
...
exists to protect any user-written omnicraft-style policy callables that have
...
executor.type is 'omnicraft' — must be one of [...]
```
...
installed. Orthogonal to `--omnicraft`. Needs either an
...
If your omnicraft YAML declares a sub-agent tool named
`worker` and you want a policy that denies calls to it, the
natural thing to write is `match_tools: [worker]`. That doesn't
work under OmniCraft. Sub-agents are invoked through a generic
...
works but it's non-obvious and authors following the omnicraft
...
### Fixed — Gap 6: `--omnicraft` hung when FunctionTools were registered on the top-level agent

**Root cause**: the adapter synthesizes `LocalToolInfo.language =
"omnicraft-python-callable"` for every user FunctionTool /
...
`_build_omnicraft_tool_schemas` but never **registered** on
OmniCraft' `ToolManager`. When the harness's LLM invoked one,
...
dispatching, so the same guardrails OmniCraft' native tool
...
- `examples/agent_with_tools.yaml` (xfail removed, passes
  end-to-end through `--omnicraft` with the claude-sdk harness).
- `tests/e2e/omnicraft/test_run_omnicraft_policy_enforcement.py::test_policy_denies_tool_call_by_name`
  (real `omnicraft run --omnicraft -p "what is 6 + 6?"` subprocess
