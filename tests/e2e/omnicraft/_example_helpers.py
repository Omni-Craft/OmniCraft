...
``omnicraft run`` subprocess. The helpers in this module keep the
...
#   args: [-m, omnicraft.inner.databricks_mcps.<X>, --profile,
#          <profile-name>]
# in their YAMLs. The Databricks MCP subprocess has three auth
# modes (see omnicraft/inner/databricks_mcps/common/workspace.py):
...
#   OMNICRAFT_E2E_MCP_PROFILE=<your-profile> pytest ...
#
# Usage — mode 1 (direct PAT, no profile needed):
#   OMNICRAFT_E2E_MCP_HOST=https://... \
#   OMNICRAFT_E2E_MCP_TOKEN=dapi... \
#   pytest ...
#
# When none of the three vars are set, tests fall back to
# structural validation (spec + AgentDef translation).
ENV_MCP_PROFILE = "OMNICRAFT_E2E_MCP_PROFILE"
ENV_MCP_HOST = "OMNICRAFT_E2E_MCP_HOST"
ENV_MCP_TOKEN = "OMNICRAFT_E2E_MCP_TOKEN"
...
def example_yaml_path(repo_root: Path, name: str) -> Path:
    ...
    :param repo_root: Unified repo root; typically the
        ``omnicraft_repo_root`` fixture value.
...
def run_one_shot(
    *,
    ...
    omnicraft_credentials_env: dict[str, str],
    example_name: str,
    ...
    """
    Invoke ``omnicraft run <yaml> -p <prompt>`` one-shot.

    :param omnicraft_python: Interpreter with omnicraft + required
        SDKs installed. Provided by the ``omnicraft_python`` fixture.
    :param omnicraft_repo_root: Cwd so the YAML's ``callable:``
        dotted paths resolve via repo-root-on-sys.path. Provided by
        the ``omnicraft_repo_root`` fixture.
    :param omnicraft_credentials_env: PAT + BASE_URL + profile env
        populated from ``--llm-api-key``. Provided by the
        ``omnicraft_credentials_env`` fixture.
    ...
    :returns: The completed subprocess. Caller decides which
        fields to assert.
    """
    yaml_path = example_yaml_path(omnicraft_repo_root, example_name)
    return run_one_shot_at_path(
        omnicraft_python=omnicraft_python,
        omnicraft_repo_root=omnicraft_repo_root,
        omnicraft_credentials_env=omnicraft_credentials_env,
        yaml_path=yaml_path,
        prompt=prompt,
        harness=harness,
        model=model,
...
def run_one_shot_at_path(
    *,
    ...
    :param omnicraft_repo_root: Cwd (repo root so module lookups
        land).
    :param omnicraft_credentials_env: Env.
...
def validate_agent_def_structure(
    *,
    ...
    :param omnicraft_python: Interpreter with omnicraft installed.
    :param omnicraft_repo_root: Cwd so module-path resolution
        matches the CLI.
...
import json
import sys
sys.path.insert(0, {str(omnicraft_repo_root)!r})
from omnicraft.inner.loader import load_agent_def_from_path
...
"""
    result = subprocess.run(
        [str(omnicraft_python), "-c", snippet],
        cwd=str(omnicraft_repo_root),
        capture_output=True,
        text=True,
        timeout=30,
...
def assert_completed_one_shot(
    ...
    """
    Assert a one-shot ``omnicraft run`` finished cleanly.
    ...
    """
    assert result.returncode == 0, (
        f"{example_name!r}: omnicraft run exited with "
...
def mcp_auth_override() -> McpAuthOverride:
    ...
    If ``OMNICRAFT_E2E_MCP_PROFILE`` is set, returns a
    profile-based override. Else if BOTH ``OMNICRAFT_E2E_MCP_HOST``
    and ``OMNICRAFT_E2E_MCP_TOKEN`` are set, returns a PAT-based
...
def _rewrite_mcp_auth_in_place(yaml_obj: object, override: McpAuthOverride) -> int:
    ...
    every ``-m omnicraft.inner.databricks_mcps.*`` subprocess.
...
def _rewrite_args_list(args: list[object], override: McpAuthOverride) -> int:
    ...
    """
    if not any(
        isinstance(x, str) and x.startswith("omnicraft.inner.databricks_mcps.") for x in args
    ):
        return 0
...
def materialize_yaml_with_mcp_auth(
    ...
    Returns the path to pass to ``omnicraft run``:
