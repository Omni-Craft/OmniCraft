"""Reusable pexpect helpers for driving the OmniCraft REPL.

The OmniCraft REPL is a prompt-toolkit app that renders a full
...
Design reference: ``designs/OMNICRAFT_INTEGRATION.md`` §Phase 0
...
# every pexpect boot.
_TEST_THEME_CONFIG = "# OmniCraft user configuration\ntui:\n  theme: light\n"
...
def ensure_repl_test_theme_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    ``$HOME/.omnicraft/config.yaml`` has no ``tui.theme`` entry.
    ...
    config_path = home / ".omnicraft" / "config.yaml"
...
def spawn_omnicraft_run(
    ...
    *,
    ...
    """
    Spawn ``omnicraft run`` under a PTY for REPL tests.

    :param omnicraft_python: Python interpreter with omnicraft
        installed, e.g.
        ``Path("/Users/.../omnicraft/.venv/bin/python")``.
    :param yaml_path: Absolute path to the agent YAML, e.g.
        ``examples/hello_world.yaml``. Pass ``None`` to exercise
        ``omnicraft run --harness ...`` without an explicit
        agent argument.
    ...
    :param env: Environment dict for the subprocess — supplied
        by the ``omnicraft_credentials_env`` fixture so PAT
        and base URL propagate.
    :param cwd: Working directory for the subprocess. Must be
        the OmniCraft repo root so YAML ``callable:`` entries
    ...
    if yaml_path is None:
        ...
        # ``python -m omnicraft``. The package ``__main__`` module
        # intentionally routes through the legacy argparse quick-chat
        # CLI, while the installed ``omnicraft`` script invokes the
        ...
        command = str(omnicraft_python.parent / "omnicraft")
    else:
        args = [
            "-m",
            "omnicraft",
        ...
        command = str(omnicraft_python)
    if system_prompt is not None:
        args.extend(["--system-prompt", system_prompt])
    if initial_prompt is not None:
        args.extend(["-p", initial_prompt])
    # NOTE: the omnicraft CLI no longer accepts ``--profile``; Databricks
    # routing for spawned CLIs comes from the ``auth:`` block written into
    # the isolated ``OMNICRAFT_CONFIG_HOME`` by ``omnicraft_credentials_env``.
