"""Snapshot loader + comparator for OmniCraft Phase 0 characterization tests.

Snapshots are JSON files under ``tests/e2e/omnicraft/snapshots/``
and capture the *structural* observations a test makes about a
live OmniCraft run. They are the golden-master contract the
Phase 0 design requires: written against current OmniCraft,
...
def load_snapshot(test_name: str) -> dict[str, SnapshotField]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            f"their snapshots captured against current OmniCraft."
        )
...
def compare_snapshot(
    ...
    :param observed: Dict of observed values the test gathered
        from the live OmniCraft run, e.g.
