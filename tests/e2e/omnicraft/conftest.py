"""Fixtures for OmniCraft e2e tests (mock LLM).
...
# Root of the OmniCraft checkout that ships the ``omnicraft``
# package, the example YAMLs, and (in the main checkout) the
# ``.venv`` with omnicraft + pexpect + openai-agents installed.
#
# Derived from the conftest's own location so git worktrees work
# naturally: this file lives at
# ``<root>/tests/e2e/omnicraft/conftest.py`` (post-unification), so
...
# exec the main-checkout ``omnicraft`` (via the editable install),
# missing any per-worktree edits.
_OMNICRAFT_REPO = Path(__file__).resolve().parents[3]
    ...
    while True:
        ...
        if current.parent == current:
            ...
            raise RuntimeError(
                ...
                f"{_OMNICRAFT_REPO} — worktrees share the main "
...
_OMNICRAFT_VENV_PYTHON = _resolve_venv_python()


@pytest.fixture(scope="session")
def omnicraft_python() -> Path:
    """
    Path to the Python interpreter that has the ``omnicraft``
    package + its harness dependencies installed.

    The OmniCraft repo ships its own ``.venv`` with
    ``omnicraft``, ``pexpect``, ``openai-agents``,
    ``claude-agent-sdk``, etc. pre-installed. Agent-plane's e2e
    tests use that interpreter directly rather than adding
    omnicraft as an omnicraft dep (omnicraft is not
    distributed as a package yet).

    :returns: Absolute path to the OmniCraft ``.venv`` Python
        interpreter, e.g.
        ``"/path/to/omnicraft/.venv/bin/python"``.
    :raises RuntimeError: If the interpreter is not present at
        the expected path — indicates the OmniCraft checkout is
        missing or its .venv hasn't been created.
    """
    if not _OMNICRAFT_VENV_PYTHON.is_file():
        raise RuntimeError(
            f"OmniCraft venv python not found at {_OMNICRAFT_VENV_PYTHON}. "
            f"These e2e tests require the sibling checkout at "
            f"{_OMNICRAFT_REPO} with .venv set up."
        )
    return _OMNICRAFT_VENV_PYTHON


@pytest.fixture(scope="session")
def omnicraft_repo_root() -> Path:
    """
    Root of the OmniCraft checkout used as the subprocess cwd.

    OmniCraft YAMLs reference example tool modules via dotted
    ...
    :returns: Absolute path to the OmniCraft repo root, e.g.
        ``"/path/to/omnicraft"``.
    """
    return _OMNICRAFT_REPO
...
def mock_credentials_env(
    ...
    config_home = tmp_path_factory.mktemp("omnicraft-mock-e2e-config")
    (config_home / "config.yaml").write_text(
        "auth:\n  type: api_key\n",
        encoding="utf-8",
    )
    env["OMNICRAFT_CONFIG_HOME"] = str(config_home)
    repo = str(_OMNICRAFT_REPO)
    omnicraft_path = str(_OMNICRAFT_REPO / "omnicraft")
    existing_pp = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = os.pathsep.join(p for p in (repo, omnicraft_path, existing_pp) if p)
    return env
...
def mock_llm_server_url(
    ...
    proc = subprocess.Popen(
        [
            sys.executable,
            str(_OMNICRAFT_REPO / "tests" / "server" / "integration" / "mock_llm_server.py"),
            str(mock_port),
        ],
        env={**os.environ, "PYTHONPATH": str(_OMNICRAFT_REPO)},
        stdout=log_handle,
