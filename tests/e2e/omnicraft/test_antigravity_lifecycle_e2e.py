...
``test_per_harness_cursor.py``): it drives real ``omnicraft run`` subprocesses
...
1. **Parallel turns / no state bleed** — two concurrent ``omnicraft run``
   ...
   :class:`~omnicraft.inner.antigravity_executor.AntigravityExecutor`) does not
...
2. **No orphaned ``localharness``** — snapshot the live ``localharness`` PIDs
   ...
   native binary).
3. **Session cleanup reaps the runner/harness** — after a single ``omnicraft
...
- ``google-antigravity`` importable in the *omnicraft* venv (the test shells
...
- A Gemini API key resolvable — either a configured ``antigravity:`` block
  (``omnicraft setup``) or an ambient ``GEMINI_API_KEY`` / ``ANTIGRAVITY_API_KEY``.
  ...
  / ``omnicraft_credentials_env`` — a missing key is a clean SKIP so the e2e
...
- The ``omnicraft run`` one-shot path stops tearing down its local server /
...
# After a clean ``omnicraft run`` exit, the local server/runner shutdown that
...
def _antigravity_prereqs_missing(omnicraft_python: Path) -> str | None:
    """Return a skip reason if the antigravity harness can't run, else ``None``.

    Probes the *omnicraft* venv (the interpreter the subprocess uses), not the
    test's own interpreter: the SDK import and the key resolution both have to
    hold for the spawned ``omnicraft run`` to do anything. Booleans only — the
    key is never printed.

    :param omnicraft_python: Interpreter with omnicraft installed, from the
    ...
    probe = subprocess.run(
        ...
        cwd=str(omnicraft_python.parent),
    )
    out = probe.stdout.strip().split()
    if probe.returncode != 0 or len(out) != 2:
        return (
            "could not probe antigravity prerequisites in the omnicraft venv "
    ...
    if not key_ok:
        return (
            ...
            "configure an 'antigravity:' key via 'omnicraft setup' or export "
...
def _write_antigravity_bundle(bundle_dir: Path, *, name: str) -> Path:
    ...
    :returns: The bundle directory path (the argument ``omnicraft run`` takes).
    """
...
def _antigravity_env() -> dict[str, str]:
    """Build the subprocess env for an antigravity ``omnicraft run``.
...
def _run_antigravity_turn(
    *,
    ...
    """Run one ``omnicraft run <bundle> --harness antigravity -p <prompt>``.
    ...
    :param omnicraft_python: Interpreter with omnicraft installed.
    :param omnicraft_repo_root: Cwd for the subprocess (puts example tool modules
        on sys.path, matching the other run_omnicraft tests).
...
@pytest.fixture
def antigravity_runnable(omnicraft_python: Path) -> None:
    """Skip the whole module's tests when antigravity can't run.

    :param omnicraft_python: Interpreter probed for the SDK + key.
    :raises pytest.skip.Exception: When a prerequisite is missing.
    """
    reason = _antigravity_prereqs_missing(omnicraft_python)
...
def test_antigravity_parallel_turns_no_state_bleed(
    ...
    :param omnicraft_python: Interpreter with omnicraft + google-antigravity.
    :param omnicraft_repo_root: Cwd for the subprocesses.
...
def test_antigravity_session_cleanup_reaps_runner(
    ...
    A single ``omnicraft run`` turn owns a self-contained process tree (local
