...
``/v1/sessions`` flow — register an inline OmniCraft agent
...
:mod:`omnicraft.inner.antigravity_executor`: ``conversation.send`` →
...
from omnicraft.onboarding.antigravity_auth import antigravity_api_key_configured
...
def _antigravity_skip_reason() -> str | None:
    ...
    if importlib.util.find_spec("google.antigravity") is None:
        return (
            ...
            "is not importable. Install it with: pip install 'omnicraft[antigravity]'."
        )
...
def _write_antigravity_agent_yaml(tmp_path: Path, *, prompt: str) -> Path:
    """Write a minimal single-file ``harness: antigravity`` OmniCraft bundle.
...
def _assistant_text_from_items(http_client: httpx.Client, session_id: str) -> str:
    ...
    (see ``omnicraft/server/API.md`` § List Conversation Items).
