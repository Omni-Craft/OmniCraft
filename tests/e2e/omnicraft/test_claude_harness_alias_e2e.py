...
Runs ``omnicraft run hello_world.yaml --harness claude-sdk -p <prompt>``
...
Run with:
    ...
    pytest tests/e2e/omnicraft/test_claude_harness_alias_e2e.py -v --profile oss
"""
...
def _clean_env(profile: str) -> dict[str, str]:
    ...
    # The omnicraft CLI no longer accepts ``--profile``; write the
    # supported replacement — an ``auth:`` block in an isolated
    # ``OMNICRAFT_CONFIG_HOME`` — so the spawned CLI routes the
    # claude-sdk harness through this Databricks profile.
    config_home = Path(tempfile.mkdtemp(prefix="omnicraft-alias-config-"))
...
def test_run_with_claude_alias_produces_output(
    ...
    """``omnicraft run --harness claude-sdk`` exits 0 with assistant text.

    Proves the "claude" alias is canonicalized through the full
    CLI → OmniCraft server → harness spawn → LLM call → output path.
