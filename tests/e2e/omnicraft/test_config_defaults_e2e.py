...
def _bare_env(home: Path, omnicraft_repo_root: Path) -> dict[str, str]:
    """
    Build a minimal subprocess env that doesn't need LLM credentials.

    :param home: Directory to use as ``$HOME``.
    :param omnicraft_repo_root: Worktree root, prepended onto
        ``PYTHONPATH``.
    :returns: Env dict suitable for ``subprocess.run(env=...)``.
    """
    existing_pp = os.environ.get("PYTHONPATH", "")
    pythonpath = os.pathsep.join(p for p in (str(omnicraft_repo_root), existing_pp) if p)
    return {
        "HOME": str(home),
...
def _run_omnicraft(
    *,
    ...
    """Spawn ``python -m omnicraft <args>`` with the given env."""
    return subprocess.run(
        [str(omnicraft_python), "-m", "omnicraft", *args],
        env=env,
...
def test_global_config_write_then_list_roundtrips(
    ...
    home = tmp_path / "home"
    home.mkdir()
    env = _bare_env(home, omnicraft_repo_root)

    write = _run_omnicraft(
...
def test_global_config_default_agent_drives_bare_omnicraft(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    ...
    assert run.returncode == 0, (
        f"bare ``omnicraft run`` with global default_agent failed: "
