"""Structural test for the fucho coding-orchestrator bundle (examples/fucho).
...
# tests/e2e/omnicraft/test_example_fucho.py -> repo root is 3 parents up.
_FUCHO_BUNDLE = Path(__file__).resolve().parents[3] / "examples" / "fucho"


@pytest.fixture(scope="module")
def fucho_spec() -> AgentSpec:
    """Load and validate the fucho bundle once for the module."""
    return load(_FUCHO_BUNDLE)


def test_orchestrator_executor(fucho_spec: AgentSpec) -> None:
    """
    ...
    configured via ``omnicraft setup --no-internal-beta`` (Anthropic key,
    ...
    default. Re-pinning a ``model`` here would re-couple fucho to one
    ...
    """
    assert fucho_spec.name == "fucho"
    ex = fucho_spec.executor
...
def test_pi_subagent_is_headless_scaffold_worker(fucho_spec: AgentSpec) -> None:
    """
    ...
    """
    pi = next(a for a in fucho_spec.sub_agents if a.name == "pi")
...
def test_spine_skills_present(fucho_spec: AgentSpec) -> None:
    """All spine skills are discovered from skills/<name>/SKILL.md."""
    assert sorted(s.name for s in fucho_spec.skills) == [
        "cross-review",
        "fanout",
        "investigate",
...
def test_subagent_dispatch_text_advertises_task_titles_and_purpose(
    fucho_spec: AgentSpec,
) -> None:
    """
    fucho's prompt and workflow examples advertise task titles and purpose.

    If the purpose convention disappears, fucho could spawn a sub-agent with
    ...
    """
    config = (_FUCHO_BUNDLE / "config.yaml").read_text(encoding="utf-8")
    fanout = (_FUCHO_BUNDLE / "skills" / "fanout" / "SKILL.md").read_text(encoding="utf-8")
    cross_review = (_FUCHO_BUNDLE / "skills" / "cross-review" / "SKILL.md").read_text(
        encoding="utf-8"
    )
    investigate = (_FUCHO_BUNDLE / "skills" / "investigate" / "SKILL.md").read_text(
        encoding="utf-8"
    )
    ...
    purpose_guard = next(
        p for p in fucho_spec.guardrails.policies if p.name == "headless_subagent_purpose_guard"
    )
...
def test_orchestrator_keeps_timer_tool_but_forbids_worker_polling(
    ...
    """
    fucho keeps timer tools, but the prompt forbids polling workers with them.
    ...
    assert fucho_spec.async_enabled is True
    assert fucho_spec.timers is True
...
def test_orchestrator_delegates_substantive_work() -> None:
    """
    The base prompt forbids fucho from doing coding work or investigations
    ...
    # ...and fucho's own sys_os_* tools may not be used to write code.
    assert (
...
def test_investigation_skill_delegates_read_only_work() -> None:
    ...
    starts permitting direct file/log/doc inspection by the orchestrator, fucho
...
def test_subagent_cancellation_guidance_present() -> None:
    """
    The orchestrator prompt and fanout skill teach fucho how to stop workers.
    ...
    this guidance disappears, fucho can abandon or re-dispatch work while the
...
def test_orchestrator_guardrails(fucho_spec: AgentSpec) -> None:
    """
    ...
    """
    assert fucho_spec.guardrails is not None
    names = sorted(p.name for p in fucho_spec.guardrails.policies)
    assert names == [
        "blast_radius",
        "headless_subagent_purpose_guard",
        "spawn_bounds",
    ]
    spawn = next(p for p in fucho_spec.guardrails.policies if p.name == "spawn_bounds")
...
def test_function_policies_have_nonempty_arguments(fucho_spec: AgentSpec) -> None:
    """
    Every fucho function-policy supplies a non-empty ``function.arguments``.
    ...
    (omnicraft/policies/function.py). With empty ``arguments: {}`` the factory
    ...
    """
    specs = [fucho_spec, *fucho_spec.sub_agents]
    checked = 0
    for spec in specs:
