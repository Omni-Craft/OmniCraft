...
Runs ``omnicraft run <spec> --harness antigravity ...`` as a real subprocess and
...
does NOT use ``patched_databrickscfg`` / ``omnicraft_credentials_env``'s gateway
URL — it authenticates purely from the configured / ambient Gemini key. This
mirrors :mod:`tests.e2e.omnicraft.test_per_harness_cursor` (the other
...
- ``google.antigravity`` importable in the OmniCraft venv (the ``antigravity``
  extra — ``pip install 'omnicraft[antigravity]'``).
- A Gemini / Antigravity API key configured (a stored ``antigravity:`` config
  block resolvable via
  :func:`omnicraft.onboarding.antigravity_auth.antigravity_api_key_configured`,
...
- ``omnicraft.cli``'s ``run`` one-shot / interactive paths stop persisting the
...
from omnicraft.entities.conversation import MessageData
...
# Subprocess timeout per ``omnicraft run`` invocation. The antigravity SDK boots
...
# ``prompt`` / ``executor``) — the form ``omnicraft run <file>`` accepts without a
...
def _antigravity_skip_reason(omnicraft_python: Path) -> str | None:
    ...
    Both prerequisites are probed in the *OmniCraft venv* interpreter (the one the
    ...
    :param omnicraft_python: Interpreter the ``omnicraft`` subprocess will use.
    ...
    if probe.returncode != 0:
        ...
        return (
            "antigravity prerequisite probe failed in the OmniCraft venv "
    ...
    if not have_sdk:
        return (
            ...
            "not importable in the OmniCraft venv (install the 'antigravity' "
            "extra: pip install 'omnicraft[antigravity]')."
        )
    if not have_key:
        return (
...
def _antigravity_env(base_env: dict[str, str], home: Path) -> dict[str, str]:
    ...
    Starts from the shared ``omnicraft_credentials_env`` (so PATH, the onboarding
    suppression knobs, and the worktree ``PYTHONPATH`` propagate) but isolates
    ``$HOME`` and the OmniCraft state/config roots into the test's temp dir, so
    the persistent conversation store this test reads (``$HOME/.omnicraft/chat.db``)
    ...
    :param base_env: The ``omnicraft_credentials_env`` fixture dict.
...
def _assistant_transcript_texts(db_path: Path) -> list[str]:
    ...
    :mod:`tests.e2e.omnicraft.test_server_remote_omnicraft_autonomous_flows`, but
    filtered to assistant-authored messages so the assertions can't be satisfied
    by the echoed user prompt.

    :param db_path: Path to ``$HOME/.omnicraft/chat.db``.
...
def _run_one_shot(
    *,
    ...
    """Run a one-shot ``omnicraft run <spec> --harness antigravity -p <prompt>``.

    Session-backed (no ``--no-session``) so the turn is persisted to
    ``$HOME/.omnicraft/chat.db`` for transcript inspection and so a later
    ``--continue`` can thread onto it.

    :param omnicraft_python: Interpreter from the ``omnicraft_python`` fixture.
    :param omnicraft_repo_root: Cwd for the subprocess (repo root on sys.path).
...
def _assert_clean_assistant_reply(
    ...
    *,
    ...
    :param result: The completed ``omnicraft run`` process.
...
def test_per_harness_antigravity_smoke(
    ...
    The end-to-end smoke gate: one ``omnicraft run --harness antigravity -p``
    ...
    :param omnicraft_python: Interpreter with omnicraft + the antigravity SDK.
    ...
    """
    skip_reason = _antigravity_skip_reason(omnicraft_python)
    ...
    _assert_clean_assistant_reply(fake_home / ".omnicraft" / "chat.db", result, label="smoke")
...
def test_per_harness_antigravity_model_selection(
    ...
    _assert_clean_assistant_reply(
        fake_home / ".omnicraft" / "chat.db", result, label=f"model={model}"
    )
...
def test_per_harness_antigravity_multi_turn_history_retention(
    ...
    fake_home.mkdir()
    env = _antigravity_env(mock_credentials_env, fake_home)
    db_path = fake_home / ".omnicraft" / "chat.db"
