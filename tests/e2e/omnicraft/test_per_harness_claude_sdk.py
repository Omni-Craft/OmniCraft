...
Runs ``omnicraft run hello_world.yaml --harness claude-sdk -p
...
- OmniCraft' ``ClaudeSDKExecutor`` regresses (auth, MCP tool
...
- The Claude Agent SDK dependency or the ``claude`` CLI binary
  goes missing from the OmniCraft venv.
...
def claude_sdk_available(omnicraft_python: Path) -> bool:
    ...
    *omnicraft* venv — the test's own venv is irrelevant because
    ...
    :param omnicraft_python: The interpreter the subprocess
...
def test_per_harness_claude_sdk_one_shot(
    ...
    """
    ``omnicraft run hello_world.yaml --harness claude-sdk -p
