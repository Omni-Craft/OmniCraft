...
Runs ``omnicraft run hello_world.yaml --harness codex --model
...
- OmniCraft' ``CodexExecutor`` regresses (``codex app-server``
...
def test_per_harness_codex_one_shot(
    ...
    """
    ``omnicraft run hello_world.yaml --harness codex -p <prompt>``
