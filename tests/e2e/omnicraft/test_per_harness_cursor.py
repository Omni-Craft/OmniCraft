...
Runs ``omnicraft run hello_world.yaml --harness cursor -p "..."`` as a real
...
``patched_databrickscfg`` / ``omnicraft_credentials_env``. Because a Cursor API
...
- The ``cursor-sdk`` API contract changes (``AsyncAgent`` / ``AsyncClient`` /
  ``run.messages()`` shape).
- ``omnicraft.cli`` for the ``-p`` one-shot path stops printing assistant text
...
def test_per_harness_cursor_one_shot(
    ...
    """``omnicraft run hello_world.yaml --harness cursor -p <prompt>`` works.

    :param omnicraft_python: Interpreter with omnicraft installed and importable.
    :param omnicraft_repo_root: Cwd for the subprocess so the YAML spec and
