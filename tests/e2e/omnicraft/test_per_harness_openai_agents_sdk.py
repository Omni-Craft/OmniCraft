...
Runs ``omnicraft run hello_world.yaml --harness openai-agents
...
- OmniCraft' ``OpenAIAgentsSDKExecutor`` regresses (the Runner
  lifecycle, the Responses-API adapter in
  ``omnicraft.open_responses_sdk``, the MCP tool bridging, or
  the event stream translation to ``ExecutorEvent`` types).
- The ``openai-agents`` Python package (``agents`` module) is
  missing from the omnicraft venv or its public API changes
  incompatibly.
- ``omnicraft.cli._run_agent`` for the ``-p`` one-shot path
...
@pytest.fixture
def openai_agents_available(omnicraft_python: Path) -> bool:
    """
    ...
    *omnicraft* venv (the subprocess interpreter) — the current
    pytest interpreter is irrelevant because the test shells out.

    :param omnicraft_python: Interpreter the subprocess will
...
def test_per_harness_openai_agents_sdk_one_shot(
    ...
    """
    ``omnicraft run hello_world.yaml --harness openai-agents -p
    ...
    :param openai_agents_available: True when the ``agents``
        package is importable in the omnicraft venv. On False
    ...
    if not openai_agents_available:
        pytest.skip(
            ...
            "installed in the OmniCraft venv. Skipping — package absent."
        )
