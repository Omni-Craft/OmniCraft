...
Runs ``omnicraft run hello_world.yaml --harness pi --model
...
- OmniCraft' ``PiExecutor`` regresses (the ``pi --mode rpc``
  ...
  OmniCraft tools with ``pi.registerTool()``).
...
def test_per_harness_pi_one_shot(
    ...
    """
    ``omnicraft run hello_world.yaml --harness pi -p <prompt>``
