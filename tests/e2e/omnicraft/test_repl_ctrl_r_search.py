...
- ``omnicraft.cli`` removes the ``@kb.add("c-r")`` binding that
  delegates to prompt-toolkit's
  ``start_reverse_incremental_search``.
- ``omnicraft.cli`` forgets to bind Enter while searching to
...
def test_repl_ctrl_r_reverse_search(
    ...
    :param omnicraft_repo_root: Working directory for the
        subprocess.
