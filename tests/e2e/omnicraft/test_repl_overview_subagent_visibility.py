...
def _check_worker_harness_available(harness: str, omnicraft_python: Path) -> None:
    """
    Fail loud if the worker harness's prerequisites are missing.

    :param harness: The worker harness identifier under test.
    :param omnicraft_python: The subprocess interpreter.
    """
...
def test_repl_overview_subagent_visibility(
    ...
    _check_worker_harness_available(harness, omnicraft_python)
    ...
    yaml_path = omnicraft_repo_root / "tests" / "resources" / "examples" / "coding_supervisor.yaml"
