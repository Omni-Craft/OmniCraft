"""Live REPL e2e for ``omnicraft run --harness`` without AGENT.

Migrated to use the mock LLM server. This test drives the user-facing
launcher shape::

    omnicraft run --harness <harness> -p <prompt>
...
def test_run_harness_without_agent_live_repl_round_trip(
    ...
    """``omnicraft run --harness`` boots and answers via each wrapped harness.
...
def test_run_harness_live_matrix_covers_registered_coding_harnesses() -> None:
    ...
    ``OMNICRAFT_HARNESSES`` also contains ``open-responses`` for the
    ...
    ``omnicraft run --harness <native>``. (``opencode-native`` is a
    ...
    (tmux pane + bridge dir), not ``omnicraft run --harness goose-native``.

    ``antigravity-native`` is excluded for the union of both reasons above: it
    is a terminal-first TUI launched via ``omnicraft antigravity`` (runner-owned
    agy tmux pane + bridge dir), not ``omnicraft run --harness antigravity-native``,
    ...
    not ``omnicraft run --harness qwen-native``. Its coverage is the dedicated
    ...
    ``omni kiro`` (tmux pane + bridge dir), not ``omnicraft run --harness
    ...
    ``omni kimi`` (tmux pane + bridge dir), not ``omnicraft run --harness
    ...
    ``omnicraft run --harness hermes-native``, AND it wraps the ``hermes`` CLI
    binary. Its coverage is the dedicated hermes-native bridge/executor/forwarder/
    approval-mirror unit tests.
    """
    expected_live_harnesses = set(OMNICRAFT_HARNESSES).intersection(_HARNESS_MODULES) - {
        "acp",
        "claude-native",
        "codex-native",
        "pi-native",
