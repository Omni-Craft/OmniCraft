"""End-to-end test: ``omnicraft run examples/coding_supervisor.yaml
--omnicraft`` works.

Exercises the full pipeline that was failing in user-reported bugs:

1. OmniCraft YAML with ``async: true``, ``cancellable: true``, and
...
2. Non-interactive ``run -p <prompt>`` path — POSTs to
   ``/v1/responses`` on an in-process omnicraft server.
3. The bidirectional translator: YAML → AgentDef →
   :func:`agent_def_to_agent_spec` → AgentSpec → (registered with
   omnicraft server) → :func:`agent_spec_to_agent_def` (inside
   :class:`OmniCraftExecutor.from_spec`) → AgentDef → omnicraft
...
- Someone reintroduces a fail-loud for ``AgentTool`` in the
  ...
  sub-agent-as-tool declarations stop loading under OmniCraft mode.
- ``_run_agent_via_omnicraft`` regresses to the "requires a prompt"
  hard-error path — interactive ``omnicraft run <yaml>``
...
- The
  :func:`omnicraft.spec.omnicraft._sub_spec_to_agent_tool`
  reverse translation stops emitting ``AgentTool`` entries —
  :class:`OmniCraftExecutor.from_spec` reconstructs an
...
from tests.e2e.omnicraft.conftest import configure_mock_llm, reset_mock_llm
...
# OmniCraft mode boot (FastAPI + uvicorn + DBOS + alembic) completes
...
# Cold-boot of ``coding_supervisor.yaml`` under OmniCraft mode —
# spawns the in-process OmniCraft server and registers supervisor +
# two sub-agents. Without OmniCraft mode boot is <10s; with OmniCraft mode
# the in-process FastAPI + uvicorn + DBOS + alembic stack adds
# ~30-60s on a cold DBOS db. 120s keeps the test from flaking
# on cold starts. (Genuine regressions in OmniCraft mode boot would
...
_EXPECTED_ROOT_ENTRIES = ("openapi.json", "omnicraft", "pyproject.toml")


def test_run_omnicraft_coding_supervisor_oneshot(
    ...
    """
    ``omnicraft run examples/coding_supervisor.yaml -p ...``
    ...
    :param omnicraft_python: Interpreter with both omnicraft and
        omnicraft installed (from the shared conftest).
    :param omnicraft_repo_root: OmniCraft repo root. Used as cwd
    ...
    yaml_path = omnicraft_repo_root / _YAML_PATH_REL
    ...
    result = subprocess.run(
        [
            ...
            # test_run_omnicraft_example_agents.py for the
    ...
    assert result.returncode == 0, (
        f"`omnicraft run --omnicraft` exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1000:]}"
    )
...
def test_run_omnicraft_coding_supervisor_exposes_subagent_tools(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    ``codex_worker``) AND at least one omnicraft task-lifecycle
    builtin (``check_task``) — proves :class:`OmniCraftExecutor`
    advertises both surfaces to the inner omnicraft harness.
    ...
    :param omnicraft_python: Interpreter with omnicraft +
        omnicraft installed.
    :param omnicraft_repo_root: OmniCraft repo root.
    :param mock_credentials_env: Mock-LLM env vars pointing at the
        mock server.
    :param mock_llm_server_url: Mock server URL for configuring
        response queues.
...
def test_run_omnicraft_coding_supervisor_spawns_codex_worker_to_list_files(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: ``omnicraft run`` on
    coding_supervisor.yaml boots the OmniCraft stack, the mock
    ...
    is that the OmniCraft boot path, sub-agent tool registration,
    ...
    :param omnicraft_python: Shared session fixture pointing at
        the repo's ``.venv`` Python.
    :param omnicraft_repo_root: OmniCraft repo root — used as
    ...
    configure_mock_llm(
        mock_llm_server_url,
        [{"text": "openapi.json omnicraft pyproject.toml"}],
    )
    ...
    for entry in _EXPECTED_ROOT_ENTRIES:
        assert (omnicraft_repo_root / entry).exists(), (
            f"Expected filesystem anchor {entry!r} missing from "
            f"{omnicraft_repo_root}. Update _EXPECTED_ROOT_ENTRIES "
...
def test_run_omnicraft_coding_supervisor_interactive_enters_repl(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
) -> None:
    """
    ``omnicraft run examples/coding_supervisor.yaml`` (no
    ...
    :func:`_run_chat_via_omnicraft` so interactive semantics match the
    legacy ``omnicraft run`` no-prompt behavior.

    :param omnicraft_python: Interpreter with omnicraft +
        omnicraft installed.
    :param omnicraft_repo_root: Cwd for the subprocess.
    :param mock_credentials_env: Mock-LLM env vars pointing at the
        mock server.
    """
    yaml_path = omnicraft_repo_root / _YAML_PATH_REL
    ...
    # Use the shared ``spawn_omnicraft_run`` harness so PTY
    ...
    try:
        ...
        assert index == 0, (
            f"`omnicraft run --omnicraft` (no prompt) did not reach the "
...
@pytest.mark.flaky(reruns=2, reruns_delay=5)
def test_run_omnicraft_coding_supervisor_codex_shell_not_disabled(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: under OmniCraft mode, the codex
    ...
    validates is that the infrastructure plumbing (OmniCraft mode
    ...
    ``shell_tool`` whenever any tools were passed. Under OmniCraft
    mode, :class:`OmniCraftExecutor` always injects omnicraft
    ...
    :param omnicraft_python: Interpreter with omnicraft +
        omnicraft installed.
    :param omnicraft_repo_root: OmniCraft repo root — also the
    ...
    fixture_path = omnicraft_repo_root / _CODEX_REGRESSION_TODO_NAME
    ...
    assert _CODEX_NONEXISTENT_ERROR not in combined, (
        ...
        f"omnicraft/codex_executor.py.\n"
        f"stdout tail:\n{result.stdout[-2500:]}\n"
        f"stderr tail:\n{result.stderr[-1500:]}"
    )
    assert result.returncode == 0, (
        f"--omnicraft exited {result.returncode}. stderr tail:\n"
