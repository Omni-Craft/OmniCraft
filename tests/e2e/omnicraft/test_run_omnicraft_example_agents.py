"""
End-to-end: every example YAML the OmniCraft adapter is
*expected* to accept can actually boot and execute under
``omnicraft run -p <prompt>``.
...
The companion file
``test_run_omnicraft_adapter_rejections.py`` covers YAMLs the adapter
...
- The OmniCraft mode CLI shim loses a dispatch for a harness.
...
LLM nondeterminism) are tracked in ``TODO_omnicraft_coverage.md`` in
...
_CASES = [
    ...
    pytest.param(
        ...
        ("pyproject.toml", "README.md", "omnicraft", "examples"),
...
def test_run_omnicraft_example_yaml(
    ...
    """
    Drive one example YAML under ``omnicraft run -p <prompt>``
    and assert the agent exercised the declared capability.

    :param omnicraft_python: Shared interpreter fixture.
    :param omnicraft_repo_root: Repo root — subprocess cwd so
    ...
    :param success_markers: Any-one-of substrings that MUST
        ...
        reply traversed the full OmniCraft mode stack. The mock
    ...
    args = [
        ...
        # ``~/.omnicraft/chat.db`` and DBOS may attempt to
        ...
        # ``HarnessProcessManager not initialized — OmniCraft lifespan
    ...
    assert result.returncode == 0, (
        f"{yaml_rel}: --omnicraft exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1500:]}"
    )
