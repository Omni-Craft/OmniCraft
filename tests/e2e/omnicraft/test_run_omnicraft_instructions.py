...
- ``omnicraft/inner/loader.py::_resolve_instructions`` regresses.
- ``omnicraft/spec/omnicraft.py::agent_def_to_agent_spec`` stops
  preferring ``AgentDef.instructions`` over ``AgentDef.prompt``.
- ``omnicraft/spec/_omnicraft_compat.py::is_omnicraft_yaml``
...
def _argv_run_omnicraft(
    *,
    ...
    """Build the ``omnicraft run -p`` argv."""
    return [
...
def test_instructions_path_field_loaded_via_omnicraft_run_omnicraft(
    omnicraft_python: Path,
    ...
    ``omnicraft run`` and the agent starts successfully.
...
"""
    ...
    assert result.returncode == 0, (
        f"omnicraft run exited {result.returncode}. "
