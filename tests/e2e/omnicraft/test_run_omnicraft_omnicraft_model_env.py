"""E2E coverage for the ``OMNICRAFT_MODEL`` env-var fallback on ``omnicraft run``.

The fallback fires in ``omnicraft/chat.py:_apply_overrides_to_raw`` when the
...
#    ``omnicraft.spec.validator._validate_executor_llm``; any other prefix
#    rejects the YAML before any FM API call happens.
# 2. ``databricks-gpt-`` routes through ``omnicraft.llms.routing.infer_
...
_BOGUS_MODEL = "databricks-gpt-this-model-does-not-exist-omnicraft-env-test-9f3a"

_PROMPT = "say hi in 5 words"
# Wall-clock budget for the subprocess. ``omnicraft run`` spawns the
...
# ``test_omnicraft_model_env_var_bogus_value_fails_with_named_error``).
...
def _run_omnicraft_with_model_env(
    *,
    model_env_value: str,
    ...
    """
    Run ``omnicraft run <minimal>.yaml -p "..."`` with ``OMNICRAFT_MODEL`` set.
    ...
    because ``omnicraft run`` spawns the AP server + runner as
    ...
    :param model_env_value: ``OMNICRAFT_MODEL`` value (real or bogus).
    ...
    env["OMNICRAFT_MODEL"] = model_env_value
...
def test_omnicraft_model_env_var_drives_successful_run(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    ...
    """
    Smoke test: a valid model in ``OMNICRAFT_MODEL`` produces a successful turn.
    ...
    :param omnicraft_repo_root: Repo root (subprocess cwd).
    ...
    result = _run_omnicraft_with_model_env(
    ...
    assert result.returncode == 0, (
        f"omnicraft run with OMNICRAFT_MODEL={_VALID_MODEL!r} exited "
...
def test_omnicraft_model_env_var_bogus_value_fails_with_named_error(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Decisive test: a bogus ``OMNICRAFT_MODEL`` fails with the bogus name in stderr.
    ...
    assert result.returncode != 0, (
        f"omnicraft run with OMNICRAFT_MODEL={_BOGUS_MODEL!r} unexpectedly "
