"""
End-to-end proof that policies declared in an omnicraft YAML
are enforced by the omnicraft workflow under OmniCraft mode.

The adapter in :mod:`omnicraft.spec.omnicraft` lifts the
YAML's ``policies:`` block into
:attr:`AgentSpec.guardrails.policies`; the omnicraft runtime
...
the legacy omnicraft ``(content, phase)`` callable signature
(``examples.tool_functions.block_long_sleep`` et al.), which
OmniCraft' :class:`FunctionPolicy` dispatcher can't invoke
(it passes ``(ctx, context)`` where ``ctx`` is an
:class:`EvaluationContext` dataclass, not a dict). This test
uses the omnicraft-shaped
``omnicraft._e2e_policy_callables.block_on_sentinel``
callable — an arity-1 callable matching OmniCraft'
convention — so the test proves the translator + engine
integration works and isn't muddied by a separate callable-
portability gap. That gap is tracked in ``TODO_omnicraft_coverage.md``.
...
- The runtime stops reading ``guardrails`` from specs
  synthesized via the omnicraft adapter.
...
# in ``omnicraft/_e2e_policy_callables.py`` DENYs on. The token
...
# The standard DENY sentinel text the omnicraft workflow
# stamps into the response when a policy returns DENY. See
# :func:`omnicraft.runtime.workflow._build_deny_sentinel` —
...
def policy_enforcement_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an omnicraft-shaped YAML registering one
    function policy on the ``input`` phase pointing at the
    omnicraft e2e callable.
    ...
    (``block_on_sentinel``) is already on ``omnicraft`` and
    matches the omnicraft FunctionPolicy calling convention.
        ...
        config = {
            ...
            "policies": {
                "block_sentinel_input": {
                    "type": "function",
                    "on": ["request"],
                    "handler": ("omnicraft._e2e_policy_callables.block_on_sentinel"),
                },
            },
        }
...
def test_policy_denies_input_containing_sentinel(
    ...
    :param omnicraft_repo_root: Subprocess cwd — the YAML's
    ...
    :param policy_enforcement_yaml_factory: Builder for the
        harness-specific omnicraft YAML.
    ...
    assert result.returncode == 0, (
        f"--omnicraft exited {result.returncode} before reaching "
...
def tool_ban_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an omnicraft-shaped YAML declaring one
    ...
    - ``OmniCraftExecutor._make_tool_executor_bridge``
...
def test_policy_denies_tool_call_by_name(
    ...
    """
    ``omnicraft run <yaml> -p "<arithmetic prompt>"``
    ...
    1. The translator expanded ``on: [tool_call] + match_tools:
       ...
       name.
    2. ``OmniCraftExecutor._make_tool_executor_bridge``
    ...
    - The OmniCraftExecutor bridge stopped calling
    ...
    :param omnicraft_python: Shared interpreter fixture.
    :param omnicraft_repo_root: Subprocess cwd — conftest's
