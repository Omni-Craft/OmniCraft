"""End-to-end: ``omnicraft run`` starts the REPL without leaking
...
- ``_quiet_omnicraft_server_logging`` stops suppressing DBOS /
...
from tests.e2e.omnicraft._repl_test_helpers import drain_for
...
def test_run_omnicraft_startup_does_not_leak_server_logs(
    ...
    assert not leaked, (
        f"``omnicraft run`` leaked server boot output onto "
