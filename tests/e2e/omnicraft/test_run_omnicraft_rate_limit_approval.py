...
flow under ``omnicraft run``.

Reproduces the user-reported scenario verbatim:

1. Spawn ``omnicraft run examples/rate_limited_search_agent.yaml``
...
def test_run_omnicraft_rate_limit_approval_round_trip(
    ...
    :param omnicraft_python: Path to the worktree's
        ``.venv/bin/python``.
    :param omnicraft_repo_root: Repo root the subprocess uses as
    ...
    yaml_path = omnicraft_repo_root / _YAML_REL
    ...
    try:
        ...
        # :func:`omnicraft.repl._repl._make_elicitation_prompt`'s
