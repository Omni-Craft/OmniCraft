"""End-to-end tests for ``omnicraft run`` conversation resumption.
...
LLM in run #2, proving that the persistent omnicraft store at
``$HOME/.omnicraft/chat.db`` carries history between invocations.

**What breaks if this fails:**

- The persistent store path regresses in
  ``omnicraft.chat._omnicraft_persistent_dir`` or
  ``omnicraft.inner.cli._build_omnicraft_stores`` — e.g. someone
  flips back to ``mkdtemp`` and ``--continue`` silently
  starts a fresh conversation.
- Idempotent agent registration regresses
  (``_omnicraft_register_yaml_bundle``) — the second subprocess
...
``designs/RUN_OMNICRAFT_SESSION_RESUMPTION.md`` feature. If it
...
# Subprocess timeout per ``omnicraft run`` invocation.
# 180s matches the existing run_omnicraft tests' headroom for DBOS
...
def _argv_run_omnicraft(
    *,
    ...
    """
    Build the ``omnicraft run`` argv for a one-shot ``-p`` invocation.

    Use for plant steps (no resumption flags). Resume steps need
    :func:`_argv_run_omnicraft_interactive` instead, since the CLI rejects
    ``--continue`` / ``--resume`` combined with ``-p``.

    :param omnicraft_python: Interpreter from the
        ``omnicraft_python`` fixture.
    :param omnicraft_repo_root: Repo root from the
        ``omnicraft_repo_root`` fixture.
...
def _argv_run_omnicraft_interactive(
    *,
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    extra_flags: list[str],
    ...
    """
    Build the ``omnicraft run`` argv for the interactive REPL.
    ...
    :param omnicraft_python: Interpreter from the fixture.
    :param omnicraft_repo_root: Repo root from the fixture.
...
def _daemon_log_tails(home: Path, *, tail_chars: int = 3000) -> str:
    ...
    Each ``omnicraft run`` subprocess spawns its own local server, host
    daemon, and runner whose logs land under ``$HOME/.omnicraft/logs/``
...
def _isolated_env(
    ...
    """
    Override ``HOME`` and the explicit OmniCraft state/config roots so
    ...
    developer's real ``~/.omnicraft/chat.db`` and could
    ...
    env = dict(base_env)
    env["HOME"] = str(home)
    env["OMNICRAFT_CONFIG_HOME"] = str(home / ".omnicraft")
    env["OMNICRAFT_DATA_DIR"] = str(home / ".omnicraft")
    return env


def test_run_omnicraft_continue_carries_history_across_invocations(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    ...
    persistent_db = fake_home / ".omnicraft" / "chat.db"
    assert persistent_db.is_file(), (
        f"Persistent store was not created at {persistent_db}. "
        f"Run #1 didn't write to ``~/.omnicraft/chat.db`` — "
        f"either ``--no-session`` slipped in, or "
        f"``_omnicraft_persistent_dir`` regressed."
    )
    ...
    result2 = subprocess.run(
        _argv_run_omnicraft_interactive(
            omnicraft_python=omnicraft_python,
            omnicraft_repo_root=omnicraft_repo_root,
...
def test_run_omnicraft_continue_with_no_prior_conversation_exits_nonzero(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
...
def test_run_omnicraft_continue_works_across_oneshot_and_interactive_paths(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    (``test_run_omnicraft_continue_carries_history_across_invocations``)
    couldn't catch it because both subprocesses there go
    through ``_omnicraft_register_yaml_bundle``.
    ...
    interactive_argv = [
        ...
        "omnicraft",
        "run",
        str(omnicraft_repo_root / "tests" / "resources" / "examples" / "hello_world.yaml"),
        "--model",
        _MODEL,
        "--harness",
        _HARNESS,
...
def test_run_omnicraft_session_id_pins_the_specific_conversation(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    *exact* conversation specified, not the most-recent one.
    ...
    # column is a SMALLINT; see omnicraft.db.enum_codecs.CONVERSATION_KIND).
...
def test_run_omnicraft_session_id_unknown_exits_nonzero(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
...
def test_run_omnicraft_no_session_does_not_pollute_persistent_store(
    omnicraft_python: Path,
    omnicraft_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
    ...
    the persistent ``$HOME/.omnicraft/chat.db`` must NOT
    ...
    # ``_omnicraft_persistent_dir`` regardless of
    ...
    assert not persistent_db.exists(), (
        ...
        f"Either the ephemeral branch in _build_omnicraft_stores "
        f"is no longer being honored, or _omnicraft_persistent_dir "
