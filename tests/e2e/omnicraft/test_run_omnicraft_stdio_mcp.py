"""Live E2E: ``omnicraft run`` calls a stdio MCP tool.

Proves the full path landed by the stdio-MCP work:

    YAML  →  omnicraft OmniCraft translator
          ...
          stdio, through the harness, through the OmniCraft mode reply
...
- The OmniCraft translator silently drops the
  :class:`~omnicraft.inner.tools.MCPTool`: the spec loads, but
  the omnicraft runtime never registers the tool, the LLM
  never calls it, and the ``echo: <probe>`` fingerprint is
  absent. The unit test
  (``test_omnicraft_adapter.py::test_load_mcp_stdio_yaml_
...
# existing OmniCraft mode example-agent tests use 240s; shorter here
...
def _write_stdio_mcp_yaml(tmp_path: Path, repo_root: Path) -> Path:
    """
    Materialize an omnicraft agent YAML that declares the
    ...
    :param repo_root: OmniCraft repo root — used to resolve the
    ...
    agent = {
        "name": "echo_omnicraft_stdio_mcp_test",
        "prompt": (
    ...
    yaml_path = tmp_path / "echo_omnicraft_stdio.yaml"
    yaml_path.write_text(yaml.dump(agent))
    return yaml_path


def test_omnicraft_stdio_mcp_tool_roundtrip(tmp_path: Path) -> None:
    """
    Run ``omnicraft run <yaml>`` with a stdio MCP; the
    ...
    # ``omnicraft_repo_root`` session fixture in conftest.py
    ...
    # same ``openai-agents`` / ``omnicraft`` install the test is
    ...
    # Ditto for Databricks env vars the OmniCraft mode shim might
    ...
    # mistake the ask for one of OmniCraft' auto-registered
    ...
    # OmniCraft path has historically swallowed a few of these
    # (see designs/OMNICRAFT_INTEGRATION.md gap log), so assert
    # up front.
    forbidden = (
    ...
    for marker in forbidden:
        assert marker not in combined, (
            f"Forbidden marker {marker!r} in output — a --omnicraft "
            f"failure mode fired. stderr tail:\n{result.stderr[-2000:]}"
        )

    assert result.returncode == 0, (
        f"--omnicraft exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-2000:]}"
    )
