...
What can break this without breaking the lower layers:
  ...
  - The OmniCraft mode SSE bridge translates tool-result events into
...
def test_run_omnicraft_url_linkify_emits_osc_8_in_pty(
    ...
    1. Spawn ``omnicraft run --omnicraft`` via pexpect.
    ...
    :param omnicraft_python: Path to the worktree's
        ``.venv/bin/python``.
    :param omnicraft_repo_root: Repo root used as cwd so the
        subprocess imports this worktree's ``omnicraft``
    ...
    # Short top-level tmpdir so the OmniCraft server's per-run
    # subdirs (incl. tmux socket paths if the agent ever
    # touched one) stay under macOS's 104-char Unix-socket
    # limit. Same shape as test_run_omnicraft_terminal_idle.py.
    ...
    try:
        ...
        # On failure, leave the tmpdir intact so the OmniCraft server
        # log is inspectable.
...
def _assert_url_was_linkified(captured: str) -> None:
    ...
    assert bare_count == 0, (
        ...
        f"``sdks/ui/omnicraft_ui_sdk/terminal/_host.py`` for "
