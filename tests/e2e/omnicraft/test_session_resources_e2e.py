"""E2E test -- Session Resources API against ``omnicraft server`` (mock LLM).

Boots ``omnicraft server --agent <yaml>`` as a subprocess and
...
def _omnicraft_server(
    *,
    ...
    """Spawn ``omnicraft server --agent <agent_path>`` and a sibling runner.

    :param python: Interpreter with omnicraft installed.
    ...
    runner_proc = subprocess.Popen(
        [str(python), "-m", "omnicraft.runner._entry"],
        env={
            **env,
...
def test_session_resources_e2e(
    ...
    with _omnicraft_server(
        ...
        with httpx.Client(
            ...
            # OmniCraft REPL terminal (``terminal_tui_main``) on bind;
