"""End-to-end regression test: ``omnicraft claude --resume`` restores history.

Reproduces the user-reported bug: running

    omnicraft claude --server <url> --resume <conv_id>
...
This is the robust, outcome-based check: it drives the REAL ``omnicraft
...
def test_claude_native_cli_resume_restores_history(
    ...
    """
    Cross-context ``omnicraft claude --resume`` restores conversation history.

    Drives the real ``omnicraft claude --server …`` CLI (gateway routing
    ...
    and must go through OmniCraft' cold-resume *synthesis* (rebuild the
    ...
    that only exercises Claude resuming its own file, not OmniCraft' code, and
