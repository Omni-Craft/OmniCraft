"""End-to-end regression test: ``omnicraft codex --resume`` restores history.
...
guard/confirmation rather than a fix — it pins that ``omnicraft codex
...
def test_codex_native_cli_resume_restores_history(
    ...
    Drives ``omnicraft codex --server …`` (gateway routing via the
