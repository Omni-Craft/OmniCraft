"""
PTY-driven e2e: ``omnicraft run --harness <harness>`` returns
...
the REPL's SSE rendering (event consumption from the OmniCraft server
...
def repl_env(mock_llm_server_url: str, tmp_path: Path) -> dict[str, str]:
    ...
    :param tmp_path: Isolated temp directory for OMNICRAFT_CONFIG_HOME.
    :returns: Env mapping for ``pexpect.spawn``.
    """
    from tests.e2e.omnicraft._pexpect_harness import ensure_repl_test_theme_env

    config_home = tmp_path / "omnicraft-config"
    config_home.mkdir()
...
def test_repl_run_routes_harness_through_new_harness_contract(
    ...
    """
    Drive the full ``omnicraft run --harness <harness>``
    ...
    2. It spawns a local OmniCraft server subprocess.
    3. It uploads the spec via ``/api/agents``.
    4. The OmniCraft server's ``_create_executor`` sees an
       ``executor.type == "omnicraft"`` +
       ``config.harness == <harness>`` spec (after the
       omnicraft-YAML translator runs) and dispatches to
...
def test_repl_pexpect_dependencies_are_present() -> None:
    ...
    # Sanity that ``omnicraft`` is importable from this
