"""Small mock-LLM e2e smoke for the fucho coding orchestrator (examples/fucho).

Mock mode: boots a throwaway LOCAL server from this working tree (which carries
the in-tree ``omnicraft.inner.nessie.policies`` module that fucho's guardrails
resolve server-side), rewrites the fucho bundle's executor to use
``openai-agents`` harness wired to the mock LLM server, and runs a one-shot
``omnicraft run`` subprocess against it. This exercises the parts a structural
...
Why a local server (not bare ``omnicraft run``): fucho's guardrail policies
(``omnicraft.inner.nessie.policies`` — the package keeps its historical
name) are resolved SERVER-SIDE when the workflow executes. Bare ``omnicraft
run`` routes to the developer's configured default server (the shared
``omnicraft`` prod app), which may not carry the in-tree policy module, so
the turn 500s at event-execution. We therefore stand up a throwaway local
``omnicraft server`` from this working tree - which DOES carry the fucho
...
cost-advisor and subagent-model tests can re-use them directly:
    ...
    from tests.e2e.test_fucho_e2e import (
        ...
        _wait_for_health,
        _mock_env,
        _mock_fucho_spec_dir,
    )

Run it manually after touching the fucho bundle, its skills, or the
openai-agents auth paths::

    pytest tests/e2e/test_fucho_e2e.py -v
"""
...
# tests/e2e/test_fucho_e2e.py -> repo root is 2 parents up.
_REPO = Path(__file__).resolve().parents[2]
_FUCHO = _REPO / "examples" / "fucho"
...
# Model key for the fucho brain in mock mode. The mock server routes responses
...
_MOCK_BRAIN_MODEL = "mock-fucho-brain"
...
def _mock_env(mock_llm_server_url: str) -> dict[str, str]:
    ...
    ``OMNICRAFT_CONFIG_HOME`` prevents the spawned process from touching
    the developer's real omnicraft state.
    ...
    config_home = Path(tempfile.mkdtemp(prefix="omnicraft-fucho-mock-config-"))
...
def _mock_fucho_spec_dir(
    tmp_path: Path,
    mock_llm_server_url: str,
    *,
    brain_model: str = _MOCK_BRAIN_MODEL,
    extra_executor_config: dict | None = None,
    fucho_src: Path = _FUCHO,
    rewrite_sub_agent_harnesses: bool = False,
) -> Path:
    """
    Copy the fucho bundle into *tmp_path* and rewrite it to use the mock LLM.
    ...
    :param fucho_src: Source fucho bundle directory; defaults to the
        shipped ``examples/fucho``.
    ...
    dst = tmp_path / "fucho"
    shutil.copytree(fucho_src, dst, symlinks=False)
...
@pytest.fixture
def local_fucho_server(tmp_path: Path) -> Iterator[str]:
    """
    Start a throwaway local ``omnicraft server`` from this working tree.

    The server carries the in-tree ``omnicraft.inner.nessie.policies`` module
    that fucho's guardrails resolve server-side, so the workflow doesn't 500
    ...
    db_uri = f"sqlite:///{tmp_path / 'fucho_e2e.db'}"
    artifacts = tmp_path / "artifacts"
    import os

    env = {
        **os.environ,
        "OMNICRAFT_SKIP_ONBOARD": "1",
        "OMNICRAFT_NO_UPDATE_CHECK": "1",
    }
...
def test_fucho_orchestrator_boots_and_responds(
    local_fucho_server: str,
    mock_llm_server_url: str,
    tmp_path: Path,
) -> None:
    """
    ``omnicraft run <mock-fucho> --server <local> -p <prompt>``
    exits 0 and emits a non-trivial reply via the mock LLM server.

    Proves the bundle loads end-to-end against a server that carries fucho's
    ...
    :param local_fucho_server: Base URL of the in-tree local server fixture.
    :param mock_llm_server_url: Base URL of the mock LLM server fixture.
    :param tmp_path: Per-test temp dir for the mock fucho spec copy.
    """
    from tests.e2e.conftest import configure_mock_llm, reset_mock_llm

    reset_mock_llm(mock_llm_server_url)
    fucho_dir = _mock_fucho_spec_dir(tmp_path, mock_llm_server_url)
    configure_mock_llm(
        mock_llm_server_url,
        [
            {
                "text": (
                    "I am fucho, a multi-agent coding orchestrator. "
    ...
    assert result.returncode == 0, (
        f"fucho run exited {result.returncode}\n--- stdout ---\n{result.stdout}\n"
        f"--- stderr ---\n{result.stderr}"
    )
    reply = result.stdout.strip()
    # A real reply, not an empty turn.
    assert len(reply) >= _MIN_REPLY_CHARS, (
        f"fucho produced no/short reply ({len(reply)} chars): {reply!r}\n"
        f"--- stderr ---\n{result.stderr}"
    )
