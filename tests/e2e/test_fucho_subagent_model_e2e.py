"""Mock-LLM e2e for per-dispatch sub-agent model control on fucho.

Boots a throwaway LOCAL server from this working tree and drives the fucho
...
Run::
    ...
    pytest tests/e2e/test_fucho_subagent_model_e2e.py -v
"""
...
# tests/e2e/test_fucho_subagent_model_e2e.py -> repo root is 2 parents up.
...
def local_fucho_server(tmp_path: Path) -> Iterator[str]:
    ...
    Mirrors ``test_fucho_e2e.local_fucho_server`` (own sqlite DB + artifact
    ...
    proc = subprocess.Popen(
        [
            ...
            str(port),
            "--database-uri",
            f"sqlite:///{tmp_path / 'fucho_model_e2e.db'}",
            "--artifact-location",
...
def _run_fucho_turn(
    ...
    *,
    ...
    """
    Run one headless fucho turn against the local server.
    ...
    :param fucho_dir: The fucho bundle to run.
    :returns: The completed ``omnicraft run`` process.
    """
...
def _fucho_parent_id(base_url: str) -> str:
    """
    Find the fucho parent session on the throwaway server.

    The server DB is per-test, so the only fucho session is ours.

    :param base_url: Local server base URL.
    :returns: The parent conversation id.
    """
    sessions = _api(base_url, "/v1/sessions").get("data", [])
    parents = [s["id"] for s in sessions if s.get("agent_name") == "fucho"]
    assert parents, f"no fucho session found among {len(sessions)} sessions"
    return parents[0]


def test_fucho_dispatches_distinct_models_per_worker(
    ...
    fucho_dir = _mock_fucho_spec_dir(
    ...
    parent = _fucho_parent_id(local_fucho_server)
    kids = _api(local_fucho_server, f"/v1/sessions/{parent}/child_sessions").get("data", [])
    ...
    for k in kids:
        ...
        snap = _api(local_fucho_server, f"/v1/sessions/{child_id}")
...
def test_fucho_rejects_cross_family_model_dispatch(
    local_fucho_server: str,
    mock_llm_server_url: str,
    tmp_path: Path,
) -> None:
    ...
    parent = _fucho_parent_id(local_fucho_server)
    items = _api(local_fucho_server, f"/v1/sessions/{parent}/items").get("data", [])
...
def test_fucho_lists_models_then_dispatches_pi_from_list(
    local_fucho_server: str,
    mock_llm_server_url: str,
    tmp_path: Path,
) -> None:
    ...
    override = _api(local_fucho_server, f"/v1/sessions/{child_id}").get("model_override")
...
def test_fucho_canonical_id_localized_for_gateway_child(
    local_fucho_server: str,
    mock_llm_server_url: str,
    tmp_path: Path,
) -> None:
