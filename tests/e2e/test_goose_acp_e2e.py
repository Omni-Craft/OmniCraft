...
:class:`omnicraft.inner.goose_executor.GooseExecutor` spawns the subprocess,
streams ``agent_message_chunk`` updates as chat text, and routes Goose's mid-turn
``session/request_permission`` through OmniCraft's TOOL_CALL policy + human-consent
...
----------------------------------------------------------
* **Opt-in only**: set ``OMNICRAFT_E2E_GOOSE=1`` to run. Needs a configured Goose
...
pytestmark = pytest.mark.skipif(
    os.environ.get("OMNICRAFT_E2E_GOOSE") != "1"
    or shutil.which("goose") is None
    or not os.environ.get("GOOSE_PROVIDER")
    or not os.environ.get("GOOSE_MODEL"),
    reason=(
        "headless goose ACP e2e is opt-in: set OMNICRAFT_E2E_GOOSE=1 with a "
