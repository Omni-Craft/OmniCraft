...
:data:`omnicraft.runtime.harnesses._HARNESS_MODULES`. For each:
...
- The runner subprocess loads the wrap module
  (e.g. :mod:`omnicraft.inner.claude_sdk_harness` or
  :mod:`omnicraft.inner.codex_harness`).
...
hop, unlike the AP-level :mod:`omnicraft.runtime.session_stream`
pub-sub).
...
async def test_harness_wrap_real_llm_smoke(
    ...
    try:
        ...
        # ``omnicraft/runtime/harnesses/_scaffold.py``. The
