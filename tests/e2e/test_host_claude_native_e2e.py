...
``omnicraft.claude_native_bridge._wait_for_claude_prompt_ready``).
...
* **Opt-in only**: set ``OMNICRAFT_E2E_CLAUDE_NATIVE=1`` to run. claude-native
...
* The workspace folder must be trusted in ``~/.claude.json`` or Claude
  ...
  teardown. It does NOT touch ``~/.omnicraft/config.yaml``: the test
...
pytestmark = pytest.mark.skipif(
    os.environ.get("OMNICRAFT_E2E_CLAUDE_NATIVE") != "1" or shutil.which("claude") is None,
    reason=(
        "claude-native e2e needs an interactive Claude login; set "
        "OMNICRAFT_E2E_CLAUDE_NATIVE=1 (and have `claude` installed + logged in) to run"
    ),
)
...
def _spawn_host_daemon(
    *,
    ...
    """
    Spawn an ``omnicraft host`` daemon under the real ``$HOME``.
    ...
    *live_server* is a fresh random-port instance with no other host of
    ...
    with open(daemon_log, "w") as log_fh:
        return subprocess.Popen(
            ...
            [runner_executable(), "-m", "omnicraft.host._daemon_entry", "--server", live_server],
...
def _plant_poisoned_omnicraft_package(workspace: Path) -> None:
    """
    Write a booby-trapped ``omnicraft/`` package inside *workspace*.
    ...
    Python prepends that cwd to ``sys.path[0]``, so ``import omnicraft``
    in the hook resolves to whatever ``omnicraft/`` lives in the
    ...
    mode: a git worktree checked out in the workspace whose ``omnicraft``
    ...
    :param workspace: Absolute workspace path the session starts in; the
        package is written as ``workspace/omnicraft/__init__.py``.
    :returns: None.
    """
    pkg_dir = workspace / "omnicraft"
    pkg_dir.mkdir()
    (pkg_dir / "__init__.py").write_text(
        "raise RuntimeError(\n"
        '    "POISONED omnicraft package imported from the session cwd -- "\n'
        '    "python -I should have excluded the workspace from sys.path"\n'
        ")\n"
    )


def test_claude_native_hooks_ignore_workspace_omnicraft_package(
    live_server: str,
    http_client: httpx.Client,
    tmp_path: Path,
) -> None:
    """
    Hooks import the installed ``omnicraft``, not a workspace shadow.

    Regression for the ``python -I`` fix (cwd import shadowing). The
    workspace contains a poisoned ``omnicraft/`` package that raises on
    ...
    workspace.mkdir()
    _plant_poisoned_omnicraft_package(workspace)
