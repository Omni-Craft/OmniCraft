...
bridge dirs (``~/.omnicraft/opencode-native``) and the daemon registry off the
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        ...
        "set OMNICRAFT_E2E_OPENCODE_NATIVE=1 (and pass --profile/--llm-api-key) to run"
    ),
)


def _spawn_host_daemon(*, tmp_path: Path, live_server: str) -> subprocess.Popen[bytes]:
    """Spawn an ``omnicraft host`` daemon pointed at the test server."""
...
def test_opencode_native_host_session_auto_creates_terminal(
    ...
    try:
        ...
        # The `omnicraft opencode` CLI launcher attaches this TTY directly to the
