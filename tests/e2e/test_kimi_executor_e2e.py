"""End-to-end tests for :class:`omnicraft.inner.kimi_executor.KimiExecutor`.

Real-binary tests gated on:

- ``OMNICRAFT_E2E_KIMI=1`` in the environment, and
...
with ``OMNICRAFT_E2E_KIMI=1 uv run pytest tests/e2e/test_kimi_executor_e2e.py``.
...
from omnicraft.inner.executor import TextChunk, TurnComplete
from omnicraft.inner.kimi_executor import KimiExecutor, _resolve_kimi_binary


def _kimi_e2e_enabled() -> bool:
    if os.environ.get("OMNICRAFT_E2E_KIMI") != "1":
        return False
...
pytestmark = pytest.mark.skipif(
    ...
    reason=(
        "Real-binary e2e: requires OMNICRAFT_E2E_KIMI=1 and the ``kimi`` (or "
        "HARNESS_KIMI_PATH) binary on PATH. Install via "
        "`curl -fsSL https://code.kimi.com/kimi-code/install.sh | bash` and "
        "run ``kimi login`` once, then re-run with OMNICRAFT_E2E_KIMI=1."
    ),
)
