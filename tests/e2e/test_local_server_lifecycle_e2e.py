...
A foreground ``omnicraft server`` registers itself in the machine-global
pidfile AND must stamp the config-signature sidecar, so a later
``omnicraft host`` / ``omnicraft run`` reuses it instead of stopping
...
under ``<home>/.omnicraft`` and never touch the developer's real
``~/.omnicraft`` or a server on the real :8000 (a busy :8000 just makes
...
# cold `omnicraft server` imports the whole stack and runs SQLite
...
# ensure_local_omnicraft_server, so a host appearing online on the ORIGINAL port
...
_ENV_TO_CLEAR = (
    ...
    # Pre-rename alias for OMNICRAFT_AUTH_ENABLED — still honored, so strip
    ...
    "OMNICRAFT_OIDC_ISSUER",
    "OMNICRAFT_AUTH_PROVIDER",
    # ensure_local_omnicraft_server honors OMNICRAFT_DATABASE_URI over the isolated
    # tmp sqlite db, and the server honors OMNICRAFT_RUNNER_TUNNEL_TOKEN — if
...
def _isolated_env(home: Path) -> dict[str, str]:
    ...
    :param home: The tmp home dir; ``<home>/.omnicraft`` holds the pidfile,
...
def _pidfile_path(home: Path) -> Path:
    ...
    :returns: ``<home>/.omnicraft/local_server.pid``.
    """
    return home / ".omnicraft" / "local_server.pid"
...
def _respawned_server_pids(home: Path) -> set[int]:
    ...
    :func:`ensure_local_omnicraft_server`, which spawns a detached
    ``omnicraft server --database-uri sqlite:///<home>/.omnicraft/chat.db
    --artifact-location <home>/.omnicraft/artifacts`` — so its argv carries
    ...
    :returns: PIDs of live ``omnicraft.cli ... server`` processes whose
    ...
    # Match the home as a directory PREFIX (``<home>/.omnicraft/...`` always
    ...
    for line in out.splitlines():
        if "omnicraft.cli" not in line or home_prefix not in line:
            continue
...
class _Procs:
    ...
    def spawn(
        ...
        :param args: CLI args after the ``python -m omnicraft.cli`` prefix.
        ...
        proc = subprocess.Popen(
            [sys.executable, "-m", "omnicraft.cli", *args],
            env=env,
            cwd=str(cwd),
            stdout=fh,
            stderr=fh,
...
def test_connect_reuses_foreground_server_without_killing_it(
    ...
    # via the pidfile rewrite that `ensure_local_omnicraft_server` happens to do. A
...
def test_foreground_server_reuses_connect_owned_server(
    ...
    second canonical ``omnicraft server`` must reuse it (print "already
    ...
    assert rc == 0, (
        f"second `omnicraft server` exited non-zero (rc={rc}).\n"
