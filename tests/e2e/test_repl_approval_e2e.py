...
Spawns ``omnicraft chat examples/agents/ask-demo/`` as a subprocess
...
# Seconds to wait for ``omnicraft run`` to reach an input-ready REPL —
...
# (``_prepare_chat_session_via_daemon`` in omnicraft/chat.py):
#
...
def repl_env(
    ...
    """
    Build the env dict for ``omnicraft chat`` — OPENAI_API_KEY plus
    whatever PYTHONPATH the outer shell already provides (so
    ``omnicraft`` + ``omnicraft_client`` resolve to this
    worktree, not the sibling editable install).

    Redirects ``HOME`` to a temp dir seeded with
    ``.omnicraft/config.yaml`` so the spawned interactive REPL starts
    ...
    - ``tui.theme`` is persisted, so the REPL's first-launch theme
      ...
      which reads ``$HOME/.omnicraft/config.yaml`` — NOT
      ``OMNICRAFT_CONFIG_HOME``) is skipped. Under pexpect's pty stdin
    ...
    - ``auto_open_conversation: false`` stops the interactive REPL from
      ...
      ``run`` flag; config is the supported path). ``OMNICRAFT_CONFIG_HOME``
    ...
    resolve, and ``OMNICRAFT_SKIP_ONBOARD`` guards against any other
    ...
    config_home = fake_home / ".omnicraft"
...
def _require_omnicraft_cli() -> str:
    """
    Resolve the CLI path. Prefers the framework's own
    ``omnicraft`` binary (via the running pytest interpreter's
    venv) over a sibling ``ap`` binary on PATH — the legacy
    ``omnicraft`` ``ap`` CLI doesn't understand
    OmniCraft-format fixtures.

    :returns: Absolute path to an executable.
    """
    venv_omnicraft = Path(sys.executable).parent / "omnicraft"
    if venv_omnicraft.exists():
        return str(venv_omnicraft)
    path = shutil.which("omnicraft") or shutil.which("ap")
    if path is None:
        pytest.skip("Neither omnicraft nor omnicraft CLI on PATH")
    return path
...
def _wait_for_prompt_ready(
    ...
    ``omnicraft chat <path>`` starts a local server, waits for
