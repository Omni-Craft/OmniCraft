...
``omnicraft run <yaml>`` under pexpect and drives approval CUJs
...
def _build_repl_env(mock_llm_server_url: str, tmp_home: Path) -> dict[str, str]:
    ...
    ``omnicraft run`` subprocess uses mock responses.
    """
    ...
    config_home = tmp_home / ".omnicraft"
...
def _spawn_sessions_repl(
    ...
    *,
    ...
    """Spawn ``omnicraft run`` under a PTY (sessions API is default)."""
...
def _spawn_repl_with_args(
    ...
    *,
    ...
    """Spawn ``omnicraft run`` with caller-supplied CLI args."""
    args = [
        "-m",
        "omnicraft",
        "run",
