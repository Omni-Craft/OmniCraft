...
def test_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from omnicraft')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from omnicraft."},
        ],
        key=model,
    )
    ...
    assert "hello from omnicraft" in combined, (
        f"Expected cowsay ASCII art with 'hello from omnicraft' "
...
def test_uv_pip_install_and_use_package(
    ...
    configure_mock_llm(
        ...
        [
                        ...
                        "arguments": json.dumps(
                            {
                                "command": (
                                    ...
                                    "\"import cowsay; cowsay.cow('hello from omnicraft via uv')\""
                                ),
                            }
                        ),
                    }
            ...
            {"text": "Here is the cowsay output with hello from omnicraft via uv."},
        ],
        key=model,
    )
    ...
    all_output = _tool_call_output_text(body)
    combined = (text + " " + all_output).lower()
    assert "hello from omnicraft via uv" in combined, (
        f"Expected cowsay output with 'hello from omnicraft via uv' -- proves "
