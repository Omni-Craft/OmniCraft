...
to Claude Code when running in an OmniCraft claude-native session.
...
sends a prompt asking it to list its omnicraft MCP tools, and
...
Usage::
    ...
    OMNICRAFT_E2E_CLAUDE_NATIVE_SESSION_TOOLS=1 \\
...
_RUN_GATE_ENV = "OMNICRAFT_E2E_CLAUDE_NATIVE_SESSION_TOOLS"
...
def claude_native_ui_agent(
    ...
    Mirrors what ``omnicraft claude`` materialises at runtime (harness
...
def _claude_code_session(
    ...
    *,
    ...
    tmp = tempfile.mkdtemp(prefix="omnicraft-e2e-session-tools-")
    ...
    base_args: tuple[str, ...] = (
        ...
        "--allowedTools",
        (
            "mcp__omnicraft__list_comments,"
            "mcp__omnicraft__update_comment,"
            "mcp__omnicraft__sys_session_list,"
            "mcp__omnicraft__sys_session_get_history"
        ),
    )
    if model:
...
def test_claude_native_session_tools_visible(
    ...
    in Claude Code's MCP tool list when running in an OmniCraft
    ...
    5. Send a prompt asking Claude to list its omnicraft MCP tools.
    ...
    **What breaks if this test fails:**
    ...
    with _claude_code_session(session_id, model=None, launch_env=launch_env) as bridge_dir:
        ...
        # ── Step 5: Ask Claude to list its omnicraft MCP tools ──────────
        send_user_message_to_session(
            http_client,
            session_id=session_id,
            content=(
                "List the names of ALL tools available to you from the "
                "'omnicraft' MCP server. Just list the tool names, one per "
...
def test_claude_native_relay_advertises_broadened_orchestrator_surface(
    ...
    **What breaks if this fails:** the relay assembly reverted to the
    ...
    ``request.tools`` on non-native harnesses. (The real ``omnicraft
