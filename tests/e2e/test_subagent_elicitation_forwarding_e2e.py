...
This drives the REAL stack — a local ``omnicraft server`` booted from
this working tree, a local runner + the native ``claude``/``codex``
CLIs spawned by ``omnicraft run --server``, and a real LLM brain. The
...
``OMNICRAFT_E2E_SUBAGENT_ELICIT=1`` and is not collected by default::
    ...
    OMNICRAFT_E2E_SUBAGENT_ELICIT=1 \\
...
def _clean_env(profile: str = _PROFILE) -> dict[str, str]:
    ...
    ``OMNICRAFT_CONFIG_HOME`` here (the supported replacement for the
    ...
    ``PYTHONPATH`` is dropped so the child imports omnicraft from
    ...
    for stale in (
        ...
        env.pop(stale, None)
    config_home = Path(tempfile.mkdtemp(prefix="omnicraft-elicit-config-"))
...
def _kill_tree(pid: int, conv_ids: set[str]) -> None:
    ...
    ``omnicraft run`` spawns a detached daemon → runner → per-conversation
    ...
    :param pid: The ``omnicraft run`` subprocess pid.
    ...
    if conv_ids:
        ...
        for line in ps.splitlines():
            if "omnicraft.runtime.harnesses._runner" in line and any(c in line for c in conv_ids):
                toks = line.split()
...
def _kill_native_terminals(conv_ids: set[str]) -> None:
    ...
    server (``/tmp/omnicraft-terminal-*/tmux.sock``). When the parent's
    one-shot ``omnicraft run`` exits, the detached worker's pane can outlive
...
def local_server(tmp_path_factory: pytest.TempPathFactory) -> Iterator[str]:
    """
    Boot a throwaway ``omnicraft server`` from this working tree.

    A bare ``omnicraft run`` would route to the developer's configured
...
def _force_codex_prompting(config_path: Path) -> str | None:
    ...
    ``OMNICRAFT_E2E_SUBAGENT_ELICIT=1`` (a deliberate, human-invoked opt-in,
...
def agent_dir(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    layout ``omnicraft run`` expects (``config.yaml`` at the root,
    ...
    :returns: Path of the bundle root to pass to ``omnicraft run``.
    """
...
def test_subagent_prompt_surfaces_on_parent_and_resolves_via_child(
    ...
    Drives ``omnicraft run ask-mode-supervisor --server <local> -p "..."``
    ...
    # is opt-in (OMNICRAFT_E2E_SUBAGENT_ELICIT=1), never CI.
    codex_config = (
    ...
    try:
        ...
        # ``omnicraft run`` exits after the parent's one-shot turn, but its
