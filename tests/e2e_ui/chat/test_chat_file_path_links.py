...
under ``Path.home()``: with no ``OMNICRAFT_RUNNER_WORKSPACE`` set (the e2e
...
def _agent_bundle(cwd: str) -> bytes:
    ...
    through the omnicraft compat adapter, matching the conftest helpers.
...
def linkify_session(
    ...
    ws = Path(tempfile.mkdtemp(prefix="omnicraft-e2e-ui-links-", dir=Path.home()))
