...
A native forwarder (:mod:`omnicraft.cursor_native_forwarder`) tails
...
by :class:`omnicraft.inner.cursor_native_executor.CursorNativeExecutor`. This
...
the same OmniCraft-owned half (the forwarder mirroring cursor's transcript OUT as
...
``omnicraft.inner.cursor_harness``), so it cannot be pointed at the mock LLM the
custom-agent suites use; instead this test seeds a cursor chat store and runs the
real :func:`omnicraft.cursor_native_forwarder.forward_cursor_store_to_session`
...
def test_native_cursor_mirror_renders_without_live_agent(
    ...
    login, so it skips on every PR. This covers the SAME OmniCraft-owned path —
