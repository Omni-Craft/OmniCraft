...
in the background and needs a connected ``omnicraft host`` daemon this
...
def test_clone_dialog_offers_cross_family_native_target_and_forks(
    ...
    assert labels.get("omnicraft.fork.carry_history") == "1", (
        f"cross-family native fork must stamp carry-history, got {labels!r}"
    )
    assert "omnicraft.fork.source_external_session_id" not in labels, (
        f"fork of an SDK source must not stamp a source native session id, got {labels!r}"
    )
    assert labels.get("omnicraft.wrapper") == "claude-code-native-ui", (
