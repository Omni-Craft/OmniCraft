...
reconnect dialog with the exact ``omnicraft run … --resume <id>`` command
...
def _find_runner_pids() -> list[int]:
    """Find PIDs of the runner entry point (``omnicraft.runner._entry``).
    ...
    result = subprocess.run(
        ["pgrep", "-f", "omnicraft.runner._entry"],
        capture_output=True,
        text=True,
    )
