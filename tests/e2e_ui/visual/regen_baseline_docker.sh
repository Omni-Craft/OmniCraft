...
#
#   --skip-build  Reuse an existing omnicraft/server/static/web-ui build (e.g.
...
NPM_VERSION="11.12.1"
BUILD_OUTPUT="omnicraft/server/static/web-ui"
...
docker run --rm --platform "$PLATFORM" -v "$PWD":/work -w /work \
  ...
  -e OMNICRAFT_PW_NO_SANDBOX=1 \
  -e OMNICRAFT_SKIP_WEB_UI=true \
