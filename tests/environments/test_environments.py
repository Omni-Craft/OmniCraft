...
The wrapper is pure re-exports of ``omnicraft.inner.os_env`` and
the ``OSEnvSpec`` / ``OSEnvSandboxSpec`` dataclasses from
``omnicraft.inner.datamodel``. Tests only verify two properties:
...
import omnicraft.inner.datamodel as inner_dm
import omnicraft.inner.os_env as inner_os_env
from omnicraft import environments


def test_environments_all_symbols_importable() -> None:
    """
    Every name in ``omnicraft.environments.__all__`` resolves on the
    wrapper module.
    ...
    for name in environments.__all__:
        assert hasattr(environments, name), f"omnicraft.environments missing re-export {name!r}"
...
def test_environments_reexports_inner_os_env_objects() -> None:
    ...
    objects as ``omnicraft.inner.os_env``.
