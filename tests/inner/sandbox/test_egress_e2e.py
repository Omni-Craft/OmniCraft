...
from omnicraft.inner.credential_proxy import SYNTHETIC_CREDENTIAL_PREFIX
...
def test_s4_same_uid_external_process_cannot_use_helper_relay(
    ...
    try:
        ...
        attacker_sock = _socket.create_connection(("127.0.0.1", relay_port), timeout=5)
        try:
            wrong_header = b"Basic " + base64.b64encode(b"omnicraft:totally-wrong-token-aa11bb22")
            attacker_sock.sendall(
                b"GET http://example.com/ HTTP/1.1\r\n"
                b"Host: example.com\r\n"
...
def test_s4_two_sandboxes_cannot_borrow_each_others_proxy(
    ...
    try:
        ...
        a_token_header = b"Basic " + base64.b64encode(f"omnicraft:{token_a}".encode())
