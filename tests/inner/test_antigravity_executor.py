"""
Unit tests for :class:`omnicraft.inner.antigravity_executor.AntigravityExecutor`.
...
from omnicraft.inner import antigravity_executor as ag
from omnicraft.inner.antigravity_executor import AntigravityExecutor, _latest_user_text
...
from omnicraft.llms._usage_observer import add_observer
...
async def test_streaming_maps_text_reasoning_and_usage(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Usage maps the SDK's UsageMetadata field names onto OmniCraft's keys and
...
def test_extract_usage_no_cache_preserves_full_input() -> None:
    """With no cached tokens, ``input_tokens`` equals the full prompt count.

    Guards the no-cache path: subtracting ``cached or 0`` (here 0) must leave the
    prompt count untouched, and no ``cache_read_input_tokens`` key is emitted.
    """
    meta = SimpleNamespace(
        prompt_token_count=1000,
        candidates_token_count=200,
