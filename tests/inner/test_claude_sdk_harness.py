...
from omnicraft.inner import claude_sdk_harness
...
def test_harness_module_registered_in_module_registry() -> None:
    ...
    """
    assert _HARNESS_MODULES.get("claude-sdk") == "omnicraft.inner.claude_sdk_harness"
    assert _HARNESS_MODULES.get("claude") == "omnicraft.inner.claude_sdk_harness"
...
def test_executor_factory_decodes_os_env_json(
    ...
    OmniCraft serializes ``spec.executor.config["os_env"]`` via
...
def test_skills_filter_env_var_decodes(
    ...
    The env-var bridge between the OmniCraft runtime and the
