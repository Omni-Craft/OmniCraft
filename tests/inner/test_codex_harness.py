...
from omnicraft.inner import codex_harness
...
def test_harness_module_registered_in_module_registry() -> None:
    ...
    """
    assert _HARNESS_MODULES.get("codex") == "omnicraft.inner.codex_harness"
...
def test_executor_factory_reads_env_vars(
    ...
    with patch(
        "omnicraft.inner.codex_harness.CodexExecutor.__init__",
        _fake_init,
    ):
...
def test_executor_factory_cwd_falls_back_to_runner_workspace(
    ...
    to ``OMNICRAFT_RUNNER_WORKSPACE``. Mirrors the claude-sdk / kimi / pi /
    hermes harnesses.
    """
    monkeypatch.delenv("HARNESS_CODEX_CWD", raising=False)
    monkeypatch.setenv("OMNICRAFT_RUNNER_WORKSPACE", "/home/bobby/code/agents")
...
def test_executor_factory_explicit_cwd_wins_over_workspace(
    ...
    ``OMNICRAFT_RUNNER_WORKSPACE`` fallback."""
...
def test_executor_factory_decodes_os_env_json(
    ...
    OmniCraft serializes ``spec.os_env`` via :func:`dataclasses.asdict`
