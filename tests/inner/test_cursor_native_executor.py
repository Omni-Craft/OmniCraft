...
from omnicraft.inner import cursor_native_executor as cne
from omnicraft.inner.cursor_native_executor import (
    CursorNativeExecutor,
    _content_to_text,
    _latest_user_text,
)
from omnicraft.inner.executor import ExecutorError
...
class TestForkPreamble:
    ...
    def test_wrap_defangs_sentinels_inside_preamble(self) -> None:
        ...
        assert "[/omnicraft_fork_history]" in wrapped
...
class TestBridge:
    ...
    def test_build_mcp_config_registers_omnicraft_relay(self, tmp_path: Path) -> None:
    ...
    def test_write_mcp_config_is_workspace_scoped(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr(
            "omnicraft.cursor_native_bridge.approve_mcp_server_for_workspace",
    ...
    def test_enable_mcp_for_workspace_removes_disabled_entry(
        ...
        disabled_path.write_text('["omnicraft", "other"]\n', encoding="utf-8")
    ...
    def test_allow_mcp_tools_in_cli_config_adds_specific_allow_rules(
        ...
        config_path.write_text(
            '{"permissions": {"allow": ["Shell(ls)", "Mcp(omnicraft:sys_os_read)"]}}\n',
            encoding="utf-8",
        )
        ...
        assert allow.count("Mcp(omnicraft:sys_os_read)") == 1
        assert "Mcp(omnicraft:sys_session_send)" in allow
    ...
    def test_approve_mcp_server_for_workspace_uses_cursor_cli(
        ...
        monkeypatch.setattr(
            "omnicraft.cursor_native.resolve_cursor_executable",
        ...
        assert calls == [
            {
                "args": (["/bin/cursor-agent-test", "mcp", "enable", "omnicraft"],),
                "kwargs": {
                    "cwd": tmp_path,
...
class TestRegistration:
    def test_harness_is_registered(self) -> None:
        ...
        assert _HARNESS_MODULES["cursor-native"] == "omnicraft.inner.cursor_native_harness"

    def test_harness_is_allowlisted(self) -> None:
        from omnicraft.spec._omnicraft_compat import OMNICRAFT_HARNESSES

        assert "cursor-native" in OMNICRAFT_HARNESSES

    def test_cursor_native_is_terminal_native(self) -> None:
        # cursor-native launches the cursor-agent TUI in an omnicraft terminal
