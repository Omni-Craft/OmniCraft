...
from omnicraft import kimi_native_bridge
from omnicraft.inner.kimi_native_executor import (
    KimiNativeExecutor,
    _content_to_text,
    _latest_user_text,
)
...
class TestSettlePaneReadiness:
    ...
    def test_marker_matches_live_kimi_footer(self) -> None:
        ...
        footer = (
            " K2.7 Code thinking  ~/omnicraft  pr521-kimi-native [+61 -8]"
...
class TestRegistration:
    def test_harness_is_registered(self) -> None:
        ...
        assert _HARNESS_MODULES["kimi-native"] == "omnicraft.inner.kimi_native_harness"

    def test_harness_is_allowlisted(self) -> None:
        from omnicraft.spec._omnicraft_compat import OMNICRAFT_HARNESSES

        assert "kimi-native" in OMNICRAFT_HARNESSES

    def test_kimi_native_is_terminal_native(self) -> None:
        # kimi-native launches the kimi TUI in an omnicraft terminal (like
