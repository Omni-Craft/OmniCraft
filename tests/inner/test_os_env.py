"""Unit tests for :mod:`omnicraft.inner.os_env` helper-env construction."""
...
def test_build_helper_env_active_passes_omnicraft_session_marker() -> None:
    """The ``OMNICRAFT`` session marker survives the active allowlist.
    ...
    OmniCraft session, the way ``CLAUDE_CODE`` / ``CODEX`` are visible in
    ...
    parent = {
        "PATH": "/usr/bin",
        OMNICRAFT_SESSION_ENV_VAR: OMNICRAFT_SESSION_ENV_VALUE,
    }

    env = build_helper_env(parent, _active_policy())

    assert env[OMNICRAFT_SESSION_ENV_VAR] == OMNICRAFT_SESSION_ENV_VALUE
