...
from omnicraft.inner.pi_executor import (
    PiExecutor,
    _build_models_json,
    ...
    async def _run_generated_bridge_tool(
        ...
        *,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            extension_path = tmp_path / "omnicraft_tools.js"
...
class TestPiExecutorConstructor(unittest.TestCase):
    ...
    def test_constructor_raises_when_pi_not_found(self):
        with patch("omnicraft.inner.pi_executor._find_pi_cli", return_value=None):
    ...
    def test_constructor_databricks_with_env(self):
        with (
            ...
            patch(
                "omnicraft.inner.pi_executor._read_databrickscfg",
    ...
    def test_constructor_databricks_with_host_override_requires_auth_command(self):
        with (
            patch("omnicraft.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"),
            patch("omnicraft.inner.pi_executor._read_databrickscfg") as read_cfg,
        ):
    ...
    def test_constructor_databricks_with_auth_command(self):
        with (
            ...
            patch("omnicraft.inner.pi_executor._read_databrickscfg") as read_cfg,
            patch(
                "omnicraft.inner.pi_executor._fetch_shell_command_token",
    ...
    def test_constructor_databricks_no_creds_raises(self):
        with (
            ...
            patch("omnicraft.inner.pi_executor._read_databrickscfg", return_value=None),
        ):
...
class TestGateNativeTool(unittest.TestCase):
    ...
    OmniCraft policy engine must become ``block=True`` so Pi refuses the
...
def test_gateway_seeds_managed_settings_from_global_agent(
    ...
    monkeypatch.setattr(
        "omnicraft.inner.pi_settings.DEFAULT_PI_AGENT_DIR",
        global_agent,
    )
    with (
        patch("omnicraft.inner.pi_executor._find_pi_cli", return_value="/usr/bin/pi"),
...
class TestSessionManagement(unittest.TestCase):
    ...
    def test_close_session(self):
        async def _test():
            ...
            from omnicraft.inner.pi_executor import _PiSessionState
...
def test_resolve_pi_skill_args_all(tmp_path: Path) -> None:
    ...
    """
    from omnicraft.inner.pi_executor import _resolve_pi_skill_args
...
def test_clean_pi_env_excludes_host_secrets(monkeypatch) -> None:
    ...
    """
    from omnicraft.inner.pi_executor import _clean_pi_env
...
def test_clean_pi_env_includes_omnicraft_session_marker(monkeypatch) -> None:
    """The ``OMNICRAFT`` session marker survives the Pi env scrub.

    The marker (set once on the runner) must reach the Pi CLI so the
    shell commands Pi runs can detect they are inside an OmniCraft
...
def test_rpc_start_spawns_with_exact_env(monkeypatch) -> None:
    ...
    :param monkeypatch: Pytest monkeypatch fixture.
    """
    from omnicraft.inner import pi_executor as pi_mod
...
def test_rpc_start_log_does_not_leak_system_prompt(monkeypatch, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG, logger="omnicraft.inner.pi_executor"):
        _run(_test())
...
# those onto omnicraft's usage schema so pi sub-agent cost is priced the same
    ...
    *,
    ...
    msg: dict[str, object] = {
        ...
        "usage": {
            ...
            # ignores it (omnicraft prices from token counts), but include
...
def test_pi_usage_captured_from_message_end() -> None:
    ...
    the omnicraft schema key. Asserts the actual numbers so a swapped or
    ...
    async def _test() -> None:
        ...
        # Each value proves a specific pi-field → omnicraft-key mapping:
