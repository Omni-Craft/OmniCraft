...
from omnicraft.inner.executor import ExecutorError, TextChunk, TurnComplete
from omnicraft.inner.qwen_executor import QwenExecutor
...
async def test_run_turn_emits_usage_on_turn_complete(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    notified: list[dict] = []
    monkeypatch.setattr(
        "omnicraft.inner.qwen_executor._notify_usage_from_dict",
...
async def test_run_turn_approval_does_not_count_against_idle_timeout(
    ...
    """
    monkeypatch.setattr("omnicraft.inner.qwen_executor._PROMPT_TIMEOUT_SECONDS", 0.05)
...
def test_qwen_in_harness_allowlist() -> None:
    """'qwen' must be in OMNICRAFT_HARNESSES."""
    from omnicraft.spec._omnicraft_compat import OMNICRAFT_HARNESSES

    assert "qwen" in OMNICRAFT_HARNESSES
...
def test_qwen_code_in_harness_aliases() -> None:
    """'qwen-code' must be in OMNICRAFT_HARNESS_ALIASES."""
    from omnicraft.spec._omnicraft_compat import OMNICRAFT_HARNESS_ALIASES

    assert "qwen-code" in OMNICRAFT_HARNESS_ALIASES
...
def test_qwen_harness_creates_fastapi_app() -> None:
    ...
    from omnicraft.inner.qwen_harness import create_app
...
def test_parse_image_data_uri_edge_cases() -> None:
    ...
    from omnicraft.inner.qwen_executor import _parse_image_data_uri
