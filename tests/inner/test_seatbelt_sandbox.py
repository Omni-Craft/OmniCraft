...
from omnicraft.inner.sandbox import SandboxPolicy, with_denied_unix_sockets
from omnicraft.inner.seatbelt_sandbox import (
...
def test_resolve_raises_on_non_darwin() -> None:
    ...
    with patch("omnicraft.inner.seatbelt_sandbox.sys.platform", "linux"):
...
def test_resolve_raises_when_sandbox_exec_missing() -> None:
    ...
    with patch("omnicraft.inner.seatbelt_sandbox.sys.platform", "darwin"):
        with patch("omnicraft.inner.seatbelt_sandbox.shutil.which", return_value=None):
...
def _safe_helper_argv(tmp_path: Path) -> list[str]:
    ...
    return [str(interpreter), "-m", "omnicraft.inner.os_env", "helper", "X"]
...
def test_profile_scratch_tmpdir_gets_read_and_write_allows(tmp_path: Path) -> None:
    ...
    scratch = sys_tmp / "omnicraft-test-scratch"
...
def test_profile_network_section_for_active_egress_emits_narrow_allows(
    ...
    sys_tmp = Path(tempfile.gettempdir())
    scratch = sys_tmp / "omnicraft-egress-test-scratch"
...
def test_ensure_executable_visible_does_not_widen_when_venv_is_under_cwd() -> None:
    ...
    argv = [str(venv_python), "-m", "omnicraft.inner.os_env", "helper"]
...
def test_helper_boots_when_cwd_lives_under_home_regression() -> None:
    ...
    home = Path.home()
    cwd = home / f".omnicraft-realpath-regression-{uuid.uuid4().hex}"
...
def test_helper_boots_when_interpreter_lives_under_home_uv_layout(
    ...
    home = Path.home()
    fake_install_root = home / f".omnicraft-uv-layout-regression-{uuid.uuid4().hex}"
...
def test_ensure_executable_visible_two_hop_proxy_finds_cpython_install_root(
    ...
    ``uv tool install omnicraft`` creates:
      ~/.local/share/uv/tools/omnicraft/bin/python  →  (proxy symlink)
          ~/.local/share/uv/python/cpython-3.12.X-.../bin/python3.12

    The LITERAL path grandparent (``tools/omnicraft/``) has no CPython
    ...
    cpython_root = home / f".omnicraft-test-cpython-{uuid.uuid4().hex}"
    ...
    tool_root = home / f".omnicraft-test-tool-{uuid.uuid4().hex}"
...
def test_run_launcher_spawn_wrap_private_tmpdir_boots_under_seatbelt(
    ...
    from omnicraft.inner.sandbox import _project_root, create_exec_launcher
    ...
    # can import ``omnicraft.inner`` — in production that import is
