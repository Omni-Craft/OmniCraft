"""Tests for the shared CLI styling layer (``omnicraft.inner.ui``)."""
...
def test_accent_is_brand_magenta() -> None:
    """The shared accent is the OmniCraft brand magenta."""

    assert ui.ACCENT == "#0fb5bd"
...
def test_show_banner_respects_no_banner_env(value: str) -> None:
    """``OMNICRAFT_NO_BANNER`` force-disables the banner even on a TTY."""
...
def test_status_lines_go_to_stdout(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    ui.step("Installing OmniCraft")
    ui.success("Verified omnicraft")
    ui.info("Using ~/.omnicraft")
    captured = capsys.readouterr()
    assert "Installing OmniCraft" in captured.out
    assert "✓ Verified omnicraft" in captured.out
    assert captured.err == ""
