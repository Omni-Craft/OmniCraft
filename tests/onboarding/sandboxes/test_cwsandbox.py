"""Tests for :mod:`omnicraft.onboarding.sandboxes.cwsandbox`."""
...
from omnicraft.onboarding.sandboxes.base import DEFAULT_HOST_IMAGE
from omnicraft.onboarding.sandboxes.cwsandbox import (
    HOST_IMAGE_ENV_VAR,
...
def test_provision_requests_host_image_and_egress(sdk: _State) -> None:
    ...
    assert sdk.run_kwargs["tags"] == ["omnicraft", "managed-x"]
...
def test_exec_foreground_records_pid_and_streams_output(sdk: _State) -> None:
    """The foreground command records its pid in an unpredictably-named
    mode-700 dir."""
    sdk.exec_processes = [
    ...
    returncode = CWSandboxLauncher().exec_foreground("sb-1", "omnicraft host --server u")
...
def test_exec_foreground_kills_remote_on_interrupt(sdk: _State) -> None:
    ...
    with pytest.raises(KeyboardInterrupt):
        CWSandboxLauncher().exec_foreground("sb-1", "omnicraft host --server u")
...
def test_exec_foreground_cleanup_failure_preserves_exit_code(sdk: _State) -> None:
    ...
    sdk.exec_processes = [
        ...
        _FakeProcess(_FakeResult(), wait_raises=RuntimeError("transport gone")),
    ]
