"""Tests for :mod:`omnicraft.onboarding.sandboxes.daytona`."""
...
from omnicraft.onboarding.sandboxes.daytona import (
    HOST_IMAGE_ENV_VAR,
...
def test_provision_defaults_official_image_and_disables_autostop(
    ...
    assert create.params.labels == {"omnicraft-name": "managed-abc"}
...
def test_provision_env_passthrough_env_var_fallback(
    ...
    ``OMNICRAFT_DAYTONA_SANDBOX_ENV`` names apply (whitespace around
    commas tolerated).
    """
...
def test_wheel_install_command_overlays_baked_install(fake_daytona: _FakeDaytonaState) -> None:
    ...
    image bakes omnicraft at the same version, so plain pip would
...
def test_exec_foreground_runs_command_over_pty(
    ...
    returncode = launcher.exec_foreground(sandbox_id, "omnicraft host --server u")
    ...
    assert process.pty_handle.sent_inputs == [
        "TERM=xterm-256color exec omnicraft host --server u\n"
    ]
...
def test_exec_foreground_kills_remote_on_interrupt(fake_daytona: _FakeDaytonaState) -> None:
    ...
    re-raises — otherwise a detached `omnicraft host` would keep the
    ...
    with pytest.raises(KeyboardInterrupt):
        launcher.exec_foreground(sandbox_id, "omnicraft host --server u")
