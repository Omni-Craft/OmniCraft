"""Tests for :mod:`omnicraft.onboarding.sandboxes.islo`."""
...
from omnicraft.onboarding.sandboxes.islo import (
    API_KEY_ENV_VAR,
    HOST_IMAGE_ENV_VAR,
...
def test_provision_builds_islo_create_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "omnicraft-fixed")
    fake = _FakeIsloAPI()
    ...
    assert sandbox_id == "omnicraft-fixed"
    assert fake.create_payloads == [
        {
            "name": "omnicraft-fixed",
            "image": "docker.io/me/omnicraft-host:latest",
            "vcpus": 4,
            "memory_mb": 8192,
            "init": {"type": "minimal"},
...
def test_provision_uses_image_and_env_var_fallbacks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "omnicraft-env")
    fake = _FakeIsloAPI()
    launcher = IsloSandboxLauncher()
    monkeypatch.setattr(launcher, "_islo", lambda: fake)
...
def test_provision_clears_seeded_helper_when_user_injects_claude_cred(
    ...
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "omnicraft-byo")
    fake = _FakeIsloAPI()
    ...
    assert strip_calls[0].sandbox_id == "omnicraft-byo"
...
def test_provision_keeps_seeded_helper_without_user_claude_cred(
    ...
    monkeypatch.setenv("OPENAI_API_KEY", "sk-test")
    monkeypatch.setattr(islo_mod, "_new_sandbox_name", lambda label: "omnicraft-gw")
    fake = _FakeIsloAPI()
    launcher = IsloSandboxLauncher(env=["OPENAI_API_KEY"])
    monkeypatch.setattr(launcher, "_islo", lambda: fake)
