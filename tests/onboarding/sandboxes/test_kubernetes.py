...
import omnicraft.onboarding.sandboxes.kubernetes as k8s
from omnicraft.host.identity import (
    HOST_ID_ENV_VAR,
    HOST_NAME_ENV_VAR,
    HOST_TOKEN_ENV_VAR,
)
from omnicraft.onboarding.sandboxes.base import SandboxCapabilityError
from omnicraft.onboarding.sandboxes.kubernetes import (
...
_MANIFEST_KW = {
    "pod_name": "omnicraft-managed-abc-1a2b3c",
    "namespace": "omnicraft-sandboxes",
    "image": "ghcr.io/omnicraft-ai/omnicraft-host:latest",
    "service_account": "omnicraft-runner",
    "host_id": "host_abcdef",
    "host_name": "managed-abcdef",
    "server_url": "http://srv.example.com",
    "token_secret_name": "omnicraft-managed-abc-1a2b3c-token",
    "harness_secret": "omnicraft-creds",
    "env_literals": {},
    "node_selector": None,
    "workspace": "/home/omnicraft/workspace",
}
...
def test_build_pod_manifest_runs_host_under_reaper_as_container_command() -> None:
    ...
    script = command[2]
    # exec the reaper (so it is PID 1) which then runs `omnicraft host`.
    assert "exec python3 -c" in script
    assert "omnicraft host --server http://srv.example.com" in script
...
def test_build_pod_manifest_init_container_prepares_and_clones_workspace() -> None:
    ...
    manifest = build_pod_manifest(
        **{**_MANIFEST_KW, "clone_dir": "/home/omnicraft/workspace/repo"},
    ...
    assert "mkdir -p /home/omnicraft/workspace" in script
    assert "git clone --branch main --single-branch -- " in script
    assert "https://github.com/org/repo.git /home/omnicraft/workspace/repo" in script
...
def test_build_pod_manifest_token_rides_secret_ref_not_the_spec() -> None:
    ...
    assert token_entry["valueFrom"]["secretKeyRef"] == {
        "name": "omnicraft-managed-abc-1a2b3c-token",
...
def test_build_token_secret_manifest_carries_token_in_stringdata() -> None:
    ...
    secret = build_token_secret_manifest(
        secret_name="omnicraft-pod-token", namespace="omnicraft-sandboxes", token=_TOKEN
    )
    assert secret["stringData"] == {HOST_TOKEN_ENV_VAR: _TOKEN}
    assert secret["metadata"]["labels"]["app.kubernetes.io/managed-by"] == "omnicraft"
...
def test_build_pod_manifest_harness_secret_projects_into_both_containers() -> None:
    ...
    assert init["envFrom"] == [{"secretRef": {"name": "omnicraft-creds"}}]
    assert host["envFrom"] == [{"secretRef": {"name": "omnicraft-creds"}}]
...
def test_new_pod_name_and_token_secret_name() -> None:
    ...
    assert name.startswith("omnicraft-managed-abc-123-")
...
def _launcher() -> KubernetesSandboxLauncher:
    ...
    return KubernetesSandboxLauncher(
        in_cluster=True, namespace="omnicraft-sandboxes", secret_name="omnicraft-creds", env=()
    )
...
def test_launch_host_creates_secret_then_pod_and_returns_workspace(
    ...
    assert workspace == "/home/omnicraft/workspace"
    ...
    assert fake_core.created_pods[0]["metadata"]["name"] == "omnicraft-pod-1"
...
def test_launch_host_with_repo_returns_clone_dir(fake_core: _FakeCore) -> None:
    ...
    assert workspace == "/home/omnicraft/workspace/repo"
...
def test_launch_host_cleans_up_on_create_failure(fake_core: _FakeCore) -> None:
    ...
    assert "omnicraft-pod-3-token" in fake_core.deleted_secrets
    assert "omnicraft-pod-3" in fake_core.deleted_pods
...
def test_launch_host_fast_fails_on_clone_failure_with_log_tail(
    ...
    assert "omnicraft-pod-4" in fake_core.deleted_pods
    assert "omnicraft-pod-4-token" in fake_core.deleted_secrets
...
def test_terminate_deletes_pod_and_secret(fake_core: _FakeCore) -> None:
    ...
    _launcher().terminate("omnicraft-pod-6")
    assert fake_core.deleted_pods == ["omnicraft-pod-6"]
    assert fake_core.deleted_secrets == ["omnicraft-pod-6-token"]
...
def test_terminate_is_idempotent_on_404(fake_core: _FakeCore) -> None:
    ...
    _launcher().terminate("omnicraft-pod-7")  # must not raise
    assert fake_core.deleted_secrets == ["omnicraft-pod-7-token"]
...
def test_terminate_retries_transient_then_gives_up_best_effort(
    ...
    _launcher().terminate("omnicraft-pod-8")  # best-effort: must not raise
    assert "could not delete Kubernetes pod 'omnicraft-pod-8'" in capsys.readouterr().err
    # The Secret delete still runs after the Pod gives up.
    assert fake_core.deleted_secrets == ["omnicraft-pod-8-token"]
...
def test_provision_reserves_pod_name_and_run_is_unsupported() -> None:
    ...
    assert name.startswith("omnicraft-managed-abc-")
