"""Tests for :mod:`omnicraft.onboarding.sandboxes.modal`."""
...
from omnicraft.onboarding.sandboxes.modal import (
    DEFAULT_HOST_IMAGE,
    HOST_IMAGE_ENV_VAR,
...
class _FakeImage:
    ...
    :param tag: The registry image reference, e.g.
        ``"ghcr.io/omnicraft-ai/omnicraft-host:latest"``.
...
def test_prepare_raises_with_install_hint_when_sdk_missing(
    ...
    assert "omnicraft[modal]" in str(exc.value)
...
def test_provision_creates_max_lifetime_sandbox_under_shared_app(
    ...
    # Image: the official prebaked host image (omnicraft + git/tmux
    ...
    assert state.sandboxes[sandbox_id].tags == {"omnicraft-name": "my-host"}


def test_provision_honors_image_override_env_var(monkeypatch: pytest.MonkeyPatch) -> None:
    """
    OMNICRAFT_MODAL_HOST_IMAGE must replace the default image ref —
    it's the escape hatch for org-internal copies of the host image.
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(HOST_IMAGE_ENV_VAR, "ghcr.io/acme/omnicraft-host:sha-abc1234")
    monkeypatch.delenv(REGISTRY_SECRET_ENV_VAR, raising=False)
    ModalSandboxLauncher().provision("my-host")

    assert state.create_calls[0].image.tag == "ghcr.io/acme/omnicraft-host:sha-abc1234"
...
def test_provision_passes_registry_secret_for_private_pulls(
    ...
    """
    OMNICRAFT_MODAL_REGISTRY_SECRET must thread the named Modal secret
...
def test_exec_foreground_records_pid_and_streams_output(
    ...
    returncode = ModalSandboxLauncher().exec_foreground("sb-1", "omnicraft host --server u")
    ...
    assert "exec omnicraft host --server u" in remote
...
def test_exec_foreground_kills_remote_on_interrupt(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(KeyboardInterrupt):
        ModalSandboxLauncher().exec_foreground("sb-1", "omnicraft host --server u")
...
def test_exec_foreground_cleanup_failure_preserves_exit_code(
    ...
    sandbox = _seed_sandbox(state)
    sandbox.exec_queue.append(_FakeProcess(stdout="host-output\n", returncode=3))
    # The cleanup exec's wait() blows up mid-teardown.
    sandbox.exec_queue.append(_FakeProcess(wait_raises=RuntimeError("transport gone")))
...
def test_wheel_install_command_overlays_baked_install() -> None:
    """
    The prebaked host image carries omnicraft at the same 0.1.0
...
def test_provision_explicit_image_overrides_env_and_gets_secret(
    ...
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(HOST_IMAGE_ENV_VAR, "ghcr.io/acme/omnicraft-host:env")
    monkeypatch.setenv(REGISTRY_SECRET_ENV_VAR, "ghcr-pull")
...
def test_provision_injects_configured_sandbox_secrets(
    ...
    ModalSandboxLauncher(secrets=["omnicraft-llm", "gateway-extras"]).provision("my-host")

    assert state.create_calls[0].secrets == [
        _FakeSecret(name="omnicraft-llm"),
...
def test_provision_resolves_sandbox_secrets_env_var(
    ...
    """
    Without constructor names, OMNICRAFT_MODAL_SANDBOX_SECRETS
    (comma-separated, whitespace tolerated) supplies the workload
    secrets — the CLI flow's path to the same feature.
    """
    state = _install_fake_modal(monkeypatch)
    monkeypatch.setenv(SANDBOX_SECRETS_ENV_VAR, "omnicraft-llm, extra-creds")
    ModalSandboxLauncher().provision("my-host")
