"""Tests for :mod:`omnicraft.onboarding.sandboxes.openshell`."""
...
from omnicraft.onboarding.sandboxes.openshell import (
    HOST_IMAGE_ENV_VAR,
...
def test_run_background_uses_exec_background(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = launcher.run_background(
        "sb-1", "ENV=val omnicraft host --server https://s", log_path="/tmp/host.log"
    )
    ...
    assert "omnicraft host --server https://s" in command[2]
...
def test_client_execute_pins_sandbox_home(sdk: _SDKState) -> None:
    ...
    ``/root`` cwd/home is unreadable to it and ``omnicraft host`` crashes.
    """
    client = _OpenShellClient()
