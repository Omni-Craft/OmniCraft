"""Unit tests for omnicraft.onboarding.ucode_setup."""
...
from omnicraft.onboarding.setup import ProfileSpec
...
def test_model_gateway_workspace_urls_excludes_mcp_only_workspaces(
    ...
    stub = types.ModuleType("omnicraft.onboarding.internal_beta")
    stub.DEFAULT_PROFILES = profiles
    monkeypatch.setitem(sys.modules, "omnicraft.onboarding.internal_beta", stub)
...
def test_ucode_workspace_exists_checks_single_workspace() -> None:
    ...
    with patch("omnicraft.onboarding.ucode_setup.read_ucode_state", return_value=object()):
