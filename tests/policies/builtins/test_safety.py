...
(:mod:`omnicraft.policies.builtins.safety`).

Covers:

- ``ask_on_os_tools`` — ASKs approval before file/shell tool calls,
  including OmniCraft ``sys_os_*`` tools, Claude Code native tools
...
from omnicraft.policies.builtins.safety import ask_on_os_tools, block_skills
from omnicraft.policies.schema import PolicyEvent
from tests.policies.builtins.helpers import tool_call_event as tc

# ── ask_on_os_tools: OmniCraft sys_os_* tools ─────────────────────────────
...
def test_ask_on_os_tools_asks_for_sys_os_tools(tool: str) -> None:
    ...
    If any returns ALLOW, the policy is not matching the OmniCraft
...
def test_ask_on_os_tools_asks_for_pi_native_tools(
    ...
    (which match the OmniCraft convention, not Claude's ``file_path``)
...
def _request_event(text: str) -> PolicyEvent:
    ...
    The OmniCraft server evaluates skill slash commands at the REQUEST phase
...
def test_block_skills_denies_slash_command_blocked_skill() -> None:
    ...
    This is the path the OmniCraft server takes when the user types
...
def test_ask_on_add_policy_asks_for_sys_add_policy() -> None:
    ...
    from omnicraft.policies.builtins.safety import ask_on_add_policy
...
def test_block_skills_denies_native_skill_tool() -> None:
    ...
    ``PreToolUse`` hook fires → OmniCraft server evaluates → this policy denies.
