...
import pytest
from omnicraft_client._sessions import Session as SessionSnapshot
from omnicraft_ui_sdk import RichBlockFormatter
from rich.text import Text

from omnicraft.repl._repl import _refresh_session_metadata, _SessionsChatReplAdapter
...
class _SnapshotClient:
    """
    OmniCraft client stub exposing only the ``sessions.get`` surface.
...
class _RaisingClient:
    """OmniCraft client stub whose ``sessions.get`` always fails."""
...
def _make_adapter(client: object, agent_name: str = "nessie") -> _SessionsChatReplAdapter:
    ...
    :param client: OmniCraft client (stub) handed to the adapter.
