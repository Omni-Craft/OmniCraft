...
def test_build_github_issue_url_base_structure() -> None:
    """URL targets the omnicraft-ai/omnicraft issues/new endpoint.
    ...
    assert url.startswith("https://github.com/omnicraft-ai/omnicraft/issues/new")
...
async def test_report_command_includes_version_in_url(
    ...
    """OmniCraft version is included in the issue body when available.
    ...
    monkeypatch.setattr(
        ...
        lambda pkg: "9.8.7" if pkg == "omnicraft" else "0.0.0",
    )
