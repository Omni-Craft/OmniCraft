"""
Tests for :mod:`omnicraft.repl._resume_picker` — the
...
from omnicraft.entities import ConversationItem, MessageData
from omnicraft.repl._resume_picker import (
...
def test_runtime_badge_claude_native() -> None:
    ...
    """
    from omnicraft.repl._resume_picker import _runtime_badge

    row = _BadgeRow(labels={"omnicraft.wrapper": "claude-code-native-ui"})
...
def test_runtime_badge_codex_native() -> None:
    ...
    row = _BadgeRow(labels={"omnicraft.wrapper": "codex-native-ui"})
...
class _FakeSessionsNamespace:
    """Stub mimicking :class:`omnicraft_client.SessionsNamespace`.
...
class _FakeConversationsNamespace:
    """Stub mimicking :class:`omnicraft_client.ConversationsNamespace`.
...
class _FakeAPClient:
    """Stub :class:`omnicraft_client.OmniCraftClient` exposing
...
async def test_wrapper_label_picker_filters_and_lists_without_agent_filter(
    ...
    monkeypatch.setenv("OMNICRAFT_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "state"))
    rows = [
        _BadgeRow(
            id="conv_claude_1",
            title="claude one",
...
def test_render_workspace_cell_no_state_returns_none(
    ...
    """
    from omnicraft.repl._resume_picker import _render_workspace_cell

    monkeypatch.setenv("OMNICRAFT_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "state"))
    row = _BadgeRow(id="conv_no_state", labels={"omnicraft.wrapper": "x"})
...
def test_render_workspace_cell_matching_cwd_no_flag(
    ...
    *where* the session was started; only the action-required
    ...
    row = _BadgeRow(id="conv_match", labels={"omnicraft.wrapper": "claude-code-native-ui"})
...
def test_render_workspace_cell_mismatched_cwd_shows_cd_flag(
    ...
    row = _BadgeRow(id="conv_mismatch", labels={"omnicraft.wrapper": "claude-code-native-ui"})
...
def test_workspace_metadata_appears_in_wrapper_picker_list(
    ...
    """
    from omnicraft.claude_native_state import write_launch_state
    from omnicraft.repl._resume_picker import pick_conversation
...
def test_render_workspace_cell_codex_native_uses_codex_state(
    ...
    monkeypatch.setenv("OMNICRAFT_CLAUDE_NATIVE_STATE_DIR", str(tmp_path / "claude-state"))
