...
``omnicraft run`` in sessions mode:

1. ``ResponseCreated`` was imported from the public ``omnicraft_client``
   re-export, which doesn't include it. ImportError on first turn.
2. The adapter passed server-shape events
   (:class:`omnicraft.server.schemas.OutputTextDeltaEvent`) through
   without translating to the SDK-shape dataclasses in
   :mod:`omnicraft_client._events`. Net: spinner spun, server-side
   LLM call completed, no assistant text reached the TUI.
3. The translator initially used wrong class names from
   ``omnicraft.server.schemas`` (``ResponseTextDeltaEvent`` vs the
...
def test_elicitation_request_event_translates() -> None:
    ...
    from omnicraft_client._events import ElicitationRequest
...
def test_elicitation_resolve_session_id_routes_mirrored_child_to_child() -> None:
    ...
    from omnicraft.repl._repl import _elicitation_resolve_session_id
...
def test_output_item_done_event_passes_through() -> None:
    ...
    from omnicraft.server.schemas import OutputItemDoneEvent
...
def test_client_task_cancel_event_passes_through() -> None:
    ...
    from omnicraft.server.schemas import ClientTaskCancelEvent
...
def test_autonomous_turn_renders_without_local_send() -> None:
    ...
    ``test_run_omnicraft_autonomous_turn_polling.py`` used to provide before
    ...
    * ``session.status idle`` → flush + stop-timer + turn-done.
    ...
    # ``_render_session_event`` in ``omnicraft/repl/_repl.py``.
    for event in events:
        if isinstance(event, _StatusEv):
            if event.status == "running":
                rendered.append("START_TIMER")
...
def test_failed_status_event_renders_error_message() -> None:
    ...
    """
    from omnicraft_ui_sdk import RichBlockFormatter

    from omnicraft.repl._repl import _render_failed_status_error
