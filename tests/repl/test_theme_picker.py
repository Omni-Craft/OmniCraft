...
from omnicraft.repl._theme_picker import (
    _build_dark_preview,
...
def test_startup_picker_non_tty_defaults_to_light(
    ...
    monkeypatch.setattr(
        "omnicraft.repl._theme_picker._detect_terminal_background",
        lambda: None,
    )
    ...
    config = (tmp_path / ".omnicraft" / "config.yaml").read_text(encoding="utf-8")
