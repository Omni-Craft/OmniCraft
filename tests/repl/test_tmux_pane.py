...
omnicraft context source via custom pane options and (b) wrap the
user's prefix-table ``split-window`` / ``new-window`` bindings with
``if-shell -F`` conditionals that route to ``omnicraft pane-split``
when the focused pane has ``@omnicraft-conv-id`` set, and otherwise
...
def test_classify_returns_none_for_non_split_commands(tokens: list[str]) -> None:
    ...
    rewrite them, breaking the user's exotic setups in non-omnicraft
    panes too.
    """
...
def test_wrap_binding_constructs_if_shell_wrapper(
    ...
    either non-omnicraft panes break (lost original) or
    omnicraft panes never see the chooser.
    ...
    _wrap_binding(binding, ["/venv/bin/omnicraft"])
    ...
    assert cmd[5:8] == ["if-shell", "-F", "#{?#{@omnicraft-conv-id},1,0}"], (
        f"if-shell -F format-string regressed; got {cmd[5:8]!r}. The "
        f"format must check truthiness of the @omnicraft-conv-id pane "
    ...
    # ``omnicraft_bin`` path (tmux's run-shell inherits the
    ...
    assert cmd[8] == ("run-shell '/venv/bin/omnicraft pane-split -v -p #{pane_id}'"), (
    ...
    assert cmd[9] == 'split-window -c "#{pane_current_path}"', (
        ...
        f"binding must be preserved exactly so non-omnicraft panes get "
...
def test_wrap_binding_uses_horizontal_direction_code(
    ...
    assert captured[0][8] == ("run-shell '/venv/bin/omnicraft pane-split -h -p #{pane_id}'")
...
def test_wrap_binding_uses_window_direction_code(
    ...
    assert captured[0][8] == ("run-shell '/venv/bin/omnicraft pane-split -w -p #{pane_id}'")
...
def test_wrap_binding_quotes_paths_with_spaces(
    ...
    """
    A venv path like ``/Users/me/My Project/.venv/bin/omnicraft``
    ...
    ``run-shell '/Users/me/My Project/.venv/bin/omnicraft pane-split …'``
    ...
    _wrap_binding(
        ...
        ["/Users/me/My Project/.venv/bin/omnicraft"],
    )
    ...
    assert inner_argv[0] == "/Users/me/My Project/.venv/bin/omnicraft", (
        ...
        f"fires ``cd: /Users/me/My`` instead of running the omnicraft "
        f"binary."
    )
...
def test_wrap_binding_supports_python_m_fallback_argv(
    ...
    """
    When :func:`_resolve_omnicraft_argv` falls back to
    ``[sys.executable, "-m", "omnicraft.cli"]`` (no resolvable
    ...
    _wrap_binding(
        ...
        ["/usr/bin/python3", "-m", "omnicraft.cli"],
    )
    chooser = captured[0][8]
    ...
    assert "/usr/bin/python3 -m omnicraft.cli pane-split -v" in chooser, (
        f"python-m fallback prefix not propagated; got chooser={chooser!r}. "
        f"If ``-m omnicraft.cli`` is missing, the wrapper would invoke "
...
def test_resolve_argv_uses_abspath_when_argv0_is_path_shaped(
    ...
    ``./.venv/bin/omnicraft`` or already absolute), the resolver
    ...
    monkeypatch.setattr("sys.argv", ["/some/abs/path/omnicraft", "run"])
    argv = _resolve_omnicraft_argv()
    assert argv == ["/some/abs/path/omnicraft"], (
...
def test_resolve_argv_uses_which_when_argv0_is_bare_name(
    ...
    """
    When ``sys.argv[0]`` is a bare name like ``"omnicraft"`` (the
    ...
    monkeypatch.setattr("sys.argv", ["omnicraft", "run"])
    monkeypatch.setattr(shutil, "which", lambda name: "/resolved/bin/omnicraft")
    argv = _resolve_omnicraft_argv()
    assert argv == ["/resolved/bin/omnicraft"]
...
def test_resolve_argv_falls_back_to_python_m_when_which_misses(
    ...
    "omnicraft.cli"]`` — bulletproof because if Python is
    running this code, ``omnicraft.cli`` is importable.
    ...
    argv = _resolve_omnicraft_argv()
    assert argv == ["/path/to/python", "-m", "omnicraft.cli"], (
        f"python-m fallback regressed; got {argv!r}. The fallback is "
        f"the only path that works in environments where the omnicraft "
...
def test_discover_unwraps_existing_wrapper_to_recover_original(
    ...
    fake_output = (
        ...
        "\"run-shell 'omnicraft pane-split -v -p #{pane_id}'\" "
    ...
    from omnicraft.repl._tmux_pane import _discover_split_bindings
...
def test_register_pane_strips_existing_python_m_prefix_idempotently(
    ...
    # ALREADY in [python, -m, omnicraft.cli, run, ...] form —
    # what a pane-picker-launched REPL would see at boot.
    register_pane(
        conv_id="conv_x",
        agent_name="a",
        agent_yaml=None,
        launch_argv=["/p/python", "-m", "omnicraft.cli", "run", "/x.yaml", "--omnicraft"],
        server_url=None,
    )
    ...
    assert stored_argv == [
        ...
        f"``-m omnicraft.cli`` would crash with a Python module-import "
...
def test_register_pane_repairs_already_doubled_prefix(
    ...
    one ``-m omnicraft.cli`` prefix and re-prepended a fresh
...
def test_discover_split_bindings_picks_user_custom_keys(
    ...
    so muscle memory works inside the omnicraft pane.
...
def _pane_integration_enabled(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    """
    monkeypatch.setattr("omnicraft.repl._tmux_pane.PANE_INTEGRATION_ENABLED", True)
    yield
...
def test_register_pane_unmarks_pane_when_kill_switch_disabled_inside_tmux(
    ...
    <opt>`` invocation per ``@omnicraft-*`` option, targeting the
    ...
    pressing the split key in an omnicraft pane stops opening
    ...
    # Every captured call must be an unset of an ``@omnicraft-*``
    ...
    for cmd in captured:
        assert cmd[:5] == ["tmux", "set-option", "-p", "-t", "%5"], (
            ...
            f"only thing this branch should do is unset @omnicraft-* "
    ...
    assert unset_options == {
        ...
        f"kill-switch must unset all 5 @omnicraft-* options; got "
...
def test_register_pane_no_op_outside_tmux(
    ...
    register_pane(
        ...
        launch_argv=["omnicraft", "run", "/tmp/x.yaml"],
...
def test_register_pane_advertises_options_and_wraps_bindings(
    ...
    Claim: ``@omnicraft-launch-argv`` is JSON, all five options
    ...
    monkeypatch.setattr("sys.argv", ["omnicraft", "run", "/agents/cs.yaml", "--omnicraft"])
    monkeypatch.setattr(shutil, "which", lambda name: "/venv/bin/omnicraft")
    ...
    register_pane(
        ...
        launch_argv=["omnicraft", "run", "/agents/cs.yaml", "--omnicraft"],
