...
from omnicraft.entities.session_resources import SessionResourceView, terminal_resource_view
from omnicraft.inner.datamodel import TerminalEnvSpec
from omnicraft.runner import create_runner_app
from omnicraft.terminals import TerminalListEntry
from tests.runner.helpers import NullServerClient, make_test_terminal_instance

# Matches ``_TOOL_RELAY_FILE`` in ``omnicraft.claude_native_bridge``.
...
def _skip_tools_changed_notification(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("omnicraft.claude_native_bridge.post_tools_changed", _noop)
...
async def relay_env(tmp_path: Path, client: httpx.AsyncClient) -> AsyncIterator[_RelayEnv]:
    ...
    does not create it, so this mirrors what ``omnicraft claude`` does on
...
async def _launch_terminal(
    ...
    *,
    ...
    """
    POST the claude terminal-launch request used by ``omnicraft claude``.
...
@pytest.mark.asyncio
async def test_relay_executor_routes_through_omnicraft_in_omnicraft_mode(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Route relay tool execution through OmniCraft ``/mcp`` for policy enforcement.
    ...
    :class:`~omnicraft.runner.proxy_mcp_manager.ProxyMcpManager` instead of
    ...
    """
    import omnicraft.claude_native_bridge as _bridge_mod

    # Records every POST sent to the fake OmniCraft server.
    ap_mcp_posts: list[dict[str, Any]] = []

    class _FakeApClient:
        """Fake OmniCraft server client that captures /mcp calls and returns a fixed result.

        Appends each POST request body to the outer ``ap_mcp_posts`` list via
        closure so the test can assert on what was sent to the OmniCraft server.
        """
    ...
    try:
        ...
        # In OmniCraft mode the executor must have POSTed a tools/call JSON-RPC to the
        # OmniCraft server's /mcp endpoint, not called the direct comment handler.
        mcp_call = next(
            (
                r
        ...
        assert mcp_call is not None, (
            "No tools/call request reached the OmniCraft /mcp endpoint. "
        ...
        assert mcp_call["json"]["params"]["arguments"] == {"status": "pending"}, (
            "Arguments were not forwarded correctly to OmniCraft /mcp."
        )
        ...
        # The OmniCraft response's text content must be parsed back to a dict.
        assert result == {"items": []}, f"Expected parsed OmniCraft response dict, got {result!r}."
    finally:
        shutil.rmtree(bridge_dir, ignore_errors=True)
