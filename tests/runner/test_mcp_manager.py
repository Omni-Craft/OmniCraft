...
from omnicraft.runner import mcp_manager as _mcp_manager_module
from omnicraft.runner.mcp_manager import (
    _POOL_SPEC_CAPACITY,
    McpSchemasResult,
    RunnerMcpManager,
    compute_server_hash,
...
def patch_connection(
    ...
    monkeypatch.setattr(
        "omnicraft.runner.mcp_manager.McpServerConnection",
        _PatchedConn,
    )
...
async def test_invalid_tool_name_is_filtered(
    ...
    """
    # Force the `omnicraft` package logger to propagate so caplog
    # captures warnings. Defensive: ``omnicraft.cli_diagnostics
    # .setup_cli_logging`` sets ``omnicraft.propagate = False`` and
    ...
    # xdist worker may have set ``omnicraft.propagate = False`` (the CLI
    ...
    mcp_logger = logging.getLogger("omnicraft.runner.mcp_manager")
    ...
    try:
        ...
        # caplog's handler lives. Passing ``logger="omnicraft.runner.
...
def test_strip_mcp_tool_prefix_preserves_bare_double_underscore() -> None:
    ...
    through.
    """
    from omnicraft.runtime.workflow import _strip_mcp_tool_prefix
