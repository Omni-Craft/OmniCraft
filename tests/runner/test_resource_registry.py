...
async def test_terminal_resource_role_moves_on_transfer(
    ...
    Native Codex can rotate ownership between OmniCraft sessions. If the role stays
...
def test_resolve_environment_creates_primary_lazily(
    ...
    os.environ["OMNICRAFT_RUNNER_OS_ENV_ROOT"] = str(tmp_path)
    try:
        reg = SessionResourceRegistry()
        ...
        os.environ.pop("OMNICRAFT_RUNNER_OS_ENV_ROOT", None)
...
def test_resolve_environment_default_pins_none_sandbox_when_no_agent_spec(
    ...
    """
    monkeypatch.setattr(
        "omnicraft.inner.sandbox.shutil.which",
        lambda name: "/usr/bin/bwrap",
    )
    monkeypatch.setenv("OMNICRAFT_RUNNER_OS_ENV_ROOT", str(tmp_path))
...
def test_compute_default_env_root_runner_workspace_overrides_relative_cwd(
    ...
    terminal cwd flows through ``OMNICRAFT_RUNNER_WORKSPACE`` and
