...
from omnicraft.inner.os_env import EditEntry, OpResult, OSEnvironment
from omnicraft.inner.terminal import TerminalInstance
from omnicraft.runner import create_runner_app
from omnicraft.runner import resource_registry as resource_registry_mod
...
from omnicraft.terminals import TerminalListEntry, TerminalRegistry
...
async def test_get_terminal_by_id_returns_404_when_tmux_exited(
    ...
    This is the signal ``omnicraft claude`` uses after its attach
...
async def test_transfer_terminal_moves_resource_without_closing(
    ...
    the old conversation, making the new OmniCraft conversation unable to
...
async def test_create_terminal_uses_runner_workspace_as_default_cwd(
    ...
    This is the resource endpoint used by ``omnicraft claude``. The
...
async def test_create_terminal_threads_agent_parent_os_env_through(
    ...
    ``omnicraft claude`` wrapper) could spawn an unsandboxed
    terminal in a session whose YAML declared an egress allow-list.
    """
    from omnicraft.inner.datamodel import AgentDef, OSEnvSandboxSpec, OSEnvSpec
...
async def test_concurrent_resource_reads_share_one_session_snapshot(
    ...
    resolution, instead of each request stampeding the OmniCraft server.
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub OmniCraft server that blocks + counts the session snapshot fetch.
    ...
    async def _spec_resolver(resolved_agent_id: str, session_id: str | None = None) -> AgentSpec:
        ...
        return AgentSpec(
            spec_version=1,
            name="burst-agent",
            executor=ExecutorSpec(type="omnicraft", config={"harness": "runner-test-default"}),
        )
...
async def test_failed_session_snapshot_is_not_cached_and_retries(
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub OmniCraft server: fail the first snapshot, succeed afterward.
    ...
    # OmniCraftError → the endpoint returned a non-200 error response.
...
async def test_unbound_agent_snapshot_is_not_cached_and_retries(
    ...
    async def _server_handler(request: httpx.Request) -> httpx.Response:
        """
        Stub OmniCraft server: 200 with no agent_id first, then the bound one.
...
async def test_claude_terminal_ensure_concurrent_calls_create_once(
    ...
    * ``create_count == 1`` proves the second request took the early-return
    ...
    monkeypatch.setattr("omnicraft.runner.app._auto_create_claude_terminal", fake_auto_create)
