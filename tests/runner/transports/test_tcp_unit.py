...
Tests the pure-logic helpers in ``omnicraft.runner.transports.tcp``:
...
def test_is_tcp_listening_returns_false_on_os_error() -> None:
    ...
    with patch("omnicraft.runner.transports.tcp.socket.create_connection", side_effect=OSError):
