"""Tests for omnicraft.runtime.credentials.databricks."""
...
def test_databricks_config_profile_env_var_typo_raises_not_falls_back_to_default(
    ...
    # Simulates: `omnicraft run foo.yaml --profile typo-profile --model databricks/m`
...
def test_sdk_value_error_does_not_emit_warning(
    ...
    with caplog.at_level(logging.DEBUG, logger="omnicraft.runtime.credentials.databricks"):
        resolve_databricks_workspace(profile="dev")

    module_records = [
        r for r in caplog.records if r.name == "omnicraft.runtime.credentials.databricks"
    ]
