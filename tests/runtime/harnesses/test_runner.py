...
from omnicraft.runtime.harnesses import _runner
...
def test_load_harness_app_import_error_exits(
    ...
    with pytest.raises(SystemExit) as excinfo:
        _runner._load_harness_app("missing", "omnicraft.does_not_exist", "conv_x")
    ...
    assert "'omnicraft.does_not_exist'" in err
...
def test_load_harness_app_module_without_create_app_exits(
    ...
    (``omnicraft.errors``) reproduces the failure mode.
    """
    with pytest.raises(SystemExit) as excinfo:
        _runner._load_harness_app("broken", "omnicraft.errors", "conv_x")
    assert excinfo.value.code == 2
    err = capsys.readouterr().err
