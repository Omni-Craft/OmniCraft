"""
Tests for :class:`omnicraft.runtime.harnesses._scaffold.HarnessApp`.
...
from omnicraft.runtime.harnesses._scaffold import HarnessApp, TurnContext
...
_TRUNCATION_MARKER = "[output truncated by omnicraft:"
...
async def test_session_events_404s_on_conversation_id_mismatch(
    ...
    OmniCraft could never correlate).
    """
    ...
    body = resp.json()
    # Error envelope shape matches OmniCraftError's serialization
    # (see _handle_omnicraft_error). Without this, AP-side error
...
async def test_session_interrupt_event_404s_when_no_turn_in_flight(
    ...
    than silently no-op so a stray interrupt from OmniCraft after a turn
...
async def test_idle_watchdog_attaches_recent_forwarder_post_failure(
    ...
    from omnicraft.runtime.harnesses import _scaffold
