...
Loads agent fixtures ported from omnicraft examples, builds
...
Fixture parity with omnicraft example YAMLs:

- ``tests/_fixtures/agents/policies-demo/`` ↔
  ``omnicraft/examples/agent_with_policies.yaml``
- ``tests/_fixtures/agents/rate-limited-search/`` ↔
  ``omnicraft/examples/rate_limited_search_agent.yaml``
- ``tests/_fixtures/agents/secure-research/`` ↔
  ``omnicraft/examples/secure_research_agent.yaml``

Corresponding omnicraft test cases ported:
...
async def test_policies_demo_allows_short_sleep(
    ...
    FunctionPolicy. Mirrors the omnicraft "Allowed" usage
...
async def test_policies_demo_denies_long_sleep(
    ...
    omnicraft "Blocked tool call" example at the top of
...
async def test_rate_limited_search_first_three_allowed(
    ...
    """Ports omnicraft ``test_first_db_query_allowed_but_escalates``
...
async def test_rate_limited_search_fourth_asks(
    ...
    """Ports omnicraft ``test_second_db_query_requires_ask``
...
async def test_secure_research_clean_agent_allows_shell(
    ...
    """Ports omnicraft ``test_clean_agent_calls_freely``. An
...
async def test_secure_research_web_then_shell_asks(
    ...
    of omnicraft ``test_full_pipeline_happy_path``."""
    engine = _load_engine("secure-research", conversation_store)
