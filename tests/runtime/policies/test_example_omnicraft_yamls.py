...
omnicraft-format example YAMLs under ``examples/*.yaml``.

Complements :mod:`test_enforcement_integration`, which loads
pre-translated omnicraft-native fixtures. The fixtures there
are hand-maintained ports; any bug in the omnicraft → omnicraft
adapter layer (e.g. ``condition: {}`` parse rejection,
``match_tools`` → ``on:`` expansion) slips past those tests. This module goes through
:func:`omnicraft.spec.load` — the same path ``omnicraft run``
...
from omnicraft.spec.types import Phase, PolicyAction
...
def _load_engine_from_yaml(
    ...
    """
    Parse an omnicraft-format example YAML and build a real
    :class:`PolicyEngine` bound to a fresh conversation.

    Goes through :func:`omnicraft.spec.load`, so the
    ``_omnicraft_compat`` adapter runs on every call. A bug
...
async def test_agent_with_policies_allows_short_sleep(
    ...
    omnicraft YAML ends at ALLOW for an in-bounds duration.
    """
...
async def test_agent_with_policies_denies_long_sleep(
    ...
    OmniCraft' engine: it DOES. The engine calls 2-arg
    ...
    Claim: the omnicraft YAML + its legacy-style example
    callable actually works through OmniCraft' engine. If
...
# composition is already covered at the omnicraft-native
...
async def test_secure_research_clean_shell_allows(
    ...
    omnicraft ``labels:`` block and the enforcement chain
...
# which collides with an omnicraft reserved builtin name. The
