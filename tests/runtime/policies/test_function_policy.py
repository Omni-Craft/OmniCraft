...
Ports and extends these omnicraft cases:
...
from omnicraft.policies.function import (
    FunctionPolicy,
...
async def test_sync_callable_allow() -> None:
    """Ports omnicraft ``test_sync_callable_allow``. A sync
...
async def test_sync_callable_block() -> None:
    """Ports omnicraft ``test_sync_callable_block``. A sync
...
async def test_async_callable() -> None:
    """Ports omnicraft ``test_async_callable``. An async
...
async def test_callable_returns_dict_allow() -> None:
    """Ports omnicraft ``test_callable_returns_dict``. A
...
async def test_callable_returns_dict_deny_with_reason() -> None:
    """Ports omnicraft ``test_deny_action_from_dict``. A
...
async def test_callable_returns_foreign_policy_result_shape() -> None:
    ...
    ``PolicyResult`` from ``omnicraft.policies`` — a different
    module than the engine's ``omnicraft.policies.types`` so
    ``isinstance`` fails.

    Uses a local stand-in dataclass so the test doesn't
    depend on whether omnicraft is installed in this
    ...
    class _ForeignAction(enum.Enum):
        """
        Mimics ``omnicraft.policies.PolicyAction`` — wire
        values match OmniCraft', but the enum class is
    ...
    class _ForeignPolicyResult:
        """
        Mimics ``omnicraft.policies.PolicyResult`` — same
...
async def test_two_arg_callable_reads_config_for_decision(
    ...
    """Ports omnicraft ``test_three_arg_callable_reads_labels_for_decision``.
...
async def test_async_two_arg_callable() -> None:
    """Ports omnicraft ``test_three_arg_async_callable``.
...
async def test_rate_limit_closure_counts() -> None:
    """Ports omnicraft ``test_tool_call_rate_limit``. A
...
async def test_factory_closure_counter_isolated_per_build(
    ...
    """Ports omnicraft ``test_rate_limit_counter_isolated``.
...
def test_function_policy_reset_turn_invokes_callable_attribute(
    ...
    omnicraft policies like ``max_tool_calls_per_turn`` clear
    per-turn accumulators between turns — see
    :meth:`omnicraft.runtime.policies.engine.PolicyEngine.reset_turn`
