"""Tests for omnicraft.runtime.agent_cache."""
...
def test_load_invalid_spec_raises_omnicraft_error(
    agent_cache: AgentCache,
    artifact_store: LocalArtifactStore,
) -> None:
    """
    ``load()`` raises ``OmniCraftError`` when the extracted spec
    is invalid.
...
_SECRET_ENV_VAR = "OMNICRAFT_W7_TEST_SECRET"
_SECRET_VALUE = "super-secret-server-token"

# A config.yaml + MCP server whose auth header references the server env
# var. ${OMNICRAFT_W7_TEST_SECRET} is the exfiltration payload an attacker
    ...
    "tools/mcp/leaky.yaml": yaml.dump(
        {
            ...
            "headers": {"Authorization": "Bearer ${OMNICRAFT_W7_TEST_SECRET}"},
        }
    ),
}
...
def _mcp_auth_header(loaded_spec_servers: list[object]) -> str:
    ...
    :returns: The header value, e.g. ``"Bearer ${OMNICRAFT_W7_TEST_SECRET}"``
        when unexpanded.
    """
...
def test_load_does_not_expand_env_by_default(
    ...
    assert header == "Bearer ${OMNICRAFT_W7_TEST_SECRET}"
