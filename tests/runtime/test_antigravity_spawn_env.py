...
from omnicraft.runtime import workflow as wf
...
def _isolate_global_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    Empty OMNICRAFT_CONFIG_HOME + file secret backend + cleared
...
def test_stored_gemini_key_used_when_spec_has_no_auth(
    ...
    """A Gemini key registered via ``omnicraft setup`` (the ``antigravity:``
