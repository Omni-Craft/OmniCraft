"""
Unit tests for :mod:`omnicraft.runtime.pending_elicitations`.
...
The wire-up between :func:`omnicraft.runtime.session_stream.publish`
...
def test_resolve_removes_outstanding_id() -> None:
    ...
    the OmniCraft server's approval dispatch calls
...
def test_record_publish_clears_index_on_elicitation_resolved_event() -> None:
    ...
    own approval wait — that's the only signal the OmniCraft server gets
...
def test_record_publish_handles_resolved_event_for_unknown_id() -> None:
    ...
    OmniCraft server's view of what's currently tracked.
    """
    pending_elicitations.record_publish(
        "conv_a",
        {
...
def test_set_elicitation_observer_none_clears_registration() -> None:
    ...
    # OmniCraft server lifespans (the lifespan callsite calls clear at
    # shutdown).
    assert received == ["conv_a"]
