"""
Tests for ``_build_pi_spawn_env`` in ``omnicraft/runtime/workflow.py``.
...
from omnicraft.runtime.workflow import _build_pi_spawn_env
...
def _isolate_global_config(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    this file so the developer's real ``~/.omnicraft/config.yaml`` (e.g.
