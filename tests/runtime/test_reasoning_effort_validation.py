...
async def test_openai_agents_coerces_max_to_xhigh(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    from omnicraft.inner.openai_agents_sdk_executor import _AgentsSessionState

    fake_agents = types.SimpleNamespace(OpenAIProvider=lambda **kwargs: types.SimpleNamespace())
    monkeypatch.setattr(
        "omnicraft.inner.openai_agents_sdk_executor._ensure_agents_sdk", lambda: fake_agents
    )
