...
from omnicraft.runner.transports.ws_tunnel.frames import HelloFrame
...
class HostStartInvocation:
    ...
    Parsed from the ``omnicraft host`` start command the managed-launch
    ...
    :param host_id: Value of the injected ``OMNICRAFT_HOST_ID``.
    :param host_name: Value of the injected ``OMNICRAFT_HOST_NAME``.
    :param token: Value of the injected ``OMNICRAFT_HOST_TOKEN`` — the
        raw launch token.
...
def _parse_host_start(command: str) -> HostStartInvocation:
    ...
    The managed flow injects ``OMNICRAFT_HOST_TOKEN`` / ``_HOST_ID`` /
    ...
    """
    values: dict[str, str] = {}
    for var in ("OMNICRAFT_HOST_TOKEN", "OMNICRAFT_HOST_ID", "OMNICRAFT_HOST_NAME"):
    ...
    return HostStartInvocation(
        host_id=values["OMNICRAFT_HOST_ID"],
        host_name=values["OMNICRAFT_HOST_NAME"],
        token=values["OMNICRAFT_HOST_TOKEN"],
        command=command,
    )
...
def install_fake_modal_launcher(
    ...
    """
    import omnicraft.onboarding.sandboxes.modal as modal_mod
...
def install_fake_daytona_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.daytona as daytona_mod
...
def install_fake_boxlite_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.boxlite as boxlite_mod
...
def install_fake_islo_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.islo as islo_mod
...
def install_fake_e2b_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.e2b as e2b_mod
...
def install_fake_openshell_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.openshell as openshell_mod
...
def install_fake_kubernetes_launcher(
    ...
    :param fake: The fake launcher to substitute.
    """
    import omnicraft.onboarding.sandboxes.kubernetes as kubernetes_mod
...
def build_agent_bundle(
    ...
    :param executor: Optional executor block to write verbatim,
        e.g. ``{"type": "omnicraft", "config": {"harness":
...
class CapturingRunnerClient:
    ...
    Records every control event the OmniCraft server POSTs to the runner's
    ...
    ``monkeypatch.setattr("omnicraft.runtime._globals._runner_client", c)``;
