...
by spawning a real ``omnicraft server`` subprocess (no ``--agent``),
...
def local_server(tmp_path: Path) -> tuple[str, int]:
    """
    Spawn a bare ``omnicraft server`` (no ``--agent``) and yield
    ``(base_url, server_pid)``.
...
def test_external_runner_connects_to_local_server(
    ...
    registers with a local ``omnicraft server`` and is reported
    online.
