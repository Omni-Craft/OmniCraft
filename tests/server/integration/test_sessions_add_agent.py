...
_CODEX_EXECUTOR: dict[str, Any] = {"type": "omnicraft", "config": {"harness": "codex"}}
...
async def test_add_claude_native_child_applies_wrapper_label(
    ...
    when its conversation carries ``omnicraft.wrapper`` =
    ...
    #  - omnicraft.wrapper → runner routes the session to Claude Code.
    #  - omnicraft.ui=terminal → AppShell renders it terminal-first.
    assert row["labels"].get("omnicraft.wrapper") == "claude-code-native-ui"
    assert row["labels"].get("omnicraft.ui") == "terminal"
...
@pytest.mark.xfail(
    reason=(
        ...
        "omnicraft/server/API.md, not wired in app.py). Flips to XPASS "
