...
def _seed_child(
    *,
    ...
    Mirrors what :func:`omnicraft.tools.builtins.spawn._spawn_one` does,
...
async def test_closed_child_session_display_is_sanitized_and_read_only(
    ...
    ``omnicraft.closed=true`` label for clients, and reject new user
...
async def test_native_subagent_session_stamps_terminal_ui_labels(
    ...
    the Chat/Terminal pill (gated on ``omnicraft.ui == "terminal"``).
    ...
    assert labels.get("omnicraft.ui") == "terminal"
...
async def test_native_subagent_message_uses_native_terminal_forward(
    ...
    ``claude-native`` or ``codex-native``, OmniCraft must forward the prompt to
    ...
    assert child["labels"].get("omnicraft.wrapper") == expected_wrapper

    forwarded: list[dict[str, Any]] = []

    def _handler(request: httpx.Request) -> httpx.Response:
        """
        Capture the event OmniCraft forwards to the fake runner.
...
async def test_non_native_subagent_session_has_no_terminal_ui_labels(
    ...
    assert resp.status_code == 201, resp.text
    labels = resp.json()["labels"]
    assert "omnicraft.wrapper" not in labels
    assert "omnicraft.ui" not in labels
...
async def test_subagent_message_heals_stale_runner_binding_via_parent(
    ...
    monkeypatch.setattr(sessions_module, "_ensure_runner_session_initialized", _no_init)
...
async def test_sdk_subagent_heal_skips_session_init(
    ...
    monkeypatch.setattr(sessions_module, "_get_runner_client", _runner_none)
    monkeypatch.setattr(sessions_module, "_heal_subagent_runner_binding_via_parent", _heal_spy)
    monkeypatch.setattr(sessions_module, "_ensure_runner_session_initialized", _init_spy)
