...
the OmniCraft server stays harness-agnostic: it forwards the control to the
...
* other harnesses **204** no-op — the OmniCraft server owns the operation.
* a failed injection (pane not attached) returns **503**.

These tests pin the OmniCraft side of that contract by stubbing the runner's
...
from omnicraft.runtime.compaction import CompactionResult
...
def _fake_runner_returning(compact_status: int) -> tuple[httpx.AsyncClient, list[dict[str, Any]]]:
    ...
    the test can assert the OmniCraft server actually forwarded the control,
...
async def test_compact_skips_omnicraft_compaction_when_runner_handles_it(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    A 200 from the runner (claude-native injected ``/compact``) makes
    the OmniCraft server skip its own compaction.
    ...
    When the runner reports it handled the control (200), the OmniCraft server
    ...
    async def _must_not_run(**_: Any) -> CompactionResult:
        ...
        raise AssertionError(
            ...
            "reported it handled /compact (200). The OmniCraft server fell "
            "through to its own compaction instead of skipping."
        )

    monkeypatch.setattr(
        "omnicraft.runtime.workflow.compact_conversation_now",
        _must_not_run,
    )
    ...
    # handled it, OmniCraft returned without running (or raising from) its own
    # compaction.
    assert resp.status_code == 202, resp.text
    assert resp.json() == {"queued": False}, resp.text
    # Exactly one compact control was forwarded to the runner. 0 = the
    # OmniCraft server never forwarded (it would have run _run_compact_locked
...
async def test_compact_runs_omnicraft_compaction_when_runner_noops(
    client: httpx.AsyncClient,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    A 204 from the runner (in-process harness) makes the OmniCraft server run
    ...
    OmniCraft server it owns the operation, so it must still forward the
    ...
    # OmniCraft server is harness-agnostic and forwards for every harness.
    assert captured == [{"type": "compact"}], (
...
async def test_compact_errors_when_runner_injection_fails(
    ...
    wrong (summarising the mirror). The OmniCraft server must surface the
    ...
    async def _must_not_run(**_: Any) -> CompactionResult:
        ...
        raise AssertionError(
            "compact_conversation_now must not run when the runner "
            "returned a non-200/204 status — OmniCraft fell through to its "
...
async def test_external_compaction_status_publishes_compaction_sse(
    ...
    post-compaction SessionStart (completed) hooks. OmniCraft must translate it
