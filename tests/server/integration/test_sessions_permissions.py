...
from omnicraft.server.auth import LEVEL_EDIT, LEVEL_MANAGE, LEVEL_OWNER, LEVEL_READ
...
def auth_app(
    ...
    return create_app(
        ...
        # create_auth_provider() so ambient OMNICRAFT_* env vars in
...
def local_auth_app(
    ...
    ``OMNICRAFT_LOCAL_SINGLE_USER=1``): requests without
...
async def test_single_user_local_can_access_own_session(
    ...
    — the bare `omnicraft run` / local web UI flow.
    """
