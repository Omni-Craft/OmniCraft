...
import omnicraft.server.routes.sessions as sessions_routes
from omnicraft.server.auth import LEVEL_OWNER, UnifiedAuthProvider
from omnicraft.server.routes.sessions import SessionLiveness, create_sessions_router
...
def test_watch_set_truncated_at_cap(
    ...
    with caplog.at_level(logging.WARNING, logger="omnicraft.server.routes.sessions"):
        with TestClient(app).websocket_connect(
            "/v1/sessions/updates", headers={"X-Forwarded-Email": ALICE}
        ) as ws:
