...
from omnicraft.server.auth import LEVEL_EDIT, LEVEL_OWNER, UnifiedAuthProvider
...
def _install_error_handler(app: FastAPI) -> None:
    """Mirror ``create_app()``'s OmniCraftError → HTTP translation.
    ...
    async def _handle_omnicraft_error(request: Request, exc: OmniCraftError) -> JSONResponse:
        """Translate OmniCraftError to its HTTP status."""
        del request
        return JSONResponse(
...
def test_create_session_rejects_cost_control_label_seed(
    ...
    resp = TestClient(app).post(
        ...
        headers={"X-Forwarded-Email": ALICE, "Origin": OMNICRAFT_INTERNAL_WS_ORIGIN},
    )
