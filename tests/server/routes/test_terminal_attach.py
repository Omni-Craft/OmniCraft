...
from omnicraft.entities import Conversation, SessionPermission
...
def server_registry(tmp_path: Path) -> Iterator[TerminalRegistry]:
    ...
    The route reads via :func:`omnicraft.runtime.get_terminal_registry`,
...
async def test_attach_terminal_local_fallback_spawns_tmux(
    ...
    monkeypatch.setattr("omnicraft.terminals.ws_bridge.os.execve", fake_execve)
    monkeypatch.setattr(
        "omnicraft.terminals.ws_bridge.os._exit",
