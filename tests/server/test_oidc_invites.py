...
1. ``OIDCConfig`` parsing of ``OMNICRAFT_OIDC_ALLOW_INVITES`` and the
...
from omnicraft.server.oidc_access import OidcAdmissionPolicy
...
@pytest.mark.parametrize(
    ("value", "expected"),
    # Follows the project env-var convention (omnicraft.server.auth
...
def test_allow_invites_env_parsing(
    ...
    """``OMNICRAFT_OIDC_ALLOW_INVITES`` parses truthy values; default off.
    ...
    monkeypatch.setenv("OMNICRAFT_OIDC_CLIENT_ID", "cid")
    monkeypatch.setenv("OMNICRAFT_OIDC_CLIENT_SECRET", "secret")
    monkeypatch.setenv("OMNICRAFT_OIDC_REDIRECT_URI", "https://app.example.com/auth/callback")
    monkeypatch.setenv("OMNICRAFT_OIDC_COOKIE_SECRET", "aa" * 32)
    monkeypatch.setenv("OMNICRAFT_OIDC_ALLOW_INVITES", value)
