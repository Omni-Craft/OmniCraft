"""
Unit tests for the OmniCraft YAML spec adapter.
...
- Dispatch detection in :func:`omnicraft.spec.load`:
  omnicraft YAMLs route to the adapter; omnicraft YAMLs
  (identified by ``spec_version``) use the existing parser.

These are the phase 2 translation unit tests + fail-loud tests
called out in ``designs/OMNICRAFT_INTEGRATION.md`` under the
phase 2 test scope.

Round-trip tests live in ``test_omnicraft_roundtrip.py`` since
...
from omnicraft.spec.omnicraft import (
    OMNICRAFT_EXECUTOR_TYPE,
    OMNICRAFT_TOOL_LANGUAGE,
    agent_def_to_agent_spec,
)
from omnicraft.spec.types import AgentSpec

if TYPE_CHECKING:
    from omnicraft.inner.datamodel import AgentDef
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal omnicraft YAML: ``name`` + ``prompt`` only.
...
def executor_block_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with an ``executor:`` block declaring
...
def function_tools_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with one function-type tool whose
...
def policies_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML declaring a ``policies:`` block. The adapter
    lifts this into ``AgentSpec.guardrails.policies`` so the
    omnicraft workflow enforces it at the configured phases.
...
def os_env_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML declaring a top-level ``os_env:`` block. The
...
def mcp_tool_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with a stdio MCP-type tool.
    ...
    :class:`~omnicraft.tools.mcp.McpServerConnection`.

    Includes ``executor.harness`` so the synthesized AgentSpec
    passes :mod:`omnicraft.spec.validator` — the adapter runs
    ...
    config = {
        ...
        "tools": {
            "glean": {
                ...
                "args": ["-m", "omnicraft.inner.databricks_mcps.glean"],
            },
        },
    }
...
def mcp_http_tool_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with an HTTP MCP-type tool (``url`` + headers).
...
def mcp_databricks_server_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with the ``databricks_server`` MCP shape —
    omnicraft has no resolver for it, so the adapter rejects.
    """
    config = {
        "name": "mcp_db_example",
...
def cancellable_tool_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML declaring a legacy ``cancellable_function``
...
@pytest.fixture()
def omnicraft_spec_dir(tmp_path: Path) -> Path:
    """
    An omnicraft spec directory (``spec_version: 1`` in
    ``config.yaml``). Routes through the existing parser, not
    the omnicraft adapter. Used by the detection tests.
    """
    config = {
        "spec_version": 1,
        "name": "ap-agent",
...
def test_agent_def_to_agent_spec_hello_world(
    ...
    executor.type='omnicraft'.

    What breaks if this fails: the baseline phase 2 dispatch —
    ``omnicraft chat hello_world.yaml`` can't produce a valid spec
    ...
    # spec_version is synthesized to the current omnicraft
    # schema version (no spec_version in omnicraft YAMLs).
    assert spec.spec_version == 1
    # executor.type drives the runtime to pick OmniCraftExecutor.
    assert spec.executor.type == OMNICRAFT_EXECUTOR_TYPE
...
def test_agent_def_to_agent_spec_accepts_claude_harness_alias(tmp_path: Path) -> None:
    """OmniCraft YAML may use ``harness: claude`` as a spec-level alias."""
...
def test_agent_def_to_agent_spec_executor_block(
    ...
    What breaks if this fails: the OmniCraftExecutor cannot
    pick a harness or workspace profile at instantiation time,
    so every non-trivial omnicraft YAML routes to the wrong
    harness (or fails).
    """
    from omnicraft.inner.loader import load_agent_def
...
def test_agent_def_to_agent_spec_function_tool(
    ...
    What breaks if this fails: OmniCraftExecutor cannot resolve
    ...
    # from an omnicraft YAML and must be re-resolved via
    # importlib.import_module rather than read off disk.
    assert tool.language == OMNICRAFT_TOOL_LANGUAGE
...
def test_agent_def_to_agent_spec_translates_catalog_path_tool(tmp_path: Path) -> None:
    ...
    """
    from omnicraft.inner.loader import load_agent_def
    from omnicraft.spec.types import ToolRuntime
...
def test_function_tool_parameters_derived_from_callable_signature(
    ...
    the OmniCraft adapter introspects the resolved Python callable's
    signature and exposes that as the LLM-facing JSON-Schema
    ``parameters`` block. Without this fallback, omnicraft
...
def test_load_omnicraft_yaml_missing_package_raises_with_install_hint(
    hello_world_yaml: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    When the ``omnicraft`` package is not importable (e.g. agent-
    plane pip-installed standalone without the sibling omnicraft
    source on PYTHONPATH), loading an omnicraft YAML must surface
    a friendly :class:`OmniCraftError` with an install hint —
    not a bare ``ModuleNotFoundError`` from deep in the import
    machinery.

    What breaks if this fails: a user running ``omnicraft chat foo.yaml``
    from an env that only has omnicraft gets a cryptic
    ``ModuleNotFoundError: No module named 'omnicraft'`` from
    ``_omnicraft_compat.py``'s import line, with no clue what
    to install. The rewritten error says what's missing and how
    to fix it.
    """
    # Simulate ``omnicraft`` not being importable by wiping it
    ...
    for mod_name in list(sys.modules):
        if mod_name == "omnicraft" or mod_name.startswith("omnicraft."):
    ...
    class _OmniCraftBlocker:
        """Meta-path finder that refuses to resolve omnicraft."""
        ...
        def find_spec(
            ...
            del path, target
            if fullname == "omnicraft" or fullname.startswith("omnicraft."):
...
def test_load_policies_yaml_lifts_into_guardrails(policies_yaml: Path) -> None:
    """
    OmniCraft YAMLs with a ``policies:`` block produce an
    AgentSpec whose ``guardrails.policies`` carries the
    translated policy, preserving ``name``, the dotted callable
    path, and the phase. The omnicraft workflow then enforces
    at runtime via the standard :class:`PolicyEngine`.

    What breaks if this fails: policies in an omnicraft YAML
    ...
    assert policy.function.path == "omnicraft.spec._omnicraft_legacy_shim.build"
...
def test_load_os_env_yaml_carries_through_top_level_field(
    ...
    """
    A top-level ``os_env:`` block on an omnicraft YAML
...
def test_load_mcp_stdio_yaml_translates_to_mcp_server(mcp_tool_yaml: Path) -> None:
    """
    OmniCraft YAMLs declaring a subprocess MCP tool translate to
    a native ``MCPServerConfig(transport="stdio", ...)`` entry on
    ``AgentSpec.mcp_servers``. At runtime
    :class:`~omnicraft.tools.mcp.McpServerConnection` spawns the
    ...
    under the OmniCraft integration path.
    """
    ...
    # spawn matches what legacy omnicraft ran.
    assert mcp.command == ".venv/bin/python"
    assert mcp.args == ["-m", "omnicraft.inner.databricks_mcps.glean"]
...
def test_load_mcp_http_yaml_translates_to_mcp_server(mcp_http_tool_yaml: Path) -> None:
    """
    OmniCraft YAMLs with an HTTP MCP (``url`` + headers)
    ...
    omnicraft-legacy to OmniCraft mode get either a translator crash
...
def test_mcp_stdio_yaml_reverse_trip_recovers_mcp_tool(mcp_tool_yaml: Path) -> None:
    ...
    path is what :meth:`OmniCraftExecutor.from_spec` calls
    when wrapping an omnicraft spec for an omnicraft
    harness; a missing reverse translation drops every MCP
    tool from the AgentDef the harness sees.

    What breaks if this fails: a live OmniCraft mode run with a
    ...
    the live E2E test under tests/e2e/omnicraft/ guards
    against.
    """
    from omnicraft.inner.tools import MCPTool
    from omnicraft.spec.omnicraft import agent_spec_to_agent_def
    ...
    assert tool.args == ["-m", "omnicraft.inner.databricks_mcps.glean"]


def test_load_mcp_databricks_server_yaml_raises(mcp_databricks_server_yaml: Path) -> None:
    """
    OmniCraft MCP tools using the ``databricks_server=<name>``
    shape fail loud — OmniCraft' MCPServerConfig doesn't
    ...
    """
    with pytest.raises(OmniCraftError, match="databricks_server"):
...
def test_load_cancellable_function_yaml_rejected_post_step_c(
    ...
    """
    OmniCraft YAMLs declaring ``type: cancellable_function``
    are rejected by the OmniCraft adapter with a clear migration hint.
    ...
    inner-stack YAML to OmniCraft mode gets pointed at the new shape.
    ...
    ``TypeError`` instead.
    """
    with pytest.raises(OmniCraftError, match="cancellable_function"):
        load(cancellable_tool_yaml)


# ── Dispatch detection in omnicraft.spec.load ──────────────


def test_load_omnicraft_yaml_routes_to_adapter(
    executor_block_yaml: Path,
) -> None:
    """
    A ``.yaml`` file with ``name`` + ``prompt`` and no
    ``spec_version`` routes through the omnicraft adapter.
    The produced spec has ``executor.type='omnicraft'``, which
    no omnicraft native spec ever emits.
    ...
    itself doesn't depend on which valid omnicraft YAML we feed.

    What breaks if this fails: ``omnicraft chat foo.yaml`` against an
    omnicraft YAML would either crash (no config.yaml in a
    file-shaped source) or silently parse as an omnicraft
...
def test_load_omnicraft_directory_uses_existing_parser(
    omnicraft_spec_dir: Path,
) -> None:
    """
    An omnicraft spec directory (``spec_version`` declared)
    routes through the existing parser unchanged. The resulting
    spec has ``executor.type='omnicraft'`` (the default)
    and no omnicraft extras.

    What breaks if this fails: all existing omnicraft specs
    would stop loading (regression surfaces in every omnicraft
    test suite, so this is a smoke check).
    """
    spec = load(omnicraft_spec_dir)
...
def test_load_yaml_with_spec_version_not_routed_to_adapter(
    ...
    an omnicraft spec. The detection rule's negative check on
    ``spec_version`` prevents misrouting.

    This case currently still fails (omnicraft YAML specs are
    directories, not files), but the dispatch MUST pick the
    non-omnicraft branch so the resulting error is an
    omnicraft "dest is required" / parser error — NOT an
    omnicraft adapter error. The assertion below verifies the
    error shape is NOT an omnicraft-adapter error.

    What breaks if this fails: a future change that starts
    supporting omnicraft YAML-file specs would silently route
    to the wrong adapter.
    """
    config = {
        "spec_version": 1,
        "name": "hybrid",
        "prompt": "Looks omnicraft-y but is omnicraft.",
    }
    path = tmp_path / "hybrid.yaml"
    path.write_text(yaml.dump(config))

    # ``spec_version`` marks this file as an omnicraft spec, but
    # omnicraft specs must live in a directory with a
    # ``config.yaml``, not a single YAML file. ``load()`` rejects with
    # an actionable diagnostic. If detection were wrong, we'd get an
    # omnicraft-adapter error (e.g. "missing system-prompt key")
    # instead — the assertion below pins the omnicraft shape.
    with pytest.raises(OmniCraftError, match="spec_version"):
        load(path)
...
def test_cancellable_function_parameters_forward_trip_preserves_input_schema() -> None:
    ...
    **What breaks if this fails**: the adapter silently translates
    ...
    from omnicraft.inner.tools import CancellableFunctionTool
...
def test_function_tool_parameters_round_trip_preserves_input_schema(
    ...
    explicit ``input_schema`` round-trips through omnicraft
    translation and back without losing the schema.

    Step (c) made plain callables the only supported function-tool
    shape on the OmniCraft path. Schema preservation matters because the
    ...
    from omnicraft.inner.datamodel import AgentDef
    from omnicraft.inner.datamodel import ExecutorSpec as OmniExecutorSpec
    from omnicraft.inner.tools import FunctionTool
    from omnicraft.spec import omnicraft as spec_omni
    from omnicraft.spec.omnicraft import agent_def_to_agent_spec
...
def test_inline_agent_tool_inherit_resolves_to_parent_os_env() -> None:
    ...
    :class:`OSEnvSpec` at translation time — omnicraft spawns
    ...
    legacy omnicraft semantics — silently breaks.
    """
    from omnicraft.inner.datamodel import (
        AgentDef,
        OSEnvSpec,
    ...
    from omnicraft.inner.tools import AgentTool
    from omnicraft.spec.omnicraft import agent_def_to_agent_spec
...
def test_inline_agent_tool_inherit_with_no_parent_os_env_yields_none() -> None:
    ...
    ``None`` — matches legacy omnicraft behavior when the
...
def test_instructions_field_falls_back_to_prompt_when_unset() -> None:
    ...
    omnicraft YAML written before the field existed.
    """
    from omnicraft.inner.datamodel import AgentDef
    from omnicraft.spec.omnicraft import agent_def_to_agent_spec
...
def test_instructions_yaml_loads_through_full_pipeline(tmp_path: Path) -> None:
    """
    End-to-end through ``load_omnicraft_yaml`` (the integration
    path the OmniCraft server hits when registering an omnicraft
    bundle): YAML with ``instructions: AGENTS.md`` produces a
    spec whose ``instructions`` field carries the file's
    contents.
    """
    from omnicraft.spec._omnicraft_compat import load_omnicraft_yaml
    ...
    spec = load_omnicraft_yaml(yaml_path)
    assert spec.instructions == "FROM AGENTS DOT MD"


# ── Terminals threading (OMNICRAFT_TERMINAL_BRIDGE §6.1) ────────────
...
def test_terminals_thread_through_translator() -> None:
    ...
    makes ``terminals:`` declarations in omnicraft YAML reach the
    AP-side ``sys_terminal_*`` tools — the whole feature from
    ``designs/OMNICRAFT_TERMINAL_BRIDGE.md`` collapses if this breaks.

    What breaks if this fails: omnicraft YAMLs that declare
    ``terminals:`` boot under OmniCraft mode with
...
def test_inline_agent_tool_inherits_parent_terminals() -> None:
    ...
    ``omnicraft/tools/manager.py:426``) and the sub-agent has
    no way to launch a terminal even though the parent has one
    configured. The supervisor pattern stops working again.
    """
    from omnicraft.inner.datamodel import AgentDef, TerminalEnvSpec
    from omnicraft.inner.datamodel import (
        ExecutorSpec as OmniExecutorSpec,
    )
    from omnicraft.inner.tools import AgentTool
...
def test_harness_auto_picks_from_model_prefix(
    ...
    """
    When an omnicraft YAML declares a model but no harness,
    the adapter fills in the right harness by matching the
    model prefix against
    :data:`~omnicraft.spec.omnicraft._HARNESS_FOR_MODEL_PREFIX`.

    Mirrors the auto-pick pure omnicraft' CLI does at
    ...
    blocking the entire OmniCraft path.
    """
    from omnicraft.inner.datamodel import AgentDef
    from omnicraft.inner.datamodel import ExecutorSpec as OmniExecutorSpec
    from omnicraft.spec.omnicraft import agent_def_to_agent_spec
...
def test_harness_auto_pick_unknown_model_raises() -> None:
    ...
    :raises OmniCraftError: With a message explaining that the
    ...
    with pytest.raises(OmniCraftError, match=r"[Hh]arness"):
...
# :func:`omnicraft.loader.load_agent_def`) because the loader
...
def _build_agent_def_with_raw_yaml(
    ...
    omnicraft loader (here stubbed with an empty ``policies``
    ...
    :param policies: Raw omnicraft ``policies:`` dict.
    :param labels: Raw omnicraft top-level ``labels:`` dict
        (initial values).
    :param label_schema: Raw omnicraft top-level
        ``label_schema:`` dict.
    :param ask_timeout: Raw omnicraft top-level
        ``ask_timeout:`` value.
...
def test_function_policy_routes_callable_through_legacy_shim() -> None:
    ...
    legacy omnicraft ``(content, phase)`` convention keep
    working under OmniCraft' ``(ctx, context)`` convention —
    see ``omnicraft.spec._omnicraft_legacy_shim``. The shim
    is a runtime no-op for omnicraft-native callables, so
...
def test_function_policy_callable_alias_resolves_identically_to_handler() -> None:
    ...
    Old omnicraft YAMLs used ``callable:`` as the key name; current
...
def test_ask_timeout_top_level_propagates_to_guardrails() -> None:
    """
    The omnicraft top-level ``ask_timeout:`` lands on
    :attr:`GuardrailsSpec.ask_timeout`.

    What breaks if this fails: ASK-action policies park forever
    (no timeout) or fall back to the omnicraft default even
...
def test_no_policies_no_guardrails_block() -> None:
    """
    An omnicraft YAML without any policies/labels/ask_timeout
    produces a spec with ``guardrails=None``, matching the
    zero-policy case documented in POLICIES.md §10.

    What breaks if this fails: omnicraft would instantiate a
...
def test_executor_extra_field_propagates_to_llm_config() -> None:
    ...
    ``parallel_tool_calls``, etc.) through the OmniCraft path, even
    though those knobs work fine via legacy omnicraft. Makes
    OmniCraft mode a downgrade rather than a compatible integration.
    """
    agent_def, raw_yaml = _build_agent_def_with_raw_yaml()
    raw_yaml["executor"] = {
        "model": "databricks-gpt-5-mini",
...
def test_executor_extra_absent_yields_empty_llm_extra() -> None:
    """
    When the omnicraft YAML omits ``executor.extra``, the
...
def test_use_responses_false_propagates_to_executor_config() -> None:
    """
    ``use_responses: false`` in an omnicraft YAML executor block lands on
    ``spec.executor.config["use_responses"]`` as ``False`` after
    ``agent_def_to_agent_spec``.

    The inner ``ExecutorSpec`` dataclass (``omnicraft.inner.datamodel``)
    has no ``use_responses`` field, so the omnicraft YAML loader silently
...
def test_use_responses_absent_omits_key_from_executor_config() -> None:
    """
    When the omnicraft YAML omits ``use_responses``, the key is absent
...
def test_unknown_policy_type_rejected_with_clear_message() -> None:
    ...
    with pytest.raises(OmniCraftError, match="weird") as exc_info:
...
def test_self_clone_string_shorthand_loader_produces_selfagent_tool(
    ...
    """
    from omnicraft.inner.loader import load_agent_def
    from omnicraft.inner.tools import SelfAgentTool
...
def test_agent_def_to_agent_spec_self_clone_propagates_parent_config(
    ...
    """
    Translating an omnicraft YAML with ``tools.subtask: self``
    ...
    assert sub.executor.type == OMNICRAFT_EXECUTOR_TYPE
...
def test_compat_yaml_executor_auth_is_not_dropped(tmp_path: Path) -> None:
    """
    ``executor.auth:`` declared in an omnicraft-compat YAML is preserved
    ...
    Uses ``load_omnicraft_yaml`` — the real production entry point — so
...
def test_compat_yaml_executor_api_key_auth_is_not_dropped(tmp_path: Path) -> None:
    """
    ``executor.auth: {type: api_key, …}`` in an omnicraft-compat YAML is
    ...
    """
    from omnicraft.spec._omnicraft_compat import load_omnicraft_yaml
    from omnicraft.spec.types import ApiKeyAuth
