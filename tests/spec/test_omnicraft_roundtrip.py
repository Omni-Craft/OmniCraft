"""
Round-trip invariant tests for the OmniCraft ↔ AgentSpec
adapter.
...
:func:`omnicraft.spec.omnicraft.agent_spec_to_agent_def`,
...
omnicraft-testing skill, skipped tests rot invisibly. The
...
from omnicraft.spec.omnicraft import (
    ...
    # ``omnicraft/spec/omnicraft.py`` per the design. Phase 1
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal omnicraft YAML — name + prompt only. Round-trip
...
def function_tool_yaml(tmp_path: Path) -> Path:
    """
    OmniCraft YAML with one function-type tool pointing at a
...
def _roundtrip(yaml_path: Path) -> None:
    """
    Load the YAML via omnicraft' loader, translate to an
    ...
    **Lossy by design.** :class:`AgentSpec` does not model every
    omnicraft :class:`FunctionTool` field — ``description``,
    ...
    :param yaml_path: Path to an omnicraft YAML fixture.
    """
    ...
    # omnicraft' CLI auto-pick) — the round-trip contract is
...
def test_roundtrip_hello_world_is_incomplete_for_omnicraft(
    ...
    model, which OmniCraft' strict spec rejects on the way
    back. This is intentional: the omnicraft validator requires
    a harness when ``executor.type == "omnicraft"``, and that
...
def test_roundtrip_executor_block(executor_block_yaml: Path) -> None:
    ...
    differs between the two directions — OmniCraftExecutor
