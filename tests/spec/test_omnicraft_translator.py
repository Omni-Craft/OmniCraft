"""Unit tests for ``omnicraft.spec.omnicraft.agent_spec_to_agent_def``.

Phase 1 ships the forward direction only. These tests hand-craft
:class:`AgentSpec` objects and assert the resulting
:class:`omnicraft.datamodel.AgentDef` has the expected shape. The
...
from omnicraft.inner.tools import AgentTool, FunctionTool
from omnicraft.spec import (
    AgentSpec,
    ExecutorSpec,
    FunctionPolicySpec,
    FunctionRef,
...
# SandboxConfig is not re-exported from omnicraft.spec's public
...
from omnicraft.spec.types import SandboxConfig
...
# via ``tests.spec.test_omnicraft_translator.sample_tool_callable``.
...
def basic_spec() -> AgentSpec:
    """
    Minimal ``AgentSpec`` targeting the omnicraft executor.

    :returns: A spec with name, instructions, one ``llm.model``, and
        ``executor.type == "omnicraft"`` carrying harness+profile.
    """
    return AgentSpec(
        spec_version=1,
        name="hello-agent",
...
def test_basic_spec_produces_agent_def_with_name_and_prompt(
    ...
    **What breaks if this fails**: the omnicraft harness can
...
def test_basic_spec_maps_llm_and_executor_config(
    ...
    **What breaks if this fails**: the omnicraft
...
def test_missing_profile_maps_to_none(
    ...
    surfaces ``None`` so the omnicraft
    :class:`~omnicraft.datamodel.ExecutorSpec.profile` field
...
def test_spec_with_local_tool_resolves_dotted_path(
    ...
    **What breaks if this fails**: the harness receives a tool
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="sample",
            path="tests.spec.test_omnicraft_translator.sample_tool_callable",
            language="python",
        ),
    ]
    agent_def = agent_spec_to_agent_def(basic_spec)
...
def test_policies_dropped_from_forward_translation(
    ...
    because omnicraft enforces them upstream of the executor.

    **What breaks if this fails**: two regressions to guard
    against:
    1. The translator starts rejecting again (``OmniCraftError``
       with ``"policies"``) — the omnicraft executor would then
    ...
    2. The translator starts round-tripping policies INTO the
       AgentDef — meaning both the omnicraft workflow AND the
       omnicraft runtime would enforce them, double-counting
       every DENY.
    """
...
def test_sandbox_block_rejected_with_clear_message(
    ...
    **What breaks if this fails**: the harness runs tools outside
    ...
    with pytest.raises(OmniCraftError, match=r"sandbox"):
...
def test_tool_with_filesystem_path_rejected(
    ...
    **What breaks if this fails**: the translator silently drops
    ...
    with pytest.raises(OmniCraftError, match=r"dotted"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_with_unimportable_module_rejected(
    ...
    **What breaks if this fails**: a typo in the tool path
    ...
    with pytest.raises(OmniCraftError, match=r"no_such_module_xyzzy"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_pointing_at_non_callable_rejected(
    ...
    **What breaks if this fails**: the omnicraft harness
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="stringy",
            path="tests.spec.test_omnicraft_translator.NOT_A_FUNCTION",
            language="python",
        ),
    ]
...
def test_missing_llm_rejected(basic_spec: AgentSpec) -> None:
    """
    A spec with ``executor.type='omnicraft'`` but no ``llm``
    block is rejected — the omnicraft harness needs a model
    name. We fail loud at translation time, not deep inside the
    harness constructor.
    """
    basic_spec.executor.model = None
    with pytest.raises(OmniCraftError, match=r"executor\.model"):
        agent_spec_to_agent_def(basic_spec)


# ── Harness inference for native OmniCraft v1 specs ────────────────────────────────
...
def test_native_omnicraft_spec_infers_harness_from_model(
    model: str,
    expected_harness: str,
) -> None:
    """
    Native OmniCraft v1 specs use ``executor.type="omnicraft"`` with no harness in
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="omnicraft", model=model, config={}),
    )
...
def test_sub_agent_infers_harness_and_forwards_os_env() -> None:
    """
    When a parent spec's sub-agent uses a native OmniCraft v1 executor (no
    ...
    sub_spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="omnicraft", model="databricks-claude-sonnet-4", config={}),
        os_env=sub_os_env,
    )
    parent_spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="omnicraft", model="databricks-gpt-5-4", config={}),
...
def test_server_runtime_tool_with_no_path_rejected(
    ...
    :func:`omnicraft.spec.validator.validate`, but the translator
    ...
    with pytest.raises(OmniCraftError, match=r"server-runtime tool has no"):
        agent_spec_to_agent_def(basic_spec)
