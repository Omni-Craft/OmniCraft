"""
Unit tests for :mod:`omnicraft.spec.types`.
...
from omnicraft.spec.types import ExecutorSpec, RetryPolicy


@pytest.mark.parametrize(
    ("executor", "expected"),
    [
        # omnicraft-type agents carry the kind in config.harness.
        (ExecutorSpec(type="omnicraft", config={"harness": "codex"}), "codex"),
        (
            ExecutorSpec(type="omnicraft", config={"harness": "claude-native"}),
            "claude-native",
        ),
        # Non-omnicraft executors carry their kind in `type` and have no
...
def test_retry_policy_from_json_drops_unknown_keys_for_forwards_compat() -> None:
    ...
    OmniCraft serializes it, an older harness wrap (still on the
...
def test_retry_policy_from_json_uses_defaults_for_missing_fields() -> None:
    ...
    Pin: backwards compat — an older OmniCraft serializing a partial
...
def test_retry_policy_from_json_rejects_non_dict_payload() -> None:
    ...
    Pin: defensive against OmniCraft regressions that might
...
def test_retry_policy_from_json_rejects_non_list_status_codes() -> None:
    ...
    Defensive: if OmniCraft serialized it as a string or dict
