"""Tests for omnicraft.spec.validator."""
...
def test_omnicraft_executor_accepts_valid_harness() -> None:
    """
    ``omnicraft`` executor with ``config.harness`` set to one of
...
def test_omnicraft_executor_accepts_antigravity_native_harness() -> None:
    """
    ``omnicraft`` executor with ``config.harness == "antigravity-native"``
    validates cleanly.

    Failure here means the antigravity-native harness is missing from
    ``OMNICRAFT_HARNESSES``, which would cause every spec that targets it
...
def test_omnicraft_executor_rejects_missing_harness() -> None:
    """
    ``omnicraft`` executor without ``config.harness`` is rejected.

    Without the harness selector the omnicraft factory cannot
...
def test_omnicraft_executor_rejects_unknown_harness() -> None:
    """
    ``omnicraft`` executor with a harness not in the allowed set
...
def test_omnicraft_executor_rejects_compaction() -> None:
    """
    ``omnicraft`` executor forbids ``compaction`` — the inner
...
def test_mcp_stdio_valid() -> None:
    ...
    spec/omnicraft.py's _translate_mcp_tool_from_def) would fail
...
# programmatically — by tests, the omnicraft compat shim, or any
