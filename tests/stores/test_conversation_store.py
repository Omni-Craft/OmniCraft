...
def test_get_conversations_bulk(
    ...
    conversation_store.set_labels(a.id, {"omnicraft.ui": "terminal"})
    ...
    assert result[a.id].labels == {"omnicraft.ui": "terminal"}
...
def test_unique_position_constraint(
    ...
    from omnicraft.db.enum_codecs import encode_item_status, encode_item_type
    from omnicraft.db.utils import generate_item_id
...
def test_list_items_type_filter_returns_only_matching_type(
    ...
    """
    from omnicraft.entities import CompactionData
...
def test_append_bumps_updated_at(
    ...
    to the current time.
    """
    import omnicraft.stores.conversation_store.sqlalchemy_store as store_mod
...
def test_create_duplicate_title_under_same_parent_raises(
    ...
    from omnicraft.stores.conversation_store import NameAlreadyExistsError
...
def test_list_conversations_filtered_by_agent_id_returns_matching_only(
    ...
    """
    Powers OmniCraft mode ``--continue`` (resume the most-recent
...
def test_list_conversations_by_runner_id_hydrates_labels(
    ...
    conversation_store.set_labels(
        bound.id,
        {
            "omnicraft.fork.carry_history": "1",
            "omnicraft.fork.source_external_session_id": "src-claude-sid",
        },
    )
...
def test_set_host_id_missing_conversation_raises(
    ...
    """
    from omnicraft.stores.conversation_store import ConversationNotFoundError
...
def test_create_session_with_agent_top_level_unchanged(
    ...
    contract (CLI ``omnicraft run`` sessions): kind stays "default",
...
def test_fork_conversation_drops_instance_scoped_labels(
    ...
    conversation_store.set_labels(
        source.id,
        {
            "omnicraft.claude_native.bridge_id": source.id,
            "omnicraft.codex_native.bridge_id": source.id,
            "omnicraft.last_context_tokens": "39903",
            "omnicraft.last_context_window": "1000000",
            # The dangerous bypass opt-in must NOT ride into the fork.
            "omnicraft.codex_native.bypass_sandbox": "1",
    ...
    # usage.
    assert fork.labels == {"omnicraft.wrapper": "claude-code-native-ui"}, (
...
def test_fork_conversation_stamps_source_external_session_id(
    ...
    ``omnicraft.fork.source_external_session_id`` to resume + branch the
    ...
    assert fork.labels.get("omnicraft.fork.source_external_session_id") == "claude-uuid-abc", (
    ...
    # yet. Copying external_session_id would make two OmniCraft sessions claim
...
def test_fork_conversation_no_external_session_id_no_directive(
    ...
    assert "omnicraft.fork.source_external_session_id" not in fork.labels, (
...
def test_fork_conversation_truncated_drops_external_session_directive(
    ...
    If ``omnicraft.fork.source_external_session_id`` were stamped, the
...
def test_fork_conversation_cross_family_drops_external_session_directive(
    ...
    must not stamp ``omnicraft.fork.source_external_session_id``: the
    ...
    the copied OmniCraft items) instead.
    """
    from omnicraft.stores.conversation_store import (
        FORK_CARRY_HISTORY_LABEL_KEY,
        FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY,
...
def test_fork_conversation_up_to_last_response_keeps_external_directive(
    ...
    """
    from omnicraft.stores.conversation_store import FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY
...
def test_instance_scoped_label_keys_match_harness_constants() -> None:
    ...
    from omnicraft.codex_native_bridge import CODEX_NATIVE_BRIDGE_ID_LABEL_KEY
    from omnicraft.stores.conversation_store import _INSTANCE_SCOPED_LABEL_KEYS
...
def test_fork_conversation_carry_history_into_native_stamps_label(
    ...
    The runner reads ``omnicraft.fork.carry_history`` to decide whether a
...
def test_switch_conversation_agent_cross_family_resets_and_relabels(
    ...
    # omnicraft.stopped marker was retired upstream.
    instance_label = "omnicraft.last_context_tokens"
    ...
    assert "omnicraft.codex_native.bypass_sandbox" not in updated.labels, (
...
def test_get_session_connectivity_batches_runner_and_host(
    ...
    fresh" — the retired ``omnicraft.stopped`` marker is no longer a
...
def test_get_session_connectivity_reports_needs_workspace_for_fork(
    ...
    ``omnicraft.fork.source_id`` label (set by ``fork_conversation``).
    ...
    conversation_store.set_labels(fork.id, {"omnicraft.fork.source_id": "conv_src"})
...
def test_get_session_owner_excludes_public_sentinel(
    ...
    from omnicraft.server.auth import RESERVED_USER_PUBLIC
    from omnicraft.stores.permission_store.sqlalchemy_store import (
        SqlAlchemyPermissionStore,
    )
...
def _stored_next_position(
    ...
    from omnicraft.db.db_models import SqlConversation
