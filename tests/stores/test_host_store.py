...
from omnicraft.db.utils import get_or_create_engine, now_epoch
from omnicraft.stores.host_store import (
    HOST_LIVENESS_TTL_S,
    Host,
    HostStore,
    host_is_live,
...
def test_upsert_reconnect_overwrites_and_nulls_configured_harnesses(
    ...
    # Reconnect with fresh values — the user ran `omnicraft setup`.
    host_store.upsert_on_connect(
        host_id="host_ch2",
        name="laptop2",
        owner="alice@example.com",
...
def test_managed_host_raw_token_never_stored(db_uri: str) -> None:
    ...
    from omnicraft.stores.host_store import hash_host_launch_token
