"""
Unit tests for :class:`omnicraft.terminals.TerminalRegistry`.
...
from omnicraft.inner.terminal import TerminalCreateResult, TerminalInstance
from omnicraft.terminals import TerminalRegistry
from omnicraft.terminals import registry as registry_mod
from omnicraft.terminals.registry import TerminalListEntry, conversation_link_for_id
...
def test_conversation_link_for_id_uses_relative_path_by_default() -> None:
    """
    Conversation links stay relative when no OmniCraft origin is known.
    ...
    at the conversation route when rendered by the OmniCraft web UI.
    """
...
def test_conversation_link_for_id_maps_workspace_hosted_server_to_ui_mount(
    ...
    (``/api/2.0/omnicraft``) — into the registry. A naive
    ``{base}/c/<id>`` would put the JSON API path in the tmux status
    bar; the link must instead land on the ``/omnicraft`` SPA mount and
    carry the ``?o=<org>`` selector ``omnicraft login`` recorded, exactly
    like the CLI's ``Web UI:`` line. Pins parity with
    :func:`omnicraft.conversation_browser.conversation_url`.
    ...
    assert (
        ...
        == "https://example.databricks.com/omnicraft/c/conv_abc123?o=2850744067564480"
    )
...
def test_transfer_moves_terminal_without_closing_tmux(tmp_path: Path) -> None:
    ...
    moving ``claude/main`` from the old OmniCraft conversation to the fresh
...
async def test_cleanup_conversation_closes_all_owners_terminals(
    ...
    sessions; orphans accumulate across the OmniCraft process lifetime.
    """
    await reg_with_tmux.launch("conv_a", "bash", "s1", bash_spec)
    await reg_with_tmux.launch("conv_a", "bash", "s2", bash_spec)
    inst_b = await reg_with_tmux.launch("conv_b", "bash", "s1", bash_spec)
...
async def test_close_with_timeout_still_returns_true(tmp_path: Path) -> None:
    ...
    import omnicraft.terminals.registry as reg_mod
