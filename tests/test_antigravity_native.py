"""Tests for the native Antigravity (``omnicraft antigravity``) launcher.
...
import omnicraft.antigravity_native as _mod
import omnicraft.antigravity_native_bridge as bridge_mod
from omnicraft._wrapper_labels import ANTIGRAVITY_NATIVE_WRAPPER_VALUE, WRAPPER_LABEL_KEY
from omnicraft.antigravity_native import antigravity_terminal_resource_id
...
def _stub_agy_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (its own module), plus the re-export in :mod:`omnicraft.antigravity_native` used
    ...
    """
    monkeypatch.setattr("omnicraft.antigravity_native_launch.agy_binary_path", lambda: "agy")
...
async def test_local_fresh_launch_reattaches_to_runner_autocreated_terminal(
    ...
    ``omnicraft antigravity`` path: the local server spawns a CLI runner, and
...
async def test_cli_cold_start_scopes_to_pane_agy(
    ...
    """
    import omnicraft.antigravity_native_rpc as rpc
