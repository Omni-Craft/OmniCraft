...
Pure unit tests for :mod:`omnicraft.antigravity_native_audit` — the
...
from omnicraft.antigravity_native_audit import (
    DEGRADE_NOTICE_TEXT,
    HARNESS_NAME,
...
def test_audit_request_skips_omnicraft_mcp_tools() -> None:
    """``mcp__omnicraft__*`` tools are relay-enforced; the audit returns None.

    Only the OmniCraft MCP tools are double-counted by the relay path, so
    :func:`build_audit_evaluation_request` (delegating to
    ``hook_payload_to_evaluation_request``) skips exactly those.
    """
    request = build_audit_evaluation_request(
        tool_name="mcp__omnicraft__sys_call", tool_input={}, model=None
    )
...
def test_audit_request_evaluates_connector_mcp_tools() -> None:
    ...
    Unlike ``mcp__omnicraft__*`` (relay-enforced), connector MCP tools are not
