...
from omnicraft import claude_native, codex_native, native_terminal
...
async def test_create_claude_session_persists_terminal_launch_args() -> None:
    ...
    ``omnicraft claude --server X <flags>`` must carry the flags to a
...
def _install_daemon_seam_mocks(
    ...
    *,
    ...
    :returns: None.
    """
    monkeypatch.setattr("omnicraft.chat._remote_headers", lambda server_url=None, **k: {})
    monkeypatch.setattr("omnicraft.chat._server_auth", lambda server_url=None, **k: None)
    monkeypatch.setattr("omnicraft.chat._bundle_agent", lambda path: b"bundle")
    monkeypatch.setattr(
        "omnicraft.cli._ensure_host_daemon",
...
def test_run_with_remote_server_detach_prints_resume_hint(
    ...
    assert (
        "Resume with: omnicraft claude --server https://example.com --resume conv_existing"
    ) in err
...
def test_run_with_remote_server_unreachable_server_raises_clean_error(
    ...
    """
    An unreachable OmniCraft server fails with an actionable message, not a
    raw httpx traceback.
    ...
    assert "Could not reach the omnicraft server at https://unreachable.example" in str(
        exc_info.value
    )
