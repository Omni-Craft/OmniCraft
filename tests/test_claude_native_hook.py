...
def test_session_start_hook_emits_conversation_url_system_message(
    ...
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in OmniCraft: http://127.0.0.1:8787/c/conv_abc"
    }
    assert captured.err == ""
    assert read_transcript_path(bridge_dir) == transcript_path
...
def test_clear_session_start_hook_rotates_before_printing_conversation_url(
    ...
    assert exit_code == 0
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in OmniCraft: http://127.0.0.1:8787/c/conv_new"
    }
    assert captured.err == ""
    assert requests == [
...
def test_fork_session_start_hook_forks_before_printing_conversation_url(
    ...
    assert exit_code == 0
    assert json.loads(captured.out) == {
        "systemMessage": "Open this session in OmniCraft: http://127.0.0.1:8787/c/conv_fork"
    }
    assert captured.err == ""
    assert requests == [
        ("GET", "http://127.0.0.1:8787/v1/sessions/conv_old", None),
