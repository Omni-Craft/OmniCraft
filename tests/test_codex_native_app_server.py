...
from omnicraft.codex_native_hook import _EVALUATE_POLICY_TIMEOUT_S

_CWD = "/home/user/repo"
_OUR_COMMAND = "/venv/bin/python -m omnicraft.codex_native_hook evaluate-policy --bridge-dir /b"
...
def test_build_codex_native_server_profile_error_names_profile(
    ...
    """
    monkeypatch.setattr(
        "omnicraft.codex_native_app_server._find_codex_cli",
        lambda: sys.executable,
    )
    monkeypatch.setattr(
        "omnicraft.codex_native_app_server._databricks_gateway_host",
...
def test_build_codex_native_server_uses_profile_host_without_static_token(
    ...
    A default OmniCraft install may not include ``databricks-sdk`` in the
...
[mcp_servers.omnicraft] # stale generated table
command = "/old/python"
args = ["old"]

[mcp_servers.omnicraft.env] # stale generated env
OLD = "1"
...
"""
    ...
    assert rendered.count("[mcp_servers.omnicraft]") == 1
    assert "[mcp_servers.omnicraft.env]" not in rendered
    parsed = tomllib.loads(rendered)
    assert parsed["mcp_servers"]["other"]["command"] == "other"
    assert parsed["mcp_servers"]["omnicraft"] == {
        "command": "/new/python",
        "args": [
            "-I",
            "-m",
...
async def test_start_writes_fresh_mcp_config_without_leading_blanks(
    ...
    assert rendered.startswith("[mcp_servers.omnicraft]\n")
    parsed = tomllib.loads(rendered)
    assert parsed["mcp_servers"]["omnicraft"] == {
        "command": "/new/python",
        "args": [
...
async def test_untrusted_hook_is_trusted_via_batchwrite() -> None:
    """
    An untrusted OmniCraft hook is trusted with its currentHash.
...
async def test_missing_hook_raises() -> None:
    """
    No discovered OmniCraft hook fails loud (anti fail-open).
...
async def test_user_hooks_are_never_trusted() -> None:
    """
    Only OmniCraft hooks are trusted; user-declared hooks are left alone.
...
def _set_codex_version(
    ...
    monkeypatch.setattr("omnicraft.codex_native_app_server._codex_cli_version", _fake_version)
...
async def test_trust_failure_is_fail_open_with_reason(
    ...
    async def _raise_trust(_self: CodexNativeAppServer) -> None:
        raise RuntimeError("OmniCraft policy hook was not discovered for cwd ...")
...
class TestPinCodexConfigModel:
    ...
    def test_replaces_top_level_model_only(self, tmp_path: Path) -> None:
        ...
        """
        from omnicraft.codex_native_app_server import _pin_codex_config_model
    ...
    def test_read_back_by_forwarder_mirror_source(self, tmp_path: Path) -> None:
        ...
        """
        from omnicraft.codex_native_app_server import _pin_codex_config_model
        from omnicraft.codex_native_bridge import read_codex_config_model
