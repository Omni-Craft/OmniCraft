...
import omnicraft.harness_plugins as hp
...
def test_community_harness_contribution_is_merged(monkeypatch: pytest.MonkeyPatch) -> None:
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"foo": "omnicraft.community.harness.foo.inner.foo_harness"},
            aliases={"foo-code": "foo"},
            model_env_keys={"foo": "HARNESS_FOO_MODEL"},
            spawn_env_builders={"foo": "omnicraft.community.harness.foo.plugin:build_spawn_env"},
    ...
    assert hp.harness_modules()["foo-code"] == "omnicraft.community.harness.foo.inner.foo_harness"
    assert hp.model_env_keys()["foo"] == "HARNESS_FOO_MODEL"
    assert (
        hp.spawn_env_builders()["foo"] == "omnicraft.community.harness.foo.plugin:build_spawn_env"
    )
...
def test_community_harness_rejects_non_community_import_path(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="omnicraft-foo",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "omnicraft_foo.inner.foo_harness"},
        )
...
def test_community_harness_rejects_builtin_collision(monkeypatch: pytest.MonkeyPatch) -> None:
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"claude-sdk": "omnicraft.community.harness.evil.inner.evil_harness"},
        )
    ...
    assert hp.harness_modules()["claude-sdk"] == "omnicraft.inner.claude_sdk_harness"
...
def test_community_harness_rejects_alias_collision_with_builtin(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="omnicraft-evil",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "omnicraft.community.harness.evil.inner.foo_harness"},
...
def test_community_harness_rejects_community_collision(
    ...
    def _second() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            name="omnicraft-bar",
            valid_harnesses=frozenset({"foo"}),
            harness_modules={"foo": "omnicraft.community.harness.bar.inner.foo_harness"},
        )
    ...
    assert hp.harness_modules()["foo"] == "omnicraft.community.harness.foo.inner.foo_harness"
...
def test_community_harness_rejects_native_terminal_metadata(
    ...
    def _contribution() -> hp.HarnessContribution:
        return hp.HarnessContribution(
            ...
            harness_modules={"foo-native": "omnicraft.community.harness.foo.inner.foo_harness"},
...
def test_community_harness_readiness_uses_install_metadata(
    ...
    from omnicraft.onboarding import harness_readiness as readiness
...
def test_community_namespace_imports_external_harness_package(
    ...
    package_dir = package_root / "omnicraft" / "community" / "harness" / "foo"
    package_dir.mkdir(parents=True)
    (package_dir / "__init__.py").write_text("VALUE = 'ok'\n", encoding="utf-8")

    monkeypatch.syspath_prepend(str(package_root))

    import omnicraft.community as community
    import omnicraft.community.harness as harnesses

    importlib.reload(community)
    importlib.reload(harnesses)
    sys.modules.pop("omnicraft.community.harness.foo", None)

    module = importlib.import_module("omnicraft.community.harness.foo")
