...
from omnicraft.kimi_native_credentials import (
...
def test_render_hooks_toml_is_valid_and_complete() -> None:
    ...
    for hook in parsed["hooks"]:
        assert "omnicraft.kimi_native_hook" in hook["command"]
        ...
        # ``omnicraft/`` shadows the install and the hook dies on ImportError
        # before publishing the approval card.
        assert " -I -m omnicraft.kimi_native_hook" in hook["command"]
...
def test_build_session_home_preserves_user_config_and_appends_hooks(
    ...
    # … and the OmniCraft hooks appended.
