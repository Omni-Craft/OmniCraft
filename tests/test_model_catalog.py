"""Unit tests for ``omnicraft/model_catalog.py``.
...
import omnicraft.model_catalog as model_catalog
from omnicraft.model_catalog import (
    catalog_for_spec,
    list_models_for_worker,
    resolve_model_provider,
    spec_harness,
)
from omnicraft.runtime.credentials.databricks import WorkspaceCreds
from omnicraft.spec.types import AgentSpec, ApiKeyAuth, DatabricksAuth, ExecutorSpec
...
def _worker_spec(harness: str, **executor_kwargs: object) -> AgentSpec:
    ...
    return AgentSpec(
        spec_version=1,
        name="worker",
        executor=ExecutorSpec(type="omnicraft", config={"harness": harness}, **executor_kwargs),  # type: ignore[arg-type]
    )
...
def test_resolve_provider_legacy_profile(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(
            type="omnicraft", config={"harness": "codex-native", "profile": "legacy-prof"}
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            SimpleNamespace(executor=SimpleNamespace(type="omnicraft", config={})),
            "claude-native",
            id="structural-stub",
        ),
    ],
...
def test_failed_auth_command_note_never_leaks_the_command(
    ...
    parent = AgentSpec(
        spec_version=1,
        name="orchestrator",
        executor=ExecutorSpec(type="omnicraft", config={"harness": "pi"}),
