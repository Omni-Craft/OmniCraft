"""Tests for the OpenCode SSE -> OmniCraft event forwarder translation."""
...
import omnicraft.opencode_native_forwarder as fwd_mod
from omnicraft.opencode_native_client import OpenCodeEvent

_SESSION = "ses_1"


class _RecordingServerClient:
    """httpx-shaped stub recording OmniCraft event POSTs."""
...
async def test_user_text_part_is_mirrored_before_the_assistant() -> None:
    ...
    For native-server harnesses omnicraft persists no separate user item, so the
...
async def test_permission_asked_rejects_when_no_policy_wired() -> None:
    ...
    assert opencode.replies == [("per_1", {"reply": "reject", "message": "omnicraft-policy"})]
...
async def test_permission_asked_passes_normalized_input_to_evaluator() -> None:
    ...
    assert seen[0]["omnicraft_session_id"] == "conv_1"
...
async def test_model_switched_mirrors_to_omnicraft_and_dedupes() -> None:
