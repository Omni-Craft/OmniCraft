...
_TEST_ENV = {"OMNICRAFT_TEST_MODE": "1"}
...
def _guardrail_warnings(records: list[logging.LogRecord]) -> list[str]:
    ...
    return [
        r.getMessage()
        for r in records
        if r.name == "omnicraft.testing.guardrails" and "TEST GUARDRAIL:" in r.getMessage()
    ]
...
@pytest.mark.parametrize(
    ...
    [
        "",
        "sqlite:////home/alice/.omnicraft/chat.db",
        "sqlite:///testing.db",
        "sqlite:///test123.db",
        "sqlite:///contest.db",
        "sqlite:///latest.db",
        "postgresql://prod-host:5432/omnicraft",
...
def test_looks_like_pytest_via_flag() -> None:
    assert looks_like_pytest({"OMNICRAFT_TEST_MODE": "1"}) is True
...
def test_real_db_warns(caplog: pytest.LogCaptureFixture) -> None:
    with caplog.at_level(logging.WARNING):
        check_test_environment(
            env=_TEST_ENV,
            db_uri="sqlite:////home/alice/.omnicraft/chat.db",
            warn_only=True,
        )
    warnings = _guardrail_warnings(caplog.records)
...
def test_escape_hatch_downgrades_hard_fail_to_warning(
    ...
    env = {**_TEST_ENV, "OMNICRAFT_DISABLE_TEST_GUARDRAILS": "yes"}
    with caplog.at_level(logging.WARNING):
        violations = check_test_environment(
            env=env,
            db_uri="sqlite:////home/alice/.omnicraft/chat.db",
