...
def test_list_files_returns_metadata(
    ...
    monkeypatch.setattr(
        "omnicraft.runtime.get_file_store",
...
def test_download_file_saves_to_workspace(
    ...
    monkeypatch.setattr(
        "omnicraft.runtime.get_artifact_store",
...
def test_parse_json_object_arguments_treats_blank_as_empty(arguments: str) -> None:
    """
    Empty or whitespace-only arguments parse to an empty object with
    no error, so required-argument tools fall through to their own
    missing-field checks.

    :param arguments: Blank raw argument string.
    """
    assert parse_json_object_arguments(arguments) == ({}, None)


def test_parse_json_object_arguments_rejects_malformed() -> None:
    """Malformed JSON returns an error string, not a raised exception."""
    parsed, error = parse_json_object_arguments("not-json")
    assert parsed is None
    assert error == "malformed JSON arguments"


def test_parse_json_object_arguments_rejects_non_object() -> None:
    """A valid JSON non-object (e.g. a list) is rejected."""
    parsed, error = parse_json_object_arguments("[]")
    assert parsed is None
