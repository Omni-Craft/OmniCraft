...
import omnicraft.tools.builtins as _builtins_pkg
...
def _all_builtin_tool_subclasses() -> list[type[Tool]]:
    """Concrete ``Tool`` subclasses defined under ``omnicraft.tools.builtins``."""
...
def test_async_builtins_override_dispatch_async_or_are_runner_dispatched() -> None:
    ...
    would crash the in-process OmniCraft loop. After the DBOS removal,
    a class of async-namespace tools (``sys_call_async``,
    ``sys_read_inbox``, ``sys_cancel_async``) are dispatched by
    the runner via ``omnicraft/runner/tool_dispatch.py`` —
