"""Tests for omnicraft.tools.local (LocalPythonTool subprocess execution)."""
...
from omnicraft.tools.local import (
    LocalPythonTool,
    LocalToolLoadError,
...
def _run_runner_with_request(tool_path: Path, tool_name: str, arguments: dict) -> dict:
    ...
    """
    runner = Path(__file__).parent.parent.parent / "omnicraft" / "tools" / "_runner.py"
    request = json.dumps(
        {
