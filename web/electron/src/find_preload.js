...
contextBridge.exposeInMainWorld("omnicraftFind", {
  /**
   ...
   */
  query: (text, opts) => {
    ipcRenderer.send("omnicraft:find-query", {
      text: String(text ?? ""),
  ...
  close: () => {
    ipcRenderer.send("omnicraft:find-close");
  },
  /**
   ...
   */
  onResult: (callback) => {
    ipcRenderer.on("omnicraft:find-result", (_event, result) => callback(result));
  },
  /** Fires when Cmd/Ctrl+F re-activates an already-open bar. */
  onActivate: (callback) => {
    ipcRenderer.on("omnicraft:find-activate", () => callback());
  },
});
