// Discovery and invocation of the local `omnicraft` CLI for the desktop shell.
//
// The desktop manages servers by shelling out to the same `omnicraft` binary a
...
// test/omnicraft_cli.test.js; the functions that actually spawn a binary are
...
/**
 * The OmniCraft local runtime data dir — `$OMNICRAFT_DATA_DIR` (with `~`
 * expanded) or `~/.omnicraft`. Mirrors `_local_data_dir()` in
 * omnicraft/host/local_server.py. The local-server pidfile lives here.
 *
 * @returns {string}
 */
function localDataDir() {
  const raw = process.env.OMNICRAFT_DATA_DIR;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
  return path.join(os.homedir(), ".omnicraft");
}

/**
 * The OmniCraft config dir — `$OMNICRAFT_CONFIG_HOME` (with `~` expanded) or
 * `~/.omnicraft`. config.yaml (machine identity) lives here; it can differ from
 * the data dir under test env overrides, but is the same by default.
 *
 * @returns {string}
 */
function localConfigDir() {
  const raw = process.env.OMNICRAFT_CONFIG_HOME;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
...
/**
 * The shared OmniCraft state dir, ALWAYS `~/.omnicraft` — it ignores
 * `$OMNICRAFT_DATA_DIR`, mirroring `state_dir()` in
 * sdks/ui/omnicraft_ui_sdk/terminal/_config.py and `_HOST_PID_PATH` in
 * omnicraft/cli.py (both hardcode `Path.home()/".omnicraft"`). The auth-token
 * store and the daemon registry live here — NOT under the data dir. Only the
 * local-server pidfile honors `$OMNICRAFT_DATA_DIR` (see {@link localDataDir}).
 *
 * @returns {string}
 */
function stateDir() {
...
/**
 * This machine's OmniCraft host id (e.g. "host_ab12…"), read from the machine
 * identity in `config.yaml` (`host: host_id:`, written by
 * omnicraft/host/identity.py) — instant, no subprocess. Present once generated,
...
/**
 * Local-server status from the pidfile + a pid-liveness check — no `omnicraft
 ...
 * omnicraft/host/local_server.py. Returns null for a stale pidfile (dead pid, a
 ...
 * point (`omnicraft.cli:main`); `omni` is the short alias. We probe `omnicraft`
 ...
 */
const CLI_NAMES = ["omnicraft", "omni"];

/**
 * Well-known install locations for the CLI binary, in priority order. For each
 * directory we list the `omnicraft` name then the `omni` alias.
 ...
 *
 * @param {string} name e.g. "omnicraft" or "omni"
 * @returns {string | null}
 */
...
/**
 * Resolve the CLI on PATH, trying `omnicraft` then the `omni` alias. Returns the
 * first hit, or null when neither is on PATH.
 *
 * @returns {string | null}
 */
function whichOmniCraft() {
...
/**
 * Locate the `omnicraft` binary. Resolution order: a user-configured path, then
 ...
 *
 * @param {string | null | undefined} configuredPath settings.omnicraft_path
 * @param {{
 *   isExecutableFile?: (p: string) => boolean,
 *   whichOmniCraft?: () => string | null,
...
function resolveCliPath(configuredPath, deps = {}) {
  ...
  const which = deps.whichOmniCraft || whichOmniCraft;
...
/**
 * Run an `omnicraft` subcommand and resolve with its captured output. Never
 ...
 * from `~/.omnicraft/auth_tokens.json` (no subprocess), mirroring
 * omnicraft/cli_auth.py: keyed by the trailing-slash-stripped URL, a record is
 * valid if it's a Databricks pointer (has `workspace_host`) or a non-expired
 * session token. The CLI's `state_dir()` is hardcoded to `~/.omnicraft`.
 *
 * @param {string} serverUrl
 * @returns {boolean}
 */
...
/**
 * Run `omnicraft login <serverUrl>` to authenticate the CLI to a server. It's a
...
async function getCliStatus(configuredPath) {
  ...
  // Must exit cleanly AND identify itself as omni — `omnicraft --version` prints
  // e.g. "omnicraft 0.3.0.dev0 (…)". The exit-code alone isn't enough: an
...
/**
 * `omnicraft server status --json`. Returns the parsed payload, or a synthetic
 ...
 * `_daemon_registry_dir()` in omnicraft/cli.py (`<state_dir>/daemons`).
 *
 * @returns {string}
 */
 ...
 * Mirrors the validation in `_record_from_json()` (omnicraft/cli.py): a usable
...
/**
 * Read every daemon registry record from disk (`~/.omnicraft/daemons/*.json`).
 * This is the fast substitute for `omnicraft host status --json`: it gives the
...
/**
 * The OmniCraft server URL a daemon record talks to, mirroring
 * `_daemon_base_url()` (omnicraft/cli.py): a local-mode daemon's URL lives in
 ...
 * non-expired session token stored by `omnicraft login` in `auth_tokens.json`,
 ...
 * for this URL.
 *
 * Unlike `_remote_headers()` (omnicraft/chat.py), this deliberately does NOT
 * honor `OMNICRAFT_REMOTE_AUTH_TOKEN`. That token is destination-independent — it
 ...
 * `~/.omnicraft/daemons/`. The desktop never needs it: it's not inherited by a
 ...
 * `_add_daemon_host_status()` (omnicraft/cli.py) makes, minus the per-session
...
/**
 * This machine's connection to `serverUrl`, resolved WITHOUT the slow `omnicraft
