...
// page. Two consumers:
//
//   1. window.omnicraftDesktop — read by the web app's nativeBridge.ts
//      (badge + notifications). Its `kind: "electron"` field is the
//      feature-detection discriminator.
//   2. window.omnicraftSetup — used only by the bundled setup page to
...
// boundary.
contextBridge.exposeInMainWorld("omnicraftDesktop", {
  kind: "electron",
  /** Paint the dock/taskbar badge; 0 clears it. Fire-and-forget. */
  setBadgeCount: (count) => {
    ipcRenderer.send("omnicraft:set-badge-count", count);
  },
  /**
   ...
   */
  notify: (params) =>
    ipcRenderer.invoke("omnicraft:notify", {
      title: params?.title,
      body: params?.body,
  ...
  onNotificationActivated: (callback) => {
    ...
    ipcRenderer.on("omnicraft:notification-activated", listener);
    return () => ipcRenderer.removeListener("omnicraft:notification-activated", listener);
  },
  /**
   ...
   */
  getServerPicker: () => ipcRenderer.invoke("omnicraft:get-server-picker"),
  /**
   ...
   */
  switchServer: (url) => ipcRenderer.invoke("omnicraft:switch-server", url),
  /** Return this window to the bundled "connect to server" setup page. */
  openServerSetup: () => {
    ipcRenderer.send("omnicraft:open-server-setup");
  },
  /**
   ...
   */
  getHostIdentity: () => ipcRenderer.invoke("omnicraft:host-get-identity"),
  /**
   * Start / stop / restart this machine's host daemon for the window's server.
   * Resolves a `{ ok, error? }` result.
   * @param {"start" | "stop" | "restart"} action
   */
  controlHost: (action) => ipcRenderer.invoke("omnicraft:host-control", action),
  /**
  ...
  onHostStatusChanged: (callback) => {
    ...
    ipcRenderer.on("omnicraft:host-status-changed", listener);
    return () => ipcRenderer.removeListener("omnicraft:host-status-changed", listener);
  },
  /**
   ...
   * binary is in use.
   */
  getCliStatus: () => ipcRenderer.invoke("omnicraft:cli-get-status"),
  /**
  ...
  getWidgetsStatus: () => ipcRenderer.invoke("omnicraft:widgets-status"),
  ...
  browserOpenOrNavigate: (conversationId, url, bounds, opts) =>
    ipcRenderer.invoke("omnicraft:browser-open-or-navigate", { conversationId, url, bounds, opts }),
  /**
  ...
  browserSetActive: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-set-active", { conversationId }),
  /**
  ...
  browserResize: (conversationId, bounds) =>
    ipcRenderer.invoke("omnicraft:browser-resize", { conversationId, bounds }),
  /**
  ...
  browserScreenshot: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-screenshot", { conversationId }),
  /**
  ...
  browserExecute: (conversationId, js) =>
    ipcRenderer.invoke("omnicraft:browser-execute", { conversationId, js }),
  /**
  ...
  browserClose: (conversationId, reason) =>
    ipcRenderer.invoke("omnicraft:browser-close", { conversationId, reason }),
  /**
  ...
  browserHasView: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-has-view", { conversationId }),
  /**
  ...
  browserGoBack: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-go-back", { conversationId }),
  /**
  ...
  browserGoForward: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-go-forward", { conversationId }),
  /**
  ...
  browserReload: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-reload", { conversationId }),
  /**
  ...
  openBrowserDevTools: (conversationId) =>
    ipcRenderer.invoke("omnicraft:open-browser-devtools", { conversationId }),
  /**
  ...
  browserEnableDesignMode: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-enable-design-mode", { conversationId }),
  /**
  ...
  browserDisableDesignMode: (conversationId) =>
    ipcRenderer.invoke("omnicraft:browser-disable-design-mode", { conversationId }),
  /**
  ...
  browserSignalDesignResult: (conversationId, result) =>
    ipcRenderer.invoke("omnicraft:browser-signal-design-result", {
      conversationId,
      id: result?.id,
      ok: result?.ok,
...
contextBridge.exposeInMainWorld("omnicraftSetup", {
  getServerUrl: () => ipcRenderer.invoke("omnicraft:get-server-url"),
  /**
   ...
   * @param {string} url
   */
  setServerUrl: (url) => ipcRenderer.invoke("omnicraft:set-server-url", url),
  /** Recently-connected server URLs, most recent first. */
  getRecentServers: () => ipcRenderer.invoke("omnicraft:get-recent-servers"),
  /**
   * Whether the `omnicraft` CLI is installed/runnable, e.g.
   * `{installed, path, version, source, installCommand}`.
   */
  getCliStatus: () => ipcRenderer.invoke("omnicraft:get-cli-status"),
  /**
   * Set an explicit path to the omnicraft binary. Resolves the CLI status plus
   * `accepted` (whether that exact path validated and was saved).
   * @param {string} path
   */
  setCliPath: (path) => ipcRenderer.invoke("omnicraft:set-cli-path", path),
  /** Native file picker for the omnicraft binary; resolves the path or null. */
  browseCliPath: () => ipcRenderer.invoke("omnicraft:browse-cli-path"),
  /**
   * Start (or reuse) the local server. Resolves `{ok, url?, error?}`; the
   * caller then connects to `url` via setServerUrl.
   */
  startLocalServer: () => ipcRenderer.invoke("omnicraft:start-local-server"),
});
