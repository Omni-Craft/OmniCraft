// Tests for the pure helpers in src/omnicraft_cli.js, run with `node --test`
...
// connected to server X?" and "which omnicraft binary do we run?".
...
describe("normalizeServerUrl", () => {
  it("strips trailing slashes and trims", () => {
    ...
    assert.equal(normalizeServerUrl("https://x.com/ml/omnicrafts"), "https://x.com/ml/omnicrafts");
  });
...
describe("candidatePaths", () => {
  it("probes both the omnicraft name and the omni alias in each location", () => {
    const paths = candidatePaths();
    // Every well-known dir contributes an `omnicraft` and an `omni` entry.
    assert.ok(paths.some((p) => p.endsWith("/.local/bin/omnicraft")));
    assert.ok(paths.some((p) => p.endsWith("/.local/bin/omni")));
    assert.ok(paths.includes("/opt/homebrew/bin/omnicraft"));
    assert.ok(paths.includes("/opt/homebrew/bin/omni"));
    assert.ok(paths.includes("/usr/local/bin/omni"));
  });

  it("lists the canonical omnicraft name before the omni alias within a dir", () => {
    const paths = candidatePaths();
    const og = paths.indexOf("/opt/homebrew/bin/omnicraft");
...
describe("resolveCliPath", () => {
  it("resolves the omni alias when only it is executable", () => {
    const got = resolveCliPath(null, {
      ...
      whichOmniCraft: () => null,
      candidatePaths: () => ["/home/me/.local/bin/omnicraft", "/home/me/.local/bin/omni"],
    });
    assert.deepEqual(got, { path: "/home/me/.local/bin/omni", source: "candidate" });
  });

  it("prefers a usable configured path", () => {
    const got = resolveCliPath("/custom/omnicraft", {
      isExecutableFile: (p) => p === "/custom/omnicraft",
      whichOmniCraft: () => "/usr/bin/omnicraft",
      candidatePaths: () => ["/home/me/.local/bin/omnicraft"],
    });
    assert.deepEqual(got, { path: "/custom/omnicraft", source: "configured" });
  });

  it("falls back to PATH when the configured path is unusable", () => {
    const got = resolveCliPath("/bad/path", {
      isExecutableFile: (p) => p === "/usr/bin/omnicraft",
      whichOmniCraft: () => "/usr/bin/omnicraft",
      candidatePaths: () => ["/home/me/.local/bin/omnicraft"],
    });
    assert.deepEqual(got, { path: "/usr/bin/omnicraft", source: "path" });
  });

  it("falls back to a candidate when PATH misses (GUI minimal PATH)", () => {
    const got = resolveCliPath(null, {
      isExecutableFile: (p) => p === "/home/me/.local/bin/omnicraft",
      whichOmniCraft: () => null,
      candidatePaths: () => ["/home/me/.local/bin/omnicraft", "/opt/homebrew/bin/omnicraft"],
    });
    assert.deepEqual(got, { path: "/home/me/.local/bin/omnicraft", source: "candidate" });
  });
...
describe("getHostConnectionFast — probe destination & token handling (S1)", () => {
  afterEach(() => {
    mock.restoreAll();
    delete process.env.OMNICRAFT_REMOTE_AUTH_TOKEN;
  });
  ...
  it("never attaches OMNICRAFT_REMOTE_AUTH_TOKEN to a probe", async () => {
    ...
    process.env.OMNICRAFT_REMOTE_AUTH_TOKEN = "secret-token";
    const record = {
      pid: process.pid,
      target: "http://localhost:6767",
      mode: "server",
