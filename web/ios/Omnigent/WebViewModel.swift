...
final class WebViewModel: ObservableObject {
  ...
  func emitNotificationActivation(_ path: String) {
    ...
    let script =
      "window.__omnicraftNativeEmitNotificationActivated?.(\(Self.javascriptString(path)));"
  ...
  /// web side folds these into its `--omnicraft-inset-*` variables so page
  ...
  func emitInsets(topBar: CGFloat, bottomBar: CGFloat) {
    let script =
      "window.__omnicraftNativeEmitInsets?.(\(jsNumber(topBar)), \(jsNumber(bottomBar)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitViewModeChanged(_ mode: WebViewMode) {
    let script =
      "window.__omnicraftNativeEmitViewModeChanged?.(\(Self.javascriptString(mode.rawValue)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitSidebarDrag(phase: String, progress: Double) {
    ...
    let script =
      "window.__omnicraftNativeEmitSidebarDrag?.(\(Self.javascriptString(phase)), \(clamped));"
    webView?.evaluateJavaScript(script)
  }
