...
3. **Create the app record** in [App Store Connect](https://appstoreconnect.apple.com)
   for bundle ID `ai.omnicraft.ios` (My Apps → +), if it doesn't exist yet.
...
- **Marketing version** (`CFBundleShortVersionString`, currently `0.1.0`) is set
  ...
  configurations of the **OmniCraft** target in Xcode (or via `fastlane
...
`OMNICRAFT_PROD_BUILD_NUMBER=4` to pin a specific processed build.

## Other commands

- `bundle exec fastlane tests` — run the `OmniCraftTests` unit suite.
- `bundle exec fastlane screenshots` — build and capture App Store screenshots
  into `fastlane/screenshots` using the `OmniCraftUITests` snapshot target. The
  lane rebuilds the web UI, starts an isolated local OmniCraft server via `uv` on
