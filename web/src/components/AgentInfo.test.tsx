...
describe("AgentInfoButton session owner row", () => {
  ...
  it("shows the session owner when someone else owns the shared session", () => {
    ...
    expect(row).not.toHaveTextContent("(você)");
  });
...
describe("SessionPoliciesSection", () => {
  ...
  it("shows the empty state when no user policies are applied", () => {
    ...
    expect(screen.getByText("Nenhuma política adicionada")).toBeInTheDocument();
  });
  ...
  it("filters the registry list and adds a callable policy", () => {
    ...
    // Filter to just Beta.
    fireEvent.change(within(dialog).getByPlaceholderText("Filtrar políticas..."), {
      target: { value: "beta" },
    });
  ...
  it("reopening the dialog after closing mid-config starts back at the list", () => {
    ...
    fireEvent.click(screen.getByTitle("Adicionar política"));
    expect(
      within(screen.getByRole("dialog")).getByPlaceholderText("Filtrar políticas..."),
    ).toBeInTheDocument();
  });
...
describe("McpServersSection", () => {
  ...
  it("creates an HTTP MCP server from the manager dialog", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Gerenciar servidores MCP" }));
    const dialog = screen.getByRole("dialog");
    fireEvent.change(within(dialog).getByLabelText("Nome"), { target: { value: "github" } });
  ...
  it("updates an existing stdio MCP server", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Gerenciar servidores MCP" }));
    const dialog = screen.getByRole("dialog");
    fireEvent.click(within(dialog).getByRole("button", { name: "Editar jira" }));
    fireEvent.change(within(dialog).getByLabelText("Comando"), {
  ...
  it("deletes an MCP server from the manager dialog", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Gerenciar servidores MCP" }));
    const dialog = screen.getByRole("dialog");
    fireEvent.click(within(dialog).getByRole("button", { name: "Excluir slack" }));
