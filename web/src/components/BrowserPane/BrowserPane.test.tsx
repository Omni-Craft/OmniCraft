...
describe("BrowserPane cold-start (no view yet)", () => {
  it("renders the URL bar in the empty state so the first page is reachable", async () => {
    ...
    const urlBar = await screen.findByRole("textbox", { name: /barra de endereço/i });
    ...
    expect(screen.getByText(/digite uma url acima para começar/i)).toBeInTheDocument();
  });
  ...
  it("disables reload and devtools while no view is attached", async () => {
    ...
    expect(screen.getByRole("button", { name: /recarregar/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: /alternar devtools/i })).toBeDisabled();
  });
  ...
  it("shows the measuring container (not the hint) once a view is created", async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByText(/digite uma url acima para começar/i)).toBeNull();
    });
    expect(screen.getByRole("textbox", { name: /barra de endereço/i })).toBeInTheDocument();
  });
});
...
describe("BrowserPane design-mode toggle", () => {
  it("renders the design-mode toggle in the toolbar", async () => {
    ...
    await screen.findByRole("textbox", { name: /barra de endereço/i });
    expect(screen.getByRole("button", { name: /entrar no modo de design/i })).toBeInTheDocument();
  });
  ...
  it("disables the design-mode toggle while no view is attached", async () => {
    ...
    // as reload / devtools.
    expect(screen.getByRole("button", { name: /entrar no modo de design/i })).toBeDisabled();
  });
  ...
  it("calls enable then disable IPC as it toggles, once a view is active", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole("button", { name: /entrar no modo de design/i })).not.toBeDisabled();
    });

    // First click enables design mode (button flips to aria-pressed + "exit").
    screen.getByRole("button", { name: /entrar no modo de design/i }).click();
    await waitFor(() => {
      expect(bridge.browserEnableDesignMode).toHaveBeenCalledWith("conv_dm3");
    });
    const pressed = screen.getByRole("button", { name: /sair do modo de design/i });
    ...
    expect(screen.getByRole("button", { name: /entrar no modo de design/i })).toHaveAttribute(
      "aria-pressed",
      "false",
    );
  });
...
describe("BrowserPane toolbar navigation + URL bar", () => {
  ...
  it("devtools button calls the open-devtools IPC", async () => {
    ...
    screen.getByRole("button", { name: /alternar devtools/i }).click();
  ...
  it("the URL bar reflects the real url pushed by browser-url-changed", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByRole("textbox", { name: /barra de endereço/i })).toHaveValue(
        "https://myhost/landed",
      ),
    );
  });
