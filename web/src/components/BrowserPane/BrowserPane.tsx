    ...
    >
      <div className="flex shrink-0 items-center gap-1 border-border border-b bg-card px-2 py-1.5">
        ...
        <button
          ...
          aria-label={designMode ? "Sair do modo de design" : "Entrar no modo de design"}
          title={
            designMode
              ? "Clique em um elemento na página e descreva o que deseja alterar"
              : "Modo de design: aponte para um elemento para solicitar algo sobre ele"
          }
          className={cn(
      ...
      {viewActive ? (
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center bg-card px-6 py-8 text-center text-muted-foreground text-sm">
          Digite uma URL acima para começar — o agente também abrirá páginas aqui.
        </div>
      )}
    </div>
  );
