...
describe("HostBadge", () => {
  it("renders the host name with an online status when reachable", () => {
    ...
    // (read together as "mac-laptop, on-line"), with `title` for hover.
    expect(badge.textContent).toBe("mac-laptop, on-line");
    expect(badge.getAttribute("title")).toBe("Host mac-laptop, on-line");
  });
  ...
  it("falls back to the host record's status when liveness is unobserved", () => {
    ...
    render(<HostBadge sessionId="conv_1" />);
    expect(screen.getByTestId("host-badge").textContent).toBe("mac-laptop, off-line");
  });
  ...
  it("shows unknown (not a stale green) when the stream reports not-host-bound", () => {
    ...
    render(<HostBadge sessionId="conv_1" />);
    expect(screen.getByTestId("host-badge").textContent).toBe("mac-laptop, status desconhecido");
  });
  ...
  it("shows unknown while the host list is still loading", () => {
    ...
    useSessionHostOnlineMock.mockReturnValue(undefined);
    render(<HostBadge sessionId="conv_1" />);
    expect(screen.getByTestId("host-badge").textContent).toBe("host_x9, status desconhecido");
  });
});
