...
describe("ZoomableImage + ImageLightboxProvider", () => {
  it("renders a button wrapping an image (keeps the img role/name)", () => {
    renderWithProvider();
    expect(screen.getByRole("button", { name: "Ampliar imagem: diagram" })).toBeInTheDocument();
  ...
  it("opens a dialog showing the full image on click", () => {
    renderWithProvider();
    fireEvent.click(screen.getByRole("button", { name: "Ampliar imagem: diagram" }));
  ...
  it("zooms in and out via the toolbar, updating the scale and label", () => {
    ...
    expect(screen.getByRole("button", { name: "Redefinir zoom" })).toHaveTextContent("100%");
    ...
    expect(screen.getByRole("button", { name: "Redefinir zoom" })).toHaveTextContent("150%");
    expect(screen.getByRole("button", { name: "Diminuir zoom" })).toBeEnabled();
