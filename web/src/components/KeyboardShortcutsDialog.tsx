...
export const KEYBOARD_SHORTCUTS_EVENT = "omnicraft:open-keyboard-shortcuts";
...
const SHORTCUT_GROUPS: ShortcutGroup[] = [
  {
    title: "Geral",
    items: [
      { label: "Abrir paleta de comandos", keys: [MOD_KEY, "K"] },
      { label: "Mostrar atalhos de teclado", keys: [MOD_KEY, "/"] },
    ],
  },
  {
    title: "Nas conversas",
    items: [
      ...
      { label: "Nova linha na mensagem", keys: [SHIFT, ENTER] },
      { label: "Recuperar prompt anterior", keys: [UP] },
      { label: "Recuperar próximo prompt", keys: [DOWN] },
      { label: "Aceitar prompt de aprovação", keys: [MOD_KEY, ENTER] },
      { label: "Parar resposta", keys: ["Esc"] },
    ],
  },
  {
    title: "Navegação",
    items: [
      { label: "Sessão anterior", keys: [MOD_KEY, UP] },
      { label: "Próxima sessão", keys: [MOD_KEY, DOWN] },
    ],
  },
  {
    title: "Visualização",
    items: [
      { label: "Alternar barra lateral de conversas", keys: [MOD_KEY, ALT, "["] },
      { label: "Alternar barra lateral do workspace", keys: [MOD_KEY, ALT, "]"] },
    ],
  },
  {
    title: "Comandos de barra",
    note: "enquanto o menu de sugestões está aberto",
    items: [
      { label: "Navegar sugestões", keys: [UP, DOWN] },
      { label: "Aplicar comando destacado", keys: ["Tab"] },
      { label: "Fechar menu", keys: ["Esc"] },
    ],
  },
];
...
function pinnedSessionShortcut(native: boolean): Shortcut {
  return {
    label: "Ir para sessão fixada (1–10)",
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="sr-only">
            Os atalhos de teclado disponíveis na conversa.
          </DialogDescription>
        </DialogHeader>
