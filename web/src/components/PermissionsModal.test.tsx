...
vi.mock("@/lib/host", async (importOriginal) => {
  ...
  return {
    ...actual,
    getOmniCraftUserSearch: vi.fn(() => undefined),
    getOmniCraftTransformShareLink: vi.fn(() => undefined),
  };
});
...
const userSearchMock = vi.mocked(host.getOmniCraftUserSearch);
const transformLinkMock = vi.mocked(host.getOmniCraftTransformShareLink);
...
describe("PermissionsModal", () => {
  ...
  it("calls grantPermission with the form values on submit", async () => {
    ...
    const grantBtn = screen.getByRole("button", { name: /conceder/i });
  ...
  it("calls revokePermission when the revoke button is clicked", async () => {
    ...
    const revokeBtn = screen.getByRole("button", { name: /remover/i });
  ...
  it("updates a grant's level inline via grantPermission (no revoke + re-add)", async () => {
    ...
    const rowTrigger = screen.getAllByRole("combobox").find((el) => el.textContent === "Leitura")!;
    rowTrigger.focus();
    fireEvent.keyDown(rowTrigger, { key: "Enter" });
    fireEvent.click(await screen.findByRole("option", { name: "Edição" }));
  ...
  it("renders the owner as non-editable with no revoke control", async () => {
    ...
    const revokeButtons = screen.queryAllByRole("button", { name: /remover/i });
  ...
  it("never offers Manage in any level dropdown", async () => {
    ...
    for (const trigger of triggers) {
      ...
      expect(options.map((o) => o.textContent)).toEqual(["Leitura", "Edição"]);
