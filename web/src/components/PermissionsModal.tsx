...
import { getOmniCraftTransformShareLink, getOmniCraftUserSearch } from "@/lib/host";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">Compartilhar esta sessão</DialogTitle>
          <DialogDescription>
            Convide outras pessoas para ver ou colaborar nesta sessão.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex items-center justify-between rounded-lg border px-3 py-2">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Qualquer pessoa pode ver esta sessão</p>
          </div>
          <Switch
            checked={isPublic}
        ...
        <div className="min-w-0" data-testid="share-grants">
          {isLoading ? (
            ...
            <p className="text-sm text-muted-foreground py-2">Nenhuma permissão concedida ainda.</p>
          ) : (
            <>
              {/* Column headers */}
        ...
        <form onSubmit={handleGrant} className="flex items-end gap-2">
          ...
          <div>
            ...
            <Select value={newLevel} onValueChange={setNewLevel}>
              ...
              <SelectContent>
                <SelectItem value="1">Leitura</SelectItem>
                <SelectItem value="2">Edição</SelectItem>
              </SelectContent>
            </Select>
          </div>
...
function AddUserField({ value, onChange }: AddUserFieldProps) {
  ...
  const searchUsers = getOmniCraftUserSearch();
  if (!searchUsers) {
    return (
      <Input
        id="perm-user"
...
function getShareableLink(sessionId: string, rebasePath: (path: string) => string): string {
  ...
  const transform = getOmniCraftTransformShareLink();
    ...
    <div className="flex items-center gap-2 rounded-md px-2 py-0.5 hover:bg-muted/50">
      ...
      {isOwner || isManage ? (
        <span className="flex h-8 w-28 items-center px-3 text-sm text-muted-foreground">
          {isOwner ? "Proprietário" : "Gerenciar"}
        </span>
      ) : (
        <Select
        ...
        >
          <SelectTrigger
            className="h-8 w-28"
            aria-label={`Nível de permissão de ${permission.user_id}`}
          >
            <SelectValue />
          </SelectTrigger>
          <SelectContent>
