...
export const BUILTIN_SLASH_COMMANDS: Record<string, string> = {
  "/compact": "Compactar o contexto da conversa para liberar espaço",
  "/context": "Mostrar o uso da janela de contexto desta sessão",
  "/effort": "Definir o esforço de raciocínio: /effort low | medium | high | default",
  "/model": "Trocar o modelo desta sessão: /model <nome> | default",
    ...
    <div className="absolute bottom-full left-0 z-10 mb-2 flex items-end gap-2">
      <div className="w-64 shrink-0 overflow-hidden rounded-xl border border-border bg-popover shadow-lg">
        <div ref={listRef} className="max-h-80 overflow-y-auto p-1">
          {builtinRows.length > 0 && sectionHeader("Comandos")}
          {builtinRows.map((row) => (
            <MenuRowButton
              key={row.name}
              row={row}
