...
describe("ApprovalCard — binary approve/reject", () => {
  ...
  it("renders Codex command approvals from structured extras instead of raw JSON", () => {
    ...
    expect(screen.getByText("Aprovação de comando")).toBeDefined();
    expect(screen.getByText("Codex quer executar este comando.")).toBeDefined();
  ...
  it("submits Codex execpolicy amendments when the remember option is clicked", () => {
    ...
    const rememberButton = screen.getByRole("button", { name: /aprovar e lembrar/i });
...
describe("ApprovalCard — accept & allow all edits", () => {
  ...
  it("renders the third button only when allowAllEdits is set", () => {
    ...
    expect(
      screen.getByRole("button", { name: /aceitar e permitir todas as edições/i }),
    ).toBeDefined();
    ...
    expect(
      screen.queryByRole("button", { name: /aceitar e permitir todas as edições/i }),
    ).toBeNull();
  });
  ...
  it("submits {action: 'accept', content: {allow_all_edits: true}} on click", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /aceitar e permitir todas as edições/i }));
  ...
  it("renders the auto-accepting-edits label in the responded state", () => {
    ...
    expect(screen.getByText(/aceitando edições automaticamente/i)).toBeDefined();
  });
});
...
describe("ApprovalCard — approve & don't ask again (persistent allow rule)", () => {
  ...
  it("labels the remember button by the WebFetch host and hides it without the hint", () => {
    ...
    const rememberButton = screen.getByRole("button", {
      name: /não perguntar novamente para github\.com/i,
    });
    ...
    expect(rememberButton.getAttribute("title")).toBe(
      "Não perguntará novamente para github.com pelo restante desta sessão",
    );
    expect(screen.getByRole("button", { name: /^aprovar$/i })).toBeDefined();
    expect(screen.getByRole("button", { name: /rejeitar/i })).toBeDefined();
  ...
  it("labels the remember button by the tool name for a tool-wide scope", () => {
    ...
    const rememberButton = screen.getByRole("button", {
      name: /não perguntar novamente para Bash/i,
    });
    expect(rememberButton).toBeDefined();
    // Tool-wide grant is broader than a domain — the tooltip says "any".
    expect(rememberButton.getAttribute("title")).toBe(
      "Não perguntará novamente para nenhuma chamada de Bash pelo restante desta sessão",
    );
  });
  ...
  it("renders the won't-ask-again label in the responded state", () => {
    ...
    expect(screen.getByText(/não perguntará novamente para github\.com/i)).toBeDefined();
  });
});
...
describe("ApprovalCard — multi-choice options", () => {
  ...
  it("renders 'Selected: <label>' on the responded card when content carries an answer", () => {
    ...
    expect(screen.getByText(/Selecionado: Alpha/)).toBeDefined();
  });
});
...
describe("ApprovalCard — AskUserQuestion form (parsed from content_preview)", () => {
  ...
  it("labels Codex structured input prompts as Codex prompts", () => {
    ...
    expect(screen.getByText("Codex precisa de informações")).toBeDefined();
    expect(screen.queryByText("Claude tem perguntas")).toBeNull();
  });
  ...
  it("labels Antigravity structured input prompts as Antigravity prompts", () => {
    ...
    expect(screen.getByText("Antigravity precisa da sua contribuição")).toBeDefined();
    expect(screen.queryByText("Claude tem perguntas")).toBeNull();
  });
  ...
  it("labels Cursor structured input prompts as Cursor prompts", () => {
    ...
    expect(screen.getByText("Cursor tem perguntas")).toBeDefined();
    expect(screen.queryByText("Claude tem perguntas")).toBeNull();
  });
  ...
  it("renders one question at a time and navigates between them via Prev/Next", () => {
    ...
    expect(screen.getByTestId("ask-user-question-progress").textContent).toBe("Pergunta 1 de 2:");
    expect(screen.getByText("First?")).toBeDefined();
    expect(screen.queryByText("Second?")).toBeNull();

    // Next reveals the second question.
    fireEvent.click(screen.getByTestId("ask-user-question-next"));
    expect(screen.getByTestId("ask-user-question-progress").textContent).toBe("Pergunta 2 de 2:");
...
describe("ApprovalCard — ExitPlanMode plan review", () => {
  ...
  it("renders the plan as markdown with the three plan-review actions", async () => {
    ...
    expect(screen.getByText("Revisão do plano")).toBeDefined();
  ...
  it("submits {accept, allow_all_edits} for 'Yes, and use auto mode'", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /sim, e usar modo automático/i }));
  ...
  it("submits a plain accept for 'Yes, manually approve edits'", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /sim, aprovar edições manualmente/i }));
  ...
  it("reveals a feedback textarea and submits decline with the typed feedback", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /rejeitar com feedback/i }));
    const textarea = screen.getByPlaceholderText(/o que deveria mudar/i);
    fireEvent.change(textarea, { target: { value: "Use a feature flag instead." } });
    fireEvent.click(screen.getByRole("button", { name: /rejeitar plano/i }));
  ...
  it("submits a plain decline when the feedback is left empty", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /rejeitar com feedback/i }));
    fireEvent.change(screen.getByPlaceholderText(/o que deveria mudar/i), {
      target: { value: "   " },
    });
    fireEvent.click(screen.getByRole("button", { name: /rejeitar plano/i }));
  ...
  it("renders plan-specific responded labels and echoes rejection feedback", () => {
    ...
    expect(screen.getByText("Plano aprovado · modo automático")).toBeDefined();
    ...
    expect(screen.getByText("Plano aprovado")).toBeDefined();
    ...
    expect(screen.getByText("Plano rejeitado")).toBeDefined();
