...
const ROUTING_VERBS = ["avaliando", "combinando", "ajustando", "dimensionando"] as const;
...
export function SmartRoutingCard({ arguments: args, output, state }: SmartRoutingCardProps) {
  ...
  const taskNoun = tasks.length === 1 ? "tarefa" : "tarefas";
  return (
    <Collapsible
      defaultOpen={false}
      className={cn(
    ...
    >
      <div className="flex items-center gap-1.5 text-xs">
        ...
        {judging ? (
          <Shimmer as="span" className="text-xs">
            {`Avaliando ${tasks.length} ${taskNoun}…`}
          </Shimmer>
        ) : (
          <span className="text-muted-foreground">
            {failed ? "· indisponível" : `· ${recommendations!.size} ${taskNoun} dimensionadas`}
          </span>
        )}
        {prettyOutput !== null && (
          <CollapsibleTrigger
            className="ml-auto cursor-pointer rounded p-0.5 text-muted-foreground hover:text-foreground"
            aria-label="Mostrar resposta bruta de roteamento"
      ...
      {failed ? (
        <p className="text-xs text-muted-foreground" data-testid="smart-routing-error">
          {output ?? "Nenhuma decisão de roteamento foi registrada para este fan-out."}
        </p>
      ) : (
        tasks.map((task, i) => {
          ...
          <CodeBlock code={prettyOutput} language="json">
            <CodeBlockHeader>
              <CodeBlockTitle className="min-w-0">
                <span className="truncate font-medium uppercase tracking-wide">Resposta</span>
              </CodeBlockTitle>
            </CodeBlockHeader>
          </CodeBlock>
        </CollapsibleContent>
