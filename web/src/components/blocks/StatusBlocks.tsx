...
export function ErrorBanner({ message, source, code }: ErrorBannerProps) {
  const display = message || code || "Erro desconhecido";
  return (
    <Alert
      variant="destructive"
...
export function PolicyDeniedBanner({ reason, phase }: PolicyDeniedBannerProps) {
  return (
    <Alert>
      <ShieldXIcon />
      <AlertTitle>Bloqueado pela política{phase ? ` · ${phase}` : ""}</AlertTitle>
...
export function RetryIndicator({
  ...
  return (
    <div className="flex items-center gap-2 text-muted-foreground text-xs">
      ...
      <span>
        Repetindo {source} · tentativa {attempt}/{maxAttempts}
        {delaySeconds > 0 ? ` · aguardando ${delaySeconds.toFixed(1)}s` : ""}
      </span>
    </div>
  );
}
...
export function RoutingDecisionChip({ model, applied, rationale }: RoutingDecisionChipProps) {
  ...
  const lead = applied ? short : `teria escolhido ${short}`;
  const summary = `Roteador de modelo inteligente · ${lead}`;
  return (
    <div
    ...
    >
      <span className="flex items-center gap-1.5">
        ...
        <span>
          ...
          {!applied && <span>teria escolhido </span>}
...
export function RoutingDecisionCard({
  ...
  const short = shortModelName(model);
  const rowLabel = agent && agent.length > 0 ? agent : "Sessão";
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1.5 text-xs">
        ...
        <span className="font-medium">Roteamento inteligente</span>
        <span className="text-muted-foreground">{applied ? "· aplicado" : "· consultivo"}</span>
        <CollapsibleTrigger
          className="ml-auto cursor-pointer rounded p-0.5 text-muted-foreground hover:text-foreground"
          aria-label="Mostrar veredito bruto de roteamento"
        ...
        <CodeBlock code={prettyOutput} language="json">
          <CodeBlockHeader>
            <CodeBlockTitle className="min-w-0">
              <span className="truncate font-medium uppercase tracking-wide">Veredito</span>
            </CodeBlockTitle>
          </CodeBlockHeader>
        </CodeBlock>
      </CollapsibleContent>
