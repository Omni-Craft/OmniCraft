...
export function selectionHintText(isMac: boolean): string {
  return isMac
    ? "Segure ⌥ e arraste para selecionar · ⌘C para copiar"
    : "Segure Shift e arraste para selecionar · clique com o botão direito para copiar";
}
    ...
    <div className="absolute inset-0 z-[10000] flex items-center justify-center bg-background/85 text-sm text-foreground backdrop-blur-[1px]">
      ...
      {state.kind === "closed" && !reconnectPending && (
        <div className="flex flex-wrap items-center justify-center gap-2 px-3">
          <span>Conexão encerrada: {state.reason}</span>
          {onResume && (
            <Button
              type="button"
              size="xs"
            ...
            >
              {resumePending ? "Retomando…" : "Retomar sessão"}
            </Button>
          )}
          {resumeError && (
      ...
      {state.kind === "error" && <span>Erro na conexão</span>}
    </div>
  );
}
...
function resumeErrorText(error: unknown): string {
  if (error instanceof Error && error.message)
    return `Não foi possível retomar a sessão: ${error.message}`;
  return "Não foi possível retomar a sessão.";
}
 ...
 * served from the OmniCraft server itself or via the Vite dev proxy.
