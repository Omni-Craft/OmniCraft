...
describe("ToolCard rendering", () => {
  ...
  it("expands to reveal the Parameters panel and output on click", () => {
    ...
    expect(screen.getAllByText("Parâmetros").length).toBeGreaterThan(0);
  ...
  it.each([
    ["cancelled", "A ferramenta foi cancelada antes que a saída chegasse."],
    ["no-output", "Nenhuma saída foi registrada para esta chamada de ferramenta."],
    ["output-error", "A ferramenta não retornou saída antes que a resposta falhasse."],
...
describe("ToolGroupSummary", () => {
  ...
  it("labels the run with a pluralized step count and renders children when expanded", () => {
    ...
    expect(screen.getByText("Ver 2 etapas")).toBeInTheDocument();
    fireEvent.click(container.querySelector<HTMLElement>('[data-slot="collapsible-trigger"]')!);
  ...
  it("uses the singular 'step' for one tool and honors an explicit count override", () => {
    ...
    expect(screen.getByText("Ver 1 etapa")).toBeInTheDocument();
    ...
    expect(screen.getByText("Ver 5 etapas")).toBeInTheDocument();
  });
});
