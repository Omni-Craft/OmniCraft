...
function OutputSection({ output }: { output: string }) {
  ...
  return (
    <div className="space-y-2">
      ...
      >
        <CodePanel title="Saída" text={preview.text} copyText={output} copyLabel="Copiar saída" />
      ...
      {canExpand && (
        <div className="flex flex-col gap-2 rounded-md border bg-muted/30 px-3 py-2 text-muted-foreground text-xs sm:flex-row sm:items-center sm:justify-between">
          <span className="min-w-0">
            {isExpanded ? "Mostrando saída completa" : "Pré-visualizando saída"} (
  ...
  return (
    <div className="rounded-md border border-dashed bg-muted/30 p-3">
      <div className="flex items-center gap-2 text-muted-foreground text-sm">
        ...
        <span>
          Aguardando saída
          {duration !== undefined ? ` por ${formatToolDuration(duration)}` : ""}
        </span>
      </div>
...
function EmptyOutputState({ state }: { state: "output-error" | "cancelled" | "no-output" }) {
  let message: string;
  if (state === "cancelled") {
    message = "A ferramenta foi cancelada antes que a saída chegasse.";
  } else if (state === "no-output") {
    message = "Nenhuma saída foi registrada para esta chamada de ferramenta.";
  } else {
    message = "A ferramenta não retornou saída antes que a resposta falhasse.";
  }
  return (
...
function CopyTextButton({ text, label }: CopyTextButtonProps) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>{isCopied ? "Copiado" : label}</TooltipContent>
    </Tooltip>
  );
}
...
function formatOutputStats(preview: OutputPreview): string {
  ...
  if (preview.hiddenLineCount > 0) {
    hidden.push(`${formatCount(preview.hiddenLineCount, "line")} ocultas`);
  }
  if (preview.hiddenCharCount > 0) {
    hidden.push(`${formatCount(preview.hiddenCharCount, "char")} ocultos`);
  }

  return `${formatCount(preview.shownLineCount, "line")} / ${formatCount(
    preview.shownCharCount,
    "char",
  )} mostrados; ${hidden.join(", ")}`;
}

function formatCount(count: number, unit: "line" | "char"): string {
  const label =
    unit === "line" ? (count === 1 ? "linha" : "linhas") : count === 1 ? "caractere" : "caracteres";
