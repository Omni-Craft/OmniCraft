...
describe("CodexGoalDialog", () => {
  ...
  it("shows validation errors without calling the API", async () => {
    ...
    fireEvent.click(screen.getByTestId("codex-goal-save"));
    expect(
      await screen.findByText("O orçamento de tokens deve ser um número inteiro positivo."),
    ).toBeInTheDocument();
  ...
  it("surfaces API errors", async () => {
    ...
    expect(
      await screen.findByText("Não foi possível ler a meta: runner is asleep"),
    ).toBeInTheDocument();
  });
});
...
describe("Codex goal budget parsing", () => {
  ...
  it("rejects non-positive, fractional, and unsafe budgets", () => {
    expect(() => parseCodexGoalBudget("0")).toThrow(/número inteiro positivo/);
    expect(() => parseCodexGoalBudget("1.5")).toThrow(/número inteiro positivo/);
    expect(() => parseCodexGoalBudget("9007199254740992")).toThrow(/número inteiro positivo/);
  });
});
