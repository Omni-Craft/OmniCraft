 ...
 * `OmniCraftHostConfig.searchUsers` (see `lib/host.ts`). When no host searcher
...
import { getOmniCraftUserSearch, type UserSuggestion } from "@/lib/host";
...
export function useUserSearch(query: string): UseUserSearchResult {
  ...
  const { data, isFetching } = useQuery({
    queryKey: ["omnicraftUserSearch", trimmed],
