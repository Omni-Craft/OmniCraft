...
export async function changePassword(body: ChangePasswordRequest): Promise<ChangePasswordResult> {
  ...
  if (res.status >= 500) {
    return { ok: false, error: "Erro no servidor. Tente novamente em instantes." };
  }
...
export async function setup(body: SetupRequest): Promise<LoginResult> {
  ...
  try {
    ...
    return { ok: false, error: "Não foi possível conectar ao servidor.", status: 0 };
  }
  if (res.ok) {
    const data = (await res.json()) as Omit<LoginSuccess, "ok">;
    return { ok: true, ...data };
  ...
  if (res.status === 409) {
    return {
      ok: false,
      error: "Este servidor já tem um administrador. Entre em vez disso.",
      status: 409,
    };
  }
  let message = "Não foi possível criar a conta de administrador.";
  if (res.status >= 500) {
    message = "Erro no servidor. Tente novamente em instantes.";
  } else {
    try {
...
async function _admin<T extends { ok: true }>(
  ...
  if (res.status === 403) message = "Permissão de administrador necessária.";
  else if (res.status === 404) message = "Não encontrado.";
  try {
    const data = (await res.json()) as { error?: string };
    if (data.error) message = data.error;
  } catch {
...
export async function deleteUser(userId: string): Promise<{ ok: true } | AdminFailure> {
  ...
  let message = `Falha ao excluir (${res.status}).`;
  try {
    const data = (await res.json()) as { error?: string };
    if (data.error) message = data.error;
  } catch {
