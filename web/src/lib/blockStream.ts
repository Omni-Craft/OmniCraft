// Mirrors sdks/python-client/omnicraft_client/_stream.py.
//
...
interface ReducerState {
  ...
  // `_translate_omnicraft_event`. Without this, the result panel renders
...
function* processEvent(state: ReducerState, event: StreamEvent): Generator<AnyBlock> {
  switch (event.type) {
    ...
    case "tool_call": {
      ...
      // (omnicraft/runtime/harnesses/_executor_adapter.py) threads
      ...
      // line twice. See designs/RUN_OMNICRAFT_REPL_PARITY.md.
      //
    ...
    case "policy_denied": {
      ...
      yield {
        type: "policy_denied",
        ctx: ctx(state),
        reason: event.reason ?? "Negado pela política",
        phase: event.phase ?? "",
 ...
 *
 * Mirrors `omnicraft_client._stream.BlockStream`. The reducer is a
