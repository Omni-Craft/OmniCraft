 ...
 * this module is a no-op until `setOmniCraftHostConfig` is called by the
...
export interface OmniCraftHostConfig {
  /**
   ...
   * `workspaceFetch` against `/ajax-api/2.0/omnicraft`). When omitted, the
   ...
   * in the UI (e.g. `"/api/2.0/omnicraft"`). When the host proxies the
   * OmniCraft API behind a path prefix, CLI users need the full URL
   * (`https://host/api/2.0/omnicraft`) — this suffix supplies the
   * non-origin part.
   */
...
let _config: OmniCraftHostConfig = {};
let _embedRoot: HTMLElement | null = null;

export function setOmniCraftHostConfig(config: OmniCraftHostConfig): void {
  // Guard: never clobber an already-installed fetcher with an empty config.
  // `OmniCraftApp` installs config during render and React may re-invoke it with
...
export function getOmniCraftHostConfig(): OmniCraftHostConfig {
  return _config;
}
 ...
 */
export function getOmniCraftUserSearch(): OmniCraftHostConfig["searchUsers"] {
 ...
 */
export function getOmniCraftTransformShareLink(): OmniCraftHostConfig["transformShareLink"] {
 ...
 * lands inside the scoped `.omnicraft-app` subtree and inherits its styles.
 ...
 * {@link OmniCraftHostConfig.cliServerUrlSuffix}.
 */
