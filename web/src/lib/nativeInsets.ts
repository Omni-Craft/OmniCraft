...
// `--omnicraft-native-top-bar` / `--omnicraft-native-bottom-bar`; index.css folds
// them (together with the web-owned visibility flags and `env(safe-area-*)`)
// into `--omnicraft-inset-top` / `--omnicraft-inset-bottom`, which `<PageScroll>`
...
export function initNativeInsets(): () => void {
  return onNativeInsets(({ topBar, bottomBar }) => {
    ...
    root.setProperty("--omnicraft-native-top-bar", `${topBar}px`);
    root.setProperty("--omnicraft-native-bottom-bar", `${bottomBar}px`);
  });
}
