...
const FORMATTERS: Record<string, ArgFormatter> = {
  ...
  sys_os_read: (args) => withPath("Ler", args.path),
  ...
  sys_session_send: (args) => {
    ...
    if (sid !== null) return { verb: "Enviar para sessão:", body: sid };
    return sessionTitle("Iniciar sessão filha:", args);
  },
  sys_session_create: (args) => {
    const id = asString(args.agent_id);
    return id === null ? verbOnly("Criar sessão") : { verb: "Criar sessão:", body: id };
  },
  sys_session_get_history: (args) => {
    const id = asString(args.conversation_id);
    return id === null
      ? verbOnly("Obter histórico da sessão")
      : { verb: "Obter histórico da sessão:", body: id };
  },
  ...
  sys_advise_models: (args) => {
    ...
    return count === null
      ? verbOnly("Roteamento inteligente")
      : {
          verb: "Roteamento inteligente:",
          body: `${count} tarefa${count === 1 ? "" : "s"}`,
        };
  },

  sys_session_close: (args) => sessionTitle("Fechar sessão filha:", args),
  sys_session_list: () => verbOnly("Listar sessões filhas"),
  sys_session_get_info: (args) => {
    const id = asString(args.session_id);
    return id === null
      ? verbOnly("Obter informações da sessão")
      : { verb: "Obter informações da sessão:", body: id };
  },

  // Agent-management tools.
  sys_agent_get: (args) => {
    const id = asString(args.session_id);
    return id === null ? verbOnly("Obter agente") : { verb: "Obter agente:", body: id };
  },
  sys_agent_download: (args) => {
    const id = asString(args.session_id);
    return id === null ? verbOnly("Baixar agente") : { verb: "Baixar agente:", body: id };
  },
  sys_agent_list: () => verbOnly("Listar agentes"),

  // Async dispatch + inbox.
  sys_call_async: (args) => {
    const tool = asString(args.tool);
    return tool === null
      ? verbOnly("Despachar assíncrono")
      : { verb: "Despachar assíncrono:", body: tool };
  },
  sys_read_inbox: () => verbOnly("Ler caixa de entrada"),
  ...
  sys_cancel_task: (args) => {
    ...
    return id === null ? verbOnly("Cancelar tarefa") : { verb: "Cancelar tarefa:", body: id };
  },
  ...
  sys_timer_set: (args) => {
    ...
    const repeat = args.repeat === true ? " (repetir)" : "";
    return { verb: "Definir timer:", body: `${seconds}s${repeat}` };
  },
  sys_timer_cancel: (args) => {
    const id = asString(args.timer_id);
    return id === null ? verbOnly("Cancelar timer") : { verb: "Cancelar timer:", body: id };
  },

  // Terminal multiplexer.
  sys_terminal_launch: (args) => terminalTitle("Iniciar terminal", args),
  sys_terminal_read: (args) => terminalTitle("Ler terminal", args),
  sys_terminal_close: (args) => terminalTitle("Fechar terminal", args),
  sys_terminal_list: () => verbOnly("Listar terminais"),
  sys_terminal_send: (args) => {
    const id = terminalId(args);
    if (id === null) return null;
    const payload = asString(args.text) ?? asString(args.keys);
    return payload === null
      ? { verb: "Enviar para", body: `'${id}'` }
      : { verb: `Enviar para '${id}':`, body: payload };
  },

  // Web tools.
  web_search: (args) => {
    const q = asString(args.query);
    return q === null ? null : { verb: "Busca na web:", body: `"${q}"` };
  },
  web_fetch: (args) => {
    const q = asString(args.query);
    if (q !== null) return { verb: "Obter página:", body: `"${q}"` };
    const url = asString(args.url);
    if (url !== null) return { verb: "Obter página:", body: url };
    return null;
  },
};
