...
function textarea() {
  return screen.getByLabelText("Mensagem para o agente") as HTMLTextAreaElement;
}
...
describe("AgentPicker trigger label", () => {
  ...
  it("prefers a claude session override over the cross-session sticky model", () => {
    ...
    // Radix's DropdownMenuTrigger opens on pointerdown (button 0), not a bare
    // MouseEvent click — jsdom's .click() never dispatches pointerdown.
...
describe("Composer Codex Plan-mode control", () => {
  ...
  it("shows the active pressed state while Plan mode is enabled", () => {
    ...
    expect(button).toHaveAccessibleName("Sair do modo de planejamento");
  });
...
describe("Composer placeholder", () => {
  ...
  it("shows the normal placeholder when the runner is live", () => {
    render(<Composer {...composerProps({})} />);
    expect(textarea().placeholder).toMatch(/pergunte qualquer coisa ao agente/i);
  });
  ...
  it("runner_asleep (reconnectHint): enabled composer nudges the user to send", () => {
    ...
    expect(textarea().placeholder).toBe("Envie uma mensagem para reconectar esta sessão");
  ...
  it("streaming wins over the reconnect hint", () => {
    ...
    expect(textarea().placeholder).toMatch(/acompanhamento/i);
  });
  ...
  it("unreachable (host offline / local-stranded): composer is blocked", () => {
    ...
    expect(textarea().placeholder).toMatch(/reconecte abaixo/i);
  });
...
describe("Composer pending elicitation", () => {
  ...
  it("locks the textarea and send button while an elicitation is pending", () => {
    ...
    expect(ta.placeholder).toBe("Responda à solicitação pendente acima para continuar");
...
describe("Composer sub-agent tray", () => {
  ...
  it("renders the sub-agent name when a label is passed", () => {
    ...
    expect(screen.getByText(/Conversando com o sub-agente/)).toBeTruthy();
  });
});
