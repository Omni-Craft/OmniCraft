...
describe("Composer structural read-only reasons", () => {
  it("uses the closed-session reason when the live snapshot is closed", () => {
    expect(
      readOnlyReasonForSessionLabels(
        { labels: { "omnicraft.closed": "true" } },
        { labels: { "omnicraft.wrapper": "claude-code-native-ui-subagent" } },
      ),
    ).toBe("Esta sessão de sub-agente está fechada");
  });
  ...
  it("falls back to the sidebar wrapper label for native read-only children", () => {
    ...
    ).toBe("Sub-agentes do Claude Code são somente leitura");
  });
...
describe("workingIndicatorLabel", () => {
  it("shows the plain Working label when no background tasks remain", () => {
    expect(workingIndicatorLabel(0)).toBe("Trabalhando…");
  });
  ...
  it("treats a negative count as no background tasks", () => {
    ...
    expect(workingIndicatorLabel(-1)).toBe("Trabalhando…");
  });

  it("uses the singular noun for exactly one background task", () => {
    expect(workingIndicatorLabel(1)).toBe("1 tarefa em segundo plano ainda em execução");
  });

  it("pluralizes the noun for more than one background task", () => {
    expect(workingIndicatorLabel(3)).toBe("3 tarefas em segundo plano ainda em execução");
  });

  it("pins the first rotation message to 'Trabalhando…'", () => {
    // A fresh tick and the default arg both land on index 0, so this label
    // must stay "Trabalhando…" — the invariant the (0) / (-1) cases rely on.
    expect(WORKING_MESSAGES[0]).toBe("Trabalhando…");
    expect(workingIndicatorLabel(0, 0)).toBe("Trabalhando…");
  });
  ...
  it("wraps the rotation modulo the pool length", () => {
    expect(workingIndicatorLabel(0, WORKING_MESSAGES.length)).toBe("Trabalhando…");
  ...
  it("ignores the tick while background tasks remain", () => {
    ...
    expect(workingIndicatorLabel(2, 5)).toBe("2 tarefas em segundo plano ainda em execução");
  });
});
