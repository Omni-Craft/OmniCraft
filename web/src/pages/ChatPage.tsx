...
export function ChatPage() {
  ...
  const subAgentTabTitle =
    activeSession?.parentSessionId != null
      ...
      : null;
  useEffect(() => {
    const fallback = urlConvId ? UNTITLED_CONVERSATION_LABEL : "OmniCraft";
                ...
                <ConversationEmptyState>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-muted-foreground text-base">
                      {agentsError
                        ? `Falha ao carregar agentes: ${agentsError instanceof Error ? agentsError.message : String(agentsError)}`
  ...
  return (
    <div className="flex flex-1 items-center justify-center px-6">
      <div className="flex max-w-md flex-col items-center gap-3 text-center">
        <h1 className="font-medium text-foreground text-lg">Conversa não encontrada</h1>
  ...
  return (
    ...
    >
      {/* The single announced string. Held stable at "Working…" so the rotating
          ...
          owns the visuals). */}
      {show && <span className="sr-only">Trabalhando…</span>}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {jumping ? "Carregando histórico…" : "Ir para o topo"}
      </Button>
    </div>
  );
}
...
export function workingIndicatorLabel(bgCount: number, tick = 0): string {
  if (bgCount > 0) {
    return bgCount === 1
      ? "1 tarefa em segundo plano ainda em execução"
      : `${bgCount} tarefas em segundo plano ainda em execução`;
  }
...
const SANDBOX_STAGE_LABELS: Record<string, string | undefined> = {
  ...
  starting: "Conectando host",
  connecting: "Iniciando agente",
};
...
export function SandboxFailedIndicator({ status }: { status: SandboxStatus }) {
  return (
    ...
    >
      ...
      <span>Falha ao iniciar sandbox{status.error ? `: ${status.error}` : ""}</span>
    </div>
  );
}
      ...
      >
        ...
        <span>
          {liveness.kind === "host_offline"
            ? "Host está offline — clique para reconectar"
            : "Agente desconectado — clique para reconectar"}
        </span>
      </button>
    );
  }
...
export function RunnerStartingIndicator({ variant }: { variant: "hero" | "row" }) {
  ...
  const line = sandboxLabel !== undefined ? `${sandboxLabel}…` : "Iniciando…";
  ...
  if (variant === "hero") {
    return (
      <ConversationEmptyState
        ...
        title={sandboxLabel !== undefined ? `${sandboxLabel}…` : "Iniciando…"}
        description={
          sandboxLabel !== undefined
            ? "Configurando seu sandbox — isso pode levar um minuto."
...
export function mcpStartingLine(starting: string[], total: number): string {
  if (total === 1 && starting.length === 1) {
    return `Iniciando servidor MCP: ${starting[0]}…`;
  }
  const shown = starting.slice(0, MCP_STARTING_NAMES_SHOWN);
  if (starting.length > MCP_STARTING_NAMES_SHOWN) shown.push("…");
  return `Iniciando servidores MCP (${total - starting.length}/${total}): ${shown.join(", ")}`;
}
...
export function mcpSettledNames(names: string[]): string {
  ...
  return `${shown.join(", ")}, +${names.length - MCP_SETTLED_NAMES_SHOWN} a mais`;
}
...
export function McpStartupIndicator() {
  ...
  const parts: string[] = [];
  if (failed.length > 0) parts.push(`falhou: ${mcpSettledNames(failed)}`);
  if (cancelled.length > 0) parts.push(`cancelado: ${mcpSettledNames(cancelled)}`);
  return (
    <Message from="assistant" data-testid="mcp-startup-indicator" role="status">
      <MessageContent>
        <span className="flex items-center gap-2 text-muted-foreground text-sm">
          <AlertTriangleIcon className="size-4 shrink-0" aria-hidden />
          {`Inicialização do MCP incompleta (${parts.join("; ")})`}
        </span>
      </MessageContent>
    </Message>
  );
      ...
      >
        <div className="flex items-center gap-0.5">
          ...
          <button
            ...
            title={terminalStartingUp ? "Terminal está iniciando…" : undefined}
  ...
  const handleCopy = useCallback(() => {
    ...
    copyText(text).then(
      () => {
        ...
        if (isMobile) {
          showToast(<span className="text-sm">Copiado para a área de transferência</span>, {
            duration: 1500,
          });
        }
      },
  ...
  return (
    <>
      ...
      {bubble.lifecycle === "failed" && (
        <p className="text-destructive text-xs">Erro: {bubble.error}</p>
      )}
    </>
  );
}
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <span
          ...
          aria-label={`${usedPct}% do contexto usado`}
        >
      ...
      <TooltipContent side="top" className="max-w-44 text-center text-xs">
        <p className="tabular-nums">{usedPct}% do contexto usado.</p>
      </TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 truncate">
        Conversando com o sub-agente <strong className="font-semibold">{label}</strong>
      </span>
    </div>
  );
}
...
export function Composer({
  ...
  const toggleCodexPlanMode = async () => {
    ...
    try {
      ...
      setCommandError(
        `Não foi possível ${codexPlanMode ? "sair do" : "entrar no"} modo de planejamento: ${message}`,
      );
    } finally {
  ...
  const executeSlashCommand = (cmd: string, arg: string): boolean => {
    switch (cmd) {
      case "/compact":
        if (!showCompact) {
          setCommandError("/compact não é compatível com este tipo de agente");
          return true;
        }
        dirtyRef.current = true;
        setValue("");
        ...
        void useChatStore
          ...
          .compact()
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : "Falha ao compactar");
          });
        return true;
      case "/effort": {
        ...
        if (!arg || !valid.includes(arg.toLowerCase())) {
          setCommandError(`Uso: /effort ${valid.join(" | ")}`);
          return true;
        }
        ...
        void useChatStore
          ...
          .setEffort(level)
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : "Falha ao definir o esforço");
          });
        return true;
      }
      case "/model": {
        ...
        if (!target) {
          ...
          const current = sessionModelOverride
            ? `${sessionModelOverride} (substituição)`
            : (llmModel ?? "padrão do agente");
          setCommandError(`Modelo: ${current}\nUso: /model <nome> · /model default para redefinir`);
          return true;
        }
        ...
        void useChatStore
          ...
          .catch((err: unknown) => {
            setCommandError(err instanceof Error ? err.message : "Falha ao definir o modelo");
          });
        return true;
      }
      case "/context": {
        ...
        const lines: string[] = [];
        if (sessionModelOverride) lines.push(`Modelo: ${sessionModelOverride} (substituição)`);
        else if (llmModel) lines.push(`Modelo: ${llmModel}`);
        ...
        if (tokensUsed != null && contextWindow != null && contextWindow > 0) {
          ...
          lines.push("(Tamanho da janela de contexto desconhecido)");
        } else {
          lines.push("Nenhum dado de uso ainda — envie uma mensagem primeiro.");
        }
        lines.push(`Itens no contexto: ${blocks.length}`);
      ...
      default:
        setCommandError(
          `Comando desconhecido: ${cmd}. Disponíveis: ${Object.keys(slashCommands).join(", ")}`,
        );
        return false;
    }
  };
            ...
            placeholder={
              readOnlyReason !== null
                ...
                : isReadOnly
                  ...
                  : unreachable
                    ? "Sessão offline — reconecte abaixo para continuar"
                    : hasPendingElicitation
                      ? "Responda à solicitação pendente acima para continuar"
                      : disabled
                        ? "Aguardando agentes…"
                        : isStreaming
                          ? "Envie um acompanhamento (na fila) — Esc para parar"
                          : sandboxAsleepHint
                            ? "O host desta sessão está offline. A próxima mensagem vai retomar o host do sandbox, o que pode levar minutos"
                            : reconnectHint
                              ? "Envie uma mensagem para reconectar esta sessão"
        ...
        <div className="flex items-center justify-between gap-2 px-2 pb-2">
          ...
          <div className="flex min-w-0 items-center gap-0.5">
            ...
            <Button
              ...
              title={showInterruptButton ? "Interromper" : "Enviar"}
              aria-label={showInterruptButton ? "Interromper" : "Enviar"}
            >
              {showInterruptButton ? (
              ...
              <span className="sr-only">{showInterruptButton ? "Interromper" : "Enviar"}</span>
            </Button>
          </div>
        </div>
      </div>
...
function AgentPicker({
  ...
  if (isLoading) {
    triggerContent = "Carregando…";
  } else if (!hasAgents) {
    triggerContent = "Nenhum agente";
  } else if (modelLabel) {
    triggerContent = (
      <>
    ...
    triggerContent = kiroLaunchFallbackLabel ?? agentDisplayName ?? "Modelo";
  } else {
    return null;
  }
      ...
      <DropdownMenuContent align="start" className="min-w-64 p-1">
        ...
        {modelOptions.length > 0 && (
          <>
            ...
            <PickerSectionHeader>Modelos</PickerSectionHeader>
            {modelOptions.map((m) => {
        ...
        {showEffort && (
          <>
            ...
            <PickerSectionHeader>Esforço</PickerSectionHeader>
