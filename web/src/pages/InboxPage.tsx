      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        {(items.length > 0 || commentInbox.items.length > 0) && (
          <span className="text-sm text-muted-foreground">
            {[
              items.length > 0 &&
                (items.length === 1 ? "1 aprovação" : `${items.length} aprovações`),
              commentInbox.items.length > 0 &&
                (commentInbox.items.length === 1
                  ? "1 comentário"
                  : `${commentInbox.items.length} comentários`),
            ]
              .filter(Boolean)
              .join(" · ")}{" "}
            aguardando
      ...
      {failedSessionCount > 0 && (
        ...
        >
          ...
          <span className="flex-1">
            Não foi possível carregar itens da caixa de entrada de {failedSessionCount}{" "}
            {failedSessionCount === 1 ? "sessão" : "sessões"}.
          </span>
          <Button
            variant="outline"
            size="sm"
        ...
        commentInbox.items.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-16 text-center">
            ...
            <p className="text-xs text-muted-foreground">
              Quando um agente precisar da sua contribuição ou alguém comentar em um arquivo, isso
              aparecerá aqui.
            </p>
          </div>
        )}
