...
export function LoginPage() {
  ...
  const [error, setError] = useState<string | null>(
    magicError === "expired"
      ? "Esse link de entrada expirou. Digite sua senha para entrar."
      : magicError === "missing"
        ? "Esse link de entrada não é mais válido. Digite sua senha para entrar."
        : null,
  );
    ...
    >
      <div className="w-full max-w-sm space-y-6">
        <div className="space-y-1 text-center">
          ...
          <p className="text-sm text-muted-foreground">Bem-vindo ao OmniCraft.</p>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Em uma instalação nova, seu usuário é o login da sua máquina (a saída de{" "}
              <code className="font-mono">whoami</code>), a menos que um admin tenha definido um
              diferente.
            </p>
          </div>
