...
describe("PoliciesPage gating", () => {
  ...
  it("blocks non-admins with a permission message", async () => {
    ...
    renderPage();
    expect(
      await screen.findByText("Você não tem permissão para gerenciar políticas globais."),
    ).toBeInTheDocument();
  });
...
describe("PoliciesPage list", () => {
  it("shows the empty state when no global policies are configured", async () => {
    renderPage();
    expect(await screen.findByText(/Nenhuma política global configurada/)).toBeInTheDocument();
  });
  ...
  it("renders each policy with its handler, a Disabled badge, and parameters", async () => {
    setPolicies([
      ...
      policy({
        id: "p2",
        name: "rate_limit",
        handler: "omnicraft.policies.rate_limit",
    ...
    expect(screen.getByText("omnicraft.policies.block_canada")).toBeInTheDocument();
    expect(screen.getByText("Desativada")).toBeInTheDocument(); // only the disabled one
...
describe("PoliciesPage actions", () => {
  it("toggling a policy's switch fires the update mutation with the new state", async () => {
    ...
    const toggle = await screen.findByRole("switch", { name: "Alternar block_canada" });
  ...
  it("deletes a policy through the confirmation dialog", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Remover política" }));

    const dialog = await screen.findByRole("dialog");
    expect(within(dialog).getByText("Remover block_canada?")).toBeInTheDocument();
  ...
  it("adds array (multi-select) values via the model combobox as a coerced list", async () => {
    ...
    vi.mocked(policies.usePolicyRegistry).mockReturnValue({
      data: [
        {
          handler: "omnicraft.policies.budget",
          kind: "factory",
          name: "Budget Guard",
    ...
    renderPage();
    await screen.findByText(/Nenhuma política global configurada/);

    fireEvent.click(screen.getByRole("button", { name: /Adicionar política/ }));
    const dialog = await screen.findByRole("dialog");
    fireEvent.click(within(dialog).getByText("Budget Guard"));

    const combo = within(dialog).getByPlaceholderText("Selecione ou digite um valor…");
    ...
    expect(payload.handler).toBe("omnicraft.policies.budget");
  ...
  it("adds a global policy from the registry via the Add dialog", async () => {
    ...
    expect(addMutate).toHaveBeenCalledWith(
      { name: "block_canada", type: "python", handler: "omnicraft.policies.block_canada" },
      expect.anything(),
    );
  });
  ...
  it("Cancel steps back to the policy list after a policy is selected", async () => {
    ...
    expect(within(dialog).queryByPlaceholderText("Filtrar políticas...")).toBeNull();

    fireEvent.click(within(dialog).getByRole("button", { name: "Cancelar" }));

    expect(within(dialog).getByPlaceholderText("Filtrar políticas...")).toBeInTheDocument();
...
describe("PoliciesPage single-user mode", () => {
  ...
  it("shows the full page without the admin gate (empty state)", async () => {
    ...
    expect(
      screen.queryByText("Você não tem permissão para gerenciar políticas globais."),
