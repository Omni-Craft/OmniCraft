...
describe("QueuedMessagesStrip", () => {
  ...
  it("calls onDelete with the row's queueId when its remove button is clicked", () => {
    ...
    const buttons = screen.getAllByRole("button", { name: "Remover mensagem da fila" });
  ...
  it("calls onEdit with the row's queueId when its edit button is clicked", () => {
    ...
    const buttons = screen.getAllByRole("button", { name: "Editar mensagem da fila" });
    expect(buttons).toHaveLength(2);
  ...
  it("shows no steer button when onSteer is omitted", () => {
    ...
    expect(screen.queryByRole("button", { name: "Enviar mensagem da fila agora" })).toBeNull();
  });
  ...
  it("calls onSteer with the row's queueId when its steer button is clicked", () => {
    ...
    const buttons = screen.getAllByRole("button", { name: "Enviar mensagem da fila agora" });
    expect(buttons).toHaveLength(2);
    fireEvent.click(buttons[1]!);
  ...
  it("shows a drag handle per row only when onReorder is provided", () => {
    ...
    expect(screen.queryByRole("button", { name: "Reordenar mensagem da fila" })).toBeNull();
    ...
    expect(screen.getAllByRole("button", { name: "Reordenar mensagem da fila" })).toHaveLength(2);
  });
});
