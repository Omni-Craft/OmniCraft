...
function fillForm(username: string, password: string, confirm: string) {
  fireEvent.change(screen.getByLabelText(/usuário/i), { target: { value: username } });
  fireEvent.change(screen.getByLabelText(/^senha/i), { target: { value: password } });
  fireEvent.change(screen.getByLabelText(/confirmar senha/i), { target: { value: confirm } });
}
...
describe("RegisterPage", () => {
  it("shows an invite-required alert and no form when the invite token is missing", () => {
    renderRegisterAt("");
    expect(screen.getByRole("alert")).toHaveTextContent(/token de convite/i);
