...
describe("AppShell header", () => {
  it("renders the sidebar toggle on all pages", () => {
    mockConversations([]);
    renderShell("/");
    expect(screen.getByRole("button", { name: /barra lateral/i })).toBeInTheDocument();
  });
  ...
  it("shows the terminal-startup spinner while a terminal-first session is coming up", () => {
    ...
    mockConversations([
      { id: "conv_terminal", permission_level: null, labels: { "omnicraft.ui": "terminal" } },
    ]);
...
describe("Right workspace card visibility", () => {
  ...
  it("starts open for a fresh session (no stored open-state)", () => {
    ...
    expect(screen.getByRole("complementary", { name: "Workspace" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Recolher painel direito" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Expandir painel direito" })).toBeNull();
  });
  ...
  it("persists the open-state per session across remounts", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Recolher painel direito" }));
    ...
    expect(screen.queryByRole("complementary", { name: "Workspace" })).toBeNull();
    expect(screen.getByRole("button", { name: "Expandir painel direito" })).toBeInTheDocument();
  });
    ...
    mockConversations([
      {
        id: "conv_sdk",
        permission_level: null,
        // Terminal-first SDK session: omnicraft.ui stamped by the
...
describe("AppShell URL sync — file param", () => {
  ...
  it("clears file/diff/comment/view params from the URL when the rail is collapsed", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Expandir painel direito" }));
...
describe("Files scope default and persistence", () => {
  const PREF_KEY = "omnicraft:files-panel-preferences";
...
describe("Mobile session menu", () => {
  ...
  function openSessionMenu() {
    const trigger = screen.getByRole("button", { name: /abrir menu da sessão/i });
  ...
  it("lists the native session's menu entries (Terminals folds into the pill)", () => {
    ...
    expect(screen.getByRole("menuitem", { name: /^Arquivos$/i })).toBeInTheDocument();
    expect(screen.queryByRole("menuitem", { name: /Terminais/i })).toBeNull();
    expect(screen.getByRole("menuitem", { name: /Agentes/i })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: /Tarefas/i })).toBeInTheDocument();
  });

  it("keeps the Terminals entry in terminal-first SDK sessions (no native wrapper)", () => {
    // Terminal-first SDK sessions (embedded OmniCraft REPL, `omnicraft.ui:
    ...
    expect(screen.getByRole("menuitem", { name: /^Arquivos$/i })).toBeInTheDocument();
    expect(screen.getByRole("menuitem", { name: /Terminais/i })).toBeInTheDocument();
  });
  ...
  it("shows the Shells entry on mobile when the agent declares shell access", () => {
    ...
    const shellsEntry = screen.getByRole("menuitem", { name: /^Terminais$/i });
  ...
  it("opens the Agents drawer and mounts the subagents panel", () => {
    ...
    openSessionMenu();
    fireEvent.click(screen.getByRole("menuitem", { name: /Agentes/i }));
  ...
  it("opens the files drawer from the Files entry in the default tree view", () => {
    ...
    openSessionMenu();
    fireEvent.click(screen.getByRole("menuitem", { name: /^Arquivos$/i }));
  ...
  it("keeps the FAB with only the Agents entry for a minimal agent", () => {
    ...
    // Radix DropdownMenu opens on pointerdown, not click.
    fireEvent.pointerDown(screen.getByRole("button", { name: /abrir menu da sessão/i }), {
      button: 0,
      ctrlKey: false,
    });
    expect(screen.getByRole("menuitem", { name: /Agentes\s*1/i })).toBeInTheDocument();
    expect(screen.queryByRole("menuitem", { name: /Arquivos/i })).toBeNull();
    expect(screen.queryByRole("menuitem", { name: /Terminais/i })).toBeNull();
  });
});
...
describe("AppShell clone/fork action", () => {
  ...
  it("opens the fork dialog (name suggested from the source title) when clicked", () => {
    ...
    expect(screen.getByText("Clonar a partir desta resposta")).toBeInTheDocument();
    ...
    expect(nameInput).toHaveAttribute("placeholder", "Clone de Auth refactor");
  });
});
...
describe("AppShell share action", () => {
  it("shows the Share button to an owner of a top-level session", () => {
    ...
    withWindowOrigin("https://app.example.com", () => {
      ...
      const shareButton = screen.getByRole("button", { name: /compartilhar sessão/i });
    ...
    withWindowOrigin("http://localhost:6767", () => {
      ...
      expect(
        screen.getByLabelText(
          "Compartilhar sessão desativado: O compartilhamento não está disponível em um servidor local.",
        ),
      ).toBeInTheDocument();
      expect(shareButton).toHaveAttribute(
        "title",
        "O compartilhamento não está disponível em um servidor local.",
      );
    });
  });
  ...
  it("hides the Share button on a sub-agent (child) session", () => {
    ...
    expect(screen.queryByRole("button", { name: /compartilhar sessão/i })).toBeNull();
  });
...
describe("Mobile header actions menu", () => {
  ...
  it("offers Share and Clone for an owner of a top-level session", () => {
    withWindowOrigin("https://app.example.com", () => {
      ...
      const shareItem = screen.getByRole("menuitem", { name: /^compartilhar$/i });
      ...
      expect(screen.getByRole("menuitem", { name: /informações do agente/i })).toBeInTheDocument();
  ...
  it("opens the agent-info dialog with the agent's tools", () => {
    ...
    fireEvent.click(screen.getByRole("menuitem", { name: /informações do agente/i }));
