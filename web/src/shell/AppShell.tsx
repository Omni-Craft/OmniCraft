  ...
  useEffect(() => {
    ...
    const unsubSubmit = desktop.onBrowserElementPromptSubmit?.((payload) => {
      ...
      if (!cid || !payload.element || !payload.prompt) {
        signal(false, "Elemento ou prompt ausente.");
        return;
      }
      if (!boundAgentId) {
        signal(false, "Nenhum agente vinculado a esta sessão ainda.");
        return;
      }
      try {
        ...
        void useChatStore
          ...
          .then(() => signal(true, "Enviado ao agente."))
          .catch((err: unknown) => signal(false, `Falha ao enviar: ${String(err)}`));
        // Clear the stashed screenshot so a later submit without a fresh pick
        // doesn't reuse a stale crop.
        designShotRef.current.delete(cid);
      } catch (err) {
        signal(false, `Erro: ${String(err)}`);
      }
    });
        ...
        AgentInfoButton popover, opened from the header's three-dot menu. */}
          {hasAgentInfo && (
            <Dialog open={agentInfoOpen} onOpenChange={setAgentInfoOpen}>
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>Agente</DialogTitle>
                  <DialogDescription className="sr-only">
                    Ferramentas e políticas configuradas para o agente ativo.
                  </DialogDescription>
                </DialogHeader>
