...
describe("ChatHeader — sub-agent affordance", () => {
  it("renders no back link or sub-agent label on a top-level session", () => {
    ...
    expect(screen.queryByRole("link", { name: "Voltar à sessão pai" })).toBeNull();
    expect(screen.queryByText("Sub-agente")).toBeNull();
  });
  ...
  it("falls back to a lone 'Sub-agent' label before the agent snapshot loads", () => {
    ...
    // name yet, the label collapses to a single "Sub-agente" — never the
    // redundant "Sub-agente" over "Sub-agente" two-line stack.
    expect(screen.getByRole("link", { name: "Voltar à sessão pai" })).toHaveAttribute(
      "href",
      "/c/parent-123",
    );
    expect(screen.getByText("Sub-agente")).toBeInTheDocument();
