...
describe("CommandPalette — sessions", () => {
  it("lists sessions by display label with their agent type", () => {
    ...
    // Null title → conversationDisplayLabel's "Nova sessão" fallback.
  ...
  it("renders the Sessions group above Actions", () => {
    ...
    const headings = screen.getAllByText(/^(Sessões|Ações)$/).map((el) => el.textContent);
    expect(headings).toEqual(["Sessões", "Ações"]);
  });
...
describe("CommandPalette — input", () => {
  it("uses the sessions-first placeholder", () => {
    ...
    expect(screen.getByPlaceholderText("Buscar sessões ou executar um comando")).toBeTruthy();
  });
});
...
describe("CommandPalette — actions", () => {
  it("lists the built-in action commands", () => {
    ...
    expect(screen.getByText("Ir para Caixa de entrada")).toBeTruthy();
    expect(screen.getByText("Ir para Configurações")).toBeTruthy();
    expect(screen.getByText("Alternar barra lateral de conversas")).toBeTruthy();
    expect(screen.getByText("Alternar barra lateral do workspace")).toBeTruthy();
  });
  ...
  it("runs a navigation action and closes the palette", () => {
    ...
    fireEvent.click(screen.getByText("Ir para Configurações"));
  ...
  it("invokes the sidebar-toggle callbacks", () => {
    ...
    fireEvent.click(screen.getByText("Alternar barra lateral de conversas"));
    expect(onToggleLeftSidebar).toHaveBeenCalledTimes(1);

    fireEvent.click(screen.getByText("Alternar barra lateral do workspace"));
