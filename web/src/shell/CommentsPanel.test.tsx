...
describe("CommentsPanel copy-comment-link", () => {
  it("shows a link button for each open comment when onCopyCommentLink is provided", () => {
    ...
    const linkButtons = screen.getAllByRole("button", {
      name: "Copiar link do comentário",
    });
  ...
  it("calls onCopyCommentLink with the correct comment ID when the link button is clicked", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Copiar link do comentário" }));
  ...
  it("shows a link button for addressed comments after switching to the Addressed tab", () => {
    ...
    expect(screen.queryByRole("button", { name: "Copiar link do comentário" })).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: /resolvidos/i }));
    ...
    const linkButton = screen.getByRole("button", { name: "Copiar link do comentário" });
...
describe("CommentsPanel read-only collaborator gating", () => {
  it("shows the read-only banner when canEdit is false", () => {
    ...
    expect(screen.getByText("Você tem acesso somente leitura a esta sessão.")).toBeInTheDocument();
  });

  it("does not show the read-only banner for editors (canEdit true)", () => {
    renderGated({ canEdit: true });
    expect(screen.queryByText("Você tem acesso somente leitura a esta sessão.")).toBeNull();
  });
  ...
  it("hides the add-comment form for a fresh selection when read-only", () => {
    ...
    expect(screen.queryByPlaceholderText("Adicionar um comentário…")).toBeNull();
    expect(screen.queryByRole("button", { name: "Adicionar comentário" })).toBeNull();
  });
  ...
  it("shows the add-comment form for the same selection when editing is allowed", () => {
    ...
    expect(screen.getByPlaceholderText("Adicionar um comentário…")).toBeInTheDocument();
  ...
  it("still lets a read-only viewer read comments and copy links", () => {
    ...
    expect(screen.getByRole("button", { name: "Copiar link do comentário" })).toBeInTheDocument();
  });
});
...
//
// A long body is clamped to a few lines with a Google-Docs-style "Mostrar mais"
...
describe("CommentsPanel show more / less", () => {
  it("shows a working Show more/less toggle when the body overflows the clamp", () => {
    ...
    try {
      ...
      // Collapsed: toggle reads "Mostrar mais" and the body carries the clamp class.
...
describe("CommentsPanel resize affordance", () => {
  ...
  it("omits the handle and inline width on a narrow (mobile) viewport", () => {
    ...
    try {
      ...
      const panel = screen.getByText("Comentários").closest("div")?.parentElement as HTMLElement;
