...
function formatCommentTime(createdAt: number): string {
  ...
  if (date.toDateString() === now.toDateString()) return `${time} Hoje`;
  const yesterday = new Date(now);
  yesterday.setDate(now.getDate() - 1);
  if (date.toDateString() === yesterday.toDateString()) return `${time} Ontem`;
...
function CommentCard({
  ...
  const statusLabel = c.status === "addressed" ? "Resolvido" : null;
          ...
          <div className="flex min-w-0 flex-col gap-0.5">
            <div className="flex min-w-0 items-center gap-1.5">
              <span
                ...
                style={avatarStyle(c.created_by ?? "Você")}
              >
                {(c.created_by ?? "V")[0].toUpperCase()}
              </span>
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>{c.created_by ?? "Você"}</TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          {(onEdit || onDelete || onCopyLink) && (
            <div className="flex shrink-0 items-center gap-2 mr-0.5">
              ...
              {onCopyLink && (
                <button
                  type="button"
                  aria-label="Copiar link do comentário"
                  className="cursor-pointer text-[11px] text-muted-foreground transition-colors hover:text-foreground"
                  onClick={(e) => {
                    e.stopPropagation();
                    onCopyLink();
