  ...
  return (
    ...
    >
      ...
      <header className="flex shrink-0 items-center justify-between border-border border-b px-4 py-3">
        <h2 className="font-medium text-sm">Logs de execução</h2>
...
function SessionItemsList({ sessionId }: { sessionId: string }) {
  ...
  if (isLoading) {
    return <div className="text-muted-foreground text-xs">Carregando…</div>;
  }
  if (error) {
    return <div className="text-destructive text-xs">Falha ao carregar itens: {error.message}</div>;
  }
  if (items.length === 0) {
    return <div className="text-muted-foreground text-xs">Nenhum item</div>;
  }
  return (
    <div
      ref={scrollRootRef}
