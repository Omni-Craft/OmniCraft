...
function FileViewerBody({
  ...
  if (lang === "markdown" && viewMode !== "diff") {
    ...
    const modeOptions: ToolbarOption[] = [
      {
        ...
        tooltip: "Pré-visualização renderizada",
      ...
      },
      {
        key: "md-source",
        label: "Origem",
        tooltip: "Código-fonte Markdown bruto",
    ...
    toolbarActions.push({
      key: "md-view-mode",
      label: `Modo de visualização: ${activeMode.label}`,
    ...
    toolbarActions.push({
      key: "preview",
      label: viewMode === "preview" ? "Ver origem" : "Ver pré-visualização",
      icon:
  ...
  toolbarActions.push({
    key: "comments",
    label: commentsOpen ? "Ocultar comentários" : "Mostrar comentários",
  ...
  if (isDiffAvailable) {
    toolbarActions.push({
      key: "diff",
      label: viewMode === "diff" ? "Sair da visualização de diferenças" : "Mostrar diferenças",
  ...
  if (viewMode === "diff" && splitToggleAvailable) {
    toolbarActions.push({
      key: "diff-layout",
      label: diffLayout === "unified" ? "Visualização dividida" : "Visualização unificada",
      icon:
        diffLayout === "unified" ? (
  ...
  if (!isDeletedFile && fileQuery.data) {
    toolbarActions.push({
      ...
      tooltip: fileQuery.data.truncated
        ? "Baixar (o arquivo foi truncado — o conteúdo pode estar incompleto)"
        : "Baixar",
  ...
  toolbarActions.push({
    ...
    tooltip: linkCopied ? "Copiado!" : "Copiar link",
    icon: linkCopied ? (
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      variant="ghost"
                      size="icon-sm"
                      aria-label="Fechar visualizador de arquivo"
        ...
        >
          ...
          {saveStatus !== "idle" && (
            <span
              ...
              title={
                saveStatus === "offline"
                  ? "Executor offline — suas alterações serão salvas ao reconectar"
                  : undefined
              }
              className={cn(
        ...
        <div ref={contentAreaRef} className="flex-1 overflow-y-auto min-w-0">
          {isDeletedFile && viewMode !== "diff" ? (
            <div className="flex flex-col items-center justify-center gap-2 p-8 text-sm text-muted-foreground">
              ...
              <span>Este arquivo foi excluído.</span>
              {isDiffAvailable && (
                <span className="text-xs">
                  Clique em <FileDiffIcon className="inline size-3.5 align-text-bottom" /> para ver
      ...
      >
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            ...
            <DialogDescription>
              Suas edições serão perdidas se você sair sem salvar.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
