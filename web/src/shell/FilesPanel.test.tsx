...
describe("FilesPanel working folder directory", () => {
  ...
  it("does not render a directory label when workingDir is null", () => {
    ...
    // "Pasta de trabalho" label is present but no directory name span
    expect(screen.getByText("Pasta de trabalho")).toBeInTheDocument();
...
describe("FilesPanel working folder header role", () => {
  // The "Pasta de trabalho" header is a static label in every mode — it is not a
  ...
  it("renders the header as a static label (no toggle button) in the standalone card", () => {
    ...
    expect(screen.getByRole("radiogroup", { name: "Escopo dos arquivos" })).toBeInTheDocument();
  });
  ...
  it("renders a static label header with a Close button in the drawer", () => {
    ...
    expect(screen.queryByRole("button", { name: /working folder/i })).toBeNull();
    expect(screen.getByText("Pasta de trabalho")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Fechar arquivos" })).toBeInTheDocument();
  });
});
...
describe("FilesPanel scope switch (Changed | All) visibility", () => {
  ...
  it("shows the scope switch in frameless (inline right-rail) mode", () => {
    ...
    expect(screen.getByRole("radio", { name: /^alterados$/i })).toHaveAttribute(
      "aria-checked",
      "false",
    );
    expect(screen.getByRole("radio", { name: /^todos$/i })).toHaveAttribute("aria-checked", "true");
  });
  ...
  it("shows the scope switch in full-screen drawer mode (onClose)", () => {
    ...
    expect(screen.getByRole("radio", { name: /^alterados$/i })).toBeInTheDocument();
  ...
  it("calls onFlatViewChange(true) when the Changed segment is clicked", () => {
    ...
    fireEvent.click(screen.getByRole("radio", { name: /^alterados$/i }));
...
describe("FilesPanel changed files search", () => {
  it("shows the search field only for the Changed view", () => {
    ...
    expect(screen.queryByRole("searchbox", { name: "Buscar arquivos alterados" })).toBeNull();
  ...
  it("filters already-loaded changed files case-insensitively", () => {
    ...
    fireEvent.change(screen.getByRole("searchbox", { name: "Buscar arquivos alterados" }), {
      target: { value: "BUTTON" },
    });
  ...
  it("clears the search query when switching from Changed to Explore view", () => {
    ...
    expect(screen.getByRole("searchbox", { name: "Buscar arquivos alterados" })).toHaveValue("App");
    ...
    expect(screen.getByRole("searchbox", { name: "Buscar arquivos alterados" })).toHaveValue("");
  });
  ...
  it("renders a Close button in full-screen drawer mode", () => {
    ...
    const closeButton = screen.getByRole("button", { name: "Fechar arquivos" });
  ...
  it("preserves eye-icon (show-hidden) state when opening the drawer", () => {
    ...
    expect(screen.getByRole("button", { name: "Mostrar arquivos ocultos" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Mostrar arquivos ocultos" }));

    // Open the drawer
    fireEvent.click(screen.getByRole("button", { name: "open drawer" }));

    // Drawer should reflect the same show-hidden state (toggle now reads "Ocultar arquivos ocultos")
    expect(screen.getByRole("button", { name: "Ocultar arquivos ocultos" })).toBeInTheDocument();
  });
  ...
  it("keeps hidden search results hidden until the hidden-file toggle is enabled", () => {
    ...
    expect(screen.getByText('Nenhum arquivo alterado corresponde a ".env"')).toBeInTheDocument();
...
describe("FilesPanel tree (Explore) search", () => {
  ...
  it("shows the search box for the Explore view but not the Changed view", () => {
    ...
    expect(screen.getByRole("searchbox", { name: "Buscar todos os arquivos" })).toBeInTheDocument();
  ...
  it("shows search results as a flat list after the debounce fires", () => {
    ...
    fireEvent.change(screen.getByRole("searchbox", { name: "Buscar todos os arquivos" }), {
      target: { value: "test" },
    });
  ...
  it("returns to the tree view when the search query is cleared", () => {
    ...
    const searchBox = screen.getByRole("searchbox", { name: "Buscar todos os arquivos" });
  ...
  it("shows 'Searching…' while the search request is in flight", () => {
    ...
    // debounce and focuses on the component that owns the "Buscando…" text.
    render(
      <TooltipProvider>
        <FolderTree
          files={[]}
  ...
  it("shows an empty-state message when the search returns no results", () => {
    ...
    expect(screen.getByText('Nenhum arquivo corresponde a "zzznotfound"')).toBeInTheDocument();
  });
  ...
  it("clears the tree search query when switching to the Changed tab", () => {
    ...
    expect(screen.getByRole("searchbox", { name: "Buscar todos os arquivos" })).toHaveValue("");
  });
  ...
  it("shows a search error message when the search request fails", () => {
    ...
    expect(screen.getByText(/Falha na busca:.*503/)).toBeInTheDocument();
    expect(screen.queryByText(/Nenhum arquivo corresponde/)).toBeNull();
  });
  ...
  it("reveals the include/exclude glob inputs when the filters toggle is clicked", () => {
    ...
    expect(screen.queryByRole("textbox", { name: "arquivos para incluir" })).toBeNull();

    fireEvent.click(screen.getByRole("button", { name: "Mostrar filtros de busca" }));

    // Both glob inputs become visible after the toggle is opened.
    expect(screen.getByRole("textbox", { name: "arquivos para incluir" })).toBeInTheDocument();
    expect(screen.getByRole("textbox", { name: "arquivos para excluir" })).toBeInTheDocument();
  });
  ...
  it("passes the include glob to useWorkspaceFileSearch alongside the query", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Mostrar filtros de busca" }));
    fireEvent.change(screen.getByRole("textbox", { name: "arquivos para incluir" }), {
      target: { value: "*.py" },
    });
    act(() => {
      vi.advanceTimersByTime(300);
  ...
  it("passes the exclude glob to useWorkspaceFileSearch alongside the query", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Mostrar filtros de busca" }));
    fireEvent.change(screen.getByRole("textbox", { name: "arquivos para excluir" }), {
  ...
  it("clears the include/exclude filters when switching to the Changed tab", () => {
    ...
    expect(screen.getByRole("textbox", { name: "arquivos para incluir" })).toHaveValue("*.ts");
    expect(screen.getByRole("textbox", { name: "arquivos para excluir" })).toHaveValue(
      "**/node_modules",
    );
    ...
    expect(screen.getByRole("textbox", { name: "arquivos para incluir" })).toHaveValue("");
    expect(screen.getByRole("textbox", { name: "arquivos para excluir" })).toHaveValue("");
  });
});
