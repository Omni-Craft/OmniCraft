...
function HiddenFilesToggle({
  ...
  const ariaLabel = showHidden ? "Ocultar arquivos ocultos" : "Mostrar arquivos ocultos";
  const tooltipLabel = showHidden
    ? "Ocultar arquivos ocultos"
    : hasHidden
      ? `${hiddenCount} arquivo${hiddenCount === 1 ? "" : "s"} em diretórios ocultos. Clique para mostrar.`
...
const SORT_OPTIONS: { value: ChangedSort; label: string; Icon: typeof ArrowDownAZIcon }[] = [
  { value: "alpha", label: "Nome do arquivo", Icon: ArrowDownAZIcon },
  { value: "recent", label: "Editado por último", Icon: FileClockIcon },
  { value: "size", label: "Tamanho", Icon: ArrowDownWideNarrowIcon },
  { value: "type", label: "Tipo", Icon: FileTypeIcon },
];
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button
          type="button"
          aria-label={`Ordenar: ${active.label}`}
  ...
  return (
    ...
    >
      <button
        ...
        aria-label="Alterados"
        title="Mostrar apenas os arquivos alterados"
      ...
      <button
        ...
        aria-label="Todos"
        title="Mostrar a árvore de pastas completa"
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-2 px-3 py-2">
        <span className="shrink-0 font-medium text-sm">Pasta de trabalho</span>
        ...
        <div className="shrink-0" onClick={(e) => e.stopPropagation()}>
          <div className="flex items-center gap-2 px-2 py-1.5 @max-[400px]/filespanel:flex-col @max-[400px]/filespanel:items-stretch">
            ...
            <div className="flex min-w-0 flex-1 items-center gap-2">
              <div className="flex min-w-0 flex-1 items-center gap-[6px] rounded-full border border-border px-[10px] py-[4px] transition-colors focus-within:border-border-strong">
                <SearchIcon className="size-4 shrink-0 text-muted-foreground" />
                <input
                  aria-label="Buscar todos os arquivos"
                  className="min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-muted-foreground"
          ...
          {showSearchFilters && (
            <div className="flex flex-col gap-1.5 border-border border-t px-3 py-2">
              <SearchFilterInput
                label="arquivos para incluir"
                placeholder="ex.: *.ts, src/**"
                value={treeInclude}
                onChange={setTreeInclude}
              />
              <SearchFilterInput
                label="arquivos para excluir"
                placeholder="ex.: **/node_modules, *.test.ts"
                value={treeExclude}
