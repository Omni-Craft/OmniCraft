...
export function FolderTree({
  ...
  if (searchQuery.trim().length > 0) {
    if (isSearching && !searchResults) {
      return <p className="px-2 py-1 text-muted-foreground text-xs">Buscando…</p>;
    }
    if (isSearchError) {
      return (
        <p className="px-2 py-1 text-destructive text-xs">
          Falha na busca: {searchError instanceof Error ? searchError.message : "Erro desconhecido"}
        </p>
      );
    }
    if (!searchResults || searchResults.length === 0) {
      return (
        <p className="px-2 py-1 text-muted-foreground text-xs">
          Nenhum arquivo corresponde a "{searchQuery.trim()}"
        </p>
      );
    }
    ...
    if (visibleResults.length === 0) {
      ...
      return (
        <p className="px-2 py-1 text-muted-foreground text-xs">
          {hiddenCount} correspondência{hiddenCount === 1 ? "" : "s"} em diretórios ocultos.{" "}
          <button
            type="button"
  ...
  if (isError) {
    ...
    if (error instanceof RunnerOfflineError) {
      ...
      return <p className="px-2 py-1 text-muted-foreground text-xs">Nenhum arquivo no workspace</p>;
    }
    return (
      <p className="px-2 py-1 text-destructive text-xs">
  ...
  if (visibleTree.length === 0) {
    return (
      <p className="px-2 py-1 text-muted-foreground text-xs">
        Todos os arquivos estão ocultos — clique no ícone de olho para revelá-los.
      </p>
    );
  }
  return (
