...
function defaultForkTitle(sourceTitle: string | null | undefined): string {
  ...
  return trimmed ? `Clone de ${trimmed}` : "";
}
...
export function ForkSessionForm({
  ...
  //      OmniCraft transcript and native targets rebuild their on-disk
  ...
  const namePlaceholder = defaultForkTitle(sourceTitle) || "Nomeie a sessão clonada";
      ...
      <div className="-mr-4 flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto pr-4 [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-border [&::-webkit-scrollbar-track]:bg-transparent">
          ...
          <div className="flex flex-col gap-2">
            ...
            {hosts === undefined ? (
              ...
              <ConnectHostInstructions
                ...
                label={
                  allHosts.length === 0
                    ? "Nenhum host conectado ainda. Conecte um pelo terminal:"
                    : "Nenhum host online. Reconecte pelo terminal para iniciar o clone:"
                }
              />
            ) : (
              <>
                ...
                >
                  <SelectTrigger className="w-full text-xs" data-testid="fork-session-host-select">
                    <SelectValue placeholder="Selecionar um host" />
                  </SelectTrigger>
                  <SelectContent>
                ...
                >
                  ...
                  Conectar outro host pelo terminal
                </button>
            ...
            >
              ...
              <SelectValue>
                {switching ? (
                  ...
                  <>
                    {sourceAgentDisplay}{" "}
                    <span className="text-muted-foreground">(mesmo da sessão original)</span>
                  </>
                )}
              </SelectValue>
            </SelectTrigger>
        ...
        {usingSourceDir && (
          <p className="text-xs text-muted-foreground" data-testid="fork-session-reuse-dir-hint">
            Por padrão, o clone reutiliza o{" "}
            <Tooltip>
            ...
            da sessão original. Abra as Configurações avançadas para alterá-lo.
          </p>
        )}
          ...
          >
            ...
            <span>
              {conflictingSessions.length === 1
                ? "1 outro agente está"
                : `${conflictingSessions.length} outros agentes estão`}{" "}
              trabalhando neste diretório, então as alterações podem conflitar. Nomeie uma branch do
              git em Configurações avançadas para trabalhar em uma cópia isolada.
            </span>
          </p>
        )}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {selectedHostId ? (
                      <>
                        ...
                        {showMismatchWarning && (
                          ...
                          >
                            <AlertTriangleIcon className="mt-0.5 size-3.5 shrink-0" />
                            <span>
                              Este diretório é diferente do da sessão original. Referências a
                              arquivos anteriores no histórico podem não se aplicar — o agente
                              precisará se reorientar.
                            </span>
                          </p>
                        )}
                      </>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Selecione um host para escolher um diretório.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {branchName.trim() !== "" && (
                      <input
                        ...
                        placeholder="Branch base (usa a branch atual por padrão)"
                    ...
                    <p className="text-xs text-muted-foreground">
                      Cria um worktree do git para uma nova branch em um diretório isolado — evita
                      que o clone e o original disputem os mesmos arquivos. Deixe em branco para
                      iniciar no diretório escolhido.
                    </p>
                  </div>
                </>
              )}
...
export function ForkSessionDialog({
  ...
  const cloneDescription = `${
    truncated
      ? "Copia o histórico desta sessão até a resposta selecionada para uma nova sessão que você possui — mensagens após ela não são copiadas"
      : "Copia o histórico desta sessão para uma nova sessão que você possui"
  }${
    sourceWorkspace ? ", e a inicia no host e diretório que você escolher" : ""
  }. Comentários não são copiados, e alterações no clone não afetam o original.`;
      ...
      >
        <DialogHeader>
          <DialogTitle className="flex items-center gap-1.5">
            {truncated ? "Clonar a partir desta resposta" : "Clonar sessão"}
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  aria-label="O que a clonagem faz?"
