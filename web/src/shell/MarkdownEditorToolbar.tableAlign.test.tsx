...
describe("TableAlignControls visibility", () => {
  it("hides alignment buttons when the cursor is outside a table", () => {
    ...
    renderToolbar();
    expect(screen.queryByTitle("Alinhar coluna à esquerda")).toBeNull();
    expect(screen.queryByTitle("Alinhar coluna ao centro")).toBeNull();
    expect(screen.queryByTitle("Alinhar coluna à direita")).toBeNull();
  });

  it("shows all three alignment buttons when the cursor is inside a table", () => {
    mockEditorState({ inTable: true, align: null });
    renderToolbar();
    expect(screen.getByTitle("Alinhar coluna à esquerda")).toBeDefined();
    expect(screen.getByTitle("Alinhar coluna ao centro")).toBeDefined();
    expect(screen.getByTitle("Alinhar coluna à direita")).toBeDefined();
  });
});
...
describe("TableAlignControls active state", () => {
  it("marks only the left button active for left-aligned columns", () => {
    ...
    renderToolbar();
    expect(screen.getByTitle("Alinhar coluna à esquerda").className).toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna ao centro").className).not.toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna à direita").className).not.toContain("bg-accent");
  });

  it("marks only the center button active for center-aligned columns", () => {
    mockEditorState({ inTable: true, align: "center" });
    renderToolbar();
    expect(screen.getByTitle("Alinhar coluna ao centro").className).toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna à esquerda").className).not.toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna à direita").className).not.toContain("bg-accent");
  });
  ...
  it("marks only the right button active for right-aligned columns", () => {
    ...
    renderToolbar();
    expect(screen.getByTitle("Alinhar coluna à direita").className).toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna à esquerda").className).not.toContain("bg-accent");
    expect(screen.getByTitle("Alinhar coluna ao centro").className).not.toContain("bg-accent");
  });
...
describe("setColumnAlign dispatch", () => {
  ...
  it("clicking 'center' dispatches one transaction updating both column cells", () => {
    ...
    fireEvent.click(screen.getByTitle("Alinhar coluna ao centro"));
  ...
  it("clicking 'right' dispatches with align='right' for all column cells", () => {
    ...
    fireEvent.click(screen.getByTitle("Alinhar coluna à direita"));
  ...
  it("clicking 'left' dispatches with align='left' for all column cells", () => {
    ...
    fireEvent.click(screen.getByTitle("Alinhar coluna à esquerda"));
