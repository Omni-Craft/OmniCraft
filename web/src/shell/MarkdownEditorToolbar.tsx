    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      >
        <p className="mb-1.5 text-xs text-muted-foreground">
          {hovered.rows > 0 ? `Tabela ${hovered.rows} × ${hovered.cols}` : "Inserir tabela"}
        </p>
            ...
            <div key={r} className="flex gap-0.5">
              {Array.from({ length: MAX }, (_, c) => (
                <button
                  key={c}
                  type="button"
                  aria-label={`Inserir tabela ${r + 1}×${c + 1}`}
                  onMouseDown={(e) => e.preventDefault()}
...
function TableAlignControls({ editor }: { editor: Editor }) {
  ...
  return (
    <>
      ...
      <ToolbarBtn
        active={current === "left"}
        title="Alinhar coluna à esquerda"
      ...
      <ToolbarBtn
        ...
        title="Alinhar coluna ao centro"
      ...
      <ToolbarBtn
        ...
        title="Alinhar coluna à direita"
...
export function ToolbarPlugin({
  ...
  const saveStatus = saveDisabled
    ? {
        label: "Offline",
        title: "Executor offline — suas alterações serão salvas ao reconectar",
    ...
    : saveError && isDirty
      ...
      : isSaving
        ...
        : isDirty
          ? {
              label: "Não salvo",
              title: "Alterações não salvas — ⌘S para salvar agora",
              tone: "pending" as const,
            }
          : { label: "Salvo", title: "Todas as alterações salvas", tone: "saved" as const };
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 border-b border-border bg-card px-2 py-1 shrink-0">
      ...
      <div className="ml-auto flex items-center gap-2">
        <ToolbarBtn title="Copiar" onClick={handleCopy}>
