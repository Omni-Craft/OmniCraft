...
describe("MonacoCodeEditor read-only / truncated gating", () => {
  it.each([
    ...
    if (banner) expect(screen.getByText(/grande demais para carregar por completo/)).toBeDefined();
    else expect(screen.queryByText(/grande demais para carregar por completo/)).toBeNull();
  });
});
...
describe("MonacoCodeEditor load failure", () => {
  it("surfaces an error instead of an infinite spinner when Monaco fails to load", async () => {
    ...
    expect(await screen.findByText(/falha ao carregar o editor/i)).toBeDefined();
