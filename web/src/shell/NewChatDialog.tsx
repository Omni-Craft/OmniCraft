...
const CLAUDE_NATIVE_PERMISSION_MODES: { value: string; label: string; description: string }[] = [
  ...
  },
  {
    value: "auto",
    label: "Automático",
    description: "Executa automaticamente; um classificador bloqueia ações arriscadas",
  },
  {
    value: "acceptEdits",
    label: "Aceitar edições",
    description: "Aplica edições de arquivo automaticamente; comandos ainda pedem confirmação",
  },
  { value: "plan", label: "Planejar", description: "Apenas planeja; não faz edições" },
  {
    value: "dontAsk",
    label: "Não perguntar",
    description: "Nega automaticamente tudo que não foi pré-aprovado",
  },
  {
    value: "bypassPermissions",
    label: "Ignorar permissões",
    description: "Executa tudo; sem confirmações ou verificações de segurança",
  },
];
...
const CURSOR_NATIVE_EXEC_MODES: {
...
}[] = [
  {
    value: "default",
    label: "Padrão",
    description: "Modo normal do agente; pede confirmação antes de rodar comandos",
    args: [],
  },
  {
    value: "auto-review",
    label: "Auto-review",
    description:
      "Auto inteligente: executa automaticamente chamadas de ferramenta seguras e pede confirmação para o resto",
    args: ["--auto-review"],
  },
  {
    value: "plan",
    label: "Planejar",
    description: "Planejamento somente leitura; analisa e propõe planos, sem edições",
    args: ["--mode", "plan"],
  },
  {
    value: "ask",
    label: "Perguntar",
    description: "Estilo perguntas e respostas; explica e responde perguntas (somente leitura)",
    args: ["--mode", "ask"],
  },
  {
    value: "yolo",
...
const CODEX_NATIVE_APPROVAL_MODES: {
...
}[] = [
  {
    ...
    label: "Padrão",
    description:
      "Lê/edita/executa no workspace; aprovação necessária para edições externas ou rede",
    args: [],
  },
  {
    value: "full-access",
    label: "Acesso total",
    description: "Edita qualquer arquivo e acessa a internet sem aprovação",
    args: ["--sandbox", "danger-full-access", "--ask-for-approval", "never"],
  },
  {
    value: "read-only",
    label: "Somente leitura",
    description: "Apenas lê arquivos; aprovação necessária para edições, comandos ou rede",
...
export async function describeCreateError(res: Response): Promise<string> {
  ...
  return `Não foi possível criar a sessão (HTTP ${res.status}).`;
}
...
export function harnessWarningBadgeText(reason: string | null): string {
  if (reason === "binary-missing") return "binário ausente";
  if (reason === "needs-auth") return "requer autenticação";
  return "requer configuração";
}
...
function harnessWarningMessage(
  ...
  if (reason === "needs-auth") {
    return (
      <>
        {agentName} precisa de autenticação do Codex em {hostName} — rode <code>codex login</code>{" "}
        naquela máquina.
      </>
    );
  }
      ...
      >
        ...
        <div className="max-h-48 overflow-y-auto">
          <button type="button" className={itemClass} onClick={() => pick("")}>
            <span className="flex-1 truncate">Sem projeto</span>
    ...
    <div className="px-2 py-1.5">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-xs font-medium text-destructive">
          ...
          <span>Ignorar aprovações e sandbox</span>
        </div>
        <Switch
          size="sm"
          checked={enabled}
          ...
          aria-label="Ignorar aprovações e sandbox"
      ...
      {!enabled && (
        <Input
          ...
          aria-label="Digite a frase de confirmação para ativar o modo sem restrições"
      ...
      {enabled && (
        ...
        >
          ...
          <span>
            Perigo: esta sessão executa o Codex com aprovações e sandbox desativados. Ela pode
            editar qualquer arquivo e rodar qualquer comando sem pedir confirmação.
          </span>
        </div>
      )}
    </div>
...
function ModelEffortOptions({
  ...
  return (
    <>
      <div className="px-2 pt-1.5 pb-0.5 text-[11px] font-medium text-muted-foreground">Modelo</div>
    ...
    if (nativeAgentHasCapability(agent, "permissionMode")) {
      return (
        <>
          ...
          />
          <DropdownMenuSeparator />
          <PickerSectionHeader>Modo de permissão</PickerSectionHeader>
      ...
      <DropdownMenuTrigger asChild>
        ...
        >
          <span className="max-w-[12rem] truncate text-xs text-foreground">
            {hasAgents ? agentLabel : "Nenhum agente"}
          </span>
      ...
      >
        {showMobileKnobs && mobileKnobsAgent ? (
          ...
          <>
            ...
            <PickerSectionHeader>Agentes</PickerSectionHeader>
...
export function NewChatLandingScreen() {
  ...
  const hostLabel = connectingThisMachine
    ...
    : sandboxSelected
      ? sandboxLabel
      : (selectedHost?.name ?? (onlineHosts.length === 0 ? "Sem máquinas" : "Selecionar máquina"));
  ...
  const worktreeLabel = branchName.trim() || "Sem worktree";
  ...
  // redundant.
  const agentLabel = selectedAgent ? selectedAgent.display_name : "Selecionar agente";
    ...
    >
        ...
        <div className="relative flex w-full flex-col gap-3">
          ...
          >
            {isDragActive && (
              <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center rounded-2xl bg-card/80">
                <span className="text-sm font-medium text-ring">Solte os arquivos aqui</span>
              </div>
            )}
                ...
                {files.map((file, i) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-0.5 rounded-full hover:text-foreground"
                      aria-label={`Remover ${file.name || "image.png"}`}
                    >
                      <XIcon className="size-3" />
                    </button>
                  </span>
                ...
                <DropdownMenuContent align="start" className="min-w-52">
                    ...
                    <>
                      {managedSandboxesEnabled ? (
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <MonitorCloudIcon className="size-4 text-muted-foreground" />
                            <span className="text-xs">Novo Sandbox</span>
                          </span>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                aria-label="Por que o Novo Sandbox está indisponível"
                  ...
                  {onlineHosts.map((host) => (
                    ...
                    >
                      <HostOption
                        host={host}
                        subtitle={host.host_id === thisMachineHostId ? "esta máquina" : undefined}
                      />
                    </DropdownMenuItem>
                  ))}
                          ...
                          <HostOption
                            ...
                            subtitle={
                              connectingThisMachine
                                ? "conectando…"
                                : "esta máquina · selecione para conectar"
                            }
                          />
                        </DropdownMenuItem>
                      );
                  ...
                  {showConnectThisMachine && (
                    ...
                    >
                      ...
                      <span className="text-xs">
                        {connectingThisMachine ? "Conectando esta máquina…" : "Rodar nesta máquina"}
                      </span>
                    </DropdownMenuItem>
                  )}
                  ...
                  <PopoverContent align="start" className="w-96 p-3">
                    <div className="flex flex-col gap-2">
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <button
                                ...
                                aria-label="Como configurar as credenciais do Git do Databricks"
                              >
                                <CircleHelpIcon className="size-3.5" />
                              </button>
                            </TooltipTrigger>
                      ...
                      <input
                        ...
                        aria-label="Branch do repositório"
                        className="rounded-md border border-input bg-background px-3 py-2 text-xs outline-none transition-colors focus-visible:border-ring"
                        data-testid="new-chat-landing-repo-branch-input"
                      />
                      <p className="text-xs text-muted-foreground">
                        Clonado no sandbox como diretório de trabalho da sessão. Deixe em branco
                        para começar em um workspace vazio.
                      </p>
                    </div>
                  </PopoverContent>
                </Popover>
                  ...
                  >
                    <div className="flex flex-col gap-2">
                      <label
                        ...
                        className="text-xs font-medium text-foreground"
                      >
                        Branch da worktree do Git (opcional)
                      </label>
                      {/* Help text sits above the field. The warning for a picked
                        existing worktree stays below the input (contextual to the
                        selection). */}
                      <p className="text-xs text-muted-foreground">
                        Nome de uma nova branch, ou escolha uma worktree existente. Deixe em branco
                        para começar diretamente no diretório de trabalho.
                      </p>
                      ...
                      <div className="relative flex flex-col">
                        ...
                        <button
                          ...
                          title="Gerar um nome de branch único"
                          aria-label="Gerar um nome de branch único"
                      ...
                      {branchName.trim() !== "" && !startInExistingWorktree && (
                        <input
                          ...
                          placeholder="Branch base (padrão: a atual)"
                      ...
                      {startInExistingWorktree && (
                        ...
                        >
                          Começa na worktree existente; edite o nome para criar uma nova.
                        </p>
                      )}
                    </div>
                  </PopoverContent>
          ...
          {supportsApprovalMode && bypassSandbox && (
            ...
            >
              <TriangleAlertIcon className="size-3.5 shrink-0" />
              <span>
                O Codex vai rodar com aprovações e sandbox desativados — ele pode editar qualquer
                arquivo e rodar qualquer comando sem pedir confirmação.
              </span>
            </p>
          )}
      ...
      <Dialog open={connectOpen} onOpenChange={setConnectOpen}>
        <DialogContent className="sm:max-w-lg" data-testid="connect-host-dialog">
          <DialogHeader>
            <DialogTitle>Conectar uma máquina</DialogTitle>
          </DialogHeader>
          <ConnectHostInstructions
            serverUrl={serverUrl}
            label="Rode isto na máquina que você quer usar e depois escolha-a no menu de máquinas:"
          />
        </DialogContent>
      </Dialog>
