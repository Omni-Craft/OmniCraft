...
describe("NewTerminalButton access gate", () => {
  it("renders nothing when the agent declares no terminals", async () => {
    ...
    expect(screen.queryByRole("button", { name: /novo shell/i })).toBeNull();
  });
  ...
  it("creates the single declared terminal on click and focuses it", async () => {
    ...
    const button = await screen.findByRole("button", { name: /novo shell/i });
  ...
  it("row variant renders a labeled list row that creates on click", async () => {
    ...
    // actionable entry.
    const row = await screen.findByRole("button", { name: /novo shell/i });
    expect(row).toHaveTextContent("Novo shell");
    fireEvent.click(row);
...
describe("NewTerminalButton native shell picker", () => {
  it("split button launches the default shell (declared[0]) on primary click", async () => {
    ...
    const primary = await screen.findByRole("button", { name: /^novo shell$/i });
  ...
  it("caret opens a picker that can launch a non-default shell", async () => {
    ...
    const caret = await screen.findByRole("button", { name: /escolher shell/i });
    ...
    expect(screen.queryByRole("menuitem", { name: /zsh \(padrão\)/i })).not.toBeNull();
  ...
  it("SDK agent with multiple terminals keeps a plain dropdown (no default click)", async () => {
    ...
    expect(screen.queryByRole("button", { name: /escolher shell/i })).toBeNull();
    const trigger = await screen.findByRole("button", { name: /novo shell/i });
