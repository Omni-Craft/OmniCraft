...
function parseNotebook(content: string): { notebook?: Notebook; error?: string } {
  ...
  if (!nb || typeof nb !== "object" || !Array.isArray(nb.cells)) {
    return { error: "não é um notebook: array de células ausente" };
  }
  return { notebook: nb };
}
...
function OutputView({ output }: { output: NotebookOutput }) {
  ...
  if (imageMime) {
    ...
    imageError = `Não foi possível decodificar a saída de imagem (${imageMime}).`;
  }
    ...
    <div>
      ...
      {suppressedHtml && (
        <div className="text-xs text-muted-foreground italic px-2 pt-1">
          Saída HTML avançada oculta — mostrando texto simples.
        </div>
      )}
  ...
  if (error || !notebook) {
    return (
      <div className="p-8 text-sm">
        <div className="text-destructive">Não é possível renderizar o notebook: {error}</div>
        <div className="mt-1 text-muted-foreground">
          Alterne para a visualização de origem para inspecionar o arquivo bruto.
        </div>
      </div>
    );
  }
