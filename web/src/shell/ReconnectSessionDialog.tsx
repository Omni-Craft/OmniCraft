...
const HOST_OWNER_DESCRIPTION =
  "O host desta sessão está offline. Execute o comando abaixo na máquina host para reconectar.";

const HOST_VIEWER_DESCRIPTION =
  "A máquina host desta sessão está offline e somente o proprietário pode reconectá-la. " +
  "Clone a sessão para continuar em uma cópia sua.";
 ...
 *   down. The owner reconnects the host (`omnicraft host`); a
 ...
 *
 * 1. `host_offline` — `omnicraft host` re-registers the host
 ...
 *    (`wrapper === "claude-code-native-ui"`) use `omnicraft claude
 *    --resume <id>`; everything else uses the generic `omnicraft run
...
export function buildReconnectCommand({
  ...
  if (state === "host_offline") {
    return ["omnicraft host \\", `  --server ${serverUrl}`].join("\n");
  }
 ...
 *
 * @param wrapper - The conversation's `omnicraft.wrapper` label
 *   (`"claude-code-native-ui"` for `omnicraft claude` sessions). Picks
...
export function ReconnectSessionDialog({
  ...
  const title = isHostReconnect ? "Host offline" : "Agente desconectado";
