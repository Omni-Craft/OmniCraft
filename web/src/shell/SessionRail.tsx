...
function TerminalsCard({ terminals, onExpand }: TerminalsCardProps) {
  ...
  return (
    <Card size="sm" data-testid="terminals-card">
      <CardHeader>
        <CardTitle className="text-sm">Terminais</CardTitle>
        <CardAction>
          <button
            type="button"
            aria-label={collapsed ? "Expandir terminais" : "Recolher terminais"}
      ...
      {!collapsed && (
        <CardContent>
          {terminals.length === 0 ? (
            <p className="text-muted-foreground text-xs">Nenhum terminal aberto</p>
          ) : (
    ...
    <Card size="sm" data-testid="execution-logs-card">
      <CardHeader>
        ...
        <CardAction>
          <button
            type="button"
            aria-label={collapsed ? "Expandir logs de execução" : "Recolher logs de execução"}
            aria-expanded={!collapsed}
            className="cursor-pointer rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground"
            onClick={() => setCollapsed((v) => !v)}
          >
