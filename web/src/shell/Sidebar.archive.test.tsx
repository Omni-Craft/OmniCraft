...
describe("archive flow", () => {
  ...
  it("toasts a pointer to Settings once the archive succeeds", () => {
    ...
    expect(within(toast).getByText(/Veja as sessões arquivadas em/)).toBeInTheDocument();
    expect(within(toast).getByRole("link", { name: "Configurações" })).toHaveAttribute(
      "href",
      "/settings/archived",
    );
  });
  ...
  it("shows an 'Archiving…' status row while the archive is in flight", () => {
    ...
    expect(within(row).getByText("Arquivando…")).toBeInTheDocument();
