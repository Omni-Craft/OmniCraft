...
describe("Sidebar session list", () => {
  ...
  it("swaps the card content to the settings section nav on /settings", () => {
    ...
    expect(screen.getByRole("link", { name: /Voltar ao OmniCraft/ })).toHaveAttribute("href", "/");
  ...
  it("renders the footer Settings as an icon-only floating control on mobile", () => {
    ...
    expect(settings).toHaveAttribute("aria-label", "Configurações");
    ...
    const label = within(settings).getByText("Configurações");
  ...
  it("keeps archived sessions out of the sidebar list (they live on the Settings page)", () => {
    ...
    const recentSection = screen.getByText("Sessões").closest("section")!;
  ...
  it("shows the session-state badge OR the timestamp, never both", () => {
    ...
    expect(within(workingRow).queryByText("agora")).toBeNull();

    // Awaiting row: same rule for the "Aguardando resposta" tag — any non-null
    ...
    expect(within(awaitingRow).queryByText("agora")).toBeNull();
    ...
    expect(within(idleRow).getByText("agora")).toBeInTheDocument();
  });
});
...
describe("Sidebar sections", () => {
  ...
  it("titles the baseline list Sessions even with no sibling group", () => {
    ...
    showSharedTab();
    expect(screen.getByText("Nenhuma sessão compartilhada com você")).toBeInTheDocument();
...
describe("Sidebar tabs", () => {
  ...
  it("gives a pinned shared session a Pinned section on the Shared tab, not My sessions", () => {
    ...
    showSharedTab();
    const pinnedSection = screen.getByText("Fixadas").closest("section")!;
...
describe("Sidebar project sections", () => {
  ...
  it("offers a pencil that starts a new session pre-filed under the project", () => {
    ...
    expect(pencil).toHaveAttribute("aria-label", "Nova sessão em Customer X");
  ...
  it("deletes a project (and all its sessions) from the folder kebab after confirming", async () => {
    ...
    expect(screen.getByText(/todas as suas sessões/i)).toBeInTheDocument();
...
describe("Sidebar collapsed project marker", () => {
  it("shows the row's session-state badge on a collapsed project", () => {
    ...
    // Collapsed by default → the row is hidden, but its "Aguardando resposta"
    // marker surfaces on the project header.
    const header = screen.getByRole("button", { name: /^Customer X/ });
    expect(header).toHaveAttribute("aria-expanded", "false");
    expect(within(header).getByText("Aguardando resposta")).toBeInTheDocument();
  });
  ...
  it("drops the header marker once the project is expanded", () => {
    ...
    expect(within(header).queryByText("Aguardando resposta")).toBeNull();
  });
...
describe("Sidebar pin marker visibility", () => {
  it("hover-reveals an unpin control on a pinned row (no persistent marker)", () => {
    ...
    expect(pinButton).toHaveAttribute("aria-label", "Desafixar conversa");
  });
...
describe("Sidebar move-to-project action", () => {
  it("moves a session into a project selected from the picker", async () => {
    ...
    fireEvent.pointerDown(within(row).getByRole("button", { name: "Ações da conversa" }), {
      button: 0,
      ctrlKey: false,
    });
  ...
  it("confirms removal only when it's the project's last session", async () => {
    ...
    fireEvent.click(await screen.findByRole("menuitem", { name: /Remover de Sprint 42/ }));
    expect(await screen.findByText(/o projeto também será removido/i)).toBeInTheDocument();
  ...
  it("removes without confirmation when other sessions remain in the project", async () => {
    ...
    expect(screen.queryByText(/o projeto também será removido/i)).toBeNull();
  });
});
...
describe("Sidebar mobile overlay background", () => {
  it("keeps the opaque bg-card-solid override for the mobile full-screen overlay", () => {
    ...
    const aside = screen.getByRole("complementary", { name: "Conversas" });
...
describe("Sidebar collapsed marker", () => {
  ...
  it("sets data-collapsed only while closed", () => {
    ...
    mockConversations(THREE_TYPE_CONVERSATIONS);
    renderSidebar(true);
    const openAside = screen.getByRole("complementary", { name: "Conversas" });
