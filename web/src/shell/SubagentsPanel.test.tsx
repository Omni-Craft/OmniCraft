...
describe("SubagentsPanel", () => {
  ...
  it("labels Pi native-wrapper main rows with the product name", () => {
    ...
    useSessionMock.mockReturnValue({
      session: {
        ...
        createdAt: 0,
        title: null,
        labels: { "omnicraft.wrapper": "pi-native-ui" },
        items: [],
        pendingElicitations: [],
        permissionLevel: 4,
        parentSessionId: null,
  ...
  it("renders an 'Add agent' button, with the dialog mounted only after a click", () => {
    ...
    expect(screen.getByTestId("add-agent-button")).toHaveTextContent(/adicionar agente/i);
  ...
  const AGENT_KIND_CASES: Array<{
    ...
    },
    {
      ...
      labels: { "omnicraft.wrapper": "opencode-native-ui" },
    ...
    },
    {
      ...
      labels: { "omnicraft.wrapper": "some-other-wrapper" },
      expectedKind: "agent",
    },
  ];
  ...
  it.each(AGENT_KIND_CASES)(
    ...
    ({ labels, agentName, expectedKind }) => {
      ...
      // and generic based on the parent session's `omnicraft.wrapper`
  ...
  it("renders an error state when the fetch fails with no cached data", () => {
    ...
    expect(screen.getByText(/Falha ao carregar agentes/i)).toBeInTheDocument();
  });
    ...
    mockChildTree({
      conv_root: [
        childInfo({
          ...
          labels: { "omnicraft.wrapper": "codex-native-ui-subagent" },
        }),
      ],
    });
    ...
    mockChildTree({
      conv_root: [
        ...
        childInfo({
          ...
          labels: { "omnicraft.wrapper": "kiro-native-ui" },
        }),
      ],
    });
    ...
    mockChildTree({
      conv_root: [
        childInfo({
          ...
          tool: "claude",
          labels: { "omnicraft.wrapper": "claude-code-native-ui-subagent" },
        }),
        childInfo({
          id: "conv_explore",
  ...
  it("renders 'Failed' for a child whose latest task ended in failure", () => {
    ...
    expect(row).toHaveTextContent(/Falhou/);
    // Should not leak the raw English status word into the translated label.
    expect(row.textContent).not.toMatch(/failed/i);
  });
  ...
  it("renders a child with last_task_error as failed and exposes the error detail", () => {
    ...
    expect(indicator).toHaveAttribute(
      "aria-label",
      "Falhou: Required terminal exited unexpectedly; the session runtime is no longer available.",
    );
  });
  ...
  it.each([
    ...
    (code, message) => {
      ...
      expect(row).not.toHaveTextContent(/Desconectado/);
      expect(row).not.toHaveTextContent(/Falhou/);
      ...
      expect(indicator).toHaveAttribute("aria-label", `Desconectado: ${message}`);
    },
  );
  ...
  it.each([
    ...
    (code, message) => {
      ...
      expect(mainRow).not.toHaveTextContent(/Desconectado/);
      expect(mainRow).not.toHaveTextContent(/Falhou/);
  ...
  it("renders red 'Failed' on the main row for a genuine parent failure (non-disconnect code)", () => {
    ...
    const mainRow = screen.getByTestId("subagent-main-row");
    const indicator = within(mainRow).getByTestId("subagent-status-dot");
    expect(mainRow).toHaveTextContent(/Falhou/);
    expect(mainRow).not.toHaveTextContent(/Desconectado/);
    expect(indicator).toHaveClass("text-destructive");
  });
  ...
  it("shows the pulsing working dot (no redundant 'Working' word) on the main row when the parent session is running", () => {
    ...
    expect(mainRow).not.toHaveTextContent(/Em execução/i);
  ...
  it("surfaces an 'awaiting input' badge for a child parked on an elicitation", () => {
    ...
    expect(row).toHaveTextContent(/Precisa de resposta/);
  ...
  it("shows the status word only for notable states, not quiet ones", () => {
    ...
    expect(childRow(container, "c_launch")).toHaveTextContent(/Iniciando/);
    ...
    expect(childRow(container, "c_work")).not.toHaveTextContent(/Em execução/);
    expect(childRow(container, "c_done")).not.toHaveTextContent(/Concluído/);
    expect(childRow(container, "c_idle")).not.toHaveTextContent(/Ocioso/);
    ...
    ).toHaveAttribute("aria-label", "Em execução");
    expect(
      within(childRow(container, "c_idle")).getByTestId("subagent-status-dot"),
    ).toHaveAttribute("aria-label", "Ocioso");
  });
  ...
  it("renders the status marker last so markers align across rows regardless of label width", () => {
    ...
    expect(failIndicator).toHaveTextContent(/Falhou/);
