...
function childStatus(child: ChildSessionInfo): AgentStatus {
  ...
  if (child.pending_elicitations_count > 0) {
    return { activity: "awaiting", label: "Precisa de resposta" };
  }
  ...
  if (child.current_task_status === "launching") {
    return { activity: "launching", label: "Iniciando" };
  }
  if (child.busy) return { activity: "working", label: "Em execução" };
  ...
  if (child.current_task_status === "failed") return { activity: "failed", label: "Falhou" };
  if (child.current_task_status === "completed") return { activity: "done", label: "Concluído" };
  if (child.current_task_status) {
    return { activity: "other", label: child.current_task_status };
  }
  return { activity: "idle", label: "Ocioso" };
}
...
function sessionStatus(
  ...
  if (status === "launching") return { activity: "launching", label: "Iniciando" };
  if (status === "running") return { activity: "working", label: "Em execução" };
  if (status === "failed") {
    ...
    return { activity: "failed", label: "Falhou" };
  }
  return { activity: "idle", label: "Ocioso" };
}
...
function StatusIndicator({ activity, label, details }: AgentStatus) {
  ...
  if (activity === "awaiting") {
    return (
      ...
      >
        <Badge className="border-transparent bg-warning/15 text-warning">Precisa de resposta</Badge>
      </span>
    );
  }
  if (activity === "failed") {
