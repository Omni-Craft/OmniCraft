  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent data-testid="switch-agent-dialog" className="flex flex-col gap-4 sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>Trocar agente</DialogTitle>
          <DialogDescription>
            Continue esta sessão em um agente diferente. A conversa, os comentários e os arquivos
            permanecem; a próxima mensagem é executada no novo agente.
          </DialogDescription>
        </DialogHeader>
                ...
                placeholder={
                  currentDisplay ? (
                    <span data-testid="switch-agent-current">
                      ...
                      <span className="text-muted-foreground">(agente atual)</span>
                    </span>
                  ) : (
                    "Escolha um agente"
                  )
        ...
        {resetsModelSettings && (
          <p data-testid="switch-agent-reset-warning" className="text-xs text-muted-foreground">
            Modelo e esforço de raciocínio serão redefinidos para os padrões de{" "}
            {chosen?.display_name} (provedor diferente).
          </p>
        )}
