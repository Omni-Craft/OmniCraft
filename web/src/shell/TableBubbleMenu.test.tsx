...
function openRowMenu(rowIndex: number, colIndex = 0): HTMLElement {
  ...
  fireEvent.click(screen.getByRole("button", { name: "Opções da linha" }));
  return screen.getByText("Excluir linha").closest("[data-table-handle-menu]") as HTMLElement;
}
...
function openColMenu(rowIndex: number, colIndex: number): HTMLElement {
  hoverCell(rowIndex, colIndex);
  fireEvent.click(screen.getByRole("button", { name: "Opções da coluna" }));
  return screen.getByText("Excluir coluna").closest("[data-table-handle-menu]") as HTMLElement;
}
...
describe("TableHandles — grip visibility", () => {
  it("reveals the row and column grips when a table cell is hovered", () => {
    ...
    renderHandles();
    expect(screen.queryByRole("button", { name: "Opções da linha" })).toBeNull();
    hoverCell(1, 1);
    expect(screen.getByRole("button", { name: "Opções da linha" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Opções da coluna" })).toBeInTheDocument();
  });
  ...
  it("opens the row menu on grip click and the column menu on its grip click", () => {
    ...
    expect(within(rowMenu).getByText("Inserir linha acima")).toBeInTheDocument();
    expect(within(rowMenu).getByText("Inserir linha abaixo")).toBeInTheDocument();
    expect(within(rowMenu).getByText("Excluir linha")).toBeInTheDocument();

    const colMenu = openColMenu(1, 1);
    expect(within(colMenu).getByText("Inserir coluna antes")).toBeInTheDocument();
    expect(within(colMenu).getByText("Inserir coluna depois")).toBeInTheDocument();
    expect(within(colMenu).getByText("Excluir coluna")).toBeInTheDocument();
  });
});
...
describe("TableHandles — row menu commands", () => {
  it("Insert row above adds an empty row above the gripped row", () => {
    ...
    fireEvent.click(within(menu).getByText("Inserir linha acima"));
    const r = rows();
  ...
  it("Insert row below adds an empty row beneath the gripped row", () => {
    ...
    renderHandles();
    const menu = openRowMenu(1);
    fireEvent.click(within(menu).getByText("Inserir linha abaixo"));
    const r = rows();
    expect(r).toHaveLength(4);
  ...
  it("Delete row removes exactly the gripped row", () => {
    ...
    renderHandles();
    const menu = openRowMenu(1);
    fireEvent.click(within(menu).getByText("Excluir linha"));
  ...
  it("closes the menu after an item is chosen", () => {
    ...
    const menu = openRowMenu(1);
    fireEvent.click(within(menu).getByText("Inserir linha acima"));
    expect(screen.queryByText("Excluir linha")).toBeNull();
  });
});
...
describe("TableHandles — column menu commands", () => {
  it("Insert column before adds an empty column at the gripped column", () => {
    ...
    fireEvent.click(within(menu).getByText("Inserir coluna antes"));
  ...
  it("Insert column after adds an empty column past the gripped column", () => {
    ...
    renderHandles();
    const menu = openColMenu(0, 1);
    fireEvent.click(within(menu).getByText("Inserir coluna depois"));
    rows().forEach((r) => expect(r.cells).toHaveLength(4));
  ...
  it("Delete column removes exactly the gripped column", () => {
    ...
    renderHandles();
    const menu = openColMenu(0, 1);
    fireEvent.click(within(menu).getByText("Excluir coluna"));
...
describe("TableHandles — context menu", () => {
  it("right-clicking a cell opens a Delete table item that removes the table", () => {
    ...
    const item = screen.getByText("Excluir tabela");
