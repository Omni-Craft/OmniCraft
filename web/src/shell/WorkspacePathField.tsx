    ...
    <div ref={containerRef} className="relative">
      <div className="flex items-center gap-2">
        <input
          ...
          autoCorrect="off"
          role="combobox"
          aria-label="Caminho do diretório de trabalho"
        ...
        <button
          type="button"
          onClick={onBrowse}
          aria-label="Procurar diretórios"
          ...
          {matches.length > 0 && (
            <>
              ...
              {hiddenMatchCount > 0 && (
                ...
                >
                  +{hiddenMatchCount} mais — continue digitando para refinar
                </div>
              )}
            </>
          )}
          {showLoading && <div className="px-3 py-2 text-xs text-muted-foreground">Carregando…</div>}
        </div>
      )}
    </div>
  );
