    ...
    >
      <div className="flex shrink-0 items-center gap-1.5 border-b px-2 py-1.5">
        ...
        <button
          ...
          aria-label={showHidden ? "Ocultar ocultos" : "Mostrar ocultos"}
          aria-pressed={showHidden}
          title={showHidden ? "Ocultar ocultos" : "Mostrar ocultos"}
          className="shrink-0 rounded p-1 text-muted-foreground hover:bg-accent hover:text-accent-foreground"
        ...
        {onSelect && (
          <Button
            ...
            title={`Selecionar esta pasta: ${basename(currentAbsolute)}`}
            className="shrink-0"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {isLoading && <div className="px-3 py-3 text-xs text-muted-foreground">Carregando…</div>}
        {error !== null && error !== undefined && !isLoading && (
          <div className="px-3 py-3 text-xs text-destructive" data-testid="workspace-picker-error">
            {error instanceof Error ? error.message : "Falha ao carregar diretório"}
          </div>
        )}
        {!isLoading && error === null && entries.length === 0 && (
          <div className="px-3 py-3 text-xs text-muted-foreground">
            {activeFilter !== null ? "Nenhuma entrada encontrada" : "(diretório vazio)"}
          </div>
        )}
        {entries.map((entry) => {
        ...
        {data?.truncated && (
          ...
          >
            Muitas entradas para listar por completo — digite um caminho acima para pular
            diretamente.
          </div>
        )}
      </div>
