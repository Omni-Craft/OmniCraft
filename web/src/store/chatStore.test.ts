...
function defaultFetchHandler(input: RequestInfo | URL, init?: RequestInit): Response {
  ...
  if (snapshotMatch && init?.method === "PATCH") {
    ...
    if ("collaboration_mode" in body && typeof body.collaboration_mode === "string") {
      labels["omnicraft.codex_native.collaboration_mode"] = body.collaboration_mode;
...
describe("chatStore — switchTo", () => {
  ...
  // opencode) — claude/codex keep it (they expose an OmniCraft model picker).
  it.each([
      ...
      fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.split("?")[0] === "/v1/sessions/conv_wrap" && (init?.method ?? "GET") === "GET") {
          return mockResponse({
            ...
            created_at: 0,
            items: [],
            labels: wrapper === null ? {} : { "omnicraft.wrapper": wrapper },
          });
        }
        return defaultFetchHandler(input, init);
      });
...
describe("chatStore — send (first-send ordering)", () => {
  ...
  it("surfaces a visible error block with friendly copy when the runner is unavailable (503)", async () => {
    ...
    expect(errorBlocks[0]).toMatchObject({
      type: "error",
      message: "O runner não ficou online a tempo. Tente novamente.",
      code: "",
    });
  });
...
describe("chatStore — bindStream sticky-pref handoff", () => {
  ...
  it("PATCHes sticky model and effort onto a claude-native session with no overrides", async () => {
    seedSession("conv_cn", []);
    withSnapshot("conv_cn", { labels: { "omnicraft.wrapper": "claude-code-native-ui" } });
  ...
  it("does NOT PATCH a non-Claude sticky model onto a claude-native session", async () => {
    ...
    withSnapshot("conv_cn_gpt", { labels: { "omnicraft.wrapper": "claude-code-native-ui" } });
  ...
  it("does NOT PATCH a non-Codex sticky model onto a codex-native session", async () => {
    ...
    withSnapshot("conv_codex_claude", { labels: { "omnicraft.wrapper": "codex-native-ui" } });
  ...
  it("PATCHes effort on an active claude-native session", async () => {
    ...
    withSnapshot("conv_supported", { labels: { "omnicraft.wrapper": "claude-code-native-ui" } });
  ...
  it("PATCHes effort on an active codex-native session", async () => {
    ...
    withSnapshot("conv_codex_supported", { labels: { "omnicraft.wrapper": "codex-native-ui" } });
  ...
  it("hydrates Codex Plan mode from the session label", async () => {
    ...
    withSnapshot("conv_plan", {
      labels: {
        ...
        "omnicraft.codex_native.collaboration_mode": "plan",
      },
    });
  ...
  it("PATCHes Codex Plan mode and settles from the returned labels", async () => {
    ...
    withSnapshot("conv_plan_toggle", { labels: { "omnicraft.wrapper": "codex-native-ui" } });
  ...
  it("rolls back Codex Plan mode when the PATCH is rejected", async () => {
    ...
    withSnapshot("conv_plan_failure", { labels: { "omnicraft.wrapper": "codex-native-ui" } });
  ...
  it("surfaces the applied sticky model as the session override (claude-native)", async () => {
    ...
    withSnapshot("conv_sticky_cn", { labels: { "omnicraft.wrapper": "claude-code-native-ui" } });
  ...
  it("does NOT surface a non-Claude sticky model as the session override (claude-native)", async () => {
    ...
    withSnapshot("conv_sticky_gpt", { labels: { "omnicraft.wrapper": "claude-code-native-ui" } });
...
describe("chatStore — startStreamPump reconnect loop", () => {
  ...
  it("retries a transient 404 within the bound and reconnects without failing", async () => {
    seedSession("conv_404t", []);
    const sinks: StreamSink[] = [];
    let opens = 0;
    fetchMock.mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      if (/\/v1\/sessions\/[^/]+\/stream$/.test(url)) {
        opens += 1;
        // A restart blip: the first few opens 404, then the route returns.
    ...
    const controller = new AbortController();
    useChatStore.setState({
      conversationId: "conv_404t",
      abortController: controller,
      sessionStatus: "running",
    });

    const loop = startStreamPump("conv_404t", controller, setState, getState);
    // Drain the backoff between the 404 retries until the 4th open succeeds
    // (~1.75s of cumulative backoff for the first three retries).
    await vi.advanceTimersByTimeAsync(30_000);
    expect(sinks).toHaveLength(1);
    // The session survived the blip — never flipped to failed.
    expect(useChatStore.getState().sessionStatus).not.toBe("failed");
  ...
  it("gives up and marks the session failed on a persistent 404 past the bound", async () => {
    ...
    // Drain all bounded retries' backoff (~7.75s total) until it gives up.
    await vi.advanceTimersByTimeAsync(30_000);
    await loop;

    // It retried the bound (STREAM_RECONNECT_MAX_404_RETRIES = 5) before
    // failing — i.e. 5 retries + the final over-cap open — not a give-up on
    // the first 404.
    expect(opens).toBe(6);
...
describe("chatStore — live delta streaming (claude-native)", () => {
  ...
  it("does not double-render pi-native text when the harness response completes before the deltas", async () => {
    ...
    // `omnicraft pi` server). The harness PiNativeExecutor completes its
    // OmniCraft response the instant it enqueues the user message — so
