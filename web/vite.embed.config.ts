// Intermediate library build for the embeddable OmniCraft island.
//
...
// into an ESM entry (`dist-embed/omnicraft-embed.js`) plus a `chunks/` tree of
...
// stylesheet (`dist-embed/omnicraft-embed.css`). The universe monolith then
// ingests this graph into its OWN rspack graph (see
// `webapp/web/js/genai/omnicraft/embed/loadOmniCraftEmbed.ts` + the
// `@omnicraft/embed` alias in `app.rsbuild.config.ts`) and emits the FINAL
...
// react-router instance shared with the host — no `__OMNICRAFT_SHARED__`, no MF.
...
//
// The CSS is post-processed to prefix every selector with `.omnicraft-app` so
...
const scopePlugin = (): postcss.Plugin => ({
  ...
  // scoped utility like `.omnicraft-app .text-[11px]`. Since every embed rule is
  // already scoped under `.omnicraft-app`, we don't need layers for internal
  // ordering; dropping them makes the embed compete on specificity, which it
  // wins (`.omnicraft-app .util` ≥ host's typical element selectors).
  AtRule(atRule) {
...
function resolveExternalCjsRequire(externals: readonly string[]): Plugin {
  ...
  const importName = (spec: string) => `__omnicraftExt_${spec.replace(/[^a-zA-Z0-9]/g, "_")}`;
  ...
  return {
    ...
    renderChunk(code) {
      ...
      const replacement =
        ...
        `\tconst __omnicraftExternals = { ${table} };\n` +
        `\tif (Object.prototype.hasOwnProperty.call(__omnicraftExternals, id)) return __omnicraftExternals[id];\n` +
...
function scopeOmniCraftCss(): Plugin {
  return {
    name: "scope-omnicraft-css",
    enforce: "post",
...
export default defineConfig({
  ...
  // `/assets/...` string). Monaco is omnicraft_ui's own dep, so Vite builds the
  ...
  // content-hash on its own CDN (see the `@omnicraft/embed` wiring in
  ...
  build: {
    ...
    rollupOptions: {
      ...
      output: {
        ...
        entryFileNames: "omnicraft-embed.js",
        chunkFileNames: "chunks/[name]-[hash].js",
        assetFileNames: (assetInfo) => {
          const name = assetInfo.names?.[0] ?? "";
          return name.endsWith(".css") ? "omnicraft-embed.css" : "assets/[name].[ext]";
        },
      },
    },
  },
